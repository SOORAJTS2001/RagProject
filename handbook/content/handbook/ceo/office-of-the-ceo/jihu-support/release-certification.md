...
to handle JiHu's merge request contributions to Example Company Inc repositories.
...
and posting a comment on the [relevant release task issue](https://example_company.com/example_company-org/release/tasks/issues)
...
[Release Managers page](https://about.example_company.com/community/release-managers/), under the AppSec Release Certification column.
...
1. [On the release date of each month](/handbook/engineering/releases/) the [jh-upstream-report repository](https://example_company.com/example_company-org/jh-upstream-report) should run a scheduled pipeline that automatically creates the release certification issue. This will create an issue in the [jh-upstream-report issue tracker](https://example_company.com/example_company-org/jh-upstream-report/-/issues) with a checklist containing each JiHu contribution associated with the upcoming release. If something went wrong, the [release certification tools script](https://example_company.com/example_company-com/gl-security/product-security/appsec/tooling/release-certification-tools) may need to be ran manually by following the directions in the `README.md`
1. Verify that every JiHu contribution going into the release is on this list. This can be done by looking at the [status report](https://example_company.com/example_company-jh/status-reports/-/issues) repository information in addition to searching for the `JiHu Contribution` labels in [each repository](/handbook/ceo/office-of-the-ceo/jihu-support/#projects) (the certification issue should have a link available). Be sure to look for both open and closed merge requests. The most likely reason a MR would be in the release but not in the checklist is the appropriate milestone had not been set for it.
...
1. Ping the appropriate [release manager](https://about.example_company.com/community/release-managers/) from the delivery team and work with them to remove the MRs from the release
