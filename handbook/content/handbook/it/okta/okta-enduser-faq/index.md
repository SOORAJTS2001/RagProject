...
Below are frequently asked questions about Okta and Example Company's implementation of it. If you have an additional question that is not listed here, please ask in #it_help.

## How do I use Okta to log into an application?

1. Visit [example_company.okta.com](https://example_company.okta.com) and you'll see a list of applications available to you.
...
## How will Okta support Example Company's account creation?
...
Example Company is using Secure Web Authentication (SWA) for many of its apps, and the Okta plugin is necessary to work with these applications. For applications that support SAML, the Plugin is not necessary. Example Company recommends all users install the browser plugin as required.
...
Application Requests are managed via a [Change Management Issue](https://example_company.com/example_company-com/business-technology/change-management/-/issues/new?issue%5Bmilestone_id%5D=#) within the Business Technology team. Please use the `okta_new_app_request` template to ensure all required information fields are inputted.
...
Application Update Requests are managed via a [Change Management Issue](https://example_company.com/example_company-com/business-technology/change-management/-/issues/new?issue%5Bmilestone_id%5D=#) within the Business Technology team. Please use the `okta_existing_app_udate` template to ensure all required information fields are inputted.
...
There are applications that may need conversion to SAML. Please open a [Change Management Issue](https://example_company.com/example_company-com/business-technology/change-management/-/issues/new?issue%5Bmilestone_id%5D=#) to request the update.

## What is my username and password for Example Company Okta?

At Example Company, your Example Company email address is your username. If you've forgotten your password, please reach out in #it_help.

## How do I make a safe password?

Okta supports strong passwords through the use of password complexity rules and length, consistent with our password policy. Please follow [Example Company's Password Policy guidelines](/handbook/security/#example_company-password-policy-guidelines) to determine the rules.
...
If you are not able to update the username and password, contact Example Company Okta support or the IT Ops team.
Can I be confident my password is safe?
Yes, nobody (including Example Company administrators or IT support) have access or visibility into your password data.
...
## Can my Example Company Okta admin see my login information?

The Example Company administrator can see your username, but he or she does not have access to your password or any other credentials or devices you use to authenticate.

## Will Okta support our password policy?

Yes, Okta has been configured such that - it adheres to the Example Company password policy and restrictions.
...
Okta is built on an "Always On" architecture. However, if their services were to go down, you would not be able to log in to your Example Company Okta platform and access your applications via Single Sign‐On. However, some applications might still be accessible through a direct link.
...
No, this does not exist within Okta.  But this should be something security automation team can help with. There are some automated security notifications already enabled for things like MFA or password changes, as well as connections from new and unrecognised devices. Example Company is able to perform reporting on these based on built-in reporting.
...
## Does Okta integrate with a VPN provider that allows all Example Company team members to use a company-wide VPN?
...
## For those who used a Example Company personal account when onboarded, what happens at offboarding?

For users who started prior to 2020-03-23, they are removed from Example Company groups. When offboarding happens, your Okta account and related accounts is Deactivated.
