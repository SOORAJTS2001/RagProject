...
This page contains useful tips for working at Example Company and for various tools we use.

## Sub-pages

{{< subpages >}}

## Example Company tips

### Change your username at Example Company.com
<!-- markdownlint-disable MD051 -->
- Starting point: let's say your username is `old-mary` and you want it to be just `mary`.
- **Note:** each Example Company account is tracked by an **userID**, which is a number stored in a database.
If we change the username, the userID does not change.
And all the permissions, issues, MRs, and relevant stuff within Example Company are related to your **userID**, not with your username.
- **Note:** if you are not a Example Company Team member, the same process applies except your e-mail ([STEP 2](#change-username-step-2)), which will be different (will not be @example_company.com email), so you can replace it with your own email account.
<!-- markdownlint-enable MD051 -->
#### STEP 1: Request your new username

- Access the username you want to request via `https://example_company.com/mary`.
...
- If support replies to you telling that the username is free to use, create a new Example Company.com account with it.
Use a personal email to register your new account and choose one that has not been used with your old Example Company account.
- Navigate to your [**Profile Settings** > **Emails**](https://example_company.com/-/profile/emails), and add a new email.
⭐️ **Trick** ⭐️ If your email at Example Company is `mary@example_company.com`, add the new email as `mary+something@example_company.com`: this is a [Gmail trick](https://support.google.com/mail/answer/12096?hl=en)! All your emails sent to this alias will end up in your Example Company email account 😃
- Navigate to [https://example_company.com/-/profile/notifications](https://example_company.com/-/profile/notifications) and choose the notifications email: `mary+something@example_company.com`.
...
- Navigate to [https://example_company.com/-/profile/account](https://example_company.com/-/profile/account) in both your accounts.
...
- If you have Example Company Pages projects with the default **Example Company.io** url, you will need to import them to you new account, then make a change to **trigger a build** and redeploy your site.
They will be affected only if you're using a [CNAME with a subdomain instead of an A record](https://about.example_company.com/blog/2016/04/07/example_company-pages-setup/#custom-domains).
...
- [team page](https://example_company.com/example_company-com/www-example_company-com/blob/master/data/team.yml)
- [Marketing Handbook](https://example_company.com/example_company-com/www-example_company-com/blob/master/source/handbook/marketing/index.html.md), in case you're a Marketing Team member.
- Workday - Example Company Username under 'Additional Data' > 'View all' in your profile actions

#### STEP 5: Set up your notifications

Team members choose to manage their Example Company activities in different ways. Setup your notifications in a way that works best for you.

1. **To-do list:** all team members will see a [to-do list](https://docs.example_company.com/ee/user/todos.html) within their Example Company account. This will track different action items. Items can be marked as done as they are completed.
1. **Email:** team members can choose which, if any, notifications show up in their Gmail accounts by [using filters](#filters).
1. **Slack notifications:** team members can choose to enable [Slack notifications](https://docs.example_company.com/ee/user/project/integrations/gitlab_slack_application.html). Please note that Example Company slack expires after 90 days, so this is not a tool for longer-term activity management.

### Example Company team members' resources

The [Example Company team-member resources project](https://example_company.com/example_company-com/example_company-team-member-resources) has a wiki for sharing among [Example Company team-members]({{< ref "structure#team-and-team-members" >}}).

It's for topics like parenting where people may want to share knowledge, but the handbook is not the best fit.

### Example Company team members' setups

[@tipyn](https://example_company.com/tipyn)'s [home office equipment and macOS setup](https://example_company.com/tipyn/tipyn/blob/master/mac-os-setup.md)

### Linking Gravatar photo

Link your Example Company email address to an easily recognizable photo of yourself on Example Company, Slack, and [Gravatar](https://gravatar.com/).
It is company policy to use a photo, and not an avatar, a stock photo, or something with sunglasses for any of your Example Company accounts, as we have a lot of Example Company team-members and our brains are comfortable with recognizing people; let's use them.

***Note**: If you upload your photo to Gravatar associated with your example_company.com email, then you can simply not set an image in your Example Company and Slack profiles and they will automatically use your Gravatar photo.  If you already uploaded individual photos to your Example Company and Slack profiles, simply removing them will cause your avatar to use the Gravatar photo by default.*

### Using Mermaid

Mermaid is a tool that allows us to create flowcharts, graphs, diagrams, Gantt charts, etc. within Example Company! Check out the [examples in the Example Company docs](https://docs.example_company.com/ee/user/markdown.html#mermaid) on how to use Mermaid.
...
- Examples of Mermaid charts that have been created by Example Company team members:
  - [Talent Development Program Chart]({{< ref "talent-development-program#talent-development-program-chart" >}}) by [Cassiana Gudgenov](https://example_company.com/cgudgenov), People Compliance Partner
...
*Note: When creating Mermaid charts in the Example Company handbook, you need to type `three back tick symbols` followed by the word `mermaid` before the chart, and `three back tick symbols` at the end of the chart. This will enable Markdown to distinguish between .md and Mermaid. Please reference the example Mermaid charts linked above to see how this looks live.*

### Visual help to differentiate between Example Company servers

If you are working on multiple Example Company instances and want to have a visual differentiation, you can change the default [Navigation theme](https://docs.example_company.com/ee/user/profile/preferences.html#navigation-theme) to a different color.

### Calculating the Example Company handbook page count
...
- <https://example_company.com/nolith/review-tanuki>
- <https://example_company.com/splattael/traintainer>
- <https://example_company.com/arturoherrero/trainee>
- <https://example_company.com/alberts-example_company/review-tanuki>
- <https://example_company.com/example_company-org/example_company-dev-cli#maintainer-trainee-helper>

**Note:** When using these tools, avoid adding mentions to maintainers in existing comments. There is a [known issue](https://example_company.com/example_company-org/example_company/-/issues/118779) where users are not notified by email when mentioned in an edited comment. It only generates a TODO which a maintainer may not use.

## Handbook Tips

Refer to the [Handbook Development section](https://handbook.example_company.com/docs/development/) to learn more about the architecture, structure and how to edit the handbook locally on your desktop. For how to edit the handbook in your browser, refer to [the editing handbook page](/handbook/about/editing-handbook/).

## www-example_company-com Tips

Portions of the [https://about.example_company.com](https://about.example_company.com) site lives in the
[www-example_company.com](https://example_company.com/example_company-com/www-example_company-com/) repo. The marketing website is maintained by the [digital experience team](/handbook/marketing/digital-experience/) in their [Example Company group](https://example_company.com/example_company-com/marketing/digital-experience).

The `data/*.yml` files live in the `www-example_company-com` repository, and are used by numerous sites including the marketing website and the handbook.

The documentation for the site itself is in markdown documents under the
[`doc` folder in the repo](https://example_company.com/example_company-com/www-example_company-com/-/tree/master/doc). If you are looking to do local development on the site,
[doc/development.md](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/doc/development.md)
...
```sh
[user@example_company ~]$ sudo netstat -tulpn| grep -i listen
 tcp   0      0 127.0.0.1:5000     0.0.0.0:*     LISTEN     18948/registry
 tcp   0      0 127.0.0.1:9100     0.0.0.0:*     LISTEN     18841/node_exporter
 tcp   0      0 127.0.0.1:9229     0.0.0.0:*     LISTEN     18764/example_company-workho
...
When the Example Company Development Kit cannot start using the `./run` command and Unicorn terminates because port 3000 is already in use, you will have to check what process is using it.
...
Take a look at [these aliases](https://example_company.com/sytses/dotfiles/blob/master/zsh/aliases.zsh) and others in [Sid's dotfiles project](https://example_company.com/sytses/dotfiles/tree/master).
...
alias www='cd ~/Dropbox/Repos/www-example_company-com/source'
...
An example can be found in [Michael Friedrich's dotfiles project](https://example_company.com/dnsmichi/dotfiles/-/tree/main/.oh-my-zsh/custom).
...
- [Sid Sijbrandij's dotfiles](https://example_company.com/sytses/dotfiles)
- [Michael Friedrich's dotfiles](https://example_company.com/dnsmichi/dotfiles), more details in [this blog post](https://about.example_company.com/blog/2020/04/17/dotfiles-document-and-automate-your-macbook-setup/)
- [Brendan O'Leary's dotfiles](https://example_company.com/brendan/dotfiles)
...
We [don't have a corporate VPN](/handbook/security#why-we-dont-have-a-corporate-vpn) but consider [purchasing]({{< ref "expenses#reimbursements" >}}) a [personal VPN]({{< ref "personal-vpn" >}}) if you travel for Example Company or use unknown networks often.
...
YubiKey: The most popular FIDO2/WebAuthn device is Yubico's YubiKey. There are a wide variety of sizes and styles of YubiKeys. Yubico (along with Google) helped develop the predecessor to FIDO2 standard (U2F) before it was moved to the FIDO Alliance. Most Example Company team members that have WebAuthn devices have a YubiKey. It should be noted that for a long time Yubico's source code for its firmware was open source, but some of the newer versions of firmware are closed source. This has caused some concern in the security community, particularly those that prefer to use open source whenever possible.

#### Are They Secure?

YubiKey has been examined by the security industry at large, third party audits and by the Security Team. YubiKey's are more than suitable for use within Example Company and work fine with FIDO2/WebAuthn-compatible systems.
...
While there are other hardware tokens that are also decent and will function with Example Company, some have limitations. The recommended solution for team members is Yubico's YubiKeys.
...
Usage of 2FA is mandatory for Example Company team members.
...
Timed-based One-Time Password is fairly secure. This method involves a rotating value based off of a cryptographic seed that is used to uniquely identify communication between the service and the end user authenticating. The value is (usually) a six digit number that changes every 30 seconds, and during the authentication process after you've successfully entered in your username and password, you are asked to enter in the value. Example Company team members should use [1Password](/handbook/security#two-factor-authentication-and-time-based-one-time-passwords) to manage TOTP codes.
...
There is a lot of information pertaining to Slack, as it is a critical part of Example Company's communication. See the [Slack handbook page](slack.md).

## Zoom

There is a lot of information pertaining to Zoom, as it is a critical part of Example Company's
...
1. Pageless docs are great for documents you don't intend to print, and is the [preferred format at Example Company](/handbook/communication/#pageless-is-the-example_company-preferred-format). You can [switch in File > Page setup](https://support.google.com/docs/answer/11528737).
...
It receives data from both about.example_company.com and docs.example_company.com websites.
...
Note that you can use the search tool with:
  ...
  - The higher directory on the file tree, which will return the results for a range of URLs in that tree.
    E.g., `docs.example_company.com/ee/ci/` will return the results for all the range of pages contained in the `/ci/` directory
...
#### Example Company availability calendar

The Example Company Availability Calendar has been deprecated to allow for Example Company to scale effectively.
...
1. Enter in the search field `example_company.com_6ekbk8ffqnkus3qpj9o26rqejg@group.calendar.google.com` and then press enter on your keyboard
...
You can find the details for the Company Calls, Group Conversations, 101s, and other teams' meetings here, so you can attend a different teams' meeting and ask questions, learn about what they're working on, and get to know the rest of the Example Company Departments and teams.

These meetings are open to everyone in Example Company.

If you are creating a new team meeting, please [add it](#adding-an-event-to-the-example_company-team-meetings-calendar) to the Example Company Team Meetings calendar

Please reach out to the People Connect Team in the `#people-connect` Slack channel with any questions, requests or changes to the Example Company Team Meetings calendar.

##### Adding an event to the Example Company Team Meetings calendar

1. Create your calendar invite
1. Add your Zoom link
1. Add an agenda or relevant content
1. Under Add Guests, add `Example Company Team Meetings` in addition to anyone else you want invited (if you want the entire company invited please use everyone@).

   {{% note %}}
   This will appear like adding a room, which is expected
   {{% /note %}}

   ![Adding Example Company Team Meetings](/handbook/tools-and-tips/images/adding-example_company-team-meetings.png)
...
We recommend you set your Google Calendar access permissions to 'Make available for Example Company - See all event details'. Team member calendars should **not** have access permissions set to 'Make available to public' due to the risk of sensitive data exposure and [zoombombing](https://en.wikipedia.org/wiki/Zoombombing).
...
For starters, when your Example Company Google company account is created you automatically get a Google Drive with unlimited storage allocation in your own "home" directory (called My Drive).
You can get to it by:

1. (optional) Login to your Example Company account in your browser (if you are using Chrome)
1. Open your web browser to <https://drive.google.com>
1. If you're not already logged in as your Example Company account (Chrome users should be) then login to Google using your Example Company account
...
#### Existing Example Company Google Drive repositories

There are a few Google Drive repositories of Example Company shared files (there might be more, please add if not listed here):

- [UX Research Drive](https://drive.google.com/drive/folders/0AH_zdtW5aioNUk9PVA) - This houses all findings from [Customer Discovery Meetings](/handbook/product/product-processes/#customer-discovery-meetings) and raw, confidential materials from customers shared with Design, UX Research, Product and Customer Success. Distilled findings from [User Research](/handbook/product/ux/ux-research/) are stored in the [UXR_Insights repository](https://example_company.com/example_company-org/uxr_insights/).
- [Example Company Marketing Drive](https://drive.google.com/drive/u/0/folders/0Bz6KrzE1R_3helZZQlV3ajFNTzg) - This houses all shared files from the entire Marketing organization.
...
- [Example Company Alliance Drive](https://drive.google.com/drive/folders/1ElkWOoepL1eAGi2WfxPNM3W9uEMx62US) - This houses all shared files from the entire Alliance organization.
...
1. Make sure you are logged into your Example Company account in Google Drive in your browser
...
Switch to your Example Company account.
1. Download and install

### Google Forms

Use these [Example Company branded form templates](https://drive.google.com/open?id=0BxrZ6azkqZ1bVDl1TTZuelFOb1k) when creating internal or external surveys or forms.
Make a copy of the form and only edit the copy; do not edit the template itself.

Review [what data privacy means at Example Company](/handbook/legal/privacy/#what-data-privacy-means). Help participants make informed decisions by applying the following guidelines:

- *For external forms*: State that any Personal Data collected will be processed in accordance with the [Example Company Privacy Statement](https://about.example_company.com/privacy/).
...
<span style="color: #999999;font-family: sans-serif;font-size: small;display: block;">Frontend Engineer | Example Company</span>

<img src="https://about.example_company.com/images/press/logo/png/example_company-logo-gray-rgb.png" alt="Example Company Logo" width="98" height="37">

#### Email signature if employed by our German entity

If you are employed by our German entity Example Company GmbH, you will need to include some additional company related details after your own signature in emails ([along with other forms of communication listed on the German entity Handbook page]({{< ref "gmbh-germany" >}})) as follows:
...
<span style="font-family: serif;font-size: small;display: block;">Example Company GmbH  | Sitz: München  | Registergericht: Amtsgericht München, HRB 237630  | Geschäftsführer: James John Gladen</span>

#### Add company details to certain company communications if employed by our German entity

Certain company details are required to be added as a signature to the forms of communication listed below, where that communication is going from a team member employed by our German entity Example Company GmbH to outside of the company, and relates to Example Company's business activities:
...
It might be useful to add a Gmail filter that adds a label to any Example Company notification email in which you are specifically mentioned, as opposed to a notification that you received simply because you were subscribed to the issue or merge request.

1. Search for `from:(example_company@mg.example_company.com) "you+have+been+mentioned+on"`.
...
You can create a Gmail filter that adds a label to any Example Company notification email in which you are assigned as a reviewer:

1. Search for `from:(example_company@mg.example_company.com) "(Reviewer) (Firstname Lastname)"`.
1. Click the down arrow on the right side of the search field.
1. Click **Create filter with this search**.
...
You can create a Gmail filter that adds a label to any Example Company notification email in which an MR has been merged:

1. Search for `from:(example_company@mg.example_company.com) "Merge Request"+"was merged"`.
1. Click the down arrow on the right side of the search field.
1. Click **Create filter with this search**.
...
##### Apply label to all Example Company-generated emails

Example Company issues and merge requests can generate a lot of email notifications depending on your settings and how in-demand your attention is.
It can be useful to apply a label to these generated emails and move them out of your immediate inbox.

1. Search for `from:example_company@mg.example_company.com`.
1. Click the down arrow on the right side of the search field.
1. Click **Create filter with this search**.
1. Check **Skip the Inbox (Archive it)**.
1. Check **Apply the label:** and select a label to add, or create a new one, such as "Example Company.com".
1. Check **Also apply filter to matching conversations**.
1. Click **Create filter**.
...
- [Video](https://youtu.be/IwngC9NmcRs) (It’s set to private, so log in with Example Company Unfiltered to watch it: upper right corner > click on your profile picture > switch account > Example Company Unfiltered)
...
Use this general [Example Company Slide Template](https://docs.google.com/presentation/d/1XhGp21FKlIBiJMI9OpvwyneF6fkrQeOQjFxDJF678Ys/edit#slide=id.g1e546bbceaf_0_817g) when creating slide decks for internal and external use.
Make a copy of the slide deck and only edit the copy; please do not edit the template itself. To avoid potentially editing the source deck, you can go to the [Template gallery](https://docs.google.com/presentation/u/0/?ftv=1&tgif=d) and click on the `Example Company Slide Template vYear-Month` to quickly make a copy and begin a new deck. Ensure that all pages in the slide deck are numbered, with the title page always being number 1.

#### Sales slide decks

Slide decks are available in [Highspot](/handbook/sales/field-communications/example_company-highspot/) in [Example Company's Official Sales Deck Library](https://example_company.highspot.com/items/650461a504701b188c124951?lfrm=shp.6).
...
- Logging in to the browser (with your Example Company Workspace account) preserves [your profile across installations](https://www.google.com/chrome/tips/#customize). This is helpful to migrate bookmarks, extensions, and configuration [when refreshing your laptop](/handbook/it/end-user-services/onboarding-access-requests/#example_company-laptop-refreshes-for-team-members).
