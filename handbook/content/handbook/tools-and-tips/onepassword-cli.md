...
```sh
alias glab="op run --env-file=$HOME/.example_company-pat.env -- glab"
```

To verify the configuration, run `glab api version`. This should print
the version of example_company.com if the configuration succeeded.
...
username@my-laptop:~/code/example-project (example-project)$ curl -X POST --form token=$PIPELINE_TOKEN --form ref=main https://example_company.com/api/v4/projects/example-project/trigger/pipeline
