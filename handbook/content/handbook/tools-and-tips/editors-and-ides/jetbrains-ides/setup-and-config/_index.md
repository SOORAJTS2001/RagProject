...
1. Decide what IDE you need to use for the project you are working on:
    ...
    - CLion for Rust
1. If you are an Example Company employee, you can request and obtain a [License](../licenses) for the IDE(s) you need to use.
...
**However, note that very large and complex projects such as the Example Company Rails monolith under RubyMine may an exception to this. Continue on with the rest of the instructions on this page for more details**.
...
And for very large projects, such as the Example Company Rails monolith, which has
...
JetBrains does offer support for [Shared Indexes for RubyMine](https://www.jetbrains.com/help/ruby/shared-indexes.html) (and other IDEs), which have the potential to greatly reduce the indexing time for people setting up the Example Company project for the first time.
...
If you want to see a current example of the RubyMine excluded folders for the `example_company` project,
you can see Chad's `example_company.iml` module at
<https://example_company.com/jetbrains-ide-config/jetbrains-ide-config-example_company/-/blob/master/dotidea/example_company.iml>,
...
When working with the Example Company project, which is BIG, RubyMine can use a lot of memory when indexing/searching/etc. It's not unusual for memory usage to peak out at **up to 10 gigabytes or more of memory during indexing**, if it is allocated.
...
On the [`#jetbrains-ide-users` internal Slack channel for Example Company team members](https://example_company.slack.com/archives/CR08PTQ6T), JetBrains has indicated that:
...
Although most basic functionality works out of the box in JetBrains IDEs, if you work on a project with very specific and strict rules around code style and formatting (such as Example Company)
...
JetBrains IDEs to work in an optimal and compatible way, specifically the Example Company monorepo.
...
- [Chad Woolley's Jetbrains IDE setup notes](https://example_company.com/cwoolley-example_company/cwoolley-example_company/-/blob/main/example_company-workstation-setup-notes.md#jetbrains-ide-setup)
...
`example_company` project committed and updated in source control (see instructions on how
you can do this in the next section).

You can clone it or copy any files from this config here:
<https://example_company.com/jetbrains-ide-config/jetbrains-ide-config-example_company/-/tree/master/dotidea>
...
<https://example_company.com/jetbrains-ide-config/jetbrains-ide-config-example_company/-/blob/master/README.md#project-level-config>
...
1. ...and you can also see some instructions Chad has written around it here: <https://example_company.com/jetbrains-ide-config/jetbrains-ide-config-example_company/-/blob/master/README.md#ide-level-config>
