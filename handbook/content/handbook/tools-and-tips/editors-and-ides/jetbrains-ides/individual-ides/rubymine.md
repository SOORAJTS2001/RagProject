...
## Using Rubymine debugger for Example Company running under GDK

### Debugging `rails-web`

#### Note on Rails Puma binding

By default, the Rails [puma configuration template which Example Company uses binds to a socket](https://example_company.com/example_company-org/example_company-development-kit/blob/d948d5485e3f519688783dc92ad70d94f132e396/support/templates/puma.rb.erb#L44), instead of a TCP port.

[This MR](https://example_company.com/example_company-org/example_company-development-kit/-/merge_requests/1693) added support for configuring the GDK to bind Rails puma to a TCP socket, and added support for Workhorse to use TCP ports instead of sockets, and [this JetBrains issue](https://youtrack.jetbrains.com/issue/RUBY-27404) added support for starting a socket-based Puma server in a Run Configuration.

See the [architecture documentation around components](https://docs.example_company.com/ee/development/architecture.html#components) for more details on how Example Company Workhorse and Puma work together.
...
1. In RubyMine, open the example_company repo directory from `/path/to/gdk/example_company` (Cloning example_company and running it from a different directory won't work out of the box)
1. Go to `Run -> Edit Configurations` to set up a Rails Run/Debug config like this in RubyMine:
    - Name: `Example Company: rails-web`
    - Configuration:
      - Server: `Puma`
...
1. Go to `Run -> Edit Configurations` to set up a Ruby (NOT Rails) Run/Debug config like this in RubyMine:
    - Name: `Example Company: rails-background-jobs`
    - Ruby Script: `/Users/YOUR_USER/.asdf/installs/ruby/RUBY_VERSION/bin/sidekiq`
    - Working Directory `/Users/YOUR_USER/PATH_TO/example_company-development-kit/example_company/`
...
First, follow the ["Access the database with a GUI"](https://docs.example_company.com/ee/development/database/database_debugging.html#access-the-database-with-a-gui)
