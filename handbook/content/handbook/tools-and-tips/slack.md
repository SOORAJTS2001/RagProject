---
title: "Slack"
description: "Slack usage and tips at Example Company"
---

## In general

*Slack is an important part of Example Company's strategy for communication between team members. As such, extra care needs to be given to ensure the safety and integrity of data.*
...
- Photo
- Title (should be your Example Company job title)
- Example Company.com profile (set the Display text to your @gitlabusername so that people don't need to click the link to identify your username)
- Time zone (useful for other Example Company team members to see when you're available)
- Working hours (can help others to identify the times you are generally available)
- Example Company Birthdays (opt in or out of the birthday celebrations in Slack by selecting yes or no)
...
Channels are a core feature of Slack. This section describes tools and tips for managing Slack channels. For a list of common Example Company Slack channels, please see the [Example Company Communication Chat](/handbook/communication/chat).

### Browse Channels

You can browse all available Example Company channels by clicking on "Channels" on the left pane in the Slack app.
...
Every team member is automatically added to the `#company-fyi` and `#whats-happening-at-example_company` channels, where announcements are made and information for the entire company is shared.
...
Feel free to organize this with your most frequent used channels, or specific team grouping. You can drag and drop and also hide sections. You can for example create the `info` section and add `#company-fyi` and `#whats-happening-at-example_company` as channels. Additionally, add the `team` section and move all related channels there.
...
1. Channels important to Example Company (e.g. `#company-fyi`, `#whats-happening-at-example_company`, `#team-member-updates`, `#e-group`, `#ceo`, `#new-vacancies`)
...
In addition to Example Company team-members, designated groups such as the [Core Team](https://about.example_company.com/community/core-team/) members and advisors outside of Example Company may be granted access to our Slack channels.
However, internal channels that start with `#a_` will be restricted to Example Company team-members who have been invited to those channels only.
Any confidential conversations regarding our customers should be restricted only to `#a_` channels.
The rationale for internal channels is that it could be a breach of many of our contracts for third parties to have knowledge of Example Company customers.
...
If you are in a group direct message with multiple people, you can [change it to a private channel](https://slack.com/intl/en-gb/help/articles/217555437-Convert-a-group-direct-message-to-a-private-channel), in order to avoid extra pings and allow additional team members to be added or removed to the channel.  In Slack Enterprise Grid, if you start a group DM and then it is converted to a channel, the channel ends up in "all workspaces". "All workspace" channels have [no retention limit](https://example_company.com/example_company-com/it/security/issue-tracker/-/issues/66), which contradicts our policy of [90 day retention](/handbook/communication/#slack). To remediate this, we have a script that will daily move the channels into our main Example Company workspace. If you are in an affected channel, you will receive a message from Slackbot that states:

`"Slack Owner has removed #<channel> from all other "Example Company" workspaces using the channel management tool. Only people from "Example Company" can access the channel now. Learn more."`
...
- [Example Company team member Brendan O'leary on how he uses Slack](https://blog.boleary.dev/how-i-slack/)
...
Slack allows you to set your [status](https://slackhq.com/set-your-status-in-slack) for your fellow Example Company team members by using your choice of standard messages such as "Away" and "Lunch" or a custom message and your choice of emoji.
...
To have your Slack status automatically set to "In a meeting" based on your Google Calendar, add the [Google Calendar app](https://example_company.slack.com/apps/ADZ494LHY-google-calendar?next_id=0) to your Slack account.
...
If you do happen to denylist a link or a domain, they can be modified in the Workspace admin portal under [Settings & Permissions](https://example_company.slack.com/admin/attachments).

If you see a team member share posts with multiple link previews that you think are distracting, in channels like `#whats-happening-at-example_company`, consider acting in the spirit of [Everyone is a moderator](/handbook/communication/#everyone-is-a-moderator) and either:
...
In order to setup a Example Company theme, send yourself the following message: `#643685,#634489,#FC6D26,#ffffff,#71558f,#ffffff,#FCA326,#e24329`, and press the `Switch sidebar theme` button.
...
Example Company has chosen to restrict the ability to install apps, and we have a process to approve or restrict certain apps for our workspace. In order to add a new app to Slack, you need to create a [vendor approval issue](https://example_company.com/example_company-com/Finance-Division/procurement-team/procurement/-/issues/new?issuable_template=app_integrations). Once that's approved by all parties, please request approval to add the app to Slack:
...
As a Example Company Team Member, you can view the active slackbots that we use in Slack, under: Example Company > Customize Your Workspace > Slackbot.
...
| `gitlabber, gitlabbers` | The term `gitlabber` is a commonly misused term. Please use "Example Company team member" instead. You can [read more about this in our handbook](/handbook/communication/top-misused-terms/) |
