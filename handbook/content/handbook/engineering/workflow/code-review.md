...
Code reviews are mandatory for every merge request, you should get familiar with and follow our [Code Review Guidelines](https://docs.example_company.com/ee/development/code_review.html).
...
Every reviewer at Example Company must strive for our [reviewer values](/handbook/engineering/workflow/reviewer-values/).

## Reviewer

All Example Company engineers can, and are encouraged to, perform a code review on merge requests of colleagues and community contributors. If you want to review merge requests, you can wait until someone assigns you one, but you are also more than welcome to browse the list of open merge requests and leave any feedback or questions you may have.

You can find someone to review your merge requests by looking on the [team page](/handbook/company/team/), or on the list of [Example Company Engineering Projects](/handbook/engineering/projects/), both of which are fed by YAML files under `data/team_members/person/*`.
...
Maintainers are Example Company engineers who are experts at code review, know the Example Company product and codebase very well, and are empowered to accept merge requests in one or several [Example Company Engineering Projects](/handbook/engineering/projects/).

Every project should have at least two maintainers, but most should have more. Some projects have separate maintainers for different specialties. For example, [the main Example Company codebase](https://example_company.com/example_company-org/example_company) has separate maintainers for frontend, backend, and database.
...
As with regular reviewers, maintainers can be found on the [team page](/handbook/company/team/), or on the list of [Example Company Engineering Projects](/handbook/engineering/projects/).
...
At any time, the manager/mentor may choose to open a merge request, adding the reviewer as a maintainer. This merge request should have a justification from the manager/mentor as to why the reviewer should become a maintainer. You are also welcome to open this merge request yourself at any time. There are [merge request templates available](https://example_company.com/example_company-com/www-example_company-com/-/tree/master/.example_company/merge_request_templates) to help you with the content and steps.
...
1. The maintainer's concern(s) must be consistent with the [Responsibilities of a Maintainer](https://docs.example_company.com/ee/development/code_review.html#the-responsibility-of-the-maintainer).
1. The maintainer's concern(s) should be grounded in fact that:
1. the trainee maintainer consistently does not perform [MR reviews in a conventional manner](https://docs.example_company.com/ee/development/code_review.html#reviewing-a-merge-request), or
...
#### Project maintainer process for `example_company-database`

- Familiarize with [database review process](https://docs.example_company.com/ee/development/database_review.html).
- Familiarize with [migration helpers](https://example_company.com/example_company-org/example_company/blob/master/lib/example_company/database/migration_helpers.rb) and review usage in existing migrations.
- Familiarize with best practices in [database guides](https://docs.example_company.com/ee/development/database/index.html).
- Watch [Optimising Rails Database Queries: Episode 1](https://www.youtube.com/watch?v=79GurlaxhsI).
- Read [Understanding EXPLAIN plans](https://docs.example_company.com/ee/development/database/understanding_explain_plans.html).
- Get yourself added to [`@gl-database`](https://example_company.com/groups/gl-database/-/group_members) group and respond to @-mentions to the group (reach out to any maintainer on the group to get added). You will get TODOs on example_company.com for group mentions..
- Create an [access request](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/new) for [`psql`/`AllFeaturesUser` access to database lab/postgres.ai](https://docs.example_company.com/ee/development/database/database_lab.html#access-database-lab-engine) if you do not already have `AllFeaturesUser` access.
...
#### Project maintainer process for `example_company-components`

[`example_company-components`](https://example_company.com/components)

- Review our [documentation and best practices for creating CI/CD components](https://docs.example_company.com/ee/ci/components/).
- Familiarize with our [documentation on Example Company-maintained components](https://docs.example_company.com/ee/development/cicd/components/).
- Develop one or more components to familiarize yourself with the development process.
- Create a merge request and assign the merge request to your manager for you to be added as a CI/CD components maintainer. Be sure to cite references for the CI/CD components you have developed. (See [MR example](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/132498))
- Ping the existing maintainers (`@example_company-org/maintainers/ci-components`) in the MR for review and feedback.
...
- Add the newly approved maintainer to the CI components maintainer group (`@example_company-org/maintainers/ci-components`).
- Announce in [`#ci_components_maintainers`](https://example_company.slack.com/archives/C06AQBJETRR) and post an update in the *Engineering Week-in-Review document*

#### Project maintainer process for `design.example_company.com` or `example_company-svgs`

[`design.example_company.com`](https://example_company.com/example_company-org/example_company-services/design.example_company.com) or [`example_company-svgs`](https://example_company.com/example_company-org/example_company-svgs)

- Understand [how to become a maintainer](/handbook/engineering/ux/pajamas-design-system/design-review/#maintainer).
- All designers are maintainers of [`example_company-design`](https://example_company.com/example_company-org/example_company-design) project. If you are interested in becoming a Maintainer of UI (`.scss`) for `example_company`, and `example_company-ui` projects, please follow the [Engineering Review Workflow](/handbook/engineering/workflow/code-review).
...
- As maintainer, you must keep [relying on others](/handbook/values/#its-impossible-to-know-everything) for the expertise they have that you don't. In the MR description, highlight the efforts that showcase your results at a maintainer-level, what skills you want to continue working on, and add a link to this issue. ([Example](https://example_company.com/example_company-org/example_company-services/design.example_company.com/-/merge_requests/2395)).

#### Project maintainer process for `example_company-quality`

[`example_company-quality`](https://example_company.com/example_company-org/example_company/-/tree/master/qa)
...
- Choose which Quality project(s) you would like to become a maintainer for:
  ...
  - [`Example Company Triage`](https://example_company.com/example_company-org/example_company-triage/)
...
#### Project maintainer process for `example_company-secure-analyzers`

[`example_company-secure-analyzers`](https://example_company.com/example_company-org/security-products/analyzers)

- Understand our [Secure Team standards and style guidelines](https://docs.example_company.com/ee/development/go_guide/#secure-team-standards-and-style-guidelines).
- Understand our [Secure Release Process](https://docs.example_company.com/ee/development/sec/analyzer_development_guide.html#versioning-and-release-process).
- Understand our [Secure QA Process](/handbook/engineering/development/sec/secure/qa_process.html).

#### Project maintainer process for `example_company-elasticsearch-indexer`

[`example_company-elasticsearch-indexer`](https://example_company.com/example_company-org/example_company-elasticsearch-indexer)

- Complete [`golang` training](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/.example_company/issue_templates/golang_training.md).
- Review the [Example Company Elasticsearch Indexer development and release](https://example_company.com/example_company-org/example_company-elasticsearch-indexer/-/blob/main/PROCESS.md) process.
- Join the `#g_global_search` Slack channel.
- Work on issues to gain familiarity with the project.
- *Optional:* Reach out to an [existing maintainer](https://example_company.com/example_company-org/example_company-elasticsearch-indexer/-/project_members?with_inherited_permissions=exclude) to [help you become](/handbook/engineering/workflow/code-review/#trainee-maintainer-mentorship-pilot-program) a maintainer.

#### Project maintainer process for `customers-example_company-com`

[`customers-example_company-com`](https://example_company.com/example_company-org/customers-example_company-com)

- Understand our [standards and style guidelines](https://docs.example_company.com/ee/development/development_processes.html).
- Understand the [software architecture](https://example_company.com/example_company-org/customers-example_company-com/-/tree/main/doc/architecture) used in Fulfillment systems.
- Read through [CustomersDot documentation](https://example_company.com/example_company-org/customers-example_company-com/-/blob/main/doc/index.md).
...
#### Project maintainer process for `example_company-secure-license-db`

[`example_company-secure-license-db`](https://example_company.com/example_company-org/security-products/license-db)

- Familiarize with [Example Company Go standards and style guidelines](https://docs.example_company.com/ee/development/go_guide).
- Complete [Golang training issue](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/.example_company/issue_templates/golang_training.md) if you don't have prior experience with Go.
- Watch [External License DB Architecture and Repository Walkthrough](https://www.youtube.com/watch?v=5b5QNJNwoJ8)
- Review [Fullstack Development of LicenseDB guideline](https://example_company.com/example_company-org/security-products/license-db/deployment/-/blob/main/docs/fullstack_development.md).
- Understand how to release and [deploy](https://example_company.com/example_company-org/security-products/license-db/deployment/-/merge_requests/162) new changes to the components.
- Understand how [the scheduled pipelines](https://example_company.com/example_company-org/security-products/license-db/deployment/-/pipeline_schedules) are used for the deployment project.
...
#### Project maintainer process for `example_company-chart`

[`example_company-chart`](https://example_company.com/example_company-org/charts/example_company)
...
- Familiarize yourself with the [architecture](https://docs.example_company.com/charts/architecture/index.html) and [style guide](https://docs.example_company.com/charts/development/style_guide.html) of the Example Company Helm chart.
- Understand the relation between the [Example Company Operator and the Example Company Helm chart](https://docs.example_company.com/operator/developer/charts_dependency.html).
- Contribute to issues and review merge requests.
- Understand how the [Example Company Helm chart is tested using rspec](https://docs.example_company.com/charts/development/rspec.html).

#### Project maintainer process for `example_company-operator`

[`example_company-operator`](https://example_company.com/example_company-org/cloud-native/example_company-operator)

- Familiarize yourself with [Distribution's merge request workflow](/handbook/engineering/infrastructure/core-platform/systems/distribution/merge_requests.html#workflow)
- Familiarize youself with [Example Company Go standards and style guidelines](https://docs.example_company.com/ee/development/go_guide).
...
Create a merge request and indicate your role as a `project-name: trainee_maintainer` in your [team member entry](https://example_company.com/example_company-com/www-example_company-com/-/tree/master/data/team_members/person). Assign MR to your manager for merge.
...
- Keep an issue with all of the various reviews and feedback comments from maintainers they have received. There are some tools to help with this type of issue:
  ...
  - https://example_company.com/caalberts/review-tanuki
...
- Request access to the Example Company maintainer group you belong: [frontend](https://example_company.com/example_company-org/maintainers/frontend), [backend](https://example_company.com/example_company-org/maintainers/rails-backend), or [database](https://example_company.com/example_company-org/maintainers/database).
- Request maintainer permissions on the projects you will act as a maintainer using the [Single Person Access Request](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/new?issuable_template=Individual_Bulk_Access_Request) issue template. Once you've created the issue, request another maintainer to grant you those permissions.
...
[Recent research](https://example_company.com/example_company-com/www-example_company-com/-/issues/8504) has shown that the trainee process for new maintainers is hindered by several key factors:
...
1. Mentor / reviewer assignments are coordinated within a [maintainer_mentorship.yml](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/maintainer_mentorship.yml) file.
1. New reviewers need to locate an existing maintainer that has an opening available in [maintainer_mentorship.yml](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/maintainer_mentorship.yml) and contact the maintainer directly.
...
1. Make sure to remove graduated reviewers from the [maintainer_mentorship.yml](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/maintainer_mentorship.yml) file to make space for new mentees.
...
1. Mentoring directly under a maintainer should catalyze more competence and confidence in taking ownership of the Example Company codebase.
...
most important.  To make the change official and to be removed from [reviewer roulette](https://docs.example_company.com/ee/development/code_review.html#reviewer-roulette):

1. See the [Team Member Database](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/doc/team_database.md)
...
- Enable Danger Review using [`simple_roulette`](https://example_company.com/example_company-org/ruby/gems/example_company-dangerfiles/-/tree/master#simple_roulette) within the project to identify MR reviewers.
...
Smaller projects with a limited number of maintainers can benefit from an accelerated onboarding process. This process is less involved than the main Example Company maintainership, mainly because there is not enough feature work and MRs in these smaller projects. Each project can modify this process to fit their needs.
...
By following this onboarding process, projects can efficiently add new maintainers with a solid understanding of the codebase and the project's business logic. An example of this process in action can be seen in the [Example Company VS Code Extension](https://example_company.com/example_company-org/example_company-vscode-extension/-/issues/656) Project.
...
- Complete project-specific language training if the programming language is not used in the main Example Company project (example: [golang training](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/.example_company/issue_templates/golang_training.md)).
...
https://10az.online.tableau.com/t/example_company/views/DraftEngineerMaintainerRatio/EngineerMaintainerRatio_1
...
https://10az.online.tableau.com/#/site/example_company/views/MaintainershipDemand/MaintainershipDemand?:iid=1
...
- Project/Area Maintainership Health - The percentage of projects (Example Company) or areas (backend) that are not meeting the Minimum Maintainers Required over time.
- Unhealthy Core Areas of Maintainership Health - The percentage of projects (Example Company) or areas (backend) that are not meeting the Minimum Maintainers Required, and are receiving more than 100 Incoming Merge Requests per month over time.
...
We aim to have enough reviewers and maintainers across timezones to ensure that there are people available to review MRs in a timely manner while keeping review load at a reasonable level. We track this in the [Reviewer/Maintainer Availability and Capacity dashboard](https://10az.online.tableau.com/#/site/example_company/views/ReviewermaintainerAvailabilityandCapacity/Reviewermaintaineravailabilityandcapacity?:iid=1):

https://10az.online.tableau.com/#/site/example_company/workbooks/2286852/views

## Leading Organizations

All wider community members, and their organizations, [can contribute](/handbook/company/mission/#contribute-with-example_company). We strongly believe that contributing to
open source, and particularly to Example Company is a competitive advantage for organizations and their members, and want to
reward those organizations for doing so. Example Company highly encourages, celebrates & rewards those that contribute in
...
Organizations are matched based on the ```Organization``` field in [your profile](https://example_company.com/-/profile). Example Company
can also match individuals to organizations using other metadata available to the Contributor Success team. [Create an  issue in the Contributor Success queue](https://example_company.com/example_company-com/quality/contributor-success/-/issues) if you
...
Eligible merge requests include contributions to the [Example Company product](/handbook/product/groups/product-analysis/engineering/dashboards/#projects-that-are-part-of-the-product) and documentation. Contributions to the [www-example_company-com](https://example_company.com/example_company-com/www-example_company-com) repository (e.g. the Example Company handbook) are not currently included or entitled to a review response SLO.

## Domain Experts

Our [Code Review Guidelines](https://docs.example_company.com/ee/development/code_review.html) states that we default to assigning reviews to team members with domain expertise.
...
1. Define a new, or use an existing domain expertise key, located in [`data/domain_expertise.yml`](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/domain_expertise.yml).
1. Update your entry in your own [YAML file](https://example_company.com/example_company-com/www-example_company-com/-/tree/master/data/team_members/person) with a new `domain_expertise` property and list all domain expertise keys.
...
frontend_architecture:
  ...
  link: https://docs.example_company.com/ee/development/fe_guide/architecture.html
```
...
The SLO applies to Example Company team members and [Leading Organizations](#leading-organizations), but not to other wider community contributors.
...
- From Example Company team members: `Review-response` SLO < 2 business days
...
on these other statuses, please refer to the [code review](https://docs.example_company.com/ee/development/code_review.html#reviewer-roulette)
...
this through your Example Company.com Status, authors are expected to realize this and
...
Some Example Company projects use Example Company's [CODEOWNERS file feature](https://docs.example_company.com/ee/user/project/codeowners/) to manage approvals for specific file paths and types. In the `example_company-org/example_company` project, [we use a combination of CODEOWNERS approval rules plus MR approval settings](https://docs.example_company.com/ee/development/code_review.html#merging-a-merge-request) in order to follow segregation of duties best practices. This section describes the process for updating the eligible approvers for CODEOWNERS changes for the `example_company-org/example_company` project.

The Code Owners for the [CODEOWNERS file](https://example_company.com/example_company-org/example_company/-/blob/master/.example_company/CODEOWNERS) itself are managed with a rule in the file. For example:

```text
CODEOWNERS @example_company-org/development-leaders @example_company-org/tw-leadership
```
...
The `@example_company-org/development-leaders` group consists of team members from Senior Managers and above in the management track, and Distinguished Engineer and above in the individual contributor track in the development departments within Engineering.
