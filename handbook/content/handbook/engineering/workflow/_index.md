...
description: "This document explains the workflow for anyone working with issues in Example Company Inc."
---

This document explains the workflow for anyone working with issues in Example Company Inc.
For the workflow that applies to the wider community see the [contributing guide](https://docs.example_company.com/ee/development/contributing/).

## Example Company Flow

Products at Example Company are built using the [Example Company Flow](https://about.example_company.com/blog/2023/07/27/example_company-flow-duo/).
...
If you notice that pipelines for the `master` branch of [Example Company](https://example_company.com/example_company-org/example_company) or [Example Company FOSS](https://example_company.com/example_company-org/example_company-foss) are failing, returning the build to a passing state takes priority over everything else development related, since everything we do while tests are broken may:
...
The cost to fix test failures increases exponentially as time passes due to [merged results pipelines](https://docs.example_company.com/ee/ci/pipelines/merged_results_pipelines.html) used. Auto-deploys, as well as monthly releases and security releases, depend on `example_company-org/example_company` master being green for tagging and [merging of backports](https://example_company.com/example_company-org/release/docs/-/blob/master/general/security/release-manager.md#regular-security-releases).
...
1. See if there is a non-resolved [broken `master` incident](https://example_company.com/example_company-org/quality/engineering-productivity/master-broken-incidents/-/issues) with a DRI assigned and check discussions there.
...
1. Identification
   - Review non-resolved [broken `master` incidents](https://example_company.com/example_company-org/quality/engineering-productivity/master-broken-incidents/-/issues) for the same failure. If the broken `master` is related to a test failure, [search the spec file in the issue search](https://example_company.com/example_company-org/example_company/-/issues?sort=created_desc&state=opened&label_name[]=failure::flaky-test) to see if there's a known `failure::flaky-test` issue.
   ...
   - If you identified that `master` fails **for a flaky reason**, and it cannot be reliably reproduced (i.e. running the failing spec locally or retrying the failing job):
      - [Quarantine](https://docs.example_company.com/ee/development/testing_guide/flaky_tests.html#quarantined-tests) the failing test to restore pipeline stability within 30 minutes if the flakiness is continuously causing master pipeline incidents.
      ...
      - Close the incident
   - Add the stacktrace of the error to the incident (if it is not already posted by example_company-bot), as well as Capybara screenshots if available in the job artifacts.
   ...
   - Identify the merge request that introduced the failures. There are a few possible approaches to try:
      ...
      - [Look at the project activity](https://example_company.com/example_company-org/example_company/activity), and search for keywords in the recent merged events.
      - [Look at the recent commits on master](https://example_company.com/example_company-org/example_company/-/commits/master) and search for keywords you might see in the failing job/specs (e.g. if you see a `geo` spec file is failing, specifically the `shard` spec, search for those keywords in the commit history).
        - You can [filter with the `Merge branch` text](https://example_company.com/example_company-org/example_company/-/commits/master?search=Merge%20branch) to only see merge commits.
      - Look at the spec file history or blame views, by clicking respectively the `History` or `Blame` button at the top of a file in the file explorer, e.g. at <https://example_company.com/example_company-org/example_company/-/blob/master/lib/backup.rb>.
   - If you identified a merge request, assign the incident to its author if they are available at the moment. If they are not available, assign to the maintainer that approved/merged the MR. If none are available, mention the team Engineering Manager and seek assistance in the `#development` Slack channel.
      - You can find the team somebody is in and who's the manager for that team by searching in https://handbook.example_company.com/handbook/product/categories/.
   ...
   - Please set the appropriate `~master-broken:*` label from the list below:
      ...
      /label ~"master-broken::infrastructure::example_company-com-overloaded"
...
1. (Optional) Pre-resolution
   ...
   - If the failures occur only in `test-on-gdk` jobs, it's possible to stop those jobs from being added to new pipelines while the cause is being fixed. See the [runbook](https://example_company.com/example_company-org/quality/runbooks/-/tree/main/test_on_gdk#disable-the-e2etest-on-gdk-pipeline) for details.

#### Pro-tips for Triage DRI

1. For an initial assessment of what might have contributed to the failure, we can try the experimental AI-assisted [root cause analysis](https://docs.example_company.com/ee/user/gitlab_duo/index.html#root-cause-analysis) feature following [this documentation](https://docs.example_company.com/ee/user/gitlab_duo_chat/examples.html#troubleshoot-failed-cicd-jobs-with-root-cause-analysis).
2. To confirm flakiness, you can use the `@example_company-bot retry_job <job_id>` or the `@example_company-bot retry_pipeline <pipeline_id>` command to retry the failed job(s), even if you are not a project maintainer.
   ...
   - **Note**, The `retry_job` command can fail for the following reasons:
     ...
     - If there is no response to either of the `retry` commands, you are likely invoking them in non-supported projects. If you'd like to request for the commands to be added to your project, please [make an issue](https://example_company.com/example_company-org/quality/triage-ops/-/issues/new) and inform `#g_engineering_productivity`. You are encouraged to self-serve the MR following [this example](https://example_company.com/example_company-org/quality/triage-ops/-/merge_requests/2536) and submit it for review for maximum efficiency.
...
1. Prioritize resolving recurring broken `master` incidents over new bug/feature work. Resolution options include:
   - **Default**: Revert the merge request which caused the broken `master`. If a revert is performed,
     ...
     of the reverted merge request.
       ...
       - The `pipeline::expedited` label, and `master:broken` or `master:foss-broken` label must be set on merge requests that fix `master` to skip some non-essential jobs in order to speed up the MR pipelines.
   - [Quarantine](https://docs.example_company.com/ee/development/testing_guide/flaky_tests.html#quarantined-tests) the failing test if you can confirm that it is flaky (e.g. it wasn't touched recently and passed after retrying the failed job).
...
1. If the broken `master` incident affects any stable branches (e.g. <https://example_company.com/example_company-org/example_company/-/merge_requests/25274>) or is caused by a flaky failure,
   ...
   See [How to fix a broken stable branch guide](https://example_company.com/example_company-org/release/docs/-/blob/master/general/how-to-fix-broken-stable-branch.md) for more details.
...
1. Create an [issue](https://example_company.com/example_company-org/quality/team-tasks/issues/new) for the [Engineering Productivity team](/handbook/engineering/infrastructure/engineering-productivity/) describing how the broken `master` incident could have been prevented in the Merge Request pipeline.
...
- Maintainers should start a new merged results pipeline (for canonical MRs)
  ...
  There's no need to rebase once `master` has been fixed since we use [merged results pipelines](https://docs.example_company.com/ee/ci/pipelines/merged_results_pipelines.html).
- (For forks only) Authors should rebase their open merge requests (since
  [merged results pipelines](https://docs.example_company.com/ee/ci/pipelines/merged_results_pipelines.html)
...
- Merge requests that need to be deployed to Example Company.com to alleviate an ongoing production incident.
...
`example_company-org/example_company` using
[merged results pipelines](https://docs.example_company.com/ee/ci/pipelines/merged_results_pipelines.html)).
...
1. There is a corresponding non-resolved [broken `master` incidents](https://example_company.com/example_company-org/quality/engineering-productivity/master-broken-incidents/-/issues).
...
- Otherwise, you must:
  1. Unset the
    ["Pipelines must succeed" setting](https://docs.example_company.com/ee/user/project/merge_requests/merge_when_pipeline_succeeds.html#require-a-successful-pipeline-for-merge)
    for the [`example_company-org/example_company` project](https://example_company.com/example_company-org/example_company/edit).
...
[`#master-broken-mirrors`](https://example_company.slack.com/archives/C01PK38VAN8) was created to remove duplicative notifications from the `#master-broken` channel which provides a space for [Release Managers](https://about.example_company.com/community/release-managers/) and the [Engineering Productivity team](/handbook/engineering/infrastructure/engineering-productivity/) to monitor failures for the following projects:

- <https://example_company.com/example_company-org/security/example_company>
- <https://dev.example_company.org/example_company/example_company-ee>
...
you need to follow the [Security Release process](https://example_company.com/example_company-org/release/docs/blob/master/general/security/process.md).

If you find a security issue in Example Company, create a **confidential issue** mentioning the relevant security and engineering managers, and post about it in `#security`.

If you accidentally push security commits to `example_company-org/example_company`, we recommend that you:
...
[documentation on security releases](https://example_company.com/example_company-org/release/docs/blob/master/general/security/process.md).
...
1. Start working on an issue you're assigned to. If you're not assigned to any issue, find the issue with the highest priority and relevant label you can work on, and assign it to yourself. [You can use this query, which sorts by priority for the started milestones](https://example_company.com/groups/example_company-org/-/issues?scope=all&utf8=%E2%9C%93&state=opened&milestone_title=%23started&assignee_id=None&sort=priority), and filter by the label for your team.
...
1. When you start working on an issue:
   ...
   - Work on the MR until it is ready, it meets Example Company's [definition of done](https://docs.example_company.com/ee/development/contributing/merge_request_workflow.html#definition-of-done), and the pipeline succeeds.
   - Edit the description and click on the **Remove the Draft: prefix from the title** button.
   - Assign it to the suggested reviewer(s) from [Reviewer Roulette](https://docs.example_company.com/ee/development/code_review.html#reviewer-roulette). If there are reviewers for multiple categories, for example: frontend, backend and database, assign all of them. Alternatively, assign someone who specifically needs to review. When assigning, also @mention them in the comments, requesting a review.
   - (Optionally) Unassign yourself from the MR. Some may find leaving the MR assigned to themselves easier to track the MRs they are responsible for by using the built in MR button/notification icon in the Example Company navigation bar.
   ...
   - The last maintainer to approve the MR, follows the [Merging a merge request](https://docs.example_company.com/ee/development/code_review.html#merging-a-merge-request) guidelines.
...
1. You (and your team, if applicable) are responsible for:
   ...
   - Ensuring that your changes [apply cleanly to Example Company Enterprise Edition](https://docs.example_company.com/ee/development/ee_features.html).
   - The testing of a new feature or fix, especially right after it has been merged and packaged.
   - Creating any [relevant feature or API documentation](https://docs.example_company.com/ee/development/documentation/workflow.html#developers)
...
Be sure to read general guidelines about [issues](https://docs.example_company.com/ee/development/contributing/issue_workflow.html) and [merge requests](https://docs.example_company.com/ee/development/contributing/merge_request_workflow.html).
...
Workflow labels are described in our [Development Documentation](https://example_company.com/example_company-org/example_company-foss/-/blob/master/doc/development/labels/index.md#workflow-labels) and [Product Development Flow](/handbook/product-development-flow/).
...
1. Consider starting a Slack channel for a new team, but remember to write all relevant information in the related issue(s). You don't want to have to read up on two threads, rather than only one, and Slack channels are not open to the greater Example Company community.
1. If an issue entails frontend and backend work, consider separating the frontend and backend code into separate MRs and merge them independently under [feature flags](https://docs.example_company.com/ee/development/feature_flags/index.html). This will ensure frontend/backend engineers can work and deliver independently.
    1. It's important to note that even though the code is merged behind a feature flag, it should still be production ready and continue to hold our [definition of done](https://example_company.com/example_company-org/example_company-foss/-/blob/master/doc/development/contributing/merge_request_workflow.md#definition-of-done).
...
`example_company.yml`. Only add configuration if it is absolutely necessary. If you
...
[Use this link to quickly set the above parameters](https://example_company.com/groups/example_company-org/-/issues?scope=all&utf8=%E2%9C%93&state=opened&milestone_title=%23started&assignee_id=None&sort=priority). You'll still need to filter by the label for your own team.
...
Make sure to follow our [Code Review Guidelines](https://docs.example_company.com/ee/development/code_review.html).

## Working with Example Company.com

Example Company.com is a very large instance of Example Company Enterprise Edition. It runs release candidates for new releases, and sees a lot of issues because of the amount of traffic it gets. There are several internal tools available for developers at Example Company to get data about what's happening in the production system:

### Performance Data

There is extensive [monitoring](https://dashboards.example_company.com/) publicly available for Example Company.com. For more on this and related tools, see the [monitoring handbook](/handbook/engineering/monitoring/).

### Error Reporting

- [Sentry](https://sentry.example_company.net/) is our error reporting tool
- [log.gprd.example_company.net](https://log.gprd.example_company.net/) has production logs
- [prometheus.example_company.com](https://prometheus.example_company.com/alerts) has alerts for the [production team](/handbook/engineering/infrastructure/#production-team)

## Scheduling Issues

Example Company Inc has to be selective in working on particular issues. We have a limited capacity to work on new things. Therefore, we have to schedule issues carefully.
...
[![''](/handbook/engineering/workflow/example_company-release-timelines.png)](https://example_company.com/example_company-org/example_company/-/snippets/3670861)
...
- **Wednesday, 11 days after the milestone begins**:
  - Example Company Bot opens [Group Retrospective](/handbook/engineering/management/group-retrospectives/) issue for the current milestone.
...
Note that deployments to Example Company.com are more frequent than monthly major/minor releases.
See [auto deploy transition](https://example_company.com/example_company-org/release/docs/blob/21cbd409dd5f157fe252f254f3e897f01908abe2/general/deploy/auto-deploy-transition.md#transition) guidance for details.
...
The [Product Kickoff page](https://about.example_company.com/direction/kickoff/) is updated each month, which follows the content on the livestream.
...
This is currently implemented as part of our [automated triage operations](https://example_company.com/example_company-org/quality/triage-ops/blob/master/policies/move-milestone-forward.yml). Additionally, issues with the `~Deliverable` label which have a milestone beyond current +1, will have the `~Deliverable` label removed.

We keep the milestone open for 3 months after it's expired, based on the [release and maintenance policy](https://docs.example_company.com/ee/policy/maintenance.html).

The milestone cleanup is currently applied to the [following groups and projects](https://example_company.com/example_company-org/quality/triage-ops/blob/master/.example_company/ci/missed-resources.yml):

- [Example Company](https://example_company.com/example_company-org/example_company), [schedule](https://example_company.com/example_company-org/quality/triage-ops/pipeline_schedules/10515/edit)
- [Example Company Runner](https://example_company.com/example_company-org/example_company-runner), [schedule](https://example_company.com/example_company-org/quality/triage-ops/pipeline_schedules/29681/edit)
- [Example Company Gitaly](https://example_company.com/example_company-org/gitaly), [schedule](https://example_company.com/example_company-org/quality/triage-ops/pipeline_schedules/29054/edit)
- [Example Company charts](https://example_company.com/example_company-org/charts), [schedule](https://example_company.com/example_company-org/quality/triage-ops/pipeline_schedules/39481/edit)
- [Example Company QA](https://example_company.com/example_company-org/example_company-qa), [schedule](https://example_company.com/example_company-org/quality/triage-ops/pipeline_schedules/29050/edit)
- [Omnibus Example Company](https://example_company.com/example_company-org/omnibus-example_company), [schedule](https://example_company.com/example_company-org/quality/triage-ops/pipeline_schedules/31880/edit) (only moving milestone for now, not labelling)
...
The monthly kickoff meeting is publicly streamed to the [Example Company Unfiltered YouTube Channel](https://www.youtube.com/channel/UCMtZ0sc1HHNtGGWZFDRTh5A?view_as=subscriber). The [EBA](/job-families/people-group/executive-business-administrator/) for Engineering is the [moderator](/handbook/company/group-conversations/#moderator) and responsible for initiating the Public Stream or designating another moderator if EBA is unable to attend.

## Use Group Labels and Group Milestones

When working in Example Company (and in particular, the Example Company.org group), use group labels and group milestones as much as you can. It is easier to plan issues and merge requests at the group level, and exposes ideas across projects more naturally. If you have a project label, you can promote it to a group milestone. This will merge all project labels with the same name into the one group label. The same is true for promoting group milestones.
...
UI polish issues are visual improvements to the existing user interface, touching mainly aesthetic aspects of the UI that are guided by [Pajamas](https://design.example_company.com/) foundations. UI polish issues generally capture improvements related to color, typography, iconography, and spacing. We apply the `UI polish` label to these issues. UI polish issues don't introduce functionality or behavior changes to a feature.

### Examples of UI polish

- **Aesthetic improvements** ([example](https://example_company.com/example_company-org/example_company/-/issues/290262)): removing unnecessary borders from a UI, updating the background color of an element, fixing the font size of a heading element.
- **Misalignment of text, buttons, etc** ([example](https://example_company.com/example_company-org/example_company/-/issues/280538)): although because many times something isn't broken, these improvements are considered UI polish. These could also be considered a bug.
- **Incorrect spacing between UI elements** ([example](https://example_company.com/example_company-org/example_company/-/issues/7905)): when two interface elements are using inconsistent spacing values, such as 10px instead of 8px. It could also be considered technical debt. Note that if two interface elements have zero space between them, its an obvious bug.
- **Visual inconsistencies across different product areas** ([example](https://example_company.com/example_company-org/example_company/-/issues/296948)): visual inconsistencies could occur when we have a series of buttons on a particular view. For example, when 3/4 of them have been migrated to use the Pajamas component, and 1/4 of them are still using a deprecated button, resulting in a visual inconsistency. This is considered a UI polish.
...
- In Scope
  ...
  - Highest priority and severity customer issues based on the [priority and severity definitions](https://example_company.com/example_company-org/example_company-foss/-/blob/master/doc/development/contributing/issue_workflow.md#priority-labels).
- Not In Scope
  - An operational issue of Example Company.com or a self managed customer environment. This falls under the [on-call](/handbook/engineering/on-call/index.html) process.
  - Self developed and maintained tools that are not officially supported products by Example Company.
  - Feature request by a specific customer.

### Process

1. Person requesting Rapid Engineering Response creates an issue supplying all known information and applies [priority and severity](https://example_company.com/example_company-org/example_company-foss/-/blob/master/doc/development/contributing/issue_workflow.md#priority-labels) (or [security severity and priority](/handbook/security/#severity-and-priority-labels-on-security-issues)) to the best of their ability.
...
A high impact issue has a direct measurable impact on Example Company.com [service levels or error budgets](/handbook/engineering/infrastructure/library/service-levels-error-budgets/).

### Scope

The [Performance Refinement issue board](https://example_company.com/groups/example_company-org/-/boards/1233204) is reviewed in this refinement exercise.
...
1. To nominate issues to the board:
   ...
   1. Ensure that the issue clearly explains the problem, the (potential) impact on Example Company.com's availability, and ideally, clearly defines a proposed solution to the problem.
...
The [infradev issue board](https://example_company.com/groups/example_company-org/-/boards/1193197?label_name[]=infradev) is the primary focus of this process.
...
Issues are nominated to the board through the inclusion of the label `infradev` and will appear on the [infradev board](https://example_company.com/groups/example_company-org/-/boards/1193197?label_name[]=infradev).
...
`~infradev` issues requiring a ~"breaking change" should not exist.  If a current `~infradev` issue requires a breaking change then it should split into two issues.  The first issue should be the immediate `~infradev` work that can be done under current SLOs.  The second issue should be  ~"breaking change" work that needs to be completed at the next major release in accordance with [handbook guidance](/handbook/product/example_company-the-product/#breaking-changes-deprecations-and-removing-features). Agreement from development DRI as well as the infrastructure DRI should be documented on the issue.
...
1. **Use the [InfraDev issue template](https://example_company.com/example_company-org/example_company/-/issues/new?issuable_template=InfraDev) to create the issue on the example_company-org/example_company issue tracker.**
1. **Clearly state the scope of the problem, and how it affects Example Company.com**. Examples could include:
   1. Reliability issues: the problem could cause a widespread outage or degradation on Example Company.com. [example](https://example_company.com/example_company-org/example_company/-/issues/244865)
   1. Saturation issues: the problem could leave to increased saturation, latency issues due to resource over-utilization. [example](https://example_company.com/example_company-org/charts/example_company/-/issues/2458)
   1. Service-level degradation: the problem is causing our service-level monitoring to degrade, impacting the overall SLA of Example Company.com and potentially leaving to SLA violations. [example](https://example_company.com/example_company-org/example_company/-/issues/292732)
   1. Unnecessary alerts: the problem does not have a major impact on users, but is leading to extraneous alerts, impacting the ability of SREs to effectively triage incidents due to alerting noise. [example](https://example_company.com/example_company-org/example_company/-/issues/292732)
   1. Problems which extend the time to diagnosis of incidents: for example, issues which degrade the observability of Example Company.com, swallow user-impacting errors or logs, etc. These could lead to incidents taking much longer to clear, and impacting availability. [example](https://example_company.com/example_company-com/gl-infra/infrastructure/-/issues/10933#note_464394760)
   1. Deficiencies in our public APIs which lead to customers compensating by generating substantially more traffic to get the required results. [example](https://example_company.com/example_company-org/example_company/-/issues/232887)
1. **Quantify the effect of the problem** to help ensure that correct prioritization occurs.
   1. Include costs to availability. The [Incident Budget Explorer](https://dashboards.example_company.net/d/general-incident-budget-explorer/general-incident-budget-explorer?orgId=1) dashboard can help here.
...
1. **Cross-reference links to Production Incidents, PagerDuty Alerts, Slack Alerts and Slack Discussions**. To help ensure that the team performing the triage have all the available data.
   1. By adding "Related" links on the infradev issue, the [Infradev Status Report](https://example_company.com/example_company-org/infradev-reports/-/issues) will display a count of the number of production incidents related to each infradev issue, for easier and clearer prioritization.
...
1. **Provide a method for validating that the original issue still exists**
   ...
   1. Ideally, provide a link to a Thanos query or an ELK query and clear instructions on how to interpret the results to determine whether the problem is still occurring. [Check the "Verfication" section in this issue as an example of this](https://example_company.com/example_company-org/gitaly/-/issues/3670#verification).
