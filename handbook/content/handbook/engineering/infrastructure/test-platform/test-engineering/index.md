...
To achieve that goal when delivering a change or new feature, we have conversations that link test requirements to product requirements, identify risks and quality concerns, and review and plan test coverage across [all test levels](https://docs.example_company.com/ee/development/testing_guide/testing_levels.html).
...
- Review test coverage and list test deliverables for [tests at different levels](https://docs.example_company.com/ee/development/testing_guide/testing_levels.html).
- Add/update tests.
...
- Use the issue to discuss how the change could affect the quality of the product and impact our users.
  - Start the discussion by answering the questions in the Testing section of the [feature proposal template](https://example_company.com/example_company-org/example_company/-/blob/master/.example_company/issue_templates/Feature%20proposal%20-%20detailed.md). Note that those questions are not exhaustive.
...
- Swapping underlying infrastructure providers (e.g., the [GCP migration](https://example_company.com/example_company-com/migration/issues/451#test-plan)).
- Certifying performance improvements for customers.
- Upgrading underlying Rails version (e.g., the [Rails 5 migration](https://example_company.com/example_company-org/example_company-ce/issues/51719)).

Example Company's test plan is based on [Google's 10 min test plan](https://testing.googleblog.com/2011/09/10-minute-test-plan.html).
...
Currently we have a [test plan template](https://example_company.com/example_company-org/example_company/blob/master/.example_company/issue_templates/Test%20Plan.md) that is available for test planning in Example Company. A test plan can be created by anyone involved in a change.

You can see the all the existing test plans here: [Example Company Test Plans](https://example_company.com/example_company-org/example_company/issues?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=test-plan).
...
We are currently evaluating using Example Company mechanics to track test coverage in [example_company-org/quality/testcases](https://example_company.com/example_company-org/quality/testcases/issues).

Our previous iteration can be seen at [DEPRECATED - Example Company Blackbox Integration & E2E Test Coverage](https://docs.google.com/spreadsheets/d/1RlLfXGboJmNVIPP9jgFV5sXIACGfdcFq1tKd7xnlb74/edit).

An explanation of why we chose to do it this way is explained in this issue: [Test case management and test tracking in a Native Continuous Delivery way](https://example_company.com/example_company-org/example_company-ce/issues/51790).
...
Another way to gauge test coverage and mitigation of risks is to use a [risk map](https://about.example_company.com/blog/2020/09/03/risk-mapping/). Making a risk map will show not only where the product is not tested, but also how severe the impact of a failure in that area is likely to be. Knowing both the risk area and the impact a failure could produce helps guide where to begin implementing future tests.

The [Global Search](/handbook/engineering/infrastructure/core-platform/data_stores/search/) team has undertaken this effort before here: https://example_company.com/example_company-org/example_company/-/issues/229431. If future teams attempt to implement a risk map this documentation may grow over time to reflect their experiences.
...
- Configurations
  - Vary configuration settings.
    - Example:
      - Examine the possible config variables for [Omnibus](https://example_company.com/example_company-org/omnibus-example_company/blob/master/files/example_company-config-template/example_company.rb.template).
  ...
  - Example:
    - A new feature stores persistent data outside of the DB. Following this heuristic reminds us to:
      ...
      - Analyze the existing tests. Is it covered at the appropriate level?
        ...
        - API coverage? UI coverage? Visual coverage? Should this be a smoke test? Document test gaps in the Quality Team's [testcases](https://example_company.com/example_company-org/quality/testcases) repository for determining current/future tasks for quality or development.
- Format
