...
- The Saturday before the new rotation begins, the handover bot will create an issue in the [pipeline-triage](https://example_company.com/example_company-org/quality/pipeline-triage/-/issues/) project.
...
- Reporting and analyzing the End to End test failures in [Production](https://ops.example_company.net/example_company-org/quality/production/pipelines), [Canary](https://ops.example_company.net/example_company-org/quality/canary/pipelines), and  [Staging](https://ops.example_company.net/example_company-org/quality/staging/pipelines) pipelines takes priority over [Example Company `master`](https://example_company.com/example_company-org/example_company/pipelines), and [Example Company FOSS `master`](https://example_company.com/example_company-org/example_company-foss/pipelines). Note this also takes priority over fixing tests.
- [Preprod](https://ops.example_company.net/example_company-org/quality/preprod/-/pipelines) pipelines have equal priority with `Production` and `Staging` pipelines during release candidate testing from the Monday through Thursday of the release week.
- If there is a time constraint, the DRI should report and analyze the failures in [Staging](https://ops.example_company.net/example_company-org/quality/staging/pipelines), [Canary](https://ops.example_company.net/example_company-org/quality/canary/pipelines) and [Production](https://ops.example_company.net/example_company-org/quality/production/pipelines) pipelines just enough to determine if it is an application or an infrastructure problem, and [escalate as appropriate](/handbook/engineering/development/processes/infra-dev-escalation/process/). All the reported failures in those pipelines should be treated as ~priority::1/~severity::1 until it's determined that they're not. That means they should be investigated ASAP, ideally within 2 hours of the report. If the DRI will not be able to do so, they should delegate any investigation they're unable to complete to the DRI from the following week. If neither DRI is available or will be able to complete the investigations, solicit help in the #test-platform slack channel.
...
- Cross-cutting issues such as https://example_company.com/example_company-org/quality/team-tasks/-/issues/530 are triaged by the on call DRI to determine the next action. Other team-members should notify the DRI if they come across such an issue. The DRI can inform the rest of the department via the #test-platform channel, if necessary.
- Everyone in the Test Platform sub-department should support the on-call DRI and be available to jump on a zoom call or offer help if needed.
  - If a change needs to be made to CI/CD variables in [example_company-org/example_company](https://example_company.com/example_company-org/example_company/-/settings/ci_cd) and the DRI does not have access, they can ask for help in the `#test-platform-maintainers` and `#development` Slack channels. Any Quality maintainer should have sufficient access to be able to assist.
- Every day the APAC/EMEA/AMER DRI summarizes the failures found for that day in the triage issue like [this](https://example_company.com/example_company-org/quality/pipeline-triage/-/issues/45#note_408358615). This serves as an asynchronous handoff to the DRI in the other timezone.
- The DRI can make use of the [dri gem](https://example_company.com/example_company-org/ruby/gems/dri) to automate reporting handoff and surface useful triage information, such as newly created failures or feature flags enabled by environment.
...
To view the pipeline triage rotation schedule, please visit the [pipeline-triage](https://example_company.com/example_company-org/quality/pipeline-triage#dri-weekly-rotation-schedule) project.
...
- The DRI reviews any [automated quarantine MRs](https://example_company.com/example_company-org/example_company/-/merge_requests?scope=all&state=opened&label_name[]=QA&search=%5BQUARANTINE%5D) and [automated dequarantine MRs](https://example_company.com/example_company-org/example_company/-/merge_requests?scope=all&state=opened&label_name[]=QA&search=%5BDEQUARANTINE%5D) and decides whether to merge or close.
...
- The DRI should periodically take a look at [the list of unassigned quarantined issues](https://example_company.com/example_company-org/example_company/issues?state=opened&label_name%5B%5D=QA&label_name[]=type::bug) and work on them.
...
NB - For assistance with supporting upgrades outside Example Company's regular business hours, please reach out to Slack channel `#infrastructure-managers`.

Some examples of issues where Test Platform have provided support include:

- [CI Decomposition Rollout](https://ops.example_company.net/example_company-com/gl-infra/db-migration/-/blob/ae6240c4bdf94a7774f9ad844dcec26f936a2946/.example_company/issue_templates/ci_decomposition.md)
- [PostgreSQL 14 upgrade](https://ops.example_company.net/example_company-com/gl-infra/db-migration/-/blob/ae6240c4bdf94a7774f9ad844dcec26f936a2946/.example_company/issue_templates/pg14_upgrade.md)
...
Test Platform Sub-Department's on-call does not include work outside Example Company's normal business hours. Weekends and [Family and Friends Days](/handbook/company/family-and-friends-day/) are excluded as well.
