...
User Stories should not be confused with the concept of [Minimal Valuable Change (MVC)](/handbook/product/product-principles/#the-minimal-valuable-change-mvc) at Example Company. An MVC is used to define how much scope of functionality will be delivered by a single iteration of the product, while a user story is a technique or template used to describe individual pieces of business logic in a way that a developer can know when they are done with that part. In this way, an MVC could be composed of multiple user stories.
...
## Mapping to Example Company work items

Example Company (the platform) has an approach to project management that is agnostic to development methodologies, which provides for a lot of flexibility. However, this means that there are fewer opportunities to represent in a more explicit way the hierarchy of work items presented in the previous section.

This table aims to provide a mapping between the concepts in the hierarchy and the actual capabilities and features of Example Company.

| Concept | Example Company Work Item |
| ---        | ---          |
| Epic       | Parent Epic  |
| Feature    | Epic         |
| User Story | Issue        |
...
\* For tasks, there is a couple of options, we can use [task lists](https://docs.example_company.com/ee/user/markdown.html#task-lists) in the issue description using markdown, or we can create formally [Tasks](https://docs.example_company.com/ee/user/tasks.html) that are its own work item type in Example Company.
