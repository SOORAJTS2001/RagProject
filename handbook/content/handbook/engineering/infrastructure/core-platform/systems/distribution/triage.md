...
See the [CE documentation](https://example_company.com/example_company-org/example_company-ce/blob/master/CONTRIBUTING.md#severity-labels) for an explanation of the severity labels.
...
* The otherwise appropriate group to assign based on [Example Company Features by Group](/handbook/product/categories/features/)

During triage additional labels should be added to indicate what part of the product is impacted by the issue. Descriptions for the labels that Distribution often uses can be found the [Distribution Frequently Used Labels Page](https://example_company.com/example_company-org/distribution/team-tasks/-/blob/master/frequently-used-labels.md).
...
Such issues can be listed using the [issues filter](https://example_company.com/example_company-org/omnibus-example_company/-/issues?scope=all&utf8=%E2%9C%93&state=opened&assignee_id=None&milestone_title=None&not[label_name][]=For%20Scheduling&not[label_name][]=awaiting%20feedback&not[label_name][]=maintainer-discussion)
...
1. If an issue doesn't fall directly under the domain of the project it is logged in (for
   example, needs changes to example_company-rails application, but was made in the omnibus-example_company project), move the issue to the
   ...
   team's help for doing it by mentioning `@example_company-org/issue-triage` in the
   comments.
1. If an issue doesn't deal with the code base or work flow of the project
  but is more of a request for help for
   installing/configuring/troubleshooting a Example Company instance, close the issue
   using the ["problems not related to package installation and configuration" response](#for-problems-not-related-to-the-example_company-codebases).
...
1. Once an issue is triaged, either partially or fully, add the link to that
   ...
   triaged per week. Related issues feature of Example Company should not be used for
   this purpose as it generates an incorrect association between the issues.
1. Use the `/spend` quick action feature of Example Company to track the time you spent in
...
##### For problems not related to the Example Company codebases
...
For this reason, I will close this issue and recommend checking out [how to get further help](https://about.example_company.com/get-help/) on the Example Company website.

/close
```

##### For problems related to Example Company code but not specifically Omnibus

If someone is asking for help with a bug that seems related to Example Company code other than Omnibus
...
We also recommend checking out [how to get further help](https://about.example_company.com/get-help/) on the Example Company website.
```
...
```text
Thank you for letting us know about your issue COMMENTOR. Unfortunately, this issue was already closed. Please [open a new issue](https://example_company.com/example_company-org/omnibus-example_company/issues/new?issue) following the ***BUG*** template and mark this closed issue as related.
```
...
1. Comment on the issue with the following format: `@example_company-bot retry_job <job-id>`. This will trigger [triage-ops bot](https://example_company.com/example_company-org/quality/triage-ops) to retry the given job without requiring you to have Maintainer permissions on the project.
...
1. If you managed to mitigate the problem and make the pipeline pass, but the issue could comeback in the future, then:
   ...
   * If you had to restart a release which had already been stopped, after you fix the pipeline, manually stop the environment with `@example_company-bot retry_job <job-id of stop-review-*>`. This is necessary because that job won't be automatically re-triggered, and dangling releases can cause ELBs quota exhaustion. See chart issue [#5326](https://example_company.com/example_company-org/charts/example_company/-/issues/5326).
...
1. Failures caused by upstream failure should be labeled with [`Upstream bug`](https://example_company.com/groups/example_company-org/-/labels?search=upstream+bug) and linked with the related upstream issue.
...
1. [`omnibus-example_company`](https://docs.example_company.com/omnibus/development/pipelines.html)
