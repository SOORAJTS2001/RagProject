...
* [The Example Company performance docs](https://docs.example_company.com/ee/development/performance.html)
...
memory use. Before shipping this to everyone on example_company.com, we first [canaried the changes and measured the results](https://example_company.com/example_company-com/gl-infra/infrastructure/-/issues/7455#note_239070865),
...
the design by moving to [batch insertions](https://docs.example_company.com/ee/development/database/insert_into_tables_in_batches.html) instead.
...
[`in_batches` helper](https://docs.example_company.com/ee/development/database/iterating_tables_in_batches.html), but the idea applies to
...
[copy-on-write](https://en.wikipedia.org/wiki/Copy-on-write).) Again, [migrating from Unicorn to Puma](https://docs.example_company.com/ee/administration/operations/puma.html) was an example of this
and resulted in a significant reduction in memory use across Example Company.
...
of the blame feature on Example Company merge request diffs, which could get very slow if there was a lot of Git history to fetch,
...
even be auto-corrected because it's a one-liner. A good example for this is the file I/O API, which offers [faster variants of certain methods](https://docs.example_company.com/ee/development/performance.html#reading-from-files-and-other-data-sources)
