...
1. If the issue is blocking or you need to escalate:
   ...
   - For operational or configuration issues, post in [#g_infra_database_reliability](https://example_company.enterprise.slack.com/archives/C02K0JTKAHJ) (internal)
   - For issues related to the packaged Postgres in Omnibus/Charts, post in [#g_distribution](https://example_company.enterprise.slack.com/archives/C1FCTU4BE) (internal)
...
If the emergency is related to an ongoing incident on example_company.com or Dedicated customer, Follow the [DBRE Escalation Process](/handbook/engineering/infrastructure/core-platform/data_stores/database-reliability/dbre-escalation-process/)

If the emergency is related to a self-managed customer, reach out to the [Distribution Team](../systems/distribution/) in [#g_distribution](https://example_company.enterprise.slack.com/archives/C1FCTU4BE) (internal) who manages self-managed configuration.
...
1. Reach out in the [#database](https://example_company.enterprise.slack.com/archives/C3NBYFJ6N), [#g_database](https://example_company.enterprise.slack.com/archives/CNZ8E900G), and [#g_infra_database_reliability](https://example_company.enterprise.slack.com/archives/C02K0JTKAHJ) channels (internal) for expert help using the `@db-team` (database capability) or `@dbre` (database reliability) group handles.

### 6. Determine Migration Source

The easiest way is using `git`, from the example_company repository, run:
...
1. Look for the corresponding file named `{table_name}.yml` in https://example_company.com/example_company-org/example_company/-/tree/master/db/docs
...
1. If development on-call needs additional database expertise, reach out in [#database](https://example_company.enterprise.slack.com/archives/C3NBYFJ6N)
...
1. File a [request for help issue](https://example_company.com/example_company-com/enablement-sub-department/section-enable-request-for-help/)
1. Reach out in [#database](https://example_company.enterprise.slack.com/archives/C3NBYFJ6N), include a link to the request for help
