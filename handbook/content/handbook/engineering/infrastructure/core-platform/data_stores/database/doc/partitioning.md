...
## Introducing PostgreSQL table partitioning to Example Company's database

This is a working document to discuss how we are going to introduce table partitioning to Example Company.

### Motivation

The PostgreSQL database we run for Example Company.com has grown to over 5 TB in total size as of early 2020. However, the total database size is *not* the driver to introduce partitioning but the size of individual tables is:

![example_company-com-table-sizes](example_company-com-table-sizes.png)

We can see here that there are individual tables larger than 100 GB, some even go up in the terabytes range.

There is one notable table that we can ignore in this discussion: `merge_request_diff_files` which is the largest table and makes up more than 30% of the total database size today. This is being addressed by [externalizing this data](https://example_company.com/example_company-com/gl-infra/infrastructure/issues/7356) and move it into cloud storage.

The other tables shown here is where Example Company's most used features have their data:
...
The [example_company-org group](https://example_company.com/groups/example_company-org/-/issues) has about 135k issues out of a total of 23M issues on Example Company.com.

We can perform a text search on the group's issues and look for all issues with "foobar" in their title or description along with other filters we can apply: [It takes about 8s to just perform the text search](https://example_company.com/groups/example_company-org/-/issues?scope=all&utf8=%E2%9C%93&state=opened&search=foobar) in this group.
...
In this example, table partitioning breaks down the `issues` table into manageable chunks aka partitions: Let's break the table down into 128 partitions using hash partitioning on the top-level namespace. Each partition contains issue records for roughly 1/128 of the total issues. Each top-level namespace, in our example that's `example_company-org`, has their issue data in exactly one partition.
...
With the much smaller trigram index, we can perform the text search in much less time. An additional improvement for this particular problem is to use a [combined GIN index](https://example_company.com/example_company-org/database-team/team-tasks/-/issues/33).

We analyzed the impact of partitioning for group-based issue search ([summary](issue-group-search-partitioning.html), [issue](https://example_company.com/example_company-org/example_company/-/issues/201871)).

### The Example Company database schema: Partitioning keys

This is a simplified view of a part of Example Company's database schema: Largest tables on Example Company.com along with a few of their references. The color indicates how large the table roughly is: Deep red (> 200 GB), orange (> 100 GB), yellow (> 60 GB), blue (else). For detailed sizes please refer to the image at the top of the doc.

![example_company-model-simplified](example_company-model-simplified.png)
...
As such, we can consider a project or a namespace as a partitioning key. While most of Example Company's features happen inside a project, there are types of searches that are scoped on a namespace. In our example above, the top-level namespace is the search scope. Hence, the top-level namespace is likely a good partitioning key in general.
...
We analyzed how issues distribute across partitions using a hash-based partitioning strategy (by top-level namespace). This is how the number of issues distributes across a set of 128 partitions (find more graphs for [64](https://example_company.com/abrandl/example_company-issue-partitioning/-/blob/master/issues_with_64_partitions.png), [256](https://example_company.com/abrandl/example_company-issue-partitioning/-/blob/master/issues_with_256_partitions.png) and [512](https://example_company.com/abrandl/example_company-issue-partitioning/-/blob/master/issues_with_512_partitions.png) partitions). The red line indicates the desired optimal distribution (1/128 per partition).
...
Issue: [Use structure.sql instead of schema.rb](https://example_company.com/example_company-org/example_company/issues/29465)
...
* [Referential integrity without foreign keys](https://example_company.com/example_company-org/example_company/issues/201873)
* [Cascading deletes without foreign keys](https://example_company.com/example_company-org/example_company/issues/201872)
...
Issue: [Migration strategy for existing tables](https://example_company.com/example_company-org/example_company/issues/202618)
...
Often, we can derive the partition key from the URL already: Many queries that happen for requests inside the `example_company-org` namespace likely should have the top-level namespace as a filter. We might consider using [activerecord-multi-tenant](https://github.com/citusdata/activerecord-multi-tenant) (by CitusData), which has an automatic approach to detect the partitioning key and make sure queries contain the appropriate filter by partitioning key for relevant models.

Issue: [Explore usage of activerecord-multi-tenant to enforce partitioning key in queries](https://example_company.com/example_company-org/example_company/issues/207397)

Results:

1. [Implementation example using the gem](https://example_company.com/example_company-org/example_company/-/commit/653752e94b905d2f6067bd5b5bc18f13c5550a46)
