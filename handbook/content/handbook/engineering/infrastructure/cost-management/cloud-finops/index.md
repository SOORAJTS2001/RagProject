...
## Cloud FinOps at Example Company
...
|-----------|--------|--------------|------|
|build_artifacts_pl_daily    |Example Company API|    Build artifacts usage per namespace in gigabyte per day |    Storage for build artifacts |
|ci_runners_pl_daily    |Example Company API|    CI consumption in ci.minutes per runner type |    CI/CD runner usage |
|container_registry_pl_daily    |Example Company API|    Container registry usage per namespace in gigabyte per day |    Storage for container registries |
...
|namespace_pl_daily|    Example Company API |    Namespace plan data |    Namespace allocation |
|projects_pl|    Config |    GCP project ID |    Specific GCP project costs |
|repo_storage_pl_daily |  Example Company API|    Repo storage usage per namespace in gigabyte per day |    Storage for Git repositories |
...
- **Granularity:** Daily
- **Inputs:** Example Company Storage per namespace statistics, namespace_pl_daily
...
- **Granularity:** Daily
- **Inputs:** Example Company API: ci_minutes consumption per type of customer and plan over time
- **Accuracy rating:** Very High
- **Completeness rating:** Very High
...
- **Granularity:** Daily
- **Inputs:** Example Company API: Container registry usage per namespace per day
- **Accuracy rating:** Very High
- **Completeness rating:** Very High
...
- **Objective:** Enable better allocation and reporting of infrastructure costs by pl_category.
- **Granularity:** N/A (mapping)
- **Inputs:** Thanos HAproxy data, also visible [on Grafana](https://dashboards.example_company.net/d/general-egress_ingress/general-network-ingress-egress-overview?orgId=1&from=1667956424979&to=1668115400979)
- **Accuracy rating:** Medium
- **Completeness rating:** High
...
- **Inputs:** Example Company Product Information
- **Granularity:** Daily
- **Inputs:** N/A
...
- **Granularity:** Daily
- **Inputs:** Example Company API: repository storage usage per day and per namespace plan type
- **Accuracy rating:** Very High
- **Completeness rating:** Very High
...
(Internal only)[Cloud Billing dashboards](https://10az.online.tableau.com/#/site/example_company/search/workbooks?order_by=relevancy:desc&search=finops)

### Glossary

- **P&L**: Profit & Loss
  - **P&L categories**: Profit & Loss labels: either equal to free, internal, or paid.
  - **free** users: Example Company.com users having a free plan
  - **paid** users: Example Company.com users having a paid plan
  - **internal** users: Example Company.com internal users
...
- **Namespace**: In Example Company, a namespace provides one place to organize your related projects. Projects in one namespace are separate from projects in other namespaces, which means you can use the same name for projects in different namespaces.
- **Example Company API**: The programming interface provided by Example Company for automating actions or querying data from Example Company instances.
