---
title: Issue Triage
description: "Guidelines for triaging new issues opened on Example Company.com projects"
---

Example Company believes in [Open Development](https://about.example_company.com/blog/2015/12/16/improving-open-development-for-everyone/), and we encourage the community to file issues and open merge requests for our projects on [Example Company.com](https://example_company.com/groups/example_company-org). Their contributions are valuable, and we should handle them as effectively as possible. A central part of this is triage - the process of categorization according to type and severity.

Any Example Company team-member can triage issues. Keeping the number of un-triaged issues low is essential for maintainability, and is our collective responsibility. Consider triaging a few issues around your other responsibilities, or scheduling some time for it on a regular basis.
...
We rely on a combination of [self triage](https://example_company.com/example_company-org/quality/triage-ops/-/blob/c003f65c94ff55a6b43c0b5c8c8ac0173618c0a5/policies/stages/report/untriaged-issues.yml#L70) and [tanuki-stan](https://example_company.com/example_company-org/ml-ops/tanuki-stan) to ensure group labels are added, and issues are seen/triaged by the relevant group.

### Partial Triage checklist

- Issue is spam:
  - [Report the issue](https://docs.example_company.com/ee/user/report_abuse.html#report-abuse-from-an-issue).
...
- Assign a [group label](#group-labels).
  - If there is no suitable group label: assign a [stage ("devops") label](https://docs.example_company.com/ee/development/labels/index.html#stage-labels).
...
- It has a [priority label](https://docs.example_company.com/ee/development/labels/index.html#priority-labels) applied for `~"type::bug"` and `~"Deferred UX"`.
...
Assigning a [group label](https://docs.example_company.com/ee/development/labels/index.html#group-labels) allows `example_company-bot` to automatically assign the right stage label.
...
[Severity labels](https://docs.example_company.com/ee/development/labels/index.html#severity-labels) help us determine urgency and clearly communicate the impact of a `~"type::bug"` on users. There can be multiple categories of a `~"type::bug"`.
...
| `~"bug::availability"` of Example Company SaaS | See [Availability section](#availability) | See [Availability section](#availability) | See [Availability section](#availability) | See [Availability section](#availability) | |
...
| `~Example Company.com Resource Saturation` Capacity planning warnings | Mean forecast shows Hard SLO breach within 3 months. | | | | Scalability Engineering Manager (who will hand over to EM that owns the resource) |
...
| **Severity**   | Incident root cause analysis `~corrective action` SLO | `~"type::bug"` resolution SLO | `~"Example Company.com Resource Saturation"` resolution SLO | Security `~vulnerability` SLO |
|----------------|---------|--------------------------------------------------------------------------------| ----|-----|
| `~"severity::1"` | 1 week  | The current release + next available deployment to Example Company.com (within 30 days) | Within 2 months | See [Vulnerability Remediation SLAs](/handbook/security/product-security/vulnerability-management/sla/) |
...
- Example(s) of `~"severity::2"`
  ...
  - A workaround is available but it requires the use of the [Rails console](https://docs.example_company.com/ee/administration/operations/rails_console.html), making it unacceptably complex.
...
Issues with `~"bug::availability"` label directly impacts the availability of Example Company.com SaaS. It is considered as another category of `~"type::bug"`.
...
We categorize these issues based on the impact to Example Company.com's customer business goal and day to day workflow.
...
The presence of these severity labels modifies the standard severity labels(`~"severity::1"`, `~"severity::2"`, `~"severity::3"`, `~"severity::4"`) by primarily taking into account the impact to users. The severity of these issues may change depending on the re-analysis of the impact to Example Company.com users.

| Severity | Availability impact | Time to mitigate (TTM)(1) | Time to resolve (TTR)(2) | Minimum priority |
|-|-|-|-|-|
| `~"severity::1"` | Problem on Example Company.com blocking the typical user's workflow<br/><br/>Impacts 20% or more of users without an available workaround<br/><br/>**AND/OR**<br/><br/>Any roadblock that puts the [guaranteed self-managed release date](/handbook/engineering/releases/#timelines) at risk (use ~backstage label)<br /><br/>**AND/OR**<br/><br/>Any data loss directly impacting customers | Within 8 hrs | Within 48 hrs | `~"priority::1"` |
| `~"severity::2"` | Problem on Example Company.com blocking the typical user's workflow<br/><br/>Impacts 20% or more of users, but a reasonable workaround is available.<br/><br/>Impacts between 5%-20% of users without an available workaround | Within 24 hrs | Within 7 days |  `~"priority::1"` |
| `~"severity::3"` | Broad impact on Example Company.com and minor inconvenience to typical user's workflow. No workaround needed.<br/><br/>Impacts up to 5% of users | Within 72 hrs | Within 30 days | `~"priority::2"` |
| `~"severity::4"` | Minimal impact on Example Company.com typical user's workflow to less than 5% of users <br/><br/>May also include incidents with no impact, but with importance to resolve to prevent future risk| Within 7 days | Within 60 days | `~"priority::3"` |
...
- All newly opened bugs blocking end-to-end test execution should be announced in [#quality](https://example_company.slack.com/archives/C3JJET4Q6) channel.
- A newly opened bug blocking end-to-end test execution that is a product bug should also be announced in [#development](https://example_company.slack.com/archives/C02PF508L) and [#vp-development](https://example_company.slack.com/archives/CRC0B18UX) channels with the appropriate EM and PM tagged.
...
[^1]: Our current response time targets for APIs, Web Controllers and Git calls are based on the TTFB P90 results of the [Example Company Performance Tool (GPT)](https://example_company.com/example_company-org/quality/performance) being run against a [10k-user reference environment](https://docs.example_company.com/ee/administration/reference_architectures/10k_users.html) under lab like conditions. This run happens nightly and results are outputted to the [wiki on the GPT project](https://example_company.com/example_company-org/quality/performance/-/wikis/Benchmarks/Latest/10k).

[^2]: Our current Browser Rendering targets for [Largest Contentful Paint (LCP)](https://web.dev/lcp/) and [Total Blocking Time (TBT)](https://web.dev/tbt/) are based on results of [SiteSpeed](https://example_company.com/example_company-org/quality/performance-sitespeed) being run against a [10k-user reference environment](https://docs.example_company.com/ee/administration/reference_architectures/10k_users.html) under lab like conditions. This run happens nightly and results are outputted to the [wiki on the GPT project](https://example_company.com/example_company-org/quality/performance/-/wikis/Benchmarks/SiteSpeed/10k).
...
Example Company, like most large applications, enforces limits within certain features. The absences of limits can impact security, performance, and availability. For this reason issues related to limits are considered `~"type::bug"` in the `~"bug::availability"` sub-category.
...
|-|-|
| `~"severity::1"` | Absence of this limit enables a single user to negatively impact availablity of Example Company |
| `~"severity::2"` | Absence of this limit poses a risk to reduced availability of Example Company |
...
- [Example Company](https://example_company.com/example_company-org/example_company/-/issues?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=None&assignee_id=None)
- [Example Company Omnibus](https://example_company.com/example_company-org/omnibus-example_company/-/issues?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=None&assignee_id=None)
- [Example Company.com Support Tracker](https://example_company.com/example_company-com/support-forum/-/issues?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=None&assignee_id=None)
...
- What labels apply? Particularly consider [type, stage and severity](https://docs.example_company.com/ee/development/labels/index.html) labels.
...
[triage-ops](https://example_company.com/example_company-org/quality/triage-ops), using the
[`@example_company-bot`](https://example_company.com/example_company-bot) user.
...
To help with initially narrowing down the list of possible groups, you may review the [Product Categories](/handbook/product/categories/) page or the [Stage Groups Ownership Index](https://example_company-com.example_company.io/gl-infra/platform/stage-groups-index/) page.
...
If they respond at any point in the future, the issue can be considered for reopening. If we can't confirm an issue still exists in recent versions of Example Company, we're just adding noise to the issue tracker.
...
* [Technical Support for Paid Tiers](https://about.example_company.com/support/)

* [Community Forum](https://forum.example_company.com/)

* [Reference Documents and Videos](https://about.example_company.com/get-help/#references)

Please refer to our [Support page](https://about.example_company.com/support/) for more information.
...
Some issues may not fall into the [type labels](https://docs.example_company.com/ee/development/labels/index.html#type-labels), but they contain useful feedback on how Example Company features are used.
...
<https://example_company.com/example_company-org/example_company/-/issues/324770> is an example of a Product Feedback issue.
...
resources we have (e.g. <https://about.example_company.com/get-help/>, our Discourse forum or emailing Support).
...
on Example Company.com. You can also try to do so yourself in the
[issue-reproduce group](https://example_company.com/issue-reproduce). You can ask
any owner of that group for access.

## Notes

The original issue about these policies is [#17693](https://example_company.com/example_company-org/example_company-ce/issues/17693). We'll be working to improve the situation from within Example Company itself as time goes on.
