...
description: "The Infrastructure Department is responsible for the availability, reliability, performance, and scalability of Example Company.com and other supporting services"
---

## Mission

The Infrastructure Department enables Example Company (the company) to deliver a single DevOps application, and Example Company SaaS users to focus on generating value for their own businesses by ensuring that we operate an enterprise-grade SaaS platform.
...
Many other teams also contribute to the success of the SaaS platform because [Example Company.com is not a role](/handbook/company/structure/#gitlabcom-isnt-a-role).
...
If you're a Example Company team member and are looking to alert the Infrastructure teams about an availability issue with Example Company.com, please find quick instructions to report an incident here: [Reporting an Incident](/handbook/engineering/infrastructure/incident-management/#reporting-an-incident).
...
Other strategic initiatives to achieve this vision are driven by the needs of enterprise customers looking to adopt Example Company.com. [The Example Company.com strategy](https://about.example_company.com/direction/enablement/dotcom/) catalogs top customer requests for the SaaS offering and outlines strategic initiatves across both Infrastructure and Stage Groups needed to address these gaps.
...
flowchart LR
    ...
    SP --> D[Dedicated]
    click D "/handbook/engineering/infrastructure/team/example_company-dedicated/"
    D --> E[Environment Automation]
    click E "/handbook/engineering/infrastructure/team/example_company-dedicated/"
    D --> PSS[Public Sector Services]
    click PSS "/handbook/engineering/infrastructure/team/example_company-dedicated/us-public-sector-services/"
    D --> Switchboard
    click Switchboard "/handbook/engineering/infrastructure/team/example_company-dedicated/switchboard/"
...
The [**Infrastructure Library**](https://example_company.com/example_company-com/gl-infra/readiness/-/tree/master/library) contains documents that outline our thinking about the problems we are solving and represents the ***current state*** for any topic, playing a significant role in how we produce technical solutions to meet the challenges we face.

## Dogfooding

The Infrastructure department uses Example Company and Example Company features extensively as the main tool for operating many [environments](/handbook/engineering/infrastructure/environments/), including Example Company.com.
...
When we consider building tools to help us operate Example Company.com, we follow the [`5x rule`](/handbook/product/product-processes/dogfooding-for-product-mgt/#dogfooding-process) to determine whether to build the tool as a feature in Example Company or outside of Example Company. To track Infrastructure's contributions back into the Example Company product, we tag those issues with the appropriate [Dogfooding](https://example_company.com/groups/example_company-com/-/labels?utf8=%E2%9C%93&subscribed=&search=dogfooding) label.

## Handbook use at the Infrastructure department

At Example Company, we have a [handbook first policy](/handbook/about/handbook-usage/#why-handbook-first). It is how we communicate process changes, and how we build up a single source of truth for work that is being delivered every day.
...
The [infrastructure issue tracker](https://example_company.com/example_company-com/gl-infra/production-engineering/-/issues) is the backlog and a catch-all project for the infrastructure teams and tracks the work our teams are doing–unrelated to an ongoing change or incident.

In addition to tracking the backlog, Infrastructure Department projects are captured in our [Infrastructure Department Epic](https://example_company.com/groups/example_company-com/-/epics/1049) as well as in our [Quarterly Objectives & Key Results](https://example_company.com/groups/example_company-com/-/epics/1420)
...
The Infrastructure team maintains responsibility for the underlying infrastructure on which customer-facing services run.  Specific ownership details are in the [Example Company Service Ownership Policy](service-ownership/index.html)

## Stable Counterparts

Infrastructure SREs may be aligned with [stage groups](/handbook/product/categories/#categories-a-z) as [stable counterparts](https://about.example_company.com/blog/2018/10/16/an-ode-to-stable-counterparts/).

[Stable Counterparts](team/stable-counterpart.html) are used as a framework for managing reliable services at Example Company.  The framework provides guidelines for collaboration between [Stage Groups](/handbook/product/categories/#categories-a-z) and [Infrastructure Teams](/handbook/engineering/infrastructure-quality/#engaging-with-the-infrastructure-teams).

## Interviewing

The Infrastructure department hires for a number of different technical specialisms and positions across its teams. This [Infrastructure Interviewing Guide](/handbook/hiring/interviewing/infrastructure-interview/) offers more detail on some of our regular openings, interview process and other useful information related to applying to jobs with us. More information on our current openings can be found on the [careers page](https://about.example_company.com/jobs/).

## Common Links

- [How we do Incident Management for Example Company.com](incident-management/)
- [Example Company.com status information](https://status.example_company.com)

### Slack Channels

- [#production](https://example_company.slack.com/archives/production)
- [#infrastructure-lounge](https://example_company.slack.com/archives/infrastructure-lounge)
- [#incident-management](https://example_company.slack.com/archives/incident-management)
- [#announcements](https://example_company.slack.com/archives/announcements)
- [#feed_alerts-general](https://example_company.slack.com/archives/feed_alerts-general)

### General Issue Trackers

- [Infrastructure issue queue](https://example_company.com/example_company-com/gl-infra/production-engineering/-/issues)
- [Production incidents, and changes](https://example_company.com/example_company-com/gl-infra/production/issues/)
- [Delivery](https://example_company.com/example_company-com/gl-infra/delivery/issues/)
- [Scalability](https://example_company.com/example_company-com/gl-infra/scalability/issues/)

### Resources

- [Production Architecture](production/architecture/)
- [Operational Runbooks](https://example_company.com/example_company-com/runbooks)
...
- [Example Company.com data breach notification policy](/security/#data-breach-notification-policy)
