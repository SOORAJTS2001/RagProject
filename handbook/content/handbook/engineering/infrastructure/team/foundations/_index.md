...
The mission of the Production Engineering Foundations team at Example Company is to own the lifecycle of the core infrastructure for Example Company Saas instances and services. We seek to reduce the effort required to provide our core infrastructure services, and to enable other teams to self-serve core infrastructure that allows them to more efficiently and effectively run their services across all Example Company offerings.

## Vision

The Foundations teams North Star consists of two pieces:

1. **Creating a platform that drives vertical ownership of engineering services.** We will do this by providing the rest of engineering the resources and tools needed for them to manage their infrastructure and core services in a way that makes it easy for other teams to adopt the best practices and conventions defined by our team. By moving towards centralized sets of tooling for all Example Company instances and all services to work from, we will create a culture of scalability for the future of Example Company infrastructure.
1. **Sustainable toil.** Toil is a given in SRE work. We will adopt processes and policies that create a balance between what is automated and is done manually so as Example Company grows, the toil involved in running it is maintained at a minimal level for the team.
...
Foundations is responsible for several services related to Example Company SaaS Platforms. The Services that the Foundations team is responsible for fall into two general categories: Core and Edge.

#### Core

Core services are services that are within Example Company.com's ecosystem, generally not talking directly to incoming traffic.

| Service | Description | Co-Ownership? |
| ------- | ----------- | --------- |
| K8s | [K8S workloads deployments](https://example_company.com/example_company-com/runbooks/-/blob/master/docs/onboarding/example_company.com_on_k8s.md), Cluster addons | Autodeploy remains with Delivery, and anything Delivery related is co-owned with Delivery |
| Config | [Terraform](https://example_company.com/example_company-com/runbooks/-/blob/master/docs/uncategorized/terraform-broken-master.md), [Chef](https://example_company.com/example_company-com/runbooks/-/tree/master/docs/config_management), Image Builds | The core TF repos are owned by Foundations, while specific modules may be maintained by the teams that use them |
| Service discovery | [Consul](https://example_company.com/example_company-com/runbooks/-/tree/master/docs/consul) | |
| Secrets Management | [Vault](https://example_company.com/example_company-com/runbooks/-/tree/master/docs/vault) | Vault is offered as a service to enable teams to manage their own secrets |
| Ops | [Ops.example_company.net](https://example_company.com/example_company-com/runbooks/-/tree/master/docs/ops), [Ops Runners](https://example_company.com/example_company-com/runbooks/-/blob/master/docs/uncategorized/subnet-allocations.md) | |
...
| Service | Description | Co-Ownership? |
| ------- | ----------- | --------- |
| CDN | [Cloudflare](https://example_company.com/example_company-com/runbooks/-/tree/master/docs/cloudflare) | |
| DNS | [AWS Route 53](https://example_company.com/example_company-com/runbooks/-/blob/master/docs/uncategorized/manage-dns-entries.md), [Cloudflare](https://example_company.com/example_company-com/runbooks/-/tree/master/docs/cloudflare) | |
| Load Balancing | [HAProxy](https://example_company.com/example_company-com/runbooks/-/blob/master/docs/frontend/haproxy.md), Ingress | |
| Networking | [Cloud VPCs](https://example_company.com/example_company-com/runbooks/-/blob/master/docs/uncategorized/subnet-allocations.md), [Cloudflare](https://example_company.com/example_company-com/runbooks/-/tree/master/docs/cloudflare) | |
| Rate Limiting | [Rate limiting](https://example_company.com/example_company-com/runbooks/-/tree/master/docs/rate-limiting) | shared ownership with development teams for specific endpoints and with abuse |
| RBAC/IAM | [Teleport](https://example_company.com/example_company-com/runbooks/-/tree/master/docs/teleport), GCP IAM permissions and project creation | |

## Getting Assistance

Should you require assistance from the Foundations team, please open an issue in the [Production engineering tracker](https://example_company.com/example_company-com/gl-infra/production-engineering/-/issues/new?issuable_template=request-foundations)

Apply one of the following templates:

- For issues pertaining to Teleport connections, use the [teleport-troubleshooting template](https://example_company.com/example_company-com/gl-infra/production-engineering/-/issues/new?issuable_template=teleport-troubleshooting)
- For issues pertaining to Cloudflare, use the [Cloudflare Troubleshooting template](https://example_company.com/example_company-com/gl-infra/production-engineering/-/issues/new?issuable_template=Cloudflare%20Troubleshooting)
- For anything else, use the [request-foundations template](https://example_company.com/example_company-com/gl-infra/production-engineering/-/issues/new?issuable_template=request-foundations)

- We also have team handles that ping the full team
  - Slack: `@infra-foundations`
  - Example Company: `@example_company-org/production-engineering/foundations`
...
We endeavor to triage incoming requests twice per week. If you have an issue that is very urgent and requires immediate attention (i.e. Priority 1), please also send us a link to the request issue in our Slack channel: [#g_infra_foundations](https://example_company.slack.com/archives/C0313V3L5T6).
...
- We support [dogfooding](/handbook/engineering/development/principles/#dogfooding) when it makes sense to do so.
  - We consider the scale at which we run example_company.com and the additional engineering effort required to dogfood, weighed against the capacity and other priorities of the team.
...
KPIs for the team are currently under development in https://example_company.com/example_company-com/gl-infra/production-engineering/-/issues/24928.
...
For more context, see the related [discussion issue](https://example_company.com/example_company-com/gl-infra/production-engineering/-/issues/19167).
...
- [Foundations Top Level Epic](https://example_company.com/groups/example_company-com/gl-infra/-/epics/1175)
...
- [Foundations OKRs](https://example_company.com/example_company-com/example_company-OKRs/-/issues/?sort=updated_desc&state=opened&label_name%5B%5D=team%3A%3AFoundations&first_page_size=20)
- Example Company Issues capture smaller, concrete pieces of work, and those labeled `~workflow-infra::In Progress` should be updated weekly or whenever a portion of work has been completed.
- [#g_infra_foundations](https://example_company.slack.com/archives/C0313V3L5T6) - work related discussions, external requests, etc
- [#g_infra_foundations_social](https://example_company.slack.com/archives/C04QVEXBVL3) - socializing and standups
- [#g_infra_foundations_notifications](https://example_company.slack.com/archives/C04RZC5TPPD) - automated MR notifications
- [#g_infra_foundations_alerts](https://example_company.enterprise.slack.com/archives/C04Q7RQC7FF) - pipeline failures and service alerts
...
We have Geekbot automated checkins on Mondays and Fridays in the [#g_infra_foundations](https://example_company.enterprise.slack.com/archives/C0313V3L5T6) channel.
...
While we rely on the `workflow-infra::*` labels to denote the current state of any given issue, we rely on the [Build Board](https://example_company.com/groups/example_company-com/gl-infra/-/boards/7578583?label_name%5B%5D=Foundations%3A%3ABuild) to understand what specific issues are ready to be picked up.
...
The goal of the team member on interrupt rotation is to purposely plan to have interrupt work for a week so that the rest of the team can have less interrupt work. The [schedule is available in pager duty](https://example_company.pagerduty.com/schedules#P7Y8O0E).
...
- Checking the [Foundations Build Board](https://example_company.com/groups/example_company-com/gl-infra/-/boards/7578583?label_name%5B%5D=Foundations%3A%3ABuild) throughout the week and ensuring new issues related to Foundations:
  - Have the correct labels (priority, workflow, any other applicable category labels)
  - Have a [weight](#issue-weighting).
  - Are assigned to an [appropriate epic](https://example_company.com/groups/example_company-com/gl-infra/-/epics?state=opened&page=1&sort=start_date_desc&label_name[]=team::Foundations) if applicable.
...
- Responding to alerts in [`#g_infra_foundations_alerts`](https://example_company.enterprise.slack.com/archives/C04Q7RQC7FF).
- Working on Renovate MRs posted in [`#g_infra_foundations_notifications`](https://example_company.enterprise.slack.com/archives/C04RZC5TPPD).
...
- Responding to Terraform drifts related to Foundations services posted in [`#infra-terraform-alerts`](https://example_company.enterprise.slack.com/archives/C06PZQCRUJH).
- Respond to any Access Requests that need Foundations provisioning.

### Project Management

The Foundations team top level Epic can be found [here](https://example_company.com/groups/example_company-com/gl-infra/-/epics/1175)
...
Open planned work for our team is located in the [Production Engineering](https://example_company.com/example_company-com/gl-infra/production-engineering/) project. Issues should be updated whenever significant work occurs. New issues are expected to:
...
We dogfood the [weight feature](https://docs.example_company.com/ee/user/project/issues/issue_weight.html) in issues. We use a modified Fibonacci sequence (the numbers match roughly with the number of days) and align our weights to t-shirt sizes to make them easy to talk about:
