...
description: "The Delivery Group enables Example Company Engineering to deliver features in a safe, scalable and efficient fashion to both Example Company.com, Example Company Dedicated and self-managed customers."
---
...
| **Example Company.com** | `@example_company-org/delivery` |
| **Issue Tracker** | [**Delivery**](https://example_company.com/example_company-com/gl-infra/delivery) |
| **Slack Channels** | [#g_delivery](https://example_company.slack.com/archives/g_delivery) / `@delivery-team` |
...
| Release Manager Runbooks | [release/docs/runbooks](https://example_company.com/example_company-org/release/docs/-/blob/master/runbooks/README.md) |

## Mission

The Delivery Group enables Example Company Engineering to deliver features in a
**safe**, **scalable** and **efficient** fashion to Example Company.com, Example Company Dedicated and self-managed customers.
The group ensures that Example Company's monthly, and patch releases are deployed to Example Company.com, Example Company Dedicated and
...
**performance**, and **scalability** of all of Example Company's user-facing services as well as self-managed
...
These are a set of statements or questions we can use within the group to evaluate whether what we are doing is the right thing to do. When deciding on a piece of work to commit to, we should ask ourselves whether the work aligns with these principles. The principles will be driven by the Delivery Group Strategy, domain experts and Delivery Group Members. These are likely to change slightly over time as we learn and should not be considered static. Additionally, these principles are additive to the [Example Company Values](/handbook/values/) and we aim not to duplicate those here.
...
We've moved this subsection to the [Delivery direction pages](https://about.example_company.com/direction/saas-platforms/delivery/) so that it's in the same place as the rest of our product direction.

## Top-level Responsibilities

The group regularly works on the following tasks, in the order of priority:

1. Ensuring continuous delivery of Example Company application software to Example Company SaaS.(e.g. [Example Company SaaS auto-deploy](https://example_company.com/groups/example_company-org/release/-/epics/13))
1. Coordination and preparation of Example Company releases for self-managed users for the monthly patch and security releases.
1. Participating in incident resolution and acting on corrective actions for SaaS and self-managed software delivery.
1. Increasing velocity of both SaaS and self-managed software delivery through foundational project work (e.g. [Running Example Company SaaS on Kubernetes](https://example_company.com/groups/example_company-com/gl-infra/-/epics/112)).
1. Improving the robustness of SaaS software delivery by creating and improving tooling (e.g. [Deployment rollback](https://example_company.com/groups/example_company-com/gl-infra/-/epics/282)).
1. Minimizing the use of custom tooling by building or enhancing features within Example Company (e.g. [Create a Changelog feature](https://example_company.com/groups/example_company-com/gl-infra/-/epics/351)).
1. Support other teams' needs related to software delivery on Example Company SaaS (e.g. [New Container Registry deployment](https://example_company.com/groups/example_company-com/gl-infra/-/epics/412)).

## Teams

The Delivery Group is composed of two teams: `Delivery:Releases` and `Delivery:Deployments`.

The Delivery:Releases and Delivery:Deployments OKRs, while contributing to the wider Example Company objectives, are tailored and structured to achieve the [Delivery Group Strategy](#strategy) as a single team.

### Delivery:Releases

The primary goal of the Releases team is to provide everything to do with creating Example Company releases for customers and enabling internal customers to get their changes into releases. Major, minor and patch releases as well as a platform on which we can verify changes to various installation types (release environments) and create visibility for Example Company team members and customers into our release schedule (release dashboard).

### Delivery:Deployments

The primary goal of the Deployments team is to provide everything to do with rolling out changes to active platforms managed by Example Company, including but not limited to continuous deployment to Example Company.com (deployment safety), changing example_company.com architecture to support continuous deployment (eliminate staging 🤞 ) and optimizing the zero downtime rollout experience of Example Company Dedicated.
...
is reaching production environment (Example Company.com).
At the moment of writing, the target for this PI is defined in this [key result](https://example_company.com/groups/example_company-com/gl-infra/-/epics/107) epic.
...
Release Managers are members of the Delivery group but during their time as release managers, they're wearing a different hat. The primary customers are Example Company users.
...
- the internal Example Company users who want to deploy & release changes i.e.,
...
1. Release Publishing: Publishing packages to various distribution sites (e.g., packages.example_company.com, Docker Hub, etc.), publishing tooling, and guaranteeing a reliable publishing process.
...
- Example Company SaaS (Example Company.com, Dedicated and Cells) customers expecting deployments of updated code to their various infrastructures.
...
| Any Priority Deployment/Release related issues | `@example_company-org/delivery` | Example Company |
...
Issues should primarily be created in the [delivery issue tracker](https://example_company.com/example_company-com/gl-infra/delivery) for
...
The [Release Velocity](https://example_company.com/groups/example_company-com/gl-infra/-/epics/170) epic tracks all work related to the group mission. Any working epic that the team creates should be directly added as a child to this top level tracking.
...
1. Issue admin section to provide the issue priority, labels and epic for quick actions. [Example](https://example_company.com/groups/example_company-com/gl-infra/-/epics/273#issue-template).
...
1. Labels:
    ...
    - Label used as part of the project scope (eg. `kubernetes`, `security-release`). The DRI should create a suitable project scope label and add it to the [Delivery-triage rules](https://example_company.com/example_company-com/gl-infra/triage-ops) if needed.
...
The canonical issue tracker for the Delivery group is at [gl-infra/delivery](https://example_company.com/example_company-com/gl-infra/delivery). Issues are automatically labeled if no labels are applied using the [triage ops](https://example_company.com/example_company-com/gl-infra/triage-ops) project.
The default labels defined in the [labeling library](https://example_company.com/example_company-com/gl-infra/triage-ops/-/blob/master/lib/delivery/default_labeling.rb).
...
Not every issue will be prioritised for building as soon as it is ready. Instead we manage a [Build board](https://example_company.com/example_company-com/gl-infra/delivery/-/boards/1918862) with all `workflow-infra::In Progress`, and `workflow-infra::Ready` issues focused on the team's current goals.
...
Tomorrow:
  * Fighting with Helm: I gave a [more detailed update on the issue's progress thread](https://example_company.com/example_company-com/gl-infra/delivery/-/issues/19471#note_1473654659)
  * Going for a marathon training in my early morning
```

### Choosing something to work on

The Delivery group generally has working epics assigned to a [DRI](/handbook/people-group/directly-responsible-individuals/) who is responsible for making sure work is broken down into issues, and appropriate issues are moved onto the [Build board](https://example_company.com/example_company-com/gl-infra/delivery/-/boards/1918862) to keep the project on track. However, anyone is welcome to pick up any tasks from the [Build board](https://example_company.com/example_company-com/gl-infra/delivery/-/boards/1918862) regardless of which project it belongs to.
...
1. Follow [Example Company Code Review Guidelines](https://docs.example_company.com/ee/development/code_review.html).
1. We [dogfood the Reviewers feature](https://docs.example_company.com/ee/development/code_review.html#dogfooding-the-reviewers-feature) to assign reviewers to an MR.
...
- Choosing reviewers: Commonly, any team member on Delivery can be assigned for reviews with a few considerations:
  ...
  - Example Company team is globally distributed, so think about timezone when assigning someone as reviewer.
  ...
  - Release manager (or capacity) - If a team member is a [release manager](https://about.example_company.com/community/release-managers/) and they're working on a release task, they should not be bothered with reviews.
...
The Delivery team officially [came into existence on 2018-10-23](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/15348/). This was the culmination of a larger alignment that was happening throughout that year, exposed by the need to streamline releases for self-managed users and creating a better experience for Example Company.com users.

All throughout Example Company's existence, Release Management had been a monthly rotating role served by developers. The idea behind it was to keep developers close to the whole lifecycle of the software they create, and ensure that they automate their work. This worked well until the number of application changes, and developer tasks grew too large for anyone to handle as a secondary task. The event that indicated the need for a change was a near miss event near the end of 2017, when the first Release Candidate was deployed to Example Company.com just 2 days before the 22nd. That whole month was riddled with challenges, from release managers struggling to deliver their day to day development tasks and RM tasks, to multiple unsuccessful deployments to Example Company.com. Most importantly, this was a first indication that the company was growing and that the processes that worked previously, might need to change to accommodate the larger growth that was planned.

After some internal discussions, we entered 2018 with an attempt to [work on process improvements](https://example_company.com/example_company-org/release/tasks/-/issues/39), rather than changing everything in one go. We went from a monthly rotation to two month [release manager rotation](https://about.example_company.com/community/release-managers/), started [noting down spent time](https://example_company.com/example_company-org/release/tasks/-/issues/1). Over the next several months we'd seen general stabilization of the process but it became apparent that spending 4 engineers time in Release Manager rotation was not getting us anywhere closer to improving the deployment process for Example Company.com, and with each developer we hired the task list grew bigger.

The initial discussion on [what is in front of us to achieve Continuous Delivery](https://example_company.com/example_company-com/gl-infra/delivery/-/issues/1) on Example Company.com exposed a clear need for a team focused on this specific task.

With the team created, we set out to work on the [release we designed](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/16028), [change the way we deploy to Example Company.com](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/17842/diffs), [merge Example Company Rails codebases](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/19037/diffs) and many more other tasks.

After a successful [team onsite (aka Fast boot)](https://example_company.com/groups/example_company-org/release/-/epics/17) where we executed on our tasks while being in the same room together for the first time, [we announced the first step towards Continous Delivery on Example Company.com](https://example_company.com/example_company-com/gl-infra/delivery/-/issues/345). This was a very large change that changed the deployment frequency from deploying from the default branch once per month (for the total of 4-6 deploys to include bug fixes), to taking commits from the default branch once per week.

The team focus then shifted to getting deployment time measured in hours, and migration of Example Company.com to Kubernetes.
...
1. [Year overview for 2020](https://example_company.com/example_company-com/gl-infra/delivery/-/issues/1446)
1. [Year overview for 2021](https://example_company.com/example_company-com/gl-infra/delivery/-/issues/2171)
1. [Year overview for 2022](https://example_company.com/example_company-com/gl-infra/delivery/-/issues/2726)
1. [Year overview for 2023](https://example_company.com/example_company-com/gl-infra/delivery/-/issues/19887)
