---
title: "Example Company Dedicated Group"
---
...
The Example Company Dedicated team's mission is to create a fully managed, single-tenant Example Company environment, served through a Example Company Dedicated platform. It is developed to remove any manual interactions with customer tenant installations, and to ensure that the customer tenants are fully focused on unlocking the power of The One DevOps Platform.

## Vision

The Example Company Dedicated group is a customer facing team, with team members focused on a high level of infrastructure automation, and enabling customer interactions with the Example Company Dedicated platform.
...
## Example Company Dedicated Architecture

For Example Company Dedicated Architecture documentation please refer to the [Architecture Page](architecture/index.html)
...
Engineering team members of Example Company Dedicated are publicly referenced with specialty for `Environment Automation`, `Switchboard` or `US Public Sector Services` team, based on their primary task.
...
To engage with the Example Company Dedicated teams:

- [Create an issue](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/new) in the Example Company Dedicated team issue tracker (**Note** The issue tracker will be made public after [work in epic 33 "Considerations for making Dedicated projects public"](https://example_company.com/groups/example_company-com/gl-infra/example_company-dedicated/-/epics/33) is completed)
  - For feature requests, use the [feature requests issue template](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/blob/main/.example_company/issue_templates/feature_request.md) and fill in the required information
...
- Slack channels
  - For Example Company Dedicated specific questions, you can find us in [#f_gitlab_dedicated](https://example_company.slack.com/archives/C01S0QNSYJ2)
  - The Dedicated Group internally leverages: [#g_dedicated-team](https://example_company.slack.com/archives/C025LECQY0M)
  - Engineering teams within Dedicated have their own work channels for team work discussions:
    - [#g_dedicated-environment-automation-team](https://example_company.enterprise.slack.com/archives/C074L0W77V0)
  ...
  - Our social channel, [#g_dedicated-team-social](https://example_company.slack.com/archives/C03QBGQ3K5W) is accessible to everyone who wants to casually interact with the team
...
These customizations involve configuration changes to [functionality already supported in Dedicated](https://docs.example_company.com/ee/subscriptions/gitlab_dedicated/#available-features) including infra-level settings like IP Allowlists and Cloud-Specific Private Networking configuration as well as Example Company application settings that cannot currently be self-served through the admin UI like SAML configuration changes.

To request functionality that is not currently supported within Dedicated, customers must open a feature request using the [feature request issue template](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/blob/main/.example_company/issue_templates/feature_request.md). To request functionality for the broader Example Company Application, customers can use the [feature proposal](https://example_company.com/example_company-org/example_company/-/blob/master/.example_company/issue_templates/Feature%20Proposal%20-%20lean.md) template.
...
it is a good practice to temporarily halt production changes during certain events such as Example Company Summit,
major global holidays,
and other times where Example Company Team Member availability is substantially reduced.
...
Therefore, we have introduced a mechanism called Production Change Lock (PCL) to Example Company Dedicated.

The Example Company Dedicated Production Change Lock is greatly inspired by the [PCL](/handbook/engineering/infrastructure/change-management/#production-change-lock-pcl) for Example Company.com,
but there are some differences worth noting.

A PCL is manually enforced once the following requirements are met:

1. A PCL [issue](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/3946) describing the PCL period is created.
...
| Dates                       | Type       | Reason                        |
|-----------------------------|------------|-------------------------------|
| 2024-03-08 23:00 UTC -> 2024-03-17 20:00 UTC | Hard | Example Company Summit (Low team members availability) |

Times for the dates without a time specified begin at 09:00 UTC and end the next day at 09:00 UTC.

As opposed to Example Company.com [PCL](/handbook/engineering/infrastructure/change-management/#production-change-lock-pcl), for Example Company Dedicated we only consider a Hard PCL type.
...
Any action during an incident while in a PCL must be associated to an issue and the EOC should inform the Example Company Dedicated engineering Leadership about the action taken.

Changes not associated to any incident must have an exemption approval by the Example Company Dedicated engineering Leadership.

### Escalation Policy

When it comes to escalating customer support issues, we follow the same definitions of severity as [provided by support](https://about.example_company.com/support/definitions/#definitions-of-support-impact) since Dedicated customers receive priority support. Only in cases where there is an [availability or security sev-1](/handbook/engineering/infrastructure/engineering-productivity/issue-triage/#severity) event it can be escalated to Sev-1 at the Support level. 'Business as usual' configuration changes cannot be escalated to sev-1. In sev-1 cases we will involve our on-call, as these incidents may affect our Availability SLA commitments to the customer. Sev-2 and below will be handled by the team during normal business hours. Any fixes identified as part of a support ticket that must go out immediately will be considered "emergency maintenance" and can be done outside the normal maintenance window. All other fixes will be done during the next available maintenance window.

### Requesting access to logs

Example Company Dedicated comes with strict [access controls for tenant environments](https://docs.example_company.com/ee/subscriptions/gitlab_dedicated/#access-controls).
By default, Example Company Dedicated logs are only accessible by members of the Example Company
Dedicated and Support Engineering teams. In cases where Example Company Dedicated
...
1. [access request](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/new?issuable_template=Individual_Bulk_Access_Request). Use `Example Company Dedicated Logs (Production)` as the system.
1. An issue in the [Example Company Dedicated tracker](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/new?issuable_template=log_access_rotation) using the `Log rotation access` template.

## Working across Example Company

### Communicating with Example Company Dedicated customers

If you need to urgently contact one or more Example Company Dedicated customers, engage the
[Example Company Dedicated Communications Manager On-Call (CMOC)](/handbook/support/workflows/dedicated_cmoc/).

Non-urgent communication should be handled through the customer's Customer Success Manager (CSM).

### Getting product fixes into Example Company Dedicated quicker

{{% alert title="Note" color="info" %}}

This section should be moved into the Example Company Dedicated incident management process page when it
becomes available.
{{% /alert %}}

Sometimes, a product fix is introduced to resolve a Example Company Dedicated incident or
[customer escalation](#escalation-policy). There can be a significant delay between when the
product fix is merged and when it is deployed to Example Company Dedicated environments due to our
[upgrade policy](https://docs.example_company.com/ee/subscriptions/gitlab_dedicated/#upgrades).
...
A main goal in designing these processes is to make it possible for every team member working on any part of Example Company Dedicated to understand **why** their work matters and how it contributes to customer results.
...
- `Demo call` - This call is scheduled every two weeks, rotating across timezones. During this call, team members show off their progress, and engage with other team members on topics related to Example Company Dedicated platform. Demo calls are supposed to be rough around the edges and unpolished. In fact, if the demo looks polished, we will discuss whether we are being ambitious enough with our goals
...
- `Example Company Dedicated Product <> Eng Sync` - This call is weekly on Mondays and Thursdays for PMs and EMs to align
...
Example Company [confidentiality levels](/handbook/communication/confidentiality-levels/) require that we do not publicly identify customers without their express permission.
...
1. Example Company Dedicated group membership: `@example_company-dedicated`
    - All permanent team-members in any of the Example Company Dedicated teams gain access to this Example Company group as part of onboarding
    - Group mention should only be used in circumstances where the information shared is pertinent for all team members of the Example Company Dedicated group
1. Individual team group membership: `@example_company-dedicated/environment-automation`, `@example_company-dedicated/switchboard`, `@example_company-dedicated/uspubsec`, etc.
    - All permanent team-members of individual teams gain access to their respective Example Company group as part of onboarding
    - Group mention should be used when the information shared is pertinent to the respective team
1. Individual team Example Company Dedicated groups have two additional subgroups `maintainers` and `reviewers`, for example: `@example_company-dedicated/switchboard/maintainers`
    - `reviewers` Example Company group access is granted to permanent team-members, external contractors, team-members on borrow, and similar. This Example Company group type is used to distinguish users without merge rights. Initial reviews should be requested from this group, using the quick action, for example `/assign_reviewer @example_company-dedicated/switchboard/reviewers`
    - `maintainers` Example Company group is granted to permanent team-members only. This group has merge rights, and the group is granted access through [CODEOWNERS approval rules](https://docs.example_company.com/ee/user/project/codeowners/#code-owners). Team members onboard into the `maintainer` subgroup after meeting the requirements defined in the [Dedicated Maintainer Training](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/blob/main/.example_company/issue_templates/maintainer_training.md)(internal only)
...
1. [SaaS Platforms Epic](https://example_company.com/groups/example_company-com/-/epics/2115)
1. [Environment Automation Epic](https://example_company.com/groups/example_company-com/gl-infra/-/epics/479)
1. [Switchboard Epic (scoped to current quarter)](https://example_company.com/groups/example_company-com/gl-infra/example_company-dedicated/-/epics/405)
1. [US Public Sector Epic](https://example_company.com/groups/example_company-com/gl-infra/-/epics/876)
...
click A "https://example_company.com/groups/example_company-com/gl-infra/-/epics/479"
click E "https://example_company.com/groups/example_company-com/gl-infra/example_company-dedicated/-/epics/276"
click I "https://example_company.com/groups/example_company-com/gl-infra/example_company-dedicated/-/epics/297"
click J "https://example_company.com/groups/example_company-com/gl-infra/example_company-dedicated/-/epics/298"
```
...
Each epic has a single DRI who is responsible for delivering the project. DRIs for each epic are listed at the top of the description of each epic per Epic Structure. Epic DRI responsibilities are in [https://handbook.example_company.com/handbook/engineering/infrastructure/team/example_company-dedicated/#epic-owner-responsibilities](/handbook/engineering/infrastructure/team/example_company-dedicated/#epic-ownership)

1. Engineering epic DRIs can be found within children epics of [Example Company Dedicated epic](https://example_company.com/groups/example_company-com/gl-infra/-/epics/479).
...
1. Ensure epic meets criteria outlined in [Epic Structure](/handbook/engineering/infrastructure/team/example_company-dedicated/#epic-structure)
1. Provide updates on DRI's epic in epic description according to process outlined in [Status Update Process](/handbook/engineering/infrastructure/team/example_company-dedicated/#Status-Update-Process) below.
...
1. When the task is in progress:
    ...
    - If you get stuck, leave a comment on the issue and highlight your comment in [Slack](https://example_company.enterprise.slack.com/archives/C025LECQY0M)
...
1. If the issue becomes blocked:
    ...
    - Be sure to use Example Company Issue Relationship to mark upstream blocking issues as such, and add a comment on the blocking issue so that subscribers to that issue can be notified that it's blocking downstream work

#### New Issue Checklist

When creating a new issue:

1. Add the appropriate project label, example: ~"Hosted Runners for Example Company Dedicated".
...
# /label ~"Hosted Runners for Example Company Dedicated"
...
The status for all work relating to Example Company Dedicated is maintained in the description of the top-level [Example Company Dedicated epic](https://example_company.com/groups/example_company-com/gl-infra/-/epics/479) so that it is visible at a glance.
...
1. **By Wednesday at 21:00 UTC** the DRI for a project is expected to update the status block in the epic description to:
    ...
    1. **Update Workflow and Health label** - After each status update, the Workflow label and Health label should be updated. See [Epic labels criteria](/handbook/engineering/infrastructure/team/example_company-dedicated/#workflow-labels)

1. **Top-Level Epic Status Update** [automation synthesizes updates from status section](/handbook/engineering/infrastructure/team/example_company-dedicated/#status-update-automation) from description of active epics to provide initiative status in the status section in the description of the top-level initiative Epic.
...
Example Company Dedicated team respects the Company principle of [everything starting with a merge request](/handbook/communication/#start-with-a-merge-request).

1. All Merge Requests (MRs) must go through the review process.
1. Ensure that merge requests include in the description `Closes #<issue>` or `Related to #<issue>` so that the merge request change is linked to the appropriate issue within Example Company.
...
1. Reviewers should review the change and leave comments with questions or suggestions. Please follow the [merge request reviewer guidelines](/handbook/engineering/infrastructure/team/example_company-dedicated/#merge-request-reviewers) and the [resolving threads guidelines](/handbook/engineering/infrastructure/team/example_company-dedicated/#resolving-threads-on-a-merge-request) documented below.
...
Example Company Dedicated follows the same pattern for author/reviewer assignment as the standard Example Company practice, documented in the [Code Review Guidelines documentation](https://docs.example_company.com/ee/development/code_review.html#dogfooding-the-reviewers-feature).
...
1. The MR author will assign a reviewer and a maintainer to an MR that is ready for review.
     ...
     - The MR author can choose who to assign for review. To spread workload and knowledge it is recommended to use the [Environment Automation Reviewer Roulette](https://example_company-org.example_company.io/example_company-roulette/?currentProject=environment-automation).
     ...
     - If the change is a significant one, considering mentioning the appropriate group such as `@example_company-dedicated/environment-automation` or `@example_company-dedicated/switchboard` in the MR description to help with knowledge sharing.
...
A [Maintainer training issue](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/new?issuable_template=maintainer_training) will be created using the `maintainer_training` template and a training buddy will assigned to support the training.
...
1. Add a comment in the code describing the workaround and a link to the follow-up issue. For example [workaround on `pyyaml` bug](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/instrumentor/-/merge_requests/1873/diffs)
1. If the temporary workaround involves running scripts by the SRE during maintenance windows, we add the actions in the triage-ops' [tooling-upgrade-toil](https://example_company.com/example_company-com/gl-infra/triage-ops/-/blob/master/policies/example_company-dedicated/tooling-upgrade-toil.yml#L55) policy
...
1. All relevant Example Company Dedicated team labels
...
Scoped support labels are applied to the issues that are opened when a Example Company Support Engineer escalates a ticket for assistance using the ["request for help"](/handbook/support/workflows/how-to-get-help.html#how-to-use-gitlabcom-to-formally-request-help-from-the-example_company-development-team) process. These requests are reviewed periodically by members of the Example Company Support team. The purpose of this review is to identify whether a request could have been deflected. These reviews primarily lead to updates to the [Example Company Dedicated Support workflows](/handbook/support/workflows/index/#example_company-dedicated) and the [Example Company docs](https://docs.example_company.com/).
...
To denote different types of components and services we are working with, we leverage `component::` scoped labels. By using these labels, we are able to track the distribution of work across different components which also allows us to change focus where needed. These labels are created on [the Example Company Dedicated group level](https://example_company.com/groups/example_company-com/gl-infra/example_company-dedicated), since they are focused on work required specifically for Example Company Dedicated.

**NOTE** We do not use `Service::` labels given that service labels are used by Example Company SaaS related projects.

Component labels with their description can be found [by searching prioritized labels](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/labels?subscribed=&search=component).
...
Scoped workaround labels are intended to track temporary workarounds applied to Example Company Dedicated tenant instances that are supposed to be removed once a permanent fix is available. These labels need to be added to the follow-up issues that describe the implementation of the permanent fix.
...
with the schedule managed in [PagerDuty](https://example_company.pagerduty.com/schedules#PAP8TMH).
...
[handover issue](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/?label_name%5B%5D=capacity-planning-handover)
...
1. Review the capacity planning issues that are:
   1. In the Open column of the [board](https://example_company.com/example_company-com/gl-infra/capacity-planning-trackers/example_company-dedicated/-/boards/7536402).
   1. [Previously assessed](https://example_company.com/example_company-com/gl-infra/capacity-planning-trackers/example_company-dedicated/-/issues/?label_name%5B%5D=violation%3Asaturation) but now and therefore in need of a new look.
...
1. For each saturation issue that is up for review,
   ...
   - If you assess that it warrants active action in the near future and is not already `capacity-planning::in-progress`:
      ...
      1. Create a remediation [issue](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/new?issuable_template=saturation_risk)
   ...
   - If a metric is of a nature (perhaps for reasons specific to Dedicated)
     ...
     or if the prediction is plausibly useful but needs tuning:
      ...
      1. Work on the tamland
         [manifest](https://example_company.com/example_company-com/runbooks/-/blob/master/reference-architectures/get-hybrid/config/tamland/manifest.json)
...
1. Check that Tamland is [running](https://example_company.com/example_company-com/gl-infra/capacity-planning-trackers/example_company-dedicated/-/pipeline_schedules).
   The pipeline should run successfuly every day.
   Investigate and fix any errors or failures.
1. Check if there are new production tenants not [listed](https://example_company.com/example_company-com/gl-infra/capacity-planning-trackers/example_company-dedicated/-/blob/main/tenants.yaml).
   Update the list as necessary and create corresponding `tenant::` [labels](https://example_company.com/example_company-com/gl-infra/capacity-planning-trackers/example_company-dedicated/-/labels?subscribed=&sort=relevance&search=tenant%3A%3A).

When your shift comes to an end,
create a [handover issue](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/new?issuable_template=capacity_planning_handover&issue[title]=Capacity%20Planning%20Triage%20handover%20notes%20YYYY-MM-DD)
...
1. Look for the component / alert name in the
   [Capacity Planning Issue Tracker](https://example_company.com/example_company-com/gl-infra/capacity-planning-tracker/example_company-dedicated/-/issues) can be a good source of information,
...
Resources used by the team to conduct work are described on the [Development Resources Page](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/blob/main/engineering/Dev-resources.md).

## History and trivia

- Name for [`Switchboard` customer portal](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/7#note_591358260) was suggested by @marin after he spent a day trying to figure out which mixing console (Also known as a switchboard/soundboard) to get for amateur music making. He didn't buy anything, but the suggestion was accepted.
- Name for [`Amp` management cluster](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/31#note_609710775) was suggested by @ccasella, as it is the instance that is "powering" supply of other instances.
- [Dedicated Group - Year in Review 2023](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/3681)
