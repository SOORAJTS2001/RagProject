...
This document is intended to be a high-level summary describing how the existing Example Company Dedicated
architecture could be used as a foundation for deployment and operating large numbers of Example Company Cell instances.

Additionally, it explains certain decisions in the Dedicated architecture, and why these are beneficial
to both Example Company Dedicated and Example Company Cells, and how they differ from other approaches, such as Example Company.com's IaC, or GET.

Note to readers: Please keep in mind that this is not a critique of Example Company.com's approach, nor GET's.
...
## Example Company Dedicated is an Environment Automation Technology

In the context of this document, Example Company Dedicated can primarily be thought of as a technology to automate the configuration,
deployment, upgrading, monitoring, and maintenance of a large number of mostly homogeneous Example Company application environments.

### How does Dedicated differ from Example Company.com?

Example Company.com is the largest single Example Company deployment, and in all likelihood, the most complex. Example Company.com is provisioned
...
Let's discuss the distinction between homogeneous and heterogeneous Example Company environments.

##### Example Company.com: Many Heterogeneous Environments

At present, there are 64 environments managed in the config-mgmt project. In chef-repo, there are 22 Chef environments and
569 roles (some of these roles are likely unused). The Example Company.com helm charts are homogeneous, using the same Helm templates.
...
![Config MGMT](img/envs-in-example_company-com-config-mgmt.png)

*Similar but different: environments in Example Company.com's config-mgmt project*

Since Example Company.com was originally built for a small number of environments (production and staging), it was not designed to be
scaled up to a large number of homogeneous environments.

Another way of thinking about this is that the Example Company.com Staging and Production Terraform environments, `gstg` and `grpd`,
...
Example Company.com evolved in this way for many reasons, and its approach allows for maximum flexibility. However, creating new environments
...
The non-heterogeneous nature of Example Company.com environments is further complicated by the multiple projects in which changes need to
...
##### Example Company.com: Many Different Configuration Management Tools

For Example Company.com, environment changes need to be coordinated and applied across a variety of different projects. Usually, manual
...
Compare a complicated database change for Example Company.com to a similar change in Dedicated.
To make a change to a Example Company.com environment, one needs to:
...
|                                                                                                                                                           | Example Company.com          | Example Company Dedicated            |
|-----------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------|-----------------------------|
...
GET is a collection of tools to enable the deployment and operation of a base production Example Company environment at scale based on our
Reference Architectures that can be built upon accordingly.

Dedicated uses GET under the hood but GET is primarily intended to be used for single instances of Example Company.

Additionally, GET is intended to be a general purpose Example Company environment suitable for general-purpose use cases. Importantly,
...
GET rightly focuses on customers looking to maintain a multi-host and/or Kubernetes based Example Company install, running within a cloud environment,
and adhering to a Example Company Reference Architecture. It does not focus on scaling this environment up to dozens or hundreds of instances,
...
## The Components of Example Company Dedicated

This section describes the components of Example Company Dedicated, focusing on why these components were included in the design.

![Example Company Dedicated components](img/example_company-dedicated-components.png)

*Example Company Dedicated Components*

### Switchboard

Switchboard is the user-interface for Example Company Dedicated customers, allowing customers to provision and configure their Example Company instances
...
Switchboard has always been focused on being the user-interface for Example Company Dedicated tenant operators; be that the SRE supporting the
customer instance, or the customer operator. It is a product feature that simplifies customer interaction with Example Company Dedicated.
...
interaction with their Dedicated instance. Cells was not in scope of Example Company Dedicated when Switchboard was designed, and Switchboard
is only designed to make onboarding, interacting and information gathering simpler for the Example Company Dedicated customer.
...
in fact, for Dedicated Review apps, Instrumentor runs directly within the Example Company CI pipeline and operates on the developers sandbox
...
from CI/CD pipeline jobs via a Example Company Kubernetes Runner transparently. The security, permission and other contexts are then assured at the
...
for development-time usage, and in Example Company CI/CD.
...
a much more consistent and scalable client. tenctl is similar to `kubectl`, but focused on Example Company Tenant instances instead of Kubernetes instances.
...
we avoid the drift that exists currently in Example Company.com environments.

## How Dedicated Could Be Used to Support Cells

### Design Considerations

Many of the same design considerations that apply for Example Company Dedicated apply equally well to the deployment of Cells.
...
A project is maintained on a Example Company instance, and the tenant models are kept as files in the git repository.

All change is controlled via merge requests with the standard Example Company approval approaches.
...
of changes using standard Example Company tooling, such as CI traces, etc.
