...
{{% alert color="warning" %}}
If you're a Example Company team member and are looking to alert Reliability Engineering about an availability issue with Example Company.com, please find quick instructions to report an incident here: [Reporting an Incident](#reporting-an-incident).
{{% /alert %}}

{{% alert color="warning" %}}
If you're a Example Company team member looking for who is currently the Engineer On Call (EOC), please see the [Who is the Current EOC?](#who-is-the-current-eoc) section.
{{% /alert %}}

{{% alert color="warning" %}}
If you're a Example Company team member looking for the status of a recent incident, please see the incident [board](https://example_company.com/example_company-com/gl-infra/production/-/boards/1717012?&label_name%5B%5D=incident). For detailed information about incident status changes, please see the [Incident Workflow](#incident-workflow) section.
{{% /alert %}}
...
in the [`#incident-management`](https://example_company.slack.com/archives/CB7P5CJS1) channel
...
| `CMOC` - Incident **Communications Manager On Call** | The CMOC disseminates information internally to stakeholders and externally to customers across multiple media (e.g. Example Company issues, status.example_company.com, etc.). | The **Communications Manager** is generally member of the support team at Example Company. Notifications to the `Incident Management - CMOC` service in PagerDuty will go to the rotations setup for CMOC. |
...
| `Infrastructure Liaison` | The Infrastructure Liaison is someone who speaks on behalf of the department to the e-team for severity 1 incidents. | A [grade 10+](/handbook/total-rewards/compensation/compensation-calculator/#example_company-job-grades) member of Infrastructure. |
...
1. **As EOC, your highest priority for the duration of your shift is the stability of Example Company.com.**
...
1. The SSOT for who is the current EOC is the [Example Company Production](https://example_company.pagerduty.com/service-directory/PATDFCE) service definition in PagerDuty.
...
1. Alerts that are routed to PagerDuty require acknowledgment within 15 minutes, otherwise they will be escalated to the oncall Incident Manager.
    1. Alert-manager alerts in [`#alerts`](https://example_company.slack.com/archives/alerts) and [`#feed_alerts-general`](https://example_company.slack.com/archives/feed_alerts-general) are an important source of information about the health of the environment and should be monitored during working hours.
...
1. Example Company.com is a complex system. It is ok to not fully understand the underlying issue or its causes. However, if this is the case, as EOC you should [page the Incident Manager](#how-to-engage-the-eoc-im-or-cmoc) to find a team member with the appropriate expertise.
    1. Requesting assistance does not mean relinquishing EOC responsibility. The EOC is still responsible for the incident.
    1. The [Example Company Organizational Chart](https://comp-calculator.example_company.net/org_chart) and the [Example Company Team Page](/handbook/company/team/), which lists areas of expertise for team members, are important tools for finding the right people.
1. As soon as an [S1/S2](/handbook/engineering/infrastructure/production/#severity) [incident is declared](#report-an-incident-via-slack), join the `The Situation Room Permanent Zoom`. The Zoom link is in the `#incident-management` topic.
    1. Example Company works in an asynchronous manner, but incidents require a synchronous response. Our collective goal is high availability of 99.95% and beyond, which means that the timescales over which communication needs to occur during an incident is measured in seconds and minutes, not hours.
...
1. Keep in mind that a Example Company.com incident is not an "infrastructure problem". It is a company-wide issue, and as EOC, you are leading the response on behalf of the company.
...
1. If applicable, coordinate the incident response with [business contingency activities](/handbook/business-technology/example_company-business-continuity-plan/).
...
1. Around the 1st Tuesday of each month:
   - The coordinator will review any open `~IM-Onboarding::Ready` and `~IM-Offboarding` issues on the [IM onboarding/offboarding board](https://example_company.com/example_company-com/gl-infra/production-engineering/-/boards/5078854?label_name%5B%5D=IM) and add these team members to the schedule.
   - The schedule is updated by creating an MR that updates [im_locals.tf](https://ops.example_company.net/example_company-com/gl-infra/config-mgmt/-/blob/main/environments/pagerduty/im_locals.tf?ref_type=heads).
...
1. An announcement will be posted in [`#im-general`](https://example_company.slack.com/archives/C01NY82EJF6) indicating that the schedule has been modified with a link to the MR and a brief overview of who was added or removed.
1. All issues on the [IM onboarding/offboarding board](https://example_company.com/example_company-com/gl-infra/production-engineering/-/boards/5078854?label_name%5B%5D=IM) need to be reviewed once a month for overdue due dates.
...
The Example Company support team staffs an oncall rotation and via the `Incident Management - CMOC` service in PagerDuty. They have a section in [the support handbook](/handbook/support/workflows/cmoc_workflows/) for getting new CMOC people up to speed.

During an incident, the CMOC will:

1. Be the voice of Example Company during an incident by updating our end-users and internal parties through updates to our [status page](https://status.example_company.com/) hosted by Status.io.
...
```markdown
:s1: **Incident on Example Company.com**
**— Summary —**
...
Scalability leadership can be reached via PagerDuty [Scalability Escalation](https://example_company.pagerduty.com/escalation_policies#PDJ160O) (further [details available on their team page](/handbook/engineering/infrastructure/team/scalability/index.html#emergency-escalation-during-s1s2-incidents)).
...
1. If wider user impact has been established during an S1 or S2 incident, as EOC you have the authority - without requiring further permission - to [Block Users](https://docs.example_company.com/ee/administration/moderate_users.html#block-a-user) as needed in order to mitigate the incident. Make sure to follow [Support guidelines regarding `Admin Notes`](../../../support/workflows/admin_note/#adding-the-note), leaving a note that contains a link to the incident, and any further notes explaining why the user is being blocked.
    1. If users are blocked, then further follow-up will be required. This can either take place during the incident, or after it has been mitigated, depending on time-constraints.
        ...
        1. If not, [open a related confidential incident issue and assign it to CMOC](https://example_company.com/example_company-com/gl-infra/production/-/issues/new?issuable_template=confidential_incident_data) to reach out to the user, explaining why we had to block their account temporarily.
...
1. An entire path or part of functionality of the Example Company.com application must be blocked.
1. Any unauthorized access to a Example Company.com production system
...
1. If your ask is not addressed via slack, escalate to  [Infrastructure Leadership](https://example_company.pagerduty.com/service-directory/PJKOEIS) in PagerDuty.
...
If assistance is needed to determine severity, EOCs and Incident Managers are encouraged to contact [Reliability Leadership via PagerDuty](https://example_company.pagerduty.com/schedules#P12EH0Z)
...
or
- Directly from PagerDuty, navigate to [Incidents page in PagerDuty](https://example_company.pagerduty.com/incidents), create a new incident and select the **Impacted Service** based on the role below.

| Role | Impacted Service |
| ----- | ----- |
| EOC | Example Company Production |
...
1. An incident which is uniquely impacting a single, or small number, of customers where their insight into how they are using Example Company.com would be valuable to finding a solution.
...
1. Page the [Infrastructure Leadership pagerduty rotation](https://example_company.pagerduty.com/schedules#PBSMJH2) for additional assistance and awareness.
...
1. The Engineering Communications Lead and CSM should appropriately set their Zoom name to indicate `Example Company`, as well as their Role, `CSM` `Engineering Communications Lead`
...
Work items identified in incidents that don't meet the Corrective Action criteria should be raised in the [Reliability project](https://example_company.com/example_company-com/gl-infra/reliability/-/issues/new) and labeled with `~work::incident` rather than `~corrective action`
...
Corrective Actions issues in the [Reliability project](https://example_company.com/example_company-com/gl-infra/reliability/-/issues/new) should be created using the [Corrective Action issue template](https://example_company.com/example_company-com/gl-infra/reliability/-/blob/master/.example_company/issue_templates/incident-corrective-action.md) to ensure consistency in format, labels and application/monitoring of [service level objectives for completion](/handbook/engineering/infrastructure/engineering-productivity/issue-triage/#severity-slos)
...
[Runbooks](https://example_company.com/example_company-com/runbooks) are available for
engineers on call. The project README contains links to checklists for each
of the above roles.

**In the event of a Example Company.com outage**, a mirror of the runbooks repository is available on at https://ops.example_company.net/example_company-com/runbooks.
...
If you are a Example Company team member and would like to report a possible incident related to Example Company.com and have the EOC paged in to respond, choose one of the reporting methods below. Regardless of the method chose, please stay online until the EOC has had a chance to come online and engage with you regarding the incident. Thanks for your help!

### Report an Incident via Slack

Type `/incident declare` in the [`#production`](https://example_company.slack.com/archives/C101F3796) channel in Example Company's Slack and follow the prompts to open an incident issue.
...
Data is based on the graphs from the [Key Service Metrics Dashboard](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1)
...
SLOs are documented in the [runbooks/rules](https://example_company.com/example_company-com/runbooks/blob/master/rules/service_apdex_slo.yml)

To check if we are Degraded or Disrupted for Example Company.com, we look at these graphs:

1. Web Service
    - [Error Ratio](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&fullscreen&panelId=8&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=web&var-stage=main&var-sigma=2)
    - [Apdex](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=web&var-stage=main&var-sigma=2&fullscreen&panelId=7)
1. API Service
    - [Error Ratio](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&fullscreen&panelId=8&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=api&var-stage=main&var-sigma=2)
    - [Apdex](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&fullscreen&panelId=7&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=api&var-stage=main&var-sigma=2)
1. Git service(public facing git interactions)
    - [Error Ratio](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&fullscreen&panelId=8&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=git&var-stage=main&var-sigma=2)
    - [Apdex](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&fullscreen&panelId=7&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=git&var-stage=main&var-sigma=2)
1. Example Company Pages service
    - [Error Ratio](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&fullscreen&panelId=8&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=pages&var-stage=main&var-sigma=2)
    - [Apdex](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&fullscreen&panelId=7&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=pages&var-stage=main&var-sigma=2)
1. Registry service
    - [Error Ratio](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&fullscreen&panelId=8&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=registry&var-stage=main&var-sigma=2)
    - [Apdex](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&fullscreen&panelId=7&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=registry&var-stage=main&var-sigma=2)
1. Sidekiq
    - [Error Ratio](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=sidekiq&var-stage=main&var-sigma=2&fullscreen&panelId=8)
    - [Apdex](https://dashboards.example_company.net/d/general-service/service-platform-metrics?orgId=1&fullscreen&panelId=7&var-PROMETHEUS_DS=Global&var-environment=gprd&var-type=sidekiq&var-stage=main&var-sigma=2)

A Partial Service Disruption is when only part of the Example Company.com services or infrastructure is experiencing an incident. Examples of partial service disruptions are instances where Example Company.com is operating normally except there are:
...
1. a dedicated Zoom call for all incidents. A link to the Zoom call can be found in the topic for the [`#incident-management`](https://example_company.slack.com/archives/incident-management) room in Slack.
...
1. a dedicated [`#incident-management`](https://example_company.slack.com/archives/incident-management) channel for internal updates
1. regular updates to status.example_company.com via status.io that disseminates to various media (e.g. Twitter)
1. a dedicated repo for issues related to [Production](https://example_company.com/example_company-com/production) separate from the queue that holds Infrastructure's workload: namely, issues for incidents and changes.

### Status

We manage incident [communication](#communication) using status.io, which updates [status.example_company.com](https://status.example_company.com). Incidents in status.io have **state** and **status** and are updated by the incident owner.
...
- If there is no usage of the URL in the last week based on searches in our logs for Example Company.com.
...
| ------------- | ------------- | -------------|
| `~severity::1` |  &emsp;  - Example Company.com is unavailable or severely degraded for the typical Example Company user<br>&emsp;  - Any data loss directly impacting customers<br>&emsp;  - The [guaranteed self-managed release date](/handbook/engineering/releases/#timelines) is put in jeopardy<br>&emsp;  - It is a [high impact security incident](/handbook/security/security-operations/sirt/severity-matrix.html#functional-impact-rating-examples)<br>&emsp; - It is an internally facing incident with full loss of metrics observability (Prometheus down)<br><br>[Incident Managers](/handbook/engineering/infrastructure/incident-management/#incident-manager-responsibilities) should be paged for all `~severity::1` incidents| Past `severity::1` [Issues](https://example_company.com/example_company-com/gl-infra/production/-/issues/?sort=updated_desc&state=closed&label_name%5B%5D=severity%3A%3A1&first_page_size=100)|
| `~severity::2` |   &emsp;  - There is a recorded impact to the availability of one or more [Example Company.com Primary Service with a weight > 0](https://dashboards.example_company.net/d/general-slas/general-slas?orgId=1&from=now-1h&to=now).  This includes `api`, `container registry`, `git access`, `API` and `web`.<br>&emsp;  - Example Company.com is unavailable or degraded for a small subset of users <br>&emsp;- Example Company.com is degraded but a reasonable workaround is available (includes widespread frontend degradations)<br>&emsp;- Any [moderate impact security incident](/handbook/security/security-operations/sirt/severity-matrix.html#functional-impact-rating-examples)<br>&emsp;- CustomersDot is offline<br><br>[Incident Managers](/handbook/engineering/infrastructure/incident-management/#incident-manager-responsibilities) should be paged for all `~severity::2` incidents| Past `severity::2` [Incidents](https://example_company.com/example_company-com/gl-infra/production/-/issues/?sort=updated_desc&state=closed&label_name%5B%5D=severity%3A%3A2&first_page_size=100)|
| `~severity::3` |   &emsp;  - Broad impact on Example Company.com and minor inconvenience to typical user's workflow <br>&emsp;- A workaround is not needed<br>&emsp;- Any [low impact security incident](/handbook/security/security-operations/sirt/severity-matrix.html#functional-impact-rating-examples)<br>&emsp;- Most internally facing issues pertaining to blocked deployments (should a higher-severity incident be blocked by deployments, the severity for the blocker is still 3)<br>&emsp;- CustomersDot is in maintenance mode | Past `severity::3` [Incidents](https://example_company.com/example_company-com/gl-infra/production/-/issues/?sort=updated_desc&state=closed&label_name%5B%5D=severity%3A%3A3&first_page_size=100)|
| `~severity::4` |   &emsp;  - Minimal impact on Example Company.com typical user's workflow | Past `severity::4` [Incidents](https://example_company.com/example_company-com/gl-infra/production/-/issues/?sort=updated_desc&state=closed&label_name%5B%5D=severity%3A%3A4&first_page_size=100)|
...
There are four data classification levels defined in Example Company's [Data Classification Standard](/handbook/security/data-classification-standard/#data-classification-levels).
...
Example Company uses the [Incident Management](/handbook/engineering/infrastructure/incident-management/) feature of the Example Company application. Incidents are [reported](/handbook/engineering/infrastructure/incident-management/#reporting-an-incident) and closed when they are resolved. A resolved incident means the degradation has ended and will not likely re-occur.
...
Incidents use the [Timeline Events](https://docs.example_company.com/ee/operations/incident_management/incident_timeline_events.html) feature, the timeline can be viewed by selecting the "Timeline" tab on the incident.
...
For adding timeline items quickly, use the [quick action](https://docs.example_company.com/ee/user/project/quick_actions.html), for example:
...
The following labels are used to track the incident lifecycle from active incident to completed incident review. [Label Source](https://example_company.com/example_company-com/gl-infra/production/-/labels?utf8=%E2%9C%93&subscribed=&search=Incident%3A%3A)
...
| `~RootCause::External-Dependency` | resulting from the failure of a dependency external to Example Company, including various service providers. Use of other causes (such as `~RootCause::SPoF` or `~RootCause::Saturation`) should be strongly considered for most incidents. |
| `~RootCause::FalseAlarm` | an incident was created by a page that isn't actionable and should result into adjusting the alert or [deleting it](https://example_company.com/example_company-com/runbooks/-/blob/master/docs/monitoring/deleting-alerts.md) |
...
| `~RootCause::Malicious-Traffic` | deliberate malicious activity targeted at Example Company or customers of Example Company (e.g. DDoS) |
...
| ----------------------- | ----------- |
| `~Incident-Comms::Status-Page` | Incident communication included use of the public [Example Company Status Page](https://status.example_company.com/) |
| `~Incident-Comms::Private` | Incident communication was limited to fewer customers or otherwise was only directly communicated to impacted customers (not via the Example Company Status Page) |
...
| A degradation in service due to missing index on a table. | Use `~Service::Example Company Rails` |

#### "Needed" and "NotNeeded" Scoped Labels

The following labels are added and removed automatically by [triage-ops](https://example_company.com/example_company-com/gl-infra/triage-ops/):
...
| `~self-managed` | Indicates that an incident is **exclusively** an incident for self-managed Example Company. [Example self-managed incident issue](https://example_company.com/example_company-com/gl-infra/production/-/issues/2651) |
...
| `~Delivery impact::*` | Indicates [the level of impact this incident is having on Example Company deployment and releases](/handbook/engineering/releases/#delivery-impact-labels) |
...
There are [related issue links](https://example_company.com/example_company-com/gl-infra/production/-/blob/5343440ac4ef41fa5a27053a6938480d229bee3e/.example_company/issue_templates/incident.md#create-related-issues) on the incident template that should be used to create related issues from an incident.
...
If an [alert silence](https://example_company.com/example_company-com/runbooks/-/blob/master/docs/monitoring/alerts_manual.md#silencing) is created for an active incident, the incident should be resolved with the `~"alertmanager-silence"` label and the appropriate root cause label if it is known.
There should also be a linked ~infradev issue for the long term solution or an investigation issue created using the [related issue links](https://example_company.com/example_company-com/gl-infra/production/-/blob/5343440ac4ef41fa5a27053a6938480d229bee3e/.example_company/issue_templates/incident.md#create-related-issues) on the incident template.

### Incident Board

The board which tracks all Example Company.com incidents from active to reviewed is located [here](https://example_company.com/example_company-com/gl-infra/production/-/boards/1717012?&label_name%5B%5D=incident).
