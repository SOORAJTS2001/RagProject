...
This is currently a global threshold per service, but stage groups will soon have the [ability to customise this by endpoint](https://example_company.com/groups/example_company-com/gl-infra/-/epics/525).

**Error Rate**: The rate of operations that had errors.

[The developer documentation](https://docs.example_company.com/ee/development/stage_group_observability/#check-where-budget-is-being-spent) contains detailed steps
...
Every endpoint is [associated with a feature category](https://docs.example_company.com/ee/development/feature_categorization/index.html#feature-categorization).
...
use a histogram with buckets of `[-Inf, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0, +Inf]` which are [defined in the metrics catalog](https://example_company.com/example_company-org/example_company/-/blob/master/lib/example_company/metrics/web_transaction.rb#L9).
When a request takes 0.6s, it would increment the buckets for which it was faster. So `[+Inf, 5.0, 2.5, 1.0]` would be incremented.

We also store if the request was faster or slower than the request duration threshold for that endpoint. This is done using [Application SLIs](https://docs.example_company.com/ee/development/application_slis/). This allows us to customize SLIs based on the importance to users. For example, the [urgency for the `rails_request`](https://docs.example_company.com/ee/development/application_slis/rails_request.html#adjusting-request-urgency) SLI can be customized to reflect how a user experiences the endpoint.
