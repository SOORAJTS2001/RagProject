...
Example Company has a [Three-Year Strategy](/handbook/company/strategy/), and we're excited to see every member of the Engineering division contribute to achieving it. Whether you're creating something new or improving something that already exists, we want you to feel empowered to bring your best ideas for influencing the product direction through improved scalability, usability, resilience, and system architectures. And when you feel like you need to expand your knowledge in a particular area, know that you're supported in having the resources to learn and improve your skills.

Our focus on FY24 is to make sure that Example Company is enterprise grade in all its abilities and to support the AI efforts required to successfully launch Code Suggestions and Example Company Duo to General Availability.

Making sure that Example Company is enterprise grade involves several teams collaborating on improving our disaster recovery and support offerings through ongoing work with Example Company Dedicated, and Cells infrastructure.  Our goal here is improved availability and service recovery.

## Engineering Culture

Engineering culture at Example Company encompasses the processes, workflows, principles
and priorities that all stem from our [Example Company Values](/handbook/company/culture/all-remote/values).
...
because at Example Company everyone can contribute.
...
Engineering is the primary advocate for the performance, availability, and security of the Example Company project. Product Management prioritizes 60% of engineering time, so everyone in the engineering function should participate in the Product Management [prioritization process](/handbook/product/cross-functional-prioritization) to ensure that our project stays ahead in these areas. Engineering prioritizes 40% of time on initiatives that improve the product, underlying platform and foundational technologies we use.
...
- Review fixes from our support team. These merge requests are tagged with the `Support Team Contributions` label.  You can [filter on open MRs](https://example_company.com/example_company-org/example_company/-/merge_requests?label_name%5B%5D=Support%20Team%20Contributions).
...
For **Free Users**, we're becoming more efficient with our open core offering, so that we can continue to support and give back to students, startups, educational institutions, open source projects, Example Company contributors, and nonprofits
...
For **Hosted Customers**, we're supporting feature parity between Self-Managed and Example Company Hosted environments through the Workspace initiative. We're also launching Example Company Dedicated for customers who want the flexibility of cloud with the security and performance of a single-tenant environment.
...
- [Example Company Innovation Program](/handbook/legal/patent-program/), managed by the Example Company Legal Team
...
- [Example Company Repositories](/handbook/engineering/example_company-repositories/)
- [Issue Triage Policies](/handbook/engineering/infrastructure/engineering-productivity/issue-triage/)
- [Contributing to Go projects](https://docs.example_company.com/ee/development/go_guide/index.html)
...
- [Critical Security Releases](https://example_company.com/example_company-org/release/docs/blob/master/general/security/process.md#critical-security-releases)
- [Incident Management](/handbook/engineering/infrastructure/incident-management/)

### Example Company in Production
...
1. Incident Review
   - Include any [S1 incidents](https://example_company.com/example_company-com/gl-infra/production/-/issues/?sort=created_date&state=all&label_name%5B%5D=incident&label_name%5B%5D=severity%3A%3A1&first_page_size=100) that have occurred since the previous meeting.
   - Include any incidents that required a [status page](https://status.example_company.com/) update.
...
If you need to test features that are built for Example Company Ultimate then you can get added to the [issue-reproduce](https://example_company.com/issue-reproduce)
group on production and staging environments by asking in the [#development](https://example_company.slack.com/archives/C02PF508L) Slack channel.
...
- The issues escalated have critical business impact to the customer, determined by Customer Success and Support Engineering leadership
  - Failure to expedite scheduling may have cascading business impact to Example Company
- Approval from a VP from Customer Success AND a Director of Support Engineering are required to expedite scheduling
  - Customer Success: approval from either [Sherrod Patching](https://example_company.com/spatching) or [David Sakamoto](https://example_company.com/dsakamoto)
  - Support Engineering: approval from either [Lee Matos](https://example_company.com/lbot) or [Lyle Kozloff](https://example_company.com/lyle) or [Shaun McCann](https://example_company.com/shaunmccann) or [Val Parsons](https://example_company.com/vparsons)
...
The DRI can use the [customer critical merge requests](https://docs.example_company.com/ee/development/code_review.html#customer-critical-merge-requests) process to expedite code review & merge.
...
There are some engineering handbook topics that we cannot be publicly transparent about.  These topics can be viewed by Example Company team members in the [engineering section of the private handbook](https://internal.example_company.com/handbook/engineering/).

If you experience a page not found (404) error when attempting to access the internal handbook, you may need to register to use it via first browsing to the [internal handbook authorization page](https://example_company.com/internal-handbook/internal.example_company.com/).
