...
Skip this part if you are familiar with how Git data transfer architecture at Example Company.
...
Example Company resulted in a shift in usage patterns. Consequently, handling Git data transfer in hosted Git servers has become challenging.
...
For Example Company, Gitaly manages all Git operations. However, it is not accessible to external sources because Workhorse and Example Company Shell handle all external communications.
Gitaly offers a gRPC server for them through specific RPCs such as [SmartHTTP](https://example_company.com/example_company-org/gitaly/-/blob/master/proto/smarthttp.proto) and
[SSH](https://example_company.com/example_company-org/gitaly/-/blob/master/proto/ssh.proto). In addition, it provides numerous other RPCs for Example Company Rails and other services. The following
...
sequenceDiagram
  ...
  participant SSHD as Example Company SSHD
  participant GitLabShell as example_company-shell
  ...
  SSHD ->> GitLabShell: spawns example_company-shell
  ...
  Note over SSHD,GitalyGit: On Example Company server
```

Source: shameless copy from [this doc](https://example_company.com/example_company-org/gitaly/-/blob/master/cmd/gitaly-ssh/README.md?plain=0)

### Git data transfer optimization with Sidechannel

In the past, we faced many performance problems when transferring huge amounts of data with gRPC. [Epic 463](https://example_company.com/groups/example_company-com/gl-infra/-/epics/463) tracks
...
implementation. For more information on Sidechannel, see [this document](https://example_company.com/example_company-org/gitaly/-/blob/master/doc/sidechannel.md). In summary, Sidechannel works
as follows:
...
flowchart LR
    ...
    GitLabShell("Example Company Shell")-- SSHUploadPack --> Port3001
    GitLabRails("Example Company Rails") -- Other RPCs --> Port3000
    subgraph Gitaly
      Port3000
      gRPCServer
      Port3001
...
migration gradually on Example Company.com, starting with UploadPack RPC. Other RPCs can be migrated after careful consideration. Self-managed instances can use the existing
...
- Clients (Workhorse and Example Company Shell) continue to use gRPC clients. A request using this approach is considered to be a normal gRPC call. Hence, it should work well
...
- Gitaly: [merge request 5885](https://example_company.com/example_company-org/gitaly/-/merge_requests/5885).
- Workhorse: [merge request 123764](https://example_company.com/example_company-org/example_company/-/merge_requests/123764).
...
Another concern is that Workhorse and Example Company Shell must maintain a separate connection pool. At the moment, they maintain one connection to each Gitaly node. This number
...
are handled by the HTTP/2 server. Example Company Rails stay intact as it doesn't handle UploadPack.
