---
title: "Backup and Restore Example Company"
status: ongoing
...
[The Unified Backups project](https://example_company.com/groups/example_company-org/-/epics/11577) provides a single command-line tool that will handle the application backup and recovery needs of Example Company installations across supported [reference architectures](https://docs.example_company.com/ee/administration/reference_architectures/index.html). It will be packaged separately from the main Example Company code base to keep it decoupled from specific release versions but will be shipped along with Example Company releases.
...
Early development on this tool will focus on providing value to self-hosted customers of Example Company by supporting the variety of installation types and common architectures. For these customers, we will focus on simplifying the disaster recovery process into a common set of recommendations. Additionally, we will work to resolve scalability problems with current backup solutions by supporting the cloud service integration capabilities of large high-usage Example Company instances.

However, we will quickly move into the next phase of the project, which will focus on the specialized needs of [Example Company Dedicated](https://about.example_company.com/dedicated/). This will provide value to the Dedicated Group in terms of automating and streamlining backup creation and restoration with our standard dedicated architectures on Google and Amazon cloud services.
...
While DR is the primary motivation for creating tooling around backup creation and restoration, we also acknowledge that administrators of self-hosted installations often need to use backup and restoration for configuration and maintenance purposes. The administrator may need to upgrade versions of a crucial component of the system such as PostgreSQL or object storage appliance. In such cases, it is often best to take down their Example Company application temporarily to prevent further updates. Then, the admin may capture a point-in-time backup of their installation. The admin then performs the necessary changes to the system such as upgrading PostgreSQL to a more recent version. When complete, the captured backup data may be restored to ensure Example Company is close to the same state as it was in prior to the maintenance work. Finally, the site may be brought back online for general use again.

Alternatively, an admin may need to migrate their current Example Company production architecture to new infrastructure. For example, an organization may be running a simple Example Company 1K [reference architecture](https://docs.example_company.com/ee/administration/reference_architectures/index.html) on a single machine the organization manages. However, over time, this organization has usage that has grown beyond the parameters of a simple 1K architecture, and now an admin has been charged with setting up a cloud-based 3K multi-node reference architecture hosted through Amazon Web Services (AWS). This is a common use case for backup and restoration tooling. The admin builds out their 3K deployment on AWS. Then, the admin takes a backup of the running 1K deployment. Finally, the admin restores the backup on the 3K architecture to ensure the organization can seamlessly switch from the small architecture to this new high-usage one.

In this last example, we are indicating how backup tools can facilitate customers to move their Example Company installations to different architectures. However, we need to discuss an important limitation for this use case. We only permit restoring a backup archive on the exact same version number of Example Company upon which the backup was created. We have seen many situations in the past where customers attempt to use Example Company backup tools to transfer their application data between different release versions of Example Company itself. As an example, a customer may have primarily used a version 15.10 installation without managing incremental releases updates over time. Now, the customer wants to jump to a much more recent version like 16.10. That customer may believe they can just use the Rake backup creation task to get a tarball of their older site data, setup a new location with the more recent version of Example Company, and then just use the Rake backup restoration task to populate the newer version installation with the same data. However, our current backup tooling does not support restorations on a different Example Company version than the one under which the backup was created.

Currently, we plan on making the new unified backup tool also uphold this restriction; backup restoration will only take place on the same version of Example Company that created the target backup archive. This is essential to make sure the database schema is exactly in line with what is expected by that version. Otherwise, there may be any degree of bugs and data corruptions that may occur when the code is programmed against a different data schema than what has been restored. We do intend to explore some degree of cross-version restoration support in later iterations of the project.

### Current Example Company Backup Offering

Currently Example Company provides recommendations for [how to create application backups](https://docs.example_company.com/ee/administration/backup_restore/backup_gitlab.html) across different installation types and different hosting architectures. We provide a fairly rudimentary set of tools to create a point-in-time application backup, as well as specialized documentation for how to handle more complex cloud backup situations. You can read more on [how Example Company backups work here](https://example_company.com/example_company-org/example_company/-/merge_requests/158058).

These are based on [Rake tasks](https://docs.example_company.com/ee/raketasks) in the Example Company repository.
...
- This is not a solution for users to backup/export their Example Company.com group or project data
...
Online Consistent Backups will be explored through [epic 12043](https://example_company.com/groups/example_company-org/-/epics/12043).

**On-going design discussions**

The results of the following on-going [technical design discussions](https://example_company.com/groups/example_company-org/-/epics/14081) will be added to the blueprint.

- [Investigate scaling backups](https://example_company.com/example_company-org/example_company/-/issues/468677)
- [How to limit concurrency of backup processes?](https://example_company.com/example_company-org/example_company/-/issues/468313)
- [How to restore a cloud backup with the `example_company-backup-cli`?](https://example_company.com/example_company-org/example_company/-/issues/465999)
- [Add how distribution will work with this tool](https://example_company.com/example_company-org/example_company/-/issues/466040)
- [Investigate how to handle mixed Portable and Cloud backups](https://example_company.com/example_company-org/example_company/-/issues/465529)
- [Investigate what is required to support Kubernetes and large reference architectures](https://example_company.com/example_company-org/example_company/-/issues/427359)
- [Investigate how to support backing up Omnibus configuration and secrets](https://example_company.com/example_company-org/example_company/-/issues/428515)
- [What infra provisioning is needed for each type of datatype being backed up in the cloud?](https://example_company.com/example_company-org/example_company/-/issues/466038)
- [How to handle Redis data (not backed up)?](https://example_company.com/example_company-org/example_company/-/issues/466000)
- [Investigate Gitaly improvements to Backup](https://example_company.com/example_company-org/example_company/-/issues/465534)

### Limitations

- We don't support the data in a Cloud-based Backup to be exportable to a Portable Backup format or vice-versa.
- We do not support backing up data in the cache store (Redis) which includes the [Sidekiq state](https://docs.example_company.com/ee/administration/backup_restore/backup_gitlab.html#other-data). TODO: [More research on Redis stored data](https://example_company.com/example_company-org/example_company/-/issues/466000)
...
For Example Company installations hosted on Google Cloud Platform (GCP) we will support the following application data types
...
- So at its most basic, a user would run something like `example_company-backup-cli restore all $BACKUP_ID`, and the tool would create the necessary jobs to copy data from the backup bucket path, to the individual buckets, then monitor them for success/failure.
...
- Currently, the Distribution team owns and maintains [backup-utility](https://example_company.com/example_company-org/build/CNG/-/blob/master/example_company-toolbox/scripts/bin/backup-utility) which is used for backups in Kubernetes enabled environments. We will need to work with them to replace that tool, with `example_company-backup-cli`
- For the [`omnibus-example_company`](https://example_company.com/example_company-org/omnibus-example_company), it does create the `example_company-backup` wrapper around the backup Rake tasks. Deprecation of the previous tool is TBD.
- The new tool (`example_company-backup-cli`) will be distributed as part of the main Example Company codebase as one of the [bundled gems](https://example_company.com/example_company-org/example_company/-/tree/master/gems/example_company-backup-cli?ref_type=heads).
- In the future, the new tool will be decouple from that codebase and may be distributed independently from the main package, to allow for supporting backing up and restoring from distinct (compatible) Example Company versions.

#### Stable Counterpart

[Robert Marshall](https://example_company.com/rmarshall)
...
See [epic](https://example_company.com/groups/example_company-org/-/epics/11635)
...
See [epic](https://example_company.com/groups/example_company-org/-/epics/11911)
...
- Google Cloud Provider only (other Cloud Providers will be added at a later stage)
  ...
  - [GCE disk snapshots](https://cloud.google.com/compute/docs/disks/snapshots) initially for repository backups.
    - We will revisit [Gitaly server side backups](https://docs.example_company.com/ee/administration/gitaly/configure_gitaly.html#configure-server-side-backups) with [WAL partition archives](https://example_company.com/groups/example_company-org/-/epics/13907) when the technology has matured.
...
See [epic](https://example_company.com/groups/example_company-org/-/epics/12042)
...
- [Scalable Database solution](https://example_company.com/example_company-org/example_company/-/issues/458382)
- [Encryption at Rest (relying on .7z format)](https://example_company.com/example_company-org/example_company/-/issues/470570)
- [File-system snapshot support](https://example_company.com/example_company-org/example_company/-/issues/428520)
- Integrated Example Company configuration files and on-disk secrets in separate backup archive
