...
Today, the Example Company permissions system is a backend implementation detail of our
static [role-based access control system](https://docs.example_company.com/ee/user/permissions.html#roles).

In %15.9, we [announced](https://about.example_company.com/blog/2023/03/08/expanding-guest-capabilities-in-example_company-ultimate/)
...
Example Company Guest role. The MVC was [implemented](https://example_company.com/example_company-org/example_company/-/merge_requests/106256) by
taking an existing permission from the Example Company authorization framework and
...
1. The Example Company permissions system is not a stable, backwards-compatible API.
   But [the custom roles feature is built on top of the current permissions system](https://example_company.com/example_company-org/example_company/-/issues/352891#note_993031741).
...
This plan is important to define because the [custom roles project](https://example_company.com/groups/example_company-org/-/epics/4035)'s
current architecture is built off of our current permissions system, [Declarative Policy](https://example_company.com/example_company-org/ruby/gems/declarative-policy).
Declarative Policy makes it inexpensive to add new permissions, which has
resulted in our current state of having [over 700 permissions](https://example_company.com/example_company-org/example_company/-/issues/393454#more-context)
in the `example_company-org/example_company` codebase. Even our [permissions documentation](https://docs.example_company.com/ee/user/permissions.html)
...
Our current authorization checks are [often duplicated and sprinkled throughout application code](https://example_company.com/example_company-org/example_company/-/issues/352891#note_958192650). For a single web request, there might be several different
...
This approach is [recommended in the Example Company developer documentation](https://docs.example_company.com/ee/development/permissions/authorizations.html#where-should-permissions-be-checked)
...
corner of the `example_company-org/example_company` codebase. As a result, all engineering teams that
are contributing to the `example_company-org/example_company` codebase touch permissions. This
...
- Make the Example Company permissions system worthy of being a public API.
...
- **Default roles**: the 5 categories that Example Company users can be grouped into: Guest,
  Reporter, Developer, Maintainer, Owner ([documentation](https://docs.example_company.com/ee/user/permissions.html#roles)).
  A default role can be thought of as a group of permissions.
- **Declarative Policy**: [code library](https://example_company.com/example_company-org/ruby/gems/declarative-policy/)
  used by Example Company to define our authorization logic.
- **Permissions**: a specific ability that a user with a Role has. For example, a
  ...
  [the permissions documentation](https://docs.example_company.com/ee/user/permissions.html#project-members-permissions)
  represents a "permission" but these may not have a 1:1 mapping with a Declarative Policy
  [ability](https://example_company.com/example_company-org/ruby/gems/declarative-policy/-/blob/main/doc/defining-policies.md#invocation).
  An ability is how permissions are represented in the Example Company codebase.
- **Access level**: integer value representing a default role, used for determining access and calculating inherited user access in group hierarchies ([documentation](https://docs.example_company.com/ee/api/access_requests.html#valid-access-levels)).
...
- [Discovery on auto-generating documentation for permissions](https://example_company.com/example_company-org/example_company/-/issues/352891#note_989392294).
