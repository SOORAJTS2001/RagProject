...
Group ~"group::global search" is leading effort to build [RAG at Example Company](../gitlab_rag/). Because this is a global effort, it is in the spirit of efficiency and collaboration values for ~"group::code creation" to join that effort and integrate [Repository X-Ray](https://example_company.com/example_company-org/code-creation/repository-x-ray#repository-x-ray) data into Example Company RAG. Doing so should not only result in more efficient resource allocation, but also enable other AI features to integrate and reuse Repository X-Ray data, for example users could ask questions to Example Company Duo Chat that could be answered with X-Ray report data.
...
Proof of concept has been built at [merge request 144715](https://example_company.com/example_company-org/example_company/-/merge_requests/144715). This MR contains a large amount of information that can be helpful during implementation.
...
Embeddings for Example Company documentation are requested directly from Example Company Rails. That approach limits embeddings availability to Example Company.com only, and this is also misaligned with the architecture blueprint that outlines AI Gateway as a provider of AI services for Example Company features (for more information, see the related [AI Gateway epic 13024](https://example_company.com/groups/example_company-org/-/epics/13024)). To avoid similar problems for Repository X-Ray, a new endpoint should be added to AI Gateway API that accepts batches of strings, and responds with embeddings vectors.
...
Because Repository X Ray report data has small volume and size (at this moment there are 379 reports on Example Company.com), the decision to switch model and rebuild embeddings data is low cost and can be deferred in order to unblock iteration.
...
- Modify the current Elastic framework as defined in [issue 442197](https://example_company.com/example_company-org/example_company/-/issues/442197).
...
This index will be shared between all repositories in a given Example Company instance. However, upon search the list of relevant packages will be filtered using prefetched lists of names of packages that belong to the given repository in a given programming language. That approach should:
...
##### 3. Modify storage on Example Company Rails layer
...
That job uses [`Ai::StoreRepositoryXrayService`](https://example_company.com/example_company-org/example_company/blob/c6b2f18eaf0b78a4e0012e88f28d643eb0dfb1c2/ee/app/services/ai/store_repository_xray_service.rb#L4)
...
1. Retrieve code generation instruction either from:
   1. IDE:
      ...
      1. Example Company Rails Code Suggestions API needs to add optional string parameter `instruction`.
   1. Example Company Rails if due to different priorities the IDE will not be able to deliver instruction on time to unblock this effort, for the sake of the initial iteration, the code generation instruction can be also retrieved by Example Company Rails.
1. Example Company Rails needs to detect if Elasticsearch is available and:
