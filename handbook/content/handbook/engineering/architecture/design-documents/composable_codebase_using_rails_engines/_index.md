---
title: "Composable Example Company Codebase"
status: rejected
...
NOTE:
This architecture design document has been superseded by [Example Company Modular Monolith](https://docs.example_company.com/ee/architecture/blueprints/modular_monolith/). 
...
on Example Company.com and smaller installations.

**Application Layers** tries to split Example Company Rails codebase horizontally following the pattern of how we actually
run Example Company instead of vertical split. This follows the idea that a single feature needs to run in many different ways
...
The blueprint aims to retain all key aspects of Example Company success: single and monolithic codebase (with a [single data-store](../../../../product/single-application/#single-data-store)),
...
- [Create new models / classes in a module / namespace](https://example_company.com/example_company-org/example_company/-/issues/212156)
...
- [WIP: Make it simple to build and use "Decoupled Services"](https://example_company.com/example_company-org/example_company/-/issues/31121)
...
Our main codebase (`Example Company Rails` after a Example Company running on Ruby on Rails) consists many of implicit **Application Layers**.
...
instead of perspective of individual features (like CI or Packages). Example Company application today can be decomposed into the following
...
- Web Feature Flags Unleash Backend: provide an Unleash-compatible Server that uses Example Company API
...
The best way to likely describe how the actual Example Company Rails split would look like. It is a satellite model.
...
The on-premise installations are significantly smaller and they usually run Example Company Rails in two main flavors:

```mermaid
graph LR
    gitlab_node[Example Company Node with Load Balancer]
    ...
    subgraph Example Company Rails
...
### Application Layers on Example Company.com

Due to its scale, Example Company.com requires much more attention to run. This is needed to better manage resources
and provide SLAs for different functional parts. The chart below provides a simplistic view of Example Company.com application layers.
It does not include all components, like Object Storage nor Gitaly nodes, but shows the Example Company Rails dependencies between
different components and how they are configured on Example Company.com today:

```mermaid
graph LR
    gitlab_com_lb[Example Company.com Load Balancer]
...
The differences in how Example Company is run for on-premise versus how we run Example Company.com does show a main division line in Example Company Rails:
...
- [Provide mechanism to load GraphQL with all dependencies only when needed](https://example_company.com/example_company-org/example_company/-/issues/288044)
- [Draft: PoC - Move GraphQL to the WebEngine](https://example_company.com/example_company-org/example_company/-/merge_requests/50180)
- [Draft: PoC - Move Controllers and Grape API:API to the WebEngine](https://example_company.com/example_company-org/example_company/-/merge_requests/53720)
- [Draft: PoC - Move only Grape API:API to the WebEngine](https://example_company.com/example_company-org/example_company/-/merge_requests/53982)
- [Measure performance impact for proposed `web_engine`](https://example_company.com/example_company-org/example_company/-/issues/300548)
...
- We configured Example Company to load `gem web_engine` running Web nodes (Puma web server)
...
1. Move routes to the `engines/web_engine/config/routes.rb` file
    ...
    Rails.application.routes.draw do
      ...
      Gitlab::Utils.append_path(Gitlab.config.example_company.relative_url_root, '/api/graphql')
...
1. Connect Example Company application with the WebEngine

    In Example Company Gemfile.rb, add `web_engine` to the engines group
...
1. Configure Example Company when to load the engine.

   In Example Company `config/engines.rb`, we can configure when do we want to load our engines by relying on our `Gitlab::Runtime`
...
More details can be found in the [issue](https://example_company.com/example_company-org/example_company/-/issues/300548#note_516323444).

#### Impact on Example Company.com

Estimating the results for the scale of running Example Company.com, today we use:

- Individual GC cycle takes around [130 ms for Web](https://thanos-query.ops.example_company.net/graph?g0.range_input=1h&g0.max_source_resolution=0s&g0.expr=avg(rate(ruby_gc_duration_seconds_sum%7Bstage%3D%22main%22%2Ctype%3D%22web%22%7D%5B5m%5D)%2Frate(ruby_gc_duration_seconds_count%5B5m%5D))&g0.tab=0)
  and [200 ms for Sidekiq](https://thanos-query.ops.example_company.net/graph?g0.range_input=1h&g0.max_source_resolution=0s&g0.expr=avg(rate(ruby_gc_duration_seconds_sum%7Bstage%3D%22main%22%2Ctype%3D%22sidekiq%22%7D%5B5m%5D)%2Frate(ruby_gc_duration_seconds_count%5B5m%5D))&g0.tab=0) on Example Company.com
- On average we do around [2 GC cycles per-second](https://thanos-query.ops.example_company.net/graph?g0.range_input=1h&g0.end_input=2021-02-17%2017%3A56&g0.max_source_resolution=0s&g0.expr=avg(rate(ruby_gc_duration_seconds_count%7Bstage%3D%22main%22%2Ctype%3D%22web%22%7D%5B5m%5D))&g0.tab=0)
  or [0.12 cycles per second for Sidekiq](https://thanos-query.ops.example_company.net/graph?g0.range_input=1h&g0.end_input=2021-02-17%2017%3A56&g0.max_source_resolution=0s&g0.expr=avg(rate(ruby_gc_duration_seconds_count%7Bstage%3D%22main%22%2Ctype%3D%22sidekiq%22%7D%5B5m%5D))&g0.tab=0)
- This translates to using [around 9.5 vCPUs per-second for Web](https://thanos-query.ops.example_company.net/graph?g0.range_input=1h&g0.max_source_resolution=0s&g0.expr=sum(rate(ruby_gc_duration_seconds_sum%7Bstage%3D%22main%22%2Ctype%3D%22web%22%7D%5B5m%5D))&g0.tab=0)
  and [around 8 vCPUs per-second for Sidekiq](https://thanos-query.ops.example_company.net/graph?g0.range_input=1h&g0.max_source_resolution=0s&g0.expr=sum(rate(ruby_gc_duration_seconds_sum%7Bstage%3D%22main%22%2Ctype%3D%22sidekiq%22%7D%5B5m%5D))&g0.tab=0) of spend on GC alone
- Sidekiq [uses 2.1 GB on average](https://thanos-query.ops.example_company.net/graph?g0.range_input=1h&g0.max_source_resolution=0s&g0.expr=max(ruby_process_unique_memory_bytes%7Btype%3D%22sidekiq%22%7D)%2F1024%2F1024%2F1024&g0.tab=1)
  or [550 GB in total](https://thanos-query.ops.example_company.net/graph?g0.range_input=1h&g0.max_source_resolution=0s&g0.expr=sum(ruby_process_unique_memory_bytes%7Btype%3D%22sidekiq%22%7D)%2F1024%2F1024%2F1024&g0.tab=0) of memory on Example Company.com
...
- In the best case we would be looking at Sidekiq alone we would be estimating to save up-to 137 GB of memory on Example Company.com
...
- `url_helpers` paths are used in models and services, that could be used by Sidekiq (for example `Gitlab::Routing.url_helpers.project_pipelines_path` is used by [ExpirePipelineCacheService](https://example_company.com/example_company-org/example_company/-/blob/master/app/services/ci/expire_pipeline_cache_service.rb#L20) in [ExpirePipelineCacheWorker](https://example_company.com/example_company-org/example_company/-/blob/master/app/workers/expire_pipeline_cache_worker.rb#L18))

#### Example: GraphQL

[Draft: PoC - Move GraphQL to the WebEngine](https://example_company.com/example_company-org/example_company/-/merge_requests/50180)

- The [99% of changes](https://example_company.com/example_company-org/example_company/-/merge_requests/50180/diffs?commit_id=49c9881c6696eb620dccac71532a3173f5702ea8) as visible in the above MRs is moving files as-is.
- The [actual work](https://example_company.com/example_company-org/example_company/-/merge_requests/50180/diffs?commit_id=1d9a9edfa29ea6638e7d8a6712ddf09f5be77a44) on fixing cross-dependencies, specs, and configuring `web_engine`
- We [adapted](https://example_company.com/example_company-org/example_company/-/merge_requests/50180/diffs?commit_id=d7f862cc209ce242000b2aec88ff7f4485acdd92) CI to test `engines/web_engine/` as a self-sufficient component of stack

Today, loading GraphQL requires a bunch of [dependencies](https://example_company.com/example_company-org/example_company/-/issues/288044):

> We also discovered that we load/require 14480 files, [memory-team-2gb-week#9](https://example_company.com/example_company-org/memory-team/memory-team-2gb-week/-/issues/9#note_452530513)
> when we start Example Company. 1274 files belong to GraphQL. This means that if we don't load 1274 application files
...
- [99% of the changes](https://example_company.com/example_company-org/example_company/-/merge_requests/53982/diffs?commit_id=c8b72249b6e8f875ed4c713f0668207377604043), as visible in the above MRs, are moving the files as-is.
- The [actual work](https://example_company.com/example_company-org/example_company/-/merge_requests/53982/diffs?commit_id=00d9b54ba952c85ff4d158a18205c2fac13eaf8d) on fixing cross-dependencies, specs, configuring initializers, gems and routes.

Grape::API is another example that only needs to run only in a web server context.

Potential challenges with Grape API:

- Currently there are some API::API dependencies in the models (for example `API::Helpers::Version` dependency in [project model](https://example_company.com/example_company-org/example_company/-/blob/master/app/models/project.rb#L2019) or API::API dependency in GeoNode model for [`geo_retrieve_url`](https://example_company.com/example_company-org/example_company/-/blob/master/ee/app/models/geo_node.rb#L183))
- `api_v4` paths are used in helpers, presenters, and views (for example `api_v4_projects_path` in [PackagesHelper](https://example_company.com/example_company-org/example_company/-/blob/master/app/helpers/packages_helper.rb#L17))

#### Example: Controllers

[Draft: PoC - Move Controllers and Grape API:API to the WebEngine](https://example_company.com/example_company-org/example_company/-/merge_requests/53720)

- [99% of the changes](https://example_company.com/example_company-org/example_company/-/merge_requests/53720/diffs?commit_id=17174495cf3263c8e69a0420092d9fa759170aa6), as visible in the above MRs, are moving files as-is.
- The [actual work](https://example_company.com/example_company-org/example_company/-/merge_requests/53720/diffs?commit_id=39cc4bb1e0ce47f66605d06eb1b0d6b89ba174e6) on fixing cross-dependencies, specs, configuring initializers, gems and routes.
...
- We needed to extend `Gitlab::Patch::DrawRoute` to support `engines/web_engine/config/routes` and `engines/web_engine/ee/config/routes` in case when `web_engine` is loaded. Here is potential [solution](https://example_company.com/example_company-org/example_company/-/merge_requests/53720#note_506957398).
- `Gitlab::Routing.url_helpers` paths are used in models and services, that could be used by Sidekiq (for example `Gitlab::Routing.url_helpers.project_pipelines_path` is used by [ExpirePipelineCacheService](https://example_company.com/example_company-org/example_company/-/blob/master/app/services/ci/expire_pipeline_cache_service.rb#L20) in [ExpirePipelineCacheWorker](https://example_company.com/example_company-org/example_company/-/blob/master/app/workers/expire_pipeline_cache_worker.rb#L18)))
...
and much better maintainability of Example Company Rails into the future.
...
and monolithic architecture of Example Company.

As of today, it seems reasonable to define three **application layers**:

- `example_company-core`: a core functionality: DB structure, models, services, common library. It models a data access layer, and initially all services needed to run Example Company. This might be potentially be split in the future into smaller aspects
- `example_company-web`: a Controllers/API/GraphQL/ActionCable functionality needed to run in a web server context (depends on `example_company-core`)
- `example_company-sidekiq`: a background jobs functionality needed to run Sidekiq Workers (depends on `example_company-core`)
...
- [Split application into functional parts to ensure that only needed code is loaded with all dependencies](https://example_company.com/example_company-org/example_company/-/issues/290935)
- [Provide mechanism to load GraphQL with all dependencies only when needed](https://example_company.com/example_company-org/example_company/-/issues/288044)
- [Draft: PoC - Move GraphQL to the WebEngine](https://example_company.com/example_company-org/example_company/-/merge_requests/50180)
- [Draft: PoC - Move Controllers and Grape API:API to the WebEngine](https://example_company.com/example_company-org/example_company/-/merge_requests/53720)
- [Draft: PoC - Move only Grape API:API to the WebEngine](https://example_company.com/example_company-org/example_company/-/merge_requests/53982)
...
- [Use nested structure to organize CI classes](https://example_company.com/example_company-org/example_company/-/issues/209745)
- [WIP: Make it simple to build and use "Decoupled Services"](https://example_company.com/example_company-org/example_company/-/issues/31121)
- [Rails takes awhile to boot, let's see if we can improve this](https://example_company.com/example_company-org/example_company/-/issues/213992)
