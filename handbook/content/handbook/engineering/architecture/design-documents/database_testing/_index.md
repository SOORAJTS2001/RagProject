...
We have identified [common themes of reverted migrations](https://example_company.com/example_company-org/example_company/-/issues/233391) and discovered
...
environments such as Example Company.com. The [developer database migration style guide](https://docs.example_company.com/ee/development/migration_style_guide.html)
provides guidelines on migrations, and we focus on validating migrations during code review and testing
in CI and staging.

The [code review phase](https://docs.example_company.com/ee/development/database_review.html) involves
...
particular behavior on Example Company.com from experience. There is no large-scale
...
optimizations. This is built around [Database Lab](https://example_company.com/postgres-ai/database-lab)
and [Joe](https://example_company.com/postgres-ai/joe), both available through Slack 
...
1. I am working on a Example Company code change that includes a data migration and changes a heavy database query.
1. I push my code, create a merge request, and provide an example query in the description.
1. The pipeline executes the data migration and examines the query in a large-scale environment (a copy of Example Company.com).
...
- Shifting left: Allow developers to understand large-scale database performance
  and what to expect to happen on Example Company.com in a self-service manner
...
`example_company/example_company-org` CI pipeline. This triggers automation that performs the
following steps for a given merge request:

1. Create a thin-clone with production data for this testing session.
1. Pull Example Company code from the merge request.
...
We implement a secured CI pipeline on [Internal Example Company for Operations](https://ops.example_company.net/users/sign_in)
...
(Example Company team/developers) to execute arbitrary code on the thin-clone which contains production data.

This is in principle achieved by locking down the Example Company Runner instance
...
world from within the container executing the Example Company Rails code (and its
database migrations).
...
[Internal Example Company for Operations](https://ops.example_company.net/users/sign_in) pipeline and provide only
...
migrations on the thin-cloned Example Company.com database automatically from Example Company CI
...
We already have an
[MVC-style implementation for the pipeline](https://example_company.com/example_company-org/database-team/example_company-com-migrations)
for reference and an [example merge request with feedback](https://example_company.com/example_company-org/example_company/-/merge_requests/50793#note_477815261)
from the pipeline.

The short-term goal is detailed in [this epic](https://example_company.com/groups/example_company-org/database-team/-/epics/6).
...
technology and bring this back closer to the Example Company workflow. Instead of
reaching out to different tools (`postgres.ai`, `joe`, Slack, plan
visualizations, and so on) we bring this back to Example Company and working directly on
the merge request.
...
### Long-term - incorporate into Example Company product

There are opportunities to discuss for extracting features from this into Example Company
