...
KEV support in Example Company aims to enhance vulnerability prioritization and
remediation efforts by highlighting these high-risk vulnerabilities.
The KEV support requirements are outlined
in [the KEV epic](https://example_company.com/groups/example_company-org/-/epics/11912). This
...
The goal is to make KEV information accessible through the Example Company GraphQL API,
...
benefits to the Example Company platform, enabling more effective and efficient
...
- Enable users to use KEV data on Example Company as another metric for their
...
Support KEV on the Example Company platform.

Following the discussions in
the [KEV epic](https://example_company.com/groups/example_company-org/-/epics/11912), the proposed
flow is:
...
1. Example Company instances pull the CVE enrichments data from the bucket.
1. Example Company instances store KEV and EPSS data in a new `pm_cve_enrichments` table.
1. Example Company instances expose KEV status through GraphQL API and present data in
...
flowchart LR
    ...
    Example Company --> |stores| GitLabDB[(pm_cve_enrichments)]
```
...
- Create a new KEV table
  in [PMDB](https://example_company.com/example_company-org/security-products/license-db) with an
  advisory identifier. This includes changing
  the [schema](https://example_company.com/example_company-org/security-products/license-db/schema)
...
- Create a new `pm_cve_enrichments` table in the Example Company database to store
...
- **PMDB** (Package metadata database, also known as License DB): PMDB is a
  ...
  application, that gathers, stores and exports packages metadata for Example Company
  instances to consume.
  See [complete documentation](https://example_company.com/example_company-org/security-products/license-db/deployment/-/blob/main/docs/DESIGN.md?ref_type=heads).
