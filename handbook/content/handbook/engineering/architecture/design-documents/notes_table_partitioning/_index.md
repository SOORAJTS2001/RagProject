...
The `notes` table is one of the largest PostgreSQL DB tables in Example Company,
whose size in the `.com` production database exceeded 1.5TB as of Nov 2023,
[increasingly endangering](https://example_company.com/groups/example_company-org/-/epics/6211)
the reliability of Example Company.com and large self-managed instances.
...
[reduce contention on the lock_manager lwlocks](https://example_company.com/example_company-com/gl-infra/scalability/-/issues/2301).
...
- Addresses [polymorphic associations](https://docs.example_company.com/ee/development/database/polymorphic_associations.html)
...
`namespace_id` could be [the better choice as the sole hash key](https://example_company.com/example_company-org/example_company/-/issues/416127#note_1467349405)
...
could be [partitioned by int range](https://docs.example_company.com/ee/development/database/partitioning/int_range.html)
...
A small number of `notes` records without `noteable_type` have been found and removed from Example Company.com's production database.
The non-null check constraint is [planned to be validated in 17.0](https://example_company.com/example_company-org/example_company/-/issues/443667)
