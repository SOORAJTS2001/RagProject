...
This is especially true for Example Company.com. Our goal is to have a reasonable and
...
1. Build a framework to define and enforce limits in Example Company Rails.
2. Build an API to consume limits in satellite service and expose them to users.
3. Extract parts of this framework into a dedicated Example Company Limits Service.
...
1. Consolidate on the application limits tooling used in Example Company Rails.
...
tooling as a Example Company feature to unlock these consolidation benefits for our
...
limiting architecture for Example Company.com. We refer to this architectural evolution
...
application limits, in the Example Company Rails project context, in a more consistent
...
their needs to customer success team at Example Company, and we will be able to
communicate how the responsible usage is defined at a given moment.

Because of how Example Company architecture has been built, Example Company Rails application, in
...
Example Company Runners, Gitaly, Workhorse, KAS could use the API to receive a set of
...
traffic will reach Example Company Rails and subsequently also other satellite services.
...
### Example Company Policy Service

_Disclaimer_: Extracting a Example Company Policy Service might be out of scope
...
[OPA - Open Policy Agent](https://www.openpolicyagent.org/). At Example Company we are
...
Example Company, but to also transform the Next Rate Limiting Architecture into
...
[Pipeline Validation Service](https://example_company.com/example_company-org/modelops/anti-abuse/pipeline-validation-service).
There is a significant opportunity here in transforming Pipeline Validation
Service into a general purpose Example Company Policy Service / Example Company Policy Agent that
will be well integrated into the Example Company product itself.

Generalizing Pipeline Validation Service into Example Company Policy Service can bring a
few interesting benefits:

1. Consolidate on our tooling across the company to improve efficiency.
1. Integrate our Example Company Rails limits framework to resolve policies using the policy service.
...
1. Make Example Company Policy Service part of the product and integrate it into the single application.

We envision using Example Company Policy Service to be place to define policies that do
...
The Example Company Policy Service might be used in two different ways:
...
internally-defined policies to ensure the stability / availability of a Example Company
instance (Example Company.com or self-managed instance). The second use-case is about
making Example Company Policy Service a feature that users will be able to build on top
of.
...
It is possible that Example Company Policy Service and Decoupled Limits Service can
...
Example Company application aggregates users, projects, groups and namespaces in a
...
> Example Company will read default limits for every operation, all overrides configured
...
- 2022-06-07: Working Group members [started submitting technical proposals](https://example_company.com/example_company-org/example_company/-/issues/364524) for the next rate limiting architecture.
...
- 2022-07-06: A fourth, [consolidated proposal](https://example_company.com/example_company-org/example_company/-/issues/364524#note_1017640650), has been submitted.
