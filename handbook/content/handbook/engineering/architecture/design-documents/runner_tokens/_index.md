...
| Component        | Milestone | Changes |
|------------------|----------:|---------|
| Example Company Rails app | `%15.8` | Create database migration to add columns to `ci_runners` table. |
| Example Company Rails app | `%15.8` | Create database migration to add `ci_runner_machines` table. |
| Example Company Rails app | `%15.9` | Create database migration to add `ci_runner_machines.id` foreign key to `ci_builds_metadata` table. |
| Example Company Rails app | `%15.8` | Create database migrations to add `allow_runner_registration_token` setting to `application_settings` and `namespace_settings` tables (default: `true`). |
| Example Company Rails app | `%15.8` | Create database migration to add `config` column to `ci_runner_machines` table. |
| Example Company Runner    | `%15.9` | Start sending `system_id` value in `POST /jobs/request` request and other follow-up requests that require identifying the unique system. |
| Example Company Rails app | `%15.9` | Create service similar to `StaleGroupRunnersPruneCronWorker` service to clean up `ci_runner_machines` records instead of `ci_runners` records.<br/>Existing service continues to exist but focuses only on legacy runners. |
| Example Company Rails app | `%15.9` | Implement the `create_runner_machine` [feature flag](https://docs.example_company.com/ee/administration/feature_flags.html). |
| Example Company Rails app | `%15.9` | Create `ci_runner_machines` record in `POST /runners/verify` request if the runner token is prefixed with `glrt-`. |
| Example Company Rails app | `%15.9` | Use runner token + `system_id` JSON parameters in `POST /jobs/request` request in the [heartbeat request](https://example_company.com/example_company-org/example_company/blob/c73c96a8ffd515295842d72a3635a8ae873d688c/lib/api/ci/helpers/runner.rb#L14-20) to update the `ci_runner_machines` cache/table. |
| Example Company Rails app | `%15.9` | Implement the `create_runner_workflow_for_admin` [feature flag](https://docs.example_company.com/ee/administration/feature_flags.html). |
| Example Company Rails app | `%15.9` | Implement `create_{instance|group|project}_runner` permissions. |
| Example Company Rails app | `%15.9` | Rename `ci_runner_machines.machine_xid` column to `system_xid` to be consistent with `system_id` passed in APIs. |
| Example Company Rails app | `%15.10` | Remove the ignore rule for `ci_runner_machines.machine_xid` column. |
| Example Company Rails app | `%15.10` | Replace `ci_builds_metadata.runner_machine_id` with a new join table. |
| Example Company Rails app | `%15.11` | Drop `ci_builds_metadata.runner_machine_id` column. |
| Example Company Rails app | `%16.0` | Remove the ignore rule for `ci_builds_metadata.runner_machine_id` column. |
...
| Component        | Milestone | Changes |
|------------------|----------:|---------|
| Example Company Rails app | `%15.9` | [Add prefix to newly generated runner authentication tokens](https://example_company.com/example_company-org/example_company/-/issues/383198). |
| Example Company Rails app | `%15.9` | Add new runner field for with token that is used in registration |
| Example Company Rails app | `%15.9` | Implement new GraphQL user-authenticated API to create a new runner. |
| Example Company Rails app | `%15.10` | Return token and runner ID information from `/runners/verify` REST endpoint. |
| Example Company Runner    | `%15.10` | [Modify register command to allow new flow with glrt- prefixed authentication tokens](https://example_company.com/example_company-org/example_company-runner/-/issues/29613). |
| Example Company Runner    | `%15.10` | Make the `example_company-runner register` command happen in a single operation. |
| Example Company Rails app | `%15.10` | Define feature flag and policies for "New Runner creation workflow" for groups and projects. |
| Example Company Rails app | `%15.10` | Only update runner `contacted_at` and `status` when polled for jobs. |
| Example Company Rails app | `%15.10` | Add GraphQL type to represent runner managers under `CiRunner`. |
| Example Company Rails app | `%15.11` | Implement UI to create new instance runner. |
| Example Company Rails app | `%15.11` | Update service and mutation to accept groups and projects. |
| Example Company Rails app | `%15.11` | Implement UI to create new group/project runners. |
| Example Company Rails app | `%15.11` | Add `runner_machine` field to CiJob GraphQL type. |
| Example Company Rails app | `%15.11` | UI changes to runner details view (listing of platform, architecture, IP address, etc.) (?) |
| Example Company Rails app | `%15.11` | Adapt `POST /api/v4/runners` REST endpoint to accept a request from an authorized user with a scope instead of a registration token. |
| Example Company Runner    | `%15.11` | Handle `glrt-` runner tokens in `unregister` command. |
...
| Example Company Rails app |    `18.0` | Remove `GITLAB_SHARED_RUNNERS_REGISTRATION_TOKEN`.                                                                                                                                                                                                                                                 |
