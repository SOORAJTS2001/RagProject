...
The latest iteration of the [container registry](https://example_company.com/example_company-org/container-registry)
has been rearchitected to use a PostgreSQL database and deployed on Example Company.com.
...
Additionally, unifying the registry used for Example Company.com and for self-managed
...
functionality, which was used to migrate millions of container images on Example Company.com,
...
of the container registry for both Example Company.com and for self-managed users.
...
container images on Example Company.com. Additionally, we should provide estimated import
...
is fully implemented. Certain database enabled features are only enabled for Example Company.com
...
| --------------------------- | ------------------------------------------------------------------- | ------------------ | ---------------------------------------------------------------------------------------------- |
| Import Tool                 | Allows existing deployments to migrate to the database.             | Completed          | [Import Tool](https://example_company.com/example_company-org/container-registry/-/issues/884)                   |
| Automatic Import Validation | Tests that the import maintained data integrity of imported images. | Backlog            | [Validate self-managed imports](https://example_company.com/example_company-org/container-registry/-/issues/938) |
...
Additionally, implementing [Validate self-managed imports](https://example_company.com/example_company-org/container-registry/-/issues/938)
...
The [import tool](https://example_company.com/example_company-org/container-registry/-/blob/master/docs/database-import-tool.md)
...
with both minimal risk and with minimal support from Example Company team members.
Therefore, the most important work remaining is crafting the UX of this tooling
such that those goals are met. This
[epic](https://example_company.com/groups/example_company-org/-/epics/8602) captures many of the
...
Example Company.com, along with an additional step to catalog all unreferenced blobs held
in common storage.
...
to stop writes to the registry, the tag import step can be ran. As with the Example Company.com
...
Example Company version bump before enabling the database by default for self-managed.
...
- The registry on Example Company.com and the registry used by self-managed will greatly diverge in supported features over time.
...
- The registry on Example Company.com stops being an effective way to validate changes before they are released to self-managed.
- Large self-managed users continue to not be able to scale the registry to suit their needs.

### Gradual Migration

This approach would be to exactly replicate the Example Company.com migration on
self-managed.
