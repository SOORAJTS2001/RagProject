...
All Cell-local data and functionality in Example Company (all data except the few
...
Many features in Example Company can link data together.
A few examples of things that would be disallowed by Organization Isolation are:

1. [Related issues](https://docs.example_company.com/ee/user/project/issues/related_issues.html): Users would not be able to take an issue in one Project in `Organization A` and relate that issue to another issue in a Project in `Organization B`.
1. [Share a project/group with a group](https://docs.example_company.com/ee/user/group/manage.html#share-a-group-with-another-group): Users would not be allowed to share a Group or Project in `Organization A` with another Group or Project in `Organization B`.
1. [System notes](https://docs.example_company.com/ee/user/project/system_notes.html): Users would not get a system note added to an issue in `Organization A` if it is mentioned in a comment on an issue in `Organization B`.

## Why?

![Broken Organization Isolation](../diagrams/organization-isolation-broken.drawio.png)

[Example Company Cells](https://docs.example_company.com/ee/architecture/blueprints/cells/index.html) depend on using the Organization as the sharding key, which will allow us to shard data between different Cells.
Initially, when we start rolling out Organizations, we will be working with a single Cell `Cell 1`.
`Cell 1` is our current Example Company.com deployment.
...
Migrating workloads off will be critical to allowing us to rebalance our data across a fleet of servers and eventually run much smaller Example Company instances (and databases).
...
Multiple POCs have been implemented to demonstrate how we will provide robust developer facing and customer facing constraints in the Example Company application and database that enforce the described isolation constraint.
These are:

1. [Enforce Organization Isolation based on `project_id` and `namespace_id` column on every table](https://example_company.com/example_company-org/example_company/-/merge_requests/133576)
1. [Enforce Organization Isolation based on `organization_id` on every table](https://example_company.com/example_company-org/example_company/-/merge_requests/129889)
1. [Validate if a top-level group is isolated to be migrated to an Organization](https://example_company.com/example_company-org/example_company/-/merge_requests/131968)

The major constraint these POCs were trying to overcome was that there is no standard way in the Example Company application or database to even determine what Organization (or Project or namespace) a piece of data belongs to.
...
[Organization Isolation - Phase 1](https://example_company.com/groups/example_company-org/-/epics/11837) [Status: Completed]
...
[Organization Isolation - Phase 1.5](https://example_company.com/groups/example_company-org/-/epics/13678) - [Status: In Progress]
...
[Organization Isolation - Phase 2](https://example_company.com/groups/example_company-org/-/epics/11838) - [Status: Pending]

1. Once all tables have sharding keys we ship an
   evolved version of the
   [POC](https://example_company.com/example_company-org/example_company/-/merge_requests/133576), which
...
[Organization Isolation - Phase 3](https://example_company.com/groups/example_company-org/-/epics/11839) - [Status: Pending]

1. Implement new functionality in Example Company similar to the [POC](https://example_company.com/example_company-org/example_company/-/merge_requests/131968), which allows a namespace owner to see if their namespace is fully isolated.
...
The implementation of this effort will be tracked in [#11670](https://example_company.com/groups/example_company-org/-/epics/11670).
...
Features like [sharing a group with a group](https://docs.example_company.com/ee/user/group/manage.html#share-a-group-with-another-group) are very tightly connected to some of the worst performing functionality in our
