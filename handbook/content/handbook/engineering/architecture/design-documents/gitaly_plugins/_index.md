...
Gitaly is a database system for storing Git repositories and strives to be decoupled from rest of Example Company. While separation of concerns in general is a good practice, Gitaly is also used outside of Example Company. The decoupling functions as a guideline on what sort of functionality should be implemented in Gitaly. Only functionality supporting Gitaly's main goal of providing repository storage and access should be implemented directly in Gitaly.
...
This logic is not built directly into Gitaly to separate concerns. Gitaly calls [the Rails application's internal API](https://docs.example_company.com/ee/development/internal_api/index.html) for both cases:
...
In addition to calling Rails application's internal API, Gitaly supports [custom hooks](https://docs.example_company.com/ee/administration/server_hooks.html). Custom hooks are
...
- Internal API is used by Example Company.
- Custom hooks are used on self-managed instances to plug in custom logic.

Maintaining redundant functionality is a burden. Custom hooks not being used by Example Company means they are not thoroughly exercised in production on Example Company.com.
The internal API calls are essentially Example Company-specific hooks that are hard-coded into Gitaly.

#### Not all writes can be hooked into

Gitaly is hard-coded to invoke the custom logic only during certain write operations. These operations are decided by where Example Company needs to invoke custom
...
Custom logic is not invoked for other writes as Example Company doesn't need it. For example, excluded writes include repository creations, repository deletions, and reference updates through `WriteRef`.

This creates a tight coupling with the needs of Example Company.
...
- Remove Example Company-specific assumptions from the API.
  - Hard-coded calls to the internal API removed and moved behind the plugin interface.
  - Invoke the plugin for every write.
  - Remove Example Company specific fields such as `gl_repository` and `gl_project_path` from Gitaly's API.
...
The proposal is written with Gitaly's upcoming [transaction management](https://docs.example_company.com/ee/architecture/blueprints/gitaly_transaction_management/index.html) in mind.
...
message PluginRequest {
  ...
  message Header {
    ...
    //
    // This can be used to pipe Example Company specific data from Rails to the plugin, such as
...
Because Gitaly would support only configuring a single plugin, a question may arise on how the Example Company plugin could be configured along with a custom one. While running multiple
plugins wouldn't be directly supported, a custom plugin could be configured and the plugin could invoke the Example Company plugin. This would allow for plugging in custom logic
alongside the Example Company access check plugin. Because the custom plugin would control calling the Example Company plugin, it would have full control on whether to run Example Company plugin before,
...
1. Create a project for the Example Company access check plugin. This is where the existing internal API logic will be migrated.
...
Currently Gitaly does not guarantee the delivery of post-receive notifications ([#5411](https://example_company.com/example_company-org/gitaly/-/issues/5411)). The delivery could fail for any reason. For example, Gitaly could crash or
