...
Example Company CI/CD is one of the most data and compute intensive components of Example Company.
Since its initial release in 2012,
the CI/CD subsystem has evolved significantly. It was [integrated into Example Company in September 2015](https://about.example_company.com/releases/2015/09/22/example_company-8-0-released/)
...
Example Company CI/CD has come a long way since the initial release, but the design of
...
creating more than 5 million builds each day on Example Company.com we are reaching
database limits that are slowing our development velocity down.

On February 1st, 2021, Example Company.com surpassed 1 billion CI/CD builds created. In
...
[created the `ci_builds` table in 2012](https://example_company.com/example_company-org/example_company/-/blob/046b28312704f3131e72dcd2dbdacc5264d4aa62/db/ci/migrate/20121004165038_create_builds.rb).
...
happened without a viable workaround or an emergency plan, Example Company.com would go
...
See the [related Epic](https://example_company.com/groups/example_company-org/-/epics/5657) for more details.
...
Example Company.com depends on.

The size of the table also hinders development velocity because queries that
seem fine in the development environment may not work on Example Company.com. The
...
Example Company.com each day. In comparison to around 5M we see created today. This is
...
reliability. We evaluated using [Redis queuing](https://example_company.com/example_company-org/example_company/-/issues/322972), or
[a separate table that would accelerate SQL queries used to build queues](https://example_company.com/example_company-org/example_company/-/issues/322766).
We decided to proceed with the latter.

In October 2021 we finished shipping the new architecture of builds queuing
[on Example Company.com](https://example_company.com/groups/example_company-org/-/epics/5909#note_680407908).
We then made the new architecture [generally available](https://example_company.com/groups/example_company-org/-/epics/6954).
...
> Making Example Company CI/CD product ready for the scale we expect to see in the
...
[CI/CD Scaling](https://example_company.com/groups/example_company-org/-/epics/5745) epic.

1. ✓ Migrate primary keys to big integers on Example Company.com.
1. ✓ Implement the new architecture of builds queuing on Example Company.com.
1. ✓ [Make the new builds queuing architecture generally available](https://example_company.com/groups/example_company-org/-/epics/6954).
