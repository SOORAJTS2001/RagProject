---
title: "Example Company Duo Workflow"
status: ongoing
...
1. CI Executor: All non-local use-cases of Duo Workflow (for example:
   issue/epic based workflows) will be triggered by the Example Company UI and will
...
1. The Duo Workflow Web UI. This will be web UI built into Example Company that manages the
   creation and interaction of all workflows. There may be many interaction
   points in the Example Company application but there should be a central workflow UI
...
1. The Duo Workflow Service. This is a Python-based service we deploy with
   ...
   execute the workflows. For reasons why LangGraph was chosen, see [this work item](https://example_company.com/example_company-org/example_company/-/work_items/457958).
   ...
   Example Company. The Workflow Service is built [in its own codebase](https://example_company.com/example_company-org/duo-workflow/duo-workflow-service/)
   and will have its own deployment but the codebase
   [may be merged with the AI Gateway codebase in the future](https://example_company.com/example_company-org/modelops/applied-ml/code-suggestions/ai-assist/-/issues/527)
1. The [Duo Workflow Executor](https://example_company.com/example_company-org/duo-workflow/duo-workflow-executor).
...
1. All state management for workflows will be inside Example Company.
1. Duo Workflow Service is expected to periodically checkpoint its state in Example Company
...
1. In order to avoid multiple Duo Workflow Service instances running on the
   ...
   Example Company before it starts running. When it suspends, it will release the lock and
   similarly there will be a timeout state if it has not checkpointed in the
   last 60 seconds. Example Company will not accept checkpoints from a timed out run of
   the Duo Workflow Service.
1. Each time a Duo Workflow Service resumes a workflow it gets a new ID and
   this is sent when checkpointing so that Example Company can drop/ignore zombie
   services running the workflow and inform the zombie service to shutdown.
1. Code is checkpointed by the executor pushing hidden Git refs to the Example Company
...
1. For workflows triggered via the UI that don't require a Duo Workflow
   Executor Example Company can call the Duo Workflow Service directly
1. All API calls from Duo Workflow Service to Example Company that access private data
   or update data will be authenticated on behalf of the user that created the
   worklow. Duo Workflow Service should not need privileged access to Example Company
...
![Duo Workflow Architecture example_company-com](diagrams/duo-workflow-architecture-example_company-com.png)

1. Initially we focus on running locally and in CI pipelines with all inputs as
   environment variables
1. State stored in Example Company so it can be accessed from the web UI and through IDE
   extensions
...
sequenceDiagram
   ...
   participant gitlab_rails as Example Company Rails
...
similar to Example Company.com . This will also allow them to use whatever customer
...
Gateway will not have access to the customers Example Company instance but we can make
use of the local executor (on the user's machine or in a CI runner) to proxy
all interactions with Example Company.
...
1. We need a sandboxed environment to safely execute arbitrary commands
   generated by the AI
1. We need a path to proxy requests to self-managed Example Company instances that may
...
Example Company APIs. And if the customer is using Example Company.com or a self-hosted AI
Gateway that has access to their Example Company instance then we can safely run all of
this inside the Duo Workflow Service making API calls to the Example Company instance.
...
1. The workflow should know how to call Example Company directly for several API calls
   and especially checkpointing
1. Proxying to Example Company via the Executor should be designed as an optional proxy
   ...
   instead of calling Example Company directly
1. The Duo Workflow Executor is optional for workflows. The workflow should
   ...
   present. When the workflow suspends it should know to checkpoint in Example Company
   before shutting down.
1. The Duo Workflow Service will need to acquire some kind of "lease" from the
   Example Company instance while it is running to prevent the possibility of 2
...
triggering from the Example Company UI using CI pipelines except that Example Company will start
...
We don't want users to have to configure a specific `.example_company-ci.yml` in order
to support Duo Workflow. In order to avoid this we'll use the same approach as
[that used by DAST site validations](https://example_company.com/example_company-org/example_company/-/blob/19e0669446f55bd29a8df29174d3b0379b8e22c2/ee/app/services/app_sec/dast/site_validations/runner_service.rb#L11)
which dynamically constructs a pipeline configuration in Example Company and triggers
the pipeline without using any `.example_company-ci.yml`.
...
1. If the UX is poor over time we might abstract the user away from the
   ...
   impacting change to Example Company as projects are a central part of Example Company.
...
The Duo Workflow state will be persisted in Example Company-Rails as the Duo Workflow
...
We will be persisting the LangGraph state object using APIs in Example Company to
...
the POC <https://example_company.com/example_company-org/example_company/-/merge_requests/153551>.

For the current working directory which contains the code the agent has written
so far we will store this by pushing hidden Git refs to Example Company for the checkpoint. Each
...
1. The IDE will use the OAuth token of Personal Access Token (PAT) that the user
   generated while setting up the Example Company editor extension.
1. The IDE uses that token to authenticate a request to a Example Company Rails API
   endpoint to obtain a short-lived user- and system-scoped JWT.
1. When the Example Company Rails instance receives this request, it loads its
...
1. Example Company Rails returns this JWT to the IDE.
...
[token flow that allows IDEs to connect direct to the AI Gateway](https://example_company.com/groups/example_company-org/-/epics/13252).
...
A CI Pipeline is created by Example Company, so there is no need to query a Example Company Rails
API endpoint to obtain a short-lived user- and system-scoped JWT. Instead, in
the process of creating the CI pipeline, Example Company Rails will:
...
#### Duo Workflow Service -> Example Company Rails API

Reasons that the Duo Workflow Service must be able to authenticate requests to
the Example Company Rails API:

1. The Duo Workflow Service will need to periodically make requests to Example Company Rails
   to sync workflow state. This means that the Duo Workflow Service must be able
   to authenticate these requests.
1. Duo Workflow may need to make other Example Company Rails API queries to gather
...
1. The end state of a Duo Workflow may take the form of a generated artifact
   (for example, Git commit or pull request) on the Example Company platform. To
   generate this artifact, the Duo Workflow Service must be able to make API
   requests to Example Company Rails.
...
1. Any artifacts created by a Duo Workflow must be auditable in order
   to maintain transparency about AI-generated activities on the Example Company platform.
...
1. Need to update Example Company Rails to accept this type of token for API authentication.
...
1. Create a new token scope called `ai_workflows`
   ([related issue](https://example_company.com/example_company-org/example_company/-/issues/467160))
1. When the IDE requests the Duo Workflow Service User JWT from Example Company Rails, we
...
1. Duo Workflow executor will send that OAuth token, along with the `base_url`
   of the Example Company Rails instance, as metadata when the Duo Workflow Service when
   the gRPC connection is opened.
1. The Duo Workflow Service will use the OAuth token for any Example Company Rails API
...
1. It offers a rapid onboarding experience for self-managed customers to try
   Duo Workflow without deploying any new Example Company components
...
See <https://example_company.com/example_company-org/example_company/-/issues/457959> for a POC and investigation.
...
#### Example Company workspaces (remote development)

See <https://example_company.com/example_company-org/example_company/-/issues/458339> for a POC and investigation.
...
1. If we wanted to build out infrastructure on Example Company.com to save customers having to bring their own K8s cluster this would be a fairly large effort from a security and infrastructure perspective. It's possible but to deal with all the complexities of security, abuse and billing would require many teams involvement in both initial development and sustained maintenance.

## Security

### Threat modeling

See <https://example_company.com/example_company-com/gl-security/product-security/appsec/threat-models/-/issues/46>.
...
1. In Example Company Rails web UI
...
[Example Company Language Server](https://example_company.com/example_company-org/editor-extensions/example_company-lsp)
...
The Duo Workflow Service will persist it's state frequently to the main Example Company Rails
...
a workflow locally which runs in Example Company CI or they may be using the web UI to
...
and Executor, but instead all communication should be happening via Example Company. Any
...
1. Tools to integrate with the [Example Company HTTP API](https://docs.example_company.com/ee/api/api_resources.html)

The fact that the Duo Workflow Service is going to require Git and Example Company API tools entails that the **Duo Workflow Service
must have the ability to establish an SSH connection and make HTTP requests to the Example Company instance.** This ability can be granted directly to the Duo Workflow Service or can be provided via the Duo Workflow Executor if a direct connection between the Duo Workflow Service and a Example Company instance is not possible due to a firewall or network partition.
...
1. Workflow locking in Example Company to ensure only 1 concurrent instance of a
   workflow
