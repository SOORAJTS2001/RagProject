...
Example Company and Google Cloud have recently [announced](https://about.example_company.com/blog/2023/08/29/example_company-google-partnership-s3c/) a partnership to combine the unique capabilities of their platforms.

As highlighted in the announcement, one key goal is the ability to
"_use Google's Artifact Registry with Example Company pipelines and packaging to create a security data plane_".
...
[project integration](https://docs.example_company.com/ee/user/project/integrations/index.html) and display
[container image artifacts](https://cloud.google.com/artifact-registry/docs/supported-formats)
in the Example Company UI.

## Motivation

Refer to the [announcement](https://about.example_company.com/blog/2023/08/29/example_company-google-partnership-s3c/)
blog post for more details about the motivation and long-term goals of the Example Company and Google Cloud partnership.
...
research on the use of external registries as a complement to the Example Company
container registry ([internal](https://example_company.com/example_company-org/ux-research/-/issues/2602)).
...
- Allow Example Company users to configure a new [project integration](https://docs.example_company.com/ee/user/project/integrations/index.html) for connecting to GAR.
- Limited to a single top-level GAR [repository](https://cloud.google.com/artifact-registry/docs/repositories) per Example Company project.
...
- Use a Google Cloud [service account](https://cloud.google.com/iam/docs/service-account-overview) provided by the Example Company project owner/maintainer to interact with GAR.
- Allow Example Company users to list container images under the connected GAR repository, including sub-repositories. The list should be paginable and sortable.
...
- Listing container images under the connected GAR repository is restricted to users with [Reporter+](https://docs.example_company.com/ee/user/permissions.html#roles) roles.
...
As previously highlighted, access to the GAR integration features is restricted to users with [Reporter+](https://docs.example_company.com/ee/user/permissions.html#roles) roles.

#### Resource Mapping

For the [Example Company container registry](https://docs.example_company.com/ee/user/packages/container_registry/index.html),
repositories within a specific project must have a path that matches the project full path.
This is essentially how we establish a resource mapping between Example Company Rails and
...
Regarding the GAR integration, since there is no equivalent entities for Example Company
...
Example Company project, regardless of their respective paths. Similarly, we do not plan
to restrict the attachment of a particular GAR repository to a single Example Company
...
have allowed Example Company to reuse [existing logic](https://example_company.com/example_company-org/example_company/-/blob/20df77103147c0c8ff1c22a888516eba4bab3c46/lib/container_registry/client.rb)
...
Example Company had previously faced significant challenges with the last two limitations,
prompting the development of a custom
[Example Company container registry API](https://example_company.com/example_company-org/container-registry/-/blob/master/docs/spec/example_company/api.md)
to address them. Additionally, Example Company decided to
[deprecate support](https://docs.example_company.com/ee/update/deprecations.html#use-of-third-party-container-registries-is-deprecated)
...
Docker/OCI API endpoints with custom API endpoints for all container registry functionalities in Example Company.
