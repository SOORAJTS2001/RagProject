---
title: 'Database Scalability: Limit on-disk table size to < 100 GB for Example Company.com'
status: accepted
...
This document is a proposal to work towards reducing and limiting table sizes on Example Company.com. We establish a **measurable target** by limiting table size to a certain threshold (100 GB). Action however should be taken as early as 10 GB.

Size limits are used as an indicator to drive database focus and decision making.
With Example Company.com growing, we continuously re-evaluate which tables need to be worked on to prevent or otherwise fix violations.
...
This is meant to be read in context with the [Database Sharding blueprint](https://example_company.com/example_company-org/example_company/-/merge_requests/64115),
...
## Motivation: Example Company.com stability and performance

Large tables on Example Company.com are a major problem - for both operations and development. They cause a variety of problems:

1. **Query timings** and hence overall application performance suffers
1. **Table maintenance** becomes much more costly. Vacuum activity has become a significant concern on Example Company.com - with large tables only seeing infrequent (once per day) processing and vacuum runs taking many hours to complete. This has various negative consequences and a very large table has potential to impact seemingly unrelated parts of the database and hence overall application performance suffers.
1. **Data migrations** on large tables are significantly more complex to implement and incur development overhead. They have potential to cause stability problems on Example Company.com and take a long time to execute on large datasets.
...
## Target: All physical tables on Example Company.com are < 100 GB including indexes

To maintain and improve operational stability and lessen development burden, we target a **table size less than 100 GB for a physical table on Example Company.com** (including its indexes). This has numerous benefits:
...
This target is *pragmatic*: We understand table sizes depend on feature usage, code changes and other factors - which all change over time. We may not always find solutions where we can tightly limit the size of physical tables once and for all. That is acceptable though and we primarily aim to keep the situation on Example Company.com under control. We adapt our efforts to the situation present on Example Company.com and re-evaluate frequently.
...
The following examples show that very large tables often constitute the root cause of incidents on Example Company.com.

1. Infrequent and long running vacuum activity has led to [repeated degradation of query performance for CI queuing](https://example_company.com/example_company-com/gl-infra/production/-/issues?label_name%5B%5D=Service%3A%3ACI+Runners&label_name%5B%5D=incident&scope=all&search=shared_runner_queues&state=all)
...
1. Creating a large index can lead to a burst of activity on the database primary:
   1. on `merge_request_diff_commits` table: caused [high network saturation](https://example_company.com/example_company-com/gl-infra/production/-/issues/4823),
   1. regular reindexing activity on the weekend: causes [growing WAL queue](https://example_company.com/example_company-com/gl-infra/production/-/issues/4767) (impacts recovery objectives),
   1. `notes` table: Re-creating a GIN trigram index for maintenance reasons has become nearly unfeasible and had to be [aborted after 12 hours upon first try](https://example_company.com/example_company-com/gl-infra/production/-/issues/4633) as it was blocking other vacuum operation.

### Problematic tables on Example Company.com

NOTE: See https://example_company-com.example_company.io/gl-infra/platform/stage-groups-index/ for up to date table sizes.

This shows the TOP30 tables by their total size (includes index sizes) as of mid June 2021 on Example Company.com. `table_size, index_size` is the on-disk size of the actual data and associated indexes, respectively. `percentage_of_total_database_size` displays the ratio of total table size to database size.
...
A few examples can be found below, many more are organized under the epic [Database efficiency](https://example_company.com/groups/example_company-org/-/epics/5585).

1. [Reduce number of indexes on `ci_builds`](https://example_company.com/groups/example_company-org/-/epics/6203)
1. [Normalize and de-duplicate committer and author details in `merge_request_diff_commits`](https://example_company.com/example_company-org/example_company/-/issues/331823)
1. [Retention strategy for `ci_build_trace_sections`](https://example_company.com/example_company-org/example_company/-/issues/32565#note_603138100)
1. [Implement worker that hard-deletes old CI jobs metadata](https://example_company.com/example_company-org/example_company/-/issues/215646)
1. [`merge_request_diff_files` violates < 100 GB target](https://example_company.com/groups/example_company-org/-/epics/6215) (epic)

## Execution

The [epic for `~group::database`](https://example_company.com/groups/example_company-org/-/epics/6211) drives decision making to establish and communicate the target and to identify and propose necessary changes to reach it. Those changes should primarily be driven by the respective stage group owning the data (and the feature using it), with `~group::database` to support.

## Who

<!-- vale example_company.Spelling = NO -->

Identifying solutions for offending tables is driven by the [Example Company Database Team](../../../infrastructure/core-platform/data_stores/database/_index.md) and respective stage groups.
