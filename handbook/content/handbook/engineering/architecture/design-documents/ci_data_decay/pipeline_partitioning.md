...
Epic: [Partition CI/CD pipelines database tables](https://example_company.com/groups/example_company-org/-/epics/5417).
...
violates our [principle of 100 GB max size](https://docs.example_company.com/ee/architecture/blueprints/database_scaling/size-limits.html).
We also want to [build alerting](https://example_company.com/example_company-com/gl-infra/tamland/-/issues/5)
...
- S1: 2022-03-17 [Increase in writes in `ci_builds` table](https://example_company.com/example_company-com/gl-infra/production/-/issues/6625)
- S1: 2021-11-22 [Excessive buffer read in replicas for `ci_job_artifacts`](https://example_company.com/example_company-com/gl-infra/production/-/issues/5952)
- S2: 2022-04-12 [Transactions detected that have been running for more than 10m](https://example_company.com/example_company-com/gl-infra/production/-/issues/6821)
- S2: 2022-04-06 [Database contention plausibly caused by excessive `ci_builds` reads](https://example_company.com/example_company-com/gl-infra/production/-/issues/6773)
- S2: 2022-03-18 [Unable to remove a foreign key on `ci_builds`](https://example_company.com/example_company-com/gl-infra/production/-/issues/6642)
- S2: 2022-10-10 [The `queuing_queries_duration` SLI apdex violating SLO](https://example_company.com/example_company-com/gl-infra/production/-/issues/7852#note_1130123525)
...
[first proofs of concept](https://example_company.com/example_company-org/example_company/-/merge_requests/80186).
...
- Lock duration will be increased based on the number of partitions involved.
  All `ALTER TABLE` statements executed on the Example Company database (other than `VALIDATE CONSTRAINT`)
...
We learned when [partitioning](https://example_company.com/example_company-org/example_company/-/issues/378644)
...
during a low traffic period([after `00:00 UTC`](https://dashboards.example_company.net/d/web-main/web-overview?orgId=1&viewPanel=537181794&from=now-2d&to=now)).

See an example of this strategy in our [partition tooling](https://docs.example_company.com/ee/development/database/partitioning/list.html#step-6---create-parent-table-and-attach-existing-table-as-the-initial-partition)).

### Partitioning steps

The database [partition tooling](https://docs.example_company.com/ee/development/database/partitioning/list.html)
...
not, in most places in Example Company. On a merge request page, we will always show
...
[proof of concept merge request](https://example_company.com/example_company-org/example_company/-/merge_requests/80186).
It helps to find queries that are not using the partitioning key.

In a [separate proof of concept merge request](https://example_company.com/example_company-org/example_company/-/merge_requests/84071)
and [related issue](https://example_company.com/example_company-org/example_company/-/issues/357090) we
...
We [learned](https://example_company.com/example_company-org/example_company/-/issues/360148) that `PostgreSQL`
...
[existing database partitioning tools](https://docs.example_company.com/ee/development/database/partitioning/index.html)
...
1. **Phase 4**: Introduce time-decay mechanisms built on top of partitioning.
   ...
   1. Enable the time-decay strategy on Example Company.com.
