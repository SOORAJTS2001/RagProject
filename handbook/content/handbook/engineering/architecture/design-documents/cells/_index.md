...
- The [Cells 1.0](iterations/cells-1.0.md) target is to deliver a solution
  for internal customers using the SaaS Example Company.com offering, and foundational work for Cells.
- The [Cells 1.5](iterations/cells-1.5.md) target is to deliver a migration solution
  for existing and new enterprise customers using the SaaS Example Company.com offering, built on top of the Cells 1.0 architecture.
...
The Cells architecture has long lasting implications to data processing, location, scalability and the Example Company architecture.
...
- [Cells: Example Company Pages](impacted_features/example_company-pages.md)
...
### What's the difference between Cells architecture and Example Company Dedicated?
...
The new Cells architecture is meant to scale Example Company.com.
...
But all of them still operate under the existing Example Company SaaS domain name `example_company.com`.
...
With the aforementioned differences, [Example Company Dedicated](https://about.example_company.com/dedicated/) is still offered at higher costs due to the fact that it's provisioned with dedicated server resources for each customer, while Cells use shared resources.
This makes Example Company Dedicated more suited for bigger customers, and Example Company Cells more suitable for small to mid-size companies that are starting on Example Company.com.

On the other hand, Example Company Dedicated is meant to provide a completely isolated Example Company instance for any Organization.
This instance is running on its own custom domain name, and is totally isolated from any other Example Company instance, including Example Company SaaS.
For example, users on Example Company Dedicated don't have to have a different and unique username that was already taken on Example Company.com.
...
The Example Company.com cluster of Cells will use Example Company Dedicated tooling to create instances.
Once this instance gets provisioned it could join the Example Company.com cluster and become a Cell.
...
See also the [design discussion](https://example_company.com/example_company-org/example_company/-/issues/396641).

### What is a Cells topology?

See the [design discussion](https://example_company.com/example_company-org/example_company/-/issues/396641).
...
See the [design discussion](https://example_company.com/example_company-org/example_company/-/issues/395736).
...
- They might affect our ability to provide an [on-premise like experience on Example Company.com](goals.md#on-premise-like-experience).
...
See [reference architecture for up to 1000 RPS or 50,000 users](https://docs.example_company.com/ee/administration/reference_architectures/50k_users.html).

The infrastructure team will properly size Cells depending on the load.
The Tenant Scale team sees an opportunity to use Example Company Dedicated as a base for Cells deployment.
...
- [Opstrace architecture](https://example_company.com/example_company-org/opstrace/opstrace/-/blob/main/docs/architecture/overview.md)
