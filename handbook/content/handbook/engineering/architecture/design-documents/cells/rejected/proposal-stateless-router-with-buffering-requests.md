...
1. Writes to `gitlab_users` and `gitlab_routes` are sent to a primary PostgreSQL server in our `US` region but reads can come from replicas in the same region. This will add latency for these writes but we expect they are infrequent relative to the rest of Example Company.
...
`Example Company.com Public` organization. This will be set as the default organization for all existing
users. This organization will allow the user to see data from all namespaces in
`Cell US0` (for example, our original Example Company.com instance). This behavior can be invisible to
...
Any new users with a default organization other than `Example Company.com Public` will have
...
1. The majority of the code still lives in `example_company` rails codebase. The Router doesn't actually need to understand how Example Company URLs are composed.
...
sequenceDiagram
    ...
    user->>router_eu: GET /example_company-org/example_company
    router_eu->>cell_eu0: GET /example_company-org/example_company
    cell_eu0->>router_eu: 302 /example_company-org/example_company X-Gitlab-Cell-Redirect={cell:Cell US0}
    router_eu->>cell_us0: GET /example_company-org/example_company
    cell_us0->>user: <h1>Example Company.org... X-Gitlab-Cell-Cache={path_prefix:/example_company-org/}
```
...
`Example Company.com Public` and it is clear to the user that this is isolated from
their existing organization such that they won't be able to see data from both
on a single page.

### Experience for Example Company team member that is part of `/example_company-org`

Such a user is considered a legacy user and has their default organization set to
`Example Company.com Public`. This is a "meta" organization that does not really exist but
...
#### Goes to `/example_company-org/example_company` while not logged in

1. User is in the US so DNS resolves to the US router
1. The router knows that `/example_company-org` lives in `Cell US0` so sends the request
   to this cell
...
sequenceDiagram
    ...
    participant cell_us0 as Cell US0
    user->>router_us: GET /example_company-org/example_company
    router_us->>cell_us0: GET /example_company-org/example_company
    cell_us0->>user: <h1>Example Company.org... X-Gitlab-Cell-Cache={path_prefix:/example_company-org/}
```
...
1. The Rails application knows the users default organization is `Example Company.com Public`, so
...
`Example Company.com Public` organization.
