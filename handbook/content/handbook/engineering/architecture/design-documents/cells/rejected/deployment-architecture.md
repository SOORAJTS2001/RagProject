...
of Example Company.com and contrasts it with the expected Cells architecture.

## 1. Before Cells - Monolithic architecture

![Deployment before cells](diagrams/deployment-before-cells.drawio.png)

The diagram represents simplified Example Company.com deployment components before the introduction of a Cells architecture.
...
- Are strongly interconnected and require to run the same version of the application.
  Read more in [!131657](https://example_company.com/example_company-org/example_company/-/merge_requests/131657#note_1563513431).
...
This could be achieved with testing Cells on top of the [Reference Architectures](https://docs.example_company.com/ee/administration/reference_architectures/index.html).
Read more in [#425197](https://example_company.com/example_company-org/example_company/-/issues/425197).
...
- Development Cells represent a simplified Example Company.com architecture by focusing only on essential services required to be split.
...
- Additional Cells are deployed, upgraded and maintained via a [Example Company Dedicated-like](https://docs.example_company.com/ee/subscriptions/gitlab_dedicated/index.html) control plane.
...
- A Cell can run its own Gitaly cluster, or can use a shared Gitaly cluster, or both.
  Read more in [!131657](https://example_company.com/example_company-org/example_company/-/merge_requests/131657#note_1569151454).
- Shared Runners provided by Example Company are expected to be run locally on the Cell.
...
- Delay the decision how to scale the **Example Company Pages - `example_company.io`** component.
- Delay the decision how to scale the **Registry - `registry.example_company.com`** component.
...
- The Routing Service is expanded to support [Example Company Pages](https://docs.example_company.com/ee/user/project/pages/index.html) and [Example Company container registry](https://docs.example_company.com/ee/user/packages/container_registry/index.html).
...
|--------------------------------|--------------|---------------------------------|-------------|
| **Routing Service**            | Example Company-built | Cluster-wide Data Provider      | A general purpose routing service that can redirect requests from all Example Company SaaS domains to the Cell |
| **Cluster-wide Data Provider** | Example Company-built | PostgreSQL, Redis, Event Queue? | Provide user profile and routing information to all clustered services |
...
| **Example Company Runners Manager** | Managed service | API, uses Google Cloud VM Instances | No problem                        | Significant changes required to API and execution of CI jobs |
...
| Service             | Type            | Uses                            | Migrate from cluster-wide to Cell                                       | Description |
|---------------------|-----------------|---------------------------------|-------------------------------------------------------------------------|-------------|
| **Example Company Pages**    | Example Company-built    | Routing Service, Rails API      | No problem                                                              | Serving CI generated pages under `.example_company.io` or custom domains |
| **Example Company Registry** | Example Company-built    | Object Storage, PostgreSQL      | Non-trivial data migration in case of split                             | Service to provide Example Company container registry |
| **Gitaly Cluster**  | Example Company-built    | Disk storage, PostgreSQL        | No problem: Built-in migration routines to balance Gitaly nodes         | Gitaly holds Git repository data. Many Gitaly clusters can be configured in application. |
...
| **Object Storage**  | Managed service |                                 | Not straightforward: Rather hard to selectively migrate between buckets | Holds all user and CI uploaded files that is served by Example Company |
