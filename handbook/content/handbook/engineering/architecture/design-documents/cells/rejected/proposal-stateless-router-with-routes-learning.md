...
1. A new concept will be introduced in Example Company called an organization
...
When asking for the endpoint Example Company Rails will return information about
the routable path. Example Company Rails will decode `path_info` and match it to
an existing endpoint and find a routable entity (like project). The router will
treat this as short-lived cache information.

1. Prefix match: `/api/v4/internal/cells/learn?method=GET&path_info=/example_company-org/example_company-test/-/issues`

   ```json
   {
      "path": "/example_company-org/example_company-test",
      "cell": "cell_0",
      "source": "routable"
   }
   ```
...
`Cell US0` (ie. our original Example Company.com instance). This behavior can be invisible to
...
sequenceDiagram
    ...
    router_eu->>cell_eu0: /api/v4/internal/cells/learn?method=GET&path_info=/example_company-org/example_company
    cell_eu0->>router_eu: {path: "/example_company-org", cell: "cell_us0", source: "routable"}
    router_eu->>cell_us0: GET /example_company-org/example_company
    cell_us0->>user: <h1>Example Company.org...
```
