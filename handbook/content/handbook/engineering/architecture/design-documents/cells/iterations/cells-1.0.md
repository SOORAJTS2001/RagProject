...
Cells 1.0 is a complete working feature that is meant to be deployed to Example Company.com SaaS.
...
1. They want to use our multi-tenant SaaS solution (Example Company.com) to serve their Organization.
1. They accept that they may receive updates later than the rest of Example Company.com. Note that when Example Company does a patch release, the Delivery team makes sure that production (every cell) runs the patch release version before making the release public. This does not mean that outside of a patch release, all the cells run the same version of Example Company. See [this private discussion](https://example_company.com/example_company-com/gl-security/product-security/appsec/threat-models/-/issues/45#note_1794904358) for more information.
...
1. Because all users and groups are unique across the cluster, the same user can access other Organizations and groups at Example Company.com in [Cells 2.0](cells-2.0.md).
1. The overhead of managing and upgrading Cells is minimal and similar to managing a Example Company Dedicated instance. Cells should not be a linear increase in operational burden.
1. The Cell should be deployed using the same tooling as Example Company Dedicated.
...
1. Terms used:
    ...
    1. Cell: A single isolated deployment of Example Company that connects to the Topology Service.
...
1. Organization properties:
    ...
    1. We allow users to create a new Organization on a Cell.
       The chosen Cell would be controlled by Example Company Administrators.
...
node "Example Company Inc. Infrastructure" {
  ...
  node "A Cell" as Cell {
    frame "Example Company Rails" as Cell_Rails {
...
The Example Company configuration in `example_company.yml` is extended with the following parameters to:

```yaml
production:
    example_company:
        topology_service:
            address: https://cell1.example_company.com
            certificate: ...
...
## Features on Example Company.com that are not supported on Cells
...
The table below is a comparison between the existing Example Company.com features, and not self-managed/Dedicated.

| No Initial Support                                              | Reasoning                                                                                                                                                                                                                                                                                                                                                                                                     |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Example Company Pages                                                    | [Complexity](#questions).                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Example Company Duo Pro license works across all projects on instance    | Example Company Duo Pro licenses, once granted, [should allow users to use Example Company Duo Pro on all projects on the instance](https://example_company.com/example_company-org/example_company/-/issues/441244). With Cells 1.0, this will only work within their own cell.                                                                                                                                                                           |
...
| Windows and Mac OS Runners                                      | Mac and Windows runners are still in beta and there are some more complex technical considerations related to cost. See the discussion: [#434982 (comment 1789275416)](https://example_company.com/example_company-org/example_company/-/issues/434982#note_1789275416) on sharing resources.                                                                                                                                            |
| Multiple Sizes for Linux Runners                                | We will only support [small linux runners on Cells 1.0](https://example_company.com/example_company-org/example_company/-/issues/434982#note_1806447839).                                                                                                                                                                                                                                                                                |
| Example Company for Jira Cloud app and Example Company for Slack app integrations | Jira and Slack apps can only be configured to post to single endpoints, so there is nothing in the configured endpoints' routes that would allow the Cells router to know which cell to route to. We may need to support at the organization level. See [#467791](https://example_company.com/example_company-org/example_company/-/issues/467791) and [#467809](https://example_company.com/example_company-org/example_company/-/issues/467809) for more details. |
...
1. How would users log in?
    ...
    - UI: The login to Organizations would be scoped to the Organization: `https://<GITLAB_DOMAIN>/users/sign_in?organization=example_company-inc`.
    - SAML: `https://<GITLAB_DOMAIN>/users/auth/saml/callback` would receive `?organization=example_company-inc` which would be routed to the correct Cell.
...
1. Is Container Registry cluster-wide or cell-local?
    ...
    We would have to ensure that the JWT token signed by Example Company is in a form that can be statically routed by the routing layer.

1. Are Example Company Pages cluster-wide or Cell-local?

    Example Company Pages was determined to be non-essential for Cells 1.0, so we would not support them for Cells 1.0.
    The discussion about this can be found [here](https://example_company.com/example_company-org/example_company/-/issues/434972#note_1763737452).

    If Example Company Pages are meant to support the `.example_company.io` domain:

    - Example Company Pages need to be run as a single service that is not run as part of a Cell.
    - Because Example Company Pages use the API we need to make them routable.
    ...
    - Run Example Company Pages in a Cell, but provide a separate domain.
...
1. How do we manage admin accounts?
    ...
    This might be solved by Example Company Dedicated already?
...
1. How are secrets are generated?
    ...
    - Example Company Runner Tokens are generated in the form: `glrt-2CR8_XYZ`
...
1. Why secrets-based routing instead of path-based routing?
    ...
    - Making all existing routes routable would be a significant effort to fix
      ...
      how many routes are already classified can be found [in this comment](https://example_company.com/example_company-org/example_company/-/issues/430330#note_1633125914).
...
1. What about data migration between Cells?
    ...
    features similar to how we migrate customers from Example Company.com to Dedicated. As a result, not all information
...
1. Why not use FQDN (`mycorp.example_company.com` or `mygitlab.mycorp.com`) instead?

    We want the `example_company.com` to feel like a single application, regardless how many organizations you interact with.
    This means that we want to share users, and possibly some data between organizations at later point.
    Following model of `mycorp.example_company.com` or `mygitlab.mycorp.com` creates a feel of a strong isolation between instances,
...
1. How the Cells 1.0 differ to [Example Company Dedicated](https://about.example_company.com/dedicated/)?

    - The Example Company Dedicated is a single-tenant hosted solution provided by Example Company Inc.
      to serve a single customer under a custom domain.
    - The Cellular architecture is meant to be multi-tenant solution using `example_company.com` domain.
    - The Example Company Dedicated by design has all resources separate between tenants.
...
1. Why not to prefix all endpoints with the `relative_path` for all organizations?
    ...
    - If we choose to fix existing not sharded [can be made](https://example_company.com/example_company-org/example_company/-/issues/430330)
      at later point we will achieve much better API consistency, and likely much less amount of the work.

1. Why not use subdomains, like `mycorp.example_company.com`?
...
1. How can I adapt a feature to be compatible with Cells?
    ...
    - For critical cases, move the feature to cluster-wide level.
      For example, users can sign in at a single location, `https://example_company.com/users/sign_in`.
