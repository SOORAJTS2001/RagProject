...
1. [Example Company Dedicated Architecture](https://example_company-com.example_company.io/gl-infra/example_company-dedicated/team/architecture/Architecture.html)
...
- **Cell-local by default**: All services should be cell-local, and not global, unless there are documented, and good reasons why they aren't cell-local.
  ...
  Example, Gitaly and Example Company Registry are cell-local.
- **Homogeneous environments**: For now, every Example Company cell should look the same. Bootstrapping and provisioning should be done in an automated way.
...
- **Fresh start, but not so much**: Brand new Example Company instances are created, so it's tempting to redo everything. We have to balance the existing infrastructure, dedicated tooling, and time.
...
- **Centralize Tooling**: We have a lot of tooling to manage Example Company.com and separate tooling for Example Company Dedicated,
  which creates silos, duplication of effort, and less portability.
  We have to provision multiple Cells for Example Company.com, we need new tooling, Example Company Dedicated built tooling just for this reason.
...
- `Deploy`: When we change the running code inside of an existing Cell. Example: We _deployed_ the new auto-deploy version on Example Company.com.
...
- `Tenant`: A Example Company instance provisioned through Example Company Dedicated tooling ([Instrumentor](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/instrumentor)). A Tenant can be _either_ a Example Company Dedicated Customer Instance, _or_ a Cell Instance.
- `Cell`: A Tenant provisioned to be a part of Example Company.com where multiple customers are served through a single Tenant.
- `Legacy Cell`: The existing Example Company.com deployment.
...
- `Cluster`: A collection of Cells, and the existing Example Company.com infrastructure. Example: We need to change the version of Registry in the Cluster.
- `Fleet`: The collection of all SaaS environments, both single-tenant and multi-tenant, that collectively form our production environments.
  This includes existing Example Company.com infrastructure, Cells, and Dedicated.

## Architecture

Below is the Cell architecture. You can find the current Example Company.com architecture (pre-Cells) in <https://handbook.example_company.com/handbook/engineering/infrastructure/production/architecture/>.
...
:User:
cloud example_company.com <<cloudflare>> {
  [DoS Protection]-->[WAF]
  [WAF]-->[RoutingService]
}
cloud "cloud.example_company.com" <<cloudflare>> {
  [Cloud Connector]-->[AI Gateway]
}
:User:->example_company.com
:User:->cloud.example_company.com
...
frame "Google Cloud Platform" <<gcp>> {
  frame "Cell Cluster" <<cluster>> {
    frame "example_company-production" <<gcp_project>>{
    ...
    frame "example_company-ci" <<gcp_project>> {
    ...
    runnerManager --> "example_company-ci-*"

    frame "example_company-gitaly-gprd-*" <<gcp_project>> {
  ...
  "Cell Cluster" -u-> cloud.example_company.com
}
...
cloud wss://kas.example_company.com <<cloudflare>> as kas.example_company.com {
    component "routing service"
}

cluster1AgentK <..d..> kas.example_company.com
cluster2AgentK <..d..> kas.example_company.com
cluster3AgentK <--d--> kas.example_company.com
kubectl <--d--> kas.example_company.com

together {
  frame "gprd-example_company-cell-1" <<gcp>> {
  ...
  frame "gprd-example_company-cell-2" <<gcp>> {
...
| Cell Sizing                      | team::Scalability-Observability   | [To-Do](https://example_company.com/example_company-com/gl-infra/scalability/-/issues/2838) | 👎         |
