...
Example Company.com is largely monolithic and we have estimated (internally) that the current architecture has scalability limitations,
particularly for the [PostgreSQL database](https://example_company-com.example_company.io/gl-infra/tamland/patroni.html), and
[Redis](https://example_company-com.example_company.io/gl-infra/tamland/redis.html) non-horizontally scalable resources,
...
Organizations should have the same user experience on our SaaS platform as they do on a self-managed Example Company instance.

### Regions

Example Company.com is only hosted within the United States of America.
Organizations located in other regions have voiced demand for local SaaS offerings.
Cells provide a path towards [Example Company Regions](https://example_company.com/groups/example_company-org/-/epics/6037) because Cells may be deployed within different geographies.
...
At this moment, Example Company.com has "social network"-like capabilities that may not fit well into a more isolated Organization model.
Removing those features, however, poses some challenges:

1. How will existing `example_company-org` contributors contribute to the namespace?
...
| Product     | All Cells are under a single Example Company.com domain    | High     |
...
| Operational | Cost per user similar or lower to Example Company.com      | Medium   |
...
| Migration   | Scale existing Example Company.com database                | High     |
| Migration   | Re-balancing of Cells                             | Low      |
| Migration   | Customer can migrate from Example Company.com to Dedicated | Low      |
...
### All Cells are under a single Example Company.com domain
...
Example Company.com has a number of open-source and open-core projects (including [`example_company-org/example_company`](https://example_company.com/example_company-org/example_company)).
...
On-premise allows to control all aspects of a Example Company installation, including but not limited to: managing users, access controls, or instance-wide settings.
...
The introduction of Cells will imply changes in the supported Example Company.com workflows.
Cells might affect how people use Example Company, as long as it provides a compelling user experience story.
...
Support for Regions should enable us to run Example Company in different availability zones,
...
and Example Company Inc. to use different cloud infrastructure providers (Google Cloud, AWS) to serve customers.

Cells enable existing customers running Organizations to move their Organization between different deployment regions/data centers that are supported by Example Company on Example Company.com.
...
### Cost per user similar or lower to Example Company.com

The current Example Company.com architecture is rather cost-effective.
...
Rollout of the Cells architecture is a fundamental change to how Example Company.com will operate.
...
### Scale existing Example Company.com database

The existing Example Company.com database will continue to grow while we deploy Cells.
The Cells architecture should provide a way to increase Cell 1 (existing Example Company.com)
...
### Customer can migrate from Example Company.com to Dedicated

Cells will enforce an architecture with more isolation. However, it is expected
that some customers would like to move from SaaS Example Company.com to Example Company Dedicated.

This implies being able to selectively migrate data from the Example Company.com cluster to Dedicated.
This could be achieved by:

- Migrating a customer to a Dedicated Cell on the Example Company.com cluster.
- Disconnecting a Cell from the Example Company.com cluster into its own standalone Dedicated Cell.
...
### Federation between distinct Example Company instances
...
### Merging Dedicated instance(s) to Example Company.com

It is not intended for any existing Dedicated instances to join the Example Company.com
cluster.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| [Cell](#cell)                       | A Cell is a set of infrastructure components that contains multiple top-level Groups that belong to different Organizations.                | Example Company instance, cluster, shard, Pod       |
| [Legacy Cell](#legacy-cell)         | The existing Example Company.com infrastructure                                                                                                      | Primary Cell                               |
| [Cluster](#cluster)                 | A cluster is a collection of Cells.                                                                                                         | Whale, Example Company Dedicated instance, instance |
...
> - Pod was renamed to Cell in <https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/121163>
...
- On initial provisioning, a Example Company instance provisioned by Example Company Dedicated tooling can be configured to join the cluster, and become a Cell

Discouraged synonyms: Example Company instance, cluster, shard, Pod

### Legacy Cell

The existing Example Company.com infrastructure will operate as a Cell from an application
perspective. We refer to this as legacy because it's where our existing
customers live, and it's using the old provisioning tooling (not Example Company Dedicated
...
Discouraged synonyms: Whale, Example Company Dedicated instance, instance
...
`https://example_company.com/example_company-org/example_company/`:

- `example_company-org` is a `top-level group`; the root for all Groups and Projects of an Organization
- `example_company` is a `project`; a Project of the Organization.

The top-level Group has served as the de facto Organization entity. With the creation of Organizations, top-level Groups will be [nested underneath Organizations](https://example_company.com/example_company-org/example_company/-/issues/394796).
...
- Users belong to one Organization. See [!395736](https://example_company.com/example_company-org/example_company/-/issues/395736).
