...
The routing layer is meant to offer a consistent user experience where all Cells are presented under a single domain (for example, `example_company.com`), instead of having to go to separate domains.

The user will be able to use `https://example_company.com` to access Cell-enabled Example Company.
...
- All requests going to `https://example_company.com/users/sign_in` are randomly distributed to all Cells.
- All requests going to `https://example_company.com/example_company-org/example_company/-/tree/master` are always directed to Cell 5, for example.
- All requests going to `https://example_company.com/my-username/my-project` are always directed to Cell 1.
...
| Single domain       | for example Example Company.com                                            | high     |
...
There is 3 primary entry points for the application; [`web`](https://example_company.com/example_company-com/runbooks/-/blob/5d8248314b343bef15a4c021ac33978525f809e3/services/service-catalog.yml#L492-537), [`api`](https://example_company.com/example_company-com/runbooks/-/blob/5d8248314b343bef15a4c021ac33978525f809e3/services/service-catalog.yml#L18-62), and [`git`](https://example_company.com/example_company-com/runbooks/-/blob/5d8248314b343bef15a4c021ac33978525f809e3/services/service-catalog.yml#L589-638).
...
The main SLI we use is the [rails requests](https://docs.example_company.com/ee/development/application_slis/rails_request.html).
It has multiple `satisfied` targets (apdex) depending on the [request urgency](https://docs.example_company.com/ee/development/application_slis/rails_request.html#how-to-adjust-the-urgency):
...
_Analysis was done in <https://example_company.com/example_company-org/example_company/-/issues/432934#note_1667993089>_
...
_Analysis was done in <https://example_company.com/example_company-org/example_company/-/issues/432934#note_1669995479>_
...
_Analysis was done in <https://example_company.com/example_company-org/example_company/-/issues/432934#note_1671385680>_
...
1. Rule-based:
   ...
   - Configured rules needs to be made compatible with all versions of Example Company running in a cluster.
    ...
    "rules": [
        {
            ...
            "action": "proxy",
            "proxy": {
                "address": "cell1.example_company.com"
            }
        }
    ]
}
...
## Response:
{
    ...
    "other_classifications": [ // list of all equivalent keys that should be put in the cache
        ...
        { "type": "project_full_path", "value": "example_company-org/example_company" },
        { "type": "project_full_path", "value": "example_company-org/example_company" },
        { "type": "namespace_full_path", "value": "example_company-org" }
    ]
}
```
...
There are several phases to fully deploy the HTTP Routing service to Example Company.com.

1. The first phase is to deploy a simple pass-through proxy in front of the webservice (`example_company.com`).
   ...
   1. (Maybe optional) The next step is to provision an internal-only DNS for
      the legacy cell (e.g. `cell-1.gprd.int.example_company.com`).
      ...
      `example_company.com` DNS record, likely with
      [custom domains](https://developers.cloudflare.com/workers/configuration/routing/custom-domains/).
1. The second phase is to deploy a simple pass-through proxy in front of
   the container registry (`registry.example_company.com`).
   This will use the same deployment of the HTTP Router for `example_company.com`.
   1. First, we will utilize [Cloudflare Routes](https://developers.cloudflare.com/workers/configuration/routing/routes/)
      to rollout the worker gradually, without the need to change DNS.
   1. (Maybe optional) The next step is to provision an internal-only DNS for
      the legacy cell (e.g. `cell-1-registry.gprd.int.example_company.com`).
      We then proxy the HTTP router to this DNS, and secure this connection with
      a solution like `mTLS`, or Cloudflare Tunnel.
      In order to do this, the HTTP Router will need to be assigned the
      `registry.example_company.com` DNS record, likely with
      [custom domains](https://developers.cloudflare.com/workers/configuration/routing/custom-domains/).
1. The third phase involves multiple cells.
   1. For any new cell the HTTP Router routes to, the cell will have:
      1. An internal-only DNS, like `cell-2.gdrd.int.example_company.com` that is only
...
1. `example_company-org` is a top-level namespace and lives in `Cell US0` in the `Example Company.com Public` organization.
...
sequenceDiagram
    ...
    ts->>-router: Proxy(address="cell-eu0.example_company.com")
    router->>cache: Cache(type=session_prefix, value=cell_eu0) = Proxy(address="cell-eu0.example_company.com"))
...
1. User signs in.
1. Example Company sees that user default organization is `my-company`, so it assigns session cookie with `cell_eu0_` to indicate that
...
sequenceDiagram
    ...
    ts->>router: Proxy(address="cell-us0.example_company.com")
...
#### Goes to `/example_company-org/example_company` after last step

User visits `/example_company-org/example_company`, and because it does have a session cookie, the request is forwarded to `Cell EU0`.
...
sequenceDiagram
    ...
    user->>router: GET /my-company/my-project<br/>_gitlab_session=cell_eu0_uwwz7rdavil9
    router->>+cache: GetClassify(type=session_prefix, value=cell_eu0)
    cache->>-router: Proxy(address="cell-eu0.example_company.com"))
    router->>cell_eu0: GET /my-company/my-project
    cell_eu0->>user: <h1>My Project...
```
...
- [Cells - Routing: Technology](https://example_company.com/groups/example_company-org/-/epics/11002)
- [Classify endpoints](https://example_company.com/example_company-org/example_company/-/issues/430330)
