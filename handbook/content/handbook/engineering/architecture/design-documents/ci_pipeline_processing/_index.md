...
Example Company CI is one of the oldest and most complex features in Example Company.
...
["Restructure CI job when keyword"](https://example_company.com/groups/example_company-org/-/epics/6788) epic.
...
Right now, the [`when`](https://docs.example_company.com/ee/ci/yaml/index.html#when) keyword has many responsibilities;
...
>   Can only be used in a [`rules`](https://docs.example_company.com/ee/ci/yaml/index.html#rules) section or `workflow: rules`.
> - `always`: Run the job regardless of the status of jobs in earlier stages. Can also be used in `workflow:rules`.
> - `manual`: Run the job only when [triggered manually](https://docs.example_company.com/ee/ci/jobs/job_control.html#create-a-job-that-must-be-run-manually).
> - `delayed`: [Delay the execution of a job](https://docs.example_company.com/ee/ci/jobs/job_control.html#run-a-job-after-a-delay)
...
We control the blocker behavior of a manual job by the [`allow_failure`](https://docs.example_company.com/ee/ci/yaml/index.html#allow_failure) keyword.
Actually, it has other responsibilities; _"determine whether a pipeline should continue running when a job fails"_.

Currently, a [manual job](https://docs.example_company.com/ee/ci/jobs/job_control.html#create-a-job-that-must-be-run-manually);
...
From [docs](https://docs.example_company.com/ee/ci/yaml/index.html#when):
...
From [docs](https://docs.example_company.com/ee/ci/yaml/index.html#allow_failure):
...
The [`dependencies`](https://docs.example_company.com/ee/ci/yaml/index.html#dependencies) keyword is used to define a list of jobs to fetch
[artifacts](https://docs.example_company.com/ee/ci/yaml/index.html#artifacts) from. It is a shared responsibility with the `needs` keyword.
...
- A failed job;
  ...
  - We know the result and [there can be artifacts](https://docs.example_company.com/ee/ci/yaml/index.html#artifactswhen).
  - `after_script` is run.
...
We [recently updated](https://example_company.com/example_company-org/example_company/-/merge_requests/117856) the documentation of
[the `when` keyword](https://docs.example_company.com/ee/ci/yaml/index.html#when) for clarification;
