---
title: "Example Company Observability - Logging"
status: proposed
...
This design document outlines a system for storing and querying logs which will be a part of Example Company Observability Backend (GOB), together with [tracing](../observability_tracing/index.md) and [metrics](../observability_metrics/).
...
The users will interact with the data through Example Company UI.
...
- **uses ClickHouse as a data storage backend**: ClickHouse has become the go-to solution for observability data at Example Company for a plethora of reasons.
...
Each top-level Example Company namespace has its own OTEL collector to which ingestion requests are directed by the cluster-wide Ingress.
...
GOB exposes an HTTP/JSON API that e.g. Example Company UI uses to query and then render logs.
...
This is currently done only for [the ingestion path](https://example_company.com/example_company-org/opstrace/opstrace/-/merge_requests/2236).
Check the MR description for more details on how it works.
The read path rate limiting implementation is tracked [here](https://example_company.com/example_company-org/opstrace/opstrace/-/issues/2356).
...
Something very similar to what Example Company currently has for searching MRs and Issues.
...
Refer to [Observability Group planning epic](https://example_company.com/groups/example_company-org/opstrace/-/epics/92) and its linked issues for up-to-date information.
