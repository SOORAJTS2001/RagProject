---
title: "Example Company Observability in Example Company.com and Self-Managed Example Company Instances"
status: ongoing
...
Example Company observability [will first be available to self-managed instances through Cloud Connector](https://example_company.com/groups/example_company-org/opstrace/-/epics/95) so self-managed users can take advantage of Example Company Observability without the need to manage an observability platform. This document outlines the architecture.

In addition to making Example Company observability available to self-managed instances through Cloud Connector, Example Company observability available in Example Company.com will also realign architecture to utilize Cloud Connector.

The following architectural overview can be applied to both self-managed Example Company instances and .com Example Company instances.
...
- **Move Observability APIs to GitLabs [project resource API](https://docs.example_company.com/ee/api/api_resources.html#project-resources).**
- **Maintain API consistency between .com and self-managed Example Company**

## Architecture

```plantuml
@startuml
node "Example Company Inc. Infrastructure" {
  package "Private Services" {
    [Example Company Observability Backend (GOB)] as GOB #yellow
  }

  package "Public Services" {
    [Example Company (SaaS)] as SAAS
...
node "Customer Infrastructure" {
  ...
  [Example Company (Self-Managed)] as SM
  [Sidekiq] as SK
}
...
note bottom of CC
  ...
  cloud.example_company.com/observe/* ---> GOB
  ...
end note
...
- Use JWKS from Customers Portal to auth
  Self-Managed Example Company requests
  ...
end note
@enduml
```

Example Company Observability Backend (GOB) deployments will be managed by Example Company Inc. as
part of the Example Company Inc. infrastructure. Example Company.com and all self-managed Example Company instances
...
Blue arrows highlight the request path from both Example Company SaaS and Example Company self-managed instances
to GOB. All requests pass through the [Cloud Connector Gateway](../cloud_connector/)
public service and on to the private GOB service. Example Company SaaS cells will all have access to the public Cloud Connector Gateway.
...
The following two diagrams highlight the request flow for Example Company.com customers and for self-managed Example Company customers.
A couple small differences exist between the two:

1. The JWT sent to GOB in Example Company.com requests is created in Rails, whereas the JWT or otherwise referred to as IJWT in
   the self-managed flow originates from CustomersDot.
1. There is no CustomersDot JWT sync in the Example Company.com flow.

### Observability offered in Example Company.com

```mermaid
sequenceDiagram
    autonumber
    participant U as Example Company.com user
    participant SDK as OpenTelemetry <br/> SDK/Collector
    participant .comUI as Example Company.com (UI)
    participant .comW as Example Company.com (WorkHorse)
    participant .comR as Example Company.com (Rails)
    participant GOB as GOB SaaS behind cloud.example_company.com <br/>(Example Company Observability Backend)
    ...
    Note over U,GOB: Example Company UI
    U->>.comUI: view
...
### Observability offered in Self-Managed Example Company
...
sequenceDiagram
    ...
    participant SDK as OpenTelemetry <br/> SDK/Collector
    participant SMUI as SM Example Company (UI)
    participant SMW as SM Example Company (WorkHorse)
    participant SMR as SM Example Company (Rails)
...
If we consider the daily data transmitted and stored concerning our observability of Example Company.com, and we extrapolate that across our self-managed customers who might have equally demanding needs, we can get an idea of how much data will pass through Workhorse and Cloud Connector. Example Company.com produces 150M+ metrics series, sampled every 30-60 seconds, and 18-22 TB of logs per day.

We could assume that any Ultimate tier root-level namespace on Example Company.com or any Ultimate tier self-managed instance could send a similar magnitude of data through `cloud.Example Company.com`.
...
Single large Ultimate tier Organization (via Example Company.com or self-managed):
...
### Observability for Example Company.com

For each customer of this magnitude on Example Company.com, there would be a demand of 1.2GB/s through Workhorse.

To maximize performance and Example Company service availability, a dedicated web fleet should be created for observability just like the recent addition for code suggestions. All incoming
...
This mitigates the risk of observability requests saturating the main Example Company web fleet and allows for horizontal autoscaling of the observability fleet based on demand.
...
1. User/Client to their Example Company instance
1. Example Company instance to GOB
...
Both Rails and Workhorse use the same secret to sign JWTs so the JWKS available at the `jwks_uri` found at `https://example_company.com/.well-known/openid-configuration` can be used to verify the identity of the JWT created from Rails or Workhorse, meaning requests to GOB could also be sent from Rails. For the IJWTs, the [JWKS can be retrieved in accordance with this](https://example_company.com/example_company-org/customers-example_company-com/-/blob/d877b173ebe6a614d41ef34d9e2a6a0fe93dac07/doc/architecture/add_ons/code_suggestions/authorization_for_self_managed.md#L74-145).

## Enabling

Self-Managed users will enable the use of Cloud Connected Example Company Observability through the Customers Portal. All relevant licenses and
...
Example Company Observability Backend already has rate limiting on its API gateway to prevent denial of service attacks.

In addition to these existing rate limits, we will also add rate limiting to Workhorse/Rails in the same idiomatic manner
to which other Example Company APIs manage rate limits to ensure rate limits are enforced at the edge, mitigating any DOS attacks at the edge
before calling on more downstream services.

Example Company Observability Quota management is enforced in GOB. GOB configures compute quota for each top-level namespace, depending on
...
Eventually, this quota management will be mapped to the Example Company Organization construct instead of top-level namespace.
...
All observability APIs will proxy through Workhorse and will live under GitLabs [project resource](https://docs.example_company.com/ee/api/api_resources.html#project-resources).
HTTP will be supported initially and gRPC will be added at a later date.

## System Monitoring

Example Company Observability Backend already has system monitoring in place that plugs into GitLabs centralized metrics and logging system. Additional metrics will also be
