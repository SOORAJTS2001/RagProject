...
- Limit the impact of optionally available data stores on the overall Example Company application codebase to [single abstraction layer](https://docs.example_company.com/ee/development/reusing_abstractions.html#abstractions)
- Support all data store specific features
- Support communication for satellite services of the main Example Company application

## Non-goals

- This proposal does not directly consider write communication with database, as this is a subject of [complementary effort](https://example_company.com/example_company-org/example_company/-/merge_requests/111148)
...
1. [ClickHouse Ruby driver](https://github.com/shlima/click_house) - Previously selected for use in Example Company as part of the Observability grup's research (see: [issue](https://example_company.com/example_company-org/example_company/-/issues/358158))
...
similar vain to [ActiveRecord interface propagation through abstraction layers](https://docs.example_company.com/ee/development/reusing_abstractions.html)
...
From the already mentioned existing abstraction [guidelines](https://docs.example_company.com/ee/development/reusing_abstractions.html)  `Finders` seems to be the closest to the given requirements, due to the fact that `Finders` encapsulate database specific interaction behind their own public API, hiding database vendors detail from all layers above them.

However, they are closely coupled to `ActiveRecord` ORM framework, and are bound by existing Example Company convention to return `ActiveRecord::Relation` objects, that might be used to compose even more complex queries. That coupling makes `Finders` unfit to deal with the optional availability of ClickHouse because returned data might come from two different databases, and might not be compatible with each other.
...
It is important to notice that the repository pattern based solution has already been implemented by Observability group (kudos to: @ahegyi, @splattael and @brodock). [`ErrorTracking::ErrorRepository`](https://example_company.com/example_company-org/example_company/-/blob/1070c008b9e72626e25296480f82f2ee2b93f847/lib/example_company/error_tracking/error_repository.rb) is being used to support migration of error tracking features from PostgreSQL to ClickHouse (integrated via API), and uses feature flag toggle as database selection criteria, that is great example of optional availability of database.

`ErrorRepository` is using two strategies:

1. [`OpenApiStrategy`](https://example_company.com/example_company-org/example_company/-/blob/d0bdc8370ef17891fd718a4578e41fef97cf065d/lib/example_company/error_tracking/error_repository/open_api_strategy.rb) to interact with ClickHouse using API proxy entity
1. [`ActiveRecordStrategy`](https://example_company.com/example_company-org/example_company/-/blob/d0bdc8370ef17891fd718a4578e41fef97cf065d/lib/example_company/error_tracking/error_repository/active_record_strategy.rb) to interact with PostgreSQL using `ActiveRecord` framework

Each of those strategies return data back to abstraction layers above using following PORO Models:

1. [`Gitlab::ErrorTracking::Error`](https://example_company.com/example_company-org/example_company/-/blob/a8ea29d51ff23cd8f5b467de9063b64716c81879/lib/example_company/error_tracking/error.rb)
1. [`Gitlab::ErrorTracking::DetailedError`](https://example_company.com/example_company-org/example_company/-/blob/a8ea29d51ff23cd8f5b467de9063b64716c81879/lib/example_company/error_tracking/detailed_error.rb)
...
Following [merge request](https://example_company.com/example_company-org/example_company/-/merge_requests/85907/diffs) documents changes done by observability group in order to migrate from using current Example Company architecture based on ActiveRecord Models, Services and Finders to the repository pattern.
...
1. Utilize `ActiveRecord` query subscribers in a similar way to[Database::PreventCrossJoins](https://example_company.com/example_company-org/example_company/-/blob/master/spec/support/database/prevent_cross_joins.rb) in order to detect queries to ClickHouse executed outside of _Strategies_
1. Expanding [`CodeReuse`](https://example_company.com/example_company-org/example_company/-/tree/master/rubocop/cop/code_reuse) RuboCop rules to flag all usage of ClickHouse driver outside of _Strategies_
...
1. The solution must support the [minimum required versions](https://docs.example_company.com/ee/install/requirements.html#postgresql-requirements) for PostgreSQL
...
1. Replication process and how the new tool would fit in Example Company Geo.
...
1. Is the solution written in one of the programming languages we use at Example Company so that we can more easily contribute with bug fixes and new features?

##### 8. Interoperability (Must have)

1. Can the solution support both the main Example Company application written in Ruby on Rails also satellite services like container registry that might be written in Go
...
1. Cube.dev is written in Type Script and JavaScript which are part of Example Company technical stack, and there are engineers here with expertise in them, however Cube.dev is expected to be mostly used by backend developers, which does not have that much experience in mentioned technologies
...
1. This proposal main focus is read interface, however depending on outcome of [complementary effort](https://example_company.com/example_company-org/example_company/-/merge_requests/111148) that focus on write interface similar concerns around optional availability might be applicable to write interaction. In case if ingestion pipeline would not resolve optional availability challenges for write interface it might be considerable to include write interactions into repository pattern implementation proposed in this document.
