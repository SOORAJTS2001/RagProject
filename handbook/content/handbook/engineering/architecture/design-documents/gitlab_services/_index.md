...
Example Company needs to have [services](https://about.example_company.com/direction/delivery/glossary.html#service) as the first-class concept.
This blueprint outlines how the service and related entities should be built in the Example Company CD solution.

## Motivation

As Example Company works towards providing a single platform for the whole DevSecOps cycle,
...
While Example Company offers some concepts, like the `environment` syntax in Example Company pipelines,
...
introduce [service](https://about.example_company.com/direction/delivery/glossary.html#service) and [release artifact](https://about.example_company.com/direction/delivery/glossary.html#release) to answer this question. The [Delivery glossary](https://about.example_company.com/direction/delivery/glossary.html#service) defines
a service as
...
A service would connect to the SCM, registry or issues through release artifacts and would be a focused view into the [environments](https://about.example_company.com/direction/delivery/glossary.html#environment) where
...
The current work in [Analyze:Observability](../../../../product/categories/#observability-group) could be integrated into Example Company if it supports services.
...
- We want to connect a [deployment](https://about.example_company.com/direction/delivery/glossary.html#deployment) of a service to the MRs, containers, packages, linter results included in the [release artifact](https://about.example_company.com/direction/delivery/glossary.html#release).
...
- We don't want to tackle Example Company observability solution built by [Analyze:Observability](../../../../product/categories/#observability-group). The proposal here should treat it as one observability integration backend.
- We don't want to cover alerting, SLOs, SLAs and incident management.
- Some infrastructures might already have better support within Example Company than others (Kubernetes is supported better than pure AWS). There is no need to discuss functionalities that we provide or plan to provide for Kubernetes and how to achieve feature parity with other infrastructures.
...
At the group-level, Example Company fetches all of the project-level environments under the specific group,
...
See [Glossary](https://about.example_company.com/direction/delivery/glossary.html) for more information.

**Discussion:** It's TBD whether we should reuse existing entities such as [`Deployment`](https://example_company.com/example_company-org/example_company/-/blob/master/app/models/deployment.rb) and [`Environment`](https://example_company.com/example_company-org/example_company/-/blob/master/app/models/environment.rb) models. Reusing the existing entities could limit us in the long run, however, users should be able to adopt the new architecture seamlessly without changing their existing CI/CD workflow drastically. This decision should be made when we have clearer answer for the ideal structure and behavior of the entities, so that we can understand how far from it the existing entity is and how feasible to migrate.

## Alternative Solutions

- [Add dynamically populated organization-level environments page](https://example_company.com/example_company-org/example_company/-/issues/241506).
  This approach was concluded as no-go in favor of Service concept.
- There is an alternative proposal to introduce [Group Environment entity](https://example_company.com/example_company-org/example_company/-/merge_requests/129696#note_1557477581) for [Group-level environment views](#aggregate-environments-and-services-at-group-level).
