---
title: "Example Company Observability - Metrics"
status: proposed
...
Within Example Company Observability Backend, we aim to add the support for our customers to ingest and query observability data around their systems & applications, helping them improve the operational health of their systems.
...
- Support for long-term storage for metrics, ingested via an OpenTelemetry-compliant agent and queried via Example Company-native UI with probable support for metadata and exemplars.
...
- We aim to support querying data via a Example Company-native UX which would mean using a custom DSL/Query Builder sending API requests to our backend which would then translate them into Clickhouse SQL. From our internal discussions around this, [Product Analytics Visualisation Designer](https://example_company.com/example_company-org/example_company-services/design.example_company.com/-/analytics/dashboards/visualization-designer) is a good source of inspiration for this.
...
We intend to use Example Company Observability Backend (GOB) as a framework for the Metrics implementation so that its lifecycle can be managed via already established components of our backend.
...
- We aim to allow our users to use Example Company itself to read ingested data, which will necessitate building a dedicated `Query Service` on our backend to be able to service API requests originating from Example Company.
...
Keeping inline with our current operational structure, we intend to deploy the metrics offering as a part of Example Company Observability Backend, deployed on the following two target environments:
...
- Kafka - non-trivial, external dependency but it can augment multiple other use-cases and help other problem domains at Example Company.

**Note**: Similar challenges have also surfaced with the CH interactions `errortracking` - the subsystem has in its current implementation. There have been multiple attempts to solve this problem domain in the past - [this MR](https://example_company.com/example_company-org/opstrace/opstrace/-/merge_requests/1660) implemented an in-memory alternative while [this one](https://example_company.com/example_company-org/opstrace/opstrace/-/merge_requests/1767) attempted an on-disk alternative.
...
- [Benchmark ClickHouse for metrics](https://example_company.com/example_company-org/opstrace/opstrace/-/issues/1666)
- [Incubation:APM ClickHouse evaluation](https://example_company.com/example_company-org/incubation-engineering/apm/apm/-/issues/4)
- [Incubation:APM ClickHouse metrics schema](https://example_company.com/example_company-org/incubation-engineering/apm/apm/-/issues/10)
- [Our research around TimescaleDB](https://example_company.com/example_company-com/gl-infra/reliability/-/issues/14137)
- [Current Workload on our Thanos-based setup](https://example_company.com/example_company-com/gl-infra/reliability/-/issues/15420#current-workload)
...
The following section enlists how we intend to implement the aforementioned proposal around building Metrics support into Example Company Observability Service. Each corresponding document and/or issue contains further details of how each next step is planned to be executed.
...
- Develop support for displaying a list of ingested metrics within Example Company UI.
