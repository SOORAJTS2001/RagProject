...
features to all users of Example Company, no matter which instance they are
using: self-managed, dedicated or Example Company.com.

Initially, all AI-gateway deployments will be managed by Example Company (the
organization), and Example Company.com and all Example Company self-managed instances
...
globally reachable `cloud.example_company.com/ai/*` route. IDEs currently use `cloud.example_company.com/ai/*`
indirectly through a Example Company Rails instance. In
future we plan to allow network endpoints such as IDEs to connect directly to
`cloud.example_company.com` (for more information, see [directly connected clients](#directly-connected-clients)).
...
By using a hosted service under the control of Example Company we can ensure
that we provide all Example Company instances with AI features in a scalable
way. It is easier to scale this small stateless service, than scaling
Example Company Rails with its dependencies (database, Redis).
...
[this epic](https://example_company.com/groups/example_company-org/-/epics/12224#proposed-solution).
...
request can be sent directly to AI Gateway or to Example Company Rails for additional
...
can still send these requests through Example Company Rails (as is now) without
...
versions. For example when working on Example Company 17.2, we would support
both Example Company 17 and Example Company 16.
...
this level of the stack as well as in Example Company Rails.

#### Protocol

The communication between the AI-Gateway service and its clients (including the Example Company Rails application) shall use a JSON-based API.
...
benefit of this is that individual Example Company milestone releases will
...
Example Company instances, JSON API, and gRPC differ on these items:
...
| + A new Ruby-gRPC server for vscode: likely faster because we can limit dependencies to load ([modular monolith](https://example_company.com/example_company-org/example_company/-/issues/365293)) | - Existing Grape API for vscode: meaning slow boot time and unneeded resources loaded             |
...
| - Hard to pass on unknown messages from vscode through Example Company to ai-gateway                                                                                             | + Easier support for newer VS Code + newer AI-gateway, through old Example Company instance                  |
...
Our goal is to minimize code that we can't update on a customer's behalf, which means avoiding hard-coding AI-related logic in the Example Company monolith codebase:
...
- We want to be able to serve all Example Company distributions (.com SaaS, Self-managed, and Dedicated) with minimal complexity.
...
Having the business logic in Example Company Rails requires customers to upgrade their Example Company instances, which affects the first point. Some of the on-premises users cannot upgrade their instances immediately due to their company policy.
For example, if we had a bug in a prompt template in Example Company Rails and fixed it in 16.6, and customers are using 16.5 and the next upgrade is scheduled in 3 months, they have to use the buggy feature for 3 months.
...
Example Company installations as the AI landscape changes.

### The AI-Gateway API protocol

It is important to build each single-purpose endpoint, in a version-agnostic way so it can be used by different Example Company instances and by external clients. To achieve this goal:
...
1. `metadata`: This field contains information about a client that built this `prompt_components` envelope. Information from the `metadata` field may, or may not be used by Example Company for
...
discussed in [#415745](https://example_company.com/example_company-org/example_company/-/issues/415745).
We will try to come up with an architecture for all AI-related features.

#### Exposing AI providers

A lot of AI functionality has already been built into Example Company Rails
that currently builds prompts and submits this directly to different
AI providers. At the time of writing, Example Company has API-clients for the
following providers:

- [Anthropic](https://example_company.com/example_company-org/example_company/blob/4344729240496a5018e19a82030d6d4b227e9c79/ee/lib/example_company/llm/anthropic/client.rb#L6)
- [Vertex](https://example_company.com/example_company-org/example_company/blob/4344729240496a5018e19a82030d6d4b227e9c79/ee/lib/example_company/llm/vertex_ai/client.rb#L6)

To make these features available to self-managed instances, we should
provide endpoints for each of these that Example Company.com, self-managed or
...
Having the proxy in between Example Company and the AI provider means that we
...
the features of older Example Company installations even if the provider's API
...
AI-supported features without having to upgrade their Example Company instance.

Features that are currently
[experimental](https://docs.example_company.com/ee/policy/experiment-beta-support.html#experiment)
can use these generic APIs, but we should aim to convert to a single
purpose API endpoint before we make the feature [generally available](https://docs.example_company.com/ee/policy/experiment-beta-support.html#generally-available-ga)
...
### API in Example Company instances

This is the API that external clients can consume on their local
Example Company instance. For example VSCode that talks to a self-managed
instance.

These versions could also widely defer: it could be that the VSCode
extension is kept up-to-date by developers. But the Example Company instance
...
to the appropriate Example Company installation. Example Company Rails can wrap the
payload in an envelope for the AI-gateway without having to interpret
it.

When we do this then the Example Company-instance that receives the request
from the extension doesn't need to understand it to enrich it and pass
it on to the AI-Gateway. Example Company can add information to the
...
Example Company Rails needs to forward the entire payload in addition to any
...
an outdated Example Company installation to a recent AI-gateway.
...
Example Company provides the first layer of authorization by authenticating the user and checking if the license permits using the requested feature. This is accomplished using the existing authentication, policy, and license checks built into Example Company.

Authenticating the Example Company instance on the AI-gateway was discussed in:

- [Issue 177](https://example_company.com/example_company-org/modelops/applied-ml/code-suggestions/ai-assist/-/issues/177)
- [Epic 10808](https://example_company.com/groups/example_company-org/-/epics/10808)

The specific mechanism by which trust is delegated between end-users, Example Company instances, and the AI-gateway is detailed in the [Cloud Connector access control documentation](https://docs.example_company.com/ee/development/cloud_connector/architecture.html#access-control).

The AI Gateway, as a backend service, handles instance authentication among other tasks. It is accessed through the Cloud Connector Load Balancer (currently implemented as Cloudflare), which acts as a Web Application Firewall (WAF) layer but does not perform authentication. The AI Gateway also needs to support end-user authentication because some requests, such as code completion, will be sent directly by clients rather than indirectly through Example Company Rails. A possible solution involving short-term user tokens is described in [Epic 13252](https://example_company.com/groups/example_company-org/-/epics/13252). The AI Gateway must distinguish between requests proxied by Example Company Rails and direct client requests, as some endpoints or parameters may not be available for direct requests (e.g., clients should only send sub-components of the final prompt, which the AI Gateway will build).

The AI Gateway uses JSON-based API communication and relies on JWTs for authentication. These JWTs are generally scoped to the Example Company instance and are obtained using a PAT or OAuth token. For Code Completions, a short-lived user-bound JWT is used, allowing direct communication between the client and the AI Gateway, bypassing the need to go through the Example Company instance for each request. This short-lived JWT is valid for one hour, reducing the number of JWTs needed for multiple prompt requests.

When a client initiates a prompt in Example Company Duo Chat or an IDE, the prompts, telemetry, context, and JWT token are packaged and transmitted using TLS. It is important to note that the payload itself is not encrypted; it is passed as plain text in JSON format within the request body. This approach is consistent for calls to third-party models where only tunnel-level TLS encryption is used via HTTPS.

The AI Gateway operates within Example Company hosted infrastructure, interfacing with APIs hosted in Example Company accounts for providers like Anthropic and Vertex. Models are hosted as frozen versions of Google's models within Example Company GCP tenancy, using private endpoints within the security perimeter. While prompts from different customers use the same shared model, each session maintains a user-level connection to ensure isolation.
...
NOTE:
For the embedding database, see [RAG for Example Company Duo](https://docs.example_company.com/ee/architecture/blueprints/gitlab_duo_rag/index.html).
...
[`example_company-org/modelops/applied-ml/code-suggestions/ai-assist`](https://example_company.com/example_company-org/modelops/applied-ml/code-suggestions/ai-assist).
...
[Runway](https://example_company.com/example_company-com/gl-infra/platform/runway/). At
...
[&10563](https://example_company.com/groups/example_company-org/-/epics/10563).
...
our [docs](https://docs.example_company.com/ee/administration/self_hosted_models/install_infrastructure.html).
...
#### AI Gateway and Example Company version matching

Customers need a clear indication of which AIGW version is compatible to their Example Company instance. 
Since AI Gateway deploument is not managed by Omnibus/GNG, we achieve this by providing image tags 
for each Example Company version: an AIGW image with TAG `example_company-17.2-ee` is compatible with Example Company version `17.2-ee`.

[More discussion on versioning](https://example_company.com/example_company-org/modelops/applied-ml/code-suggestions/ai-assist/-/issues/579)

## Alternative solutions

Alternative solutions were discussed in
[applied-ml/code-suggestions/ai-assist#161](https://example_company.com/example_company-org/modelops/applied-ml/code-suggestions/ai-assist/-/issues/161#what-are-the-alternatives).
...
AI Gateway aim is to become the primary method for the monolith to **access** machine learning models across all usages of Example Company and create a consistent user journey when developing AI-backed features. To do so, these goal is split down into three categories:
...
The AI Gateway, a standalone service, is the sole access point for all communication between Example Company installations and third-party AI models. It is designed to centralize and manage access to all Example Company features, whether they are in-app functionalities or code suggestions, irrespective of their deployment methods.

This strategy significantly simplifies enterprise management and abstracts machine learning away from the monolith. With future expansions including telemetry, embeddings API, and multi-region/customer-specific deployments, our goal is to provide a scalable, comprehensive AI solution for all Example Company users, regardless of their installation type.

[Model registry](https://docs.example_company.com/ee/user/project/ml/model_registry/index.html) is a feature that allows users to use Example Company to manage the machine learning models. While not solely focused on large language models, and currently more targeted at smaller model applications, which could be deployed in various ways: as a standalone library, a service, a pod, a cloud deployment, and so forth. For these user-deployed models, the ability to auto-configure an API that's accessible through the AI Gateway could be a significant feature.

- [AI Gateway as the Sole Access Point for Monolith to Access Models](https://example_company.com/groups/example_company-org/-/epics/13024)
...
For more details on how to implement and integrate unit primitives, refer to the [Cloud Connector documentation](https://docs.example_company.com/ee/development/cloud_connector/index.html).
