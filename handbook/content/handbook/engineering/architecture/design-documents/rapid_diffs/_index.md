...
Diffs at Example Company are spread across several places with each area using their own method. We are aiming
...
[Rapid Diffs work](https://example_company.com/groups/example_company-org/-/epics/13236) is split into 3 stages and can be tracked in the following epics:

1. [Stage 0 — foundation](https://example_company.com/groups/example_company-org/-/epics/13764):
    - Have foundational components in place.
    - Stream diffs on MR, commit and compare revisions pages.
1. [Stage 1 — baseline features](https://example_company.com/groups/example_company-org/-/epics/13765):
    - Most of the features are working (dicussions, navigation, review, etc.)
1. [Stage 2 — production ready](https://example_company.com/groups/example_company-org/-/epics/13767):
...
- We will store large bodies of research, the results of audits, etc. in the [wiki](https://example_company.com/example_company-com/create-stage/new-diffs/-/wikis/home) of the [RRD project](https://example_company.com/example_company-com/create-stage/new-diffs)
...
Example Company has an existing [definition of done](https://docs.example_company.com/ee/development/contributing/merge_request_workflow.html#definition-of-done) which is geared primarily toward identifying when an MR is ready to be merged.

In addition to the items in the Example Company definition of done, work on RRD should also adhere to the following requirements:
...
1. The RRD app can execute in total isolation from the rest of the Example Company product:
...
1. Is:
   ...
   1. Malleable.
   1. Flexible.
1. Considers itself a mission-critical part of the overall Example Company product.
...
1. Use [HTML streaming](https://example_company.com/example_company-org/frontend/rfcs/-/issues/101)
...
We recognize that in order to have an accessible experience using diffs in the context of Example Company, we need to ensure the compliance both for displaying and interacting with diffs. That's why the accessibility
...
[constant bugs](https://example_company.com/example_company-org/example_company/-/issues/427155#note_1607184794).
...
- Frontend templates (Vue) Server Side Rendered ([tested](https://example_company.com/example_company-org/example_company/-/merge_requests/33052#note_350101205)): results and impact weren't compelling and pointed in the direction of partial SSR. ([PoC MR](https://example_company.com/example_company-org/example_company/-/merge_requests/33052))
...
- Micro Code Review Frontend PoC (tested): This approach was significantly different from the application design used in the past, so it was never seriously explored as a way forward. Parts of this design - like custom elements and a reliance on events - have been incorporated into alternative approaches. ([Micro Code Review Frontend PoC](https://example_company.com/example_company-org/example_company/-/issues/389282))
- Streaming Diffs using a node server (tested): Combines streaming with a dedicated nodejs server. Percursor to the proposed SSR approach in this blueprint. ([PoC: Streaming diffs app](https://example_company.com/example_company-org/example_company/-/merge_requests/84563))
...
(See [New Diffs: Technical Architecture Design](https://example_company.com/example_company-org/example_company/-/issues/431276) for nicer visuals of this chart)
