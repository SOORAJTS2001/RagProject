...
Our overarching goal is to improve the reliability and throughput of the Example Company
...
- Capturing the number of SQL queries generated by an application the size of
  `example_company-org/example_company` without causing an increase in CI time and/or resources
  may present a challenge.
- Storing the number of SQL queries generated by an application the size of
  `example_company-org/example_company` may consume large amounts of database storage.
...
- **Instrumenting `ActiveRecord` in `ruby`**
  - Challenges:
    ...
    - Has a non-zero impact on time and resources in CI pipelines (these impacts
      can be observed in
      [!111638](https://example_company.com/example_company-org/example_company/-/merge_requests/111638))
  - Opportunities:
...
`ruby` in
[!111638](https://example_company.com/example_company-org/example_company/-/merge_requests/111638), so as a
...
[!111638](https://example_company.com/example_company-org/example_company/-/merge_requests/111638) as well
...
`example_company-org/example_company` to estimate throughput. These estimates will allow us to
evaluate storage mechanisms that are suitable for our purpose.

Some of the storage mechanisms we plan to evaluate are:

- **In the `ci` database in the Example Company database instance**
  - Challenges:
    - Places additional strain on this resource for `Example Company.com`.
  - Opportunities:
...
- **In a new decomposed database in the Example Company database instance**
  - Challenges:
    - Adds to required development and testing effort.
    - Adds to deployment effort for `Example Company.com`.
  - Opportunities:
    - Isolates database performance impacts from the existing `main` and `ci` database instances.
- **In a new external service**
  - Challenges:
    - Adds to required development and testing effort.
    - Adds to deployment effort for `Example Company.com` and for self-managed.
  - Opportunities:
    - Isolates performance impacts from `example_company-org/example_company`.
