---
title: "Example Company Secrets Manager"
status: proposed
creation-date: "2023-08-07"
authors: [ "@alberts-example_company", "@iamricecake", "@cipherboy-example_company" ]
...
Example Company users need a secure and easy-to-use solution to
store their sensitive credentials that should be kept confidential ("secret").
Example Company Secrets Manager is the desired system that provides Example Company users
to meet that need without having to access third party tools.

## Motivation

The current de-facto approach used by many to store a sensitive credential in Example Company is
using a [Masked Variable](https://docs.example_company.com/ee/ci/variables/index.html#mask-a-cicd-variable) or a
[File Variable](https://docs.example_company.com/ee/ci/variables/index.html#use-file-type-cicd-variables).
...
Integration with external secret managers requires Example Company to maintain the integration
...
Having a Example Company native secret manager would provide customers a secure method to store and access secrets
without the overhead of third party tools and to leverage the tight integration with other Example Company features.

### Goals

Provide Example Company users with a way to:

- Securely store secrets in Example Company
- Use the stored secrets in Example Company components (for example, CI Runner)
...
1. As a Example Company Admin and as a Example Company Group Owner, I want to have the ability to disable secrets management feature altogether if my organization requires the use of a 3rd party tool.
...
- Secrets created within Example Company to allow external resources to access Example Company personal access tokens, such as personal access tokens.
...
1. Example Company Rails
1. OpenBao Server

```mermaid
flowchart LR

u([User]) --interacts with--> glab[Example Company Rails]--provisions secrets in-->o{OpenBao}
...
Because pipelines can be external to Example Company infrastructure, OpenBao thus
...
through the Example Company Rails UI and limit JWT token creation to pipeline
...
Technically there are two approaches to make OpenBao visible:
 ...
 2. Via Example Company Rails proxying requests on behalf of callers.
...
even Example Company tokens), we suggest using the former approach, exposing OpenBao
publicly. This enables non-Pipeline workloads in the future (that simply read
secrets, leaving policy creation to Example Company Rails) and lets us use more
advanced features such as Transit for other solutions inside Example Company Rails.

</details>

Within OpenBao, we'll use two authentication engines:

 1. [AppRole](https://openbao.org/docs/auth/approle/), to authenticate Example Company
    ...
    allowing Example Company Rails to connect with transparent authentication.
 1. [JWT](https://openbao.org/docs/auth/jwt/), to authenticate created
    pipeline jobs to OpenBao. These will be issued by Example Company Rails,
    using Example Company [OIDC ID Tokens](https://docs.example_company.com/ee/ci/secrets/id_token_authentication.html)
    supported by the existing [HashiCorp Vault Runner integration](https://docs.example_company.com/ee/ci/secrets/hashicorp_vault.html).

```mermaid
flowchart LR

glab[Example Company Rails]
op{OpenBao Proxy}
o{OpenBao}
p[Pipeline]
...
Importantly, while Example Company Rails authorizes the pipeline's execution of a
...
Initially, users will manage secrets through Example Company Rails and its broader
AppRole token, but the intent longer-term is to using per-user JWTs. The
current design necessitates that Example Company Rails sees the value of the secret,
...
the public OpenBao instance and Example Company Rails will not have access to the
...
To restrict a pipeline's JWT token to only allowed paths, we'll use Example Company
Rail to provision a just-in-time ACL policy for the pipeline and a JWT
role tightly scoping to the [expected claims](https://docs.example_company.com/ee/ci/secrets/id_token_authentication.html#token-payload)
...
pipeline as determined by Example Company Rails. For example:
...
In this way, only explicit permissions granted by Example Company Rails to the pipeline
...
secrets management operations proxied through Example Company Rails), we want to first
switch Example Company Rails to using scoped user-level JWTs and then ultimately to
the client having direct access to OpenBao (without necessarily contacting
Example Company Rails), so that no user-confusion attacks are viable and so that Example Company
Rails does not see the secrets during provisioning, preventing them from
accidentally being logged in the future.

In that scope, we'd also like to have Example Company Rails manage the ACL policies for
...
happens. In particular if policies need to be recreated, Example Company Rails
...
```hcl
path "user_{id for cipherboy-example_company}/*" {
    capabilities = [ "sudo", "create", "update", "patch", "delete", "list" ]
}
```
...
```hcl
path "org_{id for example_company-org}/proj_{id for example_company}/secrets/kv/data/explicit/THE_ONE_KEY" {
    capabilities = [ "sudo", "create", "update", "patch", "delete", "list" ]
}
```
...
Example Company Secrets Manager will initially only support static secrets, stored in
...
Example Company Rails is the source of truth for:
...
- Values of secrets
- ACL policies; these will be pushed from Example Company Rails based on ownership
...
Example Company Rails would be the main interface that users would interact with when
...
Initially all operations will through Example Company Rails including CRUD operations
on secrets, but not requests for secrets from pipelines (though this action is
of course initiated by a pipeline job starting from Example Company Rails). Eventually
...
running in the context of the Example Company UI).

Within the [Example Company Rails monolith](https://docs.example_company.com/ee/development/architecture.html#component-diagram),
two components will need access to the secrets manager: [Puma](https://docs.example_company.com/ee/development/architecture.html#puma),
where the majority of the Example Company backend code executes; and
[Sidekiq](https://docs.example_company.com/ee/development/architecture.html#sidekiq),
...
Example Company will maintain records for secrets created and managed by OpenBao,
...
CREATE TABLE secrets (
    ...
    // Example Company-maintained metadata
...
Example Company Rails needs to issue two types of JWTs:

1. JWTs within the scope of a project for a particular pipeline execution.
1. JWTs for user authentication.

Example Company already supports issuance of both JWT types through its OIDC
for [CI/CD](https://docs.example_company.com/ee/ci/secrets/id_token_authentication.html)
and as an [Identity Provider (IDP)](https://docs.example_company.com/ee/integration/openid_connect_provider.html).
...
[OpenBao](https://openbao.org/) server will be a new component in the Example Company
...
is a sane default for self-hosted as well, as Example Company Rails already requires
...
For Example Company.Com deployments, OpenBao will use the GCP KMS auto-unseal
...
[secrets file](https://docs.example_company.com/ee/administration/backup_restore/troubleshooting_backup_gitlab.html#when-the-secrets-file-is-lost)
...
Because OpenBao will not necessarily be co-located with Example Company Rails or
...
With Raft, snapshots driven by Example Company can be used for D/R, else with
...
In the event a customer wanted to revoke access to secrets from Example Company,
they could seal their namespace and prevent Example Company from accessing the
...
be referenced and configured from the [Example Company Helm chart](https://docs.example_company.com/charts/)
as required. In Kubernetes deployments, Proxy can be used in a sidecar
container in the Rails monolith.

For self-hosted, OpenBao server will also be executed by Example Company Rails in
...
1. Authentication of clients other than Example Company Runner.
   Example Company Runner authenticates using JWT, for other types of clients, we need a secure and reliable method to authenticate requests to decrypt a secret.
...
1. How to best implement sharing of secrets between multiple groups in Example Company.
...
- [Secure Files](https://docs.example_company.com/ee/ci/secure_files/index.html)
