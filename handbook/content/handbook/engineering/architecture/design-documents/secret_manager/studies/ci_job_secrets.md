...
- To map out how users can use their native Example Company secrets in their CI jobs.
- Given OpenBao is a fork of HashiCorp Vault, we want to confirm its compatibility with our [Vault integration in Runner](https://docs.example_company.com/ee/ci/secrets/index.html).
- At a high level, gain a better understanding of how to structure OpenBao [policies](https://openbao.org/docs/concepts/policies/) and [JWT roles](https://openbao.org/docs/auth/jwt/#configuration) to be compatible with a project's varied permissions per Example Company user role.
...
bao write auth/jwt/role/project_owner - <<EOF
{
  ...
  "bound_audiences": "secrets.example_company.com",
...
1. Project owner enables Example Company Secrets manager through the Example Company UI.
1. Project owner defines additional permissions on which Example Company user roles can read, write, or create secrets through the Example Company UI.
...
1. Project owner defines secrets through the Example Company UI.
...
1. Developer uses the `secrets` keyword in the `.example_company-ci.yml`.
   ...
   - There is no need to specify `id_tokens:VAULT_ID_TOKEN` as `aud` defaults to `https://secrets.example_company.com` where OpenBao service is.
   - Unlike with HashiCorp Vault, there is no need to define CI/CD variables.
     - The `VAULT_SERVER_URL` defaults to `https://secrets.example_company.com` where OpenBao service is.
...
1. The CI job runs and `MY_SECRET_ON_OPENBAO` is available as an environment variable.
   - OpenBao verifies the integrity of the ID token and validates the `bound_claims` if it matches the custom claims, specially the `user_access_level` which contains the Example Company user role of the user.
   - Similar to HashiCorp Vault secrets, this is a [`file` variable](https://docs.example_company.com/ee/ci/variables/index.html#use-file-type-cicd-variables).
...
1. The OpenBao service needs to be properly configured to make it compatible with the workflow.
   - Configure [JWT authentication](https://openbao.org/docs/auth/jwt/#jwt-authentication) to make it work with [ID tokens authentication](https://docs.example_company.com/ee/ci/secrets/id_token_authentication.html#automatic-id-token-authentication-with-hashicorp-vault).
   - The documentation shows [instructions](https://docs.example_company.com/ee/ci/secrets/index.html#configure-your-vault-server) using the `vault` CLI, but it should work similarly for `bao`.
   - The OpenBao API is reachable through `https://secrets.example_company.com`.
...
1. Make sure [GDK is properly set up with runner](https://example_company.com/example_company-org/example_company-development-kit/blob/main/doc/howto/runner.md).
   - Tested on a [GDK with Docker executor](https://example_company.com/example_company-org/example_company-development-kit/blob/main/doc/howto/runner.md#set-up-a-local-network) and pointed `gdk.test` to `172.16.123.1` but this should also work with a shell executor.
...
1. To test the policy and role generated for a project owner with the Example Company user role `owner`, create the [templated policy](https://openbao.org/docs/concepts/policies/#templated-policies) and the JWT role for the specific `owner` role. The JWT role was based on the [Example Company Vault sample server role](https://docs.example_company.com/ee/ci/secrets/index.html#configure-vault-server-roles).
...
1. On the test project, configure the `.example_company-ci.yml` to fetch secrets from OpenBao using the existing [Vault integration](https://docs.example_company.com/ee/ci/secrets/index.html#use-vault-secrets-in-a-ci-job).
   ...
   test_openbao:
     ...
     id_tokens:
       VAULT_ID_TOKEN:
       aud: secrets.example_company.com
     secrets:
       SECRET:
   ...
   - The ID token generated in this job is matched by OpenBao using the `bound_claims`, specifically the `user_access_level` which is included in the [custom claims](https://docs.example_company.com/ee/ci/secrets/id_token_authentication.html#token-payload) of the ID token.
