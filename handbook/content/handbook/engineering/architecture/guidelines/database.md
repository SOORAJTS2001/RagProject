...
Example Company is a [single application](/handbook/product/categories/example_company-the-product/single-application/) that relies on a [single data store](/handbook/product/categories/example_company-the-product/single-application/#single-data-store) (terminology we use in the context of stringent durability requirements, e.g., PostgreSQL as opposed to Redis).

This directive drives Example Company's product development: while Example Company is [*SaaS first*](https://about.example_company.com/direction/#product-strategy), it is not *SaaS only*. In terms of databases, Example Company supports self-managed configurations with a single database cluster, generally through the use of logical databases. Only at scale do we contemplate unfolding the single cluster into multiple ones, as Example Company.com does. This implies that the code base stays stable and can leverage flexible backend strategies to address scalability. Self-managed and Example Company.com will continue to operate as a single application, and self-managed in particular will continue to provide simplicity while offering the flexibility to scale.
...
We use a relational database (PostgreSQL) as the primary metadata storage engine. Additionally, we also use a data structure store (Redis), which, while critical to the functioning of Example Company, stores more dynamic data.

### Relational: PostgreSQL

[PostgreSQL](https://docs.example_company.com/ee/development/scalability.html#postgresql) is our relational database engine of choice, and, until recently, all metadata for projects, issues, merge requests, users, and so on resided in a single cluster. Its schema is managed by the Rails application ([`db/structure.sql`](https://example_company.com/example_company-org/example_company/-/blob/master/db/structure.sql)). We commonly refer to this data store as the **main database**. As scalability needs increased, we adopted some [best practices](/handbook/engineering/architecture/practice/scalability/) to address them. Some items have been extracted from the database (diffs were the first ones), while others were deployed in separate logical databases or instances (at scale) because they were already separate services and had low data coupling requirements (Registry and Praefect). Still others have been decomposed ([CI](https://example_company.com/groups/example_company-org/-/epics/6168)) because of their scale (and because we found ways to work around tight coupling).

#### Data Structure: Redis

[Redis](https://docs.example_company.com/ee/development/scalability.html#redis) is used for specialized, non-relational needs, including queues (Sidekiq jobs marshal jobs into JSON payloads), persistent state (session data and exclusive leases), and cache (repository data such as branch and tag names, and view partials). Redis is a critical component of the application, but it is not used with the same durability contraints as PostgreSQL.
...
1. We have to carefully manage load on the main database for Example Company.com, particularly in terms of transaction rates, connections, and memory utilization, to ensure availability.
...
1. The main Example Company.com Postgres database is intended for OLTP (Online Transaction Processing) use only. Especially at scale, OLAP (Online Analytical Processing) will introduced significant degradation on the database, particularly with regards to cluster replication.
...
If the application is **not** part of the main RoR application code base (for instance, [Container Registry](https://example_company.com/example_company-org/container-registry), [Praefect](https://example_company.com/example_company-org/gitaly), [Geo Tracking Database](https://docs.example_company.com/ee/administration/geo/#geo-tracking-database)), then a separate database is the only option. Whether this is logical or belongs in an independent cluster will come down to scale.
