...
1. This process is designed for the following issues:
   1. **Example Company.com** and **self-managed hosting** **`operational emergencies`** raised by the **Infrastructure** , **Security**, and **Support** teams.
...
1. This process is **NOT** a path to reach development team for non-urgent issues that the Infrastructure, Security, and Support teams run into. Such issues can be moved forward by:
   1. Labelling with `security` and the `@example_company-com/security/appsec` team mentioned to be notified as part of the [Application Security Triage rotation](/handbook/security/#triage-rotation)
   1. Labelling with `infradev` which will be raised to [Infra/Dev triage board](https://example_company.com/groups/example_company-org/-/boards/1193197?label_name[]=example_company.com&label_name[]=infradev)
...
1. Production issue examples:
   1. Example Company.com: [DB failover and degraded Example Company.com performance](https://example_company.com/example_company-com/gl-infra/production/issues/1054)
   ...
   1. Self-managed: [Customer ticket - Our Example Company instance is down - request an emergency support](https://example_company.zendesk.com/agent/tickets/129514)
   1. Self-managed: [Customer ticket - Database overloaded due to Sidekiq stuck jobs](https://example_company.zendesk.com/agent/tickets/130598)
1. Engineering emergency examples:
   1. [A post deployment issue with version.example_company.com](https://example_company.com/example_company-com/gl-infra/production/issues/1615) that will cause self-managed deployment failure.
   1. [Example Company.com deployment](https://example_company.com/example_company-org/example_company/issues/198440) or a security release is blocked due to pipeline failure.
   1. [A severity::1 regression found in the release staging.example_company.com](https://example_company.com/example_company-org/example_company/issues/199316)
   1. [Evaluating severity and making decision on a potential release or deployment show stopper](https://example_company.com/example_company-org/example_company/-/issues/217450) collaboratively when other engineering teams are challenged to accomplish alone.
   1. [`master` pipelines are broken](https://example_company.com/example_company-org/example_company/-/issues/378474).

#### Examples of non-qualified issues

1. Production issue examples:
   1. Example Company.Com:[Errors when import from GitHub](https://example_company.com/example_company-org/example_company-ce/issues/66166)
   1. Example Company.com: [Last minute security patch to be included in an upcoming release](https://example_company.com/example_company-org/omnibus-example_company/issues/4530)
   1. Self-managed(ZD): [View switch causing browser freeze](https://example_company.com/example_company-org/example_company-ce/issues/52479)
   1. Self-managed(ZD): [Watch Everything Notification Level](https://example_company.com/example_company-org/example_company-ee/issues/14214)
1. Engineering issue examples:
   1. [A priority::1/severity::1 enhancement of CI](https://example_company.com/example_company-org/example_company/issues/36154)
   1. [A priority::1/severity::1 fix to API](https://example_company.com/example_company-org/example_company-foss/issues/65381)
   1. [Non release blocking QA failures on staging.example_company.com](https://example_company.com/example_company-org/example_company/issues/198692)

### Process Outline

**NOTE:** On-call engineer do not need to announce the beginning and end of their shift in [#dev-escalation](https://example_company.slack.com/messages/CLKLMSUR4) unless there is an active incident happening (check the chat history of the channel to know if there is an active incident). This is because many engineers have very noisy notifications enabled for that channel, and such announcements are essentially false positives which make them check the channel unnecessarily.

#### Weekdays (UTC)

Weekdays will now leverage an automated system relying upon the [Pagerslack](https://example_company.com/jameslopez/pagerslack).
...
- [Get attention from Fulfillment team](/handbook/engineering/development/fulfillment/#escalation-process-for-incidents-or-outages): issues related to CustomersDot (customers.example_company.com).

### First response time SLOs

**OPERATIONAL EMERGENCY ISSUES ONLY**

1. **Example Company.com**: Development engineers provide initial response (not solution) in both [#dev-escalation](https://example_company.slack.com/messages/CLKLMSUR4) and the tracking issue within **15 minutes**.
1. **Self-managed**: Development engineers provide initial response (not solution) in both [#dev-escalation](https://example_company.slack.com/messages/CLKLMSUR4) and the tracking issue on a best-effort basis. (SLO will be determined at a later time.)
1. In the case of a tie between Example Company.com and self-managed issues, Example Company.com issue takes priority.
1. In the case of a tie between production (Example Company.com, self-managed) and engineering issues, production issue takes priority. The preferred action is to either backout or rollback to the point before the offending MR.

### Required Slack Channel and Notification Settings

1. All on-call engineers, managers, distinguished engineers, fellows (who are not co-founders) and directors are required to join [#dev-escalation](https://example_company.slack.com/messages/CLKLMSUR4).
...
Engineers claim shifts between two and three months in advance. When signing up, fill the cell with your **full name** as it appears in the team members list (internal only), **Example Company display name**, and **phone number with country code**. This same instruction is posted on the header of schedule spreadsheet too.
...
- Ensuring assigned on-call shifts fit between the hours of 8am-8pm local time for all team members in general, and 7am-9pm for engineers in the US Pacific Time timezone (approved in [this issue](https://example_company.com/example_company-com/www-example_company-com/-/issues/9638#note_1035704658)).
...
- In the instance of an ongoing escalation, no engineer should finish
  ...
  1. Post a message to the existing incident thread in the [#dev-escalation](https://example_company.slack.com/messages/CLKLMSUR4) channel, or create a new thread requesting volunteers to take over the shift.

  1. Message the [on-call IMOC](https://dashboards.example_company.net/goto/5CwoDarSg?orgId=1) and ask them for help finding an engineer to take over the shift.
...
- In the instance of an ongoing escalation being handed over to
  ...
  - Anything that helps bring the next on-call engineer up to speed quickly
    ...
    - *Existing* threads in [#dev-escalation](https://example_company.slack.com/messages/CLKLMSUR4)
    - Incident tracking issues

    This shall be completed at the end of shifts to hand over smoothly.
- For current Infrastructure issues and status, refer to [Infra/Dev Triage](https://example_company.com/groups/example_company-org/-/boards/1193197?&label_name[]=example_company.com&label_name[]=infradev) board.
...
- If an incident is ongoing at the time of handover, outgoing engineers may
 ...
 or second shift.
- If you were involved in an incident which has been mitigated during your shift, leave a note about your involvement in the incident issue and link to it in the [#dev-escalation](https://example_company.slack.com/messages/CLKLMSUR4) Slack channel indicating you participated in the issue as an informational hand-off to future on-call engineers.

### Development on-call DRI

**Current DRI: [James Lopez](example_company.com/jameslopez)**
...
1. Select coordinators for the monthly schedule (at least 15 days before the new month starts) - a [tool](https://example_company.com/jameslopez/coordinator-finder) can be used to assist with this
...
1. Monitor `#dev-escalation` and `#dev-oncall` on Slack and resolve any problems with [the escalation bot](https://example_company.com/jameslopez/pagerslack) and the [PeopleOps integration](https://example_company.com/example_company-com/people-group/peopleops-eng/employment-automation/-/blob/main/lib/syncing/on_call_scheduling_spreadsheet.rb)
...
The coordinators should assign themselves to the corresponding monthly issue from this [epic](https://example_company.com/groups/example_company-com/-/epics/122).
...
An [Epic of execution tracking](https://example_company.com/groups/example_company-com/-/epics/122) was created, where each coordinator is expected to register an issue under this Epic for the month-on-duty to capture activities and send notifications. Here is [an example](https://example_company.com/example_company-com/www-example_company-com/issues/4965).

#### How-To

Refer to this [coordinator issue template](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/.example_company/issue_templates/dev-oncall-monthly-execution.md) for instructions or the steps below. The same template is used for monthly on-call planning and execution issues.

An [automated process](https://example_company.com/example_company-com/dev-on-call) can be followed. If there are problems with the tool, a manual process can be followed:
...
1. Go to the [Development Team BE spreadsheet](#development-team-be-spreadsheet) and filter the "timezone" column by the desired timezones for the shift. Now you have the list of possible people that can take this shift. Alternatively, you can use [this dev-on-call tool](https://example_company.com/example_company-com/dev-on-call) to find out people who may take this shift.
...
*Please report any problems by creating an issue in the [pagerslack project](https://example_company.com/jameslopez/pagerslack/-/issues).*
...
To get an idea of [what's expected](#expectation) of an on-call engineer and how often incidents occur it can be helpful to shadow another shift. To do this simply identify a time-slot that you'd like to shadow in the [On-call spreadsheet](#on-call-spreadsheet) and contact the primary to let them know you'll be shadowing. Ask them to invite you to the calendar event for this slot. During the shift keep an eye on [#dev-escalation](https://example_company.slack.com/archives/CLKLMSUR4) for incidents and observe how the primary [follows the process](#process-outline) if any arise.
...
[Tableau](https://10az.online.tableau.com/#/site/example_company/workbooks/2225419/views)
