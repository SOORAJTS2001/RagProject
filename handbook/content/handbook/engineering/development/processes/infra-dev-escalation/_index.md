...
Historically there have been challenges in consistently maintaining our service level for Example Company.com customers. See the impacts in this [Example Company.com performance degradation summary document](https://docs.google.com/document/d/1GfwzPc1uavB5ZuiA9O8l5sUD7Sq2l2BlwevM5AvOtZA/edit).

This issue is not unique to Example Company when business grows fast and the user base and workload on the hosted SaaS increases exponentially. As a result, the business growth requires corresponding changes in how we work so that customers continue experiencing best service by Example Company, which maintains and boosts our business growth momentum.

With that being said and the operation incidents recently, it becomes clear that we'll strengthen development team's DevOps practices and stand side by side with the Infrastructure team to keep Example Company.com running smoothly.

## Process

To resolve Example Company.com issues faster, the development team will establish an on-call rotation and stand behind the products we deliver. During weekdays we are running a pilot in Sep/Oct of 2020 whereby an engineer in a Slack channel will be notified when they are online (in least recently escalated to order).  During weekends (when most are not online) a spreadsheet will be used for escalation, with time slots required to be filled to ensure we have coverage.  During the pilot program the spreadsheet will be used as a backup.
...
**A:** Yes. During regular weekdays we use [Pagerslack](https://example_company.com/jameslopez/pagerslack) to  page engineers based on who is online in Slack.  During the weekends/holidays we still need the spreadsheet as most do not regularly work on the weekends.
...
Example Company.com, since part of their job description is to work towards achieving Example Company.com SLOs. And those in Engineering Productivity are focused on providing
...
**A:** Let's describe the full picture of our incident handling model and tell candidates there are chances development engineers will be on-call and assist resolving Example Company.com operational incidents.
...
**A:** It's actually both and we are working to address both.  We have also added a Infra/Dev issue board to track concerns to see through to resolution and make sure we have the right priority and severity on them.  It's likely that oncall escalations will end up with follow on items for [this board](https://example_company.com/groups/example_company-org/-/boards/1193197?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=example_company.com&label_name[]=infradev). [This page](/handbook/engineering/development/#continuous-delivery-and-infrastructure-collaboration) gives the description.

**Q: How should the infrastructure member make international phone calls to page engineers?**

**A:** Zoom supports making international calls [with low rates](https://zoom.us/zoomconference/rates). This can be done from inside an ongoing Zoom call under `Invite > Phone`. Considering that this will only be used for a quick phone call, to alert the engineer of an ongiong escalation, the cost for Example Company will be very minimal.
