...
Our mission is to empower Example Company system administrators with the toolkit they need to create their desired balance of security and accessibility for their Example Company experience. Authentication is the first impression any new customer has when they configure their shiny new Example Company instance, and we aim to make it as seamless as possible: from that moment of first logging in, to onboarding users, to managing the basic security rules for their instance in a secure, flexible and scalable manner.

#### Top Priorities for FY25

Our detailed priority list can be found at the [direction page](https://about.example_company.com/direction/govern/authentication/#priorities) however on a higher level the focus would be on:
...
6. Bringing Credentials inventory to Example Company.com
...
- [Improving support and help request experience](https://example_company.com/example_company-com/example_company-OKRs/-/work_items/5858)
- [Reducing the number of flaky or slow tests](https://example_company.com/example_company-com/example_company-OKRs/-/work_items/5853)
- [Career growth of Authentication team members](https://example_company.com/example_company-com/example_company-OKRs/-/work_items/5643)
- [Adopting Cells and sharing lessons from investigation spikes](https://example_company.com/example_company-com/example_company-OKRs/-/work_items/6280)
- [Roadmap completion around GCP Integration](https://example_company.com/example_company-com/example_company-OKRs/-/work_items/6186)
...
1. Our team's merge requests should be assigned to another Auth team member for first review in order to build more institutional knowledge across the team. This review should be done as a [reviewer](https://docs.example_company.com/ee/development/code_review.html#the-responsibility-of-the-reviewer). The Auth approval counts as the approval matching the role of the Auth Reviewer, e.g. having a Backend Review from Auth counts as a Backend Review. Once approved, the Auth Reviewer should request a review from a Maintainer from the appropriate [maintainer category](https://docs.example_company.com/ee/development/code_review.html#approval-guidelines).
...
1. Auth related merge requests require a review by an [Auth Engineer](https://example_company.com/groups/example_company-org/govern/authentication/approvers/-/group_members?with_inherited_permissions=exclude). This is guarded by using the [CODEOWNERS](https://docs.example_company.com/ee/user/project/codeowners/) feature of Example Company.
...
- We do an optional, asynchronous daily stand-up in our group stand-up channel:
  - Govern:Authentication [#g_govern_authentication_daily](https://example_company.slack.com/archives/C01311Z0LDD)
...
- [Seeking Quality's opinion if an MR would introduce flakiness in existing end-to-end tests](https://example_company.com/example_company-org/example_company/-/merge_requests/92333#note_1033648348)
- [You need help deciding the best way to fix a test](https://example_company.com/example_company-org/example_company/-/merge_requests/93467#note_1042495993)
- [Guidance on setting up your local environment to properly test](https://example_company.com/example_company-org/example_company/-/merge_requests/56923#note_541368575)
...
- [If you want to confirm whether something should be kept confidential](https://example_company.com/example_company-org/example_company/-/issues/460714#note_2026742446)
- [Confirming whether or not something is considered a vulnerability](https://example_company.com/example_company-org/example_company/-/issues/364526#note_1041178738)
- [Collaborating on feature proposals to see if they have any security implications](https://example_company.com/example_company-org/example_company/-/issues/227841#note_1025940760)
...
- We create a weekly issue to inform the team members about the company or team updates, to share important links or to be informed about the team availability. Creation of the issue is the responsibility of an Engineering Manager, who can use an issue template located in the [Govern/Auth repo](https://example_company.com/example_company-org/govern/authentication/discussion). All weekly updates can be found in the project issue list [filtered by weekly update label.](https://example_company.com/example_company-org/govern/authentication/discussion/-/issues/?sort=updated_desc&state=closed&label_name%5B%5D=weekly%20update&first_page_size=20)
...
- At the beginning of each month, Product should have created [a planning issue](https://example_company.com/example_company-org/govern/authentication/discussion/-/issues/118) for the coming release.
...
After the PM creates a [planning issue](https://example_company.com/groups/example_company-org/govern/authentication/-/issues/?sort=popularity&state=opened&label_name%5B%5D=Planning%20Issue&first_page_size=20) for the next release, the quad will gather a prioritized list of issues that should be considered. The EM will then create a "breakdown" issue with a checklist of issues that need to be estimated ([example](https://example_company.com/example_company-org/govern/authentication/discussion/-/issues/45)). All Engineers from the group should be assigned to that issue and participate in weighting or breaking down the list.
...
Engineers should assign issues to themselves based on the [current release](https://example_company.com/example_company-org/example_company/-/boards/4406395?label_name[]=group%3A%3Aauthentication&label_name[]=Deliverable&milestone_title=Started) and ~Deliverable label. We use the following workflow label cycle on our issues:
...
1. `~"workflow::awaiting security release"` - (security MRs only) the work is complete, just pending [backports](https://example_company.com/example_company-org/release/docs/-/blob/master/general/security/engineers.md#backports).
...
We have [many labels](https://docs.example_company.com/ee/development/labels/index.html) that can be applied to an issue or merge request. Besides the issue workflow labels above, here are the minimum basic labels to apply to issues and merge requests:
...
- `security` if the issue is related to application security, and `breaking change` if this work is considered a [breaking change](https://docs.example_company.com/ee/update/terminology.html#breaking-change)
...
We create a monthly issue that is assigned to all counterparts. The issue has to be created manually by an Engineering Manager, but we have an issue template in the [Authentication discussion repo](https://example_company.com/example_company-org/govern/authentication/discussion).
...
Auth group members who are part of the [Authentication group](https://example_company.com/groups/example_company-org/govern/authentication/) can be `@` mentioned on Example Company with `@example_company-org/govern/authentication`.
...
- [Error Budget](https://dashboards.example_company.net/d/stage-groups-detail-authentication/749586f0-be5b-5af4-88ce-3e3bac20b45e?orgId=1&from=now-2d&to=now)

### Links and resources {#links}

{{% include "includes/engineering/govern-shared-links.md" %}}

- [Milestone retrospectives](https://example_company.com/gl-retrospectives/govern/authentication/-/issues/53)
- Our Slack channels
  - Govern:Authentication [#g_govern_authentication](https://example_company.slack.com/archives/CLM1D8QR0)
  - Daily standups [#g_govern_authentication_daily](https://example_company.slack.com/archives/C01311Z0LDD)
- Issue boards
  - [Milestone board](https://example_company.com/example_company-org/example_company/-/boards/4374016?not%5Blabel_name%5D%5B%5D=Quality&label_name%5B%5D=group%3A%3Aauthentication&label_name%5B%5D=Deliverable&milestone_title=17.4)
  - [Task assignment board](https://example_company.com/groups/example_company-org/-/boards/4490634?label_name[]=Deliverable&label_name[]=group%3A%3Aauthentication&milestone_title=17.4)
