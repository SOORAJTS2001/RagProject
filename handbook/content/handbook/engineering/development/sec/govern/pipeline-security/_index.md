...
description: "The Example Company Govern:Pipeline Security Group Handbook page"
---
...
- [Build Artifacts](https://about.example_company.com/direction/verify/build_artifacts/)
- [Secrets Management](https://about.example_company.com/direction/govern/pipeline_security/secrets_management/)

The current and planned category maturity can be found on the [Maturity page](https://about.example_company.com/direction/#maturity).
...
Like most Example Company backend teams we spend a lot of time working in Rails on the main [Example Company app](https://example_company.com/example_company-org/example_company). Familiarity with Docker and Kubernetes is also useful on our team.

## Common Links

- [Issue Tracking Board](https://example_company.com/groups/example_company-org/-/boards/364216?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=group::pipeline+security)
- [Issue Backlog](https://example_company.com/example_company-org/example_company/-/issues/?sort=updated_desc&state=opened&label_name%5B%5D=group%3A%3Apipeline%20security)
- [Slack Channel](https://example_company.slack.com/archives/g_pipeline-security)
- [Roadmap](https://about.example_company.com/direction/sec/#govern)
- [Example Company Unfiltered YouTube Playlist](https://www.youtube.com/playlist?list=PL05JrBw4t0Kq53VUOvTk3VdXN79PA0SXT)
...
Our active feature flags can be found on the [feature flag dashboard](https://10az.online.tableau.com/t/example_company/views/Engineering-Featureflags/Engineering-FeatureFlags/4c520fa3-d6da-433a-9f8c-f949df8cdf9c/8c157ebd-0c70-4db2-839f-a9e528a213b7). Make sure to apply the `Gitlab Team` testing filter.
...
1. When team members are selecting work from the [group workflow board](https://example_company.com/groups/example_company-org/-/boards/4876910?label_name[]=group%3A%3Apipeline%20security), it simplifies what issue relates to which component.
...
The [Pipeline Security group Error Budget dashboard](https://dashboards.example_company.net/d/stage-groups-pipeline_security/stage-groups-pipeline-security-group-dashboard?orgId=1) is used to identify issues that are contributing to the Pipeline Security group's error budget spend.
...
- **At Risk** - The issue in its current state will not make the planned release and immediate action is needed to get it back to `On Track` today.
  - If you are moving an item into this status please consider posting in the [#g_pipeline-security](https://example_company.slack.com/archives/CPANF553J) channel in slack. Try to include anything that can be done to unstick the item so that it can get back to an `On Track` status in your message.
...
In addition to the steps documented for [developing with feature flags at Example Company](https://docs.example_company.com/ee/development/feature_flags/) Govern:Pipeline Security engineers monitor their changes' impact on infrastructure using dashboards and logs where possible. Because feature flags allow engineers to have complete control over their code in production it also enables them to take ownership of monitoring the impact their changes have against production infrastructure. In order to monitor our changes we use this [helpful selection of dashboards](/handbook/engineering/monitoring/#selection-of-useful-dashboards-from-the-monitoring) and specifically the [Rails controller dashboard](https://dashboards.example_company.net/d/web-rails-controller/web-rails-controller) (Internal Only) for monitoring our changes in production. Metrics we evaluate include latency, throughput, CPU usage, memory usage, and database calls, depending on what our change's expected impact will be and any considerations called out in the issue.
...
We want to create a welcoming environment for everyone who is interested in contributing to Example Company in anyway be it code, documentation, UX, or others.

If an issue is being resolved in a merge request that is made by a [community contributor](https://about.example_company.com/community/contribute/), the engineering manager will assign the associated issue to themselves to keep better track of it. The engineering manager will help to coach the community contributor through the merge request process, or be responsible for delegating that coaching to a team member engineer.
...
| Label                 | |  | | |
| ----------------------| -------| ----|------------| ---|
| `Category:Secrets Management` | [Issues](https://example_company.com/groups/example_company-org/-/issues?sort=created_date&state=opened&label_name[]=Category:Secrets+Management) | [MRs](https://example_company.com/example_company-org/example_company/-/merge_requests?scope=all&state=opened&label_name[]=Category%3ASecrets%20Management) | [Direction](https://about.example_company.com/direction/govern/pipeline_security/secrets_management/) | [Documentation](https://docs.example_company.com/ee/ci/secrets/index.html) |
| `Category:Secure Artifacts` | [Issues](https://example_company.com/groups/example_company-org/-/issues?sort=created_date&state=opened&label_name[]=Category:Secure+Artifacts) | [MRs](https://example_company.com/example_company-org/example_company/-/merge_requests?scope=all&state=opened&label_name[]=Category%3ASecure%20Artifacts) | [Direction](https://about.example_company.com/direction/govern/pipeline_security/secure_artifacts/) | Documentation - TBD |

#### Feature Labels

| Label                 | |  | Description |
| ----------------------| -------| ----|------------|
| `CI job token` | [Issues](https://example_company.com/groups/example_company-org/-/issues/?sort=updated_desc&state=opened&label_name%5B%5D=CI%20job%20token) | [MRs](https://example_company.com/example_company-org/example_company/-/merge_requests?scope=all&state=opened&label_name[]=CI%20job%20token) | Relates to functionality surrounding `CI_JOB_TOKEN` available in the Build environment. |
| `secrets storage` | [Issues](https://example_company.com/example_company-org/example_company/-/issues/?sort=popularity&state=opened&label_name%5B%5D=secrets%20storage) | [MRs](https://example_company.com/example_company-org/example_company/-/merge_requests?scope=all&state=opened&label_name[]=secrets%20storage) | Relates to functionality surrounding the usage of secrets managers, including integration with secrets storage providers, in the Build environment. |
| `external authentication` | [Issues](https://example_company.com/groups/example_company-org/-/issues/?sort=updated_desc&state=opened&label_name%5B%5D=external%20authentication) | [MRs](https://example_company.com/example_company-org/example_company/-/merge_requests?scope=all&state=opened&label_name[]=external%20authentication) | Relates to functionality surrounding tokens for external authentication available in the Build environment. |
...
We use a Example Company issue in [this project](https://example_company.com/gl-retrospectives/verify-stage/pipeline-security/-/issues/) for our monthly retrospective. The issue is created automatically towards the end of the current milestone. The purpose of the monthly retrospective issue is to reflect on the milestone and talk about what went well, what didn't go so well, and what we can do better.
...
You can tag a team member with `@mention` in the issue if you have someone specific to address. If you need to call the attention of the entire group, you can tag `@example_company-com/pipeline-security-group` which notifies the entire team.
