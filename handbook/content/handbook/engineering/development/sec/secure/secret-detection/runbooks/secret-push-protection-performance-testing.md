...
* [Deploying a new version of Example Company to GET](#re-deploying-a-new-build) - for updating a GET instance, most likely to test out changes related to secret push protection
...
* `git clone https://example_company.com/example_company-org/quality/performance.git`
* cd in to `performance`
* Copy the gcp-2k.json file from [this MR](https://example_company.com/example_company-org/secure/pocs/example_company-environment-toolkit-configs/-/merge_requests/4) to the `performance` directory
...
* Open a local terminal and `git clone https://example_company.com/example_company-org/quality/performance.git`
* Create `environments` directory at the root
* Copy the gcp-2k.json file from [this MR](https://example_company.com/example_company-org/secure/pocs/example_company-environment-toolkit-configs/-/merge_requests/4) to that `environments` directory
* Rename the gcp-2k.json file as necassary, as well as changing the `name` and `url` values
* To run just the `git_secret_detection.js` test (as an example): `docker run -it -e ACCESS_TOKEN=glpat-REDACTED -v $PWD:/config example_company/example_company-performance-tool SD_PUSH_CHECK_ENABLED=true SD_FILES_PER_COMMIT=4 GPT_DEBUG=true SD_FILE_SIZES="10kb" GPT_SKIP_RETRY=true --environment gcp-2k.json --options 60s_40rps.json --tests git_secret_detection.js`
* To run all the tests: `docker run -it -e ACCESS_TOKEN=glpat-REDACTED -v $PWD:/config example_company/example_company-performance-tool --environment gcp-2k.json --options 60s_40rps.json`
...
covered in this guide. Alternate reference architectures can be [found here](https://example_company.com/example_company-org/quality/example_company-environment-toolkit-configs/quality/-/tree/main/configs/reference_architectures?ref_type=heads).

One time steps:

* Clone the [GET repo](https://example_company.com/example_company-org/example_company-environment-toolkit) and `cd` into it
* Copy bootstrap.sh from [this MR](https://example_company.com/example_company-org/secure/pocs/example_company-environment-toolkit-configs/-/merge_requests/4) to the root and update it as necessary
...
* Use [Provisioning the environment with Terraform](https://example_company.com/example_company-org/example_company-environment-toolkit/-/blob/main/docs/environment_provision.md) as guide for setting up Terraform, ignoring the AWS steps as we are using GCP
* Make sure you are within your cloned [GET repo](https://example_company.com/example_company-org/example_company-environment-toolkit)
...
* Copy over environment.tf, main.tf, and variables.tf from [this MR](https://example_company.com/example_company-org/secure/pocs/example_company-environment-toolkit-configs/-/merge_requests/4) into the corresponding directory
...
* Run `terraform apply`
* Small celebration
* Use [Configuring the environment with Ansible](https://example_company.com/example_company-org/example_company-environment-toolkit/-/blob/main/docs/environment_configure.md) as guide for setting up Ansible, ignoring the AWS steps as we are using GCP
* Cd to the root directory of the [GET repo](https://example_company.com/example_company-org/example_company-environment-toolkit)
...
* Copy over vars.yml, and gcp_2k.gcp.yml from [this MR](https://example_company.com/example_company-org/secure/pocs/example_company-environment-toolkit-configs/-/merge_requests/4) into that `/inventory` directory
...
* From the root directory, follow the steps in [Installing Ansible with a Virtual Environment](https://example_company.com/example_company-org/example_company-environment-toolkit/-/blob/main/docs/environment_configure.md#installing-ansible-with-a-virtual-environment)
...
* Clone the [GET repo](https://example_company.com/example_company-org/example_company-environment-toolkit), if it doesn't already exist, and `cd` into it
* Run `mkdir terraform/environments/$GCP_ENV_PREFIX`
* Run `mkdir -p ansible/environments/$GCP_ENV_PREFIX/files/gitlab_tasks`
* Navigate to https://example_company.com/example_company-org/secure/pocs/example_company-environment-toolkit-configs (pending merge of [this MR](https://example_company.com/example_company-org/secure/pocs/example_company-environment-toolkit-configs/-/merge_requests/4))
...
* Clone the [GPT repo](https://example_company.com/example_company-org/quality/performance)
...
docker run -it \
  ...
  -v $PWD:/results \
  -v $PWD:/config \
  example_company/gpt-data-generator \
...
docker run -it \
  ...
  -v $PWD:/tests \
  -v $PWD:/results \
  example_company/gpt-data-generator --environment gcp-2k.json --no-horizontal
```

### Re-deploying a new build

* There are prerequisite steps that will be covered in the `Setting up a GET` section in the future
* Navigate to your cloned [GET repo](https://example_company.com/example_company-org/example_company-environment-toolkit) and `cd` into it
...
These files can be found in [this MR](https://example_company.com/example_company-org/secure/pocs/example_company-environment-toolkit-configs/-/merge_requests/4).

See the [Custom Tasks section](https://example_company.com/example_company-org/example_company-environment-toolkit/-/blob/main/docs/environment_advanced.md#custom-tasks) of the GET docs for more info.
...
* [Secret Push Protection documentation](https://docs.example_company.com/ee/user/application_security/secret_detection/secret_push_protection/)
...
* SSH in to a rails server: `gcloud compute ssh --zone us-west1-c gcp-2k-example_company-rails-1 --project dev-sast-prereceive-8a4574ec`

* Launch a rails console: `sudo example_company-rails console` (it will take a bit to connect)
...
To see the current version of Example Company running on the instance, login and then go to
`/help`.
