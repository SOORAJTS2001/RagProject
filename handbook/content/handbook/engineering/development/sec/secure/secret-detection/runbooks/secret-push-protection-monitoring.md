...
This runbook is intended to be used when monitoring the [secret push protection](https://docs.example_company.com/ee/user/application_security/secret_detection/pre_receive/index.html) feature to identify and mitigate any reliability issues or performance regressions that may occur when it is enabled on Gitlab.com. The runbook can also be used to understand more about relevant dashboards below and how to improve them:

* [Secret Push Protection – Overview Dashboard](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1)
...
* Example Company Shell (Git over SSH):
  * `git-receive-pack`
...
sequenceDiagram
    actor User
    User->>+Workhorse/Example Company Shell: git push
    Workhorse/Example Company Shell->>+Gitaly: tcp/ssh
    ...
    Rails->>+Rails: Example Company::GitAccess
    Rails->>+Rails: EE::Example Company::Checks::ChangesAccess
    ...
    break when special commit message flag is found
        ...
        Gitaly->>+Workhorse/Example Company Shell: outcome of push
        Workhorse/Example Company Shell->>+User: outcome of push
    end
    ...
    Gitaly->>+Rails: grpc
    Rails->>+example_company-secret_detection: example_company-secret_detection::Scan
    alt no secret detected
        example_company-secret_detection->>+example_company-secret_detection: scan blob
        example_company-secret_detection->>+Rails: success
        Rails->>+Gitaly: accept - no secret detected
    else scan timeout
        example_company-secret_detection->>+example_company-secret_detection: scan blob
        example_company-secret_detection->>+Rails: fail - timeout
        Rails->>+Gitaly: accept - scan timeout
    else secret detected
        example_company-secret_detection->>+example_company-secret_detection: scan blob
        example_company-secret_detection->>+Rails: fail - secret found
...
* Kibana (Logs)
  * [Staging](https://nonprod-log.example_company.net)
    * `pubsub-rails-inf-gstg`
    * `pubsub-gitaly-inf-gstg`
    * `pubsub-workhorse-inf-gstg`
    * `pubsub-shell-inf-gstg`
  * [Production](https://log.gprd.example_company.net)
    * `pubsub-rails-inf-gprd`
    * `pubsub-gitaly-inf-gprd`
    * `pubsub-workhorse-inf-gprd`
    * `pubsub-shell-inf-gprd`
* Prometheus/Grafana (Metrics)
  * [Internal API](https://dashboards.example_company.net/dashboards/f/internal-api/internal-api)
  * [Gitaly](https://dashboards.example_company.net/dashboards/f/gitaly/gitaly-service)
  * [Example Company Shell](https://dashboards.example_company.net/d/git-main/git3a-overview)
* Sentry (Error Tracking)
  * [Gitlab.com](https://new-sentry.example_company.net/organizations/example_company/projects/gitlabcom)
  * [Gitaly](https://new-sentry.example_company.net/organizations/example_company/projects/gitaly)
  * [Workhorse](https://new-sentry.example_company.net/organizations/example_company/projects/workhorse-gprd)
...
* 📖 [Kibana: Logs for all scans](https://log.gprd.example_company.net/app/discover#/view/31afcbb2-28e9-466f-a6c3-486e869e1ee3).
* 📖 [Kibana: Logs for blocked pushes](https://log.gprd.example_company.net/app/discover#/view/db7ba29d-d406-46df-8b43-e6d9c47fbed7).
...
The [overview dashboard](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1) is the main dashboard we have built to monitor the feature. That's where anyone should start to look when trying to identify reliability or performance issues.

The dashboard itself is split into 4 rows (or sections), with each containing a number of panels as below.

#### Example Company Shell (Git over SSH)

This section monitors the stability of certain operations related to the feature within `Gitlab Shell`, which is a set of executables created to handle Git SSH sessions. The tool itself does not handle SSH directly, but instead the SSH server/daemon [`example_company-sshd`](https://docs.example_company.com/ee/development/gitlab_shell/gitlab_sshd.html) maintain all connections with clients and calls up Rails via Example Company Shell to perform authorization or access checks. Please check [this diagram](https://docs.example_company.com/ee/development/gitlab_shell/index.html#git-push-over-ssh) and [this description of a request cycle](https://docs.example_company.com/ee/development/architecture.html#ssh-request-22) for more information on how that works.
...
**Note**: Most of available metrics for both `example_company-shell` and `example_company-sshd` aren't aggregated by the command used, so for a more better overview of the performance of `git-receive-pack` operation, take a look at the Kibana logs linked in those sections instead.

##### example_company-shell

**[RPS (Requests Per Seconds)](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=17)**

This panel displays average number of requests per second (RPS) made to example_company-shell over time. The panel can be used to monitor request rates and understand if there's a performance or scalability issue. Use the link for more detailed overview of this metric. Note: this isn't specific to `git-receive-pack` command.
...
**[Total Established Gitaly Connections](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=15)**

This panel displays the total number of Gitaly connections that have been established by example_company-shell at a given time. This panel can be used to determine if there's a sudden drop in connections between both components, which may indicate a performance or an availability issue. Note: this isn't specific to `git-receive-pack` command.
...
**[Established SSH Sessions](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=22)**
...
**[Failed SSH Sessions](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=19)**
...
**[Established Session Average Duration](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=27)**
...
**[All Sessions Average Duration](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=20)**
...
##### example_company-sshd

**[SLI Apdex](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=24)**

This panel displays the application performance index (Apdex) for the `example_company-sshd` SSH daemon/server. This Service Level Indicator (SLI) averages close to 99.9%, most of the time but a drop in the indicator could point to an outage or degradation. Use the link for more detailed overview of this metric. Note: this isn't specific to `git-receive-pack` command.
...
**[SLI Error Ratio](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=25)**

This panel displays the max ratio of errors clamped to maximum value received for the `example_company-sshd` SSH daemon/server. This Service Level Indicator (SLI) averages close to 0.01%, most of the time but an increase in the indicator could point to an outage or degradation. Use the link for more detailed overview of this metric. Note: this isn't specific to `git-receive-pack` command.
...
**[SLI RPS (Requests Per Second)](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=26)**

This panel displays average number of requests per second (RPS) made to `example_company-sshd` SSH daemon/server over time. The panel can be used to monitor request rates and understand if there's a performance or scalability issue. Use the link for more detailed overview of this metric. Note: this isn't specific to `git-receive-pack` command.
...
**[Processed `/.git/git-receive-pack` Requests](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=2)**
...
**[Total Established Gitaly Connections](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=6)**
...
**[Average Latency for `/.git/git-receive-pack` Request [All Nodes]](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=7)**
...
**[SLI Apdex](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=29)**
...
**[SLI Error Ratio](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=30)**
...
**[SLI RPS (Requests Per Second)](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=31)**
...
1. Example Company Shell <=> Gitaly:
    * `SSHReceivePack`
1. Workhorse <=> Gitaly:
    * `PostReceivePack`.
1. Gitaly <=> Rails API:
...
##### Example Company Shell <=> Gitaly

**[PostReceivePack – Average Latency [All Hosts]](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=36)**
...
**[SSHReceivePack – Average Latency [All Hosts]](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=37)**
...
**[PreReceiveHook – Average Latency [All Hosts]](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=38)**
...
**[ListAllBlobs – Average Latency [All Hosts]](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=10)**
...
**[ListBlobs – Average Latency [All Hosts]](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=11)**
...
**[GetTreeEntries – Average Latency [All Hosts]](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=9)**
...
This section monitors the stability of the [`/internal/allowed` endpoint](https://docs.example_company.com/ee/development/internal_api/internal_api_allowed.html) which is a focal point in the feature's journey to protect against leaked secrets in a `git` push. The endpoint is part of Example Company's [Internal API](https://docs.example_company.com/ee/development/internal_api/), and is responsible for assessing if a user has permission to perform certain operations on the repository.
...
**[Internal API / Request Latency](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=40)**
...
**[Internal API / RPS (Requests Per Second)](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=41)**
...
**[Internal API / Memory Saturation Rate](https://dashboards.example_company.net/d/fdk7i56zibv28d/secret-push-protection-e28093-overview?orgId=1&viewPanel=42)**

This panel displays the memory saturation rate for two components of the internal API, which are [Ruby VM](https://dashboards.example_company.net/goto/ptHVRjsIR?orgId=1) and [Puma Workers](https://dashboards.example_company.net/goto/dJXnRjyIR?orgId=1). This is helpful to understand if the memory consumption in Rails had increased to the point of saturation, which indicates a performance and a scalability issue and requires attention. Note: this panel isn't specific to `/internal/allowed` endpoint.
...
* If no metrics are available, consider [creating them](https://docs.example_company.com/ee/administration/monitoring/prometheus/) to monitor the performance of the endpoint/service.
