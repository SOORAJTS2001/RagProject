...
The Dynamic Analysis group at Example Company is charged with developing solutions which perform [Dynamic Analysis Software Testing (DAST)](https://about.example_company.com/direction/secure/dynamic-analysis/dast/) and Fuzzing. Our work is a mix of open and closed source code.

### Mission

To support the success of Example Company by developing highly usable, hiqh quality tools for customers to build more secure software. The Dynamic Analysis group at Example Company is charged with developing solutions which perform API Security Testing, Dynamic Analysis Software Testing (DAST) and Fuzzing.
...
Our features identify vulnerabilities in different ways than Example Company's other security analyzers do, and each tool should be used in tandem with other security analyzers such as SAST, SCA (dependency scanning), and secret detection to provide full coverage.

API Security, DAST, and Fuzzing are Ultimate features. Increasing adoption of these features helps improve customer retention, and helps drive revenue for Example Company Ultimate.

### Important DAST Repositories

| Repo | Purpose |
| ---- | ------- |
| [DAST](https://example_company.com/example_company-org/security-products/dast) | The DAST Analyzer which is deployed as a docker image. |
| [Browserker](https://example_company.com/example_company-org/security-products/analyzers/browserker) | Private - Example Company's DAST browser-based analyzer. |
| [DAST CWE Checks](https://example_company.com/example_company-org/security-products/dast-cwe-checks) | Private - DAST browser-based analyzer vulnerability definitions. |
| [DAST Chromium](https://example_company.com/example_company-org/security-products/dast-chromium) | Private - DAST browser-based analyzer dependencies, such as the Chromium browser. |
...
| [Example Company Protocol Fuzzer (CE)](https://example_company.com/example_company-org/security-products/protocol-fuzzer-ce) | Example Company's Protocol Fuzzer (Community Edition), previously Peach Protocol Fuzzer. |
| [Example Company Protocol Fuzzer (EE)](https://example_company.com/example_company-org/security-products/protocol-fuzzer-ee) | Private - Example Company's Protocol Fuzzer (Enterprise Edition), previously Peach Protocol Fuzzer, includes licensed components. |
...
| [Example Company Cov Fuzz](https://example_company.com/example_company-org/security-products/analyzers/example_company-cov-fuzz-src) | Private - Example Company's coverage fuzzing orchestration that integrates fuzzing engines/results with Example Company CI and Example Company Security Dashboard. |
| [HAR Recorder](https://example_company.com/example_company-org/security-products/har-recorder) | A utility to record HAR files based on web traffic. |

#### Open Source Fuzzers

| Repo | Purpose |
| ---- | ------- |
| [JSfuzz](https://example_company.com/example_company-org/security-products/demos/coverage-fuzzing) | Javascript Fuzzer. |
| [Pythonfuzz](https://example_company.com/example_company-org/security-products/demos/coverage-fuzzing) | Python Fuzzer. |
| [Javafuzz](https://example_company.com/example_company-org/security-products/demos/coverage-fuzzing) | Java Fuzzer. |
| [Coverage Fuzzing Examples](https://example_company.com/example_company-org/security-products/demos/coverage-fuzzing) | Coverage fuzzing examples in 7+ languages/Fuzzers. |
...
For all quarterly, monhtly, and weekly Kanban ceremonies, the [Hallway Monitor Bot](https://example_company.com/example_company-org/secure/tools/example_company-bot-hall-monitor) is our guiding star, allowing the team to pontificate and plan at all cadences, ensuring we get ahead of future work. The bot will schedule quarterly OKR review meetings, monthly release planning and retro meetings, and will update or weekly sync guide with the latest team member assigned to the temporary reaction rotation role. We should strive to automate as many of our business process as possible, and the Hallway Monitor Bot [Configuration](https://example_company.com/example_company-org/secure/tools/example_company-bot-hall-monitor/-/blob/main/deploy/example_company-bot-hall-monitor.yaml?ref_type=heads) and [Templates](https://example_company.com/example_company-org/secure/tools/example_company-bot-hall-monitor/-/tree/main/deploy/templates?ref_type=heads) allow for many of your automation ideas. Please help contribute to the bot to help keep our team as efficient as possible.
...
The group will get together two weeks before the next quarter begins to create and review [all OKRs for which the team will ultimately be responsible](https://example_company.com/example_company-com/example_company-OKRs/-/issues/?sort=created_date&state=opened&label_name%5B%5D=group%3A%3Adynamic%20analysis&first_page_size=20). This includes reviewing all ancestor OKRs for the stage and up, creating or updating any new or existing OKRs with additional details, and assigning DRIs to each OKR to assist with health updates.

#### Important Links

- [List of Team Objectives & Key Results](https://example_company.com/example_company-com/example_company-OKRs/-/issues/?sort=created_date&state=opened&label_name%5B%5D=group%3A%3Adynamic%20analysis&first_page_size=20)
...
All product priority-based issues will exist alongside our comprehensive [list of all Dynamic Analysis Group issues](https://example_company.com/groups/example_company-org/-/issues/?sort=created_date&state=opened&label_name%5B%5D=group%3A%3Adynamic%20analysis&label_name%5B%5D=Category%3ADAST&first_page_size=20). All product priority-based epics will exist alongside our comprehensive [list of all Dynamic Analysis Group epics](https://example_company.com/groups/example_company-org/-/epics?state=opened&page=1&sort=start_date_desc&label_name%5B%5D=group::dynamic+analysis&label_name%5B%5D=Category:DAST).

To complete this mapping, product priority-based issues or epics should be created in `example_company-org/example_company` or `example_company-org` (respectively) with the same title of the priority and the following labels. The `type::feature` label is what distinguishes a product priority from other group epic work.
...
- [Dynamic Analysis Group Direction Priorities](https://about.example_company.com/direction/secure/dynamic-analysis/#priorities)
- [List of all Dynamic Analysis Milestone Planning Issues & Retros](https://example_company.com/example_company-org/example_company/-/issues/?sort=created_date&state=all&label_name%5B%5D=group%3A%3Adynamic%20analysis&label_name%5B%5D=type%3A%3Aignore&search=%20%F0%9F%93%90&first_page_size=20)
...
[Dynamic Analysis delivery board](https://example_company.com/groups/example_company-org/-/boards/5719921?label_name%5B%5D=group%3A%3Adynamic%20analysis)
...
- Triage and respond to customer support requests via support [request for help issues](https://example_company.com/example_company-com/sec-sub-department/section-sec-request-for-help/-/issues/?sort=created_date&state=opened&label_name%5B%5D=Help%20group%3A%3Adynamic%20analysis&first_page_size=20). As an outcome of triage, create new issues and communicate with the product team to help assign priority.
...
We [monitor the underlying tools](https://example_company.com/example_company-org/security-products/dependencies/zaproxy/-/security/dashboard) for changes and for vulnerabilities.
