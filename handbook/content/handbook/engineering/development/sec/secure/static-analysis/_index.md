...
The Static Analysis group at Example Company is charged with developing the [Static Application Security Testing (SAST)](https://about.example_company.com/direction/secure/static-analysis/sast/) feature category for customer software repositories.
...
- Google groups: sec-secure-static-analysis@example_company.com

## How We Work

The Static Analysis group is largely aligned with Example Company's [Product Development Flow](/handbook/product-development-flow/), however there are some notable differences in
...
Issues worked by this team can span analyzers, vendored templates, and Example Company's Rails monolith.

### Issue Boards

- [Static Analysis Delivery Board](https://example_company.com/groups/example_company-org/-/boards/1590112?label_name[]=group%3A%3Astatic%20analysis&group_by=epic)
...
- [Static Analysis Planning Board](https://example_company.com/groups/example_company-org/-/boards/1229162?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=group%3A%3Astatic%20analysis)
  - Milestone-centric board primarily used by product management to gauge work in current and upcoming milestones.
- [Static Analysis EM Board](https://example_company.com/groups/example_company-org/-/boards/1655697)
...
Example Company has a labeling convention for issues and Merge Requests. We follow this convention, though there are specific labels required to route artifacts to us. We
...
| ~backend | Identifies the issue or MR as being part of Example Company's backend. |
| ~frontend | Identifies the issue or MR as being part of Example Company's frontend. |

### It all starts with planning

As is the case throughout Example Company, the Static Analysis group works on a monthly planning cadence. We are product-driven and work in response to the priorities identified by Product Management.

However, Example Company milestones start in the second half of each month, which has made a planning cadence organized around weeks in a milestone somewhat difficult to understand as there are many
...
- **Current milestone:** Engineering Manager creates milestone [release issue](https://docs.example_company.com/ee/development/sec/analyzer_development_guide.html#monthly-release-process), with assignments for individual updates.
...
See [%15.3 issue](https://example_company.com/example_company-org/example_company/-/issues/368540) as an example. This issue format can be cloned and applied as-needed.
...
For Example Company.com, we monitor performance of our code within the Rails application, metrics around our CI build performance, and traffic to our container registries. These dashboards are accessible on the [Monitoring](/handbook/engineering/monitoring) page.
...
- [Secure::Static Analysis Group Error Budget](https://dashboards.example_company.net/d/stage-groups-static_analysis/stage-groups-static-analysis-group-dashboard)
- [SAST Analyzer Registry Traffic](https://log.gprd.example_company.net/app/dashboards#/view/84aa3f10-89d2-11ec-9dd2-93d354bef8e7?_g=(filters%3A!()%2CrefreshInterval%3A(pause%3A!t%2Cvalue%3A0)%2Ctime%3A(from%3Anow-24h%2Cto%3Anow)))
- [SAST Engineering Kibana Dashboard](https://log.gprd.example_company.net/app/dashboards#/view/1eebd010-9a73-11ec-9dd2-93d354bef8e7)
...
While we follow Example Company's product development flow, our processes as an engineering team most closely resemble kanban. Engineers are empowered to choose issues from the Delivery
...
In Example Company, the `~Deliverable` label is referred to as a [release scoping label](https://docs.example_company.com/ee/development/labels/index.html#release-scoping-labels). Applying this label
...
We are responsible to ensure that what we deliver is secure. This means that we dogfood Example Company's Security
features.
...
| Triage of existing vulns | This is done by the main maintainer of each of our analyzers as defined in our [Release project's issue template](https://example_company.com/example_company-org/security-products/release/-/blob/master/scripts/templates/release_issue.md.erb). |
| Resolution of Critical / High Vulns | These should be a Product-driven priority. |
| Resolution of Medium / Low Vulns | This is done by the main maintainer of each of our analyzers as defined in our [Release project's issue template](https://example_company.com/example_company-org/security-products/release/-/blob/master/scripts/templates/release_issue.md.erb). |
...
- Dismiss vulnerability in the Example Company UI with a comment that contains:
...
not added. Then [Appsec Escalation Engine](https://example_company.com/example_company-com/gl-security/engineering-and-research/automation-team/appsec-escalator#appsec-escalation-engine)
...
We are responsible for delivering Example Company's SAST and Secret Detection features, and the analyzers we develop rely heavily upon open source software.
This means we can be dramatically affected by changes in those software packages. We will check for updates to these packages once per [Example Company release](https://about.example_company.com/releases/). New versions will be scrutinized for the following aspects:
...
through our [Release project's issue template](https://example_company.com/example_company-org/security-products/release/-/blob/master/scripts/templates/release_issue.md.erb).
...
We have a [dependencies group](https://example_company.com/example_company-org/security-products/dependencies) which contains mirrored copies of the OSS projects upon which we most rely. Prior to submitting an MR updating an analyzer to a new version of these projects, engineers are expected to do the following:
...
1. Please note, some of these projects have complicated builds. Auto DevOps works sometimes, but projects such as [spotbugs](https://example_company.com/example_company-org/security-products/dependencies/spotbugs) can require a custom CI configuration for our scans to be successful. Also, these projects include tests that can be noisy if not filtered out.
...
1. Example Company has published [Secure Coding Guidelines](https://docs.example_company.com/ee/development/secure_coding_guidelines.html), which may be a useful resource to use when trying to solve identified risks.
...
At times we will need to update our analyzers because of security updates to golang itself. In this situation, we follow the [established release process](https://docs.example_company.com/ee/development/sec/analyzer_development_guide.html#security-and-build-fixes-of-go).
...
We do, however, own and contribute to projects beyond the analyzers shipped as part of Example Company's product. Where possible, unplanned work requiring
