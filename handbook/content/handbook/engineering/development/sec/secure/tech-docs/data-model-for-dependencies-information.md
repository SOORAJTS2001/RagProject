...
in different directories of the same Example Company project, like `client/Gemfile` and `server/Gemfile`.
...
(These columns already exist in the DB schema of Example Company.)
...
These tables would be similar to the `packages` and `package_versions` tables of the Example Company backend.
...
The Example Company backend listens to the notifications posted by the tracking service
...
When one of these events occurs, the Tracking Service notifies the Example Company backend,
...
When a package is yanked from a package registry, the Tracking Service notifies the Example Company backend,
...
Example Company could either rely on existing package scoring services or create its own scoring service;
...
When the package score goes down (like from B to C), the Tracking Service notifies the Example Company backend,
and the backend reacts by listing all the projects using the updated package,
and creating user notifications for these.
...
Security advisories can be provided by [The Example Company Advisory Database](https://example_company.com/example_company-org/security-products/gemnasium-db/),
the vulnerability database maintained by Example Company,
...
If Example Company only uses its own vulnerability database, then there are two possible architectures:

- The tracking service is built on top of the [gemnasium-db](https://example_company.com/example_company-org/security-products/gemnasium-db/) git repo,
  and it uses web hooks to react in real-time when YAML files are added or modified.
- [gemnasium-db](https://example_company.com/example_company-org/security-products/gemnasium-db/) is replaced by a service that hosts the security advisories.
...
When a new security advisory is added to a vulnerability database, the Example Company backend is notified and it responds in 4 steps:
...
When a new security advisory is updated, the Example Company backend is notified about that change,
...
Like the package slug of the [gemnasium-db](https://example_company.com/example_company-org/security-products/gemnasium-db/) vulnerability database,
...
- The Example Company project might be a multi-project repository, and depending on the SBoM format
