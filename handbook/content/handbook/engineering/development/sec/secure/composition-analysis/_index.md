...
description: "The Composition Analysis group at Example Company is charged with developing solutions which perform Container and Dependency Scanning and License Compliance."
---

## Composition Analysis

The Composition Analysis group at Example Company is charged with developing solutions which perform [Container Scanning](https://about.example_company.com/direction/secure/composition-analysis/container-scanning/),
and [Software Composition Analysis](https://about.example_company.com/direction/secure/composition-analysis/software-composition-analysis/).
See [the exhaustive list of projects](#projects) the group maintains.

## Common Links

- Slack channel: [#g_secure-composition-analysis](https://example_company.slack.com/archives/CKWHYU7U2)
- Slack alias: @secure_composition_analysis_dev
- Google groups: composition-analysis-dev@example_company.com

## How we work

### Workflow

The Composition Analysis group largely follows Example Company's [Engineering Workflow](/handbook/engineering/workflow/) and [Product Development Flow](/handbook/product-development-flow/).
...
We leverage [the issue's health status feature](https://docs.example_company.com/ee/user/project/issues/managing_issues.html#health-status) to communicate the progress of the issue.
...
Access to this calendar is granted via this google group: `sec-secure-composition-analysis@example_company.com`.
...
The [rotation schedule](https://example_company.com/groups/example_company-org/secure/-/epics/2#schedule) follows the development cycle, which means using the start/end dates from the Example Company [product milestones](/handbook/product/milestones/). When creating the schedule, the Engineering Manager should aim to minimize the number of back-to-back rotations that engineers do.
...
At the end of the rotation, add the next engineers as Owners of [`@example_company-org/secure/composition-analysis-dev/reaction-rotation`](https://example_company.com/groups/example_company-org/secure/composition-analysis-dev/reaction-rotation/-/group_members?with_inherited_permissions=exclude), and remove the current engineers. This group is used to tag the engineers on rotation without having to look-up the schedule.
...
If a support engineer requests assistance via Slack and it requires investigation or debugging, they should be directed to raise an issue in [a dedicated project](https://example_company.com/example_company-com/sec-sub-department/section-sec-request-for-help).
    - [#g_secure-composition-analysis](https://example_company.slack.com/archives/CKWHYU7U2)
    - [#s_secure](https://example_company.slack.com/archives/C8S0HHM44)
    - [#sec-section](https://example_company.slack.com/archives/C02087FTL5V)
    - [#s_secure-alerts](https://example_company.slack.com/archives/CAU9SFKNU)
    - [#f_container_scanning](https://example_company.slack.com/archives/C041F2XJACB)
    - [#g_secure-composition-analysis-alerts](https://example_company.slack.com/archives/C04UX9MQNSJ)
    - [#sec-eng-requests-for-help](https://example_company.enterprise.slack.com/archives/C071W3BA87J)
1. Monitor [Section Sec Request For Help](https://example_company.com/example_company-com/sec-sub-department/section-sec-request-for-help/-/issues/?label_name=Help%20group%3A%3Acomposition%20analysis) project for support requests.
...
1. Work with community contributors to help drive [their merge requests](https://example_company.com/groups/example_company-org/-/merge_requests?scope=all&state=opened&label_name[]=group%3A%3Acomposition%20analysis&label_name[]=Community%20contribution) to completion (more information on [community contributions triaging process](/handbook/engineering/infrastructure/engineering-productivity/merge-request-triage/)).
...
1. Check for new versions of our dependencies (not related to security):
    ...
    1. Application dependencies.
    1. Programming language.
1. Check in on test failures. Check relevant slack channels ([#g_secure-composition-analysis-alerts](https://example_company.slack.com/archives/C04UX9MQNSJ), [#s_secure-alerts](https://example_company.slack.com/archives/CAU9SFKNU)).
...
1. Monitor failures and errors on license-db project, use the `#f_licese_database` Slack channel for communication about these items, so other team members can provide the support.
    1. Check latest [scheduled pipelines of license-db](https://example_company.com/example_company-org/security-products/license-db/deployment/-/pipeline_schedules) for any failures. Ensure that pipelines pass or create an issue to fix the failure.
...
1. Assign ownership of the [Reaction Rotation group](https://example_company.com/groups/example_company-org/secure/composition-analysis-dev/reaction-rotation/-/group_members?with_inherited_permissions=exclude) to the new engineers.

### Security vulnerabilities triaging process

We are responsible for triaging vulnerabilities reported on 2 sets of projects: the projects maintained by Example Company and the upstream scanner software we might depend on. Though, we have different processes that apply depending on the situation.
...
1. [Analyzers Vulnerability Report](https://example_company.com/groups/example_company-org/security-products/analyzers/-/security/vulnerabilities/?state=DETECTED&severity=CRITICAL,HIGH&projectId=13150952,15369510,24673064,52241202,6126012,9450192&activity=STILL_DETECTED)
...
1. [License-db Vulnerability Report](https://example_company.com/groups/example_company-org/security-products/license-db/-/security/vulnerabilities/?state=DETECTED&severity=CRITICAL,HIGH&projectId=39193358,39229232,39233486,39298809,39622674,40857363,45266022&activity=STILL_DETECTED)
...
> Refer to [Vulnerability status definitions](https://docs.example_company.com/ee/user/application_security/vulnerabilities/#vulnerability-status-values) in case you are unsure of what each of them mean.

#### Upstream scanners vulnerabilities

**This only applies to projects NOT maintained by Example Company.**
...
1. [Upstream Scanners Vulnerability Report](https://example_company.com/groups/example_company-org/security-products/dependencies/-/security/vulnerabilities/?state=DETECTED&projectId=30616761,30684590,35335143,39545454,39545481,51420921&severity=CRITICAL,HIGH&activity=STILL_DETECTED)
    - To configure the report manually, select all [upstream scanner](#upstream-scanner-mirrors) projects.

For vulnerabilities discovered in upstream scanners, an issue must be created in Example Company's issue tracker, and we should work with the relevant Open Source community to help provide a resolution. As a last resort, we can patch locally or fork the upstream project temporarily to fix the vulnerability sooner.
...
Instead, in our workflow we open all our issues in [the main Example Company project](https://example_company.com/example_company-org/example_company/issues).
...
all of our incidents in the [main Example Company project](https://example_company.com/example_company-org/example_company/-/issues/?sort=created_date&state=opened&type%5B%5D=incident&label_name%5B%5D=group%3A%3Acomposition%20analysis&first_page_size=20).

1. [Open a new incident](https://example_company.com/example_company-org/example_company/-/issues/new?issuable_template=incident&issue%5Bissue_type%5D=incident)
...
that have impacted composition analysis in the future.
    ...
    Ref: https://handbook.example_company.com/handbook/engineering/infrastructure/engineering-productivity/issue-triage/#severity
    -->
    /label ~"severity::1"
    /severity S1
    ...
    Ref: https://handbook.example_company.com/handbook/engineering/infrastructure/engineering-productivity/issue-triage/#priority
    -->
    /label ~"priority::1"
    /label ~"priority::2"
    /label ~"priority::3"
...
1. Link any related issues or zoom meetings with the [quick actions](https://docs.example_company.com/ee/operations/incident_management/linked_resources.html#using-a-quick-action) to record incident timeline events. Ensure that an event exists for the incident start,
...
- [PHP Composer segfaults in gemnasium analyzer](https://example_company.com/example_company-org/example_company/-/issues/383299)
...
1. Leverage the [Maintenance issues board](https://example_company.com/groups/example_company-org/-/boards/7658725?label_name[]=group%3A%3Acomposition%20analysis&label_name[]=type%3A%3Amaintenance).
...
- [common library](https://example_company.com/example_company-org/security-products/analyzers/common)

### Dependency Scanning

- [gemnasium analyzer](https://example_company.com/example_company-org/security-products/analyzers/gemnasium)
- [gemnasium-maven-plugin](https://example_company.com/example_company-org/security-products/analyzers/gemnasium-maven-plugin)
- [gemnasium-gradle-plugin](https://example_company.com/example_company-org/security-products/analyzers/gemnasium-gradle-plugin)
- **Internal only** [example_company-depscan](https://example_company.com/example_company-org/security-products/example_company-depscan)

Additional notes:

- [gemnasium-db](https://example_company.com/example_company-org/security-products/gemnasium-db) is maintained by [the Vulnerability Research group](/handbook/engineering/development/sec/secure/vulnerability-research/).

### Container Scanning

- [container-scanning analyzer](https://example_company.com/example_company-org/security-products/analyzers/container-scanning/)
- [Cluster Image Scanning related code](https://example_company.com/example_company-org/cluster-integration/example_company-agent/-/blob/master/internal/module/starboard_vulnerability/agent/scanner.go),
needed for [Operational Container Scanning](https://docs.example_company.com/ee/user/clusters/agent/vulnerabilities.html) feature.

### License-db

- [advisory-processor](https://example_company.com/example_company-org/security-products/license-db/advisory-processor)
- [deployment](https://example_company.com/example_company-org/security-products/license-db/deployment)
- [license-exporter](https://example_company.com/example_company-org/security-products/license-db/license-exporter)
- [license-feeder](https://example_company.com/example_company-org/security-products/license-db/license-feeder)
- [license-interfacer](https://example_company.com/example_company-org/security-products/license-db/license-interfacer)
- [license-processor](https://example_company.com/example_company-org/security-products/license-db/license-processor)
- [schema](https://example_company.com/example_company-org/security-products/license-db/schema)

### Operational Container Scanning

- [trivy-k8s-wrapper](https://example_company.com/example_company-org/security-products/analyzers/trivy-k8s-wrapper)
- [OCS module](https://example_company.com/example_company-org/cluster-integration/example_company-agent/-/tree/master/internal/module/starboard_vulnerability)

The OCS module is part of the [`example_company-agent`](https://example_company.com/example_company-org/cluster-integration/example_company-agent) project which is maintained by the `Environments` group. The `Composition Analysis` group is responsible for maintaining only the OCS module.

### Semver dialects gem

- [semver_dialects](https://example_company.com/example_company-org/ruby/gems/semver_dialects)

### CI/CD components

- [Dependency Scanning](https://example_company.com/components/dependency-scanning)
- [Container Scanning](https://example_company.com/components/container-scanning)
...
- [trivy](https://example_company.com/example_company-org/security-products/dependencies/trivy)
- [trivy-db](https://example_company.com/example_company-org/security-products/dependencies/trivy-db)
- [trivy-db-data](https://example_company.com/example_company-org/security-products/dependencies/trivy-db-data)
- [trivy-db-glad](https://example_company.com/example_company-org/security-products/dependencies/trivy-db-glad)
- [vuln-list-update](https://example_company.com/example_company-org/security-products/dependencies/vuln-list-update)

The vulnerabilities reported on the currently used version of the scanner are automatically reported in [the group level Vulnerability Report](https://example_company.com/groups/example_company-org/security-products/dependencies/-/security/vulnerabilities/?state=DETECTED&projectId=30616761,30684590,35335143,39545454,39545481,51420921&severity=CRITICAL,HIGH&activity=STILL_DETECTED) and triaged as part of our [security vulnerabilities triaging process](#security-vulnerabilities-triaging-process).

#### Setting up a mirror

1. create a new project in https://example_company.com/example_company-org/security-products/dependencies (blank project).
2. set up the project repository as [a pull mirror](https://docs.example_company.com/ee/user/project/repository/mirror/#pulling-from-a-remote-repository) of the upstream repository.
...
5. add a `.example_company-ci.yml` configuration file to configure all compatible security scans.
...
We check for new releases of the upstream scanners on a monthly basis, as part of our [release issue](https://example_company.com/example_company-org/security-products/release/-/blob/master/scripts/templates/release_issue.md.erb). When an update is available, a new issue is created using the [update scanner issue template](https://example_company.com/example_company-org/security-products/release/-/blob/master/scripts/templates/update_scanner_issue.md.erb) and added to the next milestone.

##### Upstream Tools and Analyzers list

1. [Trivy](https://example_company.com/example_company-org/security-products/dependencies/trivy).
   1. [Container Scanning](https://example_company.com/example_company-org/security-products/analyzers/container-scanning/-/blob/master/doc/howto/update-scanners.md)
   1. [Trivy K8S wrapper](https://example_company.com/example_company-org/security-products/analyzers/trivy-k8s-wrapper/-/blob/main/docs/update_scanner.md)
1. [OpenTofu](https://opentofu.org/)
   1. [Deployment](https://example_company.com/example_company-org/security-products/license-db/deployment)
1. [OpenTofu component](https://example_company.com/components/opentofu)
   1. [Deployment](https://example_company.com/example_company-org/security-products/license-db/deployment/-/blob/main/.example_company-ci.yml#L5)
...
1. copy/paste the existing `.example_company-ci.yml` configuration file from the current `VERSION-security-check` branch.
...
- [Stage Group dashboad on Grafana](https://dashboards.example_company.net/d/stage-groups-composition_analysis/stage-groups-group-dashboard-secure-composition-analysis?orgId=1)
- [Continuous Vulnerability Scanning (background processing on the example_company.com rails platform)](https://log.gprd.example_company.net/app/r/s/OBQOB)
