...
Following our [single application](/handbook/product/categories/example_company-the-product/single-application/) paradigm,
we integrate and build scanning tools to supply security and compliance assessment data to the main Example Company application
where we develop our vulnerability management system and other features.
While it might be technically feasible, we do not aim at building standalone products that provide this data independently from the Example Company application.

For more details about the vision for this area of the product, see the [Secure stage](https://about.example_company.com/stages-devops-lifecycle/#secure) page.

## Mission

To support the success of Example Company by developing highly usable, hiqh quality tools for customers to build more secure software.

The Secure team works on Example Company's [Secure stage](https://about.example_company.com/stages-devops-lifecycle/#secure).
...
* Example Company: `@example_company-org/secure/managers`
...
The Secure Team (previously known as the *Security Products Team*) is responsible for the security checks features in the Example Company platform, and maps to the [secure](/handbook/product/categories/#secure) transversal stage.
You can learn more about our approach on the [Secure Vision](https://about.example_company.com/direction/secure/) page.
...
We still refer to "*Security Products*" as the tools developed by the Secure Team. Hence the home of our projects in Example Company: [https://example_company.com/example_company-org/security-products/](https://example_company.com/example_company-org/security-products/).
...
[SAST](https://docs.example_company.com/ee/user/application_security/sast/) (*Static Application Security Testing*) refers to static code analysis.
Example Company leverages the power of various opensource tools to provide a wide range of checks for many languages and support.
These tools are wrapped inside docker images which ensure we get a standard output from there.
An orchestrator, [developed by Example Company](https://example_company.com/example_company-org/security-products/sast), is in charge of running these images, and gathering all the data needed to generate the final report.

#### DAST

[DAST](https://docs.example_company.com/ee/user/application_security/dast/) (*Dynamic Application Security Testing*) is used to hit a live application.
...
DAST is relying on [OWASP Zed Attack Proxy Project](https://example_company.com/example_company-org/security-products/zaproxy), modified by Example Company to enable authentication.

#### Dependency Scanning

[Dependency Scanning](https://docs.example_company.com/ee/user/application_security/dependency_scanning/) is used to detect vulnerabilities introduced by external dependencies in the application.
...
Coverage-guided fuzzing and API fuzzing are used to automatically input data into applications or web apis that has the potential to cause crashes or bugs. Coverage-guided fuzzing relies on open-sourced [language-specific fuzzers](https://example_company.com/example_company-org/security-products/analyzers/fuzzers). API Fuzzing is based on a [proprietary Example Company engine](https://example_company.com/example_company-org/security-products/analyzers/api-fuzzing-src).

#### License Compliance

[License Compliance](https://docs.example_company.com/ee/user/compliance/license_approval_policies.html) helps with the licenses introduced by third-party libraries in the application.
...
[Secure stage](https://about.example_company.com/stages-devops-lifecycle/secure/).
...
Additional labels should be added according to the [Workflow Labels Documentation](https://example_company.com/example_company-org/example_company-ce/blob/master/doc/development/contributing/issue_workflow.md#workflow-labels).
...
We provide tools for many different languages (ex: [sast](https://docs.example_company.com/ee/user/application_security/sast/#supported-languages-and-frameworks), [dependency scanning](https://docs.example_company.com/ee/user/application_security/dependency_scanning/#supported-languages-and-dependency-managers), [license compliance](https://docs.example_company.com/ee/user/compliance/license_approval_policies.html#supported-languages-and-package-managers)). It means our team is able to understand the basics of each of these languages, including their package managers. We maintain [tests projects](https://example_company.com/example_company-org/security-products/tests) to ensure our features are working release after release for each of them.

### Release process

See [Versioning and release process](https://docs.example_company.com/ee/development/sec/analyzer_development_guide.html).
...
We use the [security-triage-automation](https://example_company.com/example_company-org/secure/tools/security-triage-automation) tool in conjunction with [scheduled pipelines in the release project](https://example_company.com/example_company-org/security-products/release/-/blob/master/.example_company/ci/security-triage-automation.yml?ref_type=heads) to handle the following tasks:

1. [Create security issues for FedRAMP vulnerabilities (Container Scanning results only) still detected on the default branch](https://example_company.com/example_company-org/secure/tools/security-triage-automation#process-vulnerabilities-for-a-given-project), executed at least once, on the first day of the month to match with FedRAMP compliance report cadence.
...
1. [Resolve all vulnerabilities (both FedRAMP and non-FedRAMP) no longer detected on the default branch and close their issues](https://example_company.com/example_company-org/secure/tools/security-triage-automation#resolve-vulnerabilities-and-close-their-issues), executed every 2 days.

[The Vulnmapper tool](https://example_company.com/example_company-com/gl-security/threatmanagement/vulnerability-management/vulnerability-management-internal/vulnmapper) also provides some [automation to vulnerability management](/handbook/security/product-security/vulnerability-management/automation/) like:
...
Note: Our goal is to centralize automation for vulnerability management in the [Vulnmapper tool in the nearest future](https://example_company.com/example_company-com/gl-security/threatmanagement/vulnerability-management/vulnerability-management-internal/vulnmapper/-/milestones/4#tab-issues) and standardize our processes across the company. However, so far we're following the existing process based on the [security-triage-automation tool](https://example_company.com/example_company-org/secure/tools/security-triage-automation).

#### Automation failures

It's possible that our security automation tooling may [fail](https://example_company.com/example_company-org/security-products/release/-/pipelines?page=1&scope=all&status=failed).
...
2. Execute the `security-triage-automation` tool to [resolve vulnerabilities and close their issues](https://example_company.com/example_company-org/secure/tools/security-triage-automation#resolve-vulnerabilities-and-close-their-issues). This tool must be executed separately for each project that have vulnerabilities to resolve.
...
tool to [process vulnerabilities for a given project](https://example_company.com/example_company-org/secure/tools/security-triage-automation#process-vulnerabilities-for-a-given-project)
...
1. Open a DR issue with the [operational requirement template](https://example_company.com/example_company-com/gl-security/security-assurance/team-security-dedicated-compliance/poam-deviation-requests/-/issues/new?issuable_template=operational_requirement_template).
...
We do not yet have the same automation in place for non-FedRAMP vulnerabilities since it represents a too important volume to manage for our teams and some necessary [improvements in the vulnmapper tool](https://example_company.com/example_company-com/gl-security/threatmanagement/vulnerability-management/vulnerability-management-internal/vulnmapper/-/milestones/4#tab-issues) are required prior to enabling this.
...
500 errors on example_company.com are reported to Sentry. Below are some quick links to pull up Sentry errors related to Secure.

* StoreSecurityReports Worker - https://sentry.example_company.net/example_company/gitlabcom/?query=is%3Aunresolved+StoreSecurityReportsWorker&statsPeriod=14d
* SyncSecurityReportsToReportApprovalRules Worker - https://sentry.example_company.net/example_company/gitlabcom/?query=is%3Aunresolved+SyncSecurityReportsToReportApprovalRulesWorker&statsPeriod=14d
* Vulnerabilities - https://sentry.example_company.net/example_company/gitlabcom/?query=is%3Aunresolved+vulnerabilities&statsPeriod=14d
* On-Demand DAST - https://sentry.example_company.net/example_company/gitlabcom/?query=is%3Aunresolved+Dast&statsPeriod=14d
...
* [Reduce false positives in Security reports](https://example_company.com/example_company-org/example_company/issues/33934)
* How to manage occurrence uniqueness identification in the common report format? (CompareKey)
* [One file with syntax errors should not stop SAST and similar kind of jobs from running](https://example_company.com/example_company-org/example_company/issues/7102)
...
* [Example Company QA README](https://example_company.com/example_company-org/example_company/-/tree/master/qa)
* [Example Company QA Scenarios](https://example_company.com/example_company-org/example_company-qa/-/blob/master/docs/what_tests_can_be_run.md)
* [E2E info for Example Company developers](https://docs.example_company.com/ee/development/testing_guide/end_to_end/)
...
[Further information on the documentation process](https://docs.example_company.com/ee/development/documentation/workflow.html).
...
* ![Accomplished](img/ci-success.svg) Accomplished the `Spotbugs java analyzer compareKey is not unique` [https://example_company.com/example_company-org/example_company-ee/issues/10860](https://example_company.com/example_company-org/example_company-ee/issues/10860)
* ![In progress](img/ci-running.svg) Still working on `Allow guests to create an issue from a vulnerability` [https://example_company.com/example_company-org/example_company-ee/issues/7813](https://example_company.com/example_company-org/example_company-ee/issues/7813)
...
1. Composition Analysis: [#g_secure-composition-analysis-standup](https://example_company.slack.com/archives/g_secure-composition-analysis-standup)
1. Static Analysis: [#g_secure-static-analysis-standup](https://example_company.slack.com/archives/g_secure-static-analysis-standup)
1. Dynamic Analysis: [#g_secure-dynamic-analysis-standup](https://example_company.slack.com/archives/g_secure-dynamic-analysis-standup)
1. Frontend: [#s_secure-frontend-standup](https://example_company.slack.com/archives/s_secure-frontend-standup)

#### Recorded meetings

Our important meetings are recorded and published on YouTube, in the [Example Company Secure Stage playlist](https://www.youtube.com/playlist?list=PL05JrBw4t0Kq7yUrZazEF3diazV29RRo1).
...
We welcome team members to join meetings that are on our shared calendar. The [Secure Calendar](https://calendar.google.com/calendar?cid=Z2l0bGFiLmNvbV9tZDBhbzM2Z3B2bDV2MWY0MTI4ZXJobmo2Z0Bncm91cC5jYWxlbmRhci5nb29nbGUuY29t) is available to all logged in Example Company team members.

### Staying informed

Example Company is an extremely active organization which generates a lot of news and activity each week. Everyone in Secure are encouraged to keep themselves informed as to what is happening in the larger organzation. Everyone is also
encouraged to contribute to these channels and communication paradigms when you have information to share.

In addition to this, each group in Secure conducts a weekly synchronous meeting. These meetings are publicized on the Secure Calendar mentioned above. As always at Example Company, we strive to [make meeting attendance optional](/handbook/company/culture/all-remote/meetings/#1-make-meeting-attendance-optional).
...
Every new hire will have an assigned [onboarding issue](https://example_company.com/example_company-org/security-products/onboarding/blob/master/.example_company/issue_templates/Technical_Onboarding.md) that will guide them through the whole process.
...
[`~secure::frontend backlog` label](https://example_company.com/example_company-org/example_company/-/issues/?label_name%5B%5D=secure%3A%3Afrontend%20backlog) to issues which are ready to be picked up. Additionally,
...
The Secure Team follows the coding standards and style guidelines outlined in the company-wide [Contributor and Development Docs](https://docs.example_company.com/ee/development/), however, please consult the following guidelines which are specific to the Secure Team:

* [Secure Team Go standards and style guidelines](https://docs.example_company.com/ee/development/go_guide#secure-team-standards-and-style-guidelines)

#### Cross group collaboration

Some components of the architecture that support Secure features are shared between multiple groups like the [common](https://example_company.com/example_company-org/security-products/analyzers/common) Go library,
the [Security Report Schemas](https://example_company.com/example_company-org/security-products/security-report-schemas), the [rails parsers](https://example_company.com/example_company-org/example_company/-/tree/master/ee/lib/example_company/ci/parsers/security), etc.
...
For a complete guide about developing a new analyzer please refer to our [user documentation](https://docs.example_company.com/ee/development/sec/analyzer_development_guide.html#development-of-new-analyzers)
...
We keep a [list of data sources in our internal wiki](https://example_company.com/example_company-org/secure/data-feeds-and-sources/-/wikis/Data-Feeds-&-Sources). This includes advisory databases, package license information, and related data.
...
Each group's DRI is responsible to prepare and schedule the retrospective sync sessions and the async retrospective issues can be found in [the corresponding project](https://example_company.com/gl-retrospectives/secure-sub-dept).
...
NB: we use to have a sub-department wide retrospective whose issues are still accessible [in the deprecated project](https://example_company.com/gl-retrospectives/secure/-/issues).
...
[Secure Scan Metrics Dashboard](https://10az.online.tableau.com/t/example_company/views/PDSecureScanMetrics_17090087673440/SecureScanMetrics)
...
* [Secure team board](https://example_company.com/groups/example_company-org/-/boards/588843?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=devops%3A%3Asecure)
* [#s_secure](https://example_company.slack.com/archives/s_secure) in Slack
* [Secure and Govern Glossary of Terms](https://docs.example_company.com/ee/user/application_security/terminology/)
