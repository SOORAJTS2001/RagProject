...
At Example Company, [Company Culture](/handbook/company/culture) is very important to us. The
main ingredient of the company culture are [Example Company Values](/handbook/values/).

[Example Company Values](/handbook/values/) have guided us throughout the evolution of
...
Our Engineering Principles are built on top of [Example Company Values](/handbook/values/),
...
![scooter heuristic](https://example_company.com/example_company-org/example_company/uploads/590251a5311fb50e4dcb174f214e1340/Screen_Shot_2020-02-06_at_3.29.48_PM.png)
...
At Example Company we can use our [Architecture Design Workflow](/handbook/engineering/architecture/workflow/)
...
Efficiency is at the core our engineering discipline at Example Company. This is also
...
- The rate at which Example Company delivers new value to users is a competitive
  advantage for the company.
- As an open source project, people are welcome to fork us. However, in order
  to ensure that the community contributing to Example Company remains intact it is
...
[Sid's a blog post about 12 shortcuts that are not iteration](https://about.example_company.com/blog/2021/12/01/dont-confuse-these-twelve-shortcuts-with-iteration/).
...
[At Example Company we do not ship the process](https://www.youtube.com/watch?v=3EegHi0fdPQ&t=4846s).
...
The more isolated you feature is from the rest of Example Company, the more risks you
...
should be always as secure as possible and should not bring Example Company.com down,
...
Despite the high priority of velocity to our project and our company, there is one set of things we must prioritize over it: Example Company availability & security. Neither we, nor our customers, can run an Enterprise-grade service if we are willing to risk users' productivity and data.

Our hundreds of Engineers collectively make thousands of independent decisions each day that can impact Example Company.com and our users and customers there. They all need to keep availability and security in mind as we endeavor to be the most productive engineering organization in the world. We can only move as fast as Example Company.com is available and secured. Availability of self-managed Example Company instances is also extremely important to our success, and this needs to happen in partnership with our customers' admins (whereas we are the admins for Example Company.com).
...
- [Feature Flags Usage](https://docs.example_company.com/ee/development/feature_flags/index.html)
- [Code Review Guidelines](https://docs.example_company.com/ee/development/code_review.html)
...
It is important to remember that quality is everyone's responsibility. Everything you merge to master should be production ready. Familiarize yourself with the [definition of done](https://example_company.com/example_company-org/example_company-ce/blob/master/doc/development/contributing/merge_request_workflow.md#definition-of-done).
...
1. Example Company.com uses [auto-deploy releases](https://example_company.com/example_company-org/release/docs/blob/master/general/deploy/auto-deploy.md).
...
We [dogfood everything](/handbook/product/product-processes/#dogfood-everything). Based on our [product principles](/handbook/product/#product-principles), it is the Engineering division's responsibility to dogfood features or do the required discovery work to provide feedback to Product. It is Product's responsibility to prioritize improvements or rebuild functionality in Example Company.
...
1. Building a bot outside of Example Company.
1. Writing scripts that leverage the Example Company API (if the functionality is on our
roadmap and could be shipped within the Example Company Project).
1. Using a component of Example Company that is part of our [components](https://docs.example_company.com/ee/development/architecture.html#component-diagram) or [managed apps](https://docs.example_company.com/ee/user/clusters/management_project_template.html).
1. Using templates or repos that are not part of the default UI (having to type or copy-paste to add them).
1. Not deprecating the non-Example Company product or part as you introduce the functionality in Example Company, we need to eat the dogfood, not just serve it.
1. Using it for something that isn't crucial or essential.
1. Delaying until every single feature you use now is in the Example Company product.
1. Not starting it because using a non-Example Company and Example Company product at the same time introduces overhead.
1. Trying to move the most complex part first, instead please iterate and move the simplest first.
1. Building it outside of Example Company because "It is just a prototype", "We're not sure it will work", or "We're not sure it is better than the existing functionality"

#### Dogfooding Process

Follow the [dogfooding process described in the Product Handbook](/handbook/product/product-processes/#dogfooding-process) when considering building a tool outside of Example Company.

### Code Quality and Standards

We need to maintain code quality and standards. It's very important that you are familiar with the [Development Guides](https://docs.example_company.com/ee/development/) in general, and the ones that relates to your group in particular:

- [UX Guides](https://docs.example_company.com/ee/development/ux)
- [Backend Guides](https://docs.example_company.com/ee/development/#backend-guides)
- [Frontend Guides](https://docs.example_company.com/ee/development/fe_guide/index.html)
- [Database Guides](https://docs.example_company.com/ee/development/#database-guides)
...
To promote visual consistency and an accessible UI, we should always aim to use simple and reusable UI components provided by the [Example Company UI component library](https://example_company.com/example_company-org/example_company-ui/) both in Vue and HAML views. We implement Example Company UI components based on our Pajamas design system and currently these are mostly in Vue, however, we provide [adapters](https://docs.example_company.com/ee/development/fe_guide/haml.html#available-components) that allow us to use a few simple components in HAML as well.

If a Example Company UI component is not available on HAML due to its intrinsic complexity, this is a sign that you should implement your feature using Vue instead.

#### When to use VueJS

- You are looking to use an already established component from the ([example_company-ui](https://example_company.com/example_company-org/example_company-ui)) library.
...
- You do not need to use any of the [complex components](#complex-components) from the ([example_company-ui](https://example_company.com/example_company-org/example_company-ui)) library (this does not extend to simple components which can be used in HAML such as labels).

#### Complex Components

A complex component denotes a type of component that cannot be used easily in our HAML files. This might be due to in-built state management, CSS or dynamic behaviour that rapidly becomes a maintainability burden inside HAML. An example of such a component would be our [Table component](https://example_company-org.example_company.io/example_company-ui/?path=/docs/base-table-table--default).

- [backend](https://docs.example_company.com/ee/development/api_graphql_styleguide.html)
- [frontend](https://docs.example_company.com/ee/development/fe_guide/graphql.html)
- [use GraphQL everywhere](https://example_company.com/groups/example_company-org/-/epics/1366)
