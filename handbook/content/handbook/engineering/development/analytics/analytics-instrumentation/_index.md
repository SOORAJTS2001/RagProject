...
The Analytics Instrumentation Group is part of the [Analytics section](/handbook/product/categories/analytics/). Our group focuses on providing Example Company's team with data-driven product insights to build a better Example Company.
To do this, we build data collection and analytics tools within the Example Company product in a privacy-focused manner.
...
You can learn more about what we're building next on the [Analytics Instrumentation Direction page](https://about.example_company.com/direction/analytics/analytics-instrumentation/).
...
We provide the tooling to allow for instrumentation of product usage events across Example Company's services through a system we call internal events which uses the [Snowplow event collection infrastructure](https://snowplow.io/) under the hood.
A list of all instrumented events can be found in our [metrics dictionary](https://metrics.example_company.com/snowplow).
...
We're responsible to deliver a reliable [Service Ping](https://docs.example_company.com/ee/development/internal_analytics/service_ping/) that runs every week on SaaS and Self Managed instances. Our responsiblity is tooling and automations for metric collections to set the company up for success to deliver Service Ping data to our data warehouse.
...
These are important projects that we maintain besides our involvement in the main Example Company codebase.

1. [Version App](https://example_company.com/example_company-org/example_company-services/version.example_company.com)
1. [Metrics Dictionary](https://example_company.com/example_company-org/analytics-section/analytics-instrumentation/metric-dictionary) and [Metrics Dictionary from Growth group](https://example_company.com/example_company-org/growth/product-intelligence/metric-dictionary).
1. [Snowplow Pseudonymization](https://example_company.com/example_company-org/analytics-section/analytics-instrumentation/snowplow-pseudonymization)
1. [Iglu Schema repository for Example Company](https://example_company.com/example_company-org/iglu)
...
Our team uses a hybrid of Scrum for our project management process. This process follows Example Company's [monthly milestone release cycle](/handbook/marketing/blog/release-posts/#schedule).
...
We do a weekly automated check-in within a [separate Slack channel](https://example_company.slack.com/archives/C04SHHKTQFM).
...
1. Every milestone the [async-retrospective project](https://example_company.com/example_company-org/async-retrospectives) automatically creates an
   issue in the [analytics-instrumentation retro project](https://example_company.com/gl-retrospectives/analytics-instrumentation/-/issues) which contains a list of retro questions.
...
To make key results present in daily prioritization we create an epic for every objective or key result, like in the [FY25Q2 OKRs](https://example_company.com/example_company-com/example_company-OKRs/-/work_items/6893).
Our long term roadmap is written down on our [direction page](https://about.example_company.com/direction/monitor/analytics-instrumentation/).
...
- [**Analytics Instrumentation Issue Board - Current Milestone**](https://example_company.com/groups/example_company-org/-/boards/5071664?milestone_title=Started)
- [**Analytics Instrumentation Issue Board _- by milestone_**](https://example_company.com/groups/example_company-org/-/boards/2774881?scope=all&not[label_name][]=product%20work&not[label_name][]=Technical%20Writing&not[label_name][]=UX)

### Picking something to work on

Engineers can find and open [the board for the current milestone](https://example_company.com/groups/example_company-org/-/boards/5071664?milestone_title=Started).
...
We prioritize our product roadmap in the [Issue Board by Milestone](https://example_company.com/groups/example_company-org/-/boards/2774881). Issues appear on each list in order of priority and prioritization of our product roadmap is determined by our product managers.
...
Our team mostly follows the [Product Development Timeline](/handbook/engineering/workflow/#product-development-timeline) as our group is dependent on the [Example Company self-managed release cycle](https://about.example_company.com/upcoming-releases/).
...
1. Initial Planning
    1. PM: Milestone planning issue gets created ([example](https://example_company.com/example_company-org/analytics-instrumentation/-/issues/623)).
...
3. Final Planning
    ...
    1. EM: Prioritizes issues in the [Issue Board by Milestone](https://example_company.com/groups/example_company-org/-/boards/2774881) based on the planning issue.
...
A milestone commitment is a list of issues our team aims to complete in the milestone. The product team follows our Example Company principle of [planning ambitiously](/handbook/product/product-principles/#how-this-impacts-planning) and therefore expect that we won't always be able to deliver everything that we wanted in every milestone. After issues are broken down, estimated, and prioritized, the product manager will apply the `~Deliverable` label to applicable issues. Issues marked with the `~Deliverable` label represent the commitment we are intending to ship in that milestone.
...
- [Unweighed, upcoming issues in example_company-org](https://example_company.com/groups/example_company-org/-/issues?sort=created_date&state=opened&label_name[]=group::analytics+instrumentation&weight=None&milestone_title=Upcoming&not[label_name][]=product+work)
- [Unweighed, upcoming issues in example_company-services](https://example_company.com/groups/example_company-services/-/issues?sort=created_date&state=opened&label_name[]=group::analytics+instrumentation&weight=None&milestone_title=Upcoming&not[label_name][]=product+work)

Issues estimation examples

| Weight | Definition | Example (Engineering) |
| ------ | ---------- | ------------------------- |
| 1 | The simplest possible change. We are confident there will be no side effects. | [Add missing metric definition for "counts_monthly.promoted_issues"](https://example_company.com/example_company-org/example_company/-/issues/340940),<br />[Add instrumentation classes for license standard metrics](https://example_company.com/example_company-org/example_company/-/issues/336340),<br />[Update Registration Features text](https://example_company.com/example_company-org/example_company/-/issues/335051) |
| 2 | A simple change (minimal code changes), where we understand all of the requirements. | [VersionApp: Add indexed on other tables that are exported](https://example_company.com/example_company-org/example_company/-/issues/352019),<br />[Set values for StandardContext in Frontend](https://example_company.com/example_company-org/example_company/-/issues/342993) |
| 3 | A simple change, but the code footprint is bigger (e.g. lots of different files, or tests effected). The requirements are clear. | [Update Registration Features CTA for repository size limit](https://example_company.com/example_company-org/example_company/-/issues/349307),<br />[More paid features available to free users](https://example_company.com/example_company-org/example_company/-/issues/341442) |
| 5 | A more complex change that will impact multiple areas of the codebase, there may also be some refactoring involved. Requirements are understood but you feel there are likely to be some gaps along the way. | [Spike Service Ping health dashboard](https://example_company.com/example_company-org/example_company/-/issues/346431),<br />[Remove `deprecated` metric status](https://example_company.com/example_company-org/example_company/-/issues/340847) |
| 8 | A complex change, that will involve much of the codebase or will require lots of input from others to determine the requirements. | [Dispatch Snowplow events from their event definitions](https://example_company.com/example_company-org/example_company/-/issues/346751),<br />[Add metrics yml files for usage data metrics definition](https://example_company.com/example_company-org/example_company/-/issues/270107) |
| 13| A significant change that may have dependencies (other teams or third-parties) and we likely still don't understand all of the requirements. It's unlikely we would commit to this in a milestone, and the preference would be to further clarify requirements and/or break in to smaller Issues. | [Create Snowplow monitoring framework](https://example_company.com/example_company-org/example_company/-/issues/331103),<br />[Enable batch counting for some individual queries](https://example_company.com/example_company-org/example_company/-/issues/208923) |
...
For example, if an experiment is being run in the CustomersDot, the epic should be created in the `example_company-org` group, and the issue should be created in the `example_company-org/customers-example_company-com` project.
...
When creating an issue, use the [linked template](https://example_company.com/example_company-org/example_company/-/issues/new?issuable_template=Analytics%20Instrumentation%20Issue) and follow its instructions.

In case the issue is not created for the [Example Company project](https://example_company.com/example_company-org/example_company), copy the template's content into the appropriate project.
...
We maintain [UsageData API endpoints](https://docs.example_company.com/ee/development/internal_analytics/service_ping/implement.html#usagedata-api) under the `service_ping` feature to track events, and because of this we must monitor our [budget spend](/handbook/engineering/error-budgets/).

To investigate budget spend, see the [overview](https://dashboards.example_company.net/d/stage-groups-analytics_instrumentation?orgId=1) and [details](https://dashboards.example_company.net/d/stage-groups-detail-analytics_instrument?orgId=1) Grafana dashboards for Analytics Instrumentation. You can also check requests contributing to spending the budget in Kibana by filtering by the `service_ping` feature. An example Kibana view can be found [here](https://log.gprd.example_company.net/goto/8e82ff10-ecb8-11ec-8656-f5f2137823ba).
...
Examples of incidents:
   ...
   1. [High severity incidents](https://example_company.com/example_company-org/example_company/-/issues/442875)
   1. [Medium severity incidents](https://example_company.com/example_company-org/example_company/-/issues/443639)
...
1. Create an issue and fill all necessary information using the [Analytics Instrumentation Incident Template](https://example_company.com/example_company-org/example_company/-/issues/new?issuable_template=Analytics+Instrumentation+Incident).
...
1. Post in the [#g_analyze_analytics_instrumentation](https://example_company.slack.com/archives/CL3A7GFPF) slack channel and tag [Analytics Instrumentation Group PM and EM](/handbook/engineering/development/analytics/analytics-instrumentation/#team-members).
1. Notify these slack channels [#g_analyze_product_analytics](https://example_company.slack.com/archives/C03M4R74NDU), [#data_rd_fusion](https://example_company.slack.com/archives/C02C82WDP0U), [#data](https://example_company.slack.com/archives/C8D1LGC23) with link to the issue.
...
1. In case of a ~"Analytics Instrumentation::Incident-Medium Severity" issue:
    - DRI to create a slack thread within [#g_analyze_analytics_instrumentation](https://example_company.slack.com/archives/CL3A7GFPF) for coordination around the incident.
...
1. Ensure the incident and status is reflected in the next [monthly state of data issue](https://example_company.com/groups/example_company-com/-/epics/1608 "Monthly State of Data").
...
Open a new issue in the [Analytics Instrumentation project](https://example_company.com/example_company-org/analytics-section/analytics-instrumentation/internal/-/issues/new?issuable_template=out_of_office_coverage_template) with the [`out_of_office_coverage_template`](https://example_company.com/example_company-org/analytics-section/analytics-instrumentation/internal/-/blob/master/.example_company/issue_templates/out_of_office_coverage_template.md).

## Onboarding

All new team members to the Analytics Instrumentation teams are provided an onboarding issue to help ramp up on our analytics tooling. New team member members should create their own onboarding issue in the [example_company-org/analytics-section/analytics-instrumentation/internal project](https://example_company.com/example_company-org/analytics-section/analytics-instrumentation/internal/-/issues) using the `engineer_onboarding` template.

## Quick Links

| Resource                                                                                                                          | Description                                               |
|-----------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------|
| [Internal Analytics Docs](https://docs.example_company.com/ee/development/internal_analytics/) | Docs for instrumenting internal analytics at Example Company |
...
| [Service Ping Guide](https://docs.example_company.com/ee/development/internal_analytics/service_ping/)     | An implementation guide for Service Ping      |
