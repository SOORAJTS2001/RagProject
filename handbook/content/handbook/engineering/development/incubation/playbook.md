...
How do we intend to solve the problem? What is our overall vision for this project and how it will fit into Example Company?
...
JTBD is an important part of the process of establishing an SEG. Not only does writing job statements help the SEG to clearly identify their areas of focus, but it also helps to communicate the roadmap and rationale to the rest of the organization and the broader Example Company community.
...
The **Maturity Rating** should align with how maturity is measured in Example Company (minimal, viable, etc.), and should be measured using the same criteria as [described in the Product handbook](https://about.example_company.com/direction/#maturity).
...
Links to articles (both internal and external to Example Company), issues and epics that provide further information on our approach.
...
1. Request "Owner" access to the [Incubation Engineering group](https://example_company.com/example_company-org/incubation-engineering/) from one of your fellow department members.
1. Create a new subgroup within the [Incubation Engineering group](https://example_company.com/example_company-org/incubation-engineering/). For example: [`mobile-devops`](https://example_company.com/example_company-org/incubation-engineering/mobile-devops)
1. Create a `meta` project within that new group. For example: [mlops/meta](https://example_company.com/example_company-org/incubation-engineering/mlops/meta)
1. Create a twice montly update issue in that new `meta` project. This issue will replace the existing backlog project issue and be used to post bi-weekly demo recordings from the new SEG. For example: [jamstack](https://example_company.com/example_company-org/incubation-engineering/jamstack/meta/-/issues/5)
1. Add all labels from the original backlog issue to the issue you just created. For example: [original issue](https://example_company.com/example_company-org/example_company/-/issues/329592) | [new issue](https://example_company.com/example_company-org/incubation-engineering/mobile-devops/readme/-/issues/7)
1. Add the new project to the `seg_issues_list` in the handbook direction generator module in the handbook. See this [MR](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/93485/diffs) for an example.
1. Close out the original backlog issue with a comment pointing to the new weekly update issue ([example](https://example_company.com/example_company-org/example_company/-/issues/329592#note_676451924)).
1. Make sure the project maps to an existing stage and feature category, or create new ones if needed. For example: [Add No-code Automation SEG stage and feature category](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/118730). [Feature categorization](https://docs.example_company.com/ee/development/feature_categorization/#rspec-examples) is mandatory in RSpec examples to identify the group owning this file.
...
Field marketing has welcomed all Incubation Engineering team members to shadow upcoming field marketing events. These events are conducted with new prospects, existing customers and individual Example Company / DevOps enthusiasts.
...
* [Field & Corporate, EMEA](https://example_company.com/groups/example_company-com/marketing/-/boards/933459?label_name[]=EMEA)
* [Field & Corporate, Central Europe](https://example_company.com/groups/example_company-com/marketing/-/boards/1438243?label_name[]=Central%20Europe&label_name[]=EMEA)
* [Field & Corporate, UK/I](https://example_company.com/groups/example_company-com/marketing/-/boards/1438265?label_name[]=UK%2FI)
...
Take advantage of Example Company resources with guidance on how to [facilitate customer interviews](/handbook/product/ux/ux-research/facilitating-user-interviews/) and look for opportunities to join calls with other product leaders to learn about their approach.

For Incubation Engineering team members not familiar with customer conversations, it is perfectly okay to simply _shadow_ calls and to say nothing throughout the call. Introduce yourself if invited, and help Example Company team members by keeping notes.

Those comfortable with external interactions may offer their services to the Field Marketing event lead by means of answering questions and interacting with the participants when the Example Company speaker requests support.

### Quality & Support Guidelines for Incubation Projects

* Is your feature behind a feature flag? It's preferrable that it is.
* Can your feature be visually labeled as an `incubation` or `experimental` feature without distracting / taking away from the Example Company user experience?
...
Incubation Engineers should familiarize themselves with the [Example Company AppSec Review](/handbook/security/product-security/application-security/appsec-reviews.html) process and preferably have the AppSec review triggered early for each merge request when appropriate.
...
1. If you created a feature issue that used the [feature issue template]
   ...
   use the [release post item generator](https://example_company.com/example_company-com/www-example_company-com/blob/master/bin/release-post-item)
...
1. Sublime Text
1. Vim
1. Example Company Web IDE

### How do I

#### Add a new API endpoint?

[This MR gives an example on how to add a brand new API endpoint](https://example_company.com/example_company-org/example_company/-/merge_requests/95689)
...
Engaging with the Community and getting feedback is tricky when starting an SEG, and more so if the SEG focuses on an area Example Company has not previously engaged in. Here is a list of things that have helped others find communities:
