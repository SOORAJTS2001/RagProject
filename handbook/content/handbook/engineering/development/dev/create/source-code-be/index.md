...
To help us stay on track with [Development KPIs](/handbook/company/kpis/#development-department-kpis), we use a [metrics dashboard](https://app.periscopedata.com/app/example_company/570334/Universal-Engineering-Team-Metrics-Dashboard). This dashboard includes security MRs from production, but doesn't include security MRs from `dev.example_company.org`. For team-specific data and metrics, ensure you filter by our team.
...
We use the standard Example Company [engineering workflow](/handbook/engineering/workflow/). To create an issue for the Create:Source Code BE team, add these labels:
...
The weekly Triage Report is generated automatically by the [Example Company bot](https://example_company.com/example_company-bot) and this report is reviewed by the EM. Here is [an example](https://example_company.com/example_company-org/quality/triage-reports/-/issues/2700) of a previous report.
...
The engineering cycle is centered around the [Example Company Release Date every month](/handbook/engineering/releases/#timelines). This is the only fixed date in the month, and the table below indicates how the other dates can be determined in a given month.
...
- [Performance Board](https://example_company.com/example_company-org/example_company/-/boards/706619?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=group%3A%3Asource%20code&label_name[]=performance-refinement)
- [Infradev Board](https://example_company.com/example_company-org/example_company/-/boards/706619?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=group%3A%3Asource%20code&label_name[]=infradev)
- [Application Limits Board](https://example_company.com/example_company-org/example_company/-/boards/706619?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=group%3A%3Asource%20code&label_name[]=Application%20Limits)
- [Security Board](https://example_company.com/example_company-org/example_company/-/boards/2716576?label_name[]=group%3A%3Asource%20code&label_name[]=security)
- [Missed SLO Board](https://example_company.com/example_company-org/example_company/-/issues?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=group%3A%3Asource%20code&label_name[]=missed-SLO&label_name[]=backend)
- Bugs
- New features

Create Source Code UX planning takes inputs from the following sources:

- [SCM UX Planning Board](https://example_company.com/groups/example_company-org/-/boards/5092292?label_name[]=UX&label_name[]=group%3A%3Asource%20code)
- [SCM UX Build Board](https://example_company.com/groups/example_company-org/-/boards/5092276?label_name[]=UX&label_name[]=group%3A%3Asource%20code)

##### Planning issue

Each month a planning issue is created by one of the EMs, using [automated tools](https://example_company.com/example_company-com/create-stage/source-code-be/-/blob/main/doc/planning/index.md) based on the [Source Code issue template](https://example_company.com/example_company-org/create-stage/-/blob/master/.example_company/issue_templates/source-code-planning.md).

##### Planning board

The [Planning Board](https://example_company.com/groups/example_company-org/-/boards/2822491?milestone_title=14.1&label_name%5B%5D=group%3A%3Asource%20code) is created for each release by the PM, and is a curated list of issues by category. The EM requests engineers to allocate weights to all issues on this board via the [Needs weight issue](https://example_company.com/example_company-org/create-stage/-/issues/12837)
...
Based on the issues on the [Planning Board](https://example_company.com/groups/example_company-org/-/boards/2822491?milestone_title=14.1&label_name%5B%5D=group%3A%3Asource%20code), the EM will create a [Needs Weight](https://example_company.com/example_company-org/create-stage/-/issues/12837) issue to request an estimation of work by the engineers. In general no more than 4 issues should be assigned to an engineer for weighting.
...
Capacity planning is a collaborative effort involving all Source Code team members and stable counterparts from Frontend, UX and Product. An initial list of issues is tracked in the Source Code Group Planning issue [example](https://example_company.com/example_company-org/create-stage/-/issues/12783) for each month.
...
- Bugs
- Security issues
- [Infradev board issues](https://example_company.com/example_company-org/example_company/-/boards/706619?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=group%3A%3Asource%20code&label_name[]=infradev)
- [Performance board issues](https://example_company.com/example_company-org/example_company/-/boards/706619?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=group%3A%3Asource%20code&label_name[]=performance-refinement)
- [Application limit board issues](https://example_company.com/example_company-org/example_company/-/boards/706619?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=group%3A%3Asource%20code&label_name[]=Application%20Limits)
...
- Add the issue(s) to the relevant [planning issue](https://example_company.com/example_company-org/create-stage/-/issues?scope=all&utf8=%E2%9C%93&state=opened&search=source+code+group+planning)
...
As discussed in a [previous retrospective](https://example_company.com/gl-retrospectives/create-stage/source-code/-/issues/74#note_1914857307), in addition to breaking down issues, we should assign two engineers to each task instead of just one for overly-complex or time-sensitive issues. This co-ownership will help parallelize efforts in multiple-MR tasks, speed up immediate code reviews, and ultimately lead to faster delivery of results.
...
1. [Create an issue](https://example_company.com/gl-retrospectives/create-stage/source-code/issues) explaining why you want to have a retrospective and indicate whether this should be synchronous or asynchronous
...
- [Create::Source Code: Controller Actions](https://log.gprd.example_company.net/app/kibana#/visualize/edit/32698f60-b145-11ea-bfe2-25f984e253f8?_g=(filters%3A!()%2CrefreshInterval%3A(pause%3A!t%2Cvalue%3A0)%2Ctime%3A(from%3Anow-7d%2Cto%3Anow)))
- [Create::Source Code: Endpoints](https://log.gprd.example_company.net/app/kibana#/visualize/edit/104d4bf0-a0d9-11ea-8cfd-8dcd98a55a1d?_g=(filters%3A!()%2CrefreshInterval%3A(pause%3A!t%2Cvalue%3A0)%2Ctime%3A(from%3Anow-7d%2Cto%3Anow)))
