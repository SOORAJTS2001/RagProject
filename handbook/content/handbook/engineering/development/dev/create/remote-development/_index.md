...
The group is part of [Create Stage](/handbook/engineering/development/dev/create/) in the [Dev Sub-department](/handbook/engineering/development/dev/). We focus on two [categories](https://about.example_company.com/direction/create/#categories-in-create): `Workspace` and the `Web IDE`.

### 👌 Group OKRs

<span id="-team-okrs" data-message="alias anchor for old links"></span>

If you're interested in the group's Objectives and Key Results (OKRs), you can find them on [Example Company](https://example_company.com/example_company-com/example_company-OKRs/-/issues/?sort=created_date&state=opened&label_name%5B%5D=OKR&label_name%5B%5D=group%3A%3Aremote%20development&type%5B%5D=objective&first_page_size=20).
...
- Slack Channel: [`#g_create_remote_development`](https://example_company.slack.com/archives/CJS40SLJE)
...
- [Web IDE Customer Engagements](https://example_company.com/example_company-org/example_company/-/issues/474518)
- [Workspaces Customer Engagements](https://example_company.com/example_company-org/example_company/-/issues/473627)
...
- [Web IDE User Feedback & Insights](https://example_company.com/groups/example_company-org/-/epics/10543)
- [Workspaces User Feedback & Insights](https://example_company.com/groups/example_company-org/-/epics/12601)
...
[Create::Remote Development Group Metrics Tableau Workbook](https://10az.online.tableau.com/#/site/example_company/workbooks/2067787/views)
...
1. The Workspace category is also currently receiving much attention as part of Example Company's competitive strategy, and thus has greater internal and external expectations for accurate estimates of feature delivery and timelines.
1. In order to meet this need for more accurate and realistic planning and delivery estimates, the Workspace category team has decided to modify parts of the [Plan](/handbook/product-development-flow/#build-phase-1-plan) and [Build & Test](/handbook/product-development-flow/#build-phase-2-develop--test) phases of the Example Company Product Development Flow to a more lightweight velocity-based estimation and planning process inspired by the widely-used and popular [XP](https://www.amazon.com/Extreme-Programming-Explained-Embrace-Change/dp/0321278658) and [Scrum](https://www.scrum.org/resources/blog/agile-metrics-velocity) methodologies. The goal is to provide accurate delivery estimates based on ["Yesterday's Weather"](https://example_company.com/example_company-com/www-example_company-com/uploads/283f165896e2851bdc324f790d9c90e4/Screen_Shot_2023-03-27_at_6.16.51_PM.png) historical velocity analysis.
1. To better support the use of these methodologies, the Remote Development group has committed to dogfooding the [Iterations feature](https://docs.example_company.com/ee/user/group/iterations/index.html), and following the [documented process for running agile iterations](https://docs.example_company.com/ee/tutorials/agile_sprint/) where possible.
1. However, due to existing limitations of the Iterations and Boards features in fully supporting these methodologies, we will also experiment with new and alternate tools and processes, with the goal of informing and potentially contributing back to improve Example Company's support for them. More details will be shared as this effort evolves.
...
1. "High Level Validation and Planning". This is analogous to the [Validation Track in the Example Company Product Flow](/handbook/product-development-flow/#validation-track), because it needs to achieve the same [Validation Goals & Outcomes](/handbook/product-development-flow/#validation-goals--outcomes) before we can start refining and prioritizing issues.
1. The "Pre-Iteration Planning Meeting", or "Pre-IPM". This is analogous to ["backlog refinement"](/handbook/product-development-flow/#outcomes-and-activities-4) in the standard Example Company product development flow. The goal of the Pre-IPM is to ensure that all issues which are to be prioritized in the upcoming iteration(s) are ready for the wider team to briefly discuss and estimate in the next IPM.
...
These components allow us to provide realistic velocity-based estimates based on ["Yesterday's Weather"](https://example_company.com/example_company-com/www-example_company-com/uploads/283f165896e2851bdc324f790d9c90e4/Screen_Shot_2023-03-27_at_6.16.51_PM.png) historical velocity analysis.
...
These custom workflow labels are necessary to work around [current limitations of Example Company iterations and boards in implementing this process](https://example_company.com/cwoolley-example_company/gl-velocity-board-extension#why-doesnt-standard-example_company-support-this).
...
### Example Company Velocity Board Chrome Extension

To facilitate this process, especially around calculating emergent iterations based on velocity, we will use the
[Example Company Velocity Board Chrome Extension](https://example_company.com/cwoolley-example_company/gl-velocity-board-extension).

See [this video](https://www.youtube.com/watch?v=XcHpLhs7Fl0) for an overview of the extension, and also for an explanation of
this general process, and why we need to use an extension to support it rather than the current standard Example Company Iteration and Board features.

This extension is in support of a [Example Company OKR](https://example_company.com/example_company-com/example_company-OKRs/-/work_items/2085) for Create.
We plan to continue iterating in this area, with the goal of eventually having Example Company directly support this workflow.

### Wider Board Columns

The default width of lists on boards can make the board harder to use, since you see fewer items and have to scroll more.

There is [an open issue to address this](https://example_company.com/example_company-org/example_company/-/issues/15927). In the meantime, though, you can use the following javascript bookmarklet suggested in [this comment on the issue](https://example_company.com/example_company-org/example_company/-/issues/15927#note_214871708), which will make the lists take up the full board width. Just make a bookmark named "Wider board lists" with this as the link:
...
TODO: We need to create a Web IDE version of this report and link it in the category slack channel - there is [an issue for this](https://example_company.com/example_company-org/example_company/-/issues/452218).

In addition to the Velocity Board chrome extension, there is a [Workspace Iteration Planning Report](https://example_company-org.example_company.io/remote-development/remote-development-team-automation/workspaces-iteration-planning-report-latest.html) and [Web IDE Iteration Planning Report](https://example_company-org.example_company.io/remote-development/remote-development-team-automation/web-ide-iteration-planning-report-latest.html) which are automatically published every 6 hours.
...
The pipeline to generate it can also be run [directly from CI](https://example_company.com/example_company-org/remote-development/remote-development-team-automation/-/pipelines).
...
**TL;DR: Assign the [`Next 1-3 releases` milestone](https://example_company.com/groups/example_company-org/-/milestones/48#tab-issues) to all issues that should be refined and prioritized in the next Pre-IPM meeting(s).**

1. All higher-priority issues should be [contained in the category epic or one of its sub-epics](https://example_company.com/groups/example_company-org/-/epics/7419)
1. They should all have the `~(workspaces|webide)-workflow::unprioritized` label assigned.
1. All higher-priority issues should be assigned to the **[`Next 1-3 releases` milestone](https://example_company.com/groups/example_company-org/-/milestones/48#tab-issues)**.
...
1. Issues in `~(workspaces|webide)-workflow::unprioritized` MUST NOT be left without a release applied. This should be enforced by automation. Use [this issue search to identify Workspace issues which are missing a milestone](https://example_company.com/groups/example_company-org/-/issues/?sort=created_asc&state=opened&label_name%5B%5D=Category%3AWorkspaces&milestone_title=None&first_page_size=100)

**Explanation:**

This process identifies and validates higher-priority issues to be worked on in the upcoming releases. It is analogous to the [Validation Track in the Example Company Product Flow](/handbook/product-development-flow/#validation-track), and should consist of many of the same steps and processes.
...
See the [following discussion thread](https://example_company.com/example_company-org/create-stage/-/issues/13151#note_1417052962) for more context.
...
- **Async phase - Refinement:**
  - **Engineering Manager: Assign someone to refine each unrefined `(workspaces|webide)-workflow::unprioritized` issue which has the `%"Next 1-3 releases"` milestone using the [Workspace issue template](https://example_company.com/example_company-org/example_company/-/blob/master/.example_company/issue_templates/Workspaces%20Category%20-%20issue.md)**
  - **Refinement Assignees: Adequately refine the issue using the [Workspace issue template](https://example_company.com/example_company-org/example_company/-/blob/master/.example_company/issue_templates/Workspaces%20Category%20-%20issue.md), then apply the `refined` label**
...
1. The assignee should use the [Workspace issue template](https://example_company.com/example_company-org/example_company/-/blob/master/.example_company/issue_templates/Workspaces%20Category%20-%20issue.md) to refine the issue. The refinement should ensure
...
1. After an issue has been moved to the `~"(workspaces|webide)-workflow::prioritized"` list, it should be assigned to the **current `Iteration` of the `Iteration Cadence`**.
   ...
   Example Company Iteration feature which are incompatible with this process, and instead we rely on the
   [Example Company Velocity Board Chrome Extension](#example_company-velocity-board-chrome-extension) to automatically split the actual current iteration
   in the database into multiple future "virtual" iterations based on dynamic velocity calculations.

**Explanation:**

The "Pre-Iteration Planning" (Pre-IPM) meeting prepares for the Iteration Planning Meeting. It is analogous to ["backlog refinement"](/handbook/product-development-flow/#outcomes-and-activities-4) in the standard Example Company product development flow.
...
If a single piece of work spans multiple projects/repos, there should be a separate issue created for each MR in each repo. We cannot use the [tasks feature](https://docs.example_company.com/ee/user/tasks.html#set-task-weight), because boards cannot display individual
tasks.
...
The iteration cycle is one week long, and each category team maintains a separate dedicated board for tracking their progress - [the Workspace Iteration Planning board](https://example_company.com/groups/example_company-org/-/boards/5283620) and [the Web IDE Iteration Planning board](https://example_company.com/groups/example_company-org/-/boards/7440987).
...
At this point, once all prioritized issues have been estimated, with the use of the [Example Company Velocity Board Extension](https://example_company.com/cwoolley-example_company/gl-velocity-board-extension) we will be able to know how they break down into future iterations.
...
1. An issue is created using the [Workspace issue template](https://example_company.com/example_company-org/example_company/-/blob/master/.example_company/issue_templates/Workspaces%20Category%20-%20issue.md). The correct labels are automatically applied by the template,
...
- We also cannot rely on the Crosslinking Issues feature (https://docs.example_company.com/ee/user/project/issues/crosslinking_issues.html),
...
1. While the investigation issue work is ongoing, we create a temporary "placeholder" issue on our Iteration Board. This placeholder issue serves to capture the scope of the effort in our velocity-based capacity planning calculations while the investigation to break is down is still ongoing.
    ...
    1. The title format for the placeholder issue is `[Category] - Iteration Planning Placeholder for [description of work]`. Here's an example: https://example_company.com/example_company-org/example_company/-/issues/408093.
...
1. All work-related communication in Slack happens in the [#g_create_ide](https://example_company.slack.com/archives/CJS40SLJE) channel.
...
The Remote Development Group collates all video recordings related to the group and its team members in [a playlist](https://www.youtube.com/playlist?list=PL05JrBw4t0KrRQhnSYRNh1s1mEUypx67-) in the [Example Company Unfiltered](https://www.youtube.com/channel/UCMtZ0sc1HHNtGGWZFDRTh5A) YouTube channel.
...
- https://dashboards.example_company.net/d/stage-groups-detail-ide/stage-groups-ide-group-error-budget-detail?orgId=1
- https://dashboards.example_company.net/d/stage-groups-ide/stage-groups-ide-group-dashboard?orgId=1

## Automations

Automations should be set up via [triage-ops](https://example_company.com/example_company-org/quality/triage-ops/) if possible.

Other more complex automations may be set up in the
[Remote Development Team Automation project](https://example_company.com/example_company-org/remote-development/remote-development-team-automation).
...
We have the following automation goals for this Workflow. Unless otherwise noted, these rules are all defined in the [triage-ops `policies/groups/example_company-org/ide/remote-development-workflow.yml` config files](https://example_company.com/example_company-org/quality/triage-ops/-/tree/master/policies/groups/example_company-org/remote-development).

TODO: None of these are set up yet for the Web IDE category and `webide-workflow::*` labels yet - there is [an issue for this](https://example_company.com/example_company-org/example_company/-/issues/452218). These need to be set up, and the references to "`workspace`" in the table below should be updated accordingly to also include "`webide`".
...
| <a id="automation-02">02</a> | Ensure category issues show up on the Iteration Planning board | Issues in `~"Category:Workspace"` but no `~workspaces-workflow` label should be added to `~workspaces-workflow::unprioritized`. | [triage-ops link](https://example_company.com/example_company-org/quality/triage-ops/-/blob/master/policies/groups/example_company-org/remote-development/workspaces/workflow-02.yml) |
| <a id="automation-03">03</a> | Avoid premature assignment of specific milestones before an issue has gone through IPM, to avoid getting this warning: https://example_company.com/example_company-org/example_company/-/issues/411933#note_1419147845 | 1. Issues with a specific milestone but not in `~workspaces-workflow::prioritized` should have the milestone automatically set back to `%"Next 1-3 releases"`. <br> 2. Issues with a specific milestone but no weight should have the milestone automatically set back to `%"Next 1-3 releases"`. <br> 3. Issues with a specific milestone but no iteration should have the milestone automatically set back to `%"Next 1-3 releases"`. | [triage-ops link](https://example_company.com/example_company-org/quality/triage-ops/-/blob/master/policies/groups/example_company-org/remote-development/workspaces/workflow-03.yml) |
...
| <a id="automation-05">05</a> | Ensure closed issues are in the correct state | Every Issue in `~"Category:Workspace"` which is closed must have the `~workspaces-workflow::done` label applied. | [triage-ops link](https://example_company.com/example_company-org/quality/triage-ops/-/blob/master/policies/groups/example_company-org/remote-development/workspaces//workflow-05.yml) |
| <a id="automation-06">06</a> | Sync Workspace workflow and Example Company workflow labels | 1. Issues in `~workspaces-workflow::unprioritized` but with no Example Company workflow label should have `~"refined"` assigned. <br> 2. Unstarted issues in `~workspaces-workflow::prioritized` but with `~"refined"` assigned should get `~"workflow::ready` for development" assigned. | [triage-ops link](https://example_company.com/example_company-org/quality/triage-ops/-/blob/master/policies/groups/example_company-org/remote-development/workspaces/workflow-06.yml) |
...
| <a id="automation-10">10</a> | Ensure issues and MRs are 1-1 | - Every MR in `~"Category:Workspace"` must have the first line of the description matching: `Issue: <issue link>\n\n`. See <https://about.example_company.com/blog/2023/07/27/example_company-flow-duo/> <br> - Every Issue in `~"Category:Workspace"` must have the first line of the description matching: `"MR: <MR link>"` or `"MR: Pending"`. See <https://about.example_company.com/blog/2023/07/27/example_company-flow-duo/> | TODO: implement |
...
| <a id="automation-14">14</a> | Add an error comment if an issue is assigned to an iteration in the [RD cadence](https://example_company.com/groups/example_company-org/-/cadences/) but not assigned to `~Category:Workspace` | TODO: implement | |
