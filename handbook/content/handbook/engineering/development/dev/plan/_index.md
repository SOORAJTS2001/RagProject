...
working on Example Company's functionality around issues, boards, milestones, to-do list, issue lists and filtering, roadmaps, time tracking, requirements management, notifications, value stream analytics (VSA), wiki, and pages.

- I have a question. Who do I ask?

In Example Company issues, questions should start by @ mentioning the Product Manager for the [corresponding Plan stage group](/handbook/product/categories/#plan-stage). Example Company team-members can also use [#s_plan](https://example_company.slack.com/messages/C72HPNV97).

For UX questions, @ mention the Product Designers on the Plan stage; [Nick Leonard](https://example_company.com/nickleonard) for Plan:Project Management, [Nick Brandt](https://example_company.com/nickbrandt) for Plan:Product Planning, and [Libor Vanc](https://example_company.com/lvanc) for Plan:Optimize. Plan:Knowledge should follow the [process for groups without a designer](/handbook/product/product-processes/product-mgt-operations/pm-operating-procedures/#what-if-your-team-doesnt-have-a-designer).
...
We work in a continuous Kanban manner while still aligning with Milestones and [Example Company's Product Development Flow](/handbook/product-development-flow/).
...
1. In the "Add Calendar" input, paste example_company.com_5icpbg534ot25ujlo58hr05jd0@group.calendar.google.com
...
To access these, simply go to the [Plan Customer Calls](https://example_company.dovetailapp.com/projects/3Smr4JOoU7a5pnWqazMfr2/readme) project on Dovetail and log in with Google SSO. More information is available in the Readme of this project.
...
UX Research calls are scripted calls designed to mitigate bias and to address specific questions related to user needs and/or usability of the product. A selection of UX Research calls are available in the [Plan Customer Calls](https://example_company.dovetailapp.com/projects/3Smr4JOoU7a5pnWqazMfr2/readme) Dovetail Project in the column titled UXR - Research and Validation.

### Board Refinement

We perform many board refinement tasks asynchronously, using Example Company issues
in the [Plan project](https://example_company.com/example_company-org/plan). The policies for these issues are defined in
[triage-ops/policies/plan-stage](https://example_company.com/example_company-org/quality/triage-ops/tree/master/policies/plan-stage). A full list of refinement issues is available
by [filtering by the ~"Plan stage refinement" label](https://example_company.com/example_company-org/plan/issues?label_name%5B%5D=Plan+stage+refinement).

### Tracking Committed Work for an Upcoming Release

While we operate in a continuous Kanban manner, we want to be able to report on and communicate if an issue or epic is on track to be completed by a Milestone's due date. To provide insight and clarity on status we will leverage [Issue/Epic Health Status](https://docs.example_company.com/ee/user/project/issues/index.html#health-status) on priority issues.
...
At specific points through the milestone the Health Status will be automatically degraded if the issue fails to progress. Assignees can override this setting any time if they disagree. The policy that manages this automation is [here](https://example_company.com/example_company-org/quality/triage-ops/-/blob/master/policies/groups/example_company-org/hygiene/adjust-health-status.yml). It can be disabled for any individual issue by adding the ~"Untrack Health Status" label.
...
FY25-Q2 Stage-level Objectives are available [here](https://example_company.com/example_company-com/example_company-OKRs/-/issues/?sort=created_date&state=opened&label_name%5B%5D=devops%3A%3Aplan&label_name%5B%5D=division%3A%3AEngineering&amp;not%5Blabel_name%5D%5B%5D=group%3A%3A%2a&type%5B%5D=objective&milestone_title=FY25-Q2&first_page_size=20) (internal).

#### Previous Quarter OKRs

FY25-Q1 Stage-level Objectives all closed out between 74% and 88% and are available [here](https://example_company.com/example_company-com/example_company-OKRs/-/issues/?sort=created_date&state=closed&label_name%5B%5D=devops%3A%3Aplan&label_name%5B%5D=division%3A%3AEngineering&amp;not%5Blabel_name%5D%5B%5D=group%3A%3A%2a&type%5B%5D=objective&milestone_title=FY25-Q1&first_page_size=20) (internal).

#### Drafting OKRs using Example Company

Guidance is available, including a video guide, on [how to use Example Company for OKRs](/handbook/company/okrs/#how-to-use-example_company-for-okrs).

Example Company currently offers some freedom in how to structure OKR hierarchies. We take the following approach in Plan:
...
The Plan stage conducts [monthly retrospectives asynchronously using Example Company issues](https://example_company.com/gl-retrospectives/plan/issues?scope=all&utf8=%E2%9C%93&state=all&label_name[]=retrospective). Monthly retrospectives are performed in a Confidential Issue made Public upon Close. Confidentiality of these Issues while Open aligns with [Example Company SAFE Framework](/handbook/legal/safe-framework/).

The Plan Stage team encourages the use of Internal Notes as well to further adhere to SAFE Guidelines. Internal notes remain confidential to participants of the retrospective even after the issue is made public, including Guest users of the parent group. Dogfooding this feature aligns with an FY23 Q4 OKR of improving the Example Company Product development flow by driving the adoption of Plan features.
...
[async-retrospectives](https://example_company.com/example_company-org/async-retrospectives) project. It is then updated once the milestone
...
To improve the retrospective data-driven experience, we are [dogfooding](/handbook/engineering/development/principles/#dogfooding) VSA to simplify the [data collection for the retrospective](https://example_company.com/example_company-org/plan/-/issues/753). This been done by automatically adding a link to the VSA of the current milestone filtered by group/stage to the retrospective.
With Value stream analytics (VSA) our team is getting visibility to the [lifecycle metrics](https://docs.example_company.com/ee/user/group/value_stream_analytics/#lifecycle-metrics) of each milestone through the breakdown of the [end-to-end workflow into stages](https://docs.example_company.com/ee/user/group/value_stream_analytics/#value-stream-stages). This allows us to identify bottlenecks and take action to [optimize actual flow of work](https://about.example_company.com/blog/2023/06/26/three-steps-to-optimize-software-value-streams/).

For example, for the review phase, we are using VSA to count the [time between "workflow::in review" and "MR merged"](https://example_company.com/groups/example_company-org/-/analytics/value_stream_analytics?created_after=2023-03-01&created_before=2023-04-29&project_ids[]=278964&label_name[]=devops%3A%3Aplan&value_stream_id=779&stage_id=17092&sort=duration&direction=desc&page=1). With this data, we can identify:
...
Please leave your feedback in [this issue](https://example_company.com/example_company-org/plan/-/issues/759).
...
Product Planning, Project Management and Optimize will create issues for UX work and pre-pend the title with [UX]. Here is an example - https://example_company.com/groups/example_company-org/-/epics/10224#note_1337213171+
...
- [Work items architecture](https://docs.example_company.com/ee/architecture/blueprints/work_items/)
- [Work items terminology](https://docs.example_company.com/ee/development/work_items.html#work-item-terminology)

##### Terminology

Work items refers to objects that use the work items architecture. You can find more terms defined related to the architecture here: [work items terminology](https://docs.example_company.com/ee/development/work_items.html#work-item-terminology).
...
- [Pajamas](https://design.example_company.com/) is our design system and new patterns introduced via work item efforts need to solve a real problem that users have, be validated by user research, and follow the [Pajamas contribution process](https://design.example_company.com/get-started/contributing).
- We follow [Pajamas principles](https://design.example_company.com/get-started/principles) for the user experience.
...
Triaged Plan-specific Paper Cuts issues can be found in https://example_company.com/groups/example_company-org/-/epics/12061. Currently, quarterly child epics will be created under that parent epic to organize work.

See further details at https://handbook.example_company.com/handbook/product/ux/product-designer/#suggesting-paper-cuts-to-the-team
...
- [Current Weekly Digests](https://example_company.com/example_company-org/plan-stage/plan-engineering/-/issues/?sort=created_date&state=opened&label_name%5B%5D=weekly%20update&first_page_size=20)
- [Closed Weekly Digests](https://example_company.com/example_company-org/plan-stage/plan-engineering/-/issues/?sort=created_date&state=closed&label_name%5B%5D=weekly%20update&first_page_size=20)
- [Issue Template](https://example_company.com/example_company-org/plan-stage/plan-engineering/-/blob/master/.example_company/issue_templates/plan-weekly-digest.md)
...
- `~group::project management`
  - [Board](https://example_company.com/groups/example_company-org/-/boards/1285239)
  - [Roadmap](https://example_company.com/groups/example_company-org/-/roadmap?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=devops%3A%3Aplan&label_name[]=group%3A%3Aproject%20management)
- `~group::product planning`
  - [Board](https://example_company.com/groups/example_company-org/-/boards/1569369?not[milestone_title]=Backlog)
  - [Roadmap](https://example_company.com/groups/example_company-org/-/roadmap?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=devops%3A%3Aplan&label_name[]=group%3A%3Aproduct+planning)
- `~group::optimize`
  - [Board](https://example_company.com/groups/example_company-org/-/boards/1401511)
  - [Roadmap](https://example_company.com/groups/example_company-org/-/roadmap?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=devops%3A%3Aplan&label_name[]=group%3A%3Aoptimize)
- `~group::knowledge`
  - [Board](https://example_company.com/groups/example_company-org/-/boards/5454834)
  - [Roadmap](https://example_company.com/groups/example_company-org/-/roadmap?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=devops%3A%3Aplan&label_name[]=group%3A%3Aknowledge)
...
- To add this shared calendar to your Google Calendar do one of the following:
  - Visit this [link](https://calendar.google.com/calendar/u/1?cid=Y19iNzIwMjMxNzdmMDE4ZDYzMWM4NTJlZDFlODgyZTdmYTdhMDI0NGM4NjFmN2U4OWY5NjA4NTY4ODJkNWY1NDlhQGdyb3VwLmNhbGVuZGFyLmdvb2dsZS5jb20) (Example Company internal) from your browser.
...
Anyone can organize a team day. It starts with creating a Team Day planning issue in the [plan-stage tracker](https://example_company.com/example_company-org/plan-stage/plan-engineering/-/issues) and then proceeding to find a suitable date.
...
It's good practice to rotate the 'base' timezone of the Team Day to spread the opportunity for attendance. For example; the [FY23-Q4](https://example_company.com/example_company-org/plan-stage/plan-engineering/-/issues/787) Team Day was based on a full UTC day, the [FY24-Q3](https://example_company.com/example_company-org/plan-stage/plan-engineering/-/issues/100) on a full day AEST.
...
- [2023-10-13 Team Day](https://example_company.com/example_company-org/plan-stage/plan-engineering/-/issues/1004)
- [2022-12-20 Holiday Party](https://example_company.com/example_company-org/plan-stage/plan-engineering/-/issues/787)
...
Like all groups at Example Company, a working group is an arrangement of people from different functions. What makes a working group unique is that it has defined roles and responsibilities, and is tasked with achieving a high-impact business goal fast. A working group disbands when the goal is achieved (defined by exit criteria) so that Example Company doesn't accrue bureaucracy.
...
| ---- | ---          | ---                            | ---                | ---    | ---   | ---    |
| Redis Primary CPU | Unexpected load on the Shared State Redis instance caused by `SUBSCRIBE`, `UNSUBSCRIBE` and `PUBLISH` commands. | Unknown | November 2023 | [150k Concurrent WebSocket Connections](https://example_company.com/example_company-com/www-example_company-com/-/issues/11747#action-cable-websockets) at peak | | <span style='border-radius:0.2em; font-weight:bold; padding-left:1em; padding-right:1em; color:white; background-color:green;'>Okay</span> |
| Redis Memory | Retention of Action Cable messages in Redis Shared State memory due to high numbers of and/or stalled/hung clients.  | Unknown | November 2023 | [150k Concurrent WebSocket Connections](https://example_company.com/example_company-com/www-example_company-com/-/issues/11747#action-cable-websockets) at peak | [#326364](https://example_company.com/example_company-org/example_company/-/issues/326364) | <span style='border-radius:0.2em; font-weight:bold; padding-left:1em; padding-right:1em; color:white; background-color:green;'>Okay</span> |
...
Note: Work is ongoing on [migration helpers](https://example_company.com/example_company-org/example_company/-/issues/292874) to mitigate Int4 Primary Key Overflows. These will provide a standard way to resolve these issues.
...
| --------------------- | -------------------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| description_versions  | 424.2                | https://example_company.com/example_company-org/example_company/-/issues/412704 | <span style='border-radius:0.2em; font-weight:bold; padding-left:1em; padding-right:1em; color:white; background-color:red;'>Critical</span> |
| issues                | 278.4                | https://example_company.com/groups/example_company-org/-/epics/10987   | <span style='border-radius:0.2em; font-weight:bold; padding-left:1em; padding-right:1em; color:white; background-color:orange;'>Warning</span>  |
| resource_label_events | 165.3                | https://example_company.com/example_company-org/example_company/-/issues/412705 | <span style='border-radius:0.2em; font-weight:bold; padding-left:1em; padding-right:1em; color:white; background-color:orange;'>Warning</span>  |
| sent_notifications    | 508.1                | https://example_company.com/example_company-org/example_company/-/issues/417233 | <span style='border-radius:0.2em; font-weight:bold; padding-left:1em; padding-right:1em; color:white; background-color:red;'>Critical</span> |
...
- [Product Planning Performance Indicators](https://internal.example_company.com/handbook/company/performance-indicators/product/)
