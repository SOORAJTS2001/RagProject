...
Example Company's [Product Planning](/handbook/product/categories/#product-planning-group) category in the [Plan stage](/handbook/engineering/development/dev/plan/).
...
- Application dashboards; showing request throughput, latency, SQL query counts, cache hits, Sidekiq jobs for Web and API endpoints, git usage and error budgets. Also links to other useful resources in Kibana.
  - [Product Planning]
- The [Sitespeed Dashboard](https://dashboards.example_company.net/d/product-plan/product-performance-plan?orgId=1); showing the results of ongoing synthetic tests against representative product pages. Useful for identifying changes in page load time (TTFB), LCP, etc.

## OKRs

Beginning FY24, we have moved away from Ally.io and have been using Example Company to track OKRs. Please note that the links below are only accessible to Example Company team members.

### Active Quarter OKRs

The current quarter (FY25-Q3) OKRs are visible [here](https://example_company.com/example_company-com/example_company-OKRs/-/issues/?sort=title_asc&state=opened&label_name%5B%5D=devops%3A%3Aplan&label_name%5B%5D=group%3A%3Aproduct%20planning&first_page_size=100).

### Previous Quarter OKRs

All the previous quarter OKRs are available [here](https://example_company.com/example_company-com/example_company-OKRs/-/issues/?sort=title_asc&state=closed&label_name%5B%5D=devops%3A%3Aplan&label_name%5B%5D=group%3A%3Aproduct%20planning&first_page_size=100).
...
The week after a release of current milestone, planning issue for next milestone is created by automation and is available [here](https://example_company.com/example_company-org/plan-stage/product/-/issues/?sort=title_asc&state=opened&label_name%5B%5D=group%3A%3Aproduct%20planning&first_page_size=100).
Once the issue is created, Product Manager fills out initial information (i.e. broader theme for the milestone, product priorities, deliverable areas, etc.) within the issue description and refines the [Milestone Planning board](https://example_company.com/example_company-org/example_company/-/boards/7695201?not[label_name][]=product%20work&label_name[]=group%3A%3Aproduct%20planning&milestone_title=Started).
...
1. For efforts that are part of larger initiatives (like [Work Items](https://docs.example_company.com/ee/development/work_items.html)), architectural documents are kept up to date with larger decisions; for example, around API design or functionality.
...
1. Items that must be delivered in separate milestones are identified and prioritized first; such as migrations, security issues, and other [multi-version compatibility](https://docs.example_company.com/ee/development/multi_version_compatibility.html) issues.
...
[definition of done](https://docs.example_company.com/ee/development/contributing/merge_request_workflow.html#definition-of-done). This can become tricky when collaborating on a feature that is
behind a feature flag.

Since all feature flags start as disabled by default, we should aim to document the
feature as soon as it's safe for testing by users using the [feature flag template](https://docs.example_company.com/ee/development/documentation/feature_flags.html#features-disabled-by-default).
...
work proceeds. As the feature flag rollout proceeds, the [documentation should be updated](https://docs.example_company.com/ee/development/documentation/feature_flags.html#features-that-became-enabled-by-default).
...
The team [Build Board](https://example_company.com/groups/example_company-org/-/boards/1569369?label_name[]=devops%3A%3Aplan&label_name[]=group%3A%3Aproduct%20planning&milestone_title=Started) always shows work in the current
...
it, but please post in [#s_plan](https://example_company.slack.com/archives/s_plan) if that's the case, as this probably
...
1. We expect people to be [managers of one](/handbook/values/#efficiency), and we [use Example Company ourselves](/handbook/values/#collaboration). If you see something that you think
...
1. Share it in [#s_plan](https://example_company.slack.com/archives/s_plan) - if not even more widely (like in [#development](https://example_company.slack.com/archives/development)
   or [#backend](https://example_company.slack.com/archives/backend)).

## Useful links

- [:Plan:Product Planning](https://example_company.com/groups/example_company-org/-/boards/1569369?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=devops%3A%3Aplan&not[label_name][]=group%3A%3Aproject%20management]) - Apply a milestone filter to see work in the current release
...
- [Group Conversations](http://example_company-org.example_company.io/group-conversations/plan/) (archive; group conversations now happen at a the
