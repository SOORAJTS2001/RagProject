...
In today's competitive landscape, Example Company's success hinges on our ability to build and maintain high-performing teams capable of efficient and rapid execution.
...
We aim to create an environment where high-performing teams can thrive, innovate and execute efficiently, ultimately driving Example Company's competitive edge in the market.
...
Millions of customers use Example Company, so UX improvements can have a huge collective impact across all of these individuals. We support this effort both in the product development as well as in our architecture. This includes continued conversion of [Pajamas components](/handbook/engineering/ux/pajamas-design-system/) in order to continue to improve the performance experienced by users. 
...
1. General Information
   ...
   - Link to Example Company Profile
...
Welcome to Example Company! We are excited for you to join us.
...
[Link to dashboard](https://10az.online.tableau.com/#/site/example_company/workbooks/2203014/views)
...
At times when cross-functional, or cross-departmental architectural collaboration is needed, the [Example Company Architecture Evolution Workflow](/handbook/engineering/architecture/) should be followed.
...
At Example Company we value [freedom and responsibility over rigidity](/handbook/values/#freedom-and-responsibility-over-rigidity). However, there are some technical decisions that will require approval before moving forward. Those scenarios are outlined in our [required approvals](/handbook/engineering/development/required-approvals.html) section.
...
2. For business selected vendors that provide base images (RHEL's UBI8 for example), we need to wait for their patches, or need to log Deviation Request (DR) as viable resolutions. The [VulnMapper](https://example_company.com/example_company-com/gl-security/threatmanagement/vulnerability-management/vulnerability-management-internal/vulnmapper/-/tree/main), an automation developed by the Threat Management team, can create vendor dependency DRs to a large extent, but there are still cases that DR needs to be reported manually.
...
1. Personal Example Company board(s) if any
...
1. [Infradev triage](/handbook/engineering/workflow/#infradev)
    ...
    1. [Infradev board](https://example_company.com/groups/example_company-org/-/boards/1193197?label_name%5B%5D=infradev)
1. [Performance refinement](/handbook/engineering/workflow/#performance-refinement)
    ...
    1. [Performance board](https://example_company.com/groups/example_company-org/-/boards/1233204)
1. [Infrastructure Development Escalations](/handbook/engineering/workflow/#infradev)
    ...
    1. Sync discussions for infradev Issues are part of the [Example Company SaaS Weekly Meeting](/handbook/engineering/infrastructure/#example_company-saas-infrastructure)
...
1. [Review and approve security approvals](https://example_company.com/example_company-com/www-example_company-com/-/issues/12296) for the Example Company project when required and informing the security engineering team when a security risk is accepted rather than being resolved prior to approval.
...
1. [Engineering](https://example_company.com/example_company-com/www-example_company-com/-/issues/7253)
1. [Development](https://example_company.com/example_company-com/www-example_company-com/-/issues/7148)
...
1. [Example Company Roadmaps](https://docs.example_company.com/ee/user/group/roadmap/)
1. [Example Company Architecture Workflow](/handbook/engineering/architecture/workflow/)
...
The shared services and components below are extracted from the Example Company [product documentation](https://docs.example_company.com/ee/development/architecture.html).
...
| Example Company K8S Agent |  | Centralized with Specific Team | @nicholasklick | Configure |  |
| Example Company Pages |  | Centralized with Specific Team | @vshushlin | [Knowledge](/handbook/engineering/development/dev/plan/knowledge/) |  |
| Example Company Rails |  | Decentralized |  |  | DRI for each controller is determined by the feature category specified in the class. [app/controllers](https://example_company.com/example_company-org/example_company/-/tree/master/app/controllers) and [ee/app/controllers](https://example_company.com/example_company-org/example_company/-/tree/master/ee/app/controllers) |
| Example Company Shell |  | Centralized with Specific Team | @sean_carroll | [Create:Source Code](/handbook/engineering/development/dev/create/source-code-be/) | [Reference](/handbook/product/categories/#source-code-group-1) |
...
| Jaeger |  | Centralized with Specific Team | @dawsmith | Infrastructure:Observability | Observability team made the [initial implementation/deployment](https://example_company.com/groups/example_company-com/gl-infra/-/epics/210). |
...
|  | Example Company Product Features | Decentralized |  |  | Examples like feature specific schema changes and/or performance tuning, etc. |
...
| Redis |  | Decentralized |  |  | DRI is similar to Sidekiq which is determined by the feature category specified in the class. [app/workers](https://example_company.com/example_company-org/example_company/-/tree/master/app/workers) and [ee/app/workers](https://example_company.com/example_company-org/example_company/-/tree/master/ee/app/workers) |
| Sentry |  | Decentralized |  |  | DRI is similar to Example Company Rails which is determined by the feature category specified in the class. [app/controllers](https://example_company.com/example_company-org/example_company/-/tree/master/app/controllers) and [ee/app/controllers](https://example_company.com/example_company-org/example_company/-/tree/master/ee/app/controllers) |
| Sidekiq |  | Decentralized |  |  | DRI for each worker is determined by the feature category specified in the class. [app/workers](https://example_company.com/example_company-org/example_company/-/tree/master/app/workers) and [ee/app/workers](https://example_company.com/example_company-org/example_company/-/tree/master/ee/app/workers) |
...
For a list of resources and information on our Example Company Learn channel for Development, consult [this page](/handbook/engineering/development/training/).
...
1. Naming convention:
team@example_company.com, examples below -
    - Managers: configure-be@example_company.com includes all the engineers reporting to the Configure backend engineering manager.
    - Directors: ops-section@example_company.com includes all the engineers and managers reporting to the director of engineering, Ops.
    - VP of Development: development@example_company.com includes all engineers, managers, and directors reporting to the VP of Development.
1. Roll up:
...
- Reference documentation in your responses and make updates to Example Company documentation when needed.
- Refer to existing issues and epics to reiterate our transparency value and to invite participation from the customer.
- If you are unclear about the support-development collaboration process or workflow then please refer to the handbook page [how to use example_company.com to request help from the Example Company development team](/handbook/support/workflows/how-to-get-help.html#how-to-use-gitlabcom-to-formally-request-help-from-the-example_company-development-team)
...
- Requiring feature flag usage ([example](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/88298))
...
| ---------------- | --------------------------- |-----------------------------------------------------------------------------------------------------------------------|
| Database migrations, tooling, complex queries, metrics | impact to entire application<br/><br/>The database is a critical component where any severe degradation or outage leads to an S1 incident. | [Documentation](https://docs.example_company.com/ee/development/database_review.html#general-process)                          |
| Sidekiq changes (adding or removing workers, renaming queues, changing arguments, changing profile of work required)  | impact to multiple services<br/><br/>Sidekiq shards run groups of workers based on their profile of work, eg memory-bound. If a worker fails poorly, it has the potential to halt all work on that shard. | [Documentation](https://docs.example_company.com/ee/development/sidekiq/compatibility_across_updates.html) |
...
| [Create: Source Code product areas](/handbook/product/categories/features/#createsource-code-group) | high percentage of traffic share. Special attention should be paid to Protected Branches, CODEOWNERS, MR Approvals, Git LFS, Workhorse and the git over SSH / example_company-sshd interfaces. Please contact the EM (@sean_carroll) or PM (@tlinz) if you are unsure. | |
| Pipeline Execution product areas | high percentage of traffic share  | [Documentation](https://docs.example_company.com/ee/development/contributing/verify/)                                          |
...
| Runtime language updates | impacts to multiple services | [Ruby Upgrade Guidelines](https://docs.example_company.com/ee/development/ruby_upgrade.html#ruby-upgrade-guidelines)           |
| Application framework updates | impacts to multiple services | [Rails Upgrade Guidelines](https://docs.example_company.com/ee/development/rails_update.html)                                  |
...
| Team                                                                                                                                                           | Stake |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------| --- |
| [Create:Remote Development](/handbook/engineering/development/dev/create/remote-development)                                                                         | Owns the WebIDE ([maintainers](https://example_company-org.example_company.io/example_company-roulette/?currentProject=example_company-web-ide&mode=show&hidden=reviewer))  |
| [Editor Extensions](/handbook/engineering/development/dev/create/editor_extensions/)                                                   | Maintains the Example Company Workflow VS Code Extension ([maintainers](https://example_company-org.example_company.io/example_company-roulette/?currentProject=example_company-vscode-extension&mode=show&hidden=reviewer)), [Jetbrains](https://example_company.com/groups/example_company-org/-/epics/6349), [Neovim](https://example_company.com/groups/example_company-org/-/epics/10739), [Visual Studio](https://example_company.com/groups/example_company-org/-/epics/10668) extensions and the [language server](https://example_company.com/groups/example_company-org/-/epics/2431). Also contributes with UX improvements for Code Suggestions within Example Company Workflow. |
...
| [AI Framework](/handbook/engineering/development/data-science/ai-powered/ai-framework/)                                                           | Abstraction Layer for Example Company Chat, Code Suggestions and other AI capabilities |
| [Duo Chat](/handbook/engineering/development/data-science/ai-powered/duo-chat/)                                                           | Example Company Chat for VSCode and WebIDE|
...
- The customer will want to know when they can see the benefits of a change. They may not be familiar with Example Company practices for tracking and predicting due dates and milestones. Also, they may not be familiar with our workflows and associated labels nor the predictability of code review timelines, different timelines on releases to Example Company.com compared with releases for self-hosted customers and our use of feature flags.

```markdown
* Customers often don't rely on asynchronous communication at the level that Example Company does. Educate the customer on our practices and adapt to find a combined asynchronous and synchronous communication method and cadence that works for everyone.
...
* Consider utilizing a shared Slack channel to collaborate, adding the customers to our slack via "one Slack channel access requests".  [Example](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/16192)
...
* In the agenda for recurring meetings, track the items tracked by the customer in priority order at the top and review the status, next steps, customer DRI, and Example Company DRI for each.  Discuss in the meeting periodically.
Remind Example Company team members in Slack to update the status of items they are the DRI for before recurring meetings.
...
1. [Software Engineering at Google](https://example_company.com/example_company-com/book-clubs/-/issues/10)
...
- Credits for team members to buy Example Company swag.
...
- Create an issue in your team's issue tracker or [the Team Member Socials issue tracker](https://example_company.com/example_company-com/example_company-team-member-socials/-/issues) containing the following details:
...
* [Development department board](https://example_company.com/example_company-com/www-example_company-com/-/boards/1008667?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=Development%20Department)
* [Current OKR's](https://example_company.com/example_company-com/www-example_company-com/-/boards/1008667?scope=all&utf8=✓&state=opened&label_name[]=Development%20Department&label_name[]=OKR)
* Slack channel [#development](https://example_company.slack.com/messages/C02PF508L)
