...
Please see the [handbook usage page](../handbook-usage.md) for how and why we use the handbook, and other [docs pages](https://handbook.example_company.com/docs/) for further reference.

This page is intended to be complementary to [Using Example Company at Example Company](/handbook/people-group/using-example_company-at-example_company/#using-example_company-competency).
We suggest you complete the [Example Company 101 Tool Certification](/handbook/people-group/learning-and-development/example_company-101/) if you haven't already.
...
For how to format markdown in the handbook, see the [markdown style guide](https://handbook.example_company.com/docs/markdown-guide),
...
1. [Use the Web IDE to make changes](https://docs.example_company.com/ee/user/project/web_ide/#use-the-web-ide).
1. [Commit changes](https://docs.example_company.com/ee/user/project/web_ide/#commit-changes).
1. [Create a merge request](https://docs.example_company.com/ee/user/project/merge_requests/creating_merge_requests.html), [assign reviewers](/handbook/about/handbook-usage/#when-to-get-approval).
1. [Apply suggestions](https://docs.example_company.com/ee/user/project/merge_requests/reviews/suggestions.html#apply-suggestions) if any.
...
1. Navigate to the handbook page you want to edit. At the bottom, or on the right, click on the URL `Edit this page` to open the new Web IDE.
    - Alternatively, open the [public handbook](https://example_company.com/example_company-com/content-sites/handbook) project and select `Web IDE` from the edit actions. This requires you to navigate into the source tree where the handbook files are located.
...
1. Familiarize yourself with the [Web IDE](https://docs.example_company.com/ee/user/project/web_ide/):
...
1. Specify a new Git branch name (optional). Press `Enter` to continue. This action pushes the changes into the Git repository. The changes are persisted on the Example Company server and are immediately visible in the project view.
...
1. Check the pop-up at the bottom right, and click `Create Merge Request` to open the Example Company MR tab. Tip: The notification bell icon at the bottom brings back the notification, if accidentally dismissed.
...
1. Fill in the MR template, provide why the merge request is created, add labels (optionally using quick actions, `/label ~handbook`), assign yourself (`/assign me`), and set the branch to delete when merged. Optionally, use the [draft option](https://docs.example_company.com/ee/user/project/merge_requests/drafts.html) to prevent merging until ready.
    ...
    ![Example Company Merge Request view](/images/handbook/about/editing-handbook/practical_handbook_edits_web_ide_vs_code_gitlab_create_mr.png)
...
1. Make changes, commit and push them (follow [this issue](https://example_company.com/example_company-org/example_company/-/issues/383801#note_1214559713) for UX commit flows).
...
**Note:** The `Cmd+w` keyboard shortcut to close a file tab in the Web IDE gets overridden by the Chrome browser to close the browser tab. This is a [known problem](https://example_company.com/example_company-org/example_company/-/issues/386209), use this shortcut carefully.

### Video: Editing using the Web IDE and single file editor

Note: The video shows the old `www-example_company-com` project. The editing process, however, is the same.
...
In these videos, we run through the Example Company Handbook with experts, uncovering how to best use the handbook in our day-to-day work, and learning best-practices for handbook editing along the way. They are meant to be helpful to understand generally how Example Company works, but **the instructions cannot be followed exactly since the editor and repository in Example Company has changed**.
...
see the [handbook project readme](https://example_company.com/example_company-com/content-sites/handbook/-/tree/main) for instructions on how to set it up.

1. Set up an [SSH key](https://docs.example_company.com/ee/user/ssh.html) to use with Example Company, if you have not previously done so.
1. [Set up Git](https://docs.example_company.com/ee/example_company-basics/start-using-git.html) and clone the appropriate repository, such as the [public handbook](https://example_company.com/example_company-com/content-sites/handbook).
1. Use your editor of choice to make changes. If you use [Visual Studio Code](https://code.visualstudio.com/download) optionally along with the [Example Company Workflow extension for VS Code](https://docs.example_company.com/ee/editor_extensions/visual_studio_code/), you can follow the [Web IDE editing instructions](#use-the-web-ide-to-edit-the-handbook).
1. [Push the changes to Example Company](https://docs.example_company.com/ee/example_company-basics/start-using-git.html#send-changes-to-example_company).
...
If you recently created a merge request that was labeled as a Community contribution, you can fix this mislabeling issue going forward by updating the Example Company username in your personal entry in the team member directory to match the Example Company account you use for work.

Use the [team page editing instructions](edit-team-page.md) to find your team page entry file, and update the `example_company` attribute (typically found on line 10) to be an **exact match** for the Example Company.com username you use for work.

## Preview changes on Example Company
...
1. Look for the shortcode in the [shortcodes folder](https://example_company.com/example_company-com/content-sites/handbook/-/tree/main/layouts/shortcodes).
...
1. At the top of the shortcode file should be a comment on where to look for the relevant data file. Ensure you're using "Display source" to view the file.
   - For example, look in `https://example_company.com/example_company-com/www-example_company-com/-/tree/master/data/performance_indicators` for the `ux_department` file.
...
When clicking on `Edit this page` in the upper right corner on a handbook page in your browser, you might get a `404` error in the Example Company Web IDE.

As a team member, this problem can be related to an expired SAML session for your Example Company.com profile. In order to mitigate and solve the problem, click on `View page source` to trigger the SAML authentication with Okta again.

Alternatively, navigate into our Example Company.com profile into [your To-Do list](https://example_company.com/dashboard/todos), or try to open a confidential issue, to trigger the authentication.
...
For example: `Error: error building site: assemble: "/builds/example_company-com/content-sites/handbook/content/handbook/security/security-assurance/field-security/trust_center_guide.md:1:2": closing tag for shortcode 'details' does not match start tag`
...
If the problem was on the `main` branch, you may need to [rebase](https://docs.example_company.com/ee/user/project/quick_actions.html#issues-merge-requests-and-epics).

If you're unsure, you can [reach out for help](#need-help).

### Resolving linting errors

To the ensure consistency, quality and correctness of the Example Company Handbook we use various linting jobs that run as part of the pipeline. These jobs check that everything is as it should be, and if they detect something is wrong will cause the pipeline to fail.
...
However, the handbook (and some other projects) pull data from `www-example_company-com` `yml` files,
which are currently not fully checked by the pipeline.

If the default branch `main` (for public and internal handbook) or `master` (for `www-example_company-com`)
...
Take this [example error](https://example_company.com/example_company-com/content-sites/handbook/-/jobs/5968799321#L123):

```plain
Error: error building site: failed to render shortcode: "/builds/example_company-com/content-sites/handbook/content/handbook/product/ux/technical-writing/_index.md:126:1": failed to render shortcode "tech-writing": failed to process shortcode: "/builds/example_company-com/content-sites/handbook/layouts/shortcodes/tech-writing.html:16:28": execute of template failed: template: shortcodes/tech-writing.html:16:28: executing "shortcodes/tech-writing.html" at <ref page (printf "/handbook/product/categories#%s-section" $section)>: error calling ref: parse "/handbook/product/categories#%!s(<nil>)-section": invalid URL escape "%!s"
```

Following the error trace, notice that the last error with a full path and line number is:
`failed to process shortcode: "/builds/example_company-com/content-sites/handbook/layouts/shortcodes/tech-writing.html:16:28"`.

Looking [at the file](https://example_company.com/example_company-com/content-sites/handbook/-/blob/114d8f9bf00342360be14dce8cf6e55e1d8a6edd/layouts/shortcodes/tech-writing.html#L16),
...
From there, [line 11](https://example_company.com/example_company-com/content-sites/handbook/-/blob/114d8f9bf00342360be14dce8cf6e55e1d8a6edd/layouts/shortcodes/tech-writing.html#L11)
...
If you do not have a local build, you can still likely find it in the [www-example_company-com data folder](https://example_company.com/example_company-com/www-example_company-com/-/tree/master/data).
...
In this case, [an empty `section:` line](https://example_company.com/example_company-com/www-example_company-com/-/commit/17a5406b9a8fd33756cd5e0c4a2343ea2b4ab7a7)
was the issue.
...
In this case, [the handbook code was made more robust](https://example_company.com/example_company-com/content-sites/handbook/-/merge_requests/2820/diffs).
...
If you run into trouble editing the Example Company Handbook, help is available.

Team members, referred to as [MR Buddies](/handbook/people-group/general-onboarding/mr-buddies/), are available to help you create a merge request or debug any problems you might run into while updating the Example Company Handbook. Some common questions are covered in the videos in the [MR Buddies playlist](https://www.youtube.com/playlist?list=PL05JrBw4t0KrCVFOwSGW6M3k16yLtPO1M). Post your request with a link in the [mr-buddies](https://example_company.slack.com/archives/CLM8K5LF4/p1678812429884979) Slack channel.

For general questions about the handbook, post in the [handbook Slack channel](https://example_company.enterprise.slack.com/archives/C81PT2ALD).
