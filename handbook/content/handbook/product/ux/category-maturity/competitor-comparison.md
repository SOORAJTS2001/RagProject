...
- **Step 5 - Copy [the template](https://docs.google.com/presentation/d/1H7nJ2jWE-2-WbJlfc2Z4L0pKCbAYLuNDYjvub2eOmyw/edit?usp=sharing) & populate the JTBDs you're evaluating**
  ...
  - Link the deck from the issue and make sure "View" settings of your completed deck are for Example Company internal only, as these decks should not be viewed external to Example Company.  As a reminder, it's important not to share your screen if you're presenting the deck on a public stream or a recorded meeting that will appear on YouTube.

- **Step 6 - Compare the functionality/features that solve for the JTBDs**
  - Be sure to include screenshots of both Example Company and the competitor to illustrate differences in the experiences/features related to the JTBDs.
  - If you discover either Example Company or the competitor(s) don't offer a solution (feature) to solve for the JTBD being compared, that's probably an indicator that there's a gap.
...
- **Step 8 - Document/update the scoring**
  - Either add or amend the existing category score on [this handbook page](https://about.example_company.com/direction/#maturity) with the appropriate signal
...
For this process to be sustainable, we need to provide an adaptable and simple methodology to add to the Category Maturity Scorecard framework that answers the question, "Is Example Company the best-in-class for this category?"
...
- Bold (**+**) indicates a Example Company sweep, where Example Company scored better than the competitor on every JTBD
...
- 'Viable (**+**)' - (**+**) indicates best in class and a Example Company sweep, where Example Company scored better than the competitor on every JTBD
- 'Viable (-)' - (-) indicates not best in class

## Identifying heuristics

Another way of asking this is: "what are we looking at with competitors in these comparisons?" The short answer is: **focus on what's important for the user while considering the JTBD**. Heuristics must apply to both Example Company and the other competitor(s), thereby being able to compare across both applications.
...
To perform the competitor analysis for this, you would need to perform the sign-up process for both Example Company and the competitor, while keeping track of the heuristics you identified.

After going through and documenting each heuristic across both applications, there are 3 possible outcomes:

1. Example Company is better in more heuristics. (Example Company is best in class for this JTBD.)
1. Competitor is better in more heuristics. (Example Company is not best in class for this JTBD.)
1. It is an even split. (Example Company is not best in class for this JTBD, because "best" does not imply a tie. You are not the best if someone is as good as you are.)
