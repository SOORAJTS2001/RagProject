...
The process for creating and maintaining Example Company product documentation depends on whether the documentation is:
...
- [Changes outside a specific milestone](https://docs.example_company.com/ee/development/documentation/workflow.html): Usually not associated with a specific code change, is of lower priority, and
  is open to all Example Company contributors.

Documentation [is required](https://docs.example_company.com/ee/development/contributing/merge_request_workflow.html#definition-of-done)
...
- Required with the delivery of a feature for a specific milestone as part of the
  Example Company [definition of done](https://docs.example_company.com/ee/development/contributing/merge_request_workflow.html#definition-of-done).
...
For details, see [the guidelines](https://docs.example_company.com/ee/development/documentation/feature_flags.html).

The author of this MR, either a frontend or backend developer, should write the documentation.

{{% alert title="Note" color="primary" %}}
Community Contributors can ask for additional help from Example Company team members.
{{% /alert %}}
...
- Use the [Documentation guidelines](https://docs.example_company.com/ee/development/documentation/index.html),
  and other resources linked from there, including:
  - Documentation [Structure and template](https://docs.example_company.com/ee/development/documentation/site_architecture/folder_structure.html) page.
  - [Style Guide](https://docs.example_company.com/ee/development/documentation/styleguide/).
  - [Markdown Guide](https://docs.example_company.com/ee/user/markdown.html).
...
- If the feature has a feature flag, [follow the policy for documenting feature-flagged issues](https://docs.example_company.com/ee/development/documentation/feature_flags.html).
...
- The Technical Writer for the DevOps stage group, except in exceptional
  circumstances where a [post-merge review](https://docs.example_company.com/ee/development/documentation/workflow.html#post-merge-reviews)
  can be requested.
...
- Ensure the issue contains:
  ...
  - Overview, description, and use cases when applicable (as required by the
    [documentation structure and template](https://docs.example_company.com/ee/development/documentation/site_architecture/folder_structure.html).
...
- Assisting the developer and Product Manager with feature documentation
  delivery.
- Ensuring that issues and MRs are labeled appropriately, and that doc content has the correct [metadata](https://docs.example_company.com/ee/development/documentation/metadata.html).
...
- Updates the Technical Writing milestone plan. [Example](https://example_company.com/example_company-org/technical-writing/-/issues/521), created from the [issue template](https://example_company.com/example_company-org/technical-writing/-/blob/main/.example_company/issue_templates/tw-milestone-plan.md).
...
[**Feature Proposal** template](https://example_company.com/example_company-org/example_company/-/raw/master/.example_company/issue_templates/Feature%20proposal%20-%20detailed.md)
...
- Example Company.com users might try to use the feature before it's released, driving
  support tickets.
...
- A feature might show up in the Example Company.com user interface before any
...
- There are no timing issues connected to releases:
  ...
  - If the feature *just* makes it into a release, the documentation *just*
    makes it in too.
  - If a feature makes it to Example Company.com early, the documentation will be ready
...
- Maintainers are allowed to merge features with the documentation *as-is*,
  ...
  **documentation reviews must not be blockers**. Therefore, it's important to
  ...
  must create a [post-merge follow-up issue](https://docs.example_company.com/ee/development/documentation/workflow.html#post-merge-reviews),
...
When a new Example Company version is released, the Technical Writing team releases [version-specific published documentation](https://example_company.com/example_company-org/example_company-docs/-/blob/main/doc/releases.md).

## Documentation feedback and improvements

To make a documentation change that is not associated with a specific code change, the Technical Writing team encourages contributors to [start with an MR](/handbook/communication/) and follow the [documentation update procedures](https://docs.example_company.com/ee/development/documentation/workflow.html).

If you do start with an issue rather than an MR, use the [Documentation template](https://example_company.com/example_company-org/example_company/-/issues/new?issuable_template=Documentation). Labels should include:
...
- Labels:
  - `tw::triaged`
  - (Optional, if the issue is suitable for a community contributor) [`Seeking community contributions`](https://example_company.com/groups/example_company-org/-/issues/?sort=created_date&state=opened&label_name%5B%5D=Seeking%20community%20contributions&first_page_size=50), [`quick win`](https://example_company.com/groups/example_company-org/-/issues/?sort=created_date&state=opened&label_name%5B%5D=quick%20win&first_page_size=50)
...
- An issue triage board for each group that you are the assigned technical writer for. If you don't have an issue triage board for your group, set one up called `Docs only backlog triage - group name`. See an [example board](https://example_company.com/example_company-org/example_company/-/boards/5285286?not%5Blabel_name%5D%5B%5D=type%3A%3Afeature&amp;not%5Blabel_name%5D%5B%5D=type%3A%3Abug&label_name%5B%5D=documentation&label_name%5B%5D=group%3A%3Aproject%20management) for the `Project Management` group.
...
Specifically, we are working to implement [topic types](https://docs.example_company.com/ee/development/documentation/topic_types/).

We are currently tracking updates to the [**Use Example Company**](https://docs.example_company.com/ee/user/)
...
The Technical Writing team takes part in the [Example Company Hackathon](https://about.example_company.com/community/hackathon/)
...
1. Run Vale against the full docset. Go to the Example Company repo and run:
   `find doc -name '*.md' | sort | xargs vale --minAlertLevel suggestion --output line > ../results.txt`

1. Create issues. You have a few options:

   - Use a [script](https://example_company.com/example_company-org/example_company-docs/-/blob/main/scripts/create_issues.js) to create one issue for each markdown file listed in the Vale results.
     This script uses the [`Doc cleanup` issue template](https://example_company.com/example_company-org/example_company/-/blob/master/.example_company/issue_templates/Doc_cleanup.md).
   - Create issues one at a time by using the `Doc cleanup` issue template.
   - Create issues in bulk by using the [Issues API](https://docs.example_company.com/ee/api/issues.html#new-issue).
...
1. View the related issue. Ensure the user who authored the MR is the same user who asked to be assigned to the issue.
   - If the user is not listed in the issue, and another user has asked to work on the issue, do not merge the MR.
     Ask the MR author to find an issue that has not already been assigned or point them to [this page](https://docs.example_company.com/ee/development/contributing/).
...
[labels](https://docs.example_company.com/ee/user/project/labels.html) to describe those
...
contents of the [Issues workflow](https://example_company.com/example_company-org/example_company/blob/master/doc/development/contributing/issue_workflow.md)
page. To view a usage description of each of the labels in this section, see
[Labels](https://example_company.com/groups/example_company-org/-/labels?utf8=%E2%9C%93&subscribed=&search=docs%3A%3A)
in the Example Company interface.
...
- [`documentation`](https://example_company.com/groups/example_company-org/-/labels?utf8=%E2%9C%93&subscribed=&search=documentation) - Use this label if files in the `/doc` directory were added, changed, or removed.
- [`UI text`](https://example_company.com/groups/example_company-org/-/labels?utf8=%E2%9C%93&subscribed=&search=ui+text) - If an issue has this label:
  - If the issue describes a new feature, add the [`non-SUS`](https://example_company.com/groups/example_company-org/-/labels?subscribed=&search=non-sus) label and the `feature::addition` label.
...
- [`global nav item`](https://example_company.com/groups/example_company-org/-/labels?utf8=%E2%9C%93&subscribed=&search=global+nav+item)
- [`release post item`](https://example_company.com/groups/example_company-org/-/labels?utf8=%E2%9C%93&subscribed=&search=release+post+item)
- [`tw-style`](https://example_company.com/groups/example_company-org/-/labels?utf8=%E2%9C%93&subscribed=&search=tw-)
- [`tw-testing`](https://example_company.com/groups/example_company-org/-/labels?utf8=%E2%9C%93&subscribed=&search=tw-)
- [`development guidelines`](https://example_company.com/example_company-org/example_company/-/labels?utf8=%E2%9C%93&subscribed=&search=development+guidelines) - See the [Developer documentation](https://docs.example_company.com/ee/development/#development-guidelines-review) for information about updating pages in the `/developer` directory.
- [`docs-technical-debt`](https://example_company.com/groups/example_company-org/-/labels?utf8=%E2%9C%93&subscribed=&search=docs-technical-debt)
- `handbook`
- [`Technical Writing Leadership`](https://example_company.com/example_company-org/example_company/-/labels?subscribed=&search=tech+writing+leadership)
- [`docs-channel`](https://example_company.com/groups/example_company-org/-/labels?subscribed=&search=docs-channel) - Technical Writers use this label if they pick up an MR or issue from the `#docs` or `#tw-team` Slack channels, or from the roulette, *and* it's outside of their assigned groups. Usage of this label is *optional*.
- [`L10n-docs`](https://example_company.com/groups/example_company-org/-/labels?subscribed=&sort=relevance&search=L10n-documentation#) - Localization issue, MR, or epic that impacts the workflows of the Technical Writing team or the docs.example_company.com site and infrastructure. Can be used in combination with `Technical Writing` to show Tech Writing team input into a `docs-localization` item.
- [`Category:Docs Site`](https://example_company.com/groups/example_company-org/-/labels?subscribed=&sort=relevance&search=Category%3ADocs+Site) - Issues related to the Documentation website infrastructure or code. This is not needed for issues related to the documentation itself. Issues with this label are included on the [Docs Workflow issue board](https://example_company.com/groups/example_company-org/-/boards/4340643?label_name[]=Category%3ADocs%20Site).
...
[scoped labels](https://example_company.com/groups/example_company-org/-/labels?utf8=✓&subscribed=&search=docs%3A%3A)
...
[following labels](https://example_company.com/groups/example_company-org/-/labels?utf8=✓&subscribed=&search=tw%3A%3A)
