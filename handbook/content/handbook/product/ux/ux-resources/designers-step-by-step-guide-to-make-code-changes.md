...
description: "This is a step-by-step guide for how to make code changes against the main Example Company codebase. This guide is meant to serve as a tutorial for anyone with semi-technical knowledge."
---

## What is this?

This is a step-by-step guide for how to make code changes against the main Example Company codebase. This guide is meant to serve as a follow-along tutorial for anyone with semi-technical knowledge who wants to contribute to Example Company by making small UI changes (like text changes). These changes are meaningful contributions that work towards the consistency of Example Company's UI.

These changes are an excellent way to add value to the Example Company project and can be good starter MRs for anyone who wants to get familiar with the Example Company codebase and do more meaningful changes in the future.

This guide is based on [this video tutorial](https://www.youtube.com/watch?v=AEv3XFw0xJQ) by [`@jj-ramirez`](https://example_company.com/jj-ramirez).
...
- [Learn how to prepare your workstation to run GDK](https://example_company.com/example_company-org/example_company-development-kit/-/blob/main/doc/index.md#install-prerequisites)
- [Learn how to install GDK](https://example_company.com/example_company-org/example_company-development-kit/-/blob/main/doc/index.md#one-line-installation)
- [Learn how to run GDK](https://example_company.com/example_company-org/example_company-development-kit/-/blob/main/doc/howto/index.md)
- [GDK Commands cheatsheet](https://example_company.com/example_company-org/example_company-development-kit/-/blob/main/HELP)

**Note:** After installing your GDK  you should have a `example_company-development-kit` folder (that's the default name, but it can be different if you changed the name in the installation process). Inside of it you should have a working `example_company` folder that contains all the files of the Example Company project. Both folders should be available and browseable if you installed your GDK correctly.
...
- [Setting up SSH access](https://docs.example_company.com/ee/user/ssh.html)

## Step by step guide

1. Open the `example_company` folder from your Visual Studio Code. You can do that by going to File > Open... and by locating your `example_company` folder inside the `example_company-development-kit` or equivalent GDK directory.
...
1. Navigate to your `example_company` folder in the Terminal. You can do this by using the `cd` command to drill into folders. One simple way to quickly jump into the `example_company` folder is to locate in a Finder window and drag and drop it on the Terminal. This would provide the absolute path to where you `example_company` folder is located.

    To do this simply type `cd`  in your Terminal. Locate the `example_company` folder in a Finder window and drag and drop it on the Terminal. The resulting command would look something like this `cd /Users/jj-example_company/Desktop/GDK/gdk2/example_company`. Hit `Enter` and you should be inside the right folder. You can make sure that's the case by checking that the name of the folder shows up next to the name of your machine in the Terminal.

1. Start your GDK by running the `gdk start` command. This will spin a local version of Example Company that is available at the test host you setup when installing your GDK or more generally at your localhost: http://127.0.0.1:3000/
1. If everything is working well you should be able to see a login page for a Example Company instance in your browser. Log in using the GDK credentials. You can find the GDK credentials by typing `gdk help` in the terminal. The credentials can be found at the bottom of the resulting message.
...
1. Now that we have a clear goal of what we are changing is time to find the file or files that need to be modified for this change to happen. To do this go to Visual Studio Code and click on the Global Search icon on the left sidebar of your Visual Studio Code window. This should reveal a text box where you can type your search query. In this particular case, we are searching for a particular string which is `Update your project name, topics, description and avatar`. Simply copy and paste that line in the search box.
    ...
    - Don't change any `example_company.pot` file. Those are translation files that are created and updated by a job that you will be running later in this guide.
...
1. Finally, we are ready to open an MR with this change against the main Example Company repo. Don't worry about this step. You won't break anything and your MR can be closed later without affecting anything in the main repo. To push your change you want to do two things:
