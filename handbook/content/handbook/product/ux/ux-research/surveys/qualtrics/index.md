...
description: "How to use Qualtrics at Example Company to run surveys"
---
...
1. Logo
    - Example Company First Look Logo (stored in the UX Research & Product graphics library within Qualtrics)
    - Style: Banner
    - Placement: Left
    - Max height: 70px
    - Mobile scaling: 66%
...
When users sign-up to Example Company First Look, we automatically collect the following embedded data:

1. `Example Company user` - Whether or not the person is a Example Company user.
    - `Yes` - the person is a Example Company user.
    - `No` - the person is not a Example Company user.
...
1. `Team size` - How many people work within the user's team (Added August 2019)
    ...
    - `21-30 people`
    - `30+ people`
1. `SaaS (Example Company.com) package`
    - `Free`
    - `Bronze`
...
1. The contact frequency for Example Company First Look has been exhausted. By default, no member of Example Company First Look receives an email more than once a week or four times a month. If this is the case, you will need to either delay sending your survey (you may only need to delay sending your survey by a day - reach out to the UX Research Coordinator for confirmation) or source users outside of the Example Company First Look panel.
1. There aren't enough Example Company First Look members that match your sampling criteria.
...
## Distributing your survey to Example Company.com users

**Note: These instructions are only applicable to UX Researchers and Research Coordinators.**

There will be studies where you wish to target specific populations of users based on some criteria (for instance, users that have authored at least 5 merge requests in the last 30 days). If it's possible to generate a list of user IDs for a given criteria (typically from the [data warehouse](/handbook/enterprise-data/platform/edw/)), you can use that list to push those users contact info into Qualtrics and distribute a survey. **Note that this is limited to users of Example Company.com and does not include Self-Managed users.**

1. Generate your list of Example Company.com user IDs and put them into the first column of a Google Sheet. The first entry in the first column (A1) should be `id`, as this confirms that this is a list of ids that you want to use. **Note:** The Example Company.com user ID you specified in your Google Sheet will be included in the Qualtrics mailing list as embedded data. You can use this to associate any responses you get with your original user list.
...
1. Assure respondents as early as possible how their data will be kept secure (for example: data will be used internally at Example Company and research reports will not include customer names and/or contact information).
...
You may want to promote a survey using several methods, and keep track of which one performs the best (for example: Example Company social, ux team social, in a blog post, or in a banner within example_company.com). You can do this by [using query strings](https://www.qualtrics.com/support/survey-platform/survey-module/survey-flow/standard-elements/passing-information-through-query-strings/).
