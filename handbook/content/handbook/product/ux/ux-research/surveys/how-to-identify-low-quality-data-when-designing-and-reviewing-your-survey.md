...
* **Example:** A respondent states they only use Example Company, but mentions performing pull requests (GitHub terminology) in their role instead of merge requests (Example Company terminology). This feedback suggests they are not a Example Company user and could be removed from the dataset.
* **Example:** A respondent states they use Example Company and have experience doing merge requests in their groups and projects (Example Company specific terminology). This response is consistent with someone who has used Example Company, so they should remain in the data.
