...
The User Experience (UX) team has developed resources for usability and workflow testing using a 'sandbox' area provisioned by the Infrastructure team. This allows for tightly controlled testing on a real Example Company instance without the risk of critical data loss or a security breach to example_company.com. This page details the steps needed to access the sandbox and guidelines on how to structure a study within the sandbox.

## What's a UX Cloud Sandbox?

The UX Cloud Sandbox is a reserved instance of Example Company running in its own 'private' space. Essentially, it acts as a completely separate installation of Example Company, similar to that of a self-managed client. That space is currently at [https://ux.gitlabdemo.cloud/](https://ux.gitlabdemo.cloud/).

The UX team uses this space for usability testing, workflow studies, and usability benchmarking. The sandbox solves two problems: 1) it creates a safe environment for external participants to be able to share their screens without violating security or privacy concerns, and 2) it allows us to test Admin and Ultimate plan features not easily accessible through example_company.com.
...
**Note:** Not every type of data can be generated, especially in more complex setups. To get an idea of what is and isn't possible, check the [Example Company Ruby gem documentation](https://www.rubydoc.info/gems/example_company/Gitlab/Client).
...
1. Go to the [sample data scripts repository](https://example_company.com/leducmills/uxr-dummy-data-scripts/-/tree/master). This project has several ruby scripts that show how different types of data might be populated. Request access if needed.
...
1. If you don't have it installed already, install the ['Faker' Ruby gem](https://rubygems.org/gems/faker).
 `$ gem install example_company faker`
...
```ruby
#this sets up our connection to our example_company instance
...
If you do need MR, issue, and label data, you must repeat the import process for each session you wish to run by creating a new project, selecting import, importing from example_company export, and selecting your .tar.gz file for each upload.

### Option 2: Create a sample data request

If you would like a project (or projects) with sample data to be created for you, please create a new [UX Research issue](https://example_company.com/example_company-org/ux-research/-/issues/new#) using the 'Sample Data Request' template. **Note: It is easier for the team if you do not create your project before opening the issue, as it is easier to create and populate the project at the same time.**
...
Example defaults for the various content types can be found in [this sample project](https://example_company.com/ux-research-sandbox/examples/tanuki-inc-example-1).
...
- Mess with any projects, studies, or settings, unless you are absolutely sure you know what you're doing. If you have any doubts, please post to the [ux_research Slack channel](https://example_company.slack.com/archives/CMEERUCE4), or contact @leducmills directly on Slack.
