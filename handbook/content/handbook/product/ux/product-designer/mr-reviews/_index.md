...
We use Example Company Roulette to assign subject matter experts for MRs. Learn more about [MR reviews assignments](#how-to-assign-mr-reviews).
...
[Example Company Roulette](https://example_company-org.example_company.io/example_company-roulette/) assigns designers to stage group MRs. The design DRI acts as the reviewer for these MRs. If a stage group lacks a designer, MR reviews cannot be accommodated due to capacity issues.

### Community contributions

Community-submitted MRs are assigned to the design DRI of the affected group. If the group has no designer, `@pedroms` will review them. The Example Company Roulette suggests the correct designers automatically and generated a Slack message in the `#ux-community-contributions` channel.
...
Monitor MR review distribution using the [Example Company Review Workload Dashboard](https://example_company-org.example_company.io/example_company-roulette/?sortKey=stats.avg30&order=-1&hourFormat24=true&visible=reviewer%7CUX) and [Product Design MR review volume](/handbook/product/ux/performance-indicators/#product-design-mr-review-volume).

## Reviewing

Follow the [Code Review guidelines](https://docs.example_company.com/ee/development/code_review.html) (read in entirety). Exceptions to these guidelines are noted below.
...
- **SaaS-only features**: Run the GDK as a SaaS version. [Simulate SaaS in the GDK](https://docs.example_company.com/ee/development/ee_features.html#simulate-a-saas-instance).
- **Paid features**: Request a license via an [Access Request](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/new), using the GitLab_Team_Member_License_Request template. [Add the license to your instance](https://docs.example_company.com/ee/administration/license_file.html#add-your-license-file-during-installation).
  - You can also switch between CE and EE editions: [How to simulate a CE instance](https://docs.example_company.com/ee/development/ee_features.html#simulate-a-ce-instance-when-unlicensed).
- **Pipeline-related and Runner features**: Create or enable a runner to run a pipeline. [Create a runner in Gitpod](https://example_company.com/example_company-org/example_company-development-kit/-/blob/main/doc/howto/gitpod.md#enable-runners) or [GDK](https://example_company.com/example_company-org/example_company-development-kit/-/blob/main/doc/howto/runner.md).
- **Compliance**: To test [audit event streaming](https://docs.example_company.com/ee/administration/audit_event_streaming/) using a stream destination URL. Generate a temporary destination with [Pipedream](https://pipedream.com/).
- **Fulfillment**: Only Fulfillment Product Designers should review CustomersDot MRs.
  - [Set up CustomersDot locally](https://example_company.com/example_company-org/customers-example_company-com/-/tree/main#setup). If impractical, review screenshots and videos in the MR description or coordinate a demo with the engineer. For complex changes, keep the change behind a feature flag and review on staging post-merge.
- **Geo**:Install and configure two **GDKs** as Geo primary and secondary sites.
  - [Simple installation](https://example_company.com/example_company-org/example_company-development-kit/-/blob/main/doc/howto/geo.md).
...
- **Service Desk**: Set up `incoming_email`, `service_desk_email` and MailRoom. These MRs can't be reviewed on GitPod and need a working GDK. [GDK setup instructions](https://example_company.com/example_company-org/example_company-development-kit/-/blob/main/doc/howto/service_desk_mail_room.md). [Video walkthrough](https://youtu.be/SdqBOK43MlI).
- **Value Stream Analytics**: [Setup and seed data instructions](https://example_company.com/-/snippets/2169951/raw/main/blocks.md). [Request a developer license](/handbook/engineering/developer-onboarding/#working-on-example_company-ee-developer-licenses) as many VSA features require an EE license.
- **Product Analytics**: [GDK setup instructions](https://example_company.com/example_company-org/example_company-development-kit/-/blob/main/doc/howto/product_analytics.md). This process can only be done on a local version of the GDK, not on GitPod. Additionally, it requires Docker.
...
- **Use the checklist**
  - Follow the [design and UI changes checklist](https://docs.example_company.com/ee/development/contributing/design.html#checklist) to make sure all main aspects are covered.
...
- **Stick to UX requirements**
  ...
  - Use the [follow-ups checklist](https://docs.example_company.com/ee/development/contributing/design.html#follow-ups) to create issues for any further updates or missing elements.
- **Best practices for reviews**:
  - Refer to the best practices for [everyone](https://docs.example_company.com/ee/development/code_review.html#everyone) and [reviewers](https://docs.example_company.com/ee/development/code_review.html#reviewing-a-merge-request).
...
- **Commenting**:
  ...
  - For non-mandatory suggestions, mark them as (non-blocking) to indicate they can be resolved within the merge request or as a follow-up.
    - Try the [Chrome/Firefox add-on](https://example_company.com/conventionalcomments/conventional-comments-button) to apply [Conventional Comment](https://conventionalcomments.org/) prefixes.
- **Visual feedback**:
  ...
  - Highlight differences between the implementation and the expected result using a [Markdown table](https://docs.example_company.com/ee/user/markdown.html#tables). Use the template below:
      <details>
...
- **Provide constructive feedback**:
  ...
  - If there are concerns, consider:
    ...
    - Blocking the full release of a feature by creating an issue with a list of items that need addressing ([example](https://example_company.com/example_company-org/example_company/-/issues/398152)).
...
- Once confident the MR meets all requirements, [approve it](https://docs.example_company.com/ee/development/code_review.html#getting-your-merge-request-reviewed-approved-and-merged) by clicking the "Approve" button in the merge request widget.
- Follow the [responsibility of the reviewer](https://docs.example_company.com/ee/development/code_review.html#the-responsibility-of-the-reviewer) guidelines for handoff.
