---
title: Example Company Product Management
---
...
At Example Company, the PMs lead their specialization. That is, the Create PM decides what the Create group works on, for which release, and makes sure this furthers our goals. This includes bugs, features, and architectural changes.
...
In general terms, if you require something to happen with the product or if you need engineering staff for a particular change, you approach a PM. Preferably through creating an issue (the Example Company way), and mentioning them there.

In the same vein, PMs are required to ask for feedback from the stakeholder on particular changes. If a change will affect Example Company.com and its maintenance, a PM should proactively reach out to infrastructure engineers to help with the scope, design, and decisions regarding this change.

It is then up to the PM to weigh all these inputs and decide on a prioritization. It is to be expected that they are the best equipped to make this prioritization, while also keeping in mind all goals of Example Company.

## How to engage Product Managers

### Where to reach Product Managers

- **[Public Issue Tracker (for Product)](https://example_company.com/example_company-com/Product/issues)**; please use confidential issues for topics that should only be visible to team members at Example Company.
- **[Chat channel](https://example_company.slack.com/archives/product)**; please use the `#product` chat channel for questions that don't seem appropriate for the issue tracker.
...
All feedback must follow the [Example Company Community Code of Conduct](https://about.example_company.com/community/contribute/code-of-conduct/). Failure to do so will result in the issues or comment being deleted.

#### Customer feature requests

If a customer has a feature request that doesn't already exist, refer to the [process to create an issue](/handbook/product/product-processes/#issues) in the [example_company-org issue tracker](https://example_company.com/example_company-org/example_company/-/issues) and choose the [Feature Proposal](https://example_company.com/example_company-org/example_company/-/issues/new?issuable_template=Feature%20Proposal%20-%20lean) template, following the instructions and providing as much information as possible. Once you've created the issue,  make sure to add the appropriate labels for the [product stage and/or group](/handbook/product/categories/) (e.g `~"devops::plan"`) if known and add a comment tagging the appropriate Product Manager. If an issue already exists, make a comment on the issue with their information and use case.
...
You can copy/paste this to make sure you don't miss anything or [create a comment template](https://docs.example_company.com/ee/user/profile/comment_templates.html#create-comment-templates) for reusability:
...
- Subscription: ~"Example Company Ultimate" OR ~"Example Company Premium" OR ~"Example Company Free"
- Product: ~"self-managed" OR ~"example_company.com" OR ~"Example Company Dedicated"
- Link to request:
...
- [Customer Requested Issues (CSM)](https://10az.online.tableau.com/#/site/example_company/workbooks/2015827/views) for Sales and Customer Success
...
Customers that are assigned a Customer Success Manager typically have a [collaboration project](/handbook/customer-success/csm/customer-collaboration-project/) on Example Company.com, which is used to share information, document customer details, and track issues in a place that both the Example Company team and the customer's team can access.

Generally, CSMs maintain a main issue and/or enable the [CS-Tool - TAM issue tracker](https://example_company.com/example_company-com/cs-tools/example_company-cs-tools/tam-issue-tracking), which lists out all feature requests the customer is interested in with links to the public Example Company issue.

When a customer expresses interest in a feature, the CSM should capture that in the public Example Company issue, as well as add it as an entry in the main feature tracking issue of the [customer's collaboration project](/handbook/customer-success/csm/customer-collaboration-project/).
...
If there is a lot of discussion with the customer about a specific feature request, create an issue on the customer [collaboration project](/handbook/customer-success/csm/engagement/) about it and list that issue as a related issue on the main Example Company issue. This is another signal of the main product issue of customer interest and also allows discussion with the customer and internal Example Company team members about their needs and concerns.
...
- Product Manager prepares any additional questions they would like answered, beyond what is below.
  - What version of Example Company are you currently using? CE / Premium / Ultimate?
  - How are you currently doing source code management? Example Company merge requests or another tool? How about CI/CD?
...
In the event that a paid customer is willing to pay for us to develop a specific feature, we should still respond as above. It's great that they're willing to pay for it: that means they really need it. However, we will not make a custom version of Example Company; even example_company.com is running on Example Company Ultimate, and we move faster that way by minimizing technical complexity to determine features to follow after, it's a trade-off to make. This doesn't mean that "no" is always going to stay "no." We keep an open mind to improvements.

#### Example: Customer is looking for prescriptive advice for setting up Example Company

For example, they need support configuring a self-managed runner with a SaaS license.

If you need support with a specific customer and your Customer Success Manager is unable to configure what is being requested or you are being asked to provide very specific guidelines for use of Example Company, we suggest creating an issue using the [Product Support Request](https://example_company.com/example_company-com/Product/-/blob/main/.example_company/issue_templates/Product-Support-Request.md), and following the steps suggested in the issue.
...
Everything in Example Company should be fast and creating files falls under the repository, so you create an issue and make the PM aware of it by mentioning it.

The PM in turn will investigate whether this is a general problem or one specific to Example Company.com, in collaboration with infrastructure and others, and schedule any necessary changes for an upcoming release.

### Tableau

The Product team maintains a [Tableau dashboard](https://10az.online.tableau.com/#/site/example_company/workbooks/2015827/views) to aggregate issues and customer interest in those issues. Information is automatically gathered from [Example Company issues](https://example_company.com/example_company-org/example_company/issues) by scanning for Salesforce customer account links. A separate dashboard [Customer Requested Issues (CSM)](https://10az.online.tableau.com/#/site/example_company/workbooks/2015827/views) is maintained for Sales and CS to make the dashboard easily filterable by the Account Owner or CSM for example.
...
If your customer would like to report a bug, refer to the [example of how to express the customer's interest](/handbook/product/product-management/#customer-feature-requests) in an issue and use the [Bug](https://example_company.com/example_company-org/example_company/-/issues/new?issuable_template=Bug) template, following the instructions and the same steps as above.

You can refer to the following steps if a medium priority bug has become stale on the Product Management [triage board](https://example_company.com/groups/example_company-org/-/boards/1075672?&label_name[]=type::bug&label_name%5B%5D=customer). Another potentially helpful view is the [triage report label](https://example_company.com/example_company-org/example_company/-/issues?scope=all&utf8=%E2%9C%93&state=opened&search=triage+report).
...
The CSM team gets notified via Slack whenever a new issue is opened, and they will respond in the issue with specific customers when they are available. If you don't receive a response within a week (allowing the CSM to review with their customers), feel free to ping the <code>[@timtams](https://example_company.com/timtams)</code> group in the issue.
...
1. List the [product project's labels](https://example_company.com/example_company-com/Product/-/labels)
...
Example Company is designed and developed in a unique way.
...
should not be considered shipped, benefits of Example Company that we're not actively talking about might
as well not exist.
...
throughout Example Company's documentation and marketing materials. This means that on
release, [`features.yml`](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/features.yml) is updated, documentation is merged and deployed, and
any existing content is updated where necessary.

It's not acceptable to do this after the release. Example Company is very complex, and features
and functions are easily missed, even those that provide significant value to customers
(e.g. the many ways you can authenticate with Example Company).
...
It's important to keep [`features.yml`](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/features.yml) updated because there are a number of different pages (internal-facing and external-facing) that read from that file. These include:

**External**

- [Pricing](https://about.example_company.com/pricing/)
- [Features](https://about.example_company.com/features/)
- [Why Example Company Premium?](https://about.example_company.com/pricing/premium/)
- [Why Example Company Ultimate?](https://about.example_company.com/pricing/ultimate)
- [Feature Comparison](https://about.example_company.com/pricing/feature-comparison/)
- [DevOps Lifecycle](https://about.example_company.com/stages-devops-lifecycle/)
- [DevOps Tools Landscape](https://about.example_company.com/why-example_company/)

**Internal**

- [SaaS vs. Self-managed](https://about.example_company.com/features/)
- [Features by tier](/features/by-paid-tier/)

### Working with User Experience (UX)

The standard for working as a team at Example Company is the [Product Development Workflow](/handbook/product-development-flow/). Product Managers and Product Designers should work together as strategic counterparts to better understand the problem and discover user needs. The Product Designer and the Product Manager will pair to understand the target audience, their challenges when using a particular feature and then designing a solution that helps users solve them.
...
As the Example Company product matures, we know we must make important workflows easier to use through feedback-loop mechanisms as is captured in the "Improve" section of the [Product Development Flow](/handbook/product-development-flow/#outcomes-and-activities-7). We can use the [Category Maturity Scorecards](/handbook/product/ux/category-maturity/category-maturity-scorecards/) and [UX scorecards](/handbook/product/ux/ux-scorecards/) as mechanisms to provide insights into how might be able to improve these user workflows.
...
- PMs who need to create designs can request access to Figma by creating an [Access Request issue](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/new). **Note:** We cannot grant an Editor seat by requesting an upgrade within Figma. An access request issue is required.
- Review and follow the [Pajamas guidelines](https://design.example_company.com/).
...
Here are some practices for how PMs work with groups outside of Example Company.
...
[Community contributions](https://about.example_company.com/community/contribute/) are a critical
...
1. For contributions that impact user experience, following the [contribution guidelines](https://about.example_company.com/community/contribute/),
...
1. For contributions that are for features we do not want in the product (either because
   ...
   understands this feature is not being accepted by Example Company.
