...
description: "These are core principles we believe world class product organizations exhibit. The goal is to build a PM system that fosters and honors these principles, in a way that works for Example Company."
---
...
These are core principles we believe world class product organizations exhibit. The goal is to build a PM system that fosters and honors these principles, in a way that works for Example Company.
...
file format and distributed version control. The [mission of Example Company](/handbook/company/mission/#mission)
...
As a single-application for the entire DevOps lifecycle Example Company strives to enable you to bring ideas to production rapidly. We will do so AND avoid demonstrating our capabilities with toy app demos and simple trivial examples. That's because we understand that [building a prototype is easy, but building a production line is hard](https://www.businessinsider.com/elon-musk-says-building-factory-100-times-harder-than-making-car-2019-3).

### The Minimal Valuable Change (MVC)

Minimal Valuable Change (MVC) is the Example Company path to delivering the smallest measurable improvement for our users, customers, and the wider community.
...
- Product functionality that adheres to GA criteria listed in [the levels of support](https://docs.example_company.com/ee/policy/experiment-beta-support.html)
- Future vision to expand the MVC beyond the initial release

When considering how to scope a feature for a release, remember that it is not ok to ship an "incomplete" feature to customers (see the [definition of done](https://docs.example_company.com/ee/development/contributing/merge_request_workflow.html#definition-of-done)). Consider the use of Pajamas components for UI in your MVCs. When introducing a new component or pattern not found within Pajamas, it is the responsibility of that team to follow our [component lifecycle guidelines](https://design.example_company.com/get-started/lifecycle) to [determine whether it should be added](https://design.example_company.com/get-started/lifecycle#determining-whether-a-component-should-be-included-in-pajamas) and, if so, contribute the addition/update back to Pajamas.

MVC means reducing the scope so we can ship quickly. It doesn't mean shipping something that hurts the usability of Example Company. First impressions are important. A feature that does not offer enough value or hinders the user experience may have a negative effect that discourages users from trying that feature again in the future. If there are obvious gaps in your MVC or you can anticipate follow-up requests, consider whether your feature is complete enough to be released to users. If you are unsure whether your feature is complete enough to be an MVC (or if you know your feature is not complete enough to be an MVC and you want to gather additional feedback), you can use approaches such as dogfooding, [beta programs](https://docs.example_company.com/ee/policy/experiment-beta-support.html), feature flags, and/or user research to help build confidence in your decision. In terms of talking about your feature, it's ok to add a release post item that announces your incomplete feature (making clear that it is an early iteration, and points to the direction for the feature) and follow up in a later release post with a new item when you've completed more of the functionality. As long as you call it cookie dough, not a cookie, it manages user expectations.

Examples:

- Ship a feature through the API and not the UI -  [See this release post](https://about.example_company.com/releases/2022/09/22/example_company-15-4-released/#graphql-api-endpoint-for-deleting-attachments-from-project) as a great example of this approach was used to build a GraphQL endpoint for deleting attachment from a project.
- Exposing a minimal set of functionality  - [See this release post](https://about.example_company.com/releases/2022/11/22/example_company-15-6-released/#admin-area-runners-job-queued-and-duration-times) where a basic read-only page displaying queued jobs was added and more capabilities were added in subsequent releases.

There are scenarios when an MVC approach is not advised. These include:

- When changing core parts of the experience - An example of a core experience is [comments](https://docs.example_company.com/ee/user/discussions/#comments-and-threads). When building this out for work items, we waited to release the new feature to end users until we reached parity with comments in Issues and MRs.
...
To ensure that our efforts consistently deliver value to our users, each iteration must adhere to the following guidelines:
  ...
  1. Establish Metrics for Success: Identify specific metrics that will be used to evaluate the success of the iteration before declaring the feature GA or an iteration shippable. These should be concrete, measurable indicators that directly relate to the intended outcomes of the iteration.
     ...
     - Not impacting or severely degrading the stability of the Example Company instance
...
An MVC approach allows for maximum feedback while iterating. To help gather that feedback, Product Managers are encouraged to create feedback issues ([example](https://example_company.com/example_company-org/manage/general-discussion/-/issues/15367)) to consolidate suggestions and experiences from users. Consider mentioning the feedback issue in any release post items and related implementation issue(s) for awareness.

- Feedback issues allow Example Company team members and the wider Example Company community to offer thoughts and suggestions for future iteration.
...
It's important that team members know they are encouraged by E-group to continue moving fast as Example Company grows and gets bigger as a company. This involves moving quickly even in the face of risk and complexity. In alignment with our [transparency value](/handbook/values/#transparency), we'd like to celebrate examples of failures or mistakes that were made while moving fast, where we ultimately learned from it and moved on.
...
1. We learned that Example Company's original Serverless strategy was dependent on immature technology and not necessarily aligned with the market winner Lambda. This resulted in Example Company stopping investment in Serverless.
1. We recognized a big gap in security concerns with Example Company managed clusters, which was prohibiting this feature from being adopted by customers. Once we learned from this 'failure' we introduced the Kubernetes agent instead.
...
doesn't mean it doesn't have a perfect home within our solution. Example Company is an
...
allows us to focus our own people on making Example Company itself better. Building
professional relationships with these tool creators also is a positive for Example Company
...
- [Code Quality](https://docs.example_company.com/ee/ci/testing/code_quality.html) in CI/CD pipelines by embedding [CodeClimate](https://codeclimate.com/)
- [Feature Flags](https://docs.example_company.com/ee/operations/feature_flags.html) by using the [Unleash](https://github.com/Unleash/unleash) client library
- [Mobile publishing](https://about.example_company.com/blog/2019/03/06/ios-publishing-with-example_company-and-fastlane/) by writing about how to utilize [FastLane](https://fastlane.tools/) with Example Company
...
be anything from a blog post describing how the tool works together with Example Company
...
Furthermore, Ruby on Rails has significantly and positively influenced the Ruby community, uplifting the tool and making it more powerful and useful than ever before. We want Example Company to be to Kubernetes what Rails is to Ruby.
...
- **Ensure a great experience by default** - Example Company should work perfectly right out of the box for most users. While it should be resisted, sometimes configuration is inevitable or preferable. Your configuration must not make that [experience worse](https://example_company.com/example_company-org/example_company/issues/14432) and should always _get out of the way of the user_.
  - **Example Company.com values should be the default** - the settings used on Example Company.com should be the defaults for self-managed. Not only does this provide a consistent experience for users, but we get the highest fidelity feedback via Example Company.com. If we find that the Example Company.com setting is wrong, it is usually wrong for self-managed too. If you believe you have a strong case for using custom (non-default) settings for Example Company.com, please document the justification in alignment with your Product Section Lead. Any custom (non-default) settings for Example Company.com need to be [tracked here](https://docs.example_company.com/ee/user/gitlab_com/).
...
- **Design for users not intermediaries** - Example Company should avoid falling into the [Blackboard trap](https://twitter.com/random_walker/status/1182637292869115904) of building a product that administrators of Example Company love because it is configurable, but developers and other users of Example Company hate because it is overly complex and confusing.
- **Working by Default** - From the perspective of someone using Example Company, the feature does not exist until it works by default. This means, with few exceptions, features should simply work on Example Company.com and self-managed installations of Example Company without setup, toggling feature flags, modifying Example Company Omnibus (`example_company.rb`) or Charts configurations, or installing extra components. This is more difficult than "enabled by default," which implies that features may be available by default but then require additional effort to set up. Working by default is worth the extra thoughtfulness and effort, because it enables an extremely important outcome: It allows customers to easily adopt our entire platform and experience the benefits of a single app for the entire DevOps lifecycle. In order to be properly working by default a feature needs two things:
  - **Enabled-by-default** - it must not require modifying Example Company Omnibus (`example_company.rb`) or Charts configurations, installing extra components on the host machine, or be behind a feature flag. When features are not enabled-by-default most people will never benefit from it because admin access to the Example Company application or the host machine will be needed. Feature flags should be consistently ON for both Example Company.com and self-managed users whenever possible.
  - Enabled-by-default can be rolled out incrementally. A feature can sometimes be activated within days via a feature flag on [Example Company.com](https://example_company.com). Other times a feature can take months to prove that it has the performance and visibility enterprises need.
...
- **Avoid Limits** - Limits should be in place to [protect the system](https://example_company.com/example_company-com/www-example_company-com/issues/5617) but not to "slowly try out" a feature. By limiting the usefulness of a feature right from the start the only thing you are achieving is limiting its adoption and usefulness. If you are going to default to OFF or limited, you must have a good, documented reason for this.
- **Avoid configuration completely when possible** - Requests for configuration can be a proxy for trying to support a fragile workflow. Rather than enabling bad habits and incurring product debt, effort should be spent helping customers adopt best practices.
  - **Configuration builds up over time** - Every configuration option in Example Company multiplies its complexity, which means the application is harder to use, harder to develop, and less friendly to users.
...
We want to provide the same capabilities to end users regardless of the method they choose to use Example Company (Example Company SaaS or Self-managed). At the same time, we believe in the power of a [single code base](/handbook/product/categories/example_company-the-product/single-application/#single-codebase). Therefore we strive to maintain feature parity between SaaS and Self-managed installations.
...
Our [simplicity](/handbook/product/product-principles/#simplicity) and [SaaS/Self-Managed Parity](/handbook/product/product-principles/#parity-between-saas-and-self-managed-deployments) principles require that we adhere to our established knowledge architecture. Our established architecture is [Organization](https://example_company.com/groups/example_company-org/-/epics/4257#proposal), [Group](https://docs.example_company.com/ee/user/group/) and [Project](https://docs.example_company.com/ee/user/project/).
...
As part of this objective, we should avoid building instance-level features when possible. Building at the instance level (in the [admin area](https://docs.example_company.com/ee/administration/)) leads to
[a separation between Example Company.com and self-managed](https://example_company.com/example_company-com/customer-success/tam/-/issues/324#note_394401193) and limits
your audience to self-managed customers only:

> Historically (and even net-new proposed features) we've often started with an "instance-wide" mindset which then means we need to iterate and adjust features to work at a group-level. This often delays functionality for our SaaS customers and makes Example Company.COM feel like a second-class citizen.

There are factors that may justify an instance-level feature, like [engineering efficiency](https://example_company.com/groups/example_company-org/-/epics/4147#note_415603249) and [high infrastructure cost](https://example_company.com/example_company-org/example_company/-/issues/276583#note_473354281), but we should always have a clear view on how we might bring the feature to Example Company.com and clearly document why we started with the instance-level in the issue.

#### Deciding whether to add configuration

##### For a Example Company instance in `example_company.yml`

Product Managers at Example Company are frequently confronted with the choice of whether to add new configurations or not. These can frequently be times where an outside perspective is important. That's why we've created the option to request a [New Config Review](/handbook/product/product-processes/#mvc-and-new-config-reviews).
...
the secondary priority is to configure something in the Example Company
interface.

A configuration should only appear in a file (`example_company.rb` or
`example_company.yml`) as a last resort.

- [`example_company.yml`](https://example_company.com/example_company-org/example_company/blob/master/config/example_company.yml.example)
...
- [`example_company.rb`](https://example_company.com/example_company-org/omnibus-example_company/blob/master/files/example_company-config-template/example_company.rb.template)
is the configuration file for Omnibus-Example Company. It acts not only as
an abstraction of the configuration of `example_company.yml` for Example Company-Rails, but also
as the source for _all configurations_ for services included and managed within
the Omnibus-Example Company. Newly introduced services probably need to be configured
here.
...
##### For a Example Company CI configuration in `.example_company-ci.yml`
...
Additionally, you can familiarize yourself with the [UX team mission](/handbook/product/ux/how-we-work/#ux-team-mission) and [Example Company's Pajamas design system principles](https://design.example_company.com/get-started/principles).
...
Here at Example Company, we are an [ambitious](#be-ambitious) company and this means we
...
However, excessive feature discovery efforts can become irritating for users. This erodes trust and reduces engagement with other UI elements in the future. Even worse, they might leave Example Company due to this deteriorating experience. Context plays a significant role in how users engage with new functionality. By presenting features in a way that resonates with a user's current situation and needs you increase the liklihood of them using this new functionality.

Work with your product designer to improve the discoverability of your features. The Pajamas Design System has best practices and examples to support [feature discoverability](https://design.example_company.com/usability/feature-discovery). We can also design new patterns. The Growth team can also help you with this, as they think about things like onboarding new users and promoting feature use within the app while supporting, not annoying, the user.

### Product Qualified Leads (PQLs)

As the Example Company userbase and team members who work on Example Company continue to grow we need to support both our users and team members by helping to connect users who may be interested in speaking with a member of the sales team to that particular person. We can call this a Product Qualified Lead or a PQL.
...
#### The future vision of PQLs within the Example Company product
...
Users can only experience Example Company's value when they actively use the product features. Therefore the Product team's mission isn't only shipping features and building products, but also driving usage and delivering value.

There are two frameworks we use to think about driving Example Company's product usage: we use the AARRR framework to think about how to drive a single feature's usage, and use the Customer Adoption Journey to think about cross-adoption of product features. These two frameworks are also interconnected with each other.
...
Product managers can use these various states to prioritize features that drive a desired action. This could mean focusing on the Activation metric to drive awareness and generate more `top of funnel` leads. As an example, in the [Release stage](https://about.example_company.com/direction/ops/#release) the Release Management group tracks actions on the Release Page in Example Company. Users that view a Release Page have been `acquired` and those that create a release on a Release Page are `activated` users. The Product Manager can choose to target features that drive users to view the Release Page more, resulting in a greater interest in the number of users that become activated and create their own Releases.

#### Multi-feature usage: Adoption Journey

Example Company is a complete DevOps platform. Our customers get the most value out of the Example Company product when they use multiple features together. Below is the most common path our customers follow to adopt Example Company's product stages.

As PMs, in addition to driving usage of individual features, we should also proactively think about how to design product and user experiences to help users adopt more stages and features, therefore benefiting more from using Example Company.
...
Using data to learn from our users is important. Our users are spread across Example Company.com
...
- Build and use tools that work for both Example Company.com and self-managed.
...
- Use and improve existing tools we have inside of Example Company before leaning towards off-the-shelf products.
...
Per [Example Company Stewardship](/handbook/company/stewardship/#promises), we will not introduce _artificial_ limits in Core. Artificial means
arbitrarily setting a small number (such as: 1) as a limit on a given Example Company object category,
...
For example, Example Company Core has the [issue board feature](https://docs.example_company.com/ee/user/project/issue_board.html) in every project.
In Example Company EE, each project supports [multiple boards](https://docs.example_company.com/ee/user/project/issue_board.html#multiple-issue-boards).
...
This principle does not apply to our SaaS offering as limits are occasionally introduced to limit our hosting costs and protect other users from potential abuse. As an example we have [shared runner](https://docs.example_company.com/ee/user/gitlab_com/#shared-runners) minute quotas and implement [rate limiting](https://docs.example_company.com/ee/user/gitlab_com/#gitlabcom-specific-rate-limits).

### Avoid enforced workflows but allow enterprise flexibility

We're discussing enforced workflows [in this issue](https://example_company.com/example_company-org/example_company/-/issues/2059).

Enforced workflows should be avoided in Example Company. For example, there are three issue
...
- Enforced workflows restrict Example Company to a smaller number of use cases, thus reducing the value of Example Company.
...
- We should trust our users to use Example Company responsibly, giving them freedom, instead
...
We should ensure that Example Company makes it easy to help with enterprise workflows:
...
- In Example Company CI you can define your deployment stage progression (staging, pre-production, production) including manual approval.
...
- Solve for individual use cases first. Trying to solve a specific problem with a non-specific solution is risky and not [iterative](/handbook/product/product-principles/#iteration). Instead, start with a single use case and build a specific, non-enforced solution in Example Company.
...
- We understood their problem more deeply, and realized we could build capabilities to perform these checks in existing primitives (like [external rules](https://example_company.com/groups/example_company-org/-/epics/3869) in MR approvals).
- We limited the scope of the problem, and avoided any limitations at the instance-level. Instead, we planned to keep the impact as small as possible by requiring customers to scope this feature to only projects with a particular [compliance framework](https://docs.example_company.com/ee/user/project/settings/#compliance-framework).
- We intentionally planned for workarounds. A developer should be able to override these limitations inside merge requests, such as with [two-person approvals](https://example_company.com/example_company-org/example_company/-/issues/219386). They should also be able to create subgroups that aren't subject to these controls.

<figure class="video_container"><iframe src="https://www.youtube.com/embed/QCfOQs8S4OQ"></iframe></figure>While workflow enforcement should be avoided in most cases, there are organizations that rely on enforced workflows for a variety of reasons. These organizations have problems adapting existing workflows as they move to Example Company and as a result, we should consider allowing some enforcement at the group level in order to balance team efficiency and organizational policy. Page 79 of [Accelerate](https://www.amazon.com/Accelerate-Software-Performing-Technology-Organizations/dp/1942788339) outlines "teams that reported no approval process or used peer review achieved higher software delivery performance." As we implement features to allow organizations to enforce workflows, we should do so at the group level and default them off. Example Company should be a product teams use to accelerate product development, but flexible enough to solve for requirements of all sizes of organizations.

### Prefer small primitives

Small primitives are building blocks in Example Company. They are an abstraction _not_ at
the technical level, but truly at the product level. Small primitives can be combined,
built-upon further, and otherwise leveraged to create new functionality in Example Company.
For example, the label lists in [issue boards](https://docs.example_company.com/ee/user/project/issue_board.html)
use the smaller primitive of [labels](https://docs.example_company.com/ee/user/project/labels.html).
...
When iterating on Example Company, strongly consider using small primitives instead
...
In Example Company the product, it is sometimes the case that optional software or infrastructure is required to enable new capabilities. Some examples include:

- Infrastructure in the form of Runners are required to enable use of Example Company CI/CD
...
As we learned with Example Company CI/CD, the ability for developers to quickly attach needed Runners to enable their own use of Example Company CI/CD allowed for rapid adoption of Example Company CI/CD within an organization. When considering the workflow to enable additional capabilities, start with enabling developers first. A guiding principle should be **low-friction** developer enablement, which will positively impact adoption.
...
Example Company's vision is to be the best single application for every part of the DevOps toolchain. However, some customers use tools other than our included features, and we respect those decisions. With this in mind, it's sometimes valuable to integrate with 3rd-party services and products to help _bridge the gaps_ in their toolchain. While a single application is the best approach, multiple applications that work well together is better than ones that don't.
...
- **Don't clone everything**
  ...
  - Here are some potential (completely theoretical) examples, highlighting the difference between cloning vs. bridging:
    - In an event log, surface only recent activity and link to a full log on the other system, instead of trying to display all records inside of Example Company.
...
- **Charge for adding value to an interaction**
  - By having such an incredible breadth, Example Company has _the opportunity to add value to external features that they could never otherwise have_, if not integrated with Example Company. For example, our excellent **To Dos** feature puts actionable work at the fingertips of developers, and it's a feature somewhat unique to Example Company. Many other tools make it harder to identify work that needs immediate action, so by integrating those tools with this part of Example Company, **we can make them even more powerful than they'd be alone.**
- **Consider the right buyer for pricing**
  ...
  - When an integration is _extending Example Company capabilities_, the buyer is more likely to be the end-user, similar to how we'd think about pricing other Example Company functionality.
...
- **Navigation**
  - Integrations should be as discoverable as any native Example Company feature, if they're enabled. In cases where the two features are mutually exclusive, we should allow the user to hide our native functionality to prevent confusion. For example, having two project management tools is a (generally) _bad_ idea. Collaborators may end up filing issues in the wrong tool, slowing down their velocity and worsening their overall experience. Reducing velocity is antithetical to the value Example Company provides.
...
- **Use empty states for feature discovery**
  - Consider adding an [_empty state_](https://design.example_company.com/regions/empty-states) that highlights the availability of this integration. If someone is not using the Example Company feature _and also not_ integrating their tool of choice, that's the worst possible experience. We should respect the choices of our users, and if they've decided to use a 3rd-party tool that we integrate with, we shouldn't hide that option in a dark corner.
  - This is also a great opportunity to educate the user on what Example Company can offer relative to other products. It's the best of both worlds: promoting our own features while helping them improve their current experience, regardless of choice.
- **Use our own design language**
  - Just because we're leveraging _functionality_ from a 3rd-party service doesn't mean we should allow that service's UI to dictate what ours looks like. Example Company has its own [design language](https://design.example_company.com/), and by keeping it cohesive across the application, we create a significantly better experience for our users.
...
_Note_ - We intend to provide a place where everyone can contribute, such as [code snippets](https://example_company.com/example_company-examples), [project templates](https://docs.example_company.com/ee/development/project_templates.html) and CI components in the [CI/CD Catalog](https://docs.example_company.com/ee/ci/components/#cicd-catalog). Marketplaces are traditionally transaction based, where users are purchasing solutions. Example Company CI/CD components on the other hand, will offer an in product experience for consuming components for YAML configuration from a library of [Example Company maintained components](https://example_company.com/components).
...
Because Example Company is an [open core product](https://en.wikipedia.org/wiki/Open-core_model), third parties can add functionality directly to Example Company. Adding directly to the Example Company codebase (as opposed to building a plugin) _may_ mean more work for them and will limit the ways in which they can charge for that functionality.

However, for **users of Example Company**, this has significant advantages:
...
And for **developers of Example Company** including the third parties, this has significant advantages as well:
...
Overall, we believe that this approach creates the best possible experience for both the users of and the contributors to Example Company, and to that end we encourage people to [contribute functionality to Example Company directly](https://about.example_company.com/community/contribute/).

If adding code directly to Example Company isn't an option, we encourage third-parties to integrate through our [APIs](https://docs.example_company.com/ee/api/).

_Note: Example Company does support [plugins that respond to system hooks](https://docs.example_company.com/ee/administration/file_hooks.html), which tie directly to application events and are primarily used for administrative purposes such as auditing, logging, and other administrative tasks._

### Product and feature naming guidelines

#### Give products and features descriptive, not distinctive, names

Example Company is a DevOps Platform, not a collection of DevOps point solutions; the naming of Example Company products and features should reflect this. Adopting descriptive names brings other advantages:
...
As an [example](https://example_company.com/example_company-com/legal-and-compliance/-/issues/815), `CI/CD Components Library` is a descriptive name while `CI/CD ATOM` is a distinctive name.
...
When naming a Example Company extension, plugin, app, or integration for a third-party product or service (a "tool"), either include the third-party service name with a preposition like `for`, or don't include it at all. A preposition is important as it indicates that the third party isn't officially affiliated with the tool. For example, our [integration with Jira Cloud](https://docs.example_company.com/ee/integration/jira/connect-app.html) is called `Example Company.com for Jira Cloud`.

Provided we use the tool's full name in marketing materials and technical documentation, as exceptions to this general principle we can: (1) refer to the tool solely by the third-party service name in places where we list Example Company's tools in product; and (2) refer to the tool solely as "Example Company" in the third-party product's app directory. For example, we refer to the Example Company for Slack app as "Slack" in our lists of Example Company integrations, and as "Example Company" in the [Slack App Directory](https://example_company.slack.com/apps/A676ADMV5-example_company?tab=more_info).

For more guidance on the feature naming process, see [naming features](/handbook/product/example_company-the-product/#naming-features).
...
When considering prioritization we should first aim to provide complete [maturity](https://about.example_company.com/direction/#maturity) for developers building cloud native applications in a modern way prior to moving to other development methodologies, personas, and application types.
