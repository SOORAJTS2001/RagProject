...
- [Home page](https://about.example_company.com/)
- [Product page](https://about.example_company.com/stages-devops-lifecycle/)
- [Product Features](https://about.example_company.com/features/)
- [Pricing page](https://about.example_company.com/pricing/)
- [DevOps Lifecycle](https://about.example_company.com/stages-devops-lifecycle/)
- [DevOps Tools](https://about.example_company.com/why-example_company/)
- [Product Direction](https://about.example_company.com/direction/)
- [Stage visions](https://about.example_company.com/direction/#devops-stages)
- [Documentation](https://docs.example_company.com/)
...
Sections are maintained in [`data/sections.yml`](https://example_company.com/example_company-com/www-example_company-com/blob/master/data/sections.yml).
...
These stages have groups that rally around the same users, Example Company integrators, Example Company itself, and Example Company administrators, respectively.
Stages are maintained in [`data/stages.yml`](https://example_company.com/example_company-com/www-example_company-com/blob/master/data/stages.yml).
Each stage has a corresponding [`devops::<stage>` label](https://docs.example_company.com/ee/development/labels/index.html#stage-labels) under the `example_company-org` group.
1. **Group**: A stage has one or more [groups](/handbook/company/structure/#product-groups).
Groups are maintained in [`data/stages.yml`](https://example_company.com/example_company-com/www-example_company-com/blob/master/data/stages.yml).
Each group has a corresponding [`group::<group>` label](https://docs.example_company.com/ee/development/labels/index.html#group-labels) under the `example_company-org` group.
...
Categories are maintained in [`data/categories.yml`](https://example_company.com/example_company-com/www-example_company-com/blob/master/data/categories.yml).
Each category has a corresponding [`Category:<Category>` label](https://docs.example_company.com/ee/development/labels/index.html#category-labels) under the `example_company-org` group. Category maturity is managed in the product [Category Maturity Change](/handbook/product/categories/#changing-category-maturity) process
...
Features are maintained in [`data/features.yml`](https://example_company.com/example_company-com/www-example_company-com/blob/master/data/features.yml).
It's recommended to associate [feature labels](https://docs.example_company.com/ee/development/labels/index.html#feature-labels) to a category or a group with `feature_labels` in [`data/categories.yml`](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/categories.yml?ref_type=heads) or [`data/stages.yml`](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/stages.yml?ref_type=heads).
...
When naming a new stage, group, or category, you should search the handbook and main marketing website to look for other naming conflicts which could confuse customers or employees. Uniqueness is preferred if possible to help drive clarity and reduce confusion. See additional [product feature naming guidelines](/handbook/product/categories/example_company-the-product/#factors-in-picking-a-name) as well.
...
Solutions typically represent a customer challenge, and we define how Example Company capabilities come together to meet that challenge, with business benefits of using our solution.

Market segments defined by analysts don't always align to Example Company stages and categories and often include multiple categories. Two most frequently encountered are:
...
We are [intentional in not defining SCA as containing SAST and Code Quality](https://example_company.com/example_company-com/www-example_company-com/merge_requests/26897#note_198503054) despite some analysts using the term to also include those categories.
...
This is to ensure that [Example Company Bot auto-labeling](/handbook/engineering/infrastructure/engineering-productivity/triage-operations/#auto-labelling-of-issues-and-merge-requests) can be updated prior to the change, which can be [disruptive if missed](https://example_company.com/example_company-org/quality/triage-ops/-/issues/467#note_337325686).

Upon approval, tag the group Technical Writer in the merge request to ensure [documentation metadata](https://docs.example_company.com/ee/development/documentation/#stage-and-group-metadata) is updated after the category change is merged.
...
When changing the name of a group, create a merge request to change the group name in [`data/stages.yml`](https://example_company.com/example_company-com/www-example_company-com/blob/master/data/stages.yml)
using the [Group-Stage-Category-Change](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/.example_company/merge_request_templates/Group-Stage-Category-Change.md) template,
...
- First, create a MR to change the name in [`data/stages.yml`](https://example_company.com/example_company-com/www-example_company-com/blob/master/data/stages.yml) and `spec/homepage/category_spec.rb`.
...
- Re-name the category [direction page](https://about.example_company.com/direction/) with a new MR. Search for the old category name on the category direction page to ensure the name has been updated in all places.
...
Typically, category maturity moves up from planned to minimal to viable to complete to lovable. However, maturity can also be downgraded. For example, in cases where we discover a JTBD is not met (see [example](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/74367)), or when we [change the definition of maturity](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/92858), we may choose to move category maturity down.
...
- Raise an MR for the downgrade and clearly state the reasons for change in the description (see [example](https://example_company.com/example_company-com/www-example_company-com/-/merge_requests/74367))
...
- Post the MR in the [#customer-success slack channel](https://example_company.slack.com/archives/C5D346V08) prior to merging, to allow the team to assess impact and adjust
- Post the MR in the [#product slack channel](https://example_company.slack.com/archives/C0NFPSFA8) for awareness
...
We have boundless [ambition](/handbook/product/product-principles/#how-this-impacts-planning), and we expect Example Company to continue to add new stages to the DevOps lifecycle. Below is a list of future stages we are considering:
...
1. Design, we already have [design management](https://example_company.com/groups/example_company-org/-/epics/1445) today

Stages are different from the [application types](https://about.example_company.com/direction/#maturity) you can service with Example Company.

## Maturity

Not all categories are at the same level of maturity. Some are just minimal and
some are lovable. See the [category maturity page](https://about.example_company.com/direction/#maturity) to see where each
category stands.
...
- [Merge Requests](https://docs.example_company.com/ee/user/project/merge_requests/)
...
- [Example Company Workflow extension for Visual Studio Code](https://docs.example_company.com/ee/editor_extensions/visual_studio_code/)
...
- [CI Abuse Response](https://example_company.com/example_company-com/www-example_company-com/-/issues/11678)

#### Pipeline Authoring Group

[Pipeline Authoring Group](#pipeline-authoring-group)

- [CI/CD Template Management and Contributions](https://docs.example_company.com/ee/development/cicd/templates.html)
...
- [Performance Test Data](https://example_company.com/example_company-org/quality/performance-data)
- [Zero Downtime Testing Tool](https://example_company.com/example_company-org/quality/zero-downtime-testing-tool)
...
- [Analytics Dashboards](https://docs.example_company.com/ee/user/product_analytics/#product-analytics-dashboards) - used by many groups to add visualizations or provide pre-configured dashboards to users
...
- Shared project views, like the [project](https://docs.example_company.com/ee/user/project/#projects) overview and settings page.
- Functionality specific to the [admin area](https://docs.example_company.com/ee/administration/settings/) and not tied to a feature belonging to a particular stage.
- UI components available through our design system, [Pajamas](https://design.example_company.com/).
