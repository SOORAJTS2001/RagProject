...
You can find published dashboards in [Ad-hoc/Development/General](https://10az.online.tableau.com/#/site/example_company/projects/367746). These dashboards are safe for general use by the Tableau User population here at Example Company.
...
* Each KPI should have a standalone dashboard with a single chart representing the KPI and a text box with a link back to the handbook definition.
  ...
  * Add the `shared_dashboard`, `chart` , and the `dashboard` key-value pairs to the [corresponding Performance Indicators data file](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/performance_indicators/) under the `sisense_data` property
...
Taken from https://handbook.example_company.com/handbook/engineering/metrics/#metrics-list

### Merge Request Rate

Merge Request (MR) Rate is a measure of productivity and efficiency. The numerator is a collection of merge requests to a set of projects.  The denominator is a collection of people based on the `job title specialty` field in Workday. Both are tracked over time (usually monthly). The [stages.yml file](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/stages.yml) is the SSOT for group names. We rely on a mapping between the group name in this file to the job title specialty field in Workday. A mismatch between the two would cause team members or MRs not to be counted.
...
You can use [this MR Rate troubleshooting dashboard](https://app.periscopedata.com/app/example_company/1138219/MR-Rate-Troubleshooting) to check the number of team members that are counted each month. If the monthly team member count is less than expected, refer to the table to see which team member is missing.
...
* Team "Oranges" consists of 8 members as defined in the `job title specialty` field in Workday. In the past month, there were 20 merged MRs with the `group::Orange` label. Since the `job title specialty` does not match the group label (an extra `s` in the `job title specialty` field), we are unable to map the MRs back to the respective groups. We recommend either 1) updating the group label and [stages.yml file](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/stages.yml) or 2) updating the Workday value.

Group MR Rate can be found [here](https://10az.online.tableau.com/#/site/example_company/views/DevelopmentEmbeddedDashboard_17017859046500/DevelopmentEmbeddedDashboard) and filtered by group. It can also be queried by department or group using the following SQL:
...
1. `~"type::bug"`: Defects in shipped code and fixes for those defects. Read more about [features vs bugs](/handbook/product/product-processes/#issues).
   ...
   * `~"bug::availability"`: Defects related to Example Company SaaS availability. See [the definition](/handbook/engineering/infrastructure/engineering-productivity/issue-triage/#availability) for more guidance.
...
1. `~"type::feature"`: Effort to deliver new features, feature changes & improvements. Read more about [features vs bugs](/handbook/product/product-processes/#issues).
   * `~"feature::addition"`: The first MVC that gives Example Company users a foundation of new capabilities that were previously unavailable. Includes [good user value, usability, and tests](https://about.example_company.com/blog/2021/12/01/dont-confuse-these-twelve-shortcuts-with-iteration/). For example, these issues together helped create the first MVC for our Reviewer feature: [Create a Reviewers sidebar widget](https://example_company.com/example_company-org/example_company/-/issues/237921), [Show which reviewers have commented on an MR](https://example_company.com/example_company-org/example_company/-/issues/10294), [Add reviewers to MR form](https://example_company.com/example_company-org/example_company/-/issues/216054), [Increase MR counter on navbar when user is designated as reviewer](https://example_company.com/example_company-org/example_company/-/issues/255102)
   * `~"feature::enhancement"`: Subsequent user-facing improvements that refine the initial MVC by adding additional capabilities that make it more useful. Includes [good user value, usability, and tests](https://about.example_company.com/blog/2021/12/01/dont-confuse-these-twelve-shortcuts-with-iteration/). For example, these issues enhance the existing Reviewer feature: [Show MRs where user is designated as a Reviewer on the MR list page](https://example_company.com/example_company-org/example_company/-/issues/237922), [Display which approval rules match a given reviewer](https://example_company.com/example_company-org/example_company/-/issues/233736), [Add Reviewers quick action](https://example_company.com/example_company-org/example_company/-/issues/241244)
   * `~"feature::consolidation"`: Merging a feature into an existing feature for simplification. For example, [Workspace project: (Consolidate Groups and Projects)](https://example_company.com/groups/example_company-org/-/epics/6473) and [Combine Top Navigation Menu](https://example_company.com/groups/example_company-org/-/epics/5645) are good examples of such work.
   * _Note:_ **New** documentation or **new** feature flags that relate to `~"type::feature"` are considered `~"type::feature"`.
1. `~"type::maintenance"`: Upkeeping efforts & catch-up corrective improvements that are not Features nor Bugs. This includes removing or altering feature flags, removing whole features, merge requests that only include new specs or tests, documentation updates/changes (not including new documentation), restructuring for long-term maintainability, stability, reducing technical debt, improving the contributor experience, or upgrading dependencies and packages. For example: [Refactoring the CI YAML config parser](https://example_company.com/example_company-org/example_company-foss/-/issues/15060), [Updating software versions in our tech stack](https://example_company.com/example_company-org/ci-cd/codequality/-/issues/22), [Recalculating UUIDs for vulnerabilities using UUIDv5](https://example_company.com/example_company-org/example_company/-/issues/212322)
   ...
   * `~"maintenance::scalability"`: Modification to improve the scalability of Example Company that is not a user facing change or performance improvement. For example changing a column from INT to BIGINT.
   * `~"maintenance::usability"`: General improvements to product usability that are unrelated to feature prioritization. For example, [UI component](https://example_company.com/example_company-org/example_company/-/merge_requests/80903) and [UI text](https://example_company.com/example_company-org/example_company/-/merge_requests/80457) updates for consistency with Pajamas and [usability improvements](https://example_company.com/example_company-org/example_company/-/merge_requests/79888).
...
As of %16.7, with certain [internal projects](https://example_company.com/example_company-com/Product/-/issues/12779), type labels are locked once an associated MR is merged. More details can be found in the [feature implementation issue](https://example_company.com/example_company-org/example_company/-/issues/408676).
...
Community contributions are welcome in all areas of Example Company, so any type label can be set on `~"Community contribution"` merge requests.
...
`~"backstage"` was intended to be changes that were done to keep product development running smoothly. Over time, `~"backstage"` was also being used for pre-feature work and has become unclear and confusing. `~"backstage"` was deprecated as part of <https://example_company.com/example_company-org/quality/team-tasks/-/issues/488>. This will be removed with <https://example_company.com/example_company-org/quality/triage-ops/-/issues/483>.
...
* `~"type::maintenance"`
  ...
  * for addition or updates to specs for existing Example Company features
* `~"type::feature"`
...
The [triage bot](https://example_company.com/example_company-org/quality/triage-ops/) automatic labelling will not override existing labels.
...
The current list of projects are identified in the [`example_company-data/analytics`](https://example_company.com/example_company-data/analytics) project for the following system databases:

| System Database | File |
|-----------------|------|
| Example Company.com      | [`projects_part_of_product.csv`](https://example_company.com/example_company-data/analytics/blob/master/transform%2Fsnowflake-dbt%2Fdata%2Fprojects_part_of_product.csv) |
| ops.example_company.net  | [`projects_part_of_product_ops.csv`](https://example_company.com/example_company-data/analytics/blob/master/transform%2Fsnowflake-dbt%2Fdata%2Fprojects_part_of_product_ops.csv) |

### Updating the list of projects

The guidelines for inclusion in the `is_part_of_product` lists are:

* Included with the product as a part of a Example Company Omnibus or Cloud Native installation
* Support product development efforts
* Support the delivery and release process to Example Company SaaS

Follow these steps to request a new project to be tracked:

1. Create a merge request to the Example Company.com or ops.example_company.net project list from above.
