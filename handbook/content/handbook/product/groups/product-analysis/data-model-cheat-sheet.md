...
To collaborate on the content in this page, please either submit an MR (preferred) or start a discussion in [this Epic](https://example_company.com/groups/example_company-data/-/epics/771).
...
[Service Ping](https://docs.example_company.com/ee/development/internal_analytics/service_ping/) is Example Company's mechanism to collect data by generating a JSON payload of usage data every week to be sent to Example Company. It provides aggregated data to our Product, Customer Success, Support, and Sales teams to understand how Example Company is used. Service Ping is our only data source for understanding Self-Managed product behavior. Service Ping methodology allows us to protect our Self-Managed users' privacy by aggregating metrics at the installation level.

#### FAQs

> Is it possible to report at the namespace or user level using Service Ping data?

- Nope! As part of our [Commitment to Individual User Privacy](/handbook/product/analytics-instrumentation-guide/service-usage-data-commitment/), Example Company only collects usage metrics aggregated at the installation level.
...
- [Service Ping Overview](https://docs.example_company.com/ee/development/internal_analytics/service_ping/)
...
- [Categories of data collected: Subscription, Operational, Optional](/handbook/legal/privacy/customer-product-usage-information/index.html.md#what-are-the-data-collection-services-that-constitute-product-usage-data)
  - [Operational metrics](https://metrics.example_company.com/?q=operational)
  - [Optional metrics](https://metrics.example_company.com/?q=optional)
...
- The `milestone` field of the [metrics dictionary](https://metrics.example_company.com/) can also be used to identify the version when a metric was instrumented, but there a couple of limitations. First, many metrics are just labeled `< 13.9`, so there is a lack of more detail for older metrics. Second, metrics can be introduced on different versions for CE and EE, so `milestone` could be incorrect for one edition/distribution. For these reasons, we recommend using [common_mart_product.rpt_ping_metric_first_last_versions](https://dbt.gitlabdata.com/#!/model/model.gitlab_snowflake.rpt_ping_metric_first_last_versions) if you are looking to find out when a metric was instrumented.

</details>

### Example Company.com

Example Company.com (SaaS) is a single installation reporting a single ping within our Service Ping framework. In order to access more granular data by product tier, plan type, namespace, or user, we utilize the [Example Company.com Postgres database](/handbook/enterprise-data/programs/data-for-product-managers/#gitlabcom-postgres-database). This data source replicates any service ping events that create a [backend table](https://example_company.com/example_company-org/example_company/-/tree/master/db/docs).

#### FAQs

> Why don't all events for all Stages and Groups show up in our Example Company.com data?

- This is due to limitations in replicating Service Ping counters using the example_company.com db Postgres replica

> Is it possible to associate user level behavior in our Example Company.com data to our Snowplow events?

- No. Our snowplow user identifiers are anonymized, while our Example Company.com user identifiers are not. However, it is possible to join Snowplow and Example Company.com data at the namespace (group/project) level.

> I've heard there are some reliability issues with our Example Company.com data. How can I stay up to date on outages or known problems?

- [This Issue](https://example_company.com/example_company-data/analytics/-/issues/12921) documents all known problems with the Example Company.com replica.

#### Documentation

<details markdown="1"><summary>Click to expand</summary>

- [Data Guide for Product Managers documentation on Example Company.com postgres replica data](/handbook/enterprise-data/programs/data-for-product-managers/#gitlabcom-postgres-database)

- [DB docs](https://example_company.com/example_company-org/example_company/-/tree/master/db/docs) document which service ping metrics are replicated in a database. Click in to the .yml files for each table to access table specific descriptions.
...
- [Schema file](https://example_company.com/example_company-org/example_company/-/blob/master/db/structure.sql) containing SQL logic for the creation of each postgres table available in production.
...
- Example Company.com data sources are not exhaustive of all of the actions users can take within Example Company's SaaS offering.

</details>

### Snowplow

Snowplow is an open source event tracking tool that is used at Example Company to track Example Company.com front-end events like page views, CTA clicks, link clicks, etc. This data source does not collect identifiable user data to protect our user's privacy. Our Snowplow data source is how we implement and track experiments at Example Company.

#### FAQs

> Why doesn't the metric that my team implememented show up in the [metrics dictionary](https://metrics.example_company.com/snowplow)?

- In order to show up in the [metrics dictionary](https://metrics.example_company.com/snowplow), every event needs a [.yml file](https://example_company.com/example_company-org/example_company/-/tree/master/config/events). This will not happen automatically and should be created by the engineer that implements snowplow tracking.

> Why is the value for `gsc_namespace_id` null for some proportion of snowplow events?

- Engineers need to enable tracking for `gsc_namespace_id` when implementing new events. If tracking for `gsc_namespace_id` is already enabled and nulls are still occurring, the events may be triggered in a location within Example Company.com that is not specific to any one namespace like the ToDos page.

> What is the correct logic to identify events triggered in production environments?

- Apply the following logic `WHERE app_id IN ('example_company','gitlab_customers')`

> How should I interpret the `event_category` value in structured snowplow event payloads?

- The `event_category` value will be automatically populated according to [this codified logic](https://example_company.com/example_company-org/example_company/-/blob/master/app/helpers/application_helper.rb?ref_type=heads#L143-143) unless the engineer instrumenting the event overrides this logic, which is often the case for backend events. A great place to search for the meaning of these values is by key word searching in [this EE controllers repository]( https://example_company.com/example_company-org/example_company/-/tree/master/ee/app/controllers). Controllers outside of EE are also searchable and located [here](https://example_company.com/example_company-org/example_company/-/tree/master/app/controllers). Otherwise, you can reach out to the engineering slack channel for the team who instrumented the event of interest and ask for validation on the correct interpretation of your `event_category` value there.
...
- If you are wondering if Snowplow events are implemented in a certain area of the product, the [Snowplow Inspector](https://chrome.google.com/webstore/detail/snowplow-inspector/maplkdomeamdlngconidoefjpogkmljm?hl=en) is a good complimentary resource to the [Metrics Dicitonary](https://metrics.example_company.com/) which is not exhaustive. The Snowplow Inspector will not show server side events.

- We do not use snowplow on our self-managed instances, only on Example Company.com
...
This category of data models includes Example Company.com (SaaS) [namespaces](https://docs.example_company.com/ee/user/namespace/) (which include both projects and groups), their firmographic attributes, and individual members.

#### FAQs

> What is a namespace?

- Starting with the basics! Example Company has two categories of namespaces; groups and projects. In general, a namespace provides one place to organize your related projects. Read more [here](https://docs.example_company.com/ee/user/namespace/). Namespaces exist within Example Company SaaS and Self-Managed products, but to product the privacy of our Self-Managed users, we only collect identifiable namespace data for SaaS.
...
- No. Membership history at Example Company is not recorded in any data models.
...
- [This knowledge base page](https://docs.example_company.com/ee/topics/set_up_organization.html) covers an overview of namespaces, members and groups.

- [Member-specific knowledge base page](https://docs.example_company.com/ee/user/project/members/index.html) explaining direct and indirect memberships as well as shared group memberships.
...
| Schema | Table Name | Data Grain | Description | Notes |
| --- | --- | --- | --- | --- |
| common | [dim_namespace](https://dbt.gitlabdata.com/#!/model/model.gitlab_snowflake.dim_namespace) | `dim_namespace_id` | Dimension table that contains all Example Company.com namespaces and namespace attributes including plan. |  |
...
| common | [dim_user](https://dbt.gitlabdata.com/#!/model/model.gitlab_snowflake.dim_user) | `dim_user_id` | Dimension table that contains all Example Company.com Users. |  |
...
| legacy | [gitlab_dotcom_members](https://dbt.gitlabdata.com/#!/model/model.gitlab_snowflake.gitlab_dotcom_members) | `member_id`, `user_id` | Base model for Example Company.com members. | Only includes direct membership links. Used for invite related fields. |
...
- `member_count` fields found in any `common` models are not accurate and should not be used. Use `legacy.gitlab_dotcom_memberships` for any analyses intended to measure # members per namespace. [Here is the Issue](https://example_company.com/example_company-data/analytics/-/issues/12566) representing work to correct these accuracy problems.
...
- [How Example Company SaaS subscriptions work](/handbook/marketing/brand-and-product-marketing/product-and-solution-marketing/enablement/dotcom-subscriptions/) is a handbook page that covers SaaS subscriptions in depth.

- [Example Company Tiers](/handbook/marketing/brand-and-product-marketing/product-and-solution-marketing/tiers/) covers all SM and SaaS Tiers in the Example Company handbook.

- [This pricing page](https://about.example_company.com/pricing/) is our customer facing page covering all Example Company tiers.
...
- [SSOT Historical Namespace Subscriptions](https://example_company.com/example_company-data/analytics/-/issues/14401) is an Issue for Data to clarify and refactor models used for subscription and charge analysis.

- Findings from Issue intended to [record differences between using legacy.customers_db_charges_xf vs common.dim_order_hist for namespace paid conv. analysis](https://example_company.com/example_company-data/product-analytics/-/issues/820#note_1227834945).

- There are some known problems with our license to subscription mapping in SM and SaaS reporting. [Issue with more details](https://example_company.com/example_company-org/fulfillment-meta/-/issues/634).
