...
## How this page works
 ...
 In the spirit of "every can co-create", these procedures can be contributed by any one in the Product Division (or even Example Company!). The custodian of the procedures are Program Management. If you are interested in contributing to this page, please open a merge request and assign to Natalie Pinto, Example Company Handle `@natalie.pinto`.
...
As Example Company grows we will need to create new groups, stages, and categories. During this transition period
...
The label change affects both Issues and Merge Requests. The Stage and Group labels power the visualization and metrics in the [Quality](https://quality-dashboard.gitlap.com/groups/example_company-org) and [Example Company Insights](https://example_company.com/groups/example_company-org/-/insights) dashboards.
Please create a new issue in the [Triage Ops](https://example_company.com/example_company-org/quality/triage-ops) with the template [`label-change.md`](https://example_company.com/example_company-org/quality/triage-ops/blob/master/.example_company/issue_templates/label-change.md).
...
1. Ensure all issues remain labeled with the Stage name, like `devops::manage`
1. Ensure all issues also have a group label, like `Control` or `Framework`
1. Create a "Label change" issue in [Triage Ops](https://example_company.com/example_company-org/quality/triage-ops) listing affected label to have the change reflected retroactively in Engineering Dashboards.
1. Prior to the new groups being formed, the PM and EM prioritize the shared `devops::manage` backlog
...
The categories within a Stage will change over time, based on Example Company's direction and the market landscape. The groups within a Stage will need to
...
When Example Company decides to address additional needs within the single application, a new Stage may need to be created. For example, `Govern` may be created to
...
1. Once the issue has gotten approval from leadership and impacted parties have been made aware of the reallocation of team members, move the issue to a public repository (such as [Product](https://example_company.com/example_company-com/Product), [www-example_company-com](https://example_company.com/example_company-com/www-example_company-com), or a project specific to your team) in accordance with our [Transparency](/handbook/values/#transparency) value, making these changes transparent to all team members.
...
1. The PM doing the hand-off prepares an issue detailing process for change using the [template](https://example_company.com/example_company-com/Product/-/issues/new?issuable_template=Product_Transition).
...
1. Create an epic in the [Example Company-org group (This will link you to the epic creation page)](https://example_company.com/groups/example_company-org/-/epics/new) group describing the problem and the resolution criteria as briefly and precisely as possible.
...
- Ensuring that progress is being made toward mitigation and resolution. This may include coordinating problem-solving efforts across timezones so we can have Example Company team members working on the problem 24 hours a day.
...
1. Consider making the epic public. If the problem is related to security, ask `@example_company-com/gl-security/secops` to determine when the epic can be made public.
...
This [board](https://example_company.com/example_company-com/Product/-/boards/5874292) contains all proposed and active borrow requests.
...
#### Example Company.com In App messages (Broadcast Messaging)
...
There are two types of Broadcast Messages - banners and notifications. Currently banner notifications also send messages in Git responses, but this is [under review](https://example_company.com/groups/example_company-org/-/epics/10643).

##### How to use Broadcast Messaging

All broadcast messaging efforts must follow all guidelines in order to be deployed to Example Company.com. Create an issue in the Example Company.com/Product project using the [Broadcast Messaging template](https://example_company.com/example_company-com/Product/-/issues/new?issuable_template=PM-in-app-messaging-request) and assign it to `@justinfarris` & `@ampesta` for review. The [In App Messaging board](https://example_company.com/example_company-com/Product/-/boards/1889145?label_name%5B%5D=In-App-Messaging) is used to prioritize all messages in queue and in flight.

See [issue template](https://example_company.com/example_company-com/Product/-/blob/main/.example_company/issue_templates/PM-in-app-messaging-request.md) for usage guidelines. If the message requires a group to do work (for a banner message for instance) you may want to create an issue in the `example_company/example_company-org` project for better visibility.
