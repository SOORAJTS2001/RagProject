...
Introducing changes requires a number of steps, with some overlap, that should be completed in order. Example Company follows a dual-track [product development flow](/handbook/product-development-flow/) spanning product, engineering, UX, and quality. We [use Example Company to power product development flow](/handbook/product/product-processes/planning-with-example_company). When changes are released, we follow the [release post process](/handbook/marketing/blog/release-posts/#pm-contributors) to communicate externally about new capabilities.
...
- [**Product Function Issue Tracker**](https://example_company.com/example_company-com/Product/issues) - please use
confidential issues for topics that should only be visible to team members at Example Company.
- [**Product Function Example Company Group - @gl-product**](https://example_company.com/groups/gl-product/-/group_members)
- [**Product Management Example Company Group - @gl-product-pm**](https://example_company.com/groups/gl-product/-/group_members)
- **Bi-Weekly Meeting**
- [**Chat channel - #product**](https://example_company.slack.com/archives/product)
...
- Good: `We will next be working on [Detect and display code coverage reports on MR](https://example_company.com/example_company-org/example_company/-/issues/21549)`. OR `We will next be working on [example_company#21549](https://example_company.com/example_company-org/example_company/-/issues/21549) which will help developers view code coverage reports directly in Example Company instead of losing context by looking in another tool while reviewing an MR`.
...
that [we shipped in 8.12](https://about.example_company.com/releases/2016/09/22/example_company-8-12-released/#preventing-secrets-in-your-repositories-ee).
...
> Example Company now has a new push rule that will prevent commits with secrets from entering the repository.

- Describe how to use the feature in simple terms.

> Just check the checkbox in the repository settings, under push rules and
> Example Company will prevent common unsafe files such as .pem and .key from being committed.
...
- [Issue Board Work In Progress Limits](https://about.example_company.com/releases/2020/02/22/example_company-12-8-released/#issue-board-work-in-progress-limits)
- [Parent-Child Pipelines](https://about.example_company.com/releases/2020/01/22/example_company-12-7-released/#parent-child-pipelines)
- [Drag-and-drop Design badges](https://about.example_company.com/releases/2020/02/22/example_company-12-8-released/#drag-and-drop-design-badges)
- [Render charts in Example Company issues using a Grafana URL](https://about.example_company.com/releases/2019/11/22/example_company-12-5-released/)
...
As our documentation guidelines [actively encourage](https://docs.example_company.com/ee/development/documentation/styleguide/#videos) linking video content,
please consider following the [Documentation Style Guide section on language](https://docs.example_company.com/ee/development/documentation/styleguide/#language),
...
- [Example Company Unfiltered Speed Runs Playlist](https://www.youtube.com/playlist?list=PL05JrBw4t0KqSF4RAEzwC0qCBrM85OP7r)
...
- [Example Company for the Enterprise Demo](https://youtu.be/aIYLxMXQiLI)
...
can also use other environments as needed, such as Example Company provisioned on Kubernetes with GKE.
...
Example Company.com. If so, make sure to test out features as soon as possible so any new
issues can be addressed before final release. Also, take the FA cycle into
account when scheduling new milestone work.

If you are looking to test code that has not been merged to Example Company.com or is not yet
part of an RC, you can pull the branch down locally and test it using the [Example Company](https://example_company.com/example_company-org/example_company-development-kit)
Development Kit (GDK).
...
To enhance availability and performance of Example Company, configurable limits should be put in place for features which utilize storage, or scale in a manner which could impact performance. For example, we [limit the number of webhooks per project](https://docs.example_company.com/ee/user/project/integrations/webhooks.html), and we allow admins to set [rate limits on raw endpoints](https://docs.example_company.com/ee/administration/settings/rate_limits_on_raw_endpoints.html). These limits ensure more consistent performance, reduce the likelihood of outages, and offer admins tools to limit abuse or enforce specific standards. While these limits can be configurable, sensible default limits should be defined for our Example Company SaaS and Example Company dedicated offerings.

There is a guide about [developing application limits](https://docs.example_company.com/ee/development/application_limits.html) in the Example Company Docs.

##### When implementing application limits

Application limits should be enabled by default. If we are considering enabling or changing a limit, we should do the following (applies to Example Company.com and self-managed):

- **Evaluate if Example Company.com and self-managed should match** - Usually, the [limits on Example Company.com should be a good match for self-managed](/handbook/product/product-principles/#configuration-principles) but there may be situations in which limits on Example Company.com are not a good match for our self-managed customers. For example, the artifact expiration on Example Company.com was put in place to control costs and this did [not apply equally to self-managed customers](https://example_company.com/groups/example_company-org/-/epics/7097).
- **Evaluate the impact to current users** - How many users will be affected by this change? How much of an impact will they feel? If you need help pulling data for Example Company.com, [create an issue on the Infrastructure project](https://example_company.com/example_company-com/gl-infra/infrastructure/issues/)
...
- **Document the limits on docs.example_company.com**
  ...
  - Document the limit for customers on the [instance limits](https://example_company.com/help/administration/instance_limits.md) help page, ensuring the limit for example_company.com is specified. Include instructions on how the limit can be changed on self-managed instances.
  - If the limit is time based, link to that section from the [Rate limits page](https://docs.example_company.com/ee/security/rate_limits.html)
...
A good example where we've successfully evaluated data storage is our CI/CD Artifacts. We've set some sane default values for both [maximum artifact size](https://docs.example_company.com/ee/administration/settings/continuous_integration.html#maximum-artifacts-size) and for [default artifacts expiration](https://docs.example_company.com/ee/administration/settings/continuous_integration.html#default-artifacts-expiration), while making these both configurable for administrative users.

### Cross-stage features

See [this page](/handbook/product/cross-stage-features/) for details on working across stages at Example Company.

### Stages, Groups, and Categories

[Stages, groups, and categories](/handbook/product/product-processes/#stages-groups-and-categories) serve as a common framework for organizing and communicating the scope of Example Company.
...
1. Before:
   ...
   1. The facilitator shares the preparation work with the participants, preferably in the group's planning issue (see the template after this list and an [example](https://example_company.com/example_company-org/create-stage/-/issues/12980#note_834155059).
...
1. After:
   1. The facilitator [uploads the recording to Example Company Unfiltered](/handbook/marketing/marketing-operations/youtube/#uploading-conversations-to-youtube), sets its visibility (see [SAFE framework](/handbook/legal/safe-framework/)), adds to relevant playlists, and includes the URL of the mural and planning issue in the description.
   1. The facilitator shares the recording URL and voting results in the planning issue, preferably with a screenshot of the matrix and links to the highest voted issues (see an [example](https://example_company.com/example_company-org/create-stage/-/issues/12980#note_843540512).
...
**Reach** How many customers will benefit in the first quarter after launch? Data sources to estimate this might include qualitative customer interviews, customer requests through [Support/CS/Sales](https://10az.online.tableau.com/#/site/example_company/workbooks/2015827/views), upvotes on issues, surveys, etc.
...
**Impact** How much will this impact customers and Example Company? Impact could take the form of increased revenue, decreased risk, and/or decreased cost (for both customers and Example Company). This makes it possible to compare revenue generating opportunities vs. non-revenue generating opportunities. Potential for future impact should also be taken into account as well as the impact to the Example Company brand (for example unlocking free-to-paid conversion opportunities).
...
- Does it bring our [vision](https://about.example_company.com/direction/#vision) closer to reality?
- Does it help make our community safer through [moderation tools](https://example_company.com/example_company-org/example_company/issues/15326)?
...
[Customer Requested Issues (Product)](https://10az.online.tableau.com/#/site/example_company/workbooks/2015827/views) for product managers
[Customer Requested Issues (CSM)](https://10az.online.tableau.com/#/site/example_company/workbooks/2015827/views) for  Sales, CS and CSM
...
Example Company is open-source, encouraging and promoting a large ecosystem of contributors is critical to our success. When making prioritization decisions,
...
[reviewed and merged swiftly](https://docs.example_company.com/ee/development/contributing/#contribution-flow) allowing everyone
to contribute, including non-product teams at Example Company.

### SaaS-First Framework

The [SaaS-First product investment theme](https://about.example_company.com/direction/#saas-first) will put us in a better position to support our customer base who is expected to accelerate adoption of SaaS products in the coming years. Features will also end up more secure, resilient, performant, and scalable for our self-managed customers if initially built to the expectations of SaaS. Therefore, it is important for PMs to understand and prioritize needs related to the SaaS business. When prioritizing SaaS related issues, we follow the same [guidelines above](#prioritization). Within those guidelines there are a few areas that are especially important for PMs to focus on to ensure the success of our SaaS users.

#### Availability

Downtime of Example Company.com has a material impact on our customers. From a 2014 report [Gartner estimates](https://blogs.gartner.com/andrew-lerner/2014/07/16/the-cost-of-downtime/) that downtime costs companies on average "$5,600 per minute, which extrapolates to well over $300K per hour." Furthermore, SaaS downtime can severely disrupt the productivity of Example Company Inc since we rely heavily on Example Company.com to run our business. Finally, downtime can also lead to customer churn and damage to our reputation. Thus, it is crucial as a company we collectively work towards consistently maintaining our [99.95% SLA on Example Company.com](https://dashboards.example_company.net/d/general-slas/general-slas?orgId=1&from=1614038400000&to=1616630399000). There are a few things that PMs can do in partnership with their engineering team to help ensure overall Availability for Example Company.com.
...
While not required, PMs are encouraged to listen in on [Incident Management](/handbook/engineering/infrastructure/incident-management/#incident-management) calls for incidents related to their product areas to 1) build empathy with the SRE team by gaining insight into how they handle incidents 2) gain a better sense of the impact of the incident to their customer base, and 3) identify improvements to their product areas, whether technical or feature-related, that could have prevented the incident. PMs are not expected to be in the decision-making path on actions taken to resolve the incident. They are there to listen and learn rather than attempting to decide/influence the course of resolution. After incidents involving their product area, PMs are also encouraged to engage in the [Incident Review](/handbook/engineering/infrastructure/incident-review/), including attendance at the [Sync Incident Review](/handbook/engineering/infrastructure/incident-review/#synchronous-review-meeting-sessions) call if their incident is scheduled. PMs can periodically review incidents via the [Production Incident Board](https://example_company.com/example_company-com/gl-infra/production/-/boards/1717012?label_name%5B%5D=incident)

#### Enterprise Customer Needs

Enterprise customers interested in adopting SaaS may have common hard requirements to be able to use the product. For example, large enterprises may need certain security related features, such as Audit Logs, available before their security team will agree to the use of Example Company.com. This can also be about more than just features; it may include how and where we apply features so they can administrate their Example Company instance at enterprise-scale. For instance, permission management and shared configurations are best implemented top-down first instead of Project-up to meet the requirements of large organizations who may have 100s or 1000s of projects and only a small handful of people to perform these system-wide administrative tasks. In order to encourage more Enterprise adoption of Example Company.com, prioritize these common "hard-blockers" to adoption over "nice to have" features. PMs can use customer interviews to hone in on which issues are hard blockers to adopting SaaS vs more "nice to have" features that can be delivered later.

To track hard adoption blockers, use the ~"Example Company.com Enterprise Readiness" label within the [Example Company-Org](https://example_company.com/groups/example_company-org/-/boards/2037713?label_name%5B%5D=Example Company.com%20Enterprise%20Readiness) and [Example Company-com](https://example_company.com/groups/example_company-com/-/boards/2037722?label_name%5B%5D=Example Company.com%20Enterprise%20Readiness) groups.

#### SaaS Features

There are a few special considerations when it comes to delivering features for SaaS. In order to achieve [parity between SaaS and Self-managed installations](/handbook/product/product-principles/#parity-between-saas-and-self-managed-deployments) PMs should prioritize efforts to eliminate existing feature gaps that exist [across the two installations](https://about.example_company.com/features/). Additionally, new features should ship for SaaS and self-managed at the same time. Features should be implemented at the group level first, before being implemented at the instance level, so that they will work across [both self-managed and SaaS](https://about.example_company.com/direction/manage/#gitlabcom). Finally, in order for new features to be adequately monitored, they should include [appropriate logging and observability](https://example_company.com/example_company-com/gl-infra/readiness/blob/master/.example_company/issue_templates/production_readiness.md#monitoring-and-alerts), which makes troubleshooting much easier.

### Working with Your Group

As a product manager, you will be assigned as the [stable counterpart](/handbook/company/structure/#specialists-experts-and-mentors) to a single [group](/handbook/company/structure/#groups). At Example Company we abide by
...
clear, sequenced list of next priorities. This list should be represented as an [issue board](https://about.example_company.com/stages-devops-lifecycle/issueboard/)
...
- [Customer Commitments](https://example_company.com/groups/example_company-org/-/labels?utf8=%E2%9C%93&subscribed=&search=planning+priority)
- Infrastructure projects with IACV driver impact or those that result in significant cost savings for example_company.com
...
- [Feature usage](https://redash.example_company.com/dashboard/feature-adoption)
- [EE usage](https://version.example_company.com/): dev.example_company.org account
- [Grafana](https://dashboards.example_company.net): Google example_company.com account
- [Kibana](https://log.gprd.example_company.net): dev.example_company.org account
- [S3stat](https://www.s3stat.com): Example Company 1Password account
- [Sentry](https://sentry.example_company.net): dev.example_company.org account
...
- Improving Competitiveness against [alternative DevOps tools](https://about.example_company.com/why-example_company/) - Leads to increased Stages Per user, and sales as they add to our "Increase Operational Efficiency"
...
Section leaders are responsible for maintaining Direction pages that lay out the strategy and plan for their respective section and stages. The direction pages should include topics outlined in this [template](https://example_company.com/example_company-com/www-example_company-com/blob/master/doc/templates/product/section_direction_template.html.md).
...
We use this [category strategy template](https://example_company.com/example_company-com/www-example_company-com/blob/master/doc/templates/product/category_direction_template.html.md)
...
1. [Software Supply Chain Security Direction](https://about.example_company.com/direction/supply-chain/)
1. [AutoDevOps Direction](https://about.example_company.com/direction/delivery/auto_devops/)
1. [Monorepo Product Direction](https://about.example_company.com/direction/monorepos/)
1. [Versioned Dependencies Direction](https://about.example_company.com/direction/versioned-dependencies/)
1. [Customizable Dashboards Direction](https://about.example_company.com/direction/customizable-dashboards/)

The steps for creating and managing a cross-section or stage direction are:

1. Create a direction page merge request adding the direction page to the [Example Company direction directory](https://example_company.com/example_company-com/www-example_company-com/-/tree/master/source/direction)
...
1. Add [CODEOWNERS](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/.example_company/CODEOWNERS) by adding an entry with the direction page link and the page DRI Example Company Handle.
...
For each category, we recommend tracking the improvements required to advance to the next level of [maturity](https://internal.example_company.com/handbook/product/investment/maturity/). You are welcome to track maturity plans either with `~maturity::...` labels or maturity issues.

Maturity plans are highly encouraged - but not required - for non-marketing categories.

#### Planning and OKRs

[Example Company users quarterly OKRs](/handbook/company/okrs/) that cascade into [Product OKRs](/handbook/product/product-okrs/) and product group OKRs.
...
You can read more about the OKR process at Example Company at the two links shared above.
...
- [Verify::Pipeline Execution](https://example_company.com/example_company-org/ci-cd/pipeline-execution/-/blob/main/.example_company/issue_templates/milestone_planning.md)
- [Verify::Testing](https://example_company.com/example_company-org/ci-cd/testing-group/-/blob/master/.example_company/issue_templates/ReleasePlan.md)
- [Monitor::Respond](https://example_company.com/example_company-org/monitor/respond/-/blob/master/.example_company/issue_templates/planning-issue.md)
- [Create::Code Review](https://example_company.com/example_company-org/create-stage/-/blob/master/.example_company/issue_templates/code-review-planning.md)
- [Create::Editor](https://example_company.com/example_company-org/create-stage/-/blob/master/.example_company/issue_templates/editor-planning.md)
- [Data Stores::Database](https://example_company.com/example_company-org/database-team/team-tasks/-/blob/master/.example_company/issue_templates/Planning.md)
...
Communicating clearly changing priorities might encourage the community to contribute the issue to Example Company.
...
Our [design system](https://design.example_company.com) provides the means to work
...
- [WebIDE](https://example_company.com/example_company-org/example_company/-/issues/385787)
- [Fonts](https://example_company.com/example_company-org/example_company/-/issues/386205)
- [`master` -> `main`](https://example_company.com/example_company-org/example_company/-/issues/221164)
...
1. Leverage [internal comments](https://docs.example_company.com/ee/user/discussions/#add-an-internal-note) as needed if customer details are being shared
...
- Consider a quick POC that can be enabled for small portion of our user base, especially on Example Company.com. An example of this was search, where it was originally enabled just for a few groups to start, then slowly rolled out.
...
- If your team needs to do repetitive work on behalf of customers, partners, or other Example Company teams, consider using a framework approach so that dependent teams can self-serve.

### Community participation

Engaging directly with the community of users is an important part of a PM's job. We
encourage participation and active response alongside Example Company's [Developer Relations team](/handbook/marketing/developer-relations/).

#### Conferences

A general list of conferences the company is participating in can be found on our
[corporate marketing](https://example_company.com/example_company-com/marketing/corporate_marketing/corporate-marketing/-/issues?label_name%5B%5D=Corporate+Event&sort=due_date) project.
...
Examples of stakeholders include Leadership, Sales, Marketing, Customer Support, and Customer Success. You may have stakeholders in any area of Example Company depending on your focus area and the specific issue. Stakeholders are also present outside of Example Company, for example, when a feature is being developed for a specific customer or set of customers. If you're not sure who the stakeholder is to collaborate with or keep informed, visit [product sections, stages, groups, and categories](/handbook/product/categories/).
...
at Example Company. To ensure this is possible, and facilitate collaboration, you should
...
collaborate. Some sections and teams use the [scheduled Direction Update issue template](https://example_company.com/example_company-com/Product/blob/master/templates/DIRECTION-UPDATES.md.erb) to
...
- Engage with users in epics, issues, and merge requests on Example Company.com.
- [Meet with customers directly.](#customer-meetings)
- Participate in the [CAB](/handbook/marketing/brand-and-product-marketing/product-and-solution-marketing/customer-advocacy/#example_company-devops-customer-advisory-board).
- Talk with Example Company team-members using Example Company.
- Talk with Example Company team-members in [group conversations](/handbook/company/group-conversations/).
- Talk with other PMs and [Product leadership](/handbook/product/product-leaders/product-leadership/) to align your stage's product direction with the rest of Example Company.
...
- Seek feedback from the [CAB](/handbook/marketing/brand-and-product-marketing/product-and-solution-marketing/customer-advocacy/#example_company-devops-customer-advisory-board) once every six months.
...
provided to Example Company.

**During the meeting**, spend most of your time listening and obtaining information.
It's not your job to sell Example Company, but it should be obvious when it's the time
...
- [Create an interview snapshot](https://example_company.com/example_company-com/user-interviews/-/issues/new?issuable_template=user-interview)
summarizing the meeting in the [example_company-com/user-interviews](https://example_company.com/example_company-com/user-interviews) project.
...
- **User Need** - Identify Example Company users from key customers of your group's
...
workflows when using those components of Example Company?
...
- You can find information about how customers are using Example Company through Sales and version.example_company.com. Sales and support should also be able to bring you into contact with customers
...
- Document your findings. Create a folder (sharable only within Example Company) in [Google Drive](https://drive.google.com/drive/folders/0AH_zdtW5aioNUk9PVA) with a structure as follows:
  - Customer Meetings
    - Customer Name A
      - 2020-04-01
        - agenda (Google Doc)
  ...
  - Competitive Research
    - Vendors
      ...
      - Vendor B
    - Projects
      - product-10132-code-scan-results (reference Example Company issue number)
...
**Example Company Solution Architects** know our customers the best, especially from a technical perspective.

**Example Company Issues** customers will often comments on issues, especially when the problem described by the issue
...
Example Company user page; this page often includes contact information such as Twitter or LinkedIn. Another option is to
...
**Customer Issues Prioritization Dashboards:** The [customer issues prioritization framework](/handbook/product/product-processes/customer-issues-prioritization-framework) aggregates customer data with the issues and epics that they have requested. When [viewing the dashboard](https://10az.online.tableau.com/#/site/example_company/workbooks/2015827/views), double click on the issue or epic of interest within the "priority score by noteable" table then scroll down to "QA Table - User request weighting by customer" to see the specific customers that are interested in the issue or epic.

**Example Company.com Broadcast Messages** Broadcast Messaging is a great tool for acquiring customer feedback from within the product. You can leverage [this workflow](/handbook/product/product-processes/pm-procedures/#gitlabcom-in-app-messages-broadcast-messaging) to use broadcast messaging.

**Example Company Sales and Customer Success** You can ask for help in [Slack customer success channel](https://example_company.slack.com/archives/C5D346V08/p1583438272202100/)
...
**Social Media** can also be effective. If your personal account has a reasonable number of connections/followers, you can post your desire to connect with users on a specific question directly. When posting, remember to include the subject you want to discuss as well as how people can reach out. You can also reach out to the `#social-media` channel to have your tweet retweeted by the @example_company account.

![twitter-contactpng](/handbook/product/twitter-contact.png)

If you want to reach a wider audience, consider asking a community advocate to re-post using the official Example Company account for the relevant platform.
...
**Non-users** are often more important than Example Company users. They can provide the necessary critical view to come up with
ideas that might turn them into Example Company users in the end. The best non-users are the ones who don't even plan on switching
to Example Company. You can reach these people at local meetups, conferences or online groups like, Hacker News. In every such case,
...
is the [Example Company DevOps Customer Advisory Board](/handbook/marketing/brand-and-product-marketing/product-and-solution-marketing/customer-advocacy/#example_company-devops-customer-advisory-board).
...
1. An existing solution already exists within Example Company
...
- If Example Company does not have this feature yet, link to the issue you created
- Finish the thread with a link to the commit and issue

#### Reaching out to specific users or accounts based on Example Company usage

You may want to interview a specific account because they are exhibiting atypical usage patterns or behaviors. In this case, [request Support to contact Example Company.com user(s) on your behalf](../../support/internal-support/#example_company-changes-and-contacting-users).

If it is the weekend, and the contact request is urgent as a result of an action that might affect a users' usage of Example Company, [page the CMOC](/handbook/engineering/infrastructure/incident-management/#how-to-engage-the-cmoc)
...
other products and use Example Company for all their flows.
...
1. Product team members add notes and video recordings from the interviews to the PCSAT column in this [Dovetail project](https://example_company.dovetailapp.com/projects/36nmGVKvkaT7SGMXtUeHVg/v/70xPTo5RzTRZnCNEVz1fWH).
...
**Note:** Example Company Customer Success Managers will also follow the process above so please be mindful to coordinate with them if they reach out or if they've already signed up for a user. Users should never be contacted by more than one Example Company team member. Users should never be contacted more than twice if they do not respond to an outreach email.
...
> Hello,
> My name is X and I'm the PM for X at Example Company. Thank you for giving us the opportunity to follow up on your response to our recent survey.
>
> I would be very interested in speaking further about some of the points you raised in your survey response. Would you be willing to do a 30 minute Zoom call to give us some more detailed feedback on your experience using Example Company? You'd be able to schedule the call at a time convenient to you.
>
...
>
> At Example Company, we value transparency. We would love to share the recording of conversation publicly on Example Company. Please indicate whether you give your permission for the recording to be shared on Example Company.
>
> Yes, you may share the recording publicly on Example Company.
>
> No, you MAY NOT share the recording publicly on Example Company.

#### After the call

1. If multiple Example Company employees are on the call, it can be beneficial to debrief immediately afterwards.
1. Collect all notes that were taken and Zoom recording from the interview and add them to the PCSAT column in this [Dovetail project](https://example_company.dovetailapp.com/projects/36nmGVKvkaT7SGMXtUeHVg/v/70xPTo5RzTRZnCNEVz1fWH).
...
Every Product Manager is responsible for the user experience and cost profile of their product area regardless of how the application is hosted (self-managed or example_company.com). If a feature is unsustainable from a cost standpoint, that can erode the margins of our SaaS business while driving up the total cost of ownership for self-managed customers. If a feature is slow, it can impact the satisfaction of our users and potentially others on the platform.

There are a few questions a Product Manager should ask when thinking about their features:

- What are the costs associated with my product area? What is the impact on the margin for each tier of Example Company.com?
  - Consider network, compute, and storage costs
- Are there tools in place to help Example Company, Inc and self-managed admins optimize the cost footprint for running Example Company (e.g. node rebalancing, transitioning objects to less costly storage classes, garbage collection capabilities)
- Are there features and default settings that help users stay within their [CI and Storage limits](https://about.example_company.com/pricing/)?
- Are there configurable [application limits](/handbook/product/product-processes/#introducing-application-limits) in place for admins to enhance the availability and performance of Example Company and reduce [abuse vectors](/handbook/security/security-operations/trustandsafety/#examples-of-common-forms-of-abuse-include-but-are-not-limited-to)?
- What is the experience of users when interacting with these features on Example Company.com? Is it fast and enjoyable?

These items do not all need to be implemented in an MVC, though potential costs and application limits should be considered for deployment on Example Company.com.
...
- [Service ping](https://docs.example_company.com/ee/development/internal_analytics/service_ping/)
- Your Engineering Manager, #infrafin on Slack, and the broader Example Company team

#### Links to learn more about infrastructure cost management initiatives

- [Infrafin Board Workflow](https://example_company.com/groups/example_company-com/-/boards/1502173?label_name%5B%5D=infrafin)
- [Infrafin Board by Group](https://example_company.com/groups/example_company-com/-/boards/2054211?label_name%5B%5D=infrafin)
- [Infrafin Board by Savings Amount](https://example_company.com/groups/example_company-com/-/boards/992233?&label_name%5B%5D=infrafin)
...
- [Snowplow data](/handbook/enterprise-data/platform/snowplow/) on Example Company.com
...
As a Product Manager you may need to make a decision on whether Example Company should engineer a solution to a particular problem, or use off the shelf software to address the need.

First, consider whether our users share a similar need and if it's part of Example Company's scope. If so, strongly consider [building as a feature in Example Company](/handbook/values/#dogfooding):
...
If the need is specific to Example Company, and will not be built into the product, consider a few guidelines:
...
- Opportunity cost: Is the need core to Example Company's business? Would work on other features return more value to the company and our users?
...
1. Have the owning party [open a finance issue](https://example_company.com/example_company-com/finance/issues/new) using the `vendor_contracts` template, ensure the justification above is included in the request.
...
- **Compatibility** - Does the software utilize a [compatible open source license](https://docs.example_company.com/ee/development/licensing.html#acceptable-licenses)?
...
PM: Following the [Product Data Insights handbook](/content/handbook/product/product-analysis/#working-with-us), create an issue focused on instrumentation of products at a category level [using this templaste](https://example_company.com/example_company-data/product-analytics/-/blob/main/.example_company/issue_templates/Post-Launch%20Instrumentation%20Audit.md).
...
PM/EM: will create an instrumentation issue with the label `analytics instrumentation` and utilizing the [usage data instrumentation template](https://example_company.com/example_company-org/example_company/-/issues/new?issuable_template=Usage+Data+Instrumentation).

## Page load performance metrics

In order to better understand the perceived performance of Example Company, there is a synthetic page load performance testing framework available based on [sitespeed.io](https://www.sitespeed.io).

A [Grafana dashboard](https://dashboards.example_company.net/d/product-create/product-performance-create?orgId=1) is available for each stage, tracking the [Largest Contentful Paint](https://web.dev/lcp/) and first/last visual change times. These metrics together provide high-level insight into the experience our users have when interacting with these pages.
...
1. Add the desired URL's to the sitespeed [unauthenticated](https://example_company.com/example_company-org/frontend/sitespeed-measurement-setup/-/blob/master/example_company/desktop/urls/desktop.txt) or [authenticated](https://example_company.com/example_company-org/frontend/sitespeed-measurement-setup/-/blob/master/example_company/desktop/loggedinurls/desktop.txt) testing list. Add a new line with the URL, then a space, and an alias of the form `[Group]_[Feature]_[Detail]`. The alias needs to be one word, an example MR is [here](https://example_company.com/example_company-org/frontend/sitespeed-measurement-setup/-/merge_requests/29). Note the authenticated user account does not have any special permissions, it is simply logged in.
1. Open the relevant stage's [grafonnet dashboard file](https://example_company.com/example_company-com/runbooks/-/tree/master/dashboards/product). Find the section corresponding to the desired group, and add an additional call to `productCommon.pageDetail`. The call arguments are `Chart Title`, `Alias` from above, and the tested `URL`. Ensure the JSON formatting is correct, the easiest way is to simply copy/paste from another line. A sample MR is available [here](https://example_company.com/example_company-com/runbooks/-/merge_requests/2719/diffs).
