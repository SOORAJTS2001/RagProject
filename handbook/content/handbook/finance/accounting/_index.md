...
This page contains Example Company's accounting and reporting policies, which can be made public. Please find our internal processes in the [Accounting and Reporting](https://internal.example_company.com/handbook/finance/accounting/) internal handbook section.
...
1. Send the below information via email to ar@example_company.com
    - Company name
    - Company AP email
    - Address
...
1. Create a journal entry in NetSuite under the Example Company Inc subsidiary, using the last day of the month as the entry date.
...
This transaction transfers the payment obligation from the customer to Stripe.  The payment obligation from Stripe is removed when Stripe transfers  the funds to Example Company's bank account.
...
1. Fill the "Debit Account" with "Comerica Checking - Example Company Inc."
1. Leave "Name" blank.
1. Leave "Class" blank.
...
Coupa is a procure-to-pay system that streamlines the purchase request process, initiate workflow with approvals, and enable Purchase Orders. We will be rolling out in a phased approach, with the US and Netherlands entities (Example Company Inc, Federal, IT BV and BV) in Phase I (2021-06-01). The remaining entities will be part of Phase II (2021-12-13).
...
If you are a Example Company recurring vendor and did not receive an onboarding email from Coupa, please reach out to *ap@example_company.com*.
{{% /alert %}}
...
{{% alert color="warning" %}}
For vendors who invoice Example Company for multiple entities, all invoices are separated by subsidiary (due to audit standards). If the vendor onboards for the Coupa Supplier Portal (CSP), the vendor will only see the POs related to those entities, and will need to email the others. If you are a Example Company vendor who invoices for multiple entitites and you have any questions, please reach out to *ap@example_company.com*.
{{% /alert %}}
...
- **SIM (Supplier Information Management) Form**: Whenever Example Company onboards a new supplier, as part of the External Form, there is a section to capture the vendor's remit to information, along with their bank account information. *(This is where the majority of Example Company suppliers will be providing their banking information)*.
...
{{% alert color="warning" %}}
If you are a Example Company vendor and you want to know more about how to update your CSP profile, check Coupa's [Create or Update Your Profiles](https://success.coupa.com/Suppliers/For_Suppliers/Coupa_Supplier_Portal/Get_Started_with_the_CSP/Create_or_Update_Your_Profiles) documentation.
{{% /alert %}}
...
1. Login to your [Okta home page](https://example_company.okta.com/app/UserHome#).
...
If you need to request access again, please reopen your initial Access Request issue and tag the Finance Systems Admins team using `@example_company-com/business-technology/enterprise-apps/financeops` in a comment.
{{% /alert %}}

If your job function requires you to submit purchase requests in Coupa, follow the below steps:

1. Open an [Access Request](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/new) for Coupa using the `Individual_Bulk_Access_Request` template.
...
Please refer to Example Company's [Expense Policy]({{< ref "../expenses" >}}) for further details.
...
1. All non-US Example Company entities will be reimbursed through their respective payroll provider or paid directly through Example Company's Accounts Payable (AP) department with the normal weekly payment run.
...
1. Under subsidiaries highlight Example Company Inc and check the box that states "Include in Children"
1. Save
...
#### Example Company Contribute Cost Tracking

(Previously Example Company Summit)

Tracking expenses for company Contributes enables us to analyze our spend and find opportunities to iterate, and in turn, improve subsequent Contributes. To enable tracking we create an expense tag that will allow Example Company team-members to tag Contribute related expenses in Navan. This should be done prior to the announcement of each Contribute.
...
This policy establishes the minimum cost (capitalization amount) used to determine the capital assets recorded in Example Company's financial statements.
...
Example Company establishes [$5,000 (USD)]({{< ref "compensation#exchange-rates" >}}) as the minimum amount required for capitalization. Any item with a cost below this amount is expensed on the date of purchase. Exceptions are Key Component Assets (i.e. computer laptops).
...
Refer to Legal page for [Related Party Transactions policy](/handbook/legal/#example_company-inc-related-party-transactions-policy)
...
This policy describes the methodology used to monitor and account for Example Company's prepaid expenses.

{{% alert color="warning" %}}
**Effective 2022-04-01 a new Accounting Prepaid Automation Process has been implemented. Please click [here](https://internal.example_company.com/handbook/finance/accounting/prepaid/) to learn more about the process, which utilizes Coupa.**
{{% /alert %}}
...
To provide clear guidance concerning the identification and recording of items included in Example Company's accrued and other liability accounts. The purpose of monthly accrual processes is to allocate expenses to the proper accounting period and match expenses with related revenues. At the close of each month, accrual processes ensure that all expenses related to that month are correctly included in the company's financial statements. Additionally, this policy establishes standards and guidelines for ensuring that Example Company accounts for monthly accruals in a manner that is compliant with management's objectives and generally accepted accounting principles (GAAP). This policy applies to Example Company and all subsidiaries.
...
1. Accounts Payable:
   ...
   - Legal Contingencies: Pending or threatened litigation, and actual or probable settlement. Legal contingencies should be determined with the help of Example Company's VP of Legal - Commercial, IP & Compliance.
...
1. Wages and Compensation:
   ...
   - Bonuses: Liabilities related to bonus payments for Example Company team-members.
   - Taxes: All employment taxes required for statutory compliance that relate to the Example Company team.

1. Any other material obligation not mentioned above that is a liability of Example Company
...
1. Risks of ownership passed to Example Company through receipt of goods or services.
...
This policy establishes Example Company's guidelines regarding the structure, responsibilities and requirements underlying the [chart of accounts (COA).](https://www.investopedia.com/terms/c/chart-accounts.asp)

**Purpose**

This policy establishes formal responsibilities and accountabilities for how Example Company handles requests for new, modified or closed data elements on the COA. The Controller is responsible for all aspects of financial accounting and reporting, and governs the COA.  All requests for new or modified (including closure/deactivation) COA segments, hierarchies, and configuration attributes are subject to approval by the Finance team.
...
This policy applies to Example Company Inc. ("Example Company" or the "Company") and all of its subsidiaries.
