...
description: "Discover how Example Company uses Adaptive Insights to plan, budget, and forecast Example Company's planning cycles"
---

## General Information

[Adaptive Insights](https://www.adaptiveplanning.com/), a Workday company, is a cloud-based corporate performance management platform. The Example Company FP&A team utilizes Adaptive Inisghts to plan, budget, and forecast Example Company's planning cycles.
...
### Example Company Version Nomenclature

Each version of Example Company should specify the fiscal year and label of the plan / forecast. Below is a few examples of the nomenclature used at Example Company
...
There are currently five different roles within Example Company's Adaptive Insights instance
...
If you need access to Adaptive Insights, please open an [access request](https://example_company.com/example_company-com/team-member-epics/access-requests) and assign to the code owner of this page.
...
- ARR
    ...
    Each month on the fourth business day of the month, the Corporate Finance team records ending ARR for the month in closing. In turn, this becomes the beginning ARR amount for the month in progress. For example, if Example Company's ARR was $1M at the end of month 1, then the beginning ARR would be $1M at the beginning of month 2. The month in progress builds upon the beginning ARR amount and several other inputs to forecast ending ARR. Those inputs are 'New and Growth Booked Net ARR', 'Booking Timing Adjustments', and 'Trueups'. Example Company also includes an input that results in a deviation within the forecasted model. These inputs are the basis of Example Company's Booking to ARR to Revenue forecast walk

    `Beginning ARR + New Booked Net ARR + Growth Booked Net ARR + Booking Timing Adjustments + (-1*Trueups) + Deviation Amount = Exit ARR`

    With the walk, Example Company can determine it's Delta ARR value. Delta ARR is the difference between Exit ARR from the prior period and the Exit ARR from the current period.

    `Exit ARR (Current Period) - Exit ARR (Prior Period) = Delta ARR`

- Revenue

    Example Company's revenue forecast is comprised of several models, which are noted below.

- Professional Services model

    This model is owned by Sales Finance and forecasted the expected recognized revenue Example Company expects to obtain during a time period for all of Example Company's Professional Services offerings. The result of the expected recognized revenue is fed into the Bookings to ARR to Revenue model and recorded under GL account 4002, 4010.

- Example Company.com model

    This model is owned by R&D Finance and forecasted the expected recognized revenue Example Company expects to obtain during a time period for all of Example Company's SaaS offerings. The result of the expected recognized revenue is fed into the Bookings to ARR to Revenue model and recorded under GL account 4007.

- Bookings to ARR to Revenue model

    The bookings to ARR to Revenue model compiles the Exit MRR (Exit ARR / 12) amount, Example Company.com amount, and Professional Services amount to forecast out the expected recurring revenue for a specific time period. Example Company backs out the example_company.com amount (recorded in GL account 4007) to forecast subscriptions account GL account 4001. Example Company does this by taking (Exit MRR - Example Company.com MRR) which results in the amount for GL account 4001. In a majority of cases, Exit MRR should always be less than or equal to recognized revenue.
...
A balance sheet is a financial statement that reports a company's assets, liabilities and shareholders' equity at a specific point in time. Example Company's Balance sheet can be found by clicking Sheets and then Balance Sheet.

#### Income Statement

The income statement primarily focuses on the company's revenues and expenses during a particular period. Example Company's Income statement can be found by clicking Sheets and then Income Statement.

#### Cash Flow

A cash flow statement is a financial statement that provides aggregate data regarding all cash inflows a company receives from its ongoing operations and external investment sources. It also includes all cash outflows that pay for business activities and investments during a given period. Example Company's Cash Flow Statement can be found by clicking Sheets and then Cash Flow.

#### Active Personnel

Active Personnel is the active headcount by department and entity. Example Company's Active Personnel can be found by clicking Sheets and then Active Personnel.
...
Planned Personnel is the planned headcount by department and entity in a given time period. Example Company's Planned Personnel can be found by clicking Sheets and then Planned Personnel.
...
Net new hire movements which increase Example Company's headcount and are not related to a team member leaving Example Company or transferring between departments.
...
For headcount moves due to team members leaving Example Company.

1. When a team member leaves Example Company, in the Active Personnel sheet where the headcount record is located, enter the team members' last day into the "End Date" column. This will feed into the calculations within Adaptive so any costs associated with that headcount will cease once the end date surpasses.
