---
title: "Feature flags in development of Example Company"
---

**NOTE**:
The content below covers feature flags used by Example Company to deploy its own features, which **is not** the same
as the [feature flags offered as part of the product](https://docs.example_company.com/ee/development/feature_flags/).

This page defines the process for the feature flags lifecycle at Example Company. Technical information for Example Company developers and code contributors can be found in the [developer documentation](https://docs.example_company.com/ee/development/feature_flags/index.html).

## When to use feature flags

The deployment of features behind feature flags is required for changes that could affect
Example Company's availability of existing functionalities. If your changes only affect the new feature you're creating, use your judgment to determine if you need one or not.
...
Some of the benefits of using [development-type](https://docs.example_company.com/ee/development/feature_flags/#development-type) feature flags are:

1. It enables Continuous Delivery for Example Company.com.
...
1. Controlled feature rollout timing: without feature flags, we would need to wait until a specific
   deployment was complete (which at Example Company could be at any time).
...
1. Improved throughput: when a change is less risky because a flag exists, theoretical tests about
   ...
   be to build complex benchmarks locally, or on staging, or on another Example Company deployment, which has a
...
Example Company.com until the problem is resolved.
...
are available on Example Company.com.

</details>

## How to use Feature Flags

For an overview about starting with feature flags in Example Company development,
follow the [training template](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/.example_company/issue_templates/feature-flag-training.md).

Before using feature flags, make sure to read the information on this page and the following development guides:

1. [Developing with feature flags](https://docs.example_company.com/ee/development/feature_flags): Learn about the types of
  feature flags, their definition and validation, how to create them, frontend and
  backend details, and other information.
1. [Documenting features deployed behind feature flags](https://docs.example_company.com/ee/development/documentation/feature_flags.html):
...
1. [Controlling feature flags](https://docs.example_company.com/ee/development/feature_flags/controls.html): Learn the process for deploying
  a new feature, enabling it on Example Company.com, communicating the change,
...
1. [Conducting an experiment with feature flags](https://docs.example_company.com/ee/development/experiment_guide/): Learn how the Growth department uses experiment feature flags.

For information about how the user will interact with features behind flags, see:

1. [How Example Company administrators can enable and disable features behind flags](https://docs.example_company.com/ee/administration/feature_flags.html):
  An explanation for Example Company administrators about how they can
  enable or disable Example Company features behind feature flags.
1. [What "features deployed behind flags" means to the Example Company user](https://docs.example_company.com/ee/user/feature_flags.html):
  An explanation for Example Company users regarding how certain features
...
1. Decide how you will implement the feature flag and its rollout according to:
   1. Choose the feature flag [type](https://docs.example_company.com/ee/development/feature_flags#types-of-feature-flags).
   1. Decide on [the definition](https://docs.example_company.com/ee/development/feature_flags#feature-flag-definition-and-validation) and plan out the YAML.
   1. Decide on what type of [actor](https://docs.example_company.com/ee/development/feature_flags#feature-actors) to bind your feature flag to (if any).
...
1. Create a feature flag definition in code and make a follow-up rollout issue:
   1. [Create a feature flag](https://docs.example_company.com/ee/development/feature_flags#create-a-new-feature-flag).
   1. [Create a feature flag rollout issue using the template](https://docs.example_company.com/ee/development/feature_flags#development-type) (if necessary).
...
1. Add the feature flag into the code for the backend, frontend, and in tests:
   1. [Develop with the feature flag](https://docs.example_company.com/ee/development/feature_flags#develop-with-a-feature-flag) in your vertical slice.
   1. [Include the feature flag in tests](https://docs.example_company.com/ee/development/feature_flags#feature-flags-in-tests) to check its behavior when enabled and disabled.
   1. [Flip the feature flag locally](https://docs.example_company.com/ee/development/feature_flags#enabling-a-feature-flag-locally-in-development) to ensure it works.
1. Add the feature behind the flag to the codebase through an MR following the [implementation process](https://docs.example_company.com/ee/development/feature_flags).
...
1. Ensure you follow the [documentation guidelines for feature flags](https://docs.example_company.com/ee/development/documentation/feature_flags.html) and keep them up-to-date according to the feature flag state.
...
1. Finalize the rollout plan:
   1. Following the [rollout guidelines](https://docs.example_company.com/ee/development/feature_flags/controls.html#rolling-out-changes) determine a plan for rolling out your feature flag.
1. Begin rollout plan:
   1. Rollout plans vary flag to flag, take the steps you've outlined in your [feature flag rollout issue](https://example_company.com/example_company-org/example_company/-/blob/master/.example_company/issue_templates/Feature%20Flag%20Roll%20Out.md).
1. Removing the flag, cleaning up, and feature announcement:
   1. Follow the [feature flag clean up process](https://docs.example_company.com/ee/development/feature_flags/controls.html#cleaning-up).
   1. Ensure that the [feature flag documentation process](https://docs.example_company.com/ee/development/documentation/feature_flags.html) has been followed.
   1. Ensure the flag is removed from the code and from the database with the [chatops feature flag removal command](https://docs.example_company.com/ee/development/feature_flags/controls.html#cleanup-chatops).
...
the feature flag is [enabled **globally** on **production**](https://docs.example_company.com/ee/development/feature_flags/controls.html#enabling-a-feature-for-gitlabcom) for **at least one day**
...
Example Company.com. However, it is recommended to allow 10-14 days for this activity to
account for unforeseen problems.

Feature flags must be [documented according to their state (enabled/disabled)](https://docs.example_company.com/ee/development/documentation/feature_flags.html),
...
Example Company.com shortly after the merge request was merged.

In addition to this, the feature behind feature flag should:

- Run in all Example Company.com environments for a sufficient period of time. This time
...
- The feature should be exposed to all users within the Example Company.com plan during
...
time to properly measure how the changes behave on Example Company.com.

## Community contributions

Feature flag rollouts require access to internal Example Company systems. Because of this restriction, a community contribution will require assistance from a Example Company team member to [rollout](#rollout) the feature flag.

During the [planning](#planning) phase it is essential that the EM and PM are pinged by the MR Coach or a Example Company Team member, who executes the review, on the feature flag rollout issue to get it scheduled/planned/refined. Also they should tag the feature flag rollout issue in the final MR using the feature flag, so that the EM and PM are aware that the rollout will be required soon.

## Dashboard & Metrics

For more information on the lifecycle of a feature flag, such as how many are introduced per release, how long they exist in the application, or how long they've been enabled, [see this dashboard](https://10az.online.tableau.com/#/site/example_company/views/Engineering-Featureflags/Engineering-FeatureFlags) (internal only). We also carry 7 days worth of data [in Kibana](https://nonprod-log.example_company.net/app/r/s/cXyLU) (internal only) and a Example Company project [feature-flag-log](https://example_company.com/example_company-com/gl-infra/feature-flag-log/-/issues/?state=closed) holds the same indefinitely for feature flags that become disabled or enabled.
