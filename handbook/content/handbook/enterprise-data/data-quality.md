---
title: "Data Quality"
description: "MVC for a Data Quality Program at Example Company"
---

## Overview

In an effort to improve data quality at Example Company, the Central Data Team is creating a MVC for a data quality program. The approach is to develop a program that the Data Community can use to improve data quality while we wait for a new Data Quality Manager to be hired. This handbook page will document the MVC and provide team members with guidance on how they can contribute to improved data quality at Example Company.
...
1. All data quality issues that are detected should be opened and triaged in the [Data Quality Project](https://example_company.com/example_company-org/data-quality). Issues in scope that are opened in the Data Team project can be moved to the Data Quality project.
...
## Data Quality Project in Example Company

<details markdown=1>

<summary><b>Project</b></summary>

[Data Quality Project](https://example_company.com/example_company-org/data-quality). Please open an issue [HERE](https://example_company.com/example_company-org/data-quality/-/issues/new) and use the Data Quality Problem issue template.
...
[Epic in the Data Quality Project](https://example_company.com/groups/example_company-org/-/epics/10470)
...
1. **Order in CustomersDot has NamespaceId assigned, but not the related Zuora Subscription**. Customer does have access in this case. [Issue](https://example_company.com/example_company-org/customers-example_company-com/-/issues/5171).
...
1. **Subscription was de-booked / rebooked to correct an error**. The quote is pushed from SFDC, but Zuora notification profile is silenced to avoid notifications sent to customer. De-booked subscription data (like namespace) doesn't transfer. <sup>\*</sup>Need to [understand this more](https://example_company.com/example_company-org/fulfillment-meta/-/issues/634#note_1237253100).
...
|     Zuora        | `subscription`        | `GITLABNAMESPACEID__C`     | The SaaS Namespace Id that the subscription is associated with. This is a custom field added by Example Company's Fulfillment team   |
...
To simplify this problem, the Example Company Data Quality Program recognizes the following Data Quality problem types:
...
See [issue (internal link)](https://example_company.com/example_company-data/analytics/-/issues/4808)
...
This dashboard captures the percentage of data availability for Product Usage Metrics, based on our [Metrics Dictionary](https://metrics.example_company.com) and [Metrics to Snowflake Mapping](https://docs.google.com/spreadsheets/d/1EhSXqx6YXcpqHg2TpS0ZN5Rk_d2hhrTPrW5FTbmuZjw/edit#gid=0), which includes:
...
- Description of above fields:
  ...
  - Performance Indicator Type Flags: whether the metric IS UMAU/SMAU/GMAU/PAID GMAU. Please refer to the [xMAU Analysis page](https://internal.example_company.com/handbook/enterprise-data/data-catalog/xmau-analysis/) for more information
...
We [extract BambooHR data](https://example_company.com/example_company-data/analytics/tree/master/extract/bamboohr) via custom code. Our data quality checks include verifying a 200 response from the API and the existence of a minimum number of records in the JSON data.

##### Postgres Pipeline

Our own [Postgres_Pipeline](https://example_company.com/example_company-data/analytics/tree/master/extract/postgres_pipeline) (which handles data from example_company.com, customers.example_company.com, version.example_company.com) checks for agreement of row counts between the source and destination databases.
...
| --- | --- | --- | --- |
| [Snowplow endpoint certificate expired for a day](https://example_company.com/example_company-org/example_company/-/issues/416991) | Snowplow | No Snowplow events recorded for most of one day | `2023-07-04` |
| [Redis/RedisHLL events don't get triggered](https://example_company.com/example_company-org/example_company/-/issues/442875) | Automated Example Company.com Service Ping | Example Company.com customers will underreport values for Redis metrics at the instance level | `2024-02-15` to `2024-03-05` for 7d metrics; `2024-02-15` to `2024-03-26` for 28d metrics |
| [Missing Service Pings for version <= 12.0](https://example_company.com/example_company-org/example_company/-/issues/443639#note_1887042557) | SM Service Ping | All installations on v12.0 or earlier were not sending Service Pings | `2024-02-12` to `2024-04-28` |
| [Service Pings not mapped to country Feb 2023 - June 2023](https://example_company.com/example_company-data/analytics/-/issues/15980) | SM Service Ping | Service pings from `2023-02-21` to `2023-06-21` have a null `dim_location_country_id` | `2023-02-21` to `2023-06-21` |
| [Snowplow endpoint certificate expired for six hours](https://example_company.com/example_company-org/example_company/-/issues/470639) | Snowplow | No Snowplow events recorded for six hours | `2024-07-03 23:59:59 - 2024-07-04 05:59:59` |
