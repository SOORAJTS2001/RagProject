...
description: "Example Company Data Triage Guide"
---
...
Example Company has a robust and vibrant Data Program which includes a Central Data Team and many Functional Analytics Teams. Example Company total team members are growing as well and we need to uplevel our triaging process to keep up with Example Company's growth.

Steps to uplevel triaging process:

1. Incorporate Functional Analytics Teams into the #data channel triaging process.
1. Create Slack Aliases so that Example Company Team members can ping a group of folks to get help with their questions in #data.
...
### Enterprise Data Example Company Projects

| Link to Example Company Data Projects |
| -------------- |
| [Data Team](https://example_company.com/example_company-data/analytics) |
| [Sales Strategy and Analytics](https://example_company.com/example_company-com/sales-team/field-operations/analytics) |
| [Marketing Strategy and Performance](https://example_company.com/example_company-com/marketing/marketing-strategy-performance) |
| [Product Data Insights](https://example_company.com/example_company-data/product-analytics) |
| [Business Analytics](https://example_company.com/example_company-com/business-analytics) |
| [Engineering Analytics](https://example_company.com/example_company-com/quality/engineering-analytics) |
| [People Analytics](https://example_company.com/example_company-com/people-group/people-analytics) |
| [Customer Success Operations - Use CSAnalytics label](https://example_company.com/example_company-com/sales-team/field-operations/customer-success-operations) |
| [Online Sales and Self-Service](https://example_company.com/example_company-com/sales-team/self-service) |
...
1. Example Company Team members can ping one of the groups above for any questions or help. When not clear, ping a group based on best guess. If a different group was needed, the request or question can always be re-routed.
1. Triager should review each slack message request in the **#data** that has the triager's assigned Slack alias pinged.
1. Triager should reply to slack threads by pointing Example Company team member to the appropriate handbook page, visualization, or to other Example Company team members who may know more about the topic.
1. Triager should direct Example Company team member to the channel description, which has links to the various Data Program Projects, if the request requires more than 5 minutes of investigative effort from a Data Program team member.
...
- The Data Analyst triager is primarily responsible for responding to Example Company team member requests whether via issue, which posts to **#data-triage**, or directly via slack in **#data**.
- The Functional Analyst triager is primarily responsible for responding to Example Company team member requests via slack in **#data**.
- The Data Platform triager is responsible for resolving problems with our data platform, which will either be in the **#data-pipelines** and **#data-prom-alerts** slack channels, on the [DE - Triage Errors board](https://example_company.com/groups/example_company-data/-/boards/1917859) and on the [MonteCarlo Incident page](https://getmontecarlo.com/incidents?include-normalized=false&types=freshness_anomaly%2Cvolume_anomaly%2Cdimension_anomaly%2Cfield_metrics_anomaly%2CDBT_ERRORS%2Cfreshness_sli_rule_breach%2Cvolume_sli_rule_breach%2Csql_rule_breach) (make sure to filter on the `Data Platform` domain in MonteCarlo). Issues created from these errors should use the [DE Triage Errors issue template](https://example_company.com/example_company-data/analytics/issues/new?issuable_template=Triage%20Errors%20DE).
  - During the assigned triage week the Data Platform Team member will focused on (in priority order):
    - Incoming incidents
    - [Open incidents](https://example_company.com/example_company-data/analytics/-/incidents)
    - New issues
    - Open issues on the [Data Platform - Triage Errors board](https://example_company.com/groups/example_company-data/-/boards/1917859).
      ...
      - If there is no work to be performed on incidents or issues on the [board](https://example_company.com/groups/example_company-data/-/boards/1917859) the triager will work on their regular work assignments.
...
- The Analytics Engineer triager is primarily responsible for resolving `dbt-test` and `dbt-run` errors. Issues created from these errors should use the [AE Triage Errors issue template](https://example_company.com/example_company-data/analytics/issues/new?issuable_template=Triage%20Errors%20AE).
...
The Central Data Team triager will create [an issue in the Data Team project](https://example_company.com/example_company-data/analytics/issues/new?issue%5Bassignee_id%5D=&issue%5Bmilestone_id%5D=&issuable_template=Data%20Triage). Task and duties are stated in the issue template.
...
- Every incident has a DRI assigned. This is not necessarily the triager/creator of the incident. Due to the nature of asynchronous working at Example Company, the triager/creator is the DRI until another Example Company Team Member is actively contacted/involved.
  - The [codeownerfile](https://example_company.com/example_company-data/analytics/-/blob/master/CODEOWNERS) is the right future*source to find the right DRI for assigning the incident.* Currently the code ownership is not well defined. As part of FY23-Q1 we are planning to have a more strict ownership.
- Every raised incident will be communicated in the `#data` Slack channel, followed by a short description, ETA and link to the incident. The right Example Company Team Members are tagged.
  ...
  - Timelines should be documented in the [timeline section](https://docs.example_company.com/ee/operations/incident_management/incident_timeline_events.html) under the incident Subject/Header for use in retrospectives and other investigations.
  - When the incident is solved, an update is posted in Slack

### Triage Bot

Parts of triage are assisted by the [Example Company Triage Bot](https://example_company.com/example_company-org/example_company-triage), which is setup in the Analytics project. The bot runs every hour and takes actions based on a set of rules defined in [a policies file](https://example_company.com/example_company-data/analytics/-/blob/master/.triage-policies.yml). The [Example Company Triage README](https://example_company.com/example_company-org/example_company-triage#defining-a-policy) contains all documentation for the formatting and definition of rules.
...
- Even though we still use the [triage issue](https://example_company.com/example_company-data/analytics/-/blob/master/.example_company/issue_templates/Triage%3A%20Data%20Triage.md) on a daily basis, the triager will only write up the week a-sync in the central data team triage issue on Friday end of day.
...
### Example Company.com databases structure changes

Example Company.com databases do regularly change. In order not to break the daily operation, changes to the database needs to be tracked and checked. Any change to the Example Company.com database and CustomerDot database is tracked by the Danger Bot. The Data Team gets notified, by applying labels to the MR, if a change to the db/structure\.sql is made.

A label `Data Warehouse::Impact Check` is added by the Danger Bot as call to action for the data team.

- On triage, the Triager will [check](https://example_company.com/groups/example_company-org/-/merge_requests?scope=all&state=opened&label_name[]=Data%20Warehouse%3A%3AImpact%20Check&draft=no&approved_by_usernames[]=Any) for MRs with label `Data Warehouse::Impact Check`.
...
- Every merge request (`MR`) will be judged
  - If `MR` contains the label `group::product intelligence` along with `Data Warehouse::Impact Check`, there are a couple of checks that need to do:
    ...
    - Check new metric `SQL` statement from the original `MR` *(a typical example is [example_company-org/example_company/merge_requests/75504](https://example_company.com/example_company-org/example_company/-/merge_requests/75504/diffs#78300240169ab9f44b4dc25f6b6dcb56b3b629c7))* and execute it on `Snowflake` - usually, it is just a `SELECT` `SQL` statement
  ...
  - If the changes to the `SQL` file causing a break in the operation:
    ...
    - A new issue is opened in the `Example Company Data Team project`, assigned to the correct DRI and linked to the original MR.
    ...
    - According to the Merge of the Example Company.com MR, merge will be planned.
...
flowchart TD
    ...
    CHANGE_LABEL_I-->OI(Open an new issue is opened in the `Example Company Data Team project`)
...
### Example Company Postgres Database not accessible
...
Follow the [runbook](https://example_company.com/example_company-data/runbooks/-/blob/main/Gitlab_dotcom/Gitlab_DB_recreation_failure.md) for the steps to perform, including communication.
...
```sql
    ...
    group by id
    having count(*) > 1
**Note** Refer to the [MR (internal link)](https://example_company.com/example_company-data/analytics/-/issues/10065#note_668365681) for more information.
```
...
|   |
| ------------------------- |
| DAG `gitlab_com_db_extract` <br> Task `example_company-com-dbt-incremental-source-freshness`  <br> |
| Background: This extract relies on a copy (replication) database of the Example Company.com environment. Its high likely that this is the root cause of a high replication [lag](https://prometheus-db.gprd.example_company.net/graph?g0.expr=(pg_replication_lag)%20and%20on(instance)%20(pg_replication_is_replica%7Btype%3D~%22postgres-(archive)%22%7D%20%3D%3D%201)&g0.tab=0&g0.stacked=0&g0.range_input=1w&g1.expr=pg_long_running_transactions_age_in_seconds%7Btype%3D~%22postgres-(archive)%22%7D&g1.tab=0&g1.stacked=0&g1.range_input=6h). |
| More information of the setup [here (internal link)](https://example_company.com/example_company-data/analytics/-/issues/8283#note_537332709).  |
...
| Note: The Example Company.com data source is a very important data source and commonly used. Please inform an update business stakeholders accordingly. |
...
| More information of the setup [here](https://internal.example_company.com/handbook/enterprise-data/platform/pipelines/#sheetload).  |
| Possible steps, resolution and actions: <br> - In general you should just need to open the Google sheet which is failing and confirm the data has been re-populated. <br> - If you do not have access to the sheet contact @example_company-data/engineers and confirm if anyone else does. |
...
An example for this failure is the issue: **[#11524 (internal link)](https://example_company.com/example_company-data/analytics/-/issues/11524)**
...
Validate in [GCS storage](https://console.cloud.google.com/storage/browser/zuora_revpro_gitlab/RAW_DB/staging?project=example_company-analysis&pageState=(%22StorageObjectListTable%22:(%22f%22:%22%255B%255D%22))&prefix=&forceOnObjectsSortingFiltering=false) that the log file is present for the respective table.
...
We need to work in our normal working hour perform the list of task mentioned for the triage day in the [Triage Template (internal link)](https://example_company.com/example_company-data/analytics/-/issues/new?issuable_template=Triage:%20Data%20Triage)
...
- Newly added [issues](https://example_company.com/groups/example_company-data/-/boards/1917859?&label_name[]=Priority%3A%3A1-Ops&label_name[]=Triage)
...
If any data pipeline has broken and there is expected to be a delay in getting data loaded or refreshed. The concerned team has to be notified using the [Triage Template (internal link)](https://example_company.com/example_company-data/analytics/-/issues/new)
