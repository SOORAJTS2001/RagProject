...
In `June of 2019`, we switched sending Snowplow events from a third party to sending them to infrastructure managed by Example Company, documented on this page. From the perspective of the data team, not much changed from the third party implementation. Events are sent through the collector and enricher and dumped to S3.
...
URL wil stay the same `snowplow.trx.example_company.net`. Any `app_id` is fine if there are no other concerns around enabling tracking on `CustomersPortal` staging as well.
...
file. As an example, [here is an issue](https://example_company.com/example_company-data/analytics/-/issues/16552) to update the variable.
...
Snowplow is built with Terraform on AWS documented in the [example_company-com-infrastructure project](https://ops.example_company.net/example_company-com/example_company-com-infrastructure/blob/master/environments/aws-snowplow/README.md).

For a detailed walk-through of our setup, watch [this Example Company Unfiltered internal video](https://www.youtube.com/watch?v=fK9aw3bHFBg&feature=youtu.be).

![Lucid_chart](https://lucid.app/publicSegments/view/22c5f9f5-2b40-4474-95af-b8116edea6be/image.png)

##### S3

Enriched events are stored in TSV format in the bucket `s3://example_company-com-snowplow-events/output/`.
Bad events are stored as JSON in `s3://example_company-com-snowplow-events/enriched-bad/`.
...
Backfills are done via Airflow. The [`dbt_snowplow_backfill` DAG](https://example_company.com/example_company-data/analytics/blob/master/dags/transformation/dbt_snowplow_backfill.py) will generate a task for each month from July 2018 to the current month.
...
We only keep Duo free form feedback for 60 days in snowflake. This is managed by the [duo_data_redaction DAG](https://example_company.com/example_company-data/analytics/-/blob/master/dags/general/duo_data_redaction.py), which runs daily, removing contents of the `extendedFeedback` attribute in the `contexts` column for all feedback response Snowplow events in `RAW` and `PREP`. This timeline allows for our full-refresh process to complete, updating all downstream data, within 90 days for compliance.
