...
description: "Example Company Data Team CI Jobs"
---

---

This page documents the CI jobs used by the data team in Merge Requests in both the [Data Tests](https://example_company.com/example_company-data/data-tests) and [Analytics](https://example_company.com/example_company-data/analytics) projects.

## What to do if a pipeline fails

- If a weekend has passed re-run any CLONE steps which were performed prior, every Sunday (5:00AMUTC) all old pipeline databases are [dropped](https://example_company.com/example_company-data/analytics/-/blob/master/orchestration/drop_snowflake_objects.py) from SnowFlake older than 14 days.
...
This kind of error pops up in the pipeline like KeyError: 'GITLAB_COM_CI_DB_USER'. It means the variable is not defined in the variable section of CI/CD Settings. To resolve this, add the variable name to [CI/CD setting](https://example_company.com/example_company-data/analytics/-/settings/ci_cd) i.e. settings --> ci_cd --> variable, also provide the variable value.
...
These jobs are defined in [`.example_company-ci.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/.example_company-ci.yml). All Snowflake objects created by a CI clone job will exist until dropped, either manually or by the [weekly clean up of Snowflake objects](/handbook/enterprise-data/platform/ci-jobs/#what-to-do-if-a-pipeline-fails).
...
These jobs are defined in [`extract-ci.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/extract/extract-ci.yml)
...
**This is separate from the `pgp_test` job because it requires a CloudSQL Proxy to be running in order to connect to the example_company-ops database.**
...
These jobs are defined in [`snowflake-dbt-ci.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/transform/snowflake-dbt/snowflake-dbt-ci.yml)
...
Available selectors can be found in the [selector.yml](https://example_company.com/example_company-data/analytics/-/blob/master/transform/snowflake-dbt/selectors.yml) file. The dbt build command will run all seeds, snapshots, models, and tests that are part of the selection. This is useful for the following scenarios:
...
but also results in bigger costs for Example Company if the warehouse is running for less than a minute.
...
More implementation details can be found in the issue [here](https://example_company.com/example_company-data/analytics/-/issues/19885).
...
These jobs are defined in [`.example_company-ci.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/.example_company-ci.yml).
...
Manual job to do a dry run of [Permifrost](https://example_company.com/example_company-data/permifrost/).
...
Runs the `spec-test` cli of [Permifrost](https://example_company.com/example_company-data/permifrost/) to verify changes have been correctly configured in the database.
...
- To add new users/roles in Snowflake, add the new username(s) to [`snowflake_users.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/snowflake_users.yml?ref_type=heads).
...
Under the hood, this CI job is calling the python script [`orchestration/snowflake_provisioning_automation/provision_users/provision_user.py`](https://example_company.com/example_company-data/analytics/-/blob/master/orchestration/snowflake_provisioning_automation/provision_users/provision_users.py?ref_type=heads).
...
- To add new user entries to `roles.yml`, add the new username(s) to [`snowflake_users.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/snowflake_users.yml?ref_type=heads).
- Likewise, to remove user entries from `roles.yml`, delete the username(s) from [`snowflake_users.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/snowflake_users.yml?ref_type=heads).
...
Under the hood, this CI job is calling the python script [`orchestration/snowflake_provisioning_automation/update_roles_yaml/update_roles_yaml.py`](https://example_company.com/example_company-data/analytics/-/blob/master/orchestration/snowflake_provisioning_automation/update_roles_yaml/update_roles_yaml.py?ref_type=heads).
...
1. `USERS_TO_ADD`:
    - Defaults to the usernames **added** to [`snowflake_users.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/snowflake_users.yml?ref_type=heads) within the MR.
    - To override, pass in a string value like so `USERS_TO_ADD: username_to_add1 username_to_add2`
1. `USERS_TO_REMOVE`:
    - Defaults to the usernames **removed** from [`snowflake_users.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/snowflake_users.yml?ref_type=heads) within the MR.
