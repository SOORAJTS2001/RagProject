...
We use the [#data-pipelines](https://example_company.slack.com/archives/C0384JBNVDJ) Slack channel for MC platform related alerts.
We are planning on using the [#data-analytics](https://example_company.slack.com/archives/CBZD1BA5S) Slack channel in the near future for model related alerts, as soon as we have implemented the full notification strategy for Monte Carlo.
This work is planned under this epic for F23Q3: [Onboard Analytics Engineers to the Monte Carlo Tool](https://example_company.com/groups/example_company-data/-/epics/615)
...
de(Example Company Team Member)
...
The whole body of work covering the Monte Carlo rollout at Example Company falls under epic [Rollout Data Observability Tool with 100% coverage of Tier 1 Tables to improve Trusted Data, Data Quality, and Data Team member efficiency](https://example_company.com/groups/example_company-data/-/epics/567), where the work breakdown has been done and issues have been created to reflect the necessary steps until we are up and running with Monte Carlo on production.
...
A runbook of how everything is technically set up can be found in the [Monte Carlo Runbook](https://example_company.com/example_company-com/business-technology/team-member-enablement/runbooks/-/wikis/IT-Runbooks/App-Setup/Monte-Carlo:-How-It's-Built).
...
For that you should submit an AR (similar ARs: [Example AR 1](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/22860), [Example AR 2](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/22878)) and assign it to Rigerta Demiri (@rigerta) or ping the #data channel linking the AR.
...
Our goal is to be able to integrate Monte Carlo with Example Company so that whenever we get an alert on Slack, a triage issue would automatically be opened on Example Company and we'd follow the same [Data Triage procedure](/handbook/enterprise-data/how-we-work/triage/) as usual.
...
There is an ongoing [feature request](https://example_company.com/example_company-data/permifrost/-/issues/120) on Permifrost for adding granularity to the way permissions are set, but no solution has been agreed on yet.
...
`***` Only these two models (`snowplow_structured_events_400` and `snowplow_structured_events_all`) of the `LEGACY` schema have been included temporarily as per MR [!7049](https://example_company.com/example_company-data/analytics/-/merge_requests/7049)
...
When we initially deployed Monte Carlo at Example Company, we defaulted to Sisense as a BI tool, as it was what we were using at the time.
...
Additional internal information is available in our [Internal Example Company Handbook](https://internal.example_company.com/handbook/enterprise-data/platform/monte-carlo).
