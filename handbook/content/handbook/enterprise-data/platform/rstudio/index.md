...
[SnowflakeDSII]
Server = example_company.snowflakecomputing.com
Port =
UID =
Schema =
Warehouse =
...
Locale = en-US
Tracing = 0
Authenticator = example_company.okta.com
```
...
1. `server = "example_company.snowflakecomputing.com` is specific to the snowflake instance being accessed.

### OKTA Authenticator

Since OKTA or other authenticators are often used to connect to Snowflake, we have reference **`Authenticator`** several times in the directions above. The first is in the **odbc.ini** file, specifying the authenticator used here at Example Company (OKTA). It is then referenced in the parameters used with `DBI::dbConnect()` in the line `Authenticator = "externalbrowser"`.
...
Welcome to R!`
- You will also see the variables `uid`, `role`, and `driver` in your environment. These variables are used for connecting to your database (Snowflake here at Example Company), or for any other variables you deem necesary.
...
3. Clone an existing project from Example Company
4. Troubleshooting
...
- You will also need to have your Example Company account set up and access to the project you want to clone
...
- Create an SSH key by following the instructions it/data-team/ the [Generate an SSH Key Pair](https://docs.example_company.com/ee/user/ssh.html) section.
...
- Configure Git by setting your **Example Company user name** and **Example Company email** in RStudio
...
- To create a new project based on a remote Git repository:
  ...
  - Choose **Git**, then provide the repository URL:
    ...
    - Access the Example Company project you want to clone
...
- The Example Company Project should now be visible in R Studio
...
- Error:
  ...
  example_company.com: Permission denied (publickey).
...
## How to Update a Example Company Project with Updates from R Studio

- Before uploading changes made locally to a Example Company project ensure that you are working with the most current branch by selecting **Pull with Rebase** from the **Git** section in R (Ensure that you are rebasing from the **main** branch)
...
- Select **Commit**
- In Example Company, navigate to the project you have made updates to. You should see a merge request that needs to be created and it will have the changes you made in R. Select the relevant reviewers and approvers to merge the changes.
