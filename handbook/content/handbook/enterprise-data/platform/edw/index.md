...
1. dbt seed: This is a solid option. dbt seed is version controlled via Example Company, easy to update via MR in Example Company Web IDE. A con is that it is recommended to be used with low row counts, about a 1,000.
...
- **All dimensions must have a surrogate key:**
  ...
  - An example of the surrogate key creation can be found in [prep_order_type](https://example_company-data.example_company.io/analytics/#!/model/model.gitlab_snowflake.prep_order_type#code). In this example
...
- **All fact and dimension tables should have the following audit columns:**
  ...
  - created_by - this is a Example Company user id
  - updated_by - this is a Example Company user id
...
At Example Company, we standardize the definition of 'first day of week' across all our systems to consistently use **Monday** as the first day of the week. This standard applies to all data models, reports, and analyses within the Enterprise Data Warehouse.
...
1. A classic example of a conformed dimension is the [dim_date](https://example_company-data.example_company.io/analytics/#!/model/model.gitlab_snowflake.dim_date) model that is used across various fact tables/Subject areas such as ARR [fct_mrr](https://example_company-data.example_company.io/analytics/#!/model/model.gitlab_snowflake.fct_mrr) and Salesforce Opportunities [fct_crm_opportunity_daily_snapshot](https://example_company-data.example_company.io/analytics/#!/model/model.gitlab_snowflake.fct_crm_opportunity_daily_snapshot). Other examples of conformed dimensions include [dim_crm_account](https://example_company-data.example_company.io/analytics/#!/model/model.gitlab_snowflake.dim_crm_account) and [dim_subscription](https://example_company-data.example_company.io/analytics/#!/model/model.gitlab_snowflake.dim_subscription).
...
1. An example is [dim_instances](https://example_company-data.example_company.io/analytics/#!/model/model.gitlab_snowflake.dim_instances) that contains statistical data for instances from Service ping and is specifically used in Product Usage models like [mart_monthly_product_usage](https://example_company-data.example_company.io/analytics/#!/model/model.gitlab_snowflake.mart_monthly_product_usage).
...
|---------------|-------------------------|
| Cost of Sales | team_member1@example_company.com |
| Cost of Sales | team_member2@example_company.com |
| Cost of Sales | team_member3@example_company.com |
| G&A           | team_member1@example_company.com |
| G&A           | team_member2@example_company.com |
| Sales         | team_member1@example_company.com |
| R&D           | team_member3@example_company.com |
| Marketing     | team_member1@example_company.com |
...
1. A simple query of the Snowplow, Service Ping, and Example Company.com big data sets in Snowflake finishes in under 1 minute on a L or XL warehouse.
