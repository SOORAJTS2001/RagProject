...
Here is our `Example Company Zen of Python` proposal:
...
Here is the set of command we are using to initiate and install `Python` (in our case it is `3.10.3`), refer to the file [onboarding_script.zsh](https://example_company.com/example_company-data/analytics/-/blob/master/admin/onboarding_script.zsh).
...
authors = ["{}"]
readme = "README.md"
homepage = "https://example_company.com/example_company-data/service-ping-metrics-check/README.md"
repository = "https://example_company.com/example_company-data/service-ping-metrics-check"
...
The full file is exposed in the file [service-ping-metrics-check/pyproject.toml](https://example_company.com/example_company-data/service-ping-metrics-check/-/blob/main/pyproject.toml) as a part of [**Service ping metrics check**](https://example_company.com/example_company-data/service-ping-metrics-check) project.
...
[**Pytest**](https://docs.pytest.org/en/7.2.x/index.html) is used to run unit tests in the `/analytics` project. The tests are executed from the root directory of the project with the `python_pytest` CI pipeline job. The job produces a `JUnit` report of test results which is then processed by `Example Company` and displayed on merge requests.
...
For automating code quality and testing, we are using our own product [Example Company CI/CD pipeline](https://docs.example_company.com/ee/ci/pipelines/).
