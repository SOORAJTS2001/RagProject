...
description: "Example Company Data Team Platform"
---
...
| Role | Responsibility |
| ---- | -------------- |
| Example Company Team Members | Responsible for taking notice of how the standard that forms the Data Platform |
...
- [Meltano](https://internal.example_company.com/handbook/enterprise-data/platform/Meltano-Gitlab/)
...
We use Example Company to operate and manage the analytics function.
...
1. [Meltano](https://internal.example_company.com/handbook/enterprise-data/platform/Meltano-Gitlab/)
...
| [CustomersDot](https://internal.example_company.com/handbook/enterprise-data/platform/pipelines/#example_company-customer-dot-database) [ERD](https://example_company.com/example_company-org/customers-example_company-com/-/blob/staging/doc/db_erd.pdf) | pgp | `tap_postgres` | `customers` | Product | 24h / x | No | Tier 1 |
...
| [Gainsight Customer Success](https://example_company.gainsightcloud.com/v1/ui/home) | Fivetran | `gainsight_customer_success` | `gainsight_customer_success` | Customer Success | 24h / 48h | No | Tier 3 |
| [Example Company.com](https://internal.example_company.com/handbook/enterprise-data/platform/pipelines/saas-example_company-com/) | pgp | `tap_postgres` | `gitlab_dotcom` | Product, Engineering | 12h / 55h | No | Tier 1 |
| [Example Company Ops DB](https://internal.example_company.com/handbook/enterprise-data/platform/pipelines/#example_company-ops-database) | pgp | `tap_postgres` | `gitlab_ops` | Engineering | 6h / x | No | Tier 1 |
| Example Company Profiler DB | x | x | x | x | x / x | No | Tier 3 |
| Example Company Container Registry Logs | Airflow | `Container Registry` | `Container Registry` | Engineering | x | No | Tier 2 |
...
| [Google Analytics 4](https://developers.google.com/analytics/devguides/collection/ga4) | [BigQuery Exporter](https://internal.example_company.com/handbook/enterprise-data/platform/pipelines/#bigquery-exporter) | `google_analytics_4_bigquery` | `google_analytics_4` | Marketing | 24h / 48h | No | Tier 2 |
| [Google Cloud Billing](https://cloud.google.com/support/billing) | [BigQuery Exporter](https://internal.example_company.com/handbook/enterprise-data/platform/pipelines/#bigquery-exporter) | `gcp_billing` | `gcp_billing` | Engineering | 24h / x | No | Tier 1 |
...
| [Handbook YAML Files](https://example_company.com/example_company-data/analytics/-/tree/master/extract/gitlab_data_yaml) | Airflow | `gitlab_data_yaml` | `gitlab_data_yaml` | Multiple | 8h / 24h | No | Tier 2 |
| [Handbook MR Data](https://example_company.com/example_company-data/analytics/-/blob/master/dags/extract/handbook_mrs_extract.py) | Airflow | `handbook` | `handbook` | Multiple | 24h / 24h | No | Tier 2 |
| [Handbook Git Log Data](https://example_company.com/example_company-data/analytics/-/blob/master/dags/extract/values_page_extract.py) | Airflow | `handbook` | `handbook` | Multiple | 1w / 1m | No | Tier 2 |
...
| [Omamori](https://example_company.com/example_company-com/gl-security/security-operations/trust-and-safety/omamori) | Airflow | `omamori` | `omamori` | Engineering | 1h / 24h | No  | Tier 2 |
...
| [SaaS Service Ping](https://internal.example_company.com/handbook/enterprise-data/platform/pipelines/#service-ping) | Airflow | `saas_usage_ping` | `saas_usage_ping` | Product | 1 week / 24h ([more context](https://internal.example_company.com/handbook/enterprise-data/platform/pipelines/#slo-explanation-for-automated-service-ping)) | No | Tier 1 |
| [Salesforce](https://www.salesforce.com/) | Stitch | `salesforce_v2_stitch` | `sfdc` | Sales | 6h / 24h | Yes | Tier 1 |
| [Salesforce Sandbox](https://example_company--staging.sandbox.my.salesforce.com/)| Stitch | `salesforce_stitch_sandbox_v2` | `TBC` |Sales | 24h / 48h| Yes| Tier 3|
...
| [Thanos](https://thanos-query.ops.example_company.net/graph) | Snowflake Task | `prometheus` | `prometheus` | Engineering | 24 h / x | No | Tier 3 |
| [Version DB](https://version.example_company.com/users/sign_in) | Automatic Process | `version_db` | `version_db` | Product | 24 h / 48 h | No | Tier 1 |
...
- [Version (pings)](https://drive.google.com/file/d/1S8lNyMdC3oXfCdWhY69Lx-tUVdL9SPFe/view)
  - Note that up until October 2019, the data team referred to the entire **version** data source as "pings". However, usage ping is only one subset of the version data source which is why we now use "version" or "version app" to refer to the version.example_company.com *data source* and "usage data" or "usage pings" or "pings" to refer to the [specific usage data feature](https://docs.example_company.com/ee/administration/settings/usage_statistics.html) of the version data source. In the context of Data extraction, when it comes to `Service ping` data ingestion, specific details should be found in the [Service ping](https://internal.example_company.com/handbook/enterprise-data/platform/pipelines/#service-ping) page or in the [Readme.md](https://example_company.com/example_company-data/analytics/-/blob/master/extract/saas_usage_ping/README.md) page for Service ping
...
We use Airflow on Kubernetes for our orchestration. Our specific setup/implementation can be found [here](https://example_company.com/example_company-data/data-image). Also see the [Data Infrastructure](/handbook/enterprise-data/platform/infrastructure/) page for more information.
...
We currently use [Snowflake](https://docs.snowflake.net/manuals/index.html) as our data warehouse. The Enterprise Data Warehouse (EDW) is the single source of truth for Example Company's corporate data, performance analytics, and enterprise-wide data such as Key Performance Indicators. The EDW supports Example Company's data-driven initiatives by providing all teams a common platform and framework for reporting, dashboarding, and analytics. With the exception of point-to-point application integrations all current and future data projects will be driven from the EDW. As a recipient of data from a variety of Example Company source systems, the EDW will also help inform and drive Data Quality best-practices, measures, and remediation to help ensure all decisions are made using the best data possible.
...
```bash
- example_company-com-snowplow-events/
...
All new loads in the `S3` bucket will go into the same folder as before `example_company-com-snowplow-events/output`.

### Snowflake support portal access

To get access to snowflake support portal, please follow the below steps.

- Register using Example Company email id to [community portal](https://community.snowflake.com/CommunitiesSelfReg)
- This registration will send a welcome email to Example Company mail with the subject `Welcome to the Snowflake Community`. In the mail it will ask you to finish the registration as part of that you will be asked to set your password for the community portal.
...
- Create an issue in the [access requests project](https://example_company.com/example_company-com/team-member-epics/access-requests) documenting the level of access required.
...
- Snowflake is part of the [Access Review Procedure](/handbook/security/security-assurance/security-compliance/access-reviews/) and manager will be asked on a quarterly basis to review the access their team members have in Snowflake. It is expected from the manager to understand the available roles(structure) in Snowflake if approving an AR or reviewing their team member access.
  - In the access review, only the first level of Snowflake roles are reported (the ones that are directly attached to the user). I.e. If a team member does have the `analyst_marketing` role, only the `analyst_marketing` is reported and all inherited roles in the `analyst_marketing` are not.
    - Roles could be distinguished between functional roles and object roles
      ...
      - To know in all detail what a role entails check this YAML [file](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/roles.yml).
...
Snowflake can be used to perform analyses on the data that is available by writing SQL-code. Anything created and any outcome of the analyses is considered as an [ad-hoc analyses](handbook/it/data-team/data-development/#data-development-at-example_company). It is important to know that anything that is created (i.e. worksheets and dashboards) is not version controlled and not supported or managed by the Central Data Team. I.e. When a team member off-boards from Example Company, the worksheets and dashboards are not accessible anymore. In order to persist analyses, team members can build Tableau workbooks, store code snippets in a Example Company project, or commit code to the Data Team's [dbt project](https://example_company.com/example_company-data/analytics/-/tree/master/transform/snowflake-dbt).
...
We use [Permifrost](https://example_company.com/example_company-data/permifrost/) to help manage permissions for Snowflake.
Our configuration file for our Snowflake instance is stored in [this roles.yml file](https://example_company.com/example_company-data/analytics/blob/master/permissions/snowflake/roles.yml).
...
This list of functional roles gives a high level understanding of what the role entails. If missing or to know in all detail what a role entails check this YAML [file](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/roles.yml).
...
- Make sure we have an issue in the Example Company Data Team project linking the original request with the `Provisioning` label applied
- Login to Snowflake and switch to `securityadmin` role
  - All roles should be under `securityadmin` ownership
- Copy the [`user_provision.sql`](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/user_provision.sql) script and replace the email, firstname, and lastname values in the initial block
...
- Document in Snowflake [roles.yml](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/roles.yml) permifrost config file (this file is automatically loaded every day at 12:00a.m. UTC)
...
- Make sure we have an issue in the Example Company Data Team project linking the original source request with the `Deprovisioning` label applied.
- Login to Snowflake and switch to `securityadmin` role
  - All roles should be under `securityadmin` ownership.
- Copy the [`user_deprovision.sql`](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/user_deprovision.sql) script and replace the USER_NAME. The reason for not removing and leaving the user in snowflake and setting disabled = TRUE is to have a record of when the user lost access.
- Remove the user from `okta-snowflake-users` [Google Group](https://groups.google.com/my-groups)
- Remove the user records in Snowflake [roles.yml](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/roles.yml) permifrost config file (this file is automatically loaded every day at 12:00a.m. UTC)

For more information, watch this [recorded pairing session](https://youtu.be/-vpH0aSeO9c) (must be viewed as Example Company Unfiltered).

## Snowflake Provisioning Automation

In FY25-Q1, we are moving towards semi-automating the above `Managing Roles for Snowflake` process, [OKR epic](https://example_company.com/groups/example_company-data/-/epics/1128). This will enable **all Example Company Team Members** to create a Snowflake user themselves with minimal support by the Data Platform Team. This will speed up the provisioning process and shorten the time a Example Company Team member can get access to Snowflake.

All Example Company Team Members are encouraged to open a MR following this [runbook](https://example_company.com/example_company-data/runbooks/-/blob/main/snowflake_provisioning_automation/snowflake_provisioning_automation.md) if they need access to Snowflake.
...
Both CI jobs follow a common pattern, the end user simply has to add/remove users from within the [`snowflake_usernames.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/snowflake_usernames.yml) file, and the CI job will run based on the changes to the file.
...
In order to push to the repo from within the CI pipeline, a [Project Access Token](https://docs.example_company.com/ee/user/project/settings/project_access_tokens.html) (PAT) is needed, more info pushing to the remote repo in this [StackOverflow answer](https://stackoverflow.com/a/73394648).

The PAT is named `snowflake_provisioning_automation` and was created in the ['Example Company Data Team' project](https://example_company.com/example_company-data/analytics), using the analyticsapi@example_company.com account.

The PAT value is saved within 1Pass, and also as a CI environment variable so that it can be used by the Example Company runner.

#### snowflake_users.yml - end of file issue

When adding a user to the `snowflake_users.yml` file, specifically when appending to the bottom of the file, it causes unexpected behavior if done using the Example Company Single File Editor, more info in [this issue](https://example_company.com/example_company-data/analytics/-/issues/20730#note_1919902289).
...
Inactive Snowflake users will be deprovisioned weekly via `snowflake_cleanup` DAG, implemented in [this issue](https://example_company.com/example_company-data/analytics/-/issues/20347).
...
These users will be dropped by running the following [deprovision_user.sql](https://example_company.com/example_company-data/analytics/-/blob/master/orchestration/snowflake_provisioning_automation/provision_users/sql_templates/deprovision_user.sql?ref_type=heads) script.
...
Provisioning USAGE permissions for external tables to user roles inside snowflake is not handled by permifrost in the moment. If you have to provision access for an external table to a user role, then it must be granted manually via GRANT command in snowflake[docs](https://docs.snowflake.com/en/sql-reference/sql/grant-privilege) using a `securityadmin` role. This implies that the user role already has access to the schema and the db in which the external table is located, if not add them to the [roles.yml](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/roles.yml).

#### Logging in and using the correct role

When you apply for a Snowflake account via an AR and get access provisioned it takes until 3.00AM UTC for the change to take effect. This is because we have a script running daily to provision the access in Snowflake. When you can login, you can do this via Okta. After you logged in via Okta, you need to select the right role that is attached to your account. This is by default the same as your account and it follows the convention of your email addres minus `@example_company.com`.
...
| `gitlab_postgres`    | This is for extraction jobs that pull from Example Company internal Postgres databases                   | 10                  |
...
- Warehouses are set as t-shirt sizes. Larger warehouses are more costly for Example Company
...
There is a `snowflake` database, which contains information about the entire Example Company instance.
...
All databases not defined in our [`roles.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/permissions/snowflake/roles.yml) Permifrost file are removed on a weekly basis.
...
The table below shows a mapping of how models stored within folders in the [`models/`](https://example_company.com/example_company-data/analytics/-/tree/master/transform/snowflake-dbt/models) directory in the analytics project will be materialized in the data warehouse.

The source of truth for this is in the [`dbt_project.yml` configuration file](https://example_company.com/example_company-data/analytics/-/blob/master/transform/snowflake-dbt/dbt_project.yml).
...
- Qualtrics, [Link to the MR](https://example_company.com/example_company-data/analytics/-/merge_requests/7676)
- Clari, [Link to the MR](https://example_company.com/example_company-data/analytics/-/merge_requests/7655)
...
The extracts we do for some [yaml files](https://example_company.com/example_company-data/analytics/tree/master/extract/gitlab_data_yaml) can also be thought of as snapshots. This extraction works by taking the full file/table and storing it in its own, timestamped row in the warehouse. This means we have historical snapshots for these files/tables but these are not the same kind of snapshot as dbt. We'd have to do additional transformations to get the same `valid_to` and `valid_from` behavior.
...
This can be data manipulation action done by a Example Company Team member or by services with access to the data in Snowflake. Some examples are accidentally dropping/truncating a table or running incorrect logic in a transformation.
...
The data retention period is set via dbt This should be implemented in code via a dbt post-hook [example](https://example_company.com/example_company-data/analytics/-/blob/b898087672bfeb3e6329d76696de220fc4b9b2a9/transform/snowflake-dbt/dbt_project.yml#L658).

The following set of rules and guidelines applies to backing up data/using time travel:

- **It is the responsibility of the [CODEOWNER](https://example_company.com/example_company-data/analytics/-/blob/master/CODEOWNERS) to ensure that the backup processes has been correctly implemented for the data that their code builds or maintains.**
...
Once a table is permanent with a retention period we are able to use [Time Travel (internal runbook)](https://example_company.com/example_company-data/runbooks/-/blob/main/data_restoration/time_travel.md) in the event we need to recover one of these tables.

##### Unavailability of the Snowflake environment

For the unlikely event that Snowflake becomes unavailable for an undetermined amount of time, we additionally backup the any business critical data, where Snowflake is the primary source, to Google Cloud Storage (GCS). We execute these backup jobs using dbt's [`run-operation`](https://docs.getdbt.com/docs/using-operations) capabilities. Currently, we backup all of our **snapshots** daily and retain them for a period of 60 days (per GCS retention policy). If a table should be added to this GCS backup procedure it should be added via the [backup manifest](https://example_company.com/example_company-data/analytics/-/blob/master/dags/general/backup_manifest.yaml).
...
In order to make it easy for anyone to send data from Snowflake to other applications in the Example Company tech stack we have partnered with the Enterprise Applications Integration Engineering team to create this data integration framework, which we are calling Data Pump.
...
**Step 2:** Add Model to [`pumps.yml`](https://example_company.com/example_company-data/analytics/-/blob/master/pump/pumps.yml) using the 'Pump Changes' MR template with the following attributes:
...
- owner - your (or the business DRI's) Example Company handle

**Step 3:** Create an [issue in the platypus project](https://example_company.com/example_company-com/business-technology/enterprise-apps/integrations/platypus/-/issues/new) using the 'change' issue template so that the Integration team can map and integrate the data into the target application.
...
The [Daily Data Science Scores Pump](https://example_company.com/example_company-data/analytics/-/blob/master/pump/pumps.yml?ref_type=heads#L20) and the [Pump Churn Forecasting Scores Pump](https://example_company.com/example_company-data/analytics/-/blob/master/pump/pumps.yml?ref_type=heads#L26) are two specific use-cases of the data pump, used to bring data science related data from Snowflake into S3, so that it can be picked up by Openprise and loaded into Salesforce.

The source model for the Daily Data Science Scores pump called [mart_crm_account_id](https://example_company.com/example_company-data/analytics/-/blob/master/transform/snowflake-dbt/models/marts/pumps/mart_crm_account_id.sql?ref_type=heads) contains a combination of [PtE](https://example_company.com/example_company-data/data-science-projects/propensity-to-expand) and [PtC](https://example_company.com/example_company-data/data-science-projects/propensity-to-contract-and-churn) scores, while the Churn Forecasting Scores pump source model [mart_crm_subscription_id](https://example_company.com/example_company-data/analytics/-/blob/master/transform/snowflake-dbt/models/marts/pumps/mart_crm_subscription_id.sql?ref_type=heads) contains scores strictly related to the [Churn Forecasting](https://example_company.com/example_company-data/data-science-projects/churn-forecasting) model.
...
We use [Tableau](https://www.tableau.com/) as our Data Visualization and Business Intelligence tool. To request access, please follow submit an [access request](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/new?issuable_template=New_Access_Request). Use the template [Tableau_Rquest](https://example_company.com/example_company-com/team-member-epics/access-requests/-/blob/master/.example_company/issue_templates/Tableau_Request.md) for Tableau access requests.
...
- [Snowflake Spend ️❄](https://10az.online.tableau.com/t/example_company/views/SnowflakeSpend/SnowflakeSpend)

## <i class="fas fa-user-lock fa-fw" style="color:rgb(107,79,187); font-size:.85em" aria-hidden="true"></i>Security

### Passwords

Per Example Company's password policy, we rotate service accounts that authenticate only via passwords every 90 days. A record of systems changed and where those passwords were updated is kept in [this Google Sheet](https://docs.google.com/spreadsheets/d/17T89cBIDLkMUa3rIw1GxS-QWFL7kjeLj2rCQGZLEpyA/edit?usp=sharing).

We also rotate Snowflake user passwords the first Sunday of every 3rd month of the year (January, April, July, October) via the [Snowflake Password Reset DAG](https://example_company.com/example_company-data/analytics/blob/master/dags/general/snowflake_password_reset.py).
...
Much like Google Drive all Example Company team members have access to Google's [Data Studio](https://datastudio.google.com/) which can be used to build dashboards with data from Google Sheets or other Google data sources. Hence there is no access request needed to get access provisioned to Google Data Studio.
...
The sharing and access process in Data Studio is comparable to sharing in Google Drive / Google Docs. Google Studio Objects can be shared with individuals in our Example Company organization account or with the Organization as a whole. There are no group or role level permissions available. Given the decentralized quality of managing dashboards and data sources in Data studio it is advised that business critical data and reporting be eventually migrated to Snowflake and Tableau. This is made easy with the use of [sheetload](https://internal.example_company.com/handbook/enterprise-data/platform/pipelines/#sheetload) or FiveTran, which has a BigQuery connector.

A Example Company Team Member that creates any artifacts in Google Studio owns the owner permissions of that particular object. With the ownership the Example Company Team Member holds responsibility to keep data [SAFE](/handbook/legal/safe-framework/) within Example Company and outside the organization. Google Data Studio currently doesn't provide an admin interface that can take over the ownership. Upon off-boarding any ownership of existing objects should be carried over to ensure business continuity by the respective object owner. Note that [Red Data](/handbook/security/data-classification-standard.html#red) should never be stored or transmitted within Google Data Studio.
...
2. The Sales Analyst uploads the notebook and its respective query in the corresponding folder, depending on what schedule the notebook should run on. The available schedules (and therefore folders) under https://example_company.com/example_company-data/analytics/-/tree/master/sales_analytics_notebooks are:
...
The code for the dags can be found in the [Sales Analytics Dags in the example_company-data/analytics project](https://example_company.com/example_company-data/analytics/-/tree/master/dags/sales_analytics).

#### Example

Currently, under the `/daily/` notebooks we have [one sample notebook and its corresponding query](https://example_company.com/example_company-data/analytics/-/tree/master/sales_analytics_notebooks/daily).
...
For that the Sales Analyst can either open an MR directly into the [example_company-data/analytics](https://example_company.com/example_company-data/analytics) project, or create an issue on this project and a data platform engineer will implement the necessary dbt models.

In order to change the desired day of the week/time of these schedules, the Sales Analyst can open an issue on the [example_company-data/analytics](https://example_company.com/example_company-data/analytics) project.
...
- If the errors seem to be platform-related, the Sales Analyst can reach out to the data platform engineers either via Slack (via the `#data-engineering` channel), or by opening an issue on the [example_company-data/analytics](https://example_company.com/example_company-data/analytics) project

### GSheets & Jupyter Notebooks

A couple of new functions have been added to the Gitlabdata library ([Link to PyPi](https://pypi.org/project/gitlabdata/), [Link to the source code](https://example_company.com/example_company-data/example_company-data-utils/-/tree/master)) to allow reading from and writing to GSheets files.

#### Reading from GSheets within Jupyter Notebooks

The function is called `read_from_gsheets`([link to function source code](https://example_company.com/example_company-data/example_company-data-utils/-/blob/master/gitlabdata/orchestration_utils.py#L418)) and it accepts a `spreadsheet_id` and a `sheet_name` as parameters, it returns a `dataframe`.
...
The function is called `write_to_gsheets`([link to function source code](https://example_company.com/example_company-data/example_company-data-utils/-/blob/master/gitlabdata/orchestration_utils.py#L378)) and it accepts a `spreadsheet_id`, a `sheet_name` and a `dataframe` as parameters.
...
> :warning: The specific sheet should be shared with the service account user's email (`data-team-sheets-sa@example_company-analysis.iam.gserviceaccount.com`)  prior to calling this function, otherwise the account won't be able to write to or read from the sheet.
...
The GCP team can support with the creation of the user / GCP project. [Here is an example of the issue to create the Service Account for the Revenue Strategy and Analytics team](https://example_company.com/example_company-com/team-member-epics/access-requests/-/issues/20769).
...
- Update the repository URL for the sales analytics notebooks ([link to issue](https://example_company.com/example_company-data/analytics/-/issues/14945))
...
More detail on this use-case can be found in the original issue [#15456](https://example_company.com/example_company-data/analytics/-/issues/15456).
...
We created a runbook with a step-by-step guide on how to create the user and role for this purpose - [link to the Snowflake API User](https://example_company.com/example_company-data/runbooks/-/tree/main/snowflake_api_user) runbook.
