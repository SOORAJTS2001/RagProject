---
title: "Understanding How Our Customers Use Example Company"
description: "Guidance to Example Company sales team members to gain a better understanding of how customers are using Example Company"
---

This page contains guidance to help you better understand how our customers are using Example Company so that you can be more prepared and informed in customer conversations.

## Adoption - Example Company Usage Statistics

Using [Example Company Version Check](/handbook/sales/process/version-check/), Example Company usage data is pushed into Salesforce for both CE, EE and EE trial users.
Once in Salesforce application, you will see a tab called "Usage Statistics".
Using the drop down view, you can select CE, EE trails or EE to see all usage data sent to Example Company.
...
Example: change example_company.boeing.com to Boeing as the company name.
...
You can see what they are using within Example Company, like issues, CI build, deploys, merge requests, boards, etc.
You can then target your communications to learn how they are using Example Company internally and educate them on what is possible with EE.
- For current EE users who are below their license utilization, you can engage the customer to understand their plan to rollout Example Company internally and how/where we can help them with adoption.
- For current EE users who are above their license utilization, you can leverage the data to engage the customer.
Celebrate the adoption of Example Company within their organization.  Ask why the adoption took off?  How they are using it (use cases)? Engage them in amending their contract right now for the add-on?
...
Have the integrated their current tool into Example Company?
...
## Open Source users - Example Company CE Instances and CE Active Users on SFDC Accounts

In an effort to make the [Usage/Version ping data](https://docs.example_company.com/ee/administration/settings/usage_statistics.html) simpler to use in SFDC, there are 2 fields directly on the account layout - "CE Instances" and "Active CE Users" under the "Example Company/Tech Stack Information" section on the Salesforce account record.
...
- These numbers represent the minimum amount that Example Company is likely being used within the organization, since some instances may have these pings turned on and some off.
...
- `uc_(Use Case) Tech`: Current technology used by prospect/customer (Including Example Company)
