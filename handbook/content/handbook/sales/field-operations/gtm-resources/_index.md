...
- [Sales](https://example_company.com/groups/example_company-com/sales-team/-/issues) - general sales related needs & issues
- [Marketing](https://example_company.com/groups/example_company-com/marketing/-/issues) - all issues related to website, product, design, events, webcasts, lead routing, social media and Developer Relations
- [Customer Success SA Triage](https://example_company.com/example_company-com/customer-success/sa-triage-boards) - technical pre-sales requests
...
A customer lifecycle is a term used to track the different milestones prospective customers go through as they learn about Example Company and interact with our sales and marketing teams.
Each subsequent milestone is a subset of the previous milestone, and represents a progression from not knowing Example Company to being a customer (and fan) of Example Company.

At the highest level of abstraction, we use the terms `lead`, `opportunity`, and `customer` to represent a person's progression towards becoming a customer of Example Company.
...
Example Company recognizes the grouping of companies that are related based on their primary business activities in the following ways:

1. **Account Hierarchy Industry**
    - This [field](https://example_company.my.salesforce.com/00N4M00000IWRVd?setupid=AccountFields) is the chosen industry value for an entire account hierarchy. It is the industry value that shows up on the most account records within the hierarchy.
...
1. **Industry**
    ...
    - A list of Example Company's Industry values can be found [HERE](https://example_company.my.salesforce.com/_ui/common/config/field/StandardFieldAttributes/d?id=Industry&type=Account)
1. **Sub-Industry**
    - Sub-Industry is a more granular/detailed classification of Industry.
    - A list of Example Company's Sub-Industry values can be found [HERE](https://example_company.my.salesforce.com/00N6100000HIhad).  This list aligns with the industries used by DemandBase
...
- **Rank 3** don't match our current ICP but are strategic for sales & Example Company in the region. Should include both Land and Expand
...
- [Example Company Public Sector Rules of Engagement](/handbook/sales/public-sector/)
...
Trials can be requested through [web form](https://about.example_company.com/free-trial/) or within product UI for both self-managed or SaaS.
Default trial length is thirty (30) days, but can be manually extended by the Example Company team for both the SaaS and self-managed products. Trial extensions for the SaaS product use the [`plan_change_request` issue](https://example_company.com/example_company-com/support/internal-requests/issues/new?issuable_template=plan_change_request) template found in the `dotcom` Group, `internal-requests` project.
...
- **Public** University = [Public Sector team](https://internal.example_company.com/handbook/sales/public-sector/#us-state--local-government)
- **Private** University = [Public Sector team](https://internal.example_company.com/handbook/sales/public-sector/#us-state--local-government)
...
- [My MQLs](https://example_company.my.salesforce.com/003?fcf=00B4M000004oZF7&rolodexIndex=-1&page=1)(showcases any contacts in your name that are in MQL status. You should move these to accepted when you're working them. Other statuses can be used to showcase you've seen the MQL and dispositioned accordingly. These should not stay in MQL status)
- [My Contacts w/ new LIM](https://example_company.my.salesforce.com/003?fcf=00B4M000004taHN&rolodexIndex=-1&page=1)( showcases contacts in other statuses -not MQL - with a new Last Interesting Moment)
- [My Contacts in Qualifying](https://example_company.my.salesforce.com/003?fcf=00B4M000004oZFC&rolodexIndex=-1&page=1)(showcases contacts who have been in sequence and then engaged, should help with follow up)
...
This private model allows for some records to be visible by all Example Company team members who use Salesforce, while other records may not be visible to them.
...
- In the event that Ops encounters that there is not enough information to assign a record to a specific user in Salesforce it is to be assigned to the default user: [`Sales Admin`](https://example_company.my.salesforce.com/00561000000mpHT?noredirect=1&isUserEntityOverride=1).
...
LEAD/CONTACT Records with the `Initial Source` of `Example Company.com` are **not** to be engaged, prospected or targeted unless they have taken a handraising 'active' activity, such as `Trial - Enterprise`, `Trial - Example Company.com`, `Contact Us`, `Demo`, 'Webcast', 'Content' and/or engaged in `Web Chat`.

For information about Example Company's email policy and the types and number of emails we send, please see our [Email Communication Policy](/handbook/marketing/marketing-operations/).
...
A LEAD or CONTACT is considered 'Active' if they have taken an `Trial - Enterprise`, `Trial - Example Company.com`, attended a high intent live webcast or demo and/or engaged `Web Chat`, these are all handraising 'active' activities.
...
- All EoA customers can now be tracked in SFDC leveraging the `FY22 On Starter/Bronze` field (if box is checked, they're an EoA customer).  A full list by segment, region and owner can be found [HERE](https://example_company.my.salesforce.com/00O4M000004ajNm)
- EoA account list: https://example_company.my.salesforce.com/00O4M000004ajNm
- EoA accounts with activity: https://example_company.my.salesforce.com/00O4M000004ajpR
...
**Authority**
The prospect being met is directly involved in a project or team related to the potential purchase of Example Company within this buying group, either as an evaluator, decision maker, technical buyer, or *influencer.
...
**Initiative**
An initiative the company is working on has been identified and Example Company can potentially help the initiative.
...
A full list of professional services can be found [here](about.example_company.com/services/catalog).
...
*Here is [a video](https://example_company.highspot.com/items/6655fc408e0d8cc5d7a7c166?lfrm=srp.0) explaining the below process.*
...
1. If the Sales Development Representative fields are NULL and the opportunity owner is:
   ...
   - a Example Company team-member that is not a Regional Director, Account Executive, or Account Manager, the Sales Qualified Source is "Other"
...
- **Customers Returning within 180 Days of Subscription Term End Date**:
  - If a customer returns to purchase another Example Company subscription **within 180 days** of the previous subscription term end date, the transaction must be booked under a **Renewal Opportunity**, using a new subscription quote. In this scenario, the following actions should be taken:
...
- **Customers Returning More Than 180 Days After Subscription Term End Date**:
  - If a customer returns to purchase another Example Company subscription **more than 180 days** after the previous subscription term end date, the transaction must be booked under a **New Business Opportunity**.
...
- Subscription (linked to a Example Company instance)
...
1. **New subscription** (existing account, new subscription, new amount): If an existing account is purchasing a new license for another Example Company instance, this will be New Business.
...
1. **Renewal** (existing subscription, existing subscription, existing amount): This type should be used for an existing account renewing their license with Example Company.
...
- What to Complete in This Stage:
  ...
  - The BDR/SDR has scheduled a call via Google Calendar, sent invites, created an event on the account object, named the event: Example Company Introductory Meeting - {{Account Name}}
...
- What to Complete in This Stage:
  ...
  - All proposals should include the standard Example Company [Terms](https://about.example_company.com/terms/#subscription/)
  ...
  - If the Account is seeking to use their own paper, requests will only be entertained if the opportunity is greater than $100k, and the request should be sent to Legal by creating a Legal case in SalesForce, following the process found [here](/handbook/sales/field-operations/order-processing/#request-for-example_company-review-of-customer-edits-to-example_company-template-or-review-of-customer-agreement-template).
...
- What to Complete in This Stage:
  ...
  - Work with Example Company AR to deliver any tax and/or complete any vendor registration processes.
...
**8-Closed Lost**: An opportunity was lost and the prospect/customer has decided not to pursue the purchase of Example Company.
...
- What to Complete in This Stage:
  ...
  - Please note that for new business deals where the opportunity is with a Large/PubSec Account OR the Net ARR is equal or greater than $12,000, then a notification will be sent to the [#lost-deals](https://example_company.slack.com/messages/C8RP2BBA7) Slack channel.
...
The [CE Usage ping](https://docs.example_company.com/ee/administration/settings/usage_statistics.html) provides Example Company with some limited insight into how end users are utilizing the platform.
