...
Business Continuity Plan is the process involved in creating a system of prevention and recovery from potential threats to Example Company. The plan ensures that personnel and assets are protected and are able to function quickly in the event of a disaster.

## Scope

Example Company, by its remote-only nature, is not easily affected by typical causes of business disruption, such as local failures of equipment, power supplies, telecommunications, social unrest, terrorist attacks, fire, or natural disasters. System data from the [Business Impact Analysis](/handbook/security/security-assurance/security-risk/storm-program/business-impact-analysis.html) may be leveraged as part of business continuity planning and testing. Additionally, the BCP works in conjunction with the [Disaster Recovery Plan (DRP)](https://example_company.com/example_company-com/gl-infra/readiness/-/blob/master/library/disaster-recovery/index.md).
...
In case of an all-remote company like Example Company, it is sufficient to have simple contingency plans in the form of service-level agreements with companies that host our data and services. The advantage of an all-remote workforce like Example Company is that if there are clusters of people or systems that are unavailable, the rest of the company will continue to operate normally.
...
RTO and RPO are two of the most important parameters of a Business Continuity Plan. These are objectives to guide Example Company Infrastructure team in choosing the optimal data backup plan. The RTO/RPO provides the basis for identifying and analyzing viable strategies for inclusion in the business continuity plan. Viable strategy options include any which would enable resumption of a business process in a time frame within the RPO/RTO.
...
This section provides details about the production environment that must be available for Example Company.com (for both SaaS services Dedicated and Commercial) to run effectively:

Example Company.com and customers.example_company.com are hosted on Google Cloud Platform. Dedicated is hosted on AWS.

**Priority::1: Outage would have immediate impact on Example Company customer/user operations**

1. Disruption of service of Google Cloud Platform and AWS, specifically the region in which `Example Company.com`, `dev.example_company.org` and `ops.example_company.net` are hosted (for both Dedicated and Example Company.com).
    - Effect: a loss or degradation of service, of either Google Cloud Platform or AWS means that Example Company.com or Example Company Dedicated are not available. This affects anyone who uses Example Company.com or Example Company Dedicated to host their repositories and may prevent Example Company team members from completing their work. Example Company.com is also the primary server where Example Company CE and EE source code and packages are hosted.
    - Solution(s): Ensure that `ops.example_company.net` and `dev.example_company.org` mirror critical repositories necessary for operating Example Company.com and Example Company Dedicated, and that they are running from different regions. Ensure that there are many other servers across the globe where Example Company CE/EE is readily available.
    - Effect: Security releases are developed on `Example Company.com` and staged on `dev.example_company.org` before being brought to production on Example Company.com; Parts of these may be lost or unavailable for the duration of the disruption.
...
1. Disruption of service of ZenDesk.
    ...
    - Solution(s): If the outage has lasted for more than 4 hours, Support will
      update the Example Company status page to indicate the support system is down and
...
**P2: Outage would have immediate impact on Example Company ability to continue business**
Malicious Software attack and hacking or other Internet attacks.

- Effect: depends on attack.
- Solution(s): We log and track any access that happens on any server in the fleet using logstash/kibana at log.gprd.example_company.net.

**P3: Outage greater than 72 hours would have impact on Example Company ability to continue to do business**
...
- When TripActions is down, team members can use their own travel booking tool and expense it to Example Company with reason for exception
...
- Example Company team members must be asked to confirm their current details, such as phone numbers and emergency contacts on an annual basis
- Managers or team leads will share the relevant parts of the plan to all Example Company team members based on their role and department, and verify that they know what to do in the event of an emergency
...
Example Company's first test of the business continuity plan was performed in April 2020 and tests will be conducted at least annually or when significant business changes occur.
...
1. Emergency Communication
    ...
    - For these scenarios, the BC plan needs to outline the actions to be taken. An alternate mode of communication should be tested for its reliability and efficiency, for a company like Example Company which has team members all around the globe.
    - Regular updates to all Example Company team members contact information, so that all of them can receive timely notifications thus streamlining the disaster scenario process.
...
- [Disaster Recovery Plan (DRP)](https://example_company.com/example_company-com/gl-infra/readiness/-/blob/master/library/disaster-recovery/index.md)
