...
Each program should have an operating rhythm including when the core team meets synchronously and the executive team meets synchronously. The Program Manager is responsible for providing an async structure to document completion of tasks and report status via Example Company Epics.
...
{{% alert color="warning" %}}
The IT PMO team uses the [Example Company Roadmap](https://docs.example_company.com/ee/user/group/roadmap/#roadmap) feature to track the progress of each program.
{{% /alert %}}
...
1. Control - PC2: Changes are tested and approved by appropriate personnel in accordance with the change management policy.
   ...
   - While the work to execute the control can be assigned to the vendor, the ultimate responsibility for effective execution of the control lies with Example Company. (i.e. if the vendor executes the control on behalf of Example Company inadequately, Example Company may still be required to report the deficiency of the SOX control)
...
{{% alert color="warning" %}}
[SOX Compliance Template](https://example_company.com/example_company-com/business-technology/enterprise-apps/intake/-/issues/772)
{{% /alert %}}
