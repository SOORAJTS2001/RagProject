...
[Cloud Licensing](https://about.example_company.com/pricing/licensing-faq/cloud-licensing/) allows Example Company customers to activate paid subscription features on self-managed instances using an activation code, rather than managing license files and manually uploading them into their instance. By activating through Cloud Licensing, a customer's Example Company instance will periodically synchronize [Subscription Data](https://docs.example_company.com/ee/subscriptions/self_managed/#subscription-data-that-example_company-receives) with Example Company.

In order to further encourage Cloud Licensing, the Strict Cloud Licensing project will include several iterations to encourage customers to use Cloud License activation codes rather than a Legacy License file to activate their self-managed instances. The following [Example Company internal licensing terms page](https://internal.example_company.com/handbook/product/fulfillment/definitions/#licensing-terms) provides a technical definition of the current three types of licenses (Cloud Licensing, Offline and Legacy) associated with Cloud Licensing.

## Strict Cloud Licensing

**Effective July 7, 2022:** Cloud Licensing will be enabled by default for all new and renewal customers, as detailed in the [Strict Cloud Licensing roll out plan](https://example_company.com/example_company-org/example_company/-/issues/351682) (except OSS, EDU or Start-Up products). Customers will no longer have the ability to download a license file from the Customers Portal. In addition, Offline licenses will now be available to allow customers with air-gapped or offline instances to receive the benefits of Cloud Licensing. In order to receive a Legacy License or Offline License, customers will need to follow the below outlined process.
...
1. Using the `Sold To Email`, search for the customer account in CustomersDot [Customers](https://customers.example_company.com/admin/customer) page.
...
1. Go back to the IR ticket and note the `Example Company Version` value:
...
1. Navigate to the [Licenses](https://customers.example_company.com/admin/license) page.
...
- Offline licenses are preferred by Example Company over Legacy Licenses as they allow the customer to more easily provide usage data. These work best for customers who cannot use a Cloud License because their instance is air-gapped or not connected to the internet. However, in order to use an Offline Cloud License, the customer must be on 15.0 or higher.
- Legacy licenses can be sent when a customer is unwilling or unable to upgrade to Example Company version 14.1 or higher, or is concerned about sharing Subscription Data.
...
Please see [this comment thread](https://example_company.com/example_company-org/fulfillment-meta/-/issues/610#note_1052615060) for greater context on this workaround.
...
- Web direct: If it is the **Fulfillment API User** (`svc_zuora_fulfillment_int@example_company.com` or `ruben_APIproduction@example_company.com`).
- Not web direct: If it either the **SalesForce API User** (`svc_ZuoraSFDC_integration@example_company.com`) or a Example Company team member's email (mostly it would be a Billing team member).
...
### How does Cloud Licensing impact customers who have purchased multi-year Example Company subscriptions?
...
1. [Cloud Licensing Internal Handbook Page](https://internal.example_company.com/handbook/product/fulfillment/cloudlicensing/cloud-licensing/)
1. [Offline Cloud Licensing Internal Handbook Page](https://internal.example_company.com/handbook/product/fulfillment/cloudlicensing/offline-cloud-licensing/)
