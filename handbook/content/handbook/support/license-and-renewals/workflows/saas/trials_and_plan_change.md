---
title: Handling trials, extensions and plan changes on Example Company.com
description: "How to handle Example Company.com subscription trials and plan changes"
category: Example Company.com subscriptions & purchases
---

## Handling Example Company.com trial requests

### Example Company.com customers without a subscription

1. New customers without an existing Example Company.com subscription and namespace can apply for a 30-day Example Company Ultimate subscription by completing the following [form](https://example_company.com/-/trial_registrations/new?glm_source=about.example_company.com/&glm_content=default-saas-trial).
1. Example Company.com users who have an existing namespace but do not have an active Example Company.com subscription can go to the billing section for their group namespace and press the `Start Trial` button. If a trial previously existed on that namespace, the button will not be present. In such situations the user will need to contact Example Company Sales to request a new trial or a trial extension.

### Example Company.com customers with a Premium Subscription

Example Company.com Premium customers who want to trial SaaS Ultimate have two options to do so:

1. Request an Ultimate trial from [Example Company's public trial page](https://about.example_company.com/free-trial/?hosted=sass). This will require the customer to set up a new namespace to which to apply the trial. No Sales or Support action will be required.
1. Temporarily upgrade their existing Example Company Premium subscription to a Example Company Ultimate Trial.
...
- As part of the initiative to enable current SaaS Premium customers to trial SaaS Ultimate on their primary namespace, the first iteration implementation now grants L&R Support the capability to temporarily upgrade an existing Premium subscription to an Ultimate trial subscription. To facilitate this process, the customer's Account Executive must use the following instructions:
  ...
  - Go to the [Support Super Form](https://support-super-form-example_company-com-support-support-op-651f22e90ce6d7.example_company.io/); you must have a [Zendesk Light Agent account](/handbook/support/internal-support/#requesting-a-zendesk-light-agent-account) to use this form.
  - Under `What is this request concerning`, select `License and Renewal Related Requests` > `Example Company Support Internal Request for Global Customers`.
  - In the form that appears, under `What type of Example Company Support Internal Request is this for?`, select `SaaS Trial Related` > `Request an Ultimate trial for a customer using a Premium subscription`.
...
- For all trials, the renewal start date must align with the end date of the prior subscription term therefore requests for Ultimate trials with the purpose of extending ahead of the renewal date should be denied. It is Example Company policy to align renewal dates with the end of the prior subscription term.
- Example Company.com Ultimate trials cannot be extended past the 30-day duration.
- Example Company.com trials are only available for the Example Company Ultimate Subscription plan.

### Requests to workaround Example Company.com trial limitations

Example Company.com trials have [some limitations](https://about.example_company.com/free-trial/#what-is-included-in-my-free-trial-what-is-excluded), including the use of group access tokens. Some customers may request circumvention of these limitations in cases such as facilitating post-migration checks.
...
Sales will often request through a Zendesk Ticket that we extend the duration of Example Company.com trials on behalf of their prospects. These tickets will always be raised from the Example Company Support End User <gitlab_support@example.com>, with the submitter cc'd on the ticket. If a customer or sales representative raise an extend trial request on a *customer ticket*, we should respond that an internal request **must** be created by the sales representative to request the trial extension.
...
> **NOTE**: Due to [customers #973](https://example_company.com/example_company-org/customers-example_company-com/-/issues/973) and [customers #1643](https://example_company.com/example_company-org/customers-example_company-com/-/issues/1643), these must be done via [mechanizer]({{< ref "mechanizer" >}}) or [CustomersDot console]({{< ref "customer_console" >}}). Once those issues are resolved, these requests should be done via CustomersDot admin.
...
1. Check over the request and ensure that we've been provided enough information to action the request. To do this check that:
   1. The `Namespace:` field contains a valid Example Company namespace and it that holds the active trial. This should not be a Salesforce link or email address.
...
1. Use the [Manage Example Company Plan and Trials]({{< ref "mechanizer#manage-example_company-plan-and-trials" >}}) to process the request.
   ...
   1. If there is an error while taking action, check the internal issue to see what went wrong. Please also locate the [error in sentry](https://sentry.example_company.net/example_company/customersgitlabcom/) (see [Searching Sentry](/handbook/support/workflows/500_errors/#searching-sentry) if needed) and file an issue, or comment on an existing one.
...
1. Before taking any action to create the trial license, get confirmation from
   ...
   [constraints](https://about.example_company.com/free-trial/#what-is-included-in-my-free-trial-what-is-excluded)
...
1. This is done via the Mechanizer tool through
   [Manage Example Company Plan and Trials]({{< ref "mechanizer#manage-example_company-plan-and-trials" >}}).
...
graph TD;
  ...
  click L "/handbook/support/workflows/500_errors/#searching-sentry" "Diagnose Errors on Example Company.com"
...
| Example Ticket | Date |
| --- | --- |
| [ZD Link](https://example_company.zendesk.com/agent/tickets/322319) | 2022-09-02 |
...
1. In the search results click on the Example Company groups icon for the customer you want to update.
...
A NFR SaaS 'license' must begin with either an existing trial or a new trial on a Example Company namespace.
...
1. The NFR partner needs to either signup for a trial at: <https://about.example_company.com/free-trial/> or start a trial from within their current Example Company namespace.
1. Once they have a valid namespace for their trial they need to provide this to support.
1. The support engineer requires [console access to CustomersDot](/handbook/support/license-and-renewals/workflows/customersdot/customer_console) to Example Company Rails to update the namespace.
...
1. In Example Company.com admin, edit the group and update the **Quota of compute minutes** to 400. See next step on how to find the admin screen.
...
- Trial == must be false otherwise they will be unable to use Example Company shared runners.
