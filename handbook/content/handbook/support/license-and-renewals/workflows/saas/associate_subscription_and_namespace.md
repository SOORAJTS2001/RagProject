...
description: "How to provision Example Company.com subscriptions"
category: Example Company.com subscriptions & purchases
---

## Provisioning subscriptions for reseller customers

Customers who purchase from Example Company Partners, resellers, AWS, and/or GCP have **read-only** access to their subscriptions in [Customers Portal](https://customers.example_company.com/customers/sign_in).
...
While handling the subscription provisioning requests, we might encounter a 502 error while impersonating the user on the customer portal, this occurs when an admin accidentally links their Example Company.com admin account with the customer portal account, due to which the customer portal tries to fetch all the groups that are accessible to the admin user on Example Company.com and eventually returns a 502 error.

- We can verify that by [retrieving the token information](https://docs.example_company.com/ee/api/oauth2.html#retrieving-the-token-information)(grab the `access_token` from the customer portal and call the API endpoint to retrieve the `resource_owner_id` attribute, which should be the same as the  Example Company.com `userID`).

To fix this, we should **completely unlink the Example Company.com account with the customer portal** account using the [unlink_customer console function](/handbook/support/license-and-renewals/workflows/customersdot/customer_console#unlink_customer) or `Unlink Example Company.com account` form in the ZenDesk ticket Mechanizer app.
...
- If the namespace has more active users than the number of seats in the subscription, the system redirects to the payment page to purchase additional seats to match the Example Company.com Group's active user count, in this scenario if the customer is not interested in purchasing additional seats upfront then we can force associate the subscription and the additional seats will be reflected as true-ups on the group's billing page.
...
Please note: when using the force associate tool, ensure that a example_company.com user (with owner role in the relevant namespace) has been linked to the customersdot account that the subscription is associated with. If an association is made, but no example_company.com user is linked, then any subsequent changes to the subscription will either not reflect on the namespace in example_company.com or it will downgrade the namespace to Free.
...
[Clear subscription form](https://example_company-com.example_company.io/support/toolbox/forms_processor/LR/clear_subscription) is used to clear the subscription associated with that namespace. Please follow the below steps:

Navigate to the [Clear subscription form](https://example_company-com.example_company.io/support/toolbox/forms_processor/LR/clear_subscription) page -> enter the ```Subscription Name``` and submit the form. This will create an internal issue and updates the issue description with the response of the force association script
...
If the user has access to and sees a subscription in CustomersDot but doesn't see the paid plan details showing on their billing page on Example Company.com, have the user associate the group with the subscription via CustomersDot.

Associating a group with a subscription in CustomersDot:

1. Sign in to the [Example Company Customers Portal](https://customers.example_company.com/customers/sign_in).
...
1. Until [multiple active orders on a namespace](https://example_company.com/groups/example_company-org/-/epics/9486) are supported, a namespace is grayed out because it already has a subscription associated to it. This could be a recently expired subscription, or a consumption subscription.
