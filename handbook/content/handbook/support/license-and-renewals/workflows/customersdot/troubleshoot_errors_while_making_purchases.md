...
This guide is for troubleshooting errors on [CustomersDot](https://customers.example_company.com/customers/sign_in).
...
[CustomersDot tracker](https://example_company.com/example_company-org/customers-example_company-com/-/issues/) as necessary.

### Getting error message from Sentry

To find the error specifically related to a customer on sentry, try the following:

1. Get customer ID from [CustomersDot](https://customers.example_company.com/customers/sign_in):
   - Log into <https://customers.example_company.com/admin>
   ...
   - Take the customer ID from the URL <https://customers.example_company.com/admin/customer/customerID/pagepath>
1. Finding the error message for the customer in Sentry:
   - Go to <https://sentry.example_company.net/example_company/customersgitlabcom/issues/>
...
You can access the CustomersDot production logs or staging logs in [Kibana](https://log.gprd.example_company.net/) by filtering using `prdsub*` or `stgsub*` respectively.
...
Login to [GCP Logs Explorer dashboard](https://console.cloud.google.com/logs/query?project=example_company-subscriptions-prod) (make sure you are viewing `example_company-subscriptions-prod`).
...
Sometimes a customer reports that they did not receive an email from the portal, such as a password reset request or new account confirmation.  Example Company uses Mailgun as a service to send outgoing mail. We can login to Mailgun to view the message logs, and when appropriate, remove suppressions.
...
The process here is much the same, but since we are investigating mail from CustomerDot, in Step 4 of the general workflow ensure that `customers.example_company.com` is selected as the domain to search under.

To confirm whether the customer has access to customers.example_company.com, navigate to the `Edit` page for the customer record (pen icon on the right side of the customer record) and look at the `Login activated` checkbox. If the checkbox is ticked, the customer has access; if the checkbox is not ticked, the customer will not have access and they will not receive password reset emails.
...
1. Customer cannot purchase new subscription because of an expired subscription:
   - [ZenDesk 162279](https://example_company.zendesk.com/agent/tickets/162279)
   - [Internal request issue 2566](https://example_company.com/example_company-com/support/internal-requests/-/issues/2566)
   - [Sentry Event log](https://sentry.example_company.net/example_company/customersgitlabcom/issues/1181887/events/31651984/)
1. Customer cannot purchase because the `Address`, `City`, and `Postal code` have invalid information:
   - [ZenDesk 167403](https://example_company.zendesk.com/agent/tickets/167403)
   - [Sentry Event log](https://sentry.example_company.net/example_company/customersgitlabcom/issues/1211792/events/32521404/)
1. Customer cannot purchase compute minutes because `State` was not provided in CustomersDot:
   - [Zendesk 318385](https://example_company.zendesk.com/agent/tickets/318385)
