...
This workflow details how to use the Zendesk App that utilizes the [Mechanizer](https://example_company.com/example_company-com/support/toolbox/mechanizer) to automate CustomersDot console related tasks.

The Mechanizer is [currently in maintenance mode](https://example_company.com/example_company-com/support/support-team-meta/-/issues/4299). Our product goal is to migrate the features in Mechanizer to CustomersDot ([epic](https://example_company.com/groups/example_company-org/-/epics/6828)).
...
**Your Example Company Username** will be automatically added to the form.
...
> **WARNING:** Additional compute minutes added this way will last **indefinitely** until used. If the request is to provide "extra compute minutes" during the trial period only, use the [Manage Example Company Plan and Trials option](#manage-gitlabcom-plan-and-trials) which will change the *usage quota* to match a paid plan quota for the duration of the trial.
...
### Manage Example Company.com Plan and Trials

Updates Example Company Subscription or customer trial. You can use it for the following cases:
...
Please note that in order to extend or renew a trial, the customer **MUST** have an active trial because a CustomersDot account needs to exist.  If the prospect has not yet initiated a trial themselves, please have them do so via the [customers portal](https://customers.example_company.com/trials/new?gl_com=true).

These two  fields are mandatory:

- **Namespace**: The customer's namespace as it appears in the URL.  For example, if the customer namespace is located at <http://example_company.com/example_company-com> then the namespace would be `example_company-com`. Please confirm through your own observation and through communication with Sales, the CSM, or the customer that the namespace is the one with the subscription or trial to be updated.
...
- **Sales Manager Approval**: The Example Company.com username (without a preceding @) of the sales manager who approved a trial extension. This approval should be via chatter on the SFDC opportunity.
...
> Note: When a new trial is created, it has the default trial values (30-day, ultimate, 400 compute quota), so you need to re-run "Update Example Company Plan" again with the requested values.
...
### Unlink Example Company.com Account

Unlinks the Example Company.com account that is tied to the CustomersDot account provided.
...
For Self-Managed licences, Example Company will refuse to install a license key with less than the current number of billable users. Therefore, **User Count** for a trial license should *at least* be the same number as the current number of billable users plus any true-ups owed (if any).  For example, if 25 current billable users, and 5 true-ups owed, set **User Count** to at least 30.

### Add project-level storage to a namespace

Sets [additional project storage](https://docs.example_company.com/ee/user/usage_quotas.html#excess-storage-usage) for a namespace to the value specified in the `Extra storage (MiB)` field
...
This will change the total seats owed in the Example Company.com subscription. Before using this option check with a support manager.
