...
This workflow is for frequently used functions that are pre-loaded via the [Customers Console and Functions project](https://example_company.com/example_company-com/support/toolbox/console-training-wheels).
...
Console access is through [Teleport](https://goteleport.com/docs/connect-your-client/introduction/), and requires membership in the relevant Okta groups [documented on the CustomersDot project](https://example_company.com/example_company-org/customers-example_company-com/-/blob/main/doc/setup/teleport.md#group-membership).  You can file an [access Request](/handbook/it/end-user-services/onboarding-access-requests/access-requests/#individual-or-bulk-access-request) to gain membership.

Refer to the [CustomersDot documentation page about Teleport](https://example_company.com/example_company-org/customers-example_company-com/-/blob/main/doc/setup/teleport.md#using-teleport-for-db-rails-access) for a full overview of installing and using Teleport to access the production or staging rails console.
...
```mermaid
graph TD
  A[Console Tasks] --> B(Automate within Example Company)
...
[+] Customer linked to orders
 https://customers.example_company.com/admin/customer/111111
 id                                111111
 company                           example Ltd
 first_name                        Jane
 last_name                         Doe
...
> **Note**: Having a way to view this in the UI has been requested in [customer #3081](https://example_company.com/example_company-org/customers-example_company-com/-/issues/3081).
...
> **Note**: Finding an account based on group name is available in the UI. However, accounts with trials typically do not show up due to [customers #978](https://example_company.com/example_company-org/customers-example_company-com/-/issues/973).

Find a given Example Company.com namespace.
...
> *Note*: This can be deprecated when this is available in the UI which will require [showing expired trials (customers #1173)](https://example_company.com/example_company-org/customers-example_company-com/-/issues/1173), [ability to extend (customers #1643)](https://example_company.com/example_company-org/customers-example_company-com/-/issues/1643), and for [the Example Company account to be tied to customers portal to show Example Company Groups after a trial is initiated (customers #973)](https://example_company.com/example_company-org/customers-example_company-com/-/issues/973).
...
{"id"=>0000000,
 ...
 "parent_id"=>nil,
 "avatar_url"=>nil,
 "web_url"=>"https://example_company.com/groups/example",
...
> *Note*: The ability to do this in the UI is [customers #2165](https://example_company.com/example_company-org/customers-example_company-com/-/issues/2165).
...
| `:namespace_path` | *Yes* | The [unique Example Company namespace](https://docs.example_company.com/ee/user/group/#namespaces) *`path`*|
...
> *Note*: Ability to do this in UI is [customers #2166](https://example_company.com/example_company-org/customers-example_company-com/-/issues/2166).

Completely unlink a Example Company.com account from a CustomersDot account. **Note:** Use the customer ID (from customers portal, not Example Company.com).
...
| Name | Required | Details |
| ------ | ------ | ------ |
| `:customer_id` | *Yes* |Customer ID to be unlinked from it's Example Company account.|
...
### Identify multiple CustomersDot accounts linked to a Example Company account

#### Parameters

| Name | Required | Details |
| ------ | ------ | ------ |
| `:uid` | *Yes* |Example Company.com UID which may have been linked to multiple cdot accounts |

#### Sample

````ruby
*** PRODUCTION *** production> Customer.where(uid: 0000000)
=> [#<Customer 000000, first_name: "Jane", last_name: "Doe", created_at: "2021-12-21 16:29:53.324183000 +0000", updated_at: "2023-01-17 00:56:56.311661000 +0000", email: "jdoe@examplecorp.net", provider: "example_company", uid: "0000000", country: "USA", state: "GA", city: "Columbus", zip_code: "12345", vat_code: nil, company: "ExampleCorp", salesforce_account_id: "000001230043203, skip_email_confirmation: false>,
#<Customer 100009, first_name: "Bob", last_name: "Doe", created_at: "2022-11-12", email: "bdoe@examplecorp.net", provider: "example_company", uid: "0001235", country: "USA", state: "GA", city: "Columbus", zip_code: "12345", vat_code: nil, company: "ExampleCorp", salesforce_account_id: "00000123006988343, skip_email_confirmation: false>]
``````
...
> *Note*: Feature request is [customers #2167](https://example_company.com/example_company-org/customers-example_company-com/-/issues/2167).
...
1. [Entity changes](https://example_company.com/example_company-com/Finance-Division/finance/-/wikis/Process-for-change-of-entity)
1. System bugs
...
## Example Company.com Group methods

These functions help fix various bug issues that have surfaced on Example Company.com. These functions do *not* change anything in the CustomersDot.
...
```ruby
irb(main):035:0> account_seats("example_company-silver")
{"plan"=>
  {"code"=>"silver",
   "name"=>"Silver",
   "trial"=>false,
 ...
 "billing"=>
  {"subscription_start_date"=>"2020-01-01",
   ...
   "trial_ends_on"=>nil}}
irb(main):089:0> account_seats("example_company-silver",0)
{"plan"=>
  {"code"=>"silver",
   "name"=>"Silver",
   "trial"=>false,
...
> *Note*: Possible through Example Company.com admin.
...
> *Note*: Additional minutes can also be changed via chatops. Feature request for editing via UI in [example_company#325917](https://example_company.com/example_company-org/example_company/-/issues/325917).
...
```ruby
irb(main):180:0>  update_group_mins("example_company-gold",50000)
=> {:success=>true}
```

### update_extra_storage

> *Note*: Additional storage can also be changed via API. Feature request for editing via UI in [example_company#325918](https://example_company.com/example_company-org/example_company/-/issues/325918).
...
```ruby
irb(main):180:0>  update_extra_storage("example_company-gold",5000)
{"id"=>12345678,
 "name"=>"Example Company.com - Gold",
 "path"=>"example_company-gold",
 "kind"=>"group",
 "full_path"=>"example_company-gold",
 "parent_id"=>nil,
 "avatar_url"=>"/uploads/-/system/group/avatar/123456/example_company-icon-rgb.png",
 "web_url"=>"https://example_company.com/groups/example_company-gold",
...
- With the decision made on [Mechanizer and Hacks Maintenance mode](https://example_company.com/example_company-com/support/support-team-meta/-/issues/4299) we won't be adding more functions into the mechanizer
