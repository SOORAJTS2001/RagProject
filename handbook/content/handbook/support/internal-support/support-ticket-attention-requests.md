---
title: Example Company Support - Support Ticket Attention Requests (STAR)
description: Process documentation for getting additional attention on a ticket from Support leadership.
---

## Overview

This document outlines the process for managing support tickets that, due to the situation that is critical for the customer's business, require our attention or an initial response more quickly than what is defined in our [standard response time.](https://about.example_company.com/support/#priority-support).

## What is a Support Ticket Attention Request (STAR)?

Support Ticket Attention Requests (STAR) are the mechanism by which Example Company Team Members can request additional attention be placed on tickets. Feel free to pronounce the acronym STAR, and use it as a verb (*I starred this ticket because of a request from the customer*) or a noun (*I put a star on this ticket to make sure an engineer with the right expertise is assigned*).
...
1. [Internal Requests in Zendesk](/handbook/support/internal-support/#internal-requests) created by a Example Company team member for a customer with a license or subscription.
...
Once submitted, an issue is created in the [Support Ticket Attention Request](https://example_company.com/example_company-com/support/ticket-attention-requests/-/issues) issue tracker. The issue is automatically assigned to the [Support Manager on-call](/handbook/support/workflows/support_manager-on-call).

When you STAR a ticket, a thread is created in the [#support_ticket-attention-requests](https://example_company.slack.com/archives/CBVAE1L48) Slack channel.
...
1. When management oversight is required to bring structure and focus to a ticket, and ensure the necessary resources are allocated to it. For example, if:
    - The first-response [SLA](https://about.example_company.com/support/#service-level-agreements) of the ticket has been breached by one hour or more (see [Before you STAR a ticket](#before-you-star-a-ticket)).
    ...
    - The ticket's priority is objectively lower when assessed in relation to the [Support Impact](https://about.example_company.com/support/#priority-support) guidelines.
1. When the progress of a ticket has stalled, and the ticket needs to be elevated to a different engineer (based on their skill set) so that Example Company can resolve the ticket more quickly.

### When is a STAR not appropriate?

1. It is a **business-critical** situation for Example Company where existing or future business is at risk so attention from CSM is needed -> [raise an account escalation](/handbook/customer-success/csm/escalations/#initiating-managing-and-closing-an-escalation)
1. It is an **emergency** -> Advise the customer to [raise an emergency](https://internal.example_company.com/handbook/support/workflows/raising-an-emergency) (internal Handbook link, Example Company team members only; only Premium/Ultimate customers are eligible to trigger emergencies)
1. It is a **SaaS incident** -> [check the Example Company Status Page](https://status.example_company.com/) or [report an incident](/handbook/engineering/infrastructure/incident-management/#reporting-an-incident)
...
1. **Are we waiting for information from the customer, or from you?** Ensure the latest response is *from the customer* (or from a Example Company team member, in the case of Internal Requests). If the latest response is from Example Company Support, we are waiting for necessary information.
1. **Is the ticket within SLA?** Review the SLA associated with the ticket, and the amount of time left until breach, by logging into [Zendesk](https://example_company.zendesk.com) using Okta.
   - If a ticket was opened in the last 0-30 minutes, and has the correct [Severity](https://about.example_company.com/support/definitions/#definitions-of-support-impact), a STAR request is not necessary.
   - It is not typically necessary to STAR an issue that has multiple hours remaining on the SLA timer.
   - Our [SLAs](https://about.example_company.com/support/#service-level-agreements) apply to first-responses to tickets. Example Company Support targets a 95% SLA achievement KPI. This means that some breaching is expected.
...
1. **Is the ticket missing information?**
   - If a ticket doesn't have all the necessary information it won't qualify for STAR, for example:
      ...
      - [Proof of support entitlement](https://about.example_company.com/support/managing-support-contacts/#proving-your-support-entitlement)
...
1. Topics that fall into our general out-of-scope [definition](https://about.example_company.com/support/statement-of-support/#out-of-scope).
...
The Zendesk STAR App is available only during our [Global Support Hours](https://about.example_company.com/support/#hours-of-operation); outside of these hours it will be disabled.
...
- Urgency
- Plan
- Example Company.com group URL or name
...
- The submission time is within [Support hours](https://about.example_company.com/support/#hours-of-operation)
...
| Urgent        | The ticket/issue may become an [emergency](https://about.example_company.com/support/definitions/#definitions-of-support-impact) if no follow-up is provided within the next 30 minutes and may lead to customer dissatisfaction. This escalation is urgent, and the overall impact is `high`.|

### STAR Issue Tracker

Once submitted, an issue is created in the [Support Ticket Attention Request](https://example_company.com/example_company-com/support/ticket-attention-requests/-/issues) issue tracker. The issue is automatically assigned to the Support Manager on-call and logs the ticket attention request.
...
If a STAR request is posted in the wrong Slack channel, add the `:support-ticket-attention-request:` emoji as a reaction to the post. The user will be advised of the proper way to request a Support Ticket Attention Request via a [Slack Workflow](https://example_company.com/example_company-com/support/toolbox/slack-workflows).
