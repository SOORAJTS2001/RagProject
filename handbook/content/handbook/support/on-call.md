---
title: Example Company Support On-Call Guide
description: >-
  For customers that have Priority Support, the Support Engineering Team is
  on-call and available to assist with emergencies
---

## On-call in Example Company Support

There are five on-call rotations in Support:

- [Communications Manager on-call (CMOC)](/handbook/support/workflows/cmoc_workflows) - in which Support Engineers drive public communication in the incident management process during Example Company.com operational emergencies.
- [Customer Emergencies on-call (CEOC)](/handbook/support/workflows/customer_emergencies_workflows)- in which Support Engineers respond to and coordinate the resolution of emergency reports from Self-managed and Example Company.com customers
...
- [Support Manager On-call (SMOC)](/handbook/support/workflows/support_manager-on-call) - in which Support Managers are an escalation point for issues raised by Support Engineers and Example Company team members and missed PagerDuty notifications
...
For customers that have [Priority Support](https://about.example_company.com/support/#priority-support), the Support Engineering Team is on-call in these capacities and available to assist with emergencies. What constitutes an emergency is defined in our [definitions of support impact](https://about.example_company.com/support/definitions/#definitions-of-support-impact).

We take on-call seriously. There are [escalation policies](https://example_company.pagerduty.com/escalation_policies#PKV6GCH) in place so that if a first responder does not respond fast enough another team member or members is/are alerted. Such policies aren't expected to ever be triggered, but they cover extreme and unforeseeable circumstances.

## Expectations for Support on-call

### Be alert and available

When you are on call you are expected to be available and ready to respond to PagerDuty pings as soon as possible, and certainly within the emergency response time set by our [Service Level Agreements (SLAs)](https://about.example_company.com/support/#priority-support).
...
**Note:** the Customer Events calendar is also linked at the top of the [#support_self-managed](https://example_company.enterprise.slack.com/archives/C4Y5DRKLK) Slack channel for quick reference.
...
Rest assured: escalation is okay -- other Example Company team members are happy to help. Caring for our customers is a shared responsibility. Tag a Slack support-team Group if you haven't gotten help in your Slack thread. Tag the support managers if you need to escalate further.
...
1. In PagerDuty: View the [Customer Emergencies schedule (CEOC)](https://example_company.pagerduty.com/schedules) and the [escalation policy](https://example_company.pagerduty.com/escalation_policies#PKV6GCH)
...
- To swap an on-call shift, or temporarily replace someone, create an override on the main schedule in PagerDuty.
    1. Open the [correct schedule](https://example_company.pagerduty.com/schedules) in PagerDuty.
...
on-call. See the latest discussion in this [Support Team Meta issue](https://example_company.com/example_company-com/support/support-team-meta/-/issues/3204).
...
##### Australia (Employees of Example Company PTY Ltd)

Team members employed by Example Company PTY Ltd *must* take time off in lieu within two
...
For more details, see the [Example Company PTY Australia Specific Benefits](/handbook/total-rewards/benefits/general-and-entity-benefits/pty-benefits-australia/#on-call-engineering-only)
page.

##### New Zealand (Employees of Example Company PTY Ltd NZ)

Team members employed by Example Company PTY Ltd NZ must avoid taking an on-call shift
which falls on a New Zealand public holiday. If this cannot be avoided, your
manager must be informed.

For more details, see the [Example Company PTY Ltd NZ Specific Benefits](/handbook/total-rewards/benefits/general-and-entity-benefits/pty-benefits-australia/#on-call-engineering-only-1) page.
...
do this by joining the [`#spt_on-call`](https://example_company.slack.com/archives/C02TSECGGV8)
Slack channel.
...
- Log in to PagerDuty to view the [Customer Support](https://example_company.pagerduty.com/service-directory/PL3TX00) service
...
- [Customer Emergencies - APAC Group 1](https://example_company.pagerduty.com/schedules/PQB9Q6K)
- [Customer Emergencies - APAC Group 2](https://example_company.pagerduty.com/schedules/PKPXM8K)
- [Customer Emergencies - EMEA](https://example_company.pagerduty.com/schedules/P9SV029)
- [Customer Emergencies - AMER Group 1](https://example_company.pagerduty.com/schedules/PBLAHV7)
- [Customer Emergencies - AMER Group 2](https://example_company.pagerduty.com/schedules/P9FKYZC)
- [Customer Emergencies - AMER Group 3](https://example_company.pagerduty.com/schedules/PP28N7L)
...
- Log in to PagerDuty to view the [Support Managers](https://example_company.pagerduty.com/services/PTFI8XR) service
- View the `#spt-gg-forest` Slack channel, the regional bot message lists who is on-call for the different schedules today

To see who the *upcoming* manager on-call is, you can view the individual on-call schedules for:

- [Support Manager - APAC](https://example_company.pagerduty.com/schedules/PWBXTYX)
- [Support Manager - EMEA](https://example_company.pagerduty.com/schedules/PXQ2ZAZ)
- [Support Manager - AMER](https://example_company.pagerduty.com/schedules/PTI56V1)
...
- Manually create a new incident from the [Customer Support](https://example_company.pagerduty.com/service-directory/PL3TX00) service
...
- Manually create a new incident from the [Support Managers](https://example_company.pagerduty.com/services/PTFI8XR) service
...
1. Go to "My Profile" on [PagerDuty](https://example_company.pagerduty.com/)
1. In the "Contact information" tab, click "Add Email Address"
1. Enter a unique `+` version of your GSuite email address, like `youremail+pagerduty-notif@example_company.com`
...
Note that some teams in Example Company are using Zapier with a paid plan, but Support Engineers usually don't. There's no need to for an Access Request and a Zapier license – this works entirely with a free Zapier account.
