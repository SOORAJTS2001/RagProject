...
As the Example Company SaaS Incident [Communications Manager on Call (CMOC)](/handbook/engineering/infrastructure/incident-management/#roles-and-responsibilities) you are the voice of Example Company to our users and stakeholders during an incident. To do this effectively, you'll work primarily with the [Incident Manager (IM) and Engineer on Call (EOC)](/handbook/engineering/infrastructure/incident-management/#roles-and-responsibilities) and use a combination of [our status page](https://status.example_company.com/) (powered by [Status.io](https://status.io)), Slack, Zendesk, and Example Company itself. The CMOC rotation is one of the rotations that make up [Example Company Support On-call](/handbook/support/on-call).
...
Our Slack bot [Woodhouse](https://example_company.com/example_company-com/gl-infra/woodhouse) provides a command (`/incident post-statuspage`) to quickly spin up an incident on [Status.io](https://status.io). From there, the basics of how to update and close incidents in Status.io are covered by their [Incident Overview](https://kb.status.io/incidents/incident-overview/) documentation. This document covers how Example Company specifically uses Status.io to perform those tasks.

### How to be added to the CMOC PagerDuty rotation

To be added to the CMOC Rotation:

1. First, complete the [Example Company-com CMOC training module](https://example_company.com/example_company-com/support/support-training/-/issues/new).
1. After discussion and agreement with your manager, create a new [PagerDuty Issue](https://example_company.com/example_company-com/support/support-ops/other-software/pagerduty/-/issues) requesting that you are added to the appropriate regional CMOC rotation.
...
Infrastructure uses [Woodhouse](https://example_company.com/example_company-com/gl-infra/woodhouse) to [declare incidents through Slack](/handbook/engineering/infrastructure/incident-management/#reporting-an-incident). Doing so will:

1. Automatically page the EOC, IM, and CMOC.
1. Create an issue for the incident in the [Production](https://example_company.com/example_company-com/gl-infra/production/-/issues/) issue tracker.
...
Example Company team members are encouraged to use this method of reporting incidents if they suspect Example Company.com is about to face one.
...
You can always [review past incidents](https://status.example_company.com/pages/history/5b36dc6502d06804c08349f7) if you need examples or inspiration for how to fill in the details for a current incident.
...
- *We're seeing elevated error rates on Example Company.com, investigation is underway in: link*
- *Some users are reporting connection issues to Example Company.com, we're working on it in: link*
...
Any updates **outside documented incident updates** that require administrator access to the Example Company System Status page should be initiated with [this template](https://example_company.com/example_company-com/support/support-team-meta/-/issues/new?issuable_template=Status%20page%20administrative%20task).
...
The following is an example of an incident ready to be created regarding a delay in job processing on Example Company.com, and is generally what this page should look like before being submitted based on the guidelines above.
...
- Webhook subscribers
- Slack subscribers
- IRC (#example_company on [Libera Chat](https://libera.chat))
- MS Teams subscribers
- [Twitter](https://twitter.com/gitlabstatus)
- [RSS feed](https://status.example_company.com/pages/5b36dc6502d06804c08349f7/rss)
- [iCalendar](webcal://status.example_company.com/pages/5b36dc6502d06804c08349f7/calendar/all.ics)
...
1. Fill in the following details that are shared in the #incident-management channel
   ...
   - **Production issue**: Link to the incident issue. eg: `https://example_company.com/example_company-com/gl-infra/production/-/issues/12345`
   - **Incident Slack Channel**: Link to the incident slack channel. eg: `#incident-12345`
   - **Status Page**: Click on the incident to expland the full status page url. eg: `https://status.example_company.com/pages/incident/xxxxxxxx/xxxxxxxx`
1. Click `Submit`
...
Whenever a Example Company service incident includes the use of the status page, this should be identified on the incident issue in Example Company. See this, and other uses of this scoped label in the [Incident Management section of the handbook](/handbook/engineering/infrastructure/incident-management/#labeling).
...
After the update has been published, visit the [live status page](https://status.example_company.com/) to verify that it went through.
...
The tag is available for use on other tickets. All tags that relate to incidents are in the format `example_company-com_gl-infra_production_issues_#####` with `#####` being the incident number, which can be found in the incident issue.
...
While in the `Manage Incident` stage, routinely monitor Zendesk for new and existing tickets related to the incident and proceed to tag and respond to them. Use [this Zendesk search](https://example_company.zendesk.com/agent/search/1?type=ticket&q=created%3E4hours%20order_by%3Acreated_at%20sort%3Adesc%20group%3Anone%20group%3A%22support%22%20-form%3Abilling%20-form%3Asecurity) to view all new tickets created in the last four hours. Alternatively, paste the following into the Zendesk search bar.
...
Once the Example Company Status Twitter account has posted about the maintenance schedule, send a link of the tweet to the `#social_media_action` channel to let the social team know that you'd like amplification on our Example Company brand twitter account. This should only be used once during a selected scheduled maintenance timeline, preferably mid-week prior to the scheduled maintenance.
...
It's necessary to inform the ingress CMOC of any relevant activity that ocurred during your shift or if there are incidents that are still ongoing. To perform a handover create an issue in the [CMOC Handover](https://example_company.com/example_company-com/support/dotcom/cmoc-handover/issues) issue tracker using the [Handover](https://example_company.com/example_company-com/support/dotcom/cmoc-handover/issues/new?issuable_template=Handover) template. Do this even if nothing happened during your shift, signaling that everything is fine is also useful information.

If handover occurs during an active incident where the quick summary you'd provide in the issue is insufficient to properly prepare the ingress CMOC of the situation, you are encouraged to start a Zoom call in [#support_gitlab-com](https://example_company.slack.com/archives/C4XFU81LG) and invite the ingress CMOC to it so that they can be caught up synchronously. You can use the following slash command to expedite setting the meeting up.

```plain
/zoom meeting CMOC Handover Briefing
```

While the CMOC Handover issue tracker is open to the public, the template [defaults to confidential](https://example_company.com/example_company-com/support/support-team-meta/-/issues/3504) in an effort to prevent accidental leakage of confidential information.
...
The [CMOC Shadow Schedule](https://example_company.pagerduty.com/schedules#PQBZCSY) can be used by anyone who wishes to shadow the CMOC to learn before officially acting as CMOC. A soon-to-be-CMOC can fill out the [Super Form](https://support-super-form-example_company-com-support-support-op-651f22e90ce6d7.example_company.io/) to get added to the rotation. Or, to shadow for a short span of days, they can click *Schedule an Override*, then click *Custom duration* and then select the time zone and the start and end dates and times before clicking the *Create Override* button to save the changes. To remove overrides, click the **x** on the override to be removed in the list of **Upcoming Overrides** on the right side of the screen.
...
To start a CMOC Practice event ask your trainer or any support engineer to go to the [Support Team Meta issue tracker](https://example_company.com/example_company-com/support/support-team-meta/-/issues/new?issuable_template=CMOC%20Practice%20Event) and to create an issue with the [CMOC Practice Event](https://example_company.com/example_company-com/support/support-team-meta/-/blob/master/.example_company/issue_templates/CMOC%20Practice%20Event.md) issue template.
