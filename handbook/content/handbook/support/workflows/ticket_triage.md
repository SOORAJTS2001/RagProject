...
- Self-Managed: An issue where the customer is maintaining their own local instance of Example Company
- SaaS (Example Company.com): An issue involving the Example Company hosted SaaS offering
- SaaS Account: Single user account issue only, all others go to Example Company.com (SaaS).
...
- Level Up: For tickets that should go to the Professional Services team, such as requests about [the Example Company LevelUp platform](/handbook/people-group/learning-and-development/level-up/)(`levelup.example_company.com`).

**Please note:** Please follow the guidance under [Moving to non-support forms](#moving-to-non-support-forms) to *change* the ticket form, unless it's between Self-Managed, SaaS (Example Company.com) and SaaS Account.
...
1. Triage the Free User ticket and verify if they are a SaaS user who meets the support criteria outlined in the [free users section of the statement of support](https://about.example_company.com/support/statement-of-support/#free-users).
1. If they do not meet the above criteria or the ticket is for a self-managed Free User, in `New` tickets you can select `Free customer` in `Self-Managed Subscription` dropdown, which will activate the [`Auto-close free and self-provisioned trial SM tickets`](https://example_company.com/search?group_id=2573624&repository_ref=master&scope=blobs&search=id%3A+5475833679900&snippets=false) trigger (responds and solves the ticket). If the ticket is `Open`, then direct the customer to the [Example Company community forum](https://forum.example_company.com/) via the ["Self-managed or Example Company.com Free User Request" macro](https://example_company.com/search?utf8=%E2%9C%93&group_id=15990755&project_id=17008590&scope=&search_code=true&snippets=false&repository_ref=master&search=id%3A+360044960813) and mark it "Solved". If you wish to provide technical assistance to the user, then please do so via the [Example Company community forum](https://forum.example_company.com/).
...
If the request falls outside of such scenarios, see the [Other Requests section](#other-requests), or direct the user to the [Example Company community forum](https://forum.example_company.com/) and the ticket marked as "Solved".

## After applying the correct form

- Apply the correct `Support - Category` and `Subcategory` if possible.
- If missing, gather information about the `Example Company Install Type`, `Reference Architecture` and `Example Company Version`
...
Users of Example Company's [US Government Support](https://about.example_company.com/support/us-government-support/) offering may opt to raise a ticket in the Global Support Portal as well. The most common reason for this is the need for support outside of the US Government's [hours of operation](https://about.example_company.com/support/us-government-support/#hours-of-operation). If during the triage process, a Global Agent comes across a ticket that is from an organization which appears to be a customer entitled to US Government Support, the agent can work this case the same as they would any other ticket.
...
|Request|Workflow|
|--|--|
|Training|Redirect requester to [Education from Professional Services](https://about.example_company.com/services/education/)|
...
|Requests for swag|Ask in the #swag internal slack channel or direct to [the FAQ](https://shop.example_company.com/policies) for existing orders. For general requests for free swag, use the `General::Free Swag Request` Macro.|
|Questions related to status of job application| Use the `General::Job Application Questions` [macro](https://example_company.com/example_company-com/support/support-ops/zendesk-global/macros/-/blob/master/macros/active/General/Job%20Application%20Questions.yaml) and send a note to the internal `#talent acquisition` Slack channel.|
...
|Questions regarding [Example Company Partner Program](https://partners.example_company.com/English/) or problems with registration. | This program is under [Channel Partner](/handbook/resellers/). Kindly direct requester to contact `partnersupport@` or ask in [#partner-programs-ops](https://example_company.slack.com/archives/CTM4T5BPF) when unsure. |
