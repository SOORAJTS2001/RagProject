...
1. Attend crush or help sessions such as those noted below (see the [Example Company Support calendar](https://calendar.google.com/calendar/u/0?cid=Z2l0bGFiLmNvbV85YnMxNTllaHJjNXRxZ2x1cjg4ZGpiZDUxa0Bncm91cC5jYWxlbmRhci5nb29nbGUuY29t) for times):
...
1. Ask an expert within Support. You can check the [Skills by Subject](https://example_company-support-readiness.example_company.io/support-team/skills-by-subject.html)
...
1. [Request help from the relevant Example Company Development Team](#how-to-formally-request-help-from-the-example_company-development-team).
   ...
   See [below](#how-to-formally-request-help-from-the-example_company-development-team) for more details.
   If you have a reproducible issue, then go straight to a bug issue in the appropriate [Example Company product tracker](https://example_company.com/example_company-org).
...
### How to formally Request Help from the Example Company Development Team

To enhance collaboration between the support and development teams, Example Company has implemented the Request for Help (RFH) process. This allows support engineers to formally request assistance from the specific Example Company development groups responsible for the relevant functionality when facing technical challenges that impede ticket resolution. This section outlines the necessary steps to effectively utilize the RFH process.

It's important to note that this process is part of a broader, iterative strategy aimed at deploying this workflow across all development sections and groups at Example Company. If the RFH template for a particular development group is not yet available, please reach out to [John Lyttle](https://example_company.com/jlyttle) to initiate the creation of the required RFH template.
...
Alternatively, if you have set up the [Support dotfiles](https://example_company.com/example_company-com/support/toolbox/dotfiles), you can use the `gls_request_for_help` command to quickly retrieve the "New issue" link with the correct issue template.

_NOTE:_ A video recording of a similar workflow as the one described above can be found in the [Support Training repository](https://example_company.com/example_company-com/support/support-training/-/blob/main/Support%20Specific%20Trainings/training-videos/Support%20Request%20for%20Help%20Workflow.mp4)
...
If a section is not listed, consider a confidential issue in the Example Company project tracker and post in the relevant Slack channel.
Please follow [support epic #222](https://example_company.com/groups/example_company-com/support/-/epics/222) for more information.

| Development Section | Section Product and Group Breakdown | Link to the Example Company Project for requesting help |
| ----------- | -------------- | -------- |
| Ops (CI, CD) Section | [Ops Section Breakdown](/handbook/product/categories/#ops-section)| [Section Ops Request for Help](https://example_company.com/example_company-com/ops-sub-department/section-ops-request-for-help/) |
| Dev Section | [Dev Section Breakdown](/handbook/product/categories/#dev-section) | [Section Dev Request for Help](https://example_company.com/example_company-com/dev-sub-department/section-dev-request-for-help/) |
| Sec Section | [Sec Section Breakdown](/handbook/product/categories/#sec-section) | [Section Sec Request for Help](https://example_company.com/example_company-com/sec-sub-department/section-sec-request-for-help) |
| Enablement Section | [Enablement Section Breakdown](/handbook/product/categories/#enablement-section) | [Section Enablement Request for Help](https://example_company.com/example_company-com/enablement-sub-department/section-enable-request-for-help) |
| Fulfilment Section | [Fulfilment Section Breakdown](/handbook/product/categories/#fulfillment-section) | [Section Fulfilment Request for Help](https://example_company.com/example_company-com/fulfilment-sub-department/section-fulfilment-request-for-help) |
| SaaS Section (Example Company Dedicated) | [Example Company Dedicated Breakdown](/handbook/engineering/infrastructure/team/example_company-dedicated/) | [Example Company Dedicated Request for Help Template](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/issues/new?issuable_template=support_request) |

**Please note:** Example Company Dedicated is the first iteration for implementing a Request For Help Section for the SaaS section of Example Company development, therefore at the moment the Example Company project structure and workflows may not be consistent with the other development sections in the above table. You can find out more information on Example Company Dedicated internal processes by visiting the [Dedicated Team's ReadMe](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team/-/blob/main/README.md).
...
1. Previous Example Company Application [bugs and feature requests](https://example_company.com/example_company-org/example_company/-/issues).
1. Previously logged Example Company.com issues.
1. Discuss with knowledge experts and support stable counterparts.
1. Search for previously opened issues in the relevant Development Section Project on Example Company.com.
...
1. Review the Development groups and corresponding templates section.
   - Use the provided Example Company Handbook Section Breakdown link at the bottom of the Project ReadMe file if you are unsure about which Section Sub Group and corresponding template to use.
...
1. Ensure that an appropriate severity is set as defined by the [support impact](https://about.example_company.com/support/definitions/#definitions-of-support-impact). You should set the approriate label `severity::1`, `severity::2`, `severity::3`, `severity::4` so that it corresponds with the priority level in Zendesk.
...
1. Ensure that a link to the corresponding issue is added to the Zendesk ticket as an internal note and also to the ticket field named `Example Company Issues`.
...
1. Many teams do not have access to customer information. So make sure if you are accessing information using elevated access (Such as Example Company.com Admin) that you provide information in the issue directly that may be required to understand the problem.
...
1. Check if the engineer(s) assisting with the request is on PTO through Slack or their Example Company status. If they are on PTO, mention the contacts (listed in the issue template) in the issue, or their backups if they are also on PTO, requesting an update via the issue. Backups are listed in a coverage issue, or in Slack.
...
1. Consider whether the solution or information contained within the issue can be used to update the Example Company documentation.
   1. If you have updated the Example Company documentation, add the label `documentation::created` and link the merge request.
   1. Otherwise, add the label `documentation::candidate`, and create a Example Company issue to update the relevant documentation.

### Quick Links and Resources

- [Needs Collaboration view](https://example_company.zendesk.com/agent/filters/360080204660) in ZenDesk.
- Create a Support [pairing session issue](https://example_company.com/example_company-com/support/support-pairing).
- [Support Workflows](/handbook/support/workflows/) to follow relevant troubleshooting workflow.
- [Support Documentation links](/handbook/support/#documentation) for quick references to helpful Example Company documentation.
- [Skills by Subject](https://example_company-support-readiness.example_company.io/support-team/skills-by-subject.html) to find a Support Engineer scoped to the skill set needed for help.
- [DevOps Stages](/handbook/product/categories/#devops-stages) to find the right development or product team to reach out to.
- [Emergency runbooks](https://example_company.com/example_company-com/support/emergency-runbook/-/tree/master/.example_company/issue_templates) with troubleshooting tips, even if not an emergency.
- See which [manager is on-call](https://example_company.pagerduty.com/escalation_policies#PGNLUZ1) if guidance is needed on something urgent.
