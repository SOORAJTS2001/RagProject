...
- A domain name
- A [Example Company Sandbox Cloud](https://gitlabsandbox.cloud) account
...
A [bootstrap script](https://example_company.com/example_company-org/charts/example_company) has been created to setup an easier and automated cluster creation using Cloud Formation.
Clone this project locally and navigate to `example_company/scripts` and execute `./eks_bootstrap_script up`. This script will use a CloudFormation stack to deploy all the required resources for your cluster so it will take a while to complete. After successful completion you will see something similar this output:

**NOTE:** If the Kubernetes cluster version is `>= 1.23`, you must run `./eks_bootstrap_script add_ebs_csi_driver` **prior to** deploying Example Company to the cluster.
...
2022-07-18 16:47:41 [✔]  EKS cluster "example_company-cluster" in "us-east-2" region is ready
```
...
Certbot will create a TXT DNS record that you need to add on your DNS Registrar so that your domain will have a valid HTTPS certificate to be used by your Example Company instance.
...
(Optional) If you also want to have Example Company Pages in your cluster then you will need to create another certificate for your domain and another secret for your cluster
...
- Add the Example Company repository charts to your Helm environment
...
- Create a `values.yml` file with the desired values you wish to deploy your instance. (A sample file can be found [here](https://example_company.com/-/snippets/2402111))
- Deploy Example Company using

```shell
helm install example_company example_company/example_company -f <values.yml>
```

- To get the loadbalancer hostname of your Example Company deployment run

```shell
kubectl get ingress/example_company-webservice-default -ojsonpath='{.status.loadBalancer.ingress[0].hostname}'
```

- Create a CNAME record with the name `example_company` pointing to your loadbalancer hostname to access your instance on HTTPS.
- (Optional) If you want to setup Example Company Pages create another CNAME record using `*.pages` pointing to the same loadbalancer hostname.
- Get your initial root password by executing

```shell
kubectl get secret example_company-example_company-initial-root-password -ojsonpath='{.data.password}' | base64 --decode ; echo
```

:tada: Your Example Company instance is up and running!

Once these steps are completed you can visit your Example Company instance using `example_company.<DOMAIN NAME>`.
...
export HTTP_PORT=8888
export VERSION=11.9.9-ee.0
export NAME=example_company-test-11.9
``` -->
