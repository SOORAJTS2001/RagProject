...
[Kibana](https://log.gprd.example_company.net/) is an [open source data visualization plugin](https://www.elastic.co/kibana) for [Elasticsearch](https://en.wikipedia.org/wiki/Elasticsearch). It provides visualization capabilities on top of the content indexed on an Elasticsearch cluster. Support Engineering uses Kibana to both search for error events on Example Company.com and to detect when specific changes were made to various aspects of it by a user.

>**Note:** Kibana does not retain logs older than 7 days and defaults to the UTC time zone.

### Parameters

Knowing *where* to search in Kibana is paramount to getting the proper results. The area of the application (Example Company.com) that you're searching is known as the `Index` in Kibana. The default index used for searching is `pubsub-rails-inf-gprd-*` but you can change this by clicking the `(change)` button:

![Changing search index](/images/support/kibana_index-selection.jpg)

Indexes closely correlate for the most part with our [log structure](https://docs.example_company.com/ee/administration/logs/) in general. Some other frequently used indexes are:
...
Along with the index, knowing *when* a specific error or event ocurred that you're trying to track down  is important and it's important to keep in mind that Kibana logs on Example Company.com persist for seven days. Kibana allows you to choose relative and absolute time ranges for search results and this can be changed by manipulating the date range:

![Changing the date range](/images/support/kibana_changing-dates.jpg)

### Fields and Filters

**Note:** As of March 2022, using `Filters` is the recommended way for searching Kibana. Using the general search box is discouraged as it may generate several errors in the form of `X of Y Shards Failed`. See [Infra Issue](https://example_company.com/example_company-com/gl-infra/reliability/-/issues/15207).
...
- `json.method`
- `json.action`
- `json.controller` - for more on this see the [controller definitions in the Example Company sourcecode](https://example_company.com/example_company-org/example_company/-/tree/master/app/controllers/projects)
- `json.status`
- `json.path`
...
Not sure what to look for? Consider using a Self-Managed instance to replicate the bug/action you're investigating. This will allow you to confirm whether or not an issue is specific to Example Company.com, while also providing easily accessible logs to reference while searching through Kibana.
...
There are a number of available dashboards that are designed to create visualizations that can help in identifying patterns. It can also be useful for narrowing in on specific areas of the product or a identifying a specific issue or bug. While any Example Company team member can create a dashboard that can be used by anyone, there are a few support specific dashboards:
...
This dashboard can be used to get specific http statuses across a namespace or path/project and narrow down by class. This can be useful for understanding error patterns across a namespace or even across the entirety of Example Company.com.
...
Example group: [example_company-bronze](https://example_company.com/example_company-bronze)

We can determine if the Example Company Runner registration token was reset for a group or project and see which user reset it and when. For this example the Runner registration token was reset at the group-level in `example_company-bronze`. To find the log entry:

1. Set the date range to a value that you believe will contain the result. Set it to `Last 7 days` if you're unsure.
1. Add a positive filter on `json.path` for the path of the group, which is just `example_company-bronze` in this example.
...
- Example group: [example_company-silver](https://example_company.com/example_company-silver/)
- Example project: [example_company-silver/test-project-to-delete](https://example_company.com/example_company-silver/test-project-to-delete)

Kibana can be used to determine who triggered the deletion of a group, subgroup, or project on Example Company.com. To find the log entry:
...
1. Add a positive filter on `json.path` for the path of the project, including the group and subgroup, if applicable. This is `example_company-silver/test-project-to-delete` in this example.
...
While we do not specifically log *changes* made to CI/CD variables in our [audit logs for group events](https://docs.example_company.com/ee/administration/audit_events.html#group-events), there is a way to use Kibana to see who may have viewed the variables page. Viewing the variables page is required to change the variables in question. While this does *not* necessarily indicate someone who has viewed the page in question has made changes to the variables, it should help to narrow down the list of potential users who could have done so. (If you'd like us to log these changes, we have [an issue open here to collect your comments](https://example_company.com/example_company-org/example_company/-/issues/8070).)
...
1. In the search bar, enter in the Example Company Pages domain name. For example `json.pages_domain: "sll-error.shushlin.dev"`
...
Kibana can be used to find out if and when an account on Example Company.com was deleted if it occurred within the last seven days at the time of searching.
...
If you suspect an account was deleted by the cron job that deletes [unconfirmed accounts](https://docs.example_company.com/ee/user/gitlab_com/#email-confirmation), try searching with these filters:
...
- [Quick link to search](https://log.gprd.example_company.net/goto/5598ea40-a651-11ed-9f43-e3784d7fe3ca)

Kibana can be used to find out which admin disabled 2FA on a Example Company.com account. To see the log entries:
...
Kibana can be used to find out if and when SSO Enforcement was enabled or disabled on a group on Example Company.com, and which user did so.
...
1. Add a positive filter on `json.path` for the path of the group, `example_company-silver` in this example case.
...
1. Set the date range to a value that you believe will contain the result. Set it to `Last 7 days` if you're unsure.
1. Add a positive filter as advised in [our SAML groups docs](https://docs.example_company.com/ee/user/group/saml_sso/troubleshooting.html#searching-rails-log-for-a-saml-response).
...
1. Get the link for the container registry in question. (For exmaple: <https://example_company.example.com/group/project/container_registry/><ContainerRepository>)
1. Set the date range to a value that you believe will contain the result. Set it to `Last 7 days` if you're unsure.
...
As of [14.7, a Correlation ID is provided](https://example_company.com/example_company-org/example_company/-/issues/34113) on the 500 error page when using the interface. You can ask the customer to supply this and use Kibana to filter by `json.correlation_id`.
...
1. [Log in to Kibana](https://log.example_company.net/app/kibana#).
...
See the [500 errors workflow]({{< ref "500_errors" >}}) for more information on searching and finding errors on Example Company.com

### Filter by IP Range

Customers will sometimes give us an IP Range of their resources such as their Kubernetes cluster or other external servers that may need to access Example Company. You can search a range by using Elasticsearch Query DSL.
...
Anytime there is an error, ensure that the export originated from a [compatible version of Example Company](https://docs.example_company.com/ee/user/project/settings/import_export.html#version-history).
...
Export errors can occur when a user attempts to export via the UI or [this API endpoint](https://docs.example_company.com/ee/api/project_import_export.html#schedule-an-export). A parameter in the API allows for exporting to an external URL such as a pre-signed AWS S3 URL. Typically, the export process consists of:
...
- **Imported project's size differs from where it originated**
  - See [this comment](https://example_company.com/example_company-org/example_company/-/issues/27742#note_215721494) for an explanation as to why. As artifacts are part of repository size, whether they are present can make a big difference.
- **Repository shows 0 commits**.
  - See [15348](https://example_company.com/example_company-org/example_company/issues/15348).

### Purchase Errors

Kibana can be used to search for specific errors related to a purchase attempt. Since purchases can be made either in Example Company.com or [CustomersDot](https://customers.example_company.com/customers/sign_in), it is important to determine which system a customer was using to make the purchase; then use the tips specific to the system below. **Note:** the ticket submitter might not be the customer making the purchase.

#### Example Company.com purchase errors

**Note**: You need to have the **Example Company username** of the account used to make the purchase. Sometimes the user fills the `Example Company username` value of the ticket fields, or you can check the ticket requester's Example Company username in the [Example Company User Lookup Zendesk App](/handbook/support/readiness/operations/docs/zendesk/apps/#example_company-super-app).

1. Navigate to [Kibana](https://log.gprd.example_company.net/)
1. Ensure the `pubsub-rails-inf-gprd-*` index pattern (Example Company.com logs) is selected.
1. Set the date range to a value that you believe will contain the result. Set it to `Last 7 days` if you're unsure.
...
Feeling Lazy? Go to `https://log.gprd.example_company.net/goto/6aac4580-9d9b-11ed-85ed-e7557b0a598c` and update the value of `json.user.username`.
...
**Tip:** To see the details of a user's purchase attempt, go to `https://log.gprd.example_company.net/goto/45bb89c0-6ccc-11ed-9f43-e3784d7fe3ca` and update the value of `json.username`.
...
Feeling Lazy? Go to `https://log.gprd.example_company.net/goto/9c28ba80-9d9b-11ed-9f43-e3784d7fe3ca` and update the value of `json.customer_id`.

In case you have the namespace details, get the **Namespace ID** then go to `https://log.gprd.example_company.net/goto/b598dfe0-9d9b-11ed-9f43-e3784d7fe3ca` and update the value of `json.params.gl_namespace_id`
...
- [`action: git_upload_pack`](https://log.gprd.example_company.net/app/r/s/dLqA1) is when someone performs a clone of a repository.
- [`action: git_receive_pack`](https://log.gprd.example_company.net/app/r/s/lRA1L) is when someone push's a repository.
- [`action: archive`](https://log.gprd.example_company.net/app/r/s/Lxx9w) is when someone downloads a repository via the `Download source code` button in the UI.
...
- [`command: git-upload-pack`](https://log.gprd.example_company.net/app/r/s/B0qHS) is when someone performs a clone over SSH.
- [`command: git-receive-pack`](https://log.gprd.example_company.net/app/r/s/p3J0W) is when someone performs a push over SSH.
...
[Webhook events](https://docs.example_company.com/ee/user/project/integrations/webhooks.html) for Example Company.com can be located in Kibana, including identifying when a group or project has gone over [enforced rate limits](https://docs.example_company.com/ee/user/gitlab_com/index.html#webhooks). Rate limiting varies depending on the subscription plan *and* number of seats in the subscription.
...
#### Searching for Service Desk emails
  ...
  When searching through Kibana for the `json.to_address`, make sure this is the address that appears on the `to:` line in the email, even if this is aliased to the Example Company project email address. If you search for the project email address and the Service Desk mail was sent to an alias of that (`support@domain.ext` for example), it won't show up in the searches.
