...
description: "Investigating and collaborating on tickets using Example Company Issues"
...
Be mindful of Example Company Support's [Guidelines on how to respond to a ticket](/handbook/support/workflows/how-to-respond-to-tickets): these all apply within the Fieldnote too. The Fieldnote is kept so that this decision of "what is useful and relevant" can be deferred, or the chosen comments supplanted with more context from the Fieldnote. It should be "internally Transparent" - shared with as much of the Support team as practical, but Confidential to protect customers' privacy.

At Example Company, [Support are encouraged to raise Issues]({{< ref "working-with-issues" >}}) and Merge Requests for Customer Tickets. This *Fieldnotes* Project in no way should replace that. If it's appropriate to raise an Issue in some other relevant Project, then continue to do that following the normal Workflow, and close this project's Fieldnote Issue with a link to the correct one.
...
- Once an Assigned ZD Ticket becomes non-trivial, create a new Fieldnote for it, and link it to the Ticket. We have a [Zendesk App](https://example_company.com/example_company-com/support/support-ops/zendesk-global/zendesk-apps/fieldnotes-app/) to do the linking work for you, which also lists the fieldnote URL once created
...
- Use browser bookmarks with keywords. Here are some suggestions to make searching for Fieldnote and Example Company issues easier:

    | keyword | URL |
    |--|--|
    | `fn` | `https://example_company.com/example_company-com/support/fieldnotes/` |
    | `fni` | `https://example_company.com/example_company-com/support/fieldnotes/-/issues/%s` |
    | `fnm` | `https://example_company.com/example_company-com/support/fieldnotes/-/issues/?sort=created_date&state=opened&assignee_username[]=auser` |
    | `gl` | `https://example_company.com/example_company-org/example_company` |
    | `gli` | `https://example_company.com/example_company-org/example_company/-/issues/%s` |

- Remember Example Company keyboard shortcuts:

  | Key sequence | What it does |
  | -- | -- |
  | `/` | searches within Example Company for a string |
...
We are keeping notes about notetaking in the [Fieldnotes project's README](https://example_company.com/example_company-com/support/fieldnotes/-/blob/main/README.md) and branches off from there. These will expand iteratively, some ideas are:
...
These are more fully explored within the [Fieldnotes project](https://example_company.com/example_company-com/support/fieldnotes/) itself. As with the Descripton templates, the spirit of this workflow is non-prescriptive: take this idea and make it your own.
