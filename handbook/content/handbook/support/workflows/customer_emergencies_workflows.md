...
Support Engineers in the Customer Emergencies rotation coordinate operational emergencies from Example Company customers.

The Customer Emergencies rotation is one of the rotations that make up [Example Company Support On-call](/handbook/support/on-call/).
...
To be added to the Customer Emergency On Call Rotation, you should have first completed the [Customer Emergency On-Call training module](https://example_company.com/example_company-com/support/support-training/-/issues/new) and then after agreement with your manager, you should raise a new [Pager Duty Issue](https://example_company.com/example_company-com/support/support-ops/other-software/pagerduty/-/issues) with the Support-Ops team requesting that you are added to the appropriate Pager Duty rotation.
...
**Note:** You may sometimes be required to contact Example Company users on behalf of another Example Company team (such as the SIRT team). Please follow the [Sending Notices workflow](/handbook/support/workflows/sending_notices) to action these requests.

### Determine if the situation qualifies as an emergency

According to our [definition of Severity 1](https://about.example_company.com/support/definitions/#severity-1) an emergency exists when a "Example Company server or cluster in production is not available, or otherwise unusable". In the event that the situation does not clearly qualify under the strict definition of emergency, an exception may be granted.
...
| ------------------------------ | ------------------------------- | ---------------------------------- |
| ...meets the [definition of Severity 1](https://about.example_company.com/support/definitions/#severity-1), | `General::Emergency::Strict Definition` | ...your plan to work the emergency. |
...
[Definitions of support impact](https://about.example_company.com/support/definitions/#definitions-of-support-impact).
...
1. How we define an [emergency situation](https://about.example_company.com/support/definitions/#severity-1)
...
1. **OPTIONAL:** Create a new Issue using the [Emergency Runbook Issue Template](https://example_company.com/example_company-com/support/emergency-runbook/-/issues/new), to guide you through the emergency response process for Customer Emergency tickets.
...
1. Work with the on-call Support Manager to [determine if the situation qualifies as an emergency](#determine-if-the-situation-qualifies-as-an-emergency)
   ...
   - Example of self-managed emergency ticket which was resolved without a call: <https://example_company.zendesk.com/agent/tickets/148028>
...
[Edit an existing escalation policy](https://example_company.com/example_company-com/support/support-ops/other-software/pagerduty/-/blob/master/.example_company/issue_templates/Edit%20an%20existing%20escalation%20policy.md?ref_type=heads) issue template.
...
If you encounter a SaaS emergency at the weekend that you are unable to progress, then consider checking if the [CMOC engineer on call](https://example_company.pagerduty.com/escalation_policies#PNH1Z1L) is available to offer any help or guidance.
...
On rare occasions, you and the manager on-call may decide it's necessary to initiate the [developer escalation process](/handbook/engineering/development/processes/infra-dev-escalation/process/#escalation-process) to get the needed developer input. Keep in mind that the developer who takes the escalation might not be familiar with the aspect of Example Company that is the focus of the emergency, and it can take them time to get up to speed.
...
For non-trial subscriptions, you can remind the customer that subscriptions have [a 14-day grace period](https://about.example_company.com/pricing/licensing-faq/#what-happens-when-my-subscription-is-about-to-expire-or-has-expired). If the grace period will still be active on the next business day, kindly let the user know that their request will be handled as a standard L&R case during normal business hours. You should close the emergency ticket and ask the customer to open a [new L&R ticket](https://support.example_company.com/hc/en-us/requests/new?ticket_form_id=360000071293) in case one doesn't exist yet.
...
1. Look up the namespace details using [chatops](/handbook/support/workflows/chatops#namespace) or a Example Company.com Admin account via the namespaces API (https://example_company.com/api/v4/namespaces/<NAMESPACE>)
...
1. In the `Mechanizer` app on the ticket use the [`Manage Example Company Plan and Trials`](/handbook/support/license-and-renewals/workflows/customersdot/mechanizer#manage-gitlabcom-plan-and-trials) option to resolve the situation.
    ...
    - Leave the `Approving sales manager's Example Company username` field empty.
    - Leave the `Subscription name` empty.
    - Click `Submit request`.
1. Wait for the pipeline to complete and check the output of Mechanizer in the [Internal Requests issue tracker](https://example_company.com/example_company-com/support/internal-requests/-/issues) where Mechanizer will have automatically assigned you to a new issue.
...
1. Alert [`#support_licensing-subscription`](https://example_company.slack.com/archives/C018C623KBJ) by linking to the ticket for follow-up.
...
1. Ask the customer to login to the [Customers Portal](https://docs.example_company.com/ee/subscriptions/customers_portal.html) located at https://customers.example_company.com/customers/sign_in for subscription management.
1. Follow these [steps to ensure their Example Company.com account is linked](https://docs.example_company.com/ee/subscriptions/customers_portal.html#change-the-linked-account).
1. Follow these [steps to update the linked namespace](https://docs.example_company.com/ee/subscriptions/gitlab_com/index.html#change-the-linked-namespace).
...
- [Using Kibana](/handbook/support/workflows/kibana) - explore Example Company.com log files to find the errors customers are encountering.
...
- **broken functionality due to a regression being pushed to Example Company.com**
...
- **Example Company.com access or "performance" degradation to the level of unusability**, for example: no access in a geographical area, CI jobs aren't being dispatched. This is the hardest class, but will generally be operational emergencies.
...
- **a widespread incident causes multiple, successive PagerDuty alerts**
  - Success here means tagging and bulk responding to the issues pointing to the [Example Company.com Status Page](https://status.example_company.com) and production issue.

### Broken Functionality

If a customer is reporting that behaviour has recently changed, first check [Example Company.com Status](https://status.example_company.com) and `#incident-management` for any on-going incidents. If there's no known incident:
...
1. Initiate a call with the customer. You're specifically looking to:
   ...
   - ascertain whether or not a feature flag that may have been recently turned on (see: [Enabling Feature Flags on Example Company.com](https://docs.example_company.com/ee/development/feature_flags/controls.html#enabling-a-feature-for-gitlabcom))
...
If there is a known incident, it's acceptable to link to the public status page and related incident issue. Consider using [`Support::SaaS::Gitlab.com::Incident First Response`](https://example_company.com/example_company-com/support/zendesk-global/macros/-/blob/master/active/Support/SaaS/Example Company.com/Incident%20First%20Response.md?ref_type=heads).

#### Example tickets

- [Feature flag broke previously working behaviour](https://example_company.zendesk.com/agent/tickets/204073): resolution was to turn off a feature-flag.
- [Regression on Example Company.com broke previously working pipeline](https://example_company.zendesk.com/agent/tickets/147266): resolution was to revert a recently deployed MR.
- [Customer locked themselves out of their group by changing SAML settings](https://example_company.zendesk.com/agent/tickets/146611)
...
1. In cases where a customer is unable to complete a purchase because of a defect or outage, as a courtesy, someone with Example Company.com admin can override the storage limit on a group.

### A widespread incident causes multiple, successive PagerDuty alerts

If an incident occurs on Example Company.com and hasn't been posted on the status page, SaaS customers may raise emergencies in bulk.
...
1. Use Zendesk search to identify customer-raised emergencies:
   - [`priority:urgent order_by:created_at sort:desc`](https://example_company.zendesk.com/agent/search/1?type=ticket&q=priority%3Aurgent%20order_by%3Acreated_at%20sort%3Adesc) will show all emergency tickets, sorted by those opened most recently
   - [`priority:urgent order_by:created_at sort:desc status:new`](https://example_company.zendesk.com/agent/search/1?type=ticket&q=priority%3Aurgent%20order_by%3Acreated_at%20sort%3Adesc%20status%3Anew) will show all **new** emergencies
...
US Government on-call support is provided 7 days a week between the hours of 0500 and 1700 Pacific Time for [severity one](https://about.example_company.com/support/definitions/#severity-1) issues that arise with premium and ultimate customers who have purchased 12x5 US Gov support. Customers who have opted for the 24x7 ultimate for high and emergency coverage can page at any time.

The current on-call schedule can be viewed in [PagerDuty](https://example_company.pagerduty.com/schedules#P89ZYHZ)(Internal Link). The schedule is currently split into three, 8 hour shifts which roughly correlate with the dayshift, evening, and overnight team member hours:
...
If a non-24x7 eligible customer submits an emergency case outside the [working hours of Government Support](https://about.example_company.com/support/us-government-support/#hours-of-operation) the following will occur:
...
## Example Company Dedicated Emergencies

Emergencies from [Example Company Dedicated](https://docs.example_company.com/ee/subscriptions/gitlab_dedicated/) come through the Customer Emergency On Call rotation. The [Example Company Dedicated Handbook](/handbook/support/workflows/dedicated) has information about [working with logs](/handbook/support/workflows/dedicated_logs) and viewing [observability dashboards](/handbook/support/workflows/dedicated_instance_health/).

Consider using the `@spt_focus-dedicated` Slack handle to ping members of the Example Company Support team who focus on Example Company Dedicated for additional assistance.

As appropriate, you can use the section on [escalating emergency issues](/handbook/support/workflows/dedicated#escalating-an-emergency-issue) to engage the Engineer on Call for Example Company Dedicated.
...
1. Create a new instance at the exact same version and restore their most recent backup into it.
   ...
   - If they do not have a copy of their `example_company-secrets.json`, or if the only backups available are stored in `/var/opt/example_company/backups`, mount the volume of the compromised instance to retrieve it.
1. Rotate secrets on the new instance:
   - All secrets contained within the `example_company.rb` (such as LDAP/email passwords)
   - All secrets in CI jobs such as API keys or remote server credentials
   - Example Company Runner registration tokens and Runner environment variables
...
There have been a few documented cases of folks purchasing a single user Example Company license specifically to raise an emergency. If you
