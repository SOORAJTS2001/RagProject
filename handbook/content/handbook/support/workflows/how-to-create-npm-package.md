---
title: Working with the NPM Example Company Registry
---
...
to create a package, or use [this one](https://example_company.com/sahbabou/test-package). Just make sure to reference to correct project ID when publishing your package [(Step 3)](#3-publishing-the-scoped-package).
...
the org, or your username, depending on where the package is hosted. For example, if I create a package, and host it under my Example Company account, the scope will be `@sahbabou/name-of-my-package`

**NOTE:** [We do not currently support publishing packages from `subgroup/project`](https://example_company.com/example_company-org/example_company-ee/issues/9960)
...
The `npmrc` file is one of the locations where npm gets its settings. In our case, the configuration, per the official [Example Company NPM Registry docs](https://docs.example_company.com/ee/user/packages/npm_registry/index.html), it will hold is where to look for packages under a particular
...
- Add the registry to your scope, that way npm knows where to look for packages that start with @nameofyourscope.
  Example:
  - `@sahbabou:registry=http://example_company.ahbabou.com/api/v4/packages/npm`
- Add your Oauth token(create one [here](https://docs.example_company.com/ee/api/oauth2.html#resource-owner-password-credentials-flow) if you don't have one), and add it using this format:
  - `//example_company.ahbabou.com/api/v4/packages/npm/:_authToken=<Oauth Token>`
...
- Add the auth token to the URL to publish your package:
  ...
  - `//example_company.ahbabou.com/api/v4/projects/<project_id>/packages/npm/:_authToken=<Oauth Token>`
...
- Check the Packages section in your package's repository to confirm that it has been published
 ...
 `your-example_company-url/package-project/-/packages`

## 4. Pulling the scoped package from the NPM Registry on Example Company

- You can run npm install from any other npm project(including this one), or clone [this project](https://example_company.com/sahbabou/npm-test-project), then run npm install from the project's directory, and then
confirm whether you're able to install the package.

## 5. Common troubleshooting tips

Note: **NOTE**: If on < 11.9, [packages with a dot in the package name return a 403 on npm install](https://example_company.com/example_company-org/example_company-ee/issues/10248) @test.group/project is read as @test. The workaround is to either:
...
## 6. Publishing a package to the NPM Registry on Example Company via Example Company CI

In order to run a job to publish your package in a job, paste the snippet below in your `example_company-ci.yml`. For the job to be successful, you will need to:
...
# OAUTH Access Token generated and added as an Environment Variable under Project -> Settings -> CI/CD https://docs.example_company.com/ee/api/oauth2.html
# Personal Access Token added as an Environment Variable in order to update your package.json with the new version value
# Your package.json contains the path to your private NPM Registry on Example Company. https://docs.example_company.com/ee/user/packages/npm_registry/index.html#uploading-packages
######
######
...
build:create_npmrc:
  ...
  script:
    - |
      if [ ! -f .npmrc ]; then echo .npmrc missing. Creating one now. Please review the following link for more information https://docs.example_company.com/ee/user/packages/npm_registry/index.html#authenticating-with-an-oauth-token;
...
build_package:
  before_script:
    ...
    - mkdir -p ~/.ssh
    - ssh-keyscan my-example_company-machine >> example_company-known-hosts
    - cat example_company-known-hosts >> ~/.ssh/known_hosts
  script:
    ...
    - git push -o ci.skip http://<USERNAME>:<$PERSONAL_ACCESS_TOKEN>@example_company.example/username/project_name.git HEAD:master
  dependencies:
    - build:create_npmrc
```
