...
Tickets should have the appropriate SLA according to the [support service levels](https://about.example_company.com/support/#example_company-support-service-levels).

Some values for `Example Company Plan` for ZD organizations (called `Support Level` in SFDC)
do not receive a SLA if they are linked to a service level. These are not expected to have SLA:

- `Community`. [EDU](https://about.example_company.com/solutions/education/) and [OSS program](https://about.example_company.com/solutions/open-source/) do not include Support, but are in SFDC and synced.
...
![Checking the value of Example Company Plan](/images/support/zendesk_check_org_fields.gif)
...
Note that we have [an ongoing issue](https://example_company.com/example_company-com/sales-team/field-operations/systems/-/issues/634)
...
In other cases where there is a mismatched `Support Level` in SFDC and `Example Company Plan` in Zendesk,
...
- A ticket comes to Zendesk and you spot that `Example Company Plan` is shown as `Expired`.
- First, search for [any info about this organization]({{ ref "looking_up_customer_account_details" >}}) in SFDC and CustomersDot.
- If you found that they have a valid license or subscription i.e. it is not expired, you are likely facing [this issue](https://example_company.com/example_company-com/sales-team/field-operations/systems/-/issues/634).
- Fix SFDC side first by following [the steps above](#handling-customers-with-incorrect-expired-support).
  ...
  `Example Company Plan` value on Zendesk side will change from `Expired` to the valid one after the sync is done.
...
There is a [trigger](https://example_company.zendesk.com/agent/admin/triggers/360019008340/revisions/3)
which sends an internal note to remind people to add the user to CC and reply, see [this issue](https://example_company.com/example_company-com/support/support-team-meta/-/issues/1581) for more details.
...
1. File an issue to [report the inaccuracy](https://example_company.com/example_company-com/support/support-ops/zendesk-global/zd-sfdc-sync-global).
...
By default, `priority_prospect` tagged tickets will be shown in both SM and Example Company.com views.
