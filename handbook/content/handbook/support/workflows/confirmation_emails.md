...
the [SaaS Account Ticket Helper](/handbook/support/readiness/operations/docs/zendesk/apps/#example_company-super-app) application can activate.
...
Before the issue can be resolved we first need to locate the account in question. This can be done by either checking the [Example Company User Lookup App](#method-1-check-example_company-user-lookup-app) or by checking [Example Company Admin](#method-2-check-example_company-admin).

### Method 1: Check Example Company User Lookup App

1. Click the `Apps` button located in the top right of the Zendesk interface, while viewing the ticket.
1. Scroll down to the `Example Company User Lookup` app.
...
1. Proceed to Step 2 under Check Example Company Admin.

**If no account was found** use the Zendesk macro [`Support::SaaS::Gitlab.com::Account does not exist`](https://example_company.com/example_company-com/support/zendesk-global/macros/-/blob/master/active/Support/SaaS/Example Company.com/Account%20does%20not%20exist.md?ref_type=heads) or, if you believe it's applicable, use [`General::Verify account self-managed or .com`](https://example_company.com/example_company-com/support/support-ops/zendesk-global/macros/-/blob/master/macros/active/General/Verify%20account%20self-managed%20or%20.com.yaml) and then wait for a followup from the user.

### Method 2: Check Example Company Admin

1. In the Example Company.com Admin Area, [search for the user](https://example_company.com/admin/users) by username to confirm the account exists. Alternatively, search in your browser using [the API](https://example_company.com/api/v4/users?search=email@email.test) or [ChatOps](/handbook/support/workflows/chatops#user).
...
As of 2023-08-07, unconfirmed users are [deleted after a set number of days](https://docs.example_company.com/ee/user/gitlab_com/#email-confirmation) on Example Company.com.
Users are recommended to wait for Example Company to automatically delete the account.
...
1. Ensure that `mg.example_company.com` is set as the domain at the top of the page.
...
Once the problem has been fixed, if the email is the primary email on the account, you can send the user a [new confirmation email](https://example_company.com/users/confirmation/new). Afterwards, let the user know you've sent them a new confirmation email and ask them to check their inbox and spam folders.

>**Note:** If a user changes their primary email this will not work. See 👉 [Secondary Email](#secondary-email).

### Secondary email

Instruct the user to sign in and trigger a new confirmation email through their profile by visiting [https://example_company.com/-/profile/emails](https://example_company.com/-/profile/emails).

### Wacky State

If the user is unconfirmed, but their primary email address does not match the unconfirmed email address (see [this internal example](https://example_company.com/example_company-org/example_company/-/issues/239098#note_399726260)), then there are two options to resolve:

1. Impersonate the user and click on the "Resend confirmation email" under Email on their Settings > Profile page.
1. File a [console escalation internal request](https://example_company.com/example_company-com/support/internal-requests/-/issues/new?issuable_template=Example Company.com%20Console%20Escalation) to set the `unconfirmed_email` to `nil`.

## Customers Portal account email

If the problem is with the customers portal account email, you can send the user a [new Customers Portal confirmation email](https://customers.example_company.com/customers/confirmation/new). Afterwards, let the user know you've sent them a new confirmation email and ask them to check their inbox and spam folders.
...
NOTE:
The `mg.example_company.com` domain contains relevant data for Support troubleshooting. You will not see this list as a domain option in the drop-down on any pages. You must select the domain under Send > Sending > Domains. There is an email.mg.example_company.com domain which appears by default, but will not work.
...
1. Click on `Logs`.
1. Ensure that `mg.example_company.com` is set as the domain above the activity graph.
