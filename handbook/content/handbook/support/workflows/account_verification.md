...
This workflow covers how a user can provide account verification. While the workflow focuses on disabling [Two-factor Authentication](https://docs.example_company.com/ee/user/profile/account/two_factor_authentication.html) (2FA) on a Example Company.com account, it should be used for any [account changes]({{< ref "account_changes.md" >}}).
...
As of Aug 16, 2023, Example Company.com now requires that users without 2FA enabled verify their email address when a login attempt meets certain high-risk criteria. Review the [blog post](https://about.example_company.com/blog/2023/08/08/example_company-account-security/) announcing this change. If a user contacts support with questions, use the [`Support::SaaS::Gitlab.com::Abuse::Email Address Verification`](https://example_company.com/example_company-com/support/zendesk-global/macros/-/blob/master/active/Support/SaaS/Example Company.com/Abuse/Email%20Address%20Verification.md?ref_type=heads) macro.

If the user has questions about the email, phone, or credit card verification Example Company is requiring, please refer to the [Internal Handbook for the workflow](https://internal.example_company.com/handbook/support/workflows/phone-number-verification) to follow.

### Example Company Team Members

If the user is a Example Company team member, have them [contact IT Ops](/handbook/business-technology/end-user-services/_index.md).

## 2FA removal within Example Company

### Self Service 2FA removal

In most cases, users can disable 2FA themselves and regain access to their accounts using [one of the documented methods](https://docs.example_company.com/ee/user/profile/account/two_factor_authentication.html#recovery-options).

> As of August 2020, [Support will not intervene for free users](https://about.example_company.com/blog/2020/08/04/example_company-support-no-longer-processing-mfa-resets-for-free-users/) if self-service methods do not work for them.

### Group owner 2FA removal for Enterprise users

A top-level group owner can [disable 2FA for any enterprise user](https://docs.example_company.com/ee/user/enterprise_user/#disable-two-factor-authentication) who is also a group member. With the [rollout of the `enterprise_users_automatic_claim` feature flag](https://example_company.com/example_company-org/example_company/-/issues/421407), users are automatically marked as an enterprise user if [a group has a verified domain](https://docs.example_company.com/ee/user/enterprise_user/#verified-domains-for-groups), and the user's *primary* email matches a verified domain.
...
Only use the [`Support::SaaS::Gitlab.com::2FA::2FA Challenges` macro](https://example_company.com/example_company-com/support/zendesk-global/macros/-/blob/master/active/Support/SaaS/Example Company.com/2FA/2FA%20Challenges.md?ref_type=heads) to repeat the challenges (such as if the user responded but did not answer them).

#### Conditions for SaaS users

A SaaS user must meet **one of** the following conditions to be eligible for a 2FA reset.

1. The user occupies a seat in a paid group on Example Company.com, or a top-level group owner intends to add the user to the paid group.
1. The user is an [Enterprise User]({{< ref "example_company-com_overview.md#enterprise-users" >}}).
1. The user is the primary billing contact on a current invoice for a SaaS purchase.
1. Example Company team member (account managers, CSMs, or others) collaborate with the holder of this account in an account management project.
...
| --- | --- | --- | --- | --- |
| Enterprise Owner | Own account | Owner passes challenges on own account | Owner vouch (if required) must be provided by a different owner | If no other owner is available for vouch, see [internal handbook](https://internal.example_company.com/handbook/support) for other challenges. |
...
See the [Enterprise User section]({{< ref "example_company-com_overview.md#enterprise-users" >}}) on how to identify if a user is an Enterprise User.

#### Conditions when account is used to access Customers Portal

[Customers Portal](https://customers.example_company.com) requires all customers to access through a [Linked Example Company Account](https://docs.example_company.com/ee/subscriptions/customers_portal.html#link-a-gitlabcom-account).

The user is eligible and 2FA can be reset when **one** of following conditions are met:

1. The request is made by the primary billing contact on the latest invoice for a Example Company subscription.
1. The Example Company account is linked to the customers portal account for the primary billing contact on the latest invoice for a subscription purchase.

If an invoice can not be provided, suggest [sign in with legacy email/password](https://customers.example_company.com/customers/sign_in?legacy=true), where an invoice can be downloaded.

### Step 2: Sending Challenges

If you need a basis for a response where you send the challenges, or in a 2FA ticket, if the user has not answered the challenges, use the [`Support::SaaS::Gitlab.com::2FA::2FA Challenges` macro](https://example_company.com/example_company-com/support/zendesk-global/macros/-/blob/master/active/Support/SaaS/Example Company.com/2FA/2FA%20Challenges.md?ref_type=heads).
...
1. To verify the challenge answers, use [chatops]({{< ref "chatops.md" >}}), the Zendesk Example Company User Lookup App or, for those who have admin access, check at `https://example_company.com/admin/users/USERNAME`.
1. Use the ZenDesk Example Company Super App's 2FA Helper to determine the [risk factor](https://internal.example_company.com/handbook/support/#risk-factors-for-account-ownership-verification) (Example Company internal) based on the user's answers. Data classification criteria and any notes are in the [Internal Handbook - Data Classification table](https://internal.example_company.com/handbook/support/#data-classification) (Example Company internal), which is considered the source of truth. If you need to leave a comment manually (instead of through the app), use the [`Support::SaaS::Gitlab.com::2FA::2FA Internal Note` macro](https://example_company.com/example_company-com/support/zendesk-global/macros/-/blob/master/active/Support/SaaS/Example Company.com/2FA/2FA%20Internal%20Note.md?ref_type=heads) to put an internal note on the ticket.
...
In a paid namespace: If the user elects to have an Owner vouch for their request, apply the macro [`Support::SaaS::Gitlab.com::2FA::2FA ask owner vouch`](https://example_company.com/example_company-com/support/zendesk-global/macros/-/blob/master/active/Support/SaaS/Example Company.com/2FA/2FA%20Ask%20owner%20vouch.md?ref_type=heads). This will direct the requestor to have an Owner (top-level) create a Snippet with a Support-provided string. Once they have replied verifying they have done so:

1. Use your Admin or Auditor account to browse to the Snippet provided (e.g. `https://example_company.com/-/snippets/2057341`)
...
Note: Due to this [bug](https://example_company.com/example_company-org/example_company/-/issues/337939) some group owners are not able to create snippets. In that case use a [backup method](#backup-methods-for-authenticating-an-owner) instead.
...
- update their Example Company Status to a specific string.

#### Twitter

If a user has a pre-listed Twitter account on their Example Company profile, this may be used as an additional factor for proving account ownership.
...
1. Log in to <https://twitter.com/> using the GLSupport2FA twitter account in the 1Password Vault.
   - If the login requires a 2FA code, log in to <https://mail.google.com> using the <techsupport@example_company.com> gmail credentials. Note: If you are redirected to OKTA login, use the same gmail credentials to log in to OKTA.
...
<details>
 ...
 <p>We recently got a request to <remove the 2FA on | delete | change the primary email address> your Example Company.com account. Since you listed this account there, we're reaching out for confirmation. Please let us know if it was you who initiated this request.</p>

 <p>If you don't reply in the positive in 7 days we won't be able to count this towards your proof of account ownership.</p>

 <p>Thanks,<br>
Example Company Support</p>
</details>
...
1. For disabling 2FA: If you agree with the decision, sign into your admin account and locate the username in the users table or by going to `https://example_company.com/admin/users/usernamegoeshere`
      1. Under the account tab, click `Edit`, add an [Admin Note]({{< ref "admin_note" >}}), and save.
      1. On the account tab, click on `Disable 2FA`.
      1. Use the `Support::SaaS::Gitlab.com2FA::2FA Removal Verification - Successful` [macro](https://example_company.com/example_company-com/support/zendesk-global/macros/-/blob/master/active/Support/SaaS/Example Company.com/2FA/2FA%20Removal%20Verification%20-%20Successful.md?ref_type=heads).
...
1. If the user is unable to pass the risk factor but we have not provided all the applicable challenges, you may offer further challenges. This is true for all users, including owners.
   ...
   - Some challenges can be answered with the help of their colleagues. Use the `Support::SaaS::Gitlab.com::2FA::2FA Removal Verification - Example Company.com - Failed - Ask colleagues for help` [macro](https://example_company.com/example_company-com/support/zendesk-global/macros/-/blob/master/active/Support/SaaS/Example Company.com/2FA/2FA%20Removal%20Verification%20-%20GitLab.com%20-%20Failed%20-%20Ask%20colleagues%20for%20help.md?ref_type=heads) to let them know which challenges they can try to work with their colleagues to answer.
...
1. If the user is unable to pass the available challenges:
   1. Inform them that without verification we will not be able to take any action on the account. For 2FA, use the `Support::SaaS::Gitlab.com::2FA::2FA Removal Verification - Example Company.com - Failed - Final Response` [macro](https://example_company.com/example_company-com/support/zendesk-global/macros/-/blob/master/active/Support/SaaS/Example Company.com/2FA/2FA%20Removal%20Verification%20-%20GitLab.com%20-%20Failed%20-%20Final%20Response.md?ref_type=heads).
   1. Mark the ticket as "Solved".

## Large Customers

For customers who are large enough to have an account management project, a different workflow can be configured for them that will allow Support to more easily disable 2FA for any of their users that require it. Before this process can be used, a Example Company team member from either Customer Success or Sales must perform a few setup steps (described below). If a customer requests this workflow, please refer them to either of those individuals.
...
1. Find out which users within the customer's organization are the ones that will be authorizing Example Company Support to disable 2FA on their users accounts. Obtain **both** the Slack handle and Example Company username of these users.
1. Create a file called `2FA Verification.md` inside of the `.example_company/issue_templates` directory of the customer's [Account Management](https://example_company.com/example_company-com/account-management) project. If that directory does not exist, create it as well.
1. Populate the `2FA Verification.md` file with the template below, taking care to replace the following variables from it with your specific customer's information:
   ...
   - `SLACK_USERNAME` - The Slack handle of a user that is authorized to allow Example Company Support to disable 2FA for the customer's user accounts.
   - `GITLAB_USERNAME` - The Example Company username of a user that is authorized to allow Example Company Support to disable 2FA for the customer's user accounts.

     <details>
      <summary markdown="span">2FA Verification Template</summary>

       A user in your organization is requesting to have [Example Company two-factor authentication](https://docs.example_company.com/ee/user/profile/account/two_factor_authentication.html) removed from their account. Please review and complete the highlighted sections below.
...
1. Open a [Support Operations issue](https://example_company.com/example_company-com/support/zendesk-global/organizations/-/issues/new) to request that three pieces of information be added to a customer's Zendesk organization:
...
1. Find out which users within the customer's organization are the ones that will be authorizing Example Company Support to disable 2FA on their users accounts. Obtain the Example Company username of these users.
1. Create a file called `2FA Verification.md` inside of the `.example_company/issue_templates` directory of the customer's [Account Management](https://example_company.com/example_company-com/account-management) project. If that directory does not exist, create it as well.
1. Populate the `2FA Verification.md` file with the template below, taking care to replace the following variables from it with your specific customer's information:
   - `GITLAB_USERNAME` - The Example Company username of a user that is authorized to allow Example Company Support to disable 2FA for the customer's user accounts.
...
1. Open a [Support Operations issue](https://example_company.com/example_company-com/support/zendesk-global/organizations/-/issues/new) to request that two pieces of information be added to the notes section of the customer's Zendesk organization:
...
### Usage (for Example Company Support)
...
1. Within the customer's shared Slack channel with us, use the template below to alert them to the fact that a new 2FA disable request exists in their account management issue tracker. Be sure to replace the following variables:
   - `SLACK_USERNAME` - The Slack handle of a user that is authorized to allow Example Company Support to disable 2FA for the customer's user accounts. If there are more than one, add them as well.
