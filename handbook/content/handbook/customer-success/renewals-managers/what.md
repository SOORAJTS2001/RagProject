...
In FY24, the Global Renewals team will manage more customer-facing transactions than any other team at Example Company. As such, the Renewals team manages Renewal Opportunities with the utmost level of rigor and data quality to ensure a high degree of organization, effective collaboration with account teams, and successful Renewal outcomes.
...
- `Renewal Forecast Health` [link](https://internal.example_company.com/handbook/sales/sales-forecasting/#forecast-category-and-renewal-forecast-health-fields) - Red, Yellow, Green health rating based on the Net ARR field
...
Providing an accurate, data driven forecast is essential for Example Company's continued growth as it enables the business to:
...
- Analyze and take strategic action on trends within Example Company's book of business to mitigate risk
...
- `Renewal Risk Category` with the following selections:
  ...
  - `Will Contract`: Used when we have 100% confidence that there will be a reduction in seats on the subscription or a downtier (rarely used)
    - Examples: Significant workforce reduction at a customer that reduces the number of engineers capable of using the Example Company platform to well below their current seat count (including appropriate buffer levels).
...
Renewals Managers work closely with Customer Success Managers/Engineers and Account Executives. After a customer purchases a subscription with Example Company - they can expect to be engaged by these persona in the following ways:

- **Account Executive**: Manages the overarching customer relationship with Example Company on the account level. Primary focus is on how to grow the customers relationship with Example Company. Account Executives are sales people. Best practice - CC the CSM/AE on the account when in communication with customer.
