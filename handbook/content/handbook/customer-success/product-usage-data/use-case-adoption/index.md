...
description: "How we measure a customer's health score for general Example Company and different use cases"
---

---

To drive use case enablement and expansion with customers, we need to define exactly what it means to adopt a use case at Example Company. These health measures will appear in the **Product Usage** scorecard section in Gainsight. For more, see the [Gainsight Scorecard Attributes and Calculations](/handbook/customer-success/customer-health-scoring/#gainsight-scorecard-attributes-and-calculations).
...
|                       | **Red** | **Yellow** | **Green** |
| --------------------- | ------- | ---------- | --------  |
| [unique active users - L28D](https://example_company.com/example_company-org/example_company/-/blob/master/config/metrics/counts_28d/20220621085114_unique_active_users_monthly.yml) / [Billable Users](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/license/20210531204603_license_billable_users.yml) | <50% | ≥50 - <80% | ≥80% |
...
1. The SaaS-equivalent metric will be available later ([issue](https://example_company.com/example_company-data/analytics/-/issues/11662))
...
1. Couple that with efforts to drive awareness of Example Company within the account (eg. Example Company for Plan/PM, Example Company for non-developers, normal user enablement). A signal to SAE/AE/BDR to drive account-based outreach. Find unaware or unegaged user cohorts. Encourage them to  bring users onto the platform.
...
| ------------------ | ------------------------ | ------------------------------------------------------------ |
| SCM (Create)       | Basic Adoption           | Is my customer using the basic toolset? Most customers should adopt these features pretty quickly into their Example Company journey |
...
| Security (DevSecOps) | Enablement & Expansion   | For customers using our security features or who are trialing and wanting to [shift left](https://about.example_company.com/blog/2020/06/23/efficient-devsecops-nine-tips-shift-left/), use these metrics to help identify adoption and track growth |
| CD (Release) | Enablement & Expansion   | How much has my customer adopted Example Company for deployments? The next path along the customer journey is the [CD use case](/handbook/marketing/brand-and-product-marketing/product-and-solution-marketing/usecase-gtm/cd/) |
...
| --------------------- |-------|-------| ---------- | --------  |
| Git Operation Utilization % | [`Git Operations - Users L28D`](https://example_company.com/example_company-org/example_company/-/blob/master/config/metrics/counts_28d/20210216182040_action_monthly_active_users_project_repo.yml) ÷ [`Billable Users`](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/license/20210531204603_license_billable_users.yml) | <25%    | ≥25 - <50%     | ≥50%    |
...
| Metric | Calculation | **Red** | **Yellow** | **Green** |
|---|---|---|---|---|
| CI Pipeline Utilization % | [CI Pipelines - User L28D](https://example_company.com/example_company-org/example_company/-/blob/master/config/metrics/counts_28d/20210216175554_ci_pipelines.yml) ÷ [Billable Users](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/license/20210531204603_license_billable_users.yml) | <25% | ≥25% - <50% | ≥50% |
...
These Security (DevSecOps) metrics are **only** available for [Ultimate customers](https://about.example_company.com/direction/paid_tiers/#sts=Ultimate). Adoption timeline: 1 month after license purchase

| Metric | Calculation | **Red** | **Yellow** | **Green** |
|---|---|---|---|---|
| Secure Scanner Utilization % | [Secure Scanners - Users L28D](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210216181956_user_unique_users_all_secure_scanners.yml) ÷ [Billable Users](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/license/20210531204603_license_billable_users.yml) | ≤5% | >5% - <20% | ≥20% |
| Avg. Scan per CI Pipeline | [Secret Detection](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210830231956_secret_detection_scans.yml) + [Dependency Scan](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210216183828_dependency_scanning_scans.yml) + [Container Scan](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210830231849_container_scanning_scans.yml) + [SAST](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210830231329_sast_scans.yml) + [DAST](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210216183832_dast_scans.yml) + [Coverage Fuzzing](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210216183836_coverage_fuzzing_scans.yml) + [API Fuzzing](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210216183838_api_fuzzing_scans.yml) ÷ [CI Internal Pipelines - L28D](https://example_company.com/example_company-org/example_company/-/blob/master/config/metrics/counts_28d/20230217215050_ci_internal_pipelines.yml) | <0.1 | ≥0.1 - ≤0.5 | >0.5 |
| Number of Scanners in Use | SUM of [Secret Detection](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210830231956_secret_detection_scans.yml) + [Dependency Scan](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210216183828_dependency_scanning_scans.yml) + [Container Scan](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210830231849_container_scanning_scans.yml) + [SAST](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210830231329_sast_scans.yml) + [DAST](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210216183832_dast_scans.yml) + [Coverage Fuzzing](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210216183836_coverage_fuzzing_scans.yml) + [API Fuzzing](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/counts_28d/20210216183838_api_fuzzing_scans.yml) | ≤1 | 2 | ≥3 |
...
|       Metric       | Calculation | **Red** | **Yellow** | **Green** |
| ------------------ | ------- | ---------- | --------  | -------- |
| User Deployments Utilization % | [Deployments - User L28D](https://example_company.com/example_company-org/example_company/-/blob/master/config/metrics/counts_28d/20210216181935_deployments.yml) ÷ [Billable Users](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/license/20210531204603_license_billable_users.yml) | <5%    | 5-12%     | >12%    |
| Deployments Per User L28D | [Deployments L28D (event)](https://example_company.com/example_company-org/example_company/-/blob/master/config/metrics/counts_28d/20210201124930_deployments.yml) ÷ [Billable Users](https://example_company.com/example_company-org/example_company/-/blob/master/ee/config/metrics/license/20210531204603_license_billable_users.yml) | <2 | 2 - 7 | >7 |
| Successful Deployments % | [Successful Deployments - L28D](https://example_company.com/example_company-org/example_company/-/blob/master/config/metrics/counts_28d/20210216181923_successful_deployments.yml) ÷ (Successful + [Failed Deployments - L28D](https://example_company.com/example_company-org/example_company/-/blob/master/config/metrics/counts_28d/20210216181924_failed_deployments.yml)) | <25% | 25% - 80% | >80% |
...
1. Some customers have been given an exception for sending usage data.
   ...
   1. [INTERNAL - Customer Availability, Eligibility & Opt-Out](https://internal.example_company.com/handbook/product/fulfillment/cloudlicensing/cloud-licensing/#customer-availability-eligibility--opt-out)
   1. [Cloud Licensing Adoption Dashboard](https://10az.online.tableau.com/#/site/example_company/views/WIPCloudLicensingPOC/CloudLicensingAdoption?:iid=1)
...
1. Usage is measured at the instance level, which is attached to a subscription, attached to an account. So a "Account" health score is a view of a single instance (the most important one) but, for more complex accounts, that can hide the health of other instances and subscriptions (see [graph](/handbook/customer-success/product-usage-data/using-product-usage-data-in-gainsight/#csmcse-actions)) that shows how a single account can have multiple subscriptions and each subscription could have multiple instances (self-managed only). Process for dealing with [multiple production instances](/handbook/customer-success/product-usage-data/using-product-usage-data-in-gainsight/#multiple-production-instances-health-scoring)
   1. **Resolution**: The ideal outcome is to "split" subscriptions apart into relevant child accounts, which is being [discussed here](https://example_company.com/example_company-com/sales-team/field-operations/sales-operations/-/issues/3032)
...
1. User Engagement: the `unique_active_user` metric debuted in 15.2 and only exists for self-managed instances. This health score will be NULL for all SaaS customers and any self-managed customer on 15.1 or earlier
   ...
   1. **Resolution**: Create [the metric](https://example_company.com/example_company-data/analytics/-/issues/11662) for SaaS customers
1. Most underlying metrics were created on 13.9 or earlier, though any instance on version 12 or earlier will not have metrics
   1. **Resolution**:
1. Usage data is reliant on internet access, the IP address NOT being blocked, and our license<>subscription mapping to be functioning as expected (see [License<>Subscription Mapping epic](https://example_company.com/example_company-org/fulfillment-meta/-/issues/634) assigned to Fulfillment to resolve)
   1. **Resolution**: Focus on Cloud License adoption (self-managed)
...
In Gainsight, scorecards track customer adoption of Example Company use cases.
