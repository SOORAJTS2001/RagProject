...
1. **Insights** into freemium accounts of their current Example Company usage
...
([**SaaS Free Trial Usage Stats in SFDC: Training Video**](https://youtu.be/28_bgDL__BQ) above is Private on Example Company Unfiltered)

#### Field Definitions

Below are the fields, descriptions, and best practices for each field. These can be used to help us understand what the customer is using Example Company for and how we can help them achieve their desired outcomes. For example, high ci pipeline usage indicates we may want to start the conversation around CI, or that they've already accomplished that objective.
...
| **Number of Active Users in Company**  | The number of billable users who have logged in and used 1+ stages in the last 3 months | As a rep, how many users are actively using Example Company? What is the difference between Billable (deployed) and Active (logging in and using it)? |
