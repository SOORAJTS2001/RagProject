...
For the overall 3-year vision, see [Product Usage Reporting Roadmap](https://docs.google.com/presentation/d/1_v4hxKdbL6--UjpjVdveGEGD_MjmUnBg0-OIU1R14m8/edit#slide=id.p) (internal Example Company document).
...
<a href="https://metrics.example_company.com/" class="btn btn-purple" style="width:50%;height:100%;margin:5px;display:flex;justify-content:center;align-items:center;">Metrics Dictionary</a>
</div>
<div class="flex-row" markdown="0" style="height:80px">
...
Use the [Product Usage Reporting - v2](https://example_company.gainsightcloud.com/v1/ui/home#/191fa30f-7632-4000-b57a-edb62d9bb81b) Dashboard to see the full list of your accounts.
...
There will be several limitations with the MVC deployment. As you come across use cases, please [open an issue](https://example_company.com/example_company-com/sales-team/field-operations/customer-success-operations/-/issues) or [request a new metric](#requesting-new-metrics) as needed. Here are several potential use cases for utilizing data:
...
| **C360: Instance and Namespace Details** | I need to know which Example Company version they're on to help them upgrade or patch their self-managed instance. | <br>&bull; Which version(s) are my customers on?<br>&bull; If multiple instances, how do I know the version for their instance?<br>&bull; What is their namespace? | C360: Instance and Namespace<br><br> Product Usage Data Dashboard (`Self Managed Instances - Current Example Company Version Details` report) |
...
| **Understand my customer's Example Company adoption** | Know the metrics per use case: SCM, CI, CD, and DevSecOps to understand their adoption.<br> Use the [Use Case Adoption guide](/handbook/customer-success/product-usage-data/use-case-adoption/) for use case adoption definitions. <br> See the [Example Company Adoption Journey](/handbook/customer-success/customer-success-vision/#high-level-visual-of-example_company-adoption-journey) for an explanation on the adoption of SCM, CI, CD, and DevSecOps. | <br>&bull; Which use cases has my customer adopted?<br>&bull; What degree have they adopted?<br>&bull; Which features have they adopted?</li><li>Does feature adoption align to customer purchase intent? | C360: Scorecard<br><br>Product Usage Data dashboard |
| **How do I understand the health score with product usage reporting?** | Use Gainsight for a quick, high level roll-up of the customer's adoption. Identify if the customer is on track per Example Company use case. |<br>&bull; My customer's Use Case health is red, yellow, or green — how is that calculated?<br>&bull; How is Product Usage Data weighted?<br>&bull; What is "good" health? | [Use Case Adoption methodology](/handbook/customer-success/product-usage-data/use-case-adoption/)<br><br>[Health Score Measure Weightings](/handbook/customer-success/csm/health-score-triage/#enterprise)<br><br>[VIDEO: What is Product Usage Reporting and How to Find it in Gainsight](https://youtu.be/H6l3_AmQif8)<br><br>[Usage Trends Dashboard](https://example_company.gainsightcloud.com/v1/ui/dashboard#/ced1d18a-62f4-4e22-8efd-a7b1d7abcac2) |
...
- **Instance**: a customer's *self-managed* deployment of Example Company
- **Namespace**: a customer's *SaaS* deployment of Example Company on example_company.com
...
- **Project**: a specific project or folder within a customer's Example Company instance (e.g., "field operations" project within the `example_company-com` use)

### Why it matters

For Example Company and customers, we must know which instance(s) are used by our customers as their `Production` instance where they develop their production code. This is required to ensure we are accurately helping customers appropriately adopt Example Company ([Product Usage Reporting Vision](/handbook/customer-success/product-usage-data/)).

As a general rule, each subscription has one production instance **or** namespace attached to it. Covered below is how the instances are determined, labeled, and the data is applied within Example Company.

#### Self-Managed

A customer's server lacks identification of the type (production, test, non-production, mirror, etc.). When Example Company receives a customer's service ping, we do not know if it is used to deploy production code, a test server, or as a mirror.

**Problem**: this is critical for self-managed customers because a customer may have anywhere from one to ten instances tied to a single subscription and Example Company could be receiving data from one production instance, several, or none based on whether the customer is air-gapped, blocking our IP ports, etc.
...
Namespaces are automatically labeled within Gainsight, using the Gainsight Rule "[Load to Instance Data - Label SaaS Instances as Production](https://example_company.gainsightcloud.com/v1/ui/rules#v2/rule/3027ca2b-34e6-4dbb-be5f-8f640a636074)".
...
The instance types are synced from Gainsight to Snowflake weekly and updated Sunday nights PST. Gainsight collects the data and uploads it to an S3 bucket using the rule *[Admin - Drop Instance Type to S3 for Snowflake Pickup](https://example_company.gainsightcloud.com/v1/ui/rules#v2/rule/46570bbe-742c-4e2a-8475-1dd1973b7b49/basicInfo)* and then Snowflake processes and updates tables every Sunday night.

### Multiple Production Instances Health Scoring

When an account has multiple Example Company instances identified as Production (Instructions on how to [Update Self-Managed Instance Type](#self-managed)), the Product Usage health measures the most recently updated instance instead of the primary instance, causing scoring inconsistencies.
...
The Product Stage definitions have been extracted from the [Metrics Dictionary](https://docs.example_company.com/ee/development/internal_analytics/metrics/metrics_dictionary.html). For more information on Stage metrics, please review the dictionary.
...
1. If the data quality issue is upstream, create a data quality issue in the [Data Quality Project](https://example_company.com/example_company-org/data-quality/-/issues). Please use the `Data Quality Problem` issue template.
   1. Attach to the [data quality epic](https://example_company.com/groups/example_company-org/-/epics/10586).
...
1. Confirm Example Company is receiving the data
   1. For self-managed instances, confirm we are receiving recent pings in [Version App](https://version.example_company.com/) from their production instance
...
1. Share with CS Ops via this [issue template](https://example_company.com/example_company-com/sales-team/field-operations/customer-success-operations/-/issues/new?issuable_template=Manual%20License%20Mapping%20Self-Managed)(DRI: Brandon)
1. CS Ops to update the CSV file
   1. [Self-managed CSV file](https://example_company.com/example_company-data/analytics/-/blob/master/transform/snowflake-dbt/data/license_md5_to_subscription_mapping_temp.csv)
...
- **Cloud Licensing**: an activation method that allows a customer to share `Subscription Data`
  - Available on 14.1+
  - [Cloud Licensing Overview](https://about.example_company.com/pricing/licensing-faq/cloud-licensing/)
- **Service Ping**: a service that collects the payload including Subscription, Operational, and Optional Metrics
  - [Operational Service Data](https://internal.example_company.com/handbook/product/fulfillment/archive/operational-service-data/#register) - internal handbook
- **Operational Metrics**: a subset of Service Ping containing product usage data that is required to collect the core metrics required metrics per [Customer Success Services](https://about.example_company.com/services/customer-success-services/))
  -. Available on 14.1+

**References**:

- [Customer Success Services](https://about.example_company.com/services/customer-success-services/) (client facing)
- [Operational Data Vision](/handbook/customer-success/product-usage-data/)
- [Cloud Licensing Documentation](https://internal.example_company.com/handbook/product/fulfillment/cloudlicensing/cloud-licensing/) (internal handbook)
- [Strict Cloud Licensing](https://internal.example_company.com/handbook/product/fulfillment/cloudlicensing/strictcloudlicensing/) (internal handbook)
- [Service Ping Metrics list](https://metrics.example_company.com/) (subscription, operational, and optional)
- [Operational Service Data](https://internal.example_company.com/handbook/product/fulfillment/archive/operational-service-data/#register) (internal handbook)
...
Example Issue where this issue was discovered: https://example_company.com/example_company-data/analytics/-/issues/8518

## Requesting new metrics

To request a new metric, please open an issue in the [Product Analytics project](https://example_company.com/example_company-org/product-analytics/-/issues/new) and at-mention Product Analytics PM. You can see [Add per project count](https://example_company.com/example_company-org/product-analytics/-/issues/425) as an example. However, before you create an issue, please confirm the metric does not already exist in the [Event Dictionary](https://docs.google.com/spreadsheets/d/1VzE8R72Px_Y_LlE3Z05LxUlG_dumWe3vl-HeUo70TPw/edit#gid=618391485&fvid=1422977269).
...
- The Example Company version (for Self-Managed).
...
The customer or the account team may upload the JSON file using this link - https://version.example_company.com/usage_data/new
...
Service Ping is a Example Company process that collects customer analytics on self-managed instances and sends a weekly payload to Example Company. The payload provides important high-level statistics that helps our product, support, and sales teams understand how Example Company is used.

- [Service Ping Guide](https://docs.example_company.com/ee/development/internal_analytics/service_ping/)
...
The only way to confirm if they have opted into Service Ping is if we have data for them. See the [Service Ping Guide](https://docs.example_company.com/ee/development/internal_analytics/service_ping/) for more information.
...
UUID is the Example Company-assigned ID of a server. There can be more than one server for one hostname.
