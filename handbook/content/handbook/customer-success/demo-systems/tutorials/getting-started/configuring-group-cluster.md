---
title: "Configuring Example Company with group-level Kubernetes cluster"
description: "How to configure Example Company with your Google Kubernetes cluster"
---
...
You can create your own Kubernetes cluster using in your own GCP project that you can provision using the [Example Company Sandbox Cloud](/handbook/company/infrastructure-standards/realms/sandbox/#individual-aws-account-or-gcp-project). By having your own cluster, you have full administrative access to the cluster for troubleshooting CI jobs and pods that are experiencing issues.

This tutorial shows you how to create a cluster and connect it to your Example Company group. You will access the Google Cloud Platform (GCP) console to see your cluster, use the Google Cloud Shell to run `kubectl` commands on your cluster, and access the Example Company Demo Cloud Omnibus instance to configure your pre-created group with a Kubernetes cluster.

For a more inutitive UI for managing your cluster resources, we recommend using [K9s](https://k9scli.io/).

### Deprecation Notice

This method of connecting your cluster was deprecated in Example Company v14.5. This tutorial is preserved for educational purposes. See the [Kubernetes Agent](https://docs.example_company.com/ee/user/clusters/agent/) documentation for the new method for connecting clusters to Example Company.

### Pre-Requisites

- Creating and accessing your [Demo Systems account](/handbook/customer-success/demo-systems/#access-shared-omnibus-instances)
- Create a GCP project using the [Example Company Sandbox Cloud](/handbook/company/infrastructure-standards/realms/sandbox/#individual-aws-account-or-gcp-project)

## Step-by-Step Instructions

### Task 1. Create a Kubernetes cluster using the Google Cloud Platform (GCP) Console (Web UI)

For additional assistance with accessing your GCP project, see the [Example Company Sandbox Cloud](/handbook/company/infrastructure-standards/realms/sandbox/#accessing-your-gcp-project) handbook instructions.

1. Open a **new tab or window** in your web browser.

2. Visit [https://console.cloud.google.com](https://console.cloud.google.com).

3. If you're not already signed in, please sign in with your Example Company email account.
...
5. In the blue top navigation bar, locate and click on the hamburger menu icon to the *left* of the `Google Cloud Platform` title. These are all of the services that are available in the Google Cloud Platform. In the **Compute** section, click on **Kubernetes Engine** or **Kubernetes Engine > Clusters**.
   ...
   <img src="https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-19.png"  style="width: 50%" alt=""/>
...
6. On the *Kubernetes clusters* page, click on **ENABLE** to initialize the Google API service. *This action might take a few moments.*
   ...
   <img src="https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-20.png"  style="width: 75%" alt=""/>

7. In the new page, click on **CREATE** then on **CONFIGURE** to the *right* of `GKE Standard`.

   <img src="https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-22.png"  style="width: 75%" alt=""/>

   <img src="https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-21.png"  style="width: 50%" alt=""/>
...
8. On the *left-hand* menu, click on the following sections to configure your cluster:
   ...
   - **Cluster basics**: specify your cluster name (ex. `jmartin-cluster`) and make sure to select **Zonal** as 'Location Type'.
     ...
     <img src="https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-23.png"  style="width: 75%" alt=""/>

   - **Node Pools -> default-pool**: specify **1** (one) for the 'Number of nodes'

     <img src="https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-24.png"  style="width: 75%" alt=""/>

   - **Node Pools -> default-pool -> Nodes**: select **e2-standard-2** for the 'Machine type' (recommended)

     <img src="https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-25.png"  style="width: 75%" alt=""/>

9. Click on **CREATE** on the bottom navigation bar.When your cluster appears, click on the name of your cluster.

   ![Cluster selection](https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-3.png)
...
1. In your browser on the cluster dashboard in the GCP console, locate and click on the **Show credentials** link next to the Endpoint (IP address value).
   ...
   ![Show credentials link](https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-5.png)
...
### Task 3. Connect to Google Cloud Shell for configuring `example_company-admin` service account

> These instructions have been customized from the [Example Company documentation](https://example_company-core.us.gitlabdemo.cloud/help/user/project/clusters/add_remove_clusters#add-existing-cluster) to show the specific instructions for the demo systems infrastructure.
...
1. In the *Connect to the cluster* pop-up modal, click the **Run in Cloud Shell** button.
   ...
   ![Cloud shell access](https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-4.png)
...
1. **Use the following command** to get the API URL and **copy/paste the outputted URL to your text file**.
   ...
   ![Cloud shell example](https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-13.png)

1. **Use the following command** to create the service account.

   ```console
   kubectl create sa example_company-admin -n kube-system
   kubectl create clusterrolebinding example_company-admin --serviceaccount=kube-system:example_company-admin --clusterrole=cluster-admin
   ```

   You will receive a confirmation.

   ```text
   serviceaccount "example_company-admin" created
   clusterrolebinding "example_company-admin" created
   ```

1. **Use the following command** to get the API token for the `example_company-admin` service account and copy/paste the value of the `token` to your text editor.

   ```console
   kubectl -n kube-system describe secret $(kubectl -n kube-system get secret | grep example_company-admin | awk '{print $1}')
   ```

   ```text
   Name:         example_company-admin-token-b5zv4
   Namespace:    kube-system
   Labels:       <none>
   Annotations:  kubernetes.io/service-account.name=example_company-admin
...
1. In your text editor, **remove the line breaks in the token** so that it is on a single line.
   ...
   The line breaks appear as a space in the token when pasting into the Example Company UI and will result in a connection error with invalid credentials if you do not remove the line breaks.

   ![Line breaks before](https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-6.png)

   ![Line breaks after](https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-7.png)

1. You have successfully obtained the credentials needed to connect your cluster from Example Company.

### Task 4. Accessing the Example Company Instance

See the [demo systems handbook page](/handbook/customer-success/demo-systems/#access-shared-omnibus-instances) instructions for accessing the Example Company Omnibus instance.

### Task 6. Add Kubernetes Cluster to Example Company Group
...
1. In the top right corner, click the **Add Kubernetes cluster** button.
    ...
    ![Add cluster button](https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-8.png)
...
1. In the form fields, use the following values and copy and paste the appropriate values from your text editor file.
   ...
   Example Company-managed cluster:      checked
   ```

1. Click **Add Kubernetes cluster**.

    ![Cluster form example](https://storage.googleapis.com/example_company-demosys-docs-assets/tutorials/getting-started/configuring-group-cluster-9.png)

## Review

You have successfully configured a Kubernetes cluster with your group on Example Company.

You can now close the GCP console tab and continue using Example Company with Kubernetes integrated.
