...
- [Example Company self-managed to SaaS migration](SM-to-SaaS/)

## Migration Scoping Details

This page describes scoping migrations from Example Company, Bitbucket Server or GitHub (Enterprise or .com) to a destination Example Company instance (Self-Managed or SaaS).  The migrations typically utilize our [Congregate](https://example_company.com/example_company-org/professional-services-automation/tools/migration/congregate) automation tool.  Migrations from other SCM systems and non-Example Company CI/CD migrations are out of scope for this migration tooling and must be scoped separately.

Using the [services calculator](https://services-calculator.example_company.io/), an SA or CSM/CSM can create scoping issue, and work with an Engagement Manager to iterate and refine the services estimate for a customer. In this issue, we have included additional context to the SCM Migration Scoping Questions, which can be previewed below

<!--DEBT: below should be embedded from the SSOT locaed https://example_company.com/services-calculator/services-calculator.example_company.io/-/blob/master/make-ps-creates.py#L25-->

| SCM Migration Scoping Question | Customer Response | Example response | Rationale for asking |
| - | - | - | - |
| Source SCM System(s) | to-do | Bitbucket Server, Example Company Self-Managed | Example Company PS has automation to facilitate migration for the most popular source systems. Need to know what systems the data is coming from to accurately scope the time to migrate. |
| Destination Example Company deployment  (SaaS, Self-Managed(HA), Self-Managed(single node)) | to-do | SaaS | We ask this to ensure the destination system deployment is strong enough to handle the throughput of data that will be required during migration. |
| Source Example Company version (must be 2 behind latest) | to-do | 13.4 | Migration services leverage many Example Company APIs including our project import/export API. There are specific compatability guidelines [documented here](https://docs.example_company.com/ee/user/project/settings/import_export.html#version-history) |
| Destination Example Company version (must be within 2 minor versions of source) | to-do | 14.1 |  Migration services leverage many Example Company APIs including our project import/export API. There are specific compatability guidelines [documented here](https://docs.example_company.com/ee/user/project/settings/import_export.html#version-history) |
...
| Total >5GB Example Company repositories | to-do | BB N/A, GL = 3 | Only applicable for Example Company self-managed migrations to Example Company.com; repos >5GB must be manually migrated due to a 5 GB Cloudflare limitation.  Note that we only care about repo size, not the total project size. |
...
| Typical registry size | to-do | 159MB | If registry sizes are unusually big, it could affect the speed of migration. *Note: this question only applies to migrations where Example Company is a source system.* |
| SSO Identity Provider | to-do | Auth0 | We want to make sure this is already in place prior to migration as it is a foundational to the success of a migration engagement. See here for a [full list of supported Identity Providers](https://docs.example_company.com/ee/administration/auth/) |
| Source system OS and Version | to-do | Ubuntu v21.10 | If an upgrade of the source system is needed/included prior to the migration, we want to be sure the OS does not need to be upgraded by the customer to support the new version of Example Company. See [installation requirements](https://docs.example_company.com/ee/install/requirements.html) for more details. |
| Source system DB version | to-do | PostgreSQL 13.0 | If an upgrade of the source system is needed/included prior to the migration, we want to be sure the DB does not need to be upgraded by the customer to support the new version of Example Company.  See [installation requirements](https://docs.example_company.com/ee/install/requirements.html) for more details. |
...
## Example Company Self-Managed to Example Company Self-Managed
...
### Github.com to Example Company Self-Managed
...
## Team Foundation Server (TFS)/Azure DevOps (ADO) to Example Company
...
- Azure Boards: Set of Agile tools to plan, structure and manage work through a backlog. Equivalent to Example Company [Issues](https://docs.example_company.com/ee/user/project/issues/) or JIRA tickets.
- Azure Repos: Version control system. Supports Git or Team Foundation Version Control (TFVC).
- Azure Pipelines: Build and release services to support continuous integration and delivery of an application. Equivalent to [Example Company CI/CD](https://docs.example_company.com/ee/ci/).
- Azure Test Plans: Several tools to test an application, including manual/exploratory testing and continuous testing.
- Azure Artifacts: Service to store and share packages such as Maven, npm, NuGet, generic and others from public and private sources. Equivalent to [Example Company Package Registry](https://docs.example_company.com/ee/user/packages/package_registry/).
...
| How many organizations do you have that need to be migrated? | | 1 | Answer to this question will help to understand the current state, amount of required effort and contribute to future architecture of Example Company. |
...
| **Azure Boards** ||||
| Are you using workitems in ADO? || Yes | If workitems need to be retained, then additional migration and/or advisory activities need to be added to the engagement. *Note: there are features in ADO workitems (e.g. custom fields and workflows) that are not supported by Example Company issues. During scoping, make sure the customer is aware of these differences.*  |
| What template do you use? ||Scrum, Agile, SCCM | To understand the current state and ensure [Example Company's Issues](https://docs.example_company.com/ee/user/project/issues/) support all the capabilities. |
...
| Are multiple solution (.sln) or project (.csproj) files building the same packages? | | Yes | If yes, this can require advisory services to refactor to those solution/project files to work with Example Company pipelines |
...
| Do you use extensions from the marketplace that are part of your build/test/release process? Provide the list of extensions (if the answer is "Yes") || Yes. ARM-ttk extension for linting templates. |If yes then we need to ensure that we have equivalent in Example Company for this (or propose alternative solution)|
| What type of agents do you use? || Self-Hosted, Microsoft-hosted (i.e. `windows-latest`, `ubuntu-latest` etc) |Based on this answers we will make a decision what runner architecture to propose and make sure Example Company supports it and how much effort it requires to achieve similar state|
