---
title: "Example Company CI/CD - Hands-On Lab: Code Quality Scanning"
...
### Task B. Configure the .example_company-ci.yml with Code Quality Scanning

1. In the left navigation pane, click **Code > Repository**.

1. Click on your existing `.example_company-ci.yml` file.
...
1. Delete all of the existing code in the file. Replace the code with the following file content:
    ...
    include:
      - template: Code-Quality.example_company-ci.yml
...
1. The top row represents the pipeline that started running when you committed the `.example_company-ci.yml` file in the previous section. Wait until the status icon at the left of that pipeline says **passed**.
...
1. Notice that the scanner found 3 code quality issues in 2 different files: 2 in `HelloWorld.py` and 1 in `.example_company-ci.yml`.
