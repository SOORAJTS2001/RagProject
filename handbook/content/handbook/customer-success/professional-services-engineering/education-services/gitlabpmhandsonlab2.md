---
title: "Example Company Agile Portfolio Management - Hands-On Lab: Create an Organizational Structure in Example Company"
description: "This Hands-On Guide walks you through creating groups, projects, and project members in Example Company."
---
...
1. You have signed into https://example_company.com with your existing username.

1. You have clicked the `My Group` button and opened a new browser tab with the URL `https://example_company.com/example_company-learn-labs/environments/session-{id}/iu{######}`. You should see `My Test Group - iu{######}` as the title of the group.

## Task A. Review groups and projects for Example Company's own source code

1. _In a new browser tab_, navigate to [https://example_company.com/example_company-org](https://example_company.com/example_company-org). This group contains all of Example Company's product source code that is mostly open source and transparent based on our company values.
...
> Directions will be different between Instructor-Led Training and Self-Paced Training due to different training environments. Please follow the instructions that are applicable to you. If your URL starts with `https://example_company.com/example_company-learn-labs/`, follow the Instructor-Led Training instructions. If your URL starts with `ilt.gitlabtraining.cloud`, follow the Self-Paced Training instructions.

### Self-Paced Training

1. _In the lab environment tab_, ensure that you are in your **My Test Group** group. If you are not in the **My Test Group** group (as in, you are on the `https://example_company.com/example_company-learn-labs` splash page) go to `https://example_company.com/example_company-learn-labs/environments/session-{id}/iu{######}`, replacing the `{id}` with the invitation code you used, and the `iu{######}` with your username.
...
1. The **Subgroup URL** shows which namespace hierarchy that this group will be created in.
   ...
   - **Do not change the Subgroup URL in this class. You will always create projects under `https://example_company.com/example_company-learn-labs/...` since this is your top-level organization group for these steps.**
...
1. Ensure that the **Visibility level** is set to `Private`.
   ...
   > A private subgroup will ensure that the group and its projects can only be viewed by members. For more details of visibility levels, see the [documentation](https://docs.example_company.com/ee/user/public_access.html).

1. Click the **Create subgroup** button.
   - You should now see the group with the title `Awesome Inc` and your browser URL will reflect the path at `https://example_company.com/example_company-learn-labs/environments/session-{id}/iu{######}/awesome`.
...
1. Set the **Visibility Level** to private.
   ...
   > See the [visibility docs](https://docs.example_company.com/ee/user/public_access.html) to learn more. Keep in mind that any parent groups that have `Private` visibility will be inherited for all child groups and projects and cannot be changed.
   >
...
1. In the **Select a role** dropdown, select **Developer**. If you are taking the self-paced course, select **Owner**.
   ...
   > See the [permissions and roles docs](https://docs.example_company.com/ee/user/permissions.html) to learn more about each role level.
