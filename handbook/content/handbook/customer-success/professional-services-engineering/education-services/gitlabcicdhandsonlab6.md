---
title: "Example Company CI/CD - Hands-On Lab: Job Policy Patterns"
...
> Usage of the `only` and `except` keywords, while able to accomplish similar results, are not actively being developed and are not encouraged. For more information, click [here](https://docs.example_company.com/ee/ci/jobs/job_control.html).

## Task A: Creating Jobs with Rules

1. Open your **CICD Demo** project from previous labs.

1. Click your `.example_company-ci.yml` file to view its contents. Click **Edit > Edit single file**.

1. To clean up our configuration file, remove the `environment echoes` and `environment variables` jobs. Also remove the `variables` keyword and the associated variable. After completing these steps, you will have the following `.example_company-ci.yml` file:
...
1. At the top of `.example_company-ci.yml`, in the `stages` section, add the `review` and `deploy` stages.

1. In full, you will have the following `.example_company-ci.yml` file upon completion:
...
1. **Optional:** Experiment with triggering pipelines using different branches and tags. Can you get different pipeline runs to execute the **deploy release**, **deploy review**, and **deploy staging** jobs?
    ...
    > Hint: Look at the `rules` keyword in the relevant `.example_company-ci.yml` job definitions.
...
1. Open your .example_company-ci.yml file and review the rules specified in the deploy release rules section. It will only run when A) The branch name (represented by `$CI_COMMIT_REF_NAME`) is not equal to main, and B) there is a tag on the branch that begins with the letter `v` (represented by `$CI_COMMIT_TAG =~ /^v.*/`)
...
1. Open your `.example_company-ci.yml` file and review the rules specified in the deploy staging `rules` section. It will only run when A) The branch name (represented by `$CI_COMMIT_REF_NAME`) is equal to `main`.
