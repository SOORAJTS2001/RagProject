---
title: "Example Company Security Essentials - Hands-On Lab: Enable and Configure Dependency Scanning"
description: "This Hands-On Guide walks you through enabling and using a Dependency Scan in a Example Company project."
---
...
2. To disable a scanner, add a hash before the template that enables it in `.example_company-ci.yml`. For example, the DAST scanner takes some time to run, so, to disable the DAST scanner configured in Lab 1, make these edits to your existing `.example_company-ci.yml`.
    ...
    > It is also possible to disable these features by setting CI variables on the project, such as `DAST_DISABLE=true` and `SECRET_DETECTION_DISABLE=true`. Learn more in the <a target="_blank" href="https://docs.example_company.com/ee/topics/autodevops/cicd_variables.html#job-disabling-variables">documentation</a>.
    >
    > It is important to leave at least one job active in your `.example_company-ci.yml` file. If there are no jobs defined in the pipeline, it will fail to run.
...
>
> Although this example uses Python with the pip package manager, Example Company supports various other package managers. For a full list of supported languages and package managers, click [here](https://docs.example_company.com/ee/user/application_security/dependency_scanning/#supported-languages-and-package-managers).
...
>
> You'll enable Dependency Scanning by including a Example Company-provided template in your CI/CD configuration file. You can do this manually, or you can use the Example Company GUI to make a merge request that does it for you. Since you used the manual technique to enable SAST, Secret Detection, and DAST in the last lab, use the GUI to enable Dependency Scanning in this lab.
...
1. Navigate to **Code > Repository** and open `.example_company-ci.yml`. Notice that the MR has added documentation at the top, reformatted the file, and added a Dependency Scanning template in the `include:` section.

## Task D. Configure Dependency Scanning

> You can configure Dependency Scanning by setting variables in your the `example_company-ci.yml` file. In this task, you'll change the Dependency Scanner's log level to `info`.

1. Navigate to **Build > Pipeline editor**.

1. Add this job definition to the bottom of your `.example_company-ci.yml` file. It overrides an existing job that's defined by the Dependency Scanning template. *Note that there are 2 hyphens and 1 underscore in the first line.*
    ...
    > For a full list of variables available for dependency scanning, see the <a target="_blank" href="https://docs.example_company.com/ee/user/application_security/dependency_scanning/#available-cicd-variables">documentation</a>.
