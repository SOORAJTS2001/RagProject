---
title: "Example Company with Git Fundamentals - Hands-on Lab: Work With Git Locally"
...
1. If the output prints a version number, Git is installed. If the command throws an error, see the [documentation](https://docs.example_company.com/ee/topics/git/how_to_install_git/) for installing Git.

## Task B. Generate an SSH key

> Example Company uses the SSH protocol to securely communicate with Git. When you use SSH keys to authenticate to the Example Company remote server, you don't need to supply your username and password each time. You can learn more in the [documentation](https://docs.example_company.com/ee/user/ssh.html).
...
## Task C. Add an SSH key to your Example Company profile
...
1. You should see two key files: a public key (ex. `id_rsa.pub`) and a private key (ex. `id_rsa`). The public key ends with `.pub` and is what you need to share with Example Company.
...
1. In your web browser in the Example Company Web UI, click the **Add new key** button.
...
1. For **Usage type**, make sure **Authentication & Signing** is selected
   ...
   > This usage type allows your key to be used to authenticate with Example Company, as well as for signing commits. You can learn more about signed commits in the [documentation](https://docs.example_company.com/ee/user/project/repository/signed_commits/ssh.html)
...
1. In your terminal, run the following command to test your connection.
   ...
   If your instance URL includes `example_company-learn-labs/*` run:

   ```bash
   ssh -T git@example_company.com
   ```
...
## Task D. Clone a Example Company project repository to your local computer

> When you clone a repository, the files from the remote repository are downloaded to your computer, and a connection is created. You can learn more in the [documentation](https://docs.example_company.com/ee/example_company-basics/start-using-git.html#clone-a-repository).
...
> Branches are versions of a project's working tree. When you create a new project, Example Company creates a default branch for your repository named `main` (formerly `master`) that cannot be deleted. Default branch settings can be configured at the project, subgroup, group, or instance level. You can learn more in the [documentation](https://docs.example_company.com/ee/user/project/repository/branches/).
...
4. The red branches are on the remote server, which is the Example Company instance where your repositories are stored at.
...
## Task I. Push your changes to the Example Company instance

1. Create a new branch in the remote Git repository on the Example Company server called **temporary_branch**, and push your changes to that branch using the `git push` command.
...
1. In your local machine's text editor (not Example Company's in-browser editor), add this new line to the end of your local copy of `README.md` and save the file.
...
1. Push your commit up to the remote repository on the Example Company instance by typing in the `git push` command.

   ```bash
   git push
   ```

   > To commit your changes to the upstream branch (that is, an already-existing branch on the remote repository with the same name as the branch on your local machine), you can just run `git push` instead of the longer command you used the first time you pushed your commit up to the Example Company instance. The system only needs to set the upstream branch once.

1. Navigate to your project in the Example Company Web UI.
...
> Let's simulate someone else in your organization making a change to the **temporary_branch** that lives in the remote repository on the Example Company instance. When we're done with this section, the remote and local versions of **temporary_branch** will be different: the code on that branch will have moved under your feet (so to speak). In the section after this one, we'll see how to reconcile this difference.

1. In Example Company, navigate to the **Top Level Project** landing page. If you're not already on **temporary_branch**, go to the left-hand navigation pane and click on **Code > Branches > temporary_branch**.
...
After making this commit, the remote repository on the Example Company instance is now one commit *ahead* of your local repository.

## Task L. Get metadata about changes to the remote **temporary_branch**

> Your local **temporary_branch** is out of sync with the remote **temporary_branch** on the Example Company instance. The `git fetch` command gets the updated state of remote branches without updating the contents of your local branches. In other words, it tells you how many commits your local branches are behind the remote branches, but it doesn't make any changes to the files in your local branches.
...
1. View the updated contents of the file by typing in the `cat README.md` command. You should see the fourth line that you added in the Example Company Web IDE.
...
1. Return to the Example Company page in your browser and view `README.md` in your project's **main** branch to view the changes you just pushed to the remote copy of **main**.
