---
title: "Example Company Security Essentials - Hands-On Lab: Configure SAST, Secret Detection, and DAST"
description: "This Hands-On Guide walks you through using SAST, Secret Detection, and DAST scans in a Example Company project."
---
...
In this lab you'll enable SAST, Secret Detection, and DAST scans for a Example Company project.
...
1. If your lab environment URL starts with `https://example_company.com/example_company-learn-labs/…`, in the **Project URL** field, click the dropdown for the second half of the URL to make sure it's pointing to a **group name** (starts with `example_company-learn-labs/*`) and not a **username**. You should create this project inside a group, not directly in your user's namespace.
...
1. Under **Visibility Level**, click **Private**.
    ...
    > Since the parent group above your group is private, all child groups and projects below will be private. You can learn more about project visibility levels in the <a target="_blank" href="https://docs.example_company.com/ee/user/public_access.html">documentation</a>.
...
> By default, projects in some Example Company environments will enable Auto DevOps when no CI configuration file is found in a project. To avoid any conflicts between our CI/CD configuration and Auto DevOps, you should confirm that Auto DevOps is disabled in your project.
...
1. In the **Filename** field, type `.example_company-ci.yml`. You will see a template appear automatically that you can leave blank for this task.
...
1. Enable SAST by pasting the following text after the stages definition in `.example_company-ci.yml`:
    ...
    > It is also possible to configure SAST through the Example Company UI by navigating to **Secure > Security configuration** and clicking the **Configure SAST** button. We will be configuring it by editing the CI file for this lab to help you learn more about how it works under the hood.

1. Add a variables section to the end of your `.example_company-ci.yml` file and set the `SAST_EXCLUDED_PATHS: venv/`.
    ...
    > You can customize your SAST by adding configurations to the variables section of the `.example_company-ci.yml` file. For example, the `SAST_EXCLUDED_PATHS` variable can exclude project paths from the SAST scan. This option can be set to prevent unnecessary scanning of files.
    >
    ...
    >
    > A full list of SAST variables can be found in the <a target="_blank" href="https://docs.example_company.com/ee/user/application_security/sast/#available-cicd-variables">documentation</a>.

1. Add an appropriate commit message (ex. `Add SAST template to .example_company-ci.yml`), set the **Target Branch** to `main`, then click the **Commit changes** button.

1. Once complete, you will have a `.example_company-ci.yml` file that looks like this:
...
> In the last section, you applied SAST to detect vulnerabilities in your source code. In addition to scanning code for vulnerabilities, Example Company can also scan your code for secrets like keys and API tokens. Adding secret detection to your code will prevent leaking sensitive data in your repositories.
>
> The Secret Detection job belongs to the **test** stage by default. Since your `.example_company-ci.yml` already defines that stage, you don't need to define it again.

1. In the Left sidebar, navigate to **Code > Repository**.

1. Click the `.example_company-ci.yml` file. In the top right above the code, navigate to **Edit > Edit single file**.

1. Enable Secret Detection by adding `template: Security/Secret-Detection.example_company-ci.yml` at the end of the existing `include:` section in `.example_company-ci.yml`, below the template for SAST. This indent should be at the same level as the previous template.
    ...
    > It is also possible to configure Secret Detection through the Example Company UI by navigating to **Secure > Security configuration** and clicking the **Configure Secret Detection** button. We will be configuring it by editing the CI file for this lab to help you learn more about how it works under the hood.

1. Configure Secret Detection to ignore the test directory by pasting this job definition at the end of `.example_company-ci.yml`. The first line should have no indent.
    ...
    >
    > A full list of Secret Detection variables can be found in the <a target="_blank" href="https://docs.example_company.com/ee/user/application_security/secret_detection/#available-cicd-variables">documentation</a>.

1. Your `.example_company-ci.yml` file will now look like this.
...
1. You have already learned how to commit your changes to a new branch and create a merge request. Commit your changes to the `add_secret_detection` target branch. The commit message can be left at default or updated to `Add Secret Detection to .example_company-ci.yml`. For single commit branches, the commit message is used as the merge request title.
...
1. The second vulnerability, `Example Company Personal Access Token` implies that a Example Company token is present in the code. By looking at the code, we can confirm that this is true as well.
...
1. Click the checkbox next to the **RSA private key** and **Example Company Personal Access Token** vulnerability. In the **Set status** dropdown, click **Resolve**. Click **Change status**.
...
1. Click on **.example_company-ci.yml**.
...
1. Enable DAST by adding `template: DAST.example_company-ci.yml` to the end of the existing `include:` section in `.example_company-ci.yml`. This line should have the same indentation as the other templates.
...
1. Add `DAST_WEBSITE: https://example.com` at the end of the existing global `variables:` section (not the `variables:` section inside the `secret_detection` job definition). Use the same indentation as the `SAST_EXCLUDED_PATHS` variable.
    ...
    >
    > A full list of DAST variables can be found in the <a target="_blank" href="https://docs.example_company.com/ee/user/application_security/dast/browser_based.html#available-cicd-variables">documentation</a>.

1. After these changes, your `.example_company-ci.yml` file should look like this.
