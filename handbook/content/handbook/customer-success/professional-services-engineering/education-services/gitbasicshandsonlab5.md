---
title: "Example Company with Git Fundamentals - Hands-on Lab: Auto DevOps With a Predefined Project Template"
...
1. Click on the pipeline's **running** status icon and note the stages (represented by columns in the pipeline graph) and the jobs that Auto DevOps has created.
   ...
   > You may see the `dast` job fail in your pipeline. This job requires additional configurations to scan successfully. See the [documentation](https://docs.example_company.com/ee/user/application_security/dast/#configuration) to learn more about DAST scan configurations.
   ...
   ```js
      app.get("/", (req, res) => {
         return res.status(200).send({
            message: "Hello World from Example Company!",
         });
      });
   ```
...
1. Click **Edit > Edit single file** and modify the line `assert.equal(res.body.message, 'Hello World!');` to `assert.equal(res.body.message, 'Hello World from Example Company!');`. After completing the edits, your code will look like this:
   ...
   it('responds responds to the world', async function() {
      ...
      assert.equal(res.body.message, 'Hello World from Example Company!');
      });
   });
