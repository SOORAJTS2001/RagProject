...
1. Review when the referral was submitted in Greenhouse
    ...
    - If the referral was submitted between **2022-06-01** and **2023-11-19**, then the following referral amounts apply:
        ...
        - $1,500 supplemental referral bonus for a hire from a ["select underrepresented group"](https://about.example_company.com/culture/inclusion/#examples-of-select-underrepresented-groups).
...
1. Check to see if the vacancy is posted on the external [Job Board](https://boards.greenhouse.io/example_company/) and if it is, **tag** the *Referrer*, **comment** that they'll need to submit their referral directly in Greenhouse per [these instructions](/handbook/hiring/referral-process/#submitting-a-referral-in-greenhouse), and **close** the Issue.
...
1. Send a email to the referred candidate. Select `Email REFERRAL_NAME` in the right column and select the **Default Referral Confirmation Receipt** email template.
    - Make sure that the email is sent from the `no-reply@example_company` alias.
...
The Talent Acquisition Team uses the a Example Company Service Desk to track incoming emails to the `referral@example_company` email. These emails will, in turn, show up as `Issues`.

To set-up notifications for the [Referral Service Desk Project](https://example_company.com/gl-talent-acquisition/operations/), please do the following:
...
1. Respond to the Issue by commenting and please be sure to do so just as you would with any other Example Company communication.
