...
The CES team utilizes [Example Company Service Desk](https://about.example_company.com/stages-devops-lifecycle/service-desk/) to track all incoming requests to the CES team. If you are a Example Company team member and you have a request or question for the Candidate Experience team, email `ces@example_company.com` and a ticket will automatically be created within the CES Service Desk. Please see the [alignment page]({{< ref "recruiting-alignment#recruiter-coordinator-and-sourcer-alignment-by-department" >}}) to learn more about which CES team member will be working on your requests.
...
1. Under this [CES Service Desk Project](https://example_company.com/gl-talent-acquisition/ces-service-desk) set up the proper notifications
...
- After availability is received, CES will schedule interviews based on the interview plans in Greenhouse and will track preferences within the CES Service Desk [repo](https://example_company.com/gl-talent-acquisition/ces-service-desk/-/tree/master).
...
- Candidates must submit a Example Company username *before* CES can schedule their interview
...
- When a candidate successfully reaches the Team Interview stage the recruiter will send the availability request email to the candidate.
  - If the candidate did not submit a Example Company username during the application or screening stage, the recruiter will also send the Example Company Username Form (this will be found in the Team Interview Stage or Evergreen stage). This generates an email to the candidate requesting they fill out the form with their Example Company Username.
- CES will receive two separate tickets to the CES service desk:
  ...
  - The completed Example Company Username form. This will only come into the service desk once the candidate replies to the email, and the username will automatically be added to the Details tab of the candidate's profile under Details > Other. CES should check that the GL username is valid and ensure that the GL username is in a URL format.
...
On occasion, candidates may need to interview in person with a team member at Example Company or they may require a reasonable accommodation to be successful in their interview process. In these cases, candidates are eligible for reimbursement from Example Company to cover any expenses incurred during this process. Please see process below on to handle candidate reimbursement in these cases. DRI - Senior CES, CES Manager
...
1. CES will send "Candidate Reimbursement" email template from Greenhouse from their own Example Company email address, not ces@example_company.com. **Note:** these emails will need to stay out of the CES service desk, hence the reason the CES responsible for this process will send from their personal Example Company email.
...
Example Company will obtain references and complete a criminal [records check](/handbook/people-group/contracts-probation-periods/#background-screenings) with employment verifications.
...
1. Next, select "Example Company" from the "Job Position" drop down menu.
...
1. Set reminders to follow up to see if Background check has cleared and/or reach out to the Background Check specialist at Example Company to confirm.  You can set reminders in the candidates GH profile and/or due dates on your ticket in Example Company.
1. Once confirmation of background check clearance is received, the offer can then be sent and the Reference Check form ticket can be closed. Normal [offer process]({{< ref "ces-contract-processes#employment-agreements-for-example_company-team-members-in-japan" >}}) will continue from there.
...
1. Once the recruiter has sent CES a contract ticket, CES will then complete **NYC Step 2** of the background screening through the Greenhouse integration. Once in Sterling, select the NYC Step 2 screening package and send.
    **Troubleshooting Tip:** If you receive an "expired" error when initiating Step 2 through the Greenhouse Sterling integration, you can navigate to the [Sterling Direct dashboard](https://secure.sterlingdirect.com/sys/). On the right hand side, under "E-Invite Applicant to Background Check", enter in the candidate's email address and select "Invite".
    ...
    - Under "Order Information", select Example Company as the Job Position and under "Screening Package" select NYC Step 2.
...
1. CES will visit the [TA Candidate PIAA and Outside Work Disclosure Group](https://example_company.com/groups/gl-ta-candidate-disclosures)
...
As a Example Company team member taking part in these calls, we advise you to start with a short introduction to you and your role here at Example Company. From here, we advise you to let the candidate lead the conversation as the goal is for you to answer their questions and offer insight into how we work.
...
1. The Candidate Experience Specialist will email the new team member the Welcome Email from Greenhouse with a cc to IT Ops, the Hiring Manager and the Recruiter.  For new team members in USA, use 'Example Company Welcome - US only' template.  For team members located outside the US, use 'Example Company Welcome - non US' template

   - Instructions on the [Notebook Ordering Process](/handbook/it/end-user-services/onboarding-access-requests/#example_company-laptops) are included with this email.
...
**SETTING GREENHOUSE REMINDERS** - CES make use of the due date feature in the Example Company issue to keep track of the start date and maintain consistent communication with the recruiters to ensure the candidates in the requisition are cleared completely. CES can make use of the "Need to hire into Workday" label and set due dates in the Example Company issue to track as well.
...
People Connect Specialist will create the onboarding issue and start the [onboarding tasks](https://internal.example_company.com/handbook/people-group/people-operations/people-connect/onboarding_process/) no later than two weeks before the new team member joins. Should a contract not be signed prior to 10 working days from the start date, a new start date will be required.

For questions about the new team member's onboarding status, you can @mention them in the `#people-connect-ces` Slack channel.

For questions about the new team member's laptop, ping [<i class="fa-brands fa-slack"></i> IT Ops](https://example_company.slack.com/archives/CK4EQH50E) in Slack. If the questions arise through email, forward the email to itops@example_company.com and ping IT Ops in #it-ops Slack, and @it-ops-team too due to volume.

### Employment Offboarding

When team members depart Example Company, People Connect will create an offboarding issue. The CES team is responsible for completing tasks under the `@example_company-com/gl-ces` section. Please note that offboarding issues are time sensitive and should be completed by the due date set on the issue. Completion of these tasks also ensure Example Company remains compliant.

1. Go to [Employment Issues](https://example_company.com/example_company-com/team-member-epics/employment/-/issues) where the offboarding issues can be found. In the search bar, filter issues by the "CES: To Do" label and work on the issues by due date. Once you open up an offboarding issue, navigate to the `@example_company-com/gl-ces` section and complete the following steps.
...
1. If the offboarding team member is from an engineering department, CES to check the hiring repo both [backend](https://example_company.com/example_company-com/people-group/hiring-processes/-/blob/master/Engineering/Frontend/2-TechnicalInterview/Interviewers.md) and [frontend](https://example_company.com/example_company-com/people-group/hiring-processes/-/blob/master/Engineering/Backend/Interviewers.md?plain=0#rubyrails-technical-interviewers) to remove the departing interviewer from the pool.
...
1. Check for reqs that the team member might be on by searching the team member's name under `Users` in Greenhouse. When searching the team member's name, select the profile with the team member's Example Company email address; this will navigate to their Greenhouse profile settings.
