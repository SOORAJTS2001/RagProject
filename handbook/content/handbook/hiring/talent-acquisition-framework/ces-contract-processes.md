...
1. The candidate lives in a country we are able to hire in. The SSOT for all country hiring is [here](https://comp-calculator.example_company.net/hiring_status/index).
...
This chart reflects the available signatories for each of Example Company's entities. They are listed in order of preferred signatory first. This chart should only be used for new hire contracts where Example Company has entities. This is not for internal moves such as [contract renewals]({{< ref "contracts-probation-periods#contract-renewals" >}}), [job change letters]({{< ref "contracts-probation-periods#job-change-letter" >}}), or [relocations]({{< ref "relocation" >}}).

| Entity | Signatory |
|-------- | -----|
| Example Company Inc | All Recruiting Leads (Jake Foster, Marissa Farris), Jess Dallmar, Rob Allen, Other People Group Team Members who are employed by Inc. and of the appropriate grade to sign the document. JCLs can be signed by Morgan Wilkins  |
| Example Company Federal LLC    |       Bob Stevens     |
| Example Company BV Belgium, Dutch & English | Marissa Farris, Pattie Egan|
| Example Company BV, Netherlands | Marissa Farris & Pattie Egan|
| Example Company Canada Corp | Simon Mundy |
| Example Company France S.A.S. | Rob Allen & Pattie Egan & Jake Foster|
| Example Company GK, Japan | Jess Dallmar, Rob Allen & Jack Connors Backup: Simon Mundy|
| Example Company GmbH | Marissa Farris, Rob Allen & Pattie Egan|
| Example Company Iberia S.L (Spain) | Jake Foster, Rob Allen, Pattie Egan. Backups: Jim Gladen & Simon Mundy |
| Example Company Ireland LTD | Marissa Farris, Rob Allen, Jake Foster, Jack Connors & Pattie Egan|
| Example Company Israel LTD | Jim Gladen, Simon Mundy|
| Example Company IT BV | Marissa Farris, Pattie Egan, Jake Foster, Jack Connors |
| Example Company PTY Ltd, Australia | Jess Dallmar & Rob Allen, Jack Connors. Backup: Jim Gladen|
| Example Company PTY Ltd, New Zealand | Jess Dallman & Rob Allen, Jack Connors. Backup: Jim Gladen|
| Example Company South Korea | Jess Dallmar, Jack Connors & Rob Allen |
| Example Company Singapore PTE. LTD. | Jess Dallmar & Rob Allen. Backup: Simon Mundy |
| Example Company UK Ltd | Marissa Farris, Rob Allen, Pattie Egan|
...
1. Ensure the start date is on a Monday, unless Monday is a holiday/[no start date](https://internal.example_company.com/handbook/people-group/people-operations/people-connect/people_connect_team/#people-connect-availability) or the non-Monday date is legally required.
...
1. Choose the correct template according to country, or if no country template is available select `Offer through DocuSign`. In the "To User" field, choose the Example Company signatory for the contract. In the "CC" field, add the recruiter and the hiring manager. Then click "Preview on DocuSign".
1. You will be redirected to DocuSign.
1. On the top left of the screen, click where it says the candidate's name, then click "Edit Recipients". Change the `2` next to the Example Company signatory's name to a `1`, change the `1` next to the candidate's name to a `2` and change the `1` next to the hiring manager's name to a `2`. This ensures that the contract goes to the Example Company signatory to sign first, as well as the recruiter for a Cc and once signed by them it will go to the candidate and the hiring manager with a Cc. Then click "Done".
1. Scroll throughout the contract and double check that the candidate and Example Company signatory signature and date fields are accurately added.
...
1. There may also be other fields you'll need to add a textbox for, so double check if there are any other fields that need to be completed by either the candidate or Example Company signatory. Examples include adding a text box for "Place" on German contracts, or adding text boxes for PIAA at the bottom of most contracts. Once you've verified that all the information is correct and appropriately assigned, click "Send" at the top right corner.
...
1. In the internal Example Company Handbook, you'll navigate to [this link](https://internal.example_company.com/handbook/people-group/people-operations/people-connect/employment_contracts/) as a SSOT for all contract templates. Only the People Group, Finance, and Legal should have access to this folder.
...
1. Enter the template name (The convention is typically "Example Company `Entity` `employment-type`, `with/without bonus`", e.g. `Example Company Inc full-time, with variable bonus/commission`).
...
Consistent with Example Company's policy governing Hiring Significant Others or Family Members, Example Company is committed to a policy of employment and advancement based on **qualifications and merit** and does not discriminate in favor of or in opposition to the employment of significant others or family members. Any new hire that has a family member relationship that is currently employed at Example Company must sign [this acknowledgment](https://docs.google.com/document/d/1IseZy4zJZMgP0VCAsGqCP3I6sfnavGdD81THnqljaPI/edit?usp=sharing) along with the Example Company team member. This process is done in conjunction with sending out the contract.
...
1. The Recruiter will post the name of the family member in the Private Notes within the Greenhouse profile. If not there, the CES will reach out to the recruiter to find out who the current Example Company team member is.
1. Once the family member is identified, you will send the [Family Member Relationship Acknowledgment](https://docs.google.com/document/d/1IseZy4zJZMgP0VCAsGqCP3I6sfnavGdD81THnqljaPI/edit?usp=sharing) via DocuSign to the new hire as well as the Example Company team member for signing.
...
- #1 Example Company Team Member (needs to sign)
...
1. Upload Family Member Relationship Acknowledgement form into Workday.
   ...
   - Upload
   - Repeat same steps above this time finding the Example Company team member in Workday
...
To change a start date after a **Example Company entity** contract has been signed and the new team member has been "hired" in GreenHouse the Candidate Experience Specialist will complete the following steps:
...
### How to Update a Start Date After a French or Spain contract has been signed
   ...
   1. In Docusign, create a new envelope and set the signing order as follows:
      - 1 - Example Company France or Spain Signatory (sign)
...
### Amendments prior to starting with Example Company
...
### Amendments after starting with Example Company

A contractor requests a modification to their contract due to a name change/company incorporation (Example: The individual recently incorporated a company, and would like to invoice Example Company through their company versus individually)
...
1. On the top left of the screen, click where it says the candidate's name, then click "Edit Recipients". Change the order of recipients to reflect the following order:
    ...
    - This ensures that the contract goes to the Example Company signatory to sign first, as well as the recruiter for a Cc and once signed by them it will go to the People Experience for an update of records.
1. Then click "Done".
...
1. Open the saved unsigned contract in Google Drive and replace Signatory's name with "Example Company Signatory". Save the contract as a PDF format.
...
A contractor conversion is when a Example Company contractor is moving into a permanent position. Recruiters should communicate to their CES partner that a candidate is going through a contractor conversion process. You can confirm this by checking that the "source" says Contractor Conversion, which will show up on the requisition. CES team members will follow the same steps as a regular contract, however there are a few nuances to note:
...
1. **Background Checks:** Please check with the Senior Background Check Specialist in the [backgroundchecks-ces](https://example_company.enterprise.slack.com/archives/C05ATK8B4V9) Slack channel to confirm if you need to initiate a new background check. If there is no gap between the contract and the permanent position, you may not need to initiate a new background check.
1. **Mark as Hired:** After you mark a candidate as hired in Greenhouse, you will need to leave a comment notifying People Connect in the [peopleops-alerts](https://example_company.enterprise.slack.com/archives/CLTBQ9XC7) Slack channel that it is a contractor conversion.
1. **Laptop/IT:** For all contractor conversions, CES will need to confirm with IT Ops via the [people-connect_CES](https://example_company.enterprise.slack.com/archives/GJT1K944Q) Slack channel whether the candidate will need a new laptop or not.
...
## Process for Example Company team-members in the Netherlands
...
It is common for Dutch employers to offer a second temporary contract when the first expires, but it's not guaranteed. As a Dutch employer, this is standard procedure for Example Company. As of 2015-07-01, employees who have worked with an employer on temporary contracts for at least two years are entitled to an indefinite contract if the work agreement continues, and this is known as the chain rule (ketenregeling).
...
The People Operation Specialist are in charge of [contract renewals]({{< ref "contracts-probation-periods#contract-renewals" >}}). The process the **end of the first 12-month Example Company BV Netherlands temporary contract** is listed in their [Netherlands Renewal Process]({{< ref "contracts-probation-periods#netherlands-renewal-process" >}}) section of the [Contracts, Probation Periods & PIAA]({{< ref "contracts-probation-periods" >}}) handbook page.

*Example Company IT BV contracts should only be used for contractors. All Netherlands **employees** should be issued the Example Company BV contract.*

## Process for Example Company Team Members in Australia, New Zealand, or Ireland

Example Company has an entity in this location. All team members in this location are employees. Please note that Northern Ireland is under the United Kingdom, so if you see an address that includes UK you should hold and verify on google maps where the candidate is located and whether a different contract and background check are needed.

Always ensure the start date is on a Monday, unless Monday is a holiday or the non-Monday date is legally required.

## Process for Example Company Team Members in Singapore, South Korea, or France

Example Company has entities in Singapore, South Korea, and France. All team members in this location are employees, but payroll will be processed through Global Upside. Please be sure to select the correct docusign email template so they will be CC'ed as the final step of the offer process.
...
1. Ensure the start date is on a Monday, unless Monday is a holiday or the non-Monday date is legally required.
1. Send a Slack message to the Legal Team in [ces_legal_contracts](https://example_company.slack.com/archives/C05R0R495L6) requesting assistance with the offer. When making the request in the channel, please indicate the candidate's name and include the link to the GH profile
...
Example Company is working in partnership with [CXC Global](http://cxcglobal.com/) to employ Example Company team-members located in **Poland**, **Ukraine**, **Romania**, **Russia**, **Serbia**, & **Slovenia**. The actual employment contracts will be sent and issued by CXC and are in accordance with local labor law. CXC also handles the processing and payment of payroll and associated taxes and compliance in each of the countries on behalf of Example Company. The contracts themselves are between the individual and CXC.

CXC provides a 12 month contract in these locations, and this can be extended. They are only able to support contractors that have an established entity/company in these countries (listed above). The offer details will be provided to CXC by Example Company's hiring team.
...
1. The Candidate Experience Specialist should add the candidate to the [PEO Tracking Sheet](https://docs.google.com/spreadsheets/d/1gSOVTJ1Yv-YsSaliJwKVf-1H0vBZXxb5NWFiqCwT-uo/edit#gid=0) and keep track of communication between Example Company.
...
1. Kindly allow a duration of one week for CXC to complete their process. This might mean that a two week notice period to start at Example Company, could increase to three weeks, its important to communicate this duration to new hires in this location.
...
## Preparing Employment Agreements for Example Company team members employed via Global Expansion (GX)

Example Company is working in partnership with Global Expansion to employ Example Company team-members located in:
...
1. GX will then review the details and compose the employee’s contract. They will then email ces@example_company.com when it is complete and ask us to review. You will need to log into the GX platform FIRST, and then click the link in the email sent by GX
...
1. GX will also be sending a 1 page docusign to ces@example_company.com where we need to confirm employment details again. Please review and sign the document if accurate. Docusign will auto populate a signature of “GL CES Team” but you can adjust and add your name in front of that title. Add your title and send
...
## Preparing Employment Agreements for Example Company team members employed via Global Upside

Example Company is working in partnership with [Global Upside](https://globalupside.com) for employing Example Company team-members located in:
...
1. Once in Egnyte, navigate to *"/Shared/GPS/Active Clients/Example Company/IN/HR/Employee Master/Client Upload/New Employee Information"*
...
Example Company has partnered with [Remote.com](https://remote.com/) to hire in Brazil, Denmark, Switzerland, Italy, and Hungary. The actual employment contracts will be sent and issued by Remote and are in accordance with local labor law.  The offer details will be provided to Remote by Example Company's hiring team.
...
1. Once the additional information is received, The Candidate Experience Specialist will generate the PEO form out of GreenHouse.
    ...
    - Select "PEO New Hire Template" from the dropdown
    - Click "Generate"
1. The Candidate Experience Specialist will choose template *Offer through DocuSign -Remote.com* and will copy example_company@remote.com on the email.
...
1. Log into the [Papaya](https://example_company.papayaglobal.com/backoffice/org/1826/dashboard) platform
...
1. Amit Bengal - AmitBe@papayaglobal.com (Customer Success Manager at Papaya who looks after Example Company) will reach out to the CES to confirm that they have received the alert. Amit should ask for permission to contact the candidate directly - please confirm this ASAP. Papaya will validate the information submitted into the platform and reach out to CES or the candidate should there be any immediate questions regarding the contract information shared
...
## Advance Funds process for locations Example Company can not order laptops
...
1. This email uses a different Google form than the regular Welcome Email. The new hire will be presented with two options and be asked to select their preference:
    - Purchase and expense for reimbursement through NAVAN on their first day at Example Company
    - Request advanced funds to self-procure
1. IT will receive an issue in their [IT Equipment Order Processing](https://example_company.com/example_company-com/it/end-user-services/issues/it-equipment-order-processing/-/issues/?sort=closed_at_desc&state=opened&first_page_size=100) desk with the new hire's selection. If the candidate has requested advanced funds, IT will then tag AP within the issue and provide approval for the laptop purchase. AP will gather the new hire's banking details and release the funds.

**Example Company and New Hire:**

1. Once the verbal offer has been made by the recruiter [hiring process](/handbook/hiring) complete the [Template-Example Company China Employee Offer letter](https://docs.google.com/document/d/1c69dG9TuAB0MgiKj_gLDuHTHpzjocvEAm2okUOKqRIs/edit#) as per How to use this page to prepare a contract.
...
1. Once this has been done send the Example Company (Chinese & English) versions of the Letter of Employment Intent to the new hire for their review, completion and signature using DocuSign. Ensure that Peopleops and CIIC are copied.
...
**Example Company & CIIC:**

1. Example Company has a Secondment Agreement in place with CIIC, this may need to be updated but CIIC will confirm.
1. Once CIIC have received the documents they will prepare a payment notice and send this to Example Company (peopleops) for payment. This must be paid upfront and may need CFO approval.
1. After CIIC receive payment they will reach out to the new hire to complete a Labor Contract.

**CIIC & New Hire**

Once the Labor Contract has been signed by both CIIC and the new hire the individual can now commence their work with Example Company.

## Employment Agreements for Example Company team members in Germany

According to the revised German Contract process, contracts require wet signatures, which means that besides contracts being e-signed in Docusign, Example Company also requires physical copies of the contract to be signed by both parties.
...
- Open the saved unsigned contract in Google Drive, remove Example Company signatory from the below fields -
    1. **Summary Key of Terms** - Delete the Example Company Signatory's name and leave the field blank.
    1. **Scroll down to the end of the contract** - replace Signatory's name with "Example Company Signatory". Save the contract as a PDF format.
...
## Employment Agreements for Example Company team members in Japan

Example Company has an entity in Japan (Example Company GK) and use the Example Company GK contracts in this location. All team members in this location are employees.

Always ensure the start date is on a Monday, unless Monday is a holiday or the non-Monday date is legally required.

## Intern Employment Contracts

Example Company participates in hiring interns within the INC entity as well as the Code2College program. Code2College is the FIRST and ONLY program that places high school students into paid, technical internships.

The Intern contract template will be used for both C2C interns and all other interns hired into Example Company. However there are some different necessary steps when processing a C2C intern. Processes are described below.
...
1. Once the offers are signed by the candidate, CES will send the Example Company Inc Intern Welcome Email. **Note: Example Company will provide laptops to these interns. The Welcome Email asks them to fill out form ASAP**
...
## Employment Agreements for Example Company team members located everywhere else (IT BV contractor agreements)
...
1. Update the "To" field to include the Example Company signatory and include the hiring manager in CC field
