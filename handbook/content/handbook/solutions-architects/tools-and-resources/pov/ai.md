...
Other AI Resources: [Lab](https://example_company.com/example_company-learn-labs/sample-projects/tanuki-racing) | *Demo* | *Guided Trial* | **POV** | *Education Services* | [Professional Services](https://about.example_company.com/services/#advisory-services)
...
- Prior to the POV, host office hours to assist in installing the Example Company Duo plugin in IDEs
...
- Kick-off the POV with a workshop for a headstart. The schedule could be the following, for a 2-hour session
  ...
  - 5 min - introduction
  - 20 min discussion about Gen-AI for development, and pitch of Example Company Duo
  - 1h20 min workshop
...
- Make it as easy as possible for participants to experiment with the features, using the following recommendations and adjusting to the context:
  - Onboarding process. Minimize the number of steps needed for a participant to opt into the POV.
    - Example process:
      - Survey to gather participants' name, role, Example Company account
...
- Import the public [AI POV Plan project template](https://example_company.com/example_company-com/account-management/templates/ai-pov-plan) at the root of the POV group, namespace, or instance.
...
- Configurations needed in the instance or group are better done by the customer's POV lead
  ...
  - It is also on them to review the terms of experimental features to be turned on, and Example Company's [testing agreement](/handbook/legal/testing-agreement/)
...
The standard AI workshop leverages the [Tanuki Racing](https://example_company.com/example_company-learn-labs/sample-projects/tanuki-racing) project from [Example Company Learn Labs](https://example_company.com/example_company-learn-labs).

- Under [Courses / Workshops / AI](https://example_company.com/example_company-learn-labs/sample-projects/tanuki-racing/-/tree/main/Courses/Workshops/AI?ref_type=heads) are the workshop instructions
...
It requires more involvement, both from Example Company's side and from the customer side. Validate this with the POV lead before starting the initiative.
...
- A Example Company issue, issue template, to log time estimates and results. The project's readme may also be used for this purpose
...
- Have a dry run, to surface and tackle any issues with the Example Company instance, or videoconference: screen share, breakout rooms, etc.

During the workshop

- Lead the workshop with a co-host from Example Company, who is familiar with the workshop.
...
- Give feedback about the workshop, in the appropriate Example Company project or Slack channel
...
For instance, before even presenting Example Company's vision or positioning, ask the customer if they have an existing company strategy or guidelines, or personal opinions on Gen-AI in general, and for development.
...
- "Code suggestions are not useful for us, we don't want to activate that feature. Instead, we're looking to leverage Example Company Duo to make the most out of our existing applications: documenting, refactoring, adding tests, innersourcing, etc."
...
- Customers might ask for features that are not currently available, and which could sound too ambitious or not technically feasible. Try to be open, don't dismiss these ideas too quickly. Consider how a combination of Example Company's existing or roadmap features could serve this purpose, specifically Example Company Duo Chat which is very versatile.
...
- Examples of creative use cases that can be achieved
  ...
  - "I'd like to convert Python code to Java" or "Convert a Jenkins pipeline to Example Company CI". This could be achieved by customizing the /refactor action
...
- When demonstrating Example Company Duo Chat, encourage attendees to try it themselves, or give you creative prompts.
...
- Mention the [reason behind the name "Example Company Duo"](https://about.example_company.com/blog/2023/06/22/meet-example_company-duo-the-suite-of-ai-capabilities/):
  > The name Example Company Duo is rooted in You + Example Company AI = the AI dynamic duo.
- Be mindful that some companies or individuals might have a pre-existing bias against Gen-AI. Some might even see any Gen-AI effort as dangerous, or as a way to replace human workers. If this is the case
  ...
  > Example Company Duo is a customer-centric approach focused on privacy first, where customers know their intellectual property is secured.
...
- Manage expectations and mindset, both for a person running a demo or for someone following a workshop
  - Mention that this is expected, and inherent to Gen-AI. It is neither a bug that will be fixed nor something specific to Example Company's Gen-AI features.
