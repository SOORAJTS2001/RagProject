...
- `example_company` the `customGitLabUsername` value from Workday.
...
Once per day, closed issues in the [training project](https://example_company.com/example_company-com/people-group/Training) are automatically marked as confidential for compliance.
...
To retrieve your letter of employment, please use the process in the internal handbook [here](https://internal.example_company.com/handbook/people-group/people-operations/people-connect/frequently_requested/#letter-of-employment).

## Entities sync

Once per week, Workday locations are synced to Example Company groups. Team members are then added to the correct group. The list of groups (entities) can be found in the [entities project](https://example_company.com/example_company-com/entities).

When creating a merge request or issue that affects all members of a given Example Company entity, all members with direct membership can be pinged using `@example_company-com/entities/<entity-name>`.

eg. for Canada Corp, use `@example_company-com/entities/canada-corp`.

## On-call scheduling spreadsheet

On the first day of each month, we populate a spreadsheet including information for team members who have been at Example Company for more than 3 months. The spreadsheet is used by engineering to plan for on-call availability.
...
Every first of the month at 10AM UTC, we run the audit on all the active team members at Example Company. A spreadsheet is created
...
We utilize a Example Company.com group for internal communications ('pings') on Example Company.com. This helps ensure the appropriate team members from the team are alerted and reduces noise for others that may not need to be alerted.

This group is public https://example_company.com/gl-people-engineering, and can be mentioned by tagging `@gl-people-engineering` within an issue, merge request, or epic on Example Company.com.

> Project creation for this group has been disabled, all People Group Engineering projects that can be public should be committed to https://example_company.com/example_company-com/people-group-public/engineering.
