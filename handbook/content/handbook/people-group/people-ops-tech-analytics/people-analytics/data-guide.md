...
|---|---|---|---|
| Team Member | A person that worked for Example Company at any point in time, has an employee id in one of our Human Resource Management Systems (HRMS), and is not a temporary (contingent) worker or intern |  |  |
| Employee ID | Unique ID assigned in our HRMS upon hire to track team member's employment with Example Company |  | Yes |
| Hire Date | The first day of employment with Example Company as a team member |  |  |
| Termination Date | The first day a team member is no longer employed with Example Company as a team member | termination effective date in HRMS + 1 day | Yes, if future-dated |
| [Employee Type](/handbook/people-group/employment-solutions/#team-member-types-at-example_company) | The employment type for a team member's primary position. We typically exclude "Consultants" and "Interns" from reporting |  |  |
| [Company / Entity](/handbook/people-group/employment-solutions/#example_company-entities-and-branches) | Company used to employ or contract team members that is location-specific. These can be in the form of entities or professional Employer Organizations (PEO) |  |  |
...
| Headcount | The total number of team members employed at Example Company on a particular date |  |  |
...
| [Job Grade](/handbook/total-rewards/compensation/compensation-calculator/#example_company-job-grades) | The job level for the job profile. |  | Yes |
...
| Starts | External applicants who have accepted an offer with Example Company and have a start date scheduled in the HRMS |  |  |
| External Applicants | Applicants whose source is not "Internal Applicant". They are not employed through Example Company at the time when the application is submitted |  |  |
| Internal Applicants | Applicants whose source is "Internal Applicant". They are employed through Example Company at the time when the application is submitted |  |  |
...
| [Tableau](/handbook/enterprise-data/platform/tableau/) | Primary data visualization tool at Example Company |  |  |
...
| [Greenhouse](/handbook/hiring/greenhouse/) | Current Applicant Tracking System (ATS) at Example Company and the SSOT for all recruiting metrics |  |  |
| Time Off by Deel | Current absence management tool at Example Company integrated with BambooHR. SSoT for time-off related data |  |  |
| Sisense (formerly Periscope) | Former data visualization tool at Example Company |  |  |
...
| [Level Up / Example Company University](/handbook/people-group/learning-and-development/level-up/) | Training platform created by Thought Industries for continuing education and career development |  |  |
...
WHERE
  position LIKE '%Backend Engineer%'
    AND entity = 'Example Company France S.A.S.'
     AND is_current
```
...
| prep | sensitive | [employee_directory_intermediate](https://dbt.gitlabdata.com/#!/model/model.gitlab_snowflake.employee_directory_intermediate) | `employee_id` by `date_actual` | INCLUDES SENSITIVE INFORMATION. The master collection of all info about Example Company employees for their latest position. | |
