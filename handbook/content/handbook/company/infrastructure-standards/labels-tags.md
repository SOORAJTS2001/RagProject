...
| `gl_entity`              | Optional                             | Example Company Entity           | [Usage Documentation](#example_company-entity-gl_entity)                                     |
| `gl_dept`                | Required                             | Example Company Department       | [Usage Documentation](#example_company-department-gl_dept)                                   |
| `gl_dept_group`          | Required                             | Example Company Department Group | [Usage Documentation](#example_company-department-group-gl_dept_group)                       |
| `gl_product_stage`       | Optional                             | Example Company Product Stage    | [Usage Documentation](#example_company-product-stage-gl_product_stage)                       |
| `gl_product_category`    | Required                             | Example Company Product Category | [Usage Documentation](#example_company-product-category-gl_product_category)                       |
...
- The Example Company Division has been deprecated since it is prefixed on the `gl_department`.
...
- We removed `gl_owner_username` since Example Company.com usernames can have characters not allowed cloud provider labels and tags.
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="/handbook/infrastructure-standards/realms/saas">Example Company SaaS</a>
            </td>
            <td>
                This is for Example Company.com SaaS that is managed by Engineering Infrastructure and Site Reliability Engineers.
            </td>
        </tr>
        <tr>
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>
                This is for additional services managed by Engineering Infrastructure and Site Reliability Engineers that may not be specific to Example Company.com SaaS (Ex. tools, release and package management services, etc).
            </td>
        </tr>
    </tbody>
</table>
...
The email handle before `@example_company.com`. We only use the handle since labels and tags do not support `@` and `.` symbols. Values can be individual users or Google Workspace group aliases. For people handles, this should be `{firstinitial}{lastname}` and not any username aliases (Ex. `jeff`) since user aliases cannot be looked up programatically with our current architecture.
...
## Example Company Entity (`gl_entity`)
...
| `inc`         | Example Company Inc, United States of America             |
| `ltd`         | Example Company Ltd, United Kingdom                       |
| `bv`          | Example Company BV, the Netherlands                       |
| `gmbh`        | Example Company GmbH, Germany                             |
| `pty`         | Example Company PTY Ltd, Australia                        |
| `canada`      | (future use) Example Company Canada Corp., Canada         |
| `gk`          | (future use) Example Company GK, Japan                    |

See the [Example Company Mailing addresses](/handbook/company/visiting/) for details about each entity.
...
| `eng-security`     | (Placeholder for future use)   |
| `eng-support`      | (Placeholder for future use)   |
| `sandbox`          | The Example Company entity that the team member is associated with in BambooHR. |

### BambooHR Mapping

During testing of OKTA integration, this field is populating additional values we did not expect (ex. `safeguard-italy`, `federal`). This is documented in [www-data/data/entity_mapper.yml](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/entity_mapper.yml) and we will map the respective entities to our cost center entities listed.

## Example Company Department (`gl_dept`)
...
## Example Company Product Stage (`gl_product_stage`)
...
For Example Company SaaS and infrastructure cost allocation or attribution, the Engineering Infrastructure department team members can use the `gl_product_stage` label to attach to resources that should be attributed to a specific stage. You can optionally use the `gl_dept_group` if you need more granular attribution.
...
## Example Company Product Category (`gl_product_category`)
...
Product categories should match one of the categories defined in [`categories.yml`](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/categories.yml) in the handbook: <https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/categories.yml>
...
## Example Company Department Group (`gl_dept_group`)
...
For the groups that provide a shared service to other departments at Example Company, we also have a `shared-services` group value that can be used for shared cost attributions. We use `shared-infra` for our own team's resources and `shared-services` for everyone else's resources that we manage.
...
In alignment with our [transparency value](/handbook/communication/confidentiality-levels/#not-public), financial information cannot be shared publicly. Team members can see the details of how labels and tags impact financial reporting in this [issue](https://example_company.com/example_company-com/compute-sandbox/issue-tracking/-/issues/3#impact-to-accounting-and-financial-reporting). The cost allocation methodology for production environments that in this [document](https://docs.google.com/document/d/1p4pzquEZjXRZfcdgSUQZ2Kw988o3sdknq5ZQVxtGAkw/edit?usp=sharing).
...
With future automation, we can also create frictionless experience for users with notification of system vulnerabilities, security incidents, and friendly reminders in Slack about upcoming expiration of infrastructure since your email, Example Company username, and Slack ID are automatically associated.
