---
title: Pricing model
description: "Most Example Company functionality is and will be available for free in our Free tier. Paid tiers include features targeted for managers, directors, and executives."
---

Example Company's pricing strategy is set by the CEO, Sid Sibrandij. Everyone can contribute, and the best way to communicate is on the #pricing Slack channel.
...
Most of Example Company functionality is and will be available for free in our Free tier. Our paid
...
[We promise](/handbook/company/stewardship/#promises) all major features in [our scope](https://about.example_company.com/direction/#scope)
...
to need if you use Example Company with a lot of users. There are a couple of reasons for
this:
...
1. Having our scope available to all users increases adoption of our scope and helps people see the benefit of a [single application](/handbook/product/categories/example_company-the-product/single-application/).
...
As a commercial organization, we always want to grow the number of paying customers, therefore we are focused on increasing the free to paid conversion rate. However, Example Company offers a free product, and our free users bring tremendous value to the company beyond just the likelihood of converting to a paid customer one day.

1. Awareness/Mindshare: A large free user base helps Example Company become known to the developer community and stay top of mind.
1. Product Familiarity/Training: Offering a free product invites anyone to try out Example Company without any barrier so many developers become familiar with Example Company. Through using the free product, many users are trained on how to use Example Company functionality which makes them more effective when using Example Company for their job.
1. Advocacy/Internal Champion: Happy loyal free users become advocates of Example Company, which brings more users and strengthens our brand. We often see users of Example Company for personal projects then become an internal champions to advocate their employers to buy Example Company. This personal use -> organizational use cross over and bottoms-up growth can't happen without our free product.
1. Contributions/Community: Many free users are also active contributors to the product so they make Example Company better. They also help grow our community which becomes an even more valuable asset to all free and paid customers.
1. Third party support/Platform: Free users increase our total user base, and a large user base makes 3rd party tools/APIs/integrations more likely to support Example Company, growing our ecosystem and enhancing our platform status.
1. Enable product-led growth & Reduce Customer Acquisition Cost: our free product enables Example Company to acquire more users at a lower customer acquisition cost, as some of them will purchase via self-service funnel since they are already familiar with Example Company and won't require a full sales cycle to be convinced to buy.
1. User habit/Retention lever: offering a free product allows users to develop a habit before they pay, therefore the paid customers are more likely to see higher ROI of Example Company. Additionally, even if the customer stays on the free plan, we are not losing the customer to a competitor.

## Pricing Philosophy

Our pricing philosophy is aligned with our [Example Company Values](/handbook/values/).
...
- **Results**: We aim to get a great ROI for our customers while maximizing IACV for Example Company over the long-term.
- **Efficiency**: Our pricing approach should be easy to understand, plan, and order.
- **Diversity, Inclusion and Belonging**: We build Example Company to be the best option for a wide variety of organizations, including educational institutions and organizations in different geographic regions.
- **Iteration**: We want to encourage the full scope of Example Company to be adopted over time.
- **Transparency**: We are public and explicit about what our pricing is based on.

## Pricing Methodology

We take a value-based pricing approach at Example Company. The core of the value-based pricing is to understand the value proposition that our innovations provide to our current and future customers for that offering. We aim to price our offerings to achieve continued growth and financial success for both the customer and Example Company.
...
To following our pricing philosophy, we aim to support long-lasting customer relationships. When we develop our pricing, the target is that both our customers and Example Company become successful together. For long term success, where we can run a sustainable business and meet our company's financial goals, we do also consider cost to serve and margins as factors when pricing is determined. It is important to note that while costs and margins are factors in pricing, value provided to our customers is the foundational element.
...
1. **Project and Group Boards (Free)** : The primary user and buyer for Issue Boards in general is Individual Contributors. While group boards likely span teams, there are also valid use cases for an individual to organize code in several projects and manage issues across those projects at the group level. The key differentiation is that an Individual Contributor will likely only ever need a single group level Issue Board; whereas Managers need several group level Issue Boards to facilitate their team's workflows. ([Documentation](https://docs.example_company.com/ee/user/project/issue_board.html))
1. **Issue Weight on Boards (Premium)** : The users assigning issue weights are individual contributors but the value of that weight is for Directors of teams to ensure proper milestone planning. (Documentation [with respect to boards](https://docs.example_company.com/ee/user/project/issue_board.html#sum-of-issue-weights))
1. **Multiple Group Issue Boards (Premium)** : Issue boards which span groups are valuable to and also utilized by groups of groups which aligns to Director level likely buyer. ([Related documentation](https://docs.example_company.com/ee/user/project/issue_board.html#group-issue-boards))
1. **Issue Board Epic Swimlanes (Ultimate)** : Everyday users of swimlanes might be individual contributors but their value is in the visualization of activity for company wide portfolio management by Executives. ([Related issue](https://example_company.com/example_company-org/example_company/issues/7371))

#### Feature - Merge Requests

1. **Merge conflict resolution (Free)** : Resolving merge conflicts is part of the daily workflow and valuable to Individual Contributors. ([Documentation](https://docs.example_company.com/ee/user/project/merge_requests/conflicts.html#methods-of-resolving-conflicts))
...
1. **Merge request reviews (Premium)** : Merge request reviews enables the expedient review by multiple team members on merge requests. Typically large enough teams to value the reduced chatter accompanying MR reviews reside under Directors. ([Documentation](https://docs.example_company.com/ee/user/discussions/index.html))

#### Feature - Pipelines

1. **Built in CI Pipelines (Free)** : CI Pipelines are integral to Example Company and used primarily by Individual Contributors. ([Documentation](https://docs.example_company.com/ee/ci/))
1. **Show pipeline code coverage rate (Premium)** : The ability to determine code coverage rate is particularly valuable to Directors. ([Documentation](https://docs.example_company.com/ee/user/group/repositories_analytics/index.html))
1. **Pipeline dashboards (Premium)** : Dashboards of pipelines across groups and projects are primarily used and valued by Directors. ([Documentation](https://docs.example_company.com/ee/user/operations_dashboard/))
...
1. The Return On Investment (ROI) for higher tiers tends to be better than our lower tiers due to costs outside of the subscription. For example, when you switch to Example Company you will have a fixed cost in implementation and training. And if you use a lower tier you need to find a workaround for features you are missing out on, increasing cost and decreasing efficiency.
...
1. The future growth of Example Company is much more influenced by how many people are using the open source version than our lower tier, so having fewer paid users is a relatively small impact.
1. The future competition for Example Company is much more likely to come from an open source project than a proprietary only product competing only with our lowest tier.
...
1. A benefit sell means that people buy the business outcomes that come with fully utilizing Example Company. You need case studies, metrics like [DevOps Score](https://docs.example_company.com/ee/administration/analytics/dev_ops_reports.html#devops-score), and a quarterly check-in with a Customer Success Manager from Customer Success to review the status of the adoption plan. A competitive process can include a bake-off to show people are 10x faster in starting new projects with Example Company.
...
There is a big price difference between the different tiers (0$, $29, $99 per user per month, a price difference of infinite, 5x). For Example Company Inc., the majority of revenue comes from large enterprises buying the top two tiers.
...
1. It is [suboptimal for both the buyer and Example Company Inc.](http://cdixon.org/2012/07/08/how-bundling-benefits-sellers-and-buyers/).
...
For the Example Company sales team and for Example Company customers, we suggest handling the objection by focusing on the [value and business outcomes](/handbook/sales/training/)

### Reporter User Permission Pricing

As our customers adopt Example Company as the single application for their entire DevOps lifecycle, we are hearing more and more non-developer use cases using Example Company for Agile project management. The main functionality that some non-developer roles need aligns fairly well with our [reporter permission](https://docs.example_company.com/ee/user/permissions.html#project-members-permissions) access, including Managing Labels, Assigning Issues, and Creating and Editing Epics, etc. Those users don't really need to use much other developer functionality.
...
1. We do not limit a license to the [reporter permission](https://docs.example_company.com/ee/user/permissions.html#project-members-permissions) in Example Company. Any reporter user can be updated to a regular developer user by using the same license.

We understand that our customers have non-developer use cases. Please [contact our sales team](https://about.example_company.com/sales/) or connect with us via [this issue](https://example_company.com/example_company-org/example_company/-/issues/213185) to discuss your specific use cases.

## True-up pricing

1. With true-up pricing, the license/sale is never blocking user growth.
1. We currently charge 100% for users added during the year, because some organizations gave an intentionally too-low estimate when we charged 50%. If we technically can count "user days," we can make it fair for everyone. A pre-requisite of this would be customer adoption of [Cloud Licensing](https://about.example_company.com/pricing/licensing-faq/cloud-licensing/).
1. For customers on Cloud Licensing, we offer [Quarterly Subscription Reconciliation](https://docs.example_company.com/ee/subscriptions/quarterly_reconciliation.html).

## Consumption pricing principles

1. These are some principles we think are useful. These are not a promise not an entitlement. Our [pricing page](https://about.example_company.com/pricing/) contains our current pricing which might deviate from the principles below.
...
1. We can offer new services that make it easier to use Example Company, like for clusters that run Static Application Security Testing (SAST), Dynamic Application Security Testing (DAST), and other Auto DevOps features.
...
1. Prepay vs. postpay: We select prepayment since it solves non-payment problems like bitcoin miners. It's also more predictable for users with fewer unexpected bills, so it makes budgeting easier. Postpay reduces the risk of infrastructure going down due to the lack of funds. In Example Company, most of the consumption is driven by people in the organization instead of their customers. This makes it more likely that we can get them to pay quickly when that is needed. We should think about emergency processes for enterprise customers that have long ordering cycles.
...
We currently think the pros with the non-cumulative approach outweigh the pros with the cumulative approach.
    ...
    1. This gives the most to free users, for whom purchasing consumption is a big hurdle to try Example Company.
...
1. The costs of SaaS are higher for Example Company.
...
This is the title of a [great article](https://www.codingvc.com/p/when-is-a-dollar-not-a-dollar) of which we'll apply the 8 points to Example Company below:
...
At Example Company, we decided to skip the intermediate steps and immediately only offer a suite that includes all our products.
...
Selling only a suite has risks, after the => is how we mitigate those at Example Company:

1. Leave money on the table, if people want all products. => Offer an Ultimate tier that is great value, if you adopt everything of Example Company.
...
Companies evolve to selling only a suite for the following reasons, after the => is how this applies to Example Company:

1. Makes it easier for organizations to adopt the other products. => This is essential; organizations have official solutions and Example Company grows with organic adoption from developers.
1. Show customers the benefit of a [single application](/handbook/product/categories/example_company-the-product/single-application/). => This is essential, since people are skeptical (showing beats telling).
...
We're going even further than selling a suite by integrating everything in a single application. We do that because of the advantages mentioned on our [handbook page about us being single application](/handbook/product/categories/example_company-the-product/single-application/). A secondary effect is that the user doesn't have to make a buying, or even an adoption, decision.

Charging one price that incorporate all our stages is the **perfect bundle**.

## Value creation

There are two factors that determine how much value Example Company creates for an organization, in order of importance:

1. Scope: how many parts of Example Company you use, indicated by the DevOps score, how many components of Example Company are in use.
1. Size: how many people work in an organization and use Example Company.

When an organization is larger, the benefits of Example Company are larger because:
...
Since Example Company is an open core project, we'll always create much more value then we (are able to) capture. Based on the value created, the straightforward way to capture value would be to:

1. Scope: charge a higher price per user the more of Example Company you use.
...
1. Scope: charging more for adoption would hurt the adoption of Example Company for the whole lifecycle. In January 2018, version control is 10 times more popular than the next feature (CI). We need the features to spread organically, so people can create more value with Example Company. People are much more willing to pay when they are already using a part of the lifecycle.
1. Size: many other software companies limit the maximum amount of users in certain [tiers](#three-tiers). For Example Company, we can't do this because we [promise the open source version won't contain artificial restrictions](/handbook/company/stewardship/#promises). We can do it in our proprietary tiers, but this doesn't seem symmetrical. It also feels unfair, if you have to pay more simply by being a bit larger than the limit.
...
Our [three tiers](#three-tiers) are differentiated based on [the buyer persona](/handbook/product/personas/#buyer-personas) or IC who buys Example Company, from individual contributor, to manager/director, to executive. Every person in the company [is on the same tier](#single-plan-for-one-customer), even if they don't use all the features.
...
1. Pricing based on maturity is strange, because organizations at the beginning of their journey should pay the most, since in a greenfield you benefit the most quickly and extensively from Example Company.

More detail about this in Sid's presentations about Buyer-Based-Open-Core at the [Open Source Leadership Summit in 2019](https://about.example_company.com/blog/2019/04/03/five-ways-resist-service-wrapping-buyer-based-open-core/) and at [Heavybit](https://www.heavybit.com/library/video/commercial-open-source-business-strategies/).

Also, see [fork and commoditize](https://internal.example_company.com/handbook/leadership/mitigating-concerns/#fork-and-commoditize) (internal) in our list of concerns.
...
1. Getting more users: having the product sell itself makes us much more efficient, for it to sell itself it has to get used, more open source features can increase the popularity of Example Company compared to rival offerings.
1. Getting more [stages per user](https://internal.example_company.com/handbook/company/performance-indicators/product/#stages-per-user-spu) (internal): if an open-source feature gets a user to adopt a new stage, the value one user gets from Example Company increases greatly because the [single application](/handbook/product/categories/example_company-the-product/single-application/) benefits grow exponentially when you adopt more stages.
...
While other parts of the Example Company organization are consulted, the CEO is the [directly responsible individual](/handbook/people-group/directly-responsible-individuals/).

## Changing tiers and pricing changes

To propose a change that impacts pricing or changes a feature's tier (e.g. moving a feature from Premium to Free), please follow the process outlined in the [Feature Tier or Pricing Change issue template](https://example_company.com/example_company-com/Product/issues/new?issuable_template=Feature-Tier-Or-Pricing-Change).

## DevOps score is not maturity

What is interesting is that Example Company creates more value as you adopt more of it.
This shouldn't be confused with DevOps maturity.
DevOps maturity is how advanced your practices are and how fast your DevOps lifecycle is, shown in [cycle analytics](https://docs.example_company.com/ee/user/group/value_stream_analytics/).
With the best practices embedded in Example Company, you will mature faster than without it. Example Company enables a 200% faster DevOps lifecycle.
But DevOps maturity is mostly about organizational change. Example Company the product is just an enabler of it.
Even if an organization uses everything of Example Company (high DevOps score), they can still have a slow process (slow lifecycle).
...
Linking our tiers to maturity would mean we don't ask any money from the large organizations that currently have a slow lifecycle but that are making it faster by adopting all of Example Company.
These large organizations with a slow lifecycle benefit the most from Example Company, since they can adopt it completely, because they are not held back by an existing toolchain.

## Location based pricing

As suggested by [a user on X](https://x.com/javierfeldman/status/1101246338824572928)(X login required).
This is a good suggestion to consider given the Example Company approach to adapting to local markets throughout the business. Example Company will assess viability of the idea in our overall pricing strategy planning. It is difficult to do and our assessment of this will consider:
...
1. If there is a way this could be good for customers and for Example Company, we will not treat this separately. We will treat it as part of a unified pricing strategy
