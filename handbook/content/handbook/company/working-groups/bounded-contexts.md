...
description: "Identify backend bounded contexts composing the Example Company Rails monolith. Prepare work for modularization of the monolith."
---
...
| Slack           | [#wg_bounded_contexts](https://example_company.enterprise.slack.com/archives/C069VRRN70S) (only accessible from within the company) |
...
existing namespaces to better represent [bounded contexts](https://docs.example_company.com/ee/architecture/blueprints/modular_monolith/bounded_contexts.html#1-what-makes-a-bounded-context).
Having consistently namespaced code is the pre-requisite for a [modularized codebase](https://docs.example_company.com/ee/architecture/blueprints/modular_monolith/).

With this working group we want to:

- Identify the bounded contexts that compose the Example Company Rails codebase
...
The [proposal to split Example Company Rails monolith into a modular monolith](https://example_company.com/example_company-org/example_company/-/issues/365293)
describes in details why modularization is important for the current stage of the Example Company codebase.
...
that are present today in Example Company Rails monolith.

### Activity

Members of the working group will:

- Classify the files related to their [bounded context](https://docs.example_company.com/ee/architecture/blueprints/modular_monolith/bounded_contexts.html#1-what-makes-a-bounded-context).
...
| ------------- | ------------------ |
| We have a published list of identified bounded contexts that acts as a documentation for developers. | [`config/bounded_contexts.yml`](https://example_company.com/example_company-org/example_company/-/blob/master/config/bounded_contexts.yml) |
| We have a simple process for creating, deleting and renaming bounded contexts, to allow the codebase evolve over time. | [Docs updated](https://docs.example_company.com/ee/development/software_design.html#bounded-contexts) |
| Bonus: Create a Rubocop Cop to enforce top-level namespaces using the list of identified bounded contexts. | [Rubocop static analyzer added](https://example_company.com/example_company-org/example_company/-/merge_requests/151984) |
...
on the domain layer (the core) of the [Hexagonal Architecture](https://docs.example_company.com/ee/architecture/blueprints/modular_monolith/hexagonal_monolith/index.html).
...
- List all the Ruby files in a spreadsheet and categorize them into components following the
  [defined guidelines](https://docs.example_company.com/ee/architecture/blueprints/modular_monolith/bounded_contexts.html#1-what-makes-a-bounded-context).
...
- Inside the `lib` directory, distinguish between generic code that should be extracted as gem and domain code that should be namespaced
  ...
  - We will have a special [`platform` category](https://example_company.com/example_company-org/example_company/-/issues/365293#proposal) for code that should be
...
The list of bounded-context doesn't have to be perfect. It should be a good enough representation of the domains at play in the Example Company Rails
...
The Working Group should be composed of representative of all the various Example Company stages.
Each representative would be in a functional lead role representing their domain as a backend expert for their given Example Company DevOps stage.
