...
description: "On this page, we explain the different factors that make up our Compensation Calculator and its alignment to Example Company's values and Compensation Principles."
---

## Introduction

On this page, we explain the different factors that make up our Compensation Calculator and its alignment to Example Company's values and Compensation Principles.
...
Team members can access [the calculator](https://comp-calculator.example_company.net/users/sign_in)
at any time with their Example Company credentials. Applicants will be issued credentials and
have access to the calculator during the interview process. The information from the calculator should only be shared with those who have access (Example Company team members and candidates).

You can use the calculator to determine the overall Total Rewards Package Offered by Example Company (Cash + Equity + Benefits). Internal team members can also use the [following spreadsheet](https://docs.google.com/spreadsheets/d/1SJnsj-IOTDrAIhTki-oKaJPydiF6vlS1nMKcE88xax8/edit?usp=sharing) by making a copy and adding in your specific information to calculate the Example Company Total Rewards annual amount.
...
1. **Transparency** - The calculator is open to Example Company team members and candidates in the pipeline. This means that at the time of hiring, candidates don't have to blindly negotiate compensation. They simply need to enter the individual inputs on the Compensation Calculator and it comes up with a band consistent with the individual's circumstance - where they live, level, currency, etc. When team members move across levels, departments and locations during their time at Example Company, they'll also be able to see the appropriate band for the change.
...
The compensation calculator is a tool to assist the Total Rewards team in determining a compensation package for new and existing team members. The results of the calculator are not binding. Written correspondence through a [contract](https://internal.example_company.com/handbook/people-group/people-operations/people-connect/employment_contracts/) or [job change letter](/handbook/people-group/contracts-probation-periods/#job-change-letter) specify all official compensation changes. We reserve the right to change the calculator at any point in time.

### Reporting a Discrepancy

As with all things at Example Company, the compensation calculator is a constant work in progress. There are a few options for reporting a discrepancy if you find the calculator isn't outputting data that is true to market.

**If you are an internal Example Company team member or external to Example Company:**

- Create an issue utilizing the issue templates.
  - [Requesting a review of a new location factor.](https://example_company.com/example_company-com/people-group/total-rewards/-/blob/master/.example_company/issue_templates/new_location_factor.md)
  - [Requesting a review of an existing location factor.](https://example_company.com/example_company-com/people-group/total-rewards/-/blob/master/.example_company/issue_templates/review_existing_location_factor.md)
  - [Requesting a review of a benchmark.](https://example_company.com/example_company-com/people-group/total-rewards/-/blob/master/.example_company/issue_templates/review_benchmark.md)
...
Note: Where there is no variable component offered (ICs and Managers) Example Company runs the benchmark evaluation off of base salary only. Where there is a variable component offered, Example Company runs the benchmark evaluation off of Total Target Case (TTC).
...
- A minimum of 4 **qualified** candidates reject an offer extended by Example Company, or withdraw from the hiring process, due to a reason of compensation for an open requisition
...
- A voluntary departure rate of 12% or more by current Example Company team members in any given department due to the reason of compensation.
...
1. The Total Rewards team will update the [Compensation Calculator](https://comp-calculator.example_company.net/users/sign_in) and all other appropriate documentation
1. If applicable, team members are notified of a pay adjustment and related information.

### Job Codes

To automate the process of pulling survey data from Comptryx and Radford to review benchmarks, Example Company has mapped each job title with a corresponding job code. As a first iteration Example Company will use the job codes Radford has outlined. Each job family and level must have a unique job code. The following structure is used:
...
- If there is no job code, Example Company will generate a four digit unique job code starting with "N".
...
Example Company will gather and analyze the data for each location factor annually as part of annual compensation review. We will also iterate on location factors as needed throughout the year.
...
### Example Company Job Grades

Example Company job grades aid in mapping a role for internal equity with respect to cash and equity. For example, if there is a stock option update, this mapping can act as a reference to update the compensation calculator for the various roles to ensure alignment. Job Grades can also provide an alternative path to finding the current number of options offered without having to fill out the compensation calculator.
...
The Compa Ratio where within the range spread a team member falls in the calculator. Specifically, the Example Company compensation calculator has a 40% spread (+/- 20% from the median). It is common to see [range spreads up to 50%](https://www.erieri.com/blog/post/common-compensation-terms-formulas).
