...
1. Once the new benchmarks are reviewed and approved, these are updated in the [handbook](https://example_company.com/example_company-com/people-group/peopleops-eng/compensation-calculator/-/blob/master/data/job_families.yml).
...
1. Once the new locations factors are reviewed and approved, these are updated in the [handbook](https://example_company.com/example_company-com/people-group/peopleops-eng/compensation-calculator/-/blob/main/data/location_factors.yml).
...
1. Pull in the previously matched Example Company localities from the previous year's location factor review into the ERI tab.
1. On the Analysis tab, review locations without a match to ERI to see if there is now a match on the ERI tab.
    1. For cities, only add the exact match, not everything that would fall into the locality. For example, for San Francisco, California, I would match it to San Francisco in ERI's data set, but not Oakland. If two cities are listed in Example Company's locality, for example Berlin/Leipzig, Germany, match to both cities if available in the data.
...
1. Once differentials have been calculated for each global location, any new locations that do not currently have a match to one of Example Company's localities should be matched.
...
1. Match any Dice locations to the corresponding Example Company locality, if applicable.
...
1. Match the new location(s) to a Example Company locality in this tab as well as any location that haven't previously been matched, if applicable.
...
- Ethnicity (only in the United States as we work on adding globally relevant [ethnicity categories to Workday](https://example_company.com/example_company-com/people-group/dib-diversity-inclusion-and-belonging/diversity-and-inclusion/-/issues/347) for self-identification)
- Tenure
- Gender (as self reported in Workday)
- Geographic Region
- [Grade]({{< ref "compensation-calculator#example_company-job-grades" >}})
