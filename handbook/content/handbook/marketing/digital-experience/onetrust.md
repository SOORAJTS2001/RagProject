...
For any OneTrust related request, please create a new issue under [Marketing Operations](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new) using the `onetrust_change_log_request` description template. View [OneTrust change logs here](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/?sort=created_date&state=all&or%5Blabel_name%5D%5B%5D=OneTrust%3A%3ACookieCategory&or%5Blabel_name%5D%5B%5D=OneTrust%3A%3ACookieUpdate&or%5Blabel_name%5D%5B%5D=OneTrust%3A%3AGeolocation&or%5Blabel_name%5D%5B%5D=OneTrust%3A%3AOptControl&first_page_size=20), which lists issues with a `OneTrust::` label.
...
[See the epic](https://example_company.com/groups/example_company-com/-/epics/1265) for more information.
...
1. Choose the `Example Company` organization to assign the domain scan to.
...
1. **Limit to this path within site:** OneTrust considers `about.example_company.com/fr` and `about.example_company.com` 2 separate domains with this option enabled.
...
1. **Scan pages with query parameters:** scan URLs with query parameters (ex: about.example_company.com?utm_source=marketo); Input field example: `name=first,name=last`. Separate multiple parameters with commas. The scan will search through the domain with those noted parameters. Ensure the domain you enter includes `?` at the end of the URL.
...
1. Click the pencil icon to edit the cooke category description. **Important:** You must confer with the Example Company legal team before updating these descriptions as they must strictly align with our policies regarding cookie use.
...
1. Name the template.
1. Choose the `Example Company` organization.
...
1. Choose `Example Company` organization.
