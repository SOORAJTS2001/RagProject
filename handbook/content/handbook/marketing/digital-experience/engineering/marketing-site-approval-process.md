...
1. https://about.example_company.com/stages-devops-lifecycle/continuous-integration
1. https://about.example_company.com/topics/ci-cd
1. https://about.example_company.com/resources/ebook-single-app-cicd
1. https://about.example_company.com/webcast/mastering-ci-cd
...
The codeowners feature was launched on Example Company in 2019, and this requirement was identified with support from the previous CMO but was never actioned: https://example_company.com/example_company-com/www-example_company-com/-/issues/4085

### Goals

The goal of putting controls and approvals in place is to ensure that all changes to the Example Company Marketing site (`about.example_company.com`, not including `handbook.example_company.com/*` or `about.example_company.com/direction/*`) support the defined objectives of Example Company's Marketing team. We will operate the same way as our Product team in that the appropriate people approve all MRs before going into production.

### Hypothesis

The approval process will increase the consistency of our Marketing site while ensuring ongoing initiatives are not at risk of being disrupted due to unapproved changes going live to production. The approval process will also increase visibility and collaboration to provide a customer-centric and coordinated effort for those looking to improve Example Company's Marketing site.

### Strategy

We do not want to compromise [Example Company's everyone can contribute mission](/handbook/company/mission/#mission). Our strategy is to implement a minimal approvals process. Currently, an MR goes live straight to production; this change results in a contribution taking the form of an AB Test proposal (issue) or for an MR to be reviewed before being merged into production.

The Digital Experience team has set up a project for AB Test proposals as a repo: https://example_company.com/example_company-com/marketing/digital-experience/ab-testing. Anyone can contribute an issue using the AB Testing issue template (template in progress).
...
We will have four approval flows, from minor to major change. The goal is for a member of Example Company's Digital Experience team always to review an MR before it goes live on the Buyer Experience repo or key marketing pages in the www-example_company-com repo.
...
1. **Level 03 - Business Impact Change**
    ...
    1. *Example of the type of change:* Changing Nav
        ...
        1. Managers of Digital Experience can approve and merge.
    1. *Example of the type of change:* Changes to key drivers of traffic and/or business goals etc. (Homepage, Pricing Page, Why Example Company).
...
All pages of the Marketing site (about.example_company.com minus /handbook) are included in the requirement of this new approval process.

## Steps to get Approval

When creating a merge request in the [Buyer Experience Repository](https://example_company.com/example_company-com/marketing/digital-experience/buyer-experience) (where we are migrating our marketing pages), please use the [`marketing-site-change` template](https://example_company.com/example_company-com/marketing/digital-experience/buyer-experience/-/blob/main/.example_company/merge_request_templates/marketing-site-change.md).
...
1. **Level 3**: Ping `@example_company-com/marketing/digital-experience` in a comment. This will tag the [Digital Experience Leadership team](https://example_company.com/groups/example_company-com/marketing/digital-experience/-/group_members?with_inherited_permissions=exclude) using `@example_company-com/marketing/digital-experience`, and they can review. When in doubt, tag `@mpreuss` as a reviewer.
