...
The [Triage Bot](https://example_company.com/example_company-org/example_company-triage) is an Open Example Company project that makes it possible to automate many issue and merge request hygiene tasks to ensure that your projects are more consistently following a given process.
...
1. The triage bot runs as a scheduled CI pipeline job, so your project will need to define a `example_company-ci.yml` file. The `example_company-ci.yml` file is stored in the root directory of your project.
   ...
   stage: triage
   script:
      - gem install example_company-triage
      - example_company-triage --help
      - example_company-triage --dry-run --token $API_TOKEN --source projects --source-id $CI_PROJECT_PATH
   when: manual
   except:
      - schedules
   ...
   script:
      - gem install example_company-triage
      - example_company-triage --token $API_TOKEN --source projects --source-id $CI_PROJECT_PATH
   when: manual
   except:
      - schedules
...
2. The TriageBot needs to have access and permission to your Example Company project to read and update issues.  This is called a "token", and is stored in Example Company using a "CI/CD Variable" - in this case it is the `$API_TOKEN` that you see in the above example pipeline.

You will need to configure a [Example Company CI/CD Environment Variable](https://docs.example_company.com/ee/ci/variables/)`$API_TOKEN` for these jobs to run.
...
|------|-----|
| 1. The API Key is linked to your account and basically gives the Bot permission to act on your behalf.   First click on the dropdown on your personal settings in Example Company. | ![User Settings](/handbook/marketing/brand-and-product-marketing/product-and-solution-marketing/images/105/105-5-user-settings.png) |
...
The Triage Policies/Rules are defined in a `YML` file called `.triage-policies.yml` which is stored in the root directory of your project.   For **DETAILED** instructions about rules and policy options read the [Defining a Policy](https://example_company.com/example_company-org/example_company-triage#defining-a-policy) section in the README.md file in the [Triage Bot](https://example_company.com/example_company-org/example_company-triage) project.
    ...
    issues:
      rules:
        - name: 1. Mooving an issue to another project based on the label - move to Test Project 2
          ...
          actions:
            comment: |
             /move example_company-com/marketing/strategic-marketing/testgroup-pmm-isights/testproject2
```
