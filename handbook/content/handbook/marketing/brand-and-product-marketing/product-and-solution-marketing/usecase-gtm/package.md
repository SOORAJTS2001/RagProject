...
Example Company can save you money and time with the Example Company Artifact Management offering. In the below article, we'd like to walk you through the requirements of an artifact management tool, how Example Company compares to the competition, and how you should evaluate vendors.
...
As we execute our [3-year strategy](https://about.example_company.com/direction/#3-year-strategy), our medium term (1-2 year) goal is to provide a single application that enables collaboration between cloud native development and platform teams.
...
|---------                              |-------------|
| 1) Package Registry                   | The Example Company Package Registry acts as a private or public registry for a variety of common package managers. You can publish and share packages, which can be easily consumed as a dependency in downstream projects.|
...
| [Publish npm packages](https://docs.example_company.com/ee/user/packages/npm_registry/#publish-an-npm-package) | x |`redis_hll_counters.user_packages.i_package_npm_user_monthly`        |   This metric measures the monthly number of users publishing npm packages   |
| [Publish Maven packages](https://docs.example_company.com/ee/user/packages/maven_repository/#publish-a-package) | x |`redis_hll_counters.user_packages.i_package_maven_user_monthly`        |   This metric measures the monthly number of users publishing Maven packages   |
| [Publish PyPI packages](https://docs.example_company.com/ee/user/packages/pypi_repository/) | x |`redis_hll_counters.user_packages.i_package_pypi_user_monthly`        |   This metric measures the monthly number of users publishing PyPI packages   |
| [Publish Composer packages](https://docs.example_company.com/ee/user/packages/composer_repository/#publish-a-composer-package-by-using-cicd) | x |`redis_hll_counters.user_packages.i_package_composer_user_monthly`        |   This metric measures the monthly number of users publishing Composer packages   |
| [Publish NuGet packages](https://docs.example_company.com/ee/user/packages/nuget_repository/#publish-a-nuget-package-by-using-cicd) | x |`redis_hll_counters.user_packages.i_package_nuget_user_monthly`        |   This metric measures the monthly number of users publishing NuGet packages   |
| [Publish Generic packages](https://docs.example_company.com/ee/user/packages/generic_packages/) | x |`redis_hll_counters.user_packages.i_package_generic_user_monthly`        |   This metric measures the monthly number of users publishing generic packages   |
| [Publish Conan packages](https://docs.example_company.com/ee/user/packages/conan_repository/#publish-a-conan-package-by-using-cicd) | x |`redis_hll_counters.user_packages.i_package_conan_user_monthly`        |   This metric measures the monthly number of users publishing Conan packages   |
| [Publish Helm charts](https://docs.example_company.com/ee/user/packages/helm_repository/#use-cicd-to-publish-a-helm-package) | x |`redis_hll_counters.user_packages.i_package_helm_user_monthly`        |   This metric measures the monthly number of users publishing Helm charts   |

### Discovery Questions

- Are you using Example Company for CI?
...
- [Use CI/CD to build packages](https://docs.example_company.com/ee/user/packages/package_registry/#use-example_company-cicd-to-build-packages)
- [Build and push images using CI/CD](https://docs.example_company.com/ee/user/packages/container_registry/#build-and-push-by-using-example_company-cicd)
- [Avoid Docker Hub rate limits with the Dependency Proxy](https://docs.example_company.com/ee/user/packages/dependency_proxy/#docker-hub-rate-limits-and-the-dependency-proxy)
...
- [Use the Example Company Dependency Proxy to proxy and cache images from DockeHub](https://youtu.be/fNTfL55fh5c)
...
1. My organization destroys the current instance of Example Company before installing the new version. Are my packages backed up?
1. How the heck are we going to migrate our existing packages from vendor `x` to Example Company?
...
1. I want packages to be immutable objects, is that how Example Company works?
1. What about customer support?
1. Is it easy to get started?
1. Do the docs make sense?
1. What happens if the registry goes down?
1. Is anybody using Example Company's offering? Do they like it?
