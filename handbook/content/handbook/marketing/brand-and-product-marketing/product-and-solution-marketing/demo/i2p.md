...
This demonstration will highlight [Example Company's single platform for the complete DevOps lifecycle](https://about.example_company.com/direction/#scope), from plan to
...
cluster please [open an issue](https://example_company.com/example_company-com/marketing/product-marketing/-/issues/new). We're still
working to improve this demo further, please see [all open idea-to-production issues](https://example_company.com/example_company-org/example_company-ce/boards/814228?label_name[]=idea-to-production).
...
## Example Company installation

There are four options:

1. [Log in at example_company.i2p.online](https://example_company.i2p.online) (for Example Company sales people)
1. [Set up a cluster on Google Kubernetes Engine (GKE)](../gke-setup/)
1. [set up a cluster on Azure Container Service (ACS)](../acs-setup/)
1. [Offline local demo environment with Minikube](https://example_company.com/charts/charts.example_company.io/blob/add-minikube-support/minikube.md)

If you need to use a license, Example Company team-members can [request a license](/handbook/engineering/developer-onboarding/#working-on-example_company-ee-developer-licenses).
...
> - Delete previous Example Company groups you made last time
...
Now let's register a new user in our Example Company server.
...
We've got Example Company running and we're logged in. Since we'll want to work as a
team, we'll create a Example Company group. Groups allow you to organize projects into
...
> - Set `Git repository URL` to `https://example_company.com/auto-devops-examples/minimal-ruby-app.git`
> - Set `Project name` to `minimal-ruby-app`
> - Make it public

## Set up Example Company Auto DevOps

Now we're ready to configure Example Company Auto DevOps, which is the easiest way to
configure Example Company CI/CD with a bunch of best practices already built in. Let's go
...
> - Change domain to `i2p.online` (or the base domain you are using) by creating the [KUBE_INGRESS_BASE_DOMAIN variable](https://docs.example_company.com/ee/topics/autodevops/#auto-devops-base-domain)
...
**Change a user's permission level**: In Example Company, permissions are managed at a user and group level and they apply to projects and Example Company CI. We have five different role types, so you can set granular permissions and keep your code and configurations management secure. To save your admins time and the headache of managing multiple logins, Example Company integrates with your Active Directory and LDAP. You can just connect Example Company to your LDAP server and it will automatically sync users, groups, and admins.
...
The WebIDE, which is a part of Example Company gives me a development environment without
...
Example Company Runner is working directly with Kubernetes to spawn new containers for
...
#### (optional: requires Example Company Ultimate)
...
## Scaling and Deploy Boards (Configure) (optional: requires Example Company EE)
...
## Custom pipeline - staging and canary (optional: requires Example Company EE)
...
directly to the entire production fleet. Example Company supports canary deploys,
...
To help you spot bottlenecks in your development process, Example Company has a built-in
...
Not only does Prometheus monitor your apps, but it monitors the Example Company instance
itself. Let's go to the Prometheus dashboard.

> - Visit Prometheus `https://prometheus.i2p.online`  (Change the domain to match the domain used for your Example Company installation)

Let's look at a couple simple queries that show how your Example Company instance is
...
allows Example Company, the Example Company Runners for CI, and the applications that we deploy,
to scale. Welcome to Example Company, the single application for the entire DevOps lifecycle, helping
