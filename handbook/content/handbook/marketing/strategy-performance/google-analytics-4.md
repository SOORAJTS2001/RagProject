...
[Google Analytics 4](https://analytics.google.com/analytics/web/#/p267713722/) (GA4) is a web analytics tool and the successor to Universal Google Analytics, which was discontinued in July 2024. GA4 provides metrics and dimensions for website traffic and user experience on Example Company's web properties, including:

* about.example_company.com
* docs.example_company.com
* page.example_company.com
* learn.example_company.com
* university.example_company.com
* forum.example_company.com
* customers.example_company.com (checkout pages)
* example_company.com (registration pages)
  * /sign_up/
  * /users/sign_in
  * /users/almost_there
...
| saas_trial_form                 |                                                                                                        | Form submission on the "Start Your Ultimate Trial" page. This is a conversion point for SaaS trial. Returning users can reach this page with an existing Example Company account. |
...
| navattic_demo                   | "navattic_event_name<br>navattic_flow_name<br>navattic_project_name<br>navattic_click_url<br>navattic_step_number" | Navattic interactive demos (e.g. https://about.example_company.com/small-business/)                                                             |
...
| saas_trial_duopro               |                                                                                                        | Duo Pro trial registration on example_company.com/-/trials/duo_pro/new - 4/1/24                                                                 |
...
| Sixsense Company       | The visitor’s company name                     | Example Company                | SSENSE_COMPANY       |
...
The eCommerce Purchase report, under the Monetization tab, details e-commerce activities on the [Pricing](https://about.example_company.com/pricing/) and [Checkout](https://customers.example_company.com/subscriptions/new?plan_id=2c92a01176f0d50a0176f3043c4d4a53&test=capabilities) pages. Item parameters can be used as dimensions to break down the desired event. The following table outlines the e-commerce events and the associated item parameters.
...
|---------------------------|-----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------|
| `generate_lead`             | `form_submit` event where the `form_type` dimension does not equal "self managed trial" or "newsletter" | All Marketo form submissions across example_company.com web properties, excluding blog subscription and self-managed trials.      |
| `saas_trial`                | `saas_trial_form` event and `saas_trial_company` event where the `saas_trial_company_type` equals "ultimate_trial" | SaaS trial form submissions on the [About Your Company](https://example_company.com/users/sign_up/company/new) and [Start Your Free Ultimate Trial](https://example_company.com/-/trials/new) pages.                            |
| `self_managed_trial`        | `form_submit` event where the `form_type` dimension equals “self managed trial”        | Self-managed trial form submission on the [Free Trial](https://about.example_company.com/free-trial/?hosted=self-managed) page.                                                                  |
...
GA4 natively exports daily events into BigQuery project name `mktg-goog-analytics4-5e6dc7d6`. The Enterprise Data team [exports the GA4 events](https://example_company.com/groups/example_company-com/marketing/-/epics/5047) from BigQuery into these Snowflake tables:
...
Snowplow is a SQL-based web analytics tool and the main tool for analyzing product data. Snowplow includes a column that pulls in the GA4 client ID, made available by the Analytics Implementation team leveraging the [GA cookie plug-in from Snowplow](https://example_company.com/example_company-org/example_company/-/issues/469410). The client ID can be used as a primary key between GA4 and Snowplow. The namespace ID in Snowplow is available to join with other tables regarding web-to-product usage.
...
Marketo forms include hidden fields that collect the GA4 client ID on about.example_company.com, page.example_company.com, and learn.example_company.com. Look for the `GACLIENTID__c` field to find it. Marketo will then [feed the client ID to SFDC tables](https://example_company.com/example_company-com/marketing/marketing-strategy-performance/-/issues/1639), allowing us to analyze web-to-sales assisted subscriptions.
