...
Also in this section, you can see where teams are from a forecast to plan standpoint by clicking into the `view-roll up` button. [Video walk through](https://youtu.be/TnY8T9s3o-E) in Example Company Unfiltered.
...
- Spend type - relates to the account code, but slightly more granular detail to help the marketer.
  - Field Marketing (FM) only - using the [Allocadia Impact Modeller](https://example_company.com/example_company-com/marketing/marketing-strategy-performance/-/issues/159), the spend type you choose, will spit out the expected ROI based on historical conversion rates.
...
- Example Company Marketing issue URL - a link to the issue where the marketer is managing the activity (not procurement issue - see below); leave blank if N/A
...
- Example Company Procurement issue URL - this will auto populate for you if you've created the issue with a click of a button within Allocadia
...
#### 🗓 Example Company Internal Calendars

Certain groups within Example Company have the ability to create a calendar invite directly from the details panel within Allocadia.
...
- Based on the inputs from the details panel, the logic is built as follows:
  ...
  - Next Column: `Spend Type` (Campaign Type)
    - If value is `Self-Service Virtual Event` use the `Example Company Hosted Self-Service Virtual Events` calendar
...
While Workshops & Webcasts DO require a calendar invite, we intentionally left the auto create calendar invite out of the plug in. We did this because the `Example Company Hosted Zoom Webcasts` calendar requires Zoom integration work and is a locked calendar, given the technical work required to run the campaign types. For more information on booking a Example Company-hosted Workshop or Webcast, please visit [this handbook page](/handbook/marketing/field-marketing/field-marketing-owned-virtual-events/#securing-a-virtual-workshop-or-webcast-date).
...
Today our Field Marketing team is the only team using this function. If you are interested in having this added to your view please open [this issue](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new) for Marketing Ops.
...
Live MQL>SAO regional data [can be found here](https://10az.online.tableau.com/#/site/example_company/views/DraftFieldMarketingMetrics_v2/KPITrend?:iid=1) and live SAO>Won data [can be found here](https://10az.online.tableau.com/#/site/example_company/views/DraftFieldMarketingMetrics_v2/KPITrend?:iid=1).

The average deal size by campaign by region can be found [here](https://10az.online.tableau.com/#/site/example_company/views/DraftFieldMarketingMetrics_v2/KPITrend?:iid=13) (you'll have to refer back to dashboard to change the region if you want to see a specific regions data) are also loaded for calculation purposes. If there were less than 3 closed deals then we did NOT use that deal size information, but relied on the ASP from the TOPO model to fulfill that deal size.

A calculation is then done by then taking into account the MQLs, region, segment data and campaign type (specifically the Example Company Issue type) that has been given at the line item level in the details panel.
...
- To view all videos posted, you must be logged in to Example Company Unfiltered.
...
### Allocadia - Example Company Issue Integration

An efficiency we are able to gain by using Allocaida is that we create a Example Company issue and can push information gathered in the details panel directly to that issue. This saves the team's time by not having to reenter information which has already been entered.
...
1. Go back up to the sub-category details panel and scroll all the way down to the `Actions` section where you will see a button called Example Company issue - click that - and 💥 - your issue is created

   - [Video walk through on Example Company unfiltered.](https://youtu.be/EsGlPNeH2P4)
...
Should a date on a campaign need to change and the issue has already been created, you can systematically change the date & name. In order to do so, all changes must be made in Allocadia and then overnight will push the change to the Example Company FMM issue. Should you want to manually change both the issue AND Allocadia, you should feel empowered to do so. The automation is there to help you.

##### Help! The feature to create a Example Company issue is missing from the details panel

You can **ONLY** create a Example Company issue from within a **SUB-CATEGORY**. Please double check you have created a subcategory, and a line item as listed in the [instructions here.](/handbook/marketing/strategy-performance/allocadia/#how-to-create-a-fmm-issue-from-within-allocadia)
...
#### Making changes to the Example Company issues

If you find yourself needing to update a procurement issue template for any of the issues above - please [submit an issue](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new) for the Marketing Operations team, as this must be done through our Allocadia CSM.

#### Wrong issue template was pushed...what now?

If you've hit the Example Company issue template button and the wrong issue template has been created, not to fear! Someone with owner or admin rights to your folder can assist you. To determine who has owner or admin rights to your folder from the home screen you would find your folder, click in the `Users` column that person icon with (number), from there, you see a list of people who have access + their rights. Search until you find `Owner` or `Administrator` and then slack that person in the #mktg-budget-holders slack room. [Video walk through](https://youtu.be/PwbWpt3pMX4) (must be logged into Example Company Unfiltered!).
