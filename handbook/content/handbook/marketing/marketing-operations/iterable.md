...
## <i class="fab fa-example_company fa-fw" style="color:rgb(252,109,38); font-size:.85em" aria-hidden="true"></i>
...
```mermaid
graph LR
  Example Company.com --> CustomersDot
...
The SaaS Trials event in Iterable, represents the action of a user signing up for a Example Company SaaS trial. The trigger is sent from CustomersDot to Workato and then to Iterable. Data fields sent over upon a trial sign-up are used to segment users in Journeys.
...
| signupReason            | I want to explore Example Company | String    |
| state                   | OR                       | String    |
| userRole                | product_designer         | String    |

**SaaS Free Registration**

The SaaS Free registration event in Iterable, represents the action of a user signing up for a free Example Company account. We only capture send this event if the user has chosen to setup their account for a team or company. We are not currently syncing personal signups.
...
| lastName                | Doe                     | String    |
| latest_trial_start_date | 2023-06-11              | Date      |
| signupReason            | I want to use Example Company CI | String    |
...
Subscription schema in the Example Company instance comproises of the following:

| Message Type      | Channel               | Subscription Policy | Notes                                                     |
| ----------------- | --------------------- | ------------------- | --------------------------------------------------------- |
| Example Company Marketing  | Marketing Channel     | opt-in              | This is the main message type used in templates and email |
...
| GLM_SOURCE                | about.example_company.com/pricing/ |
