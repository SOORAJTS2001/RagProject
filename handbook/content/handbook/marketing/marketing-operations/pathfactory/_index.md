...
- **Sharing individual assets.** It's meant to encourage a visitor to "binge" the content and self-nurture towards MQL. For sharing individual assets, please direct to the Example Company website.
...
- **Marketo nurture:** To keep the Example Company top of mind and deliver relevant content (via PathFactory)
- **PathFactory:** To increase consumption/engagement with Example Company content
...
PathFactory is not a single source of truth (SSoT) for all Example Company content, but rather content that has been activated in a marketing campaign.
...
There are four levels of access - `Admin`, `Author`, `Reporter`, `Viewer`, and `Uploader` - which are granted based on the persons' role and team function within Example Company. All access levels have the ability to view analtyics within the tool.
...
- `Viewer` access provides general visibility to content within PathFactory but does not allow end user to create or modify any of the content, tracks or tags. This level of access is granted for the general Example Company team member both within Marketing and elsewhere who have a business need to have access.
...
1. [Getting started video series](http://successwith.pathfactory.com/c/lookbookhq-tutorial-?x=Blrk3E) (must be provisioned to PathFactory and use your Example Company email address to view)
...
1. [PathFactory Author Onboarding](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new?issuable_template=pathfactory_author_onboarding)
1. [Generic PathFactory request](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new?issuable_template=pathfactory_request)
1. [PathFactory QA Review request](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new?issuable_template=pathfactory_qa_review)
1. [New PathFactory form request](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new?issuable_template=pathfactory_form)
1. [PathFactory Content Audit](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new?issuable_template=pathfactory_content_audit)
1. [PathFactory Bulk Upload](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new?issuable_template=pathfactory_bulk_upload)
1. [PathFactory Microsite Request](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new?issuable_template=pathfactory_microsite_request)

**[Campaigns](/handbook/marketing/demand-generation/campaigns/)**

1. [Pathfactory Upload Request](https://example_company.com/example_company-com/marketing/demand-generation/campaigns/-/issues/new?issuable_template=request-pathfactory-upload) - per [rules of engagement](/handbook/marketing/marketing-operations/pathfactory/#rules-of-engagement-by-team), the content owner is DRI on uploading their new content
   - _The upload issue may be created in the content owner's Example Company project; this issue template may be helpful in outlining the steps, campaigns team is not responsible for uploads._
1. [Pathfactory Bulk Upload Request](https://example_company.com/example_company-com/marketing/demand-generation/campaigns/-/issues/new?issuable_template=request-pathfactory-upload-bulk)
...
1. [Pathfactory Track Request](https://example_company.com/example_company-com/marketing/demand-generation/campaigns/-/issues/new?issuable_template=request-pathfactory-track)
...
For instance, if an asset is deleted from the content library and that asset exists in campaign-related content tracks, the user that visits that asset within that track will be redirected to `about.example_company.com`, which has the following impacts:
...
1. Add a bullet with your Example Company username and document the changes you made, making sure to include links to issues or relevant PathFactory links.
...
| internal use only | content track is distributed to Example Company team members only |
...
| website | content track is linked on a web page on `about.example_company.com` |
...
- Removing an asset or changing the custom URL slug of an asset in a `[LIVE]` track can disrupt the user experience for the visitor and activate the `#all` track or fallback URL (`about.example_company.com`) instead of the intended content track. Please ensure that the link to the asset is not being used in any other marketing channels before deleting.
...
- These tracks live under [Target Tracks](https://example_company.lookbookhq.com/authoring/content-library/target) > Demand Gen > `Segment Campaigns` (subject to change if the folders get massive - HB to update as needed)
...
- The [Example "PBJ" Track](https://example_company.lookbookhq.com/authoring/content-library/target/57384) has the following elements and settings applied (for easy cloning) but only contains one asset when the future ones would have 3-4 assets:
...
To request a new form to use in PathFactory, the form must first exist in Marketo. If one does not exist, first create an issue in the marketing ops project using the [`form_request.md` template](https://example_company.com/example_company-com/marketing/marketing-operations/-/blob/master/.example_company/issue_templates/form_request.md). Once the Marketo form is completed, then create an issue in the marketing ops project using the [`pathfactory_form.md` template](https://example_company.com/example_company-com/marketing/marketing-operations/-/blob/master/.example_company/issue_templates/pathfactory_form.md).
...
- If the link breaks or an asset is deleted, the user will be redirected from your content track to the `#all` track, which includes all assets uploaded to PathFactory. In a case where the user is not redirected to the `#all` track, they will be redirected to the `Fallback URL` which is set to `about.example_company.com`.
- **Do not use the `lb_email=` parameter when sharing `about.example_company.com` links in Marketo emails. This parameter is used on `learn.example_company.com` links to identify anonymous track visitors.**
...
1. If there is a question mark `?`, first add an ampersand `&` at the end of the PF link, followed by the UTMs.
    - For example:
    - PF Link: `https://learn.example_company.com/c/10-strategies-to-red?x=53kuPb`
    - PF Link with UTMs: `https://learn.example_company.com/c/10-strategies-to-red?x=53kuPb&utm_source=email&utm_campaign=test`
1. If there is no question mark `?`, first add a question mark `?` at the end of the PF link, followed by the UTMs.
    - PF Link: `https://learn.example_company.com/c/10-strategies-to-red`
    - PF Link with UTMs: `https://learn.example_company.com/c/10-strategies-to-red?utm_source=email&utm_campaign=test`
...
1. For a PF Marketo link, it will typically already include a question mark "?". To add UTMs, first add an ampersand `&` at the end of the PF link, followed by the UTMs.
    - Example: `https://learn.example_company.com/c/devops-explained-git?x=GVFK3F&lb_email={{lead.Email Address}}&utm_source=email&utm_campaign=test`
1. Remove any extraneous `?` (there should only be one immediately after the end of the URL).
1. Watch for extra `&`.
...
`https://learn.example_company.com/c/gcn-dev-sec-ops-how-?x=XOIXTl&lb_email=`
...
1. If your content track has a custom URL, you will notice the `?` in a different location than with content tracks that don't have a custom URL.
    - With no custom URL: `https://learn.example_company.com/c/gcn-dev-sec-ops-how-?x=XOIXTl&lb_email=` (`x=XOIXTl` = content track ID)
    - With custom URL: `https://learn.example_company.com/cicd/cloud-ci-tools-matur?lb_email={{lead.Email Address}}`

**Hiding Top Navigation When Adding Pages from the Marketing Site to a Track**

1. Add `?menu=hide` to the end of the asset's source URL. For example, `https://about.example_company.com/blog/2021/04/28/devops-platform-supply-chain-attacks/?menu=hide`
...
You can request a new microsite, or an update to an existing microsite via [this issue](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new?issuable_template=pathfactory_microsite_request).
...
1. If there are sections of the langauge configuration that are not translated in your selected language, [please open an issue](https://example_company.com/example_company-com/marketing/marketing-operations/-/issues/new?issuable_template=pathfactory_request).
