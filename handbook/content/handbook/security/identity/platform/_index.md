...
- CI/CD Pipeline
  ...
  - Provisioning Stage
    ...
    - [Example Company Groups Provisioning](/handbook/security/identity/platform/provisioning/example_company)
    - [Example Company Projects Provisioning](/handbook/security/identity/platform/provisioning/example_company)
...
Our Identity Platform open source projects can be explored at [https://example_company.com/example_company-identity](https://example_company.com/example_company-identity). Our team members also maintain side project packages that are useful dependencies and development helpers at [https://example_company.com/provisionesta](https://example_company.com/provisionesta).
...
We have different categories of users (ex. employees, contractors, service accounts, system administrators) that need birthright access to various systems. We use our [access level wristbands](https://internal.example_company.com/handbook/it/it-self-service/access-level-wristband-colors/) color coding to specify each Okta user's type as `blue`, `purple`, `gray`, `brown`, or `black`.
...
This is a next generation iteration of a Example Company Identity v2 [job family](/job-families/) and [role-based baseline entitlements](https://internal.example_company.com/handbook/it/end-user-services/access-request/baseline-entitlements/).
...
As our organization structure ebbs and flows, we have 200-250 unique Identity Roles at any given time.  See all of the roles in the [policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies) and [manifests](https://example_company.com/example_company-com/gl-security/identity/data-poc/manifests) repositories.

| Division/Function | Identity Roles | Upstream Data Source |
|-------------------|----------------|----------------------|
| Administrators | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/black_ops.yml?ref_type=heads) | Identity Engineering |
| Executive | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/business_executive.yml?ref_type=heads) | Identity Engineering |
| Finance | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/business_finance.yml?ref_type=heads) | Managers and Job Titles |
| Legal | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/business_legal.yml?ref_type=heads) | Identity Engineering |
| Marketing | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/business_marketing.yml?ref_type=heads) | Department Names |
| People | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/business_people.yml?ref_type=heads) | [Handbook Page](/handbook/people-group/#how-to-reach-the-right-member-of-the-people-group) and Manager |
| Sales | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/business_sales.yml?ref_type=heads) | Sales EBA Team and Department/Manager |
| Product Development | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/product_dev.yml?ref_type=heads) | [DevOps Stages](/handbook/product/categories/#devops-stages) / [YAML](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/stages.yml?ref_type=heads) |
| Product Production | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/product_prd.yml?ref_type=heads) | [Handbook Page](/handbook/engineering/infrastructure/#organization-structure) |
| Security | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/security.yml?ref_type=heads) | [Handbook Page](/handbook/security/#division-structure) |
| Service Accounts | [Policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies/-/blob/main/role/policies/service_accounts.yml?ref_type=heads) | Identity Engineering |
...
See all of the organization units in the [policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies) and [manifests](https://example_company.com/example_company-com/gl-security/identity/data-poc/manifests) repositories.

### Identity Groups

We use **Identity Types**, **Identity Roles** and **Identity Organization Units** across multiple systems for role-based access control (RBAC) by creating an Okta Group, Google Group, and Example Company SaaS Group with the same name. Each of these groups can be attached to one or more Okta Applications, Google Cloud projects or folders, or Example Company Groups or Projects. Additional systems may be added in the future.
...
#### Example Company SaaS Groups

The Example Company SaaS group for the type, role, or organization unit can be invited as a Direct Member on any other Example Company group (with child projects) or a Example Company project with the appropriate [permission role](https://docs.example_company.com/ee/user/permissions.html).

- `@example_company-rbac/type/{color}`
- `@example_company-rbac/ou/{ou_name}`
- `@example_company-rbac/role/{role_name}`

This allows us to grant minimum permissions at the top level of the `example_company-com` and `example_company-org` namespaces, and programatically add elevated `Developer`, `Maintainer`, and `Owner` [permissions](https://docs.example_company.com/ee/user/permissions.html) in downstream groups and projects for specific teams to self manage their own child namespaces without having full read-write access across all projects inherited by the top-level namespace.

In the (distant) future, we will manage Example Company groups and project configuration settings using Terraform.
...
See all of the policies  in the [policies](https://example_company.com/example_company-com/gl-security/identity/data-poc/policies) repository.
...
// https://docs.example_company.com/ee/api/members.html#add-a-member-to-a-group-or-project
// https://docs.example_company.com/ee/api/members.html#roles
$gitlab_response = GitlabApiClient::post(
    connection: config('connections.example_company.saas'),
...
// https://docs.example_company.com/ee/api/members.html#remove-a-member-from-a-group-or-project
...
See the mirrored [PoC data repositories](https://example_company.com/example_company-com/gl-security/identity/data-poc) for a sneak peak of the future of Identity v3.
...
- The SSOT repository is hosted on the Black OPs self-managed Example Company instance to protect the data integrity, and repository data is mirrored to Example Company.com SaaS for team member read-only visibility.
- The runtime pipelines, CI/CD variables, and secrets management are on the Black Ops Example Company instance that no team members have elevated permissions on other than break glass access by the Identity Engineering Team. We use the CLI for generating properly formatted changes via API calls to avoid users making freeform changes or clicking buttons.
...
CI_REST_GITLAB["Example Company API"]
end
end
...
subgraph Example Company API for SaaS and ops.example_company.net
GITLAB_GROUPS["@example_company-rbac/* Groups"]:::orange
GITLAB_GROUPS_USERS["Group Members"]:::orange
GITLAB_USERS["@example_company.com Users"]
...
CTL_UI["ctl.example_company.systems<br/>accessctl Identity UI<br />Team Member Self Service"]:::red
end
end

subgraph Data Plane
direction TB
CTL_API["ctl.example_company.black<br/>accessctl Identity API<br />(behind Teleport bastion)"]:::re
...
Access Control is an open source monolith [code base](https://example_company.com/example_company-identity/accessctl) created by Jeff Martin that is built with the [Laravel framework](https://laravel.com) with multiple functional components included that are deployed in a distributed fashion to dogfood Example Company CI/CD pipelines and Example Company Runner for background jobs, Example Company repository for policy and manifest storage, CODEOWNERS and merge request approvals for change management.

Although Laravel has [batteries included](https://laravel.com/docs) for all of the full stack web application and SQL database capabilities that rivals Django or Ruby on Rails, we are using it as more of a scripting engine that is easier for team members to contribute to and leverages Example Company for backend processing wherever possible. We still use Laravel native capabilities to expose a REST API, Web UI, and CLI for improved frontend user experience.

We use [Laravel Actions](https://laravelactions.com) for our business logic service classes. By using Laravel Actions, we simplify our architecture since each single responsibility principle (SRP) action can be invoked as a class, a console CLI command, or a background job. This also allows us to get data from the Example Company API for repository data instead of needing to use models and migrations for a large SQL database.

See the [example_company-identity/accessctl](https://example_company.com/example_company-identity/accessctl) repository and README to learn more.
...
You may have already seen what's possible with Example Company Sandbox Cloud, Demo Systems, and Training Lab Manager. It just works and only requires 1 engineer part time to take us to production. We've taught more than a dozen team members how Laravel works without any trouble. This isn't part of the Example Company product itself so that's a non factor. Please reach out to Jeff for a code pair if you're interested.

### CI/CD Scripts

We use a Docker image with the `accessctl` application with Example Company Runner to run Artisan Console commands that from Laravel actions. Each CI/CD job is usually related to ETL operations for (E)xtracting IAM/RBAC data from vendor APIs, (T)ransforming the API responses into standardized YAML and JSON array schemas, and (L)oading the data into the Example Company repositories (`accessctl-manifests`, `accessctl-auditlog` and `accessctl-policies` for system automation) and Google Sheets (for human use).
...
We have the `accessctl` application deployed at `https://ctl.example_company.black` that provides a REST API for data stored in the Example Company repository that is proxied and formatted as if we used a traditional SQL database. This improves the consumability of our data, and provides an endpoint for CLI application.
...
We have the `accessctl` application deployed at `https://ctl.example_company.systems` that provides Web UI with Okta authentication (with 2FA and Okta Verify device trust) for end user team members and managers to perform day-to-day tasks without administrative access.
