...
Example Company receives vulnerability reports by various pathways, including:
...
issues as `~bug::vulnerability` and @-mention `@example_company-com/gl-security/product-security/appsec` on issues.
...
- Remember to prepare patches, blog posts, email templates, etc. [following the security engineer process](https://example_company.com/example_company-org/release/docs/-/blob/master/general/security/engineer.md) or in other non-public ways even if there is a reason to believe that the vulnerability is already out in the public domain (e.g. the original report was made in a public issue that was later made confidential).
...
We currently request CVEs [through our CVE project](https://about.example_company.com/security/cve/). Keep in mind that some of our security releases contain *security related* enhancements which may not have an associated [CWE](https://cwe.mitre.org/) or vulnerability. These particular issues are not required to obtain a CVE since there's no associated vulnerability.

### On Release Day

On the day of the security release several things happen in order:

- The new Example Company packages are published.
- All security patches are pushed to the public repository.
- The public is notified via the Example Company blog release post, security alerts email, and Twitter.

The Example Company issue should then be closed and - after 30 days - sanitized and made public. If the report was received via HackerOne, follow the [HackerOne process]({{< ref "hackerone-process#closing-out-and-disclosing-issues" >}}).

### Process for disclosing security issues

At Example Company we value [being as transparent as possible](/handbook/values/#transparency), even [when it costs](/handbook/values/#transparency-is-most-valuable-if-you-continue-to-do-it-when-there-are-costs). Part of this is making confidential Example Company issues about security vulnerabilities public 30 days after a patch. The process is as follows:
...
To facilitate this process the [Example Company Security Bot](https://example_company.com/example_company-com/gl-security/engineering-and-research/automation-team/appsec-escalator) comments on confidential issues 30 days after issue closure when they are not labeled `~keep confidential`.

### Handling Disruptive Researcher Activity

Even though many of our 3rd-party dependencies, hosted services, and the static
`about.example_company.com` site are listed explicitly as out of scope, they are sometimes
targeted by researchers. This results in disruption to normal Example Company operations.
...
program or has resulted in activity that is disruptive to normal Example Company
...
Example Company operations, as described in policy under "Rules of Engagement,
Testing, and Proof-of-concepts", may result in additional restrictions on
participation in our program:

  Activity that is disruptive to Example Company operations will result in account bans and disqualification of the report.

Further details and some examples are available in the full policy available at:

https://hackerone.com/example_company


Best Regards,

Security Department | Example Company
https://handbook.example_company.com/handbook/security/
```

## Creating New Security Issues

Use [the Vulnerability Disclosure issue template](https://example_company.com/example_company-org/example_company/-/issues/new?issuable_template=Vulnerability%20Disclosure) to report a new security vulnerability, or use our [HackerOne bug bounty program](https://hackerone.com/example_company). Please note that we are only able to pay out bounties for eligible vulnerabilities through the HackerOne platform.

New security issue should follow these guidelines when being created on `Example Company.com`:
...
the triage process:
  ...
  - `~internal customer` should be added by team members when the issue
    impacts Example Company operations.
  - `~dependency update` if issue is related to updating to newer versions of the dependencies Example Company requires.
...
resources only available to Example Company team member, for example, the originating
ZenDesk ticket. Label the link with `(Example Company internal)` for clarity.
...
For review by the Application Security team, @ mention `@example_company-com/gl-security/product-security/appsec`.
...
Severity label is determined by CVSS score, using the [Example Company CVSS calculator](https://example_company-com.example_company.io/gl-security/product-security/appsec/cvss-calculator/).
...
The intent of tying `~severity/~priority` labels to remediation times is to measure and improve Example Company's
...
a external
metric that may be evaluated by users as an indication of Example Company's commitment
...
Furthermore, the [Security Release Process deadlines](https://example_company.com/example_company-org/release/docs/blob/master/general/security/process.md#release-deadlines)
...
Implementation of security feature issues should be done publicly in line with our [Transparency](/handbook/values/#transparency) value, i.e. not following the [security developer workflow](https://example_company.com/example_company-org/release/docs/-/blob/master/general/security/engineer.md).
...
The security team may also apply `~internal customer` and [~`security request`](https://example_company.com/groups/example_company-org/-/issues?scope=all&utf8=%E2%9C%93&state=opened&label_name%5B%5D=security%20request) to issue as an
...
support of Example Company.com.
...
- Add [group label](https://example_company.com/example_company-org/example_company-foss/blob/master/doc/development/contributing/issue_workflow.md#group-labels) (`~group::editor`, `~group::package`, etc.)
- Add [stage label](https://example_company.com/example_company-org/example_company-foss/blob/master/doc/development/contributing/issue_workflow.md#stage-labels)
...
that triaged the issue to confirm that the vulnerability is not present anymore. <b>Note that it might still be necessary to backport a patch to previous releases according to our [maintenance policy](https://docs.example_company.com/ee/policy/maintenance.html#security-releases)</b>.
In case no backports are necessary, the issue can be closed.

### Reducing the number of backports

With the approval of an Application Security Engineer a security issue may be fixed on the current stable release only, with no backports. Follow the [Example Company Maintenance Policy](https://docs.example_company.com/ee/policy/maintenance.html#security-releases) and apply the `~reduced backports` label to the issue.
