...
- Does the role description include writing or reviewing code that may interact with Example Company RED data, and in a language supported by Secure Code Warrior?
...
If a role should be added or removed, please open an MR and @ mention `@example_company-com/gl-security/product-security/appsec`. If your role is in the list already but SCW doesn't show up in Okta, ask for help in `#secure-code-warrior` on Slack. An AppSec team member will add you to [the Google Group](https://groups.google.com/a/example_company.com/g/okta-securecodewarrior-users/members) and update any baseline entitlements / onboarding checklists to ensure those in your role are included in the future.
