...
in service of supporting Example Company.com.
...
- [Example Company's Security Controls]({{< ref "sec-controls#gitlabs-security-controls" >}})
...
1. Individual access for disaster recovery MAY be granted to system owners as
   specified in the [tech stack tracking file](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/tech_stack.yml).
...
#### GCP Security Group (gcp-*-sg@example_company.com) Conventions

Security groups for `example_company.com` organization resources should be named `gcp-:grouppurpose-sg@example_company.com`.
...
`example_company.com` organization due to existing workloads, it should enforced at
...
For applications requiring shell access for daily work, [Okta ASA](https://example_company.com/example_company-cookbooks/gitlab_okta_asa) should be
