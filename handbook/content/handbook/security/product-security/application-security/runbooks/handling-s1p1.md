...
The following process is a supplement to the first few steps of the [critical release process](https://example_company.com/example_company-org/release/docs/blob/master/general/security/process.md#critical-security-releases)
...
1. If help from the Example Company Dedicated team is needed, [follow the runbook to escalate to their engineer on call](https://example_company-com.example_company.io/gl-infra/example_company-dedicated/team/runbooks/on-call.html#escalating-to-an-on-call-person).
...
Due to differences in settings, feature availability, and configuration between Example Company self-managed, Example Company Dedicated, and Example Company SaaS (Example Company.com), the CVSS for a single vulnerability may differ depending on environment.
...
When assessing if a Example Company vulnerability impacts Example Company Dedicated, consider the following features that are [**not available** in Example Company Dedicated](https://docs.example_company.com/ee/subscriptions/gitlab_dedicated/#features-that-are-not-available):

#### Application Features that are Unavailable in Example Company Dedicated
...
- [ ]  Example Company-managed runners (hosted runners)
- [ ]  Example Company AI capabilities ([More Info](https://about.example_company.com/direction/saas-platforms/dedicated/#supporting-ai-features-on-example_company-dedicated))
- [ ]  Features that must be configured outside of the Example Company user interface, including those behind [feature flags](https://docs.example_company.com/ee/user/feature_flags.html) which are disabled-by-default
- [ ]  Mattermost
- [ ]  Server-side Git hooks (Due to security concerns and potential service SLA impact. Consider using [push rules](https://docs.example_company.com/ee/user/project/repository/push_rules.html) or [webhooks](https://docs.example_company.com/ee/user/project/integrations/webhooks.html) as alternatives.)

If a vulnerability requires using features listed above for successfuly exploitation, it most likely **does not** impact Example Company Dedicated. Always cross-check with the specific details of the vulnerability to ensure accurate assessment.

## Mitigate

Mitigation of critical security issues has to strike a balance between securing Example Company and our users as fast as possible and doing it in a reliable way that will not require another patch shortly after.
The patch will first be deployed to Example Company-managed environments (.com, Dedicated, etc.) and then will be released to our self-managed users.
...
- Deploy a [hotpatch](https://example_company.com/example_company-org/release/docs/blob/master/general/deploy/post-deployment-patches.md).

### Releasing to self-managed customers

Since moving to a bi-weekly release schedule the need to follow the [critical security releases workflow](https://example_company.com/example_company-org/release/docs/blob/master/general/security/security-engineer.md##critical-security-releases) to patch critical vulnerabilities is much lower.
