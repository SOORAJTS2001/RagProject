...
Example Company utilizes [HackerOne](https://hackerone.com/example_company) for its bug bounty program. Security researchers can report vulnerabilities in Example Company applications or the Example Company infrastructure via the HackerOne website. Team members authorized to respond to HackerOne reports use procedures outlined here.
...
- `Example Company Team` contains reports that have been validated by the HackerOne triage team, but are yet to be assigned to a specific Example Company team member
...
## Example Company Team On-boarding

- New members of the Example Company security team are granted access to the Example Company HackerOne team via an access request issue using the appropriate [role based entitlement template](https://internal.example_company.com/handbook/it/end-user-services/access-request/baseline-entitlements/#role-entitlements-for-a-specific-job), which should be submitted by their manager during onboarding
- During onboarding, new Example Company security team members will be invited to join the HackerOne program if their role requires it.

## Working the Queue

### Namespace with Ultimate license for triaging

Please ensure that you use [this namespace](https://example_company.com/example_company-com/gl-security/product-security/appsec/bug-reproduction) to create projects and groups required for testing vulnerabilities. This namespace is dedicated to reproduction of HackerOne issues.

### HackerOne Triage Team

Example Company's bug bounty program is managed by HackerOne. The HackerOne triage team are the first responders, and will work with researchers to validate reports before assigning to `Example Company Team`.

We usually trust the HackerOne Triage Team and don't necessarily validate the report a second time. There are however cases when the Example Company team member on rotation may want to re-validate it, for example (non exhaustive list):
...
- When starting a triage work cycle, team members should prioritize as follows:
  ...
  1. Triage the `Example Company Team` queue.
...
- Potential, non-bounty outcomes:
  ...
  - Report is otherwise [Informative, Not Applicable, or Spam](#closing-reports-as-informative-not-applicable-or-spam).
    For example, we've gotten a number of reports for [inline HTML in markdown](https://docs.example_company.com/ee/user/markdown.html#inline-html)
  ...
  - Report is a duplicate. Verify that the issue was not previously reported in HackerOne, or
    that an issue does not already exist in Example Company. If it is a duplicate:
    ...
    - Fill in the `01 - Duplicate` common response. Include the link to the Example Company issue.
    - The team member may use their discretion if the reporter asks to be added as a contributor to the original H1 report;
        however, the default is to not add because the corresponding Example Company issue will
...
the responsible engineering team:
  - [Calculate the CVSS score](https://example_company-com.example_company.io/gl-security/product-security/appsec/cvss-calculator/) and post the resulting vector string (e.g.: `AV:N/AC:H/PR:N/UI:N/S:U/C:L/I:N/A:L`) as an internal comment on the report, this will be used later when requesting a CVE ID
  ...
  - If the report is [permissions related](https://docs.example_company.com/ee/development/secure_coding_guidelines.html#permissions), check for similar issues in the API, GraphQL, and Elasticsearch, as appropriate. Also check with alternate authentication mechanisms like Deploy Tokens, Deploy Keys, Trigger Tokens, etc.
  - Add your initial [suggested bounty](https://docs.hackerone.com/en/articles/8524543-bounties#h_13d3d2c2b7) in H1
  - Import the report into a Example Company issue using `/h1 import <report> [project] [options]` in Slack
    - Note: by default a placeholder [CVE issue](https://example_company.com/example_company-org/cves/-/issues) is created and a brief note is added to the latest [bug bounty council issue](https://example_company.com/example_company-com/gl-security/security-department-meta/issues?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=Bug%20Bounty%20Council). Pass `~no-cve` or `~no-bounty` respectively to the `/h1 import` command to prevent their creation.
  - On the imported Example Company issue:
  ...
  - Change the state of the report to "Triaged" in HackerOne:
    - See [Example Company's H1 Policy](https://hackerone.com/example_company), under `Rewards`, for portions of bounty rewards which are awarded at the time of triage
  ...
  - Update the CVE issue and Bug Bounty Council note with relevant details, while they are still fresh in your mind
    - If the CVSS score is higher on Example Company.com than self-managed, calculate both scores and share them in the Bug Bounty Council issue. If the council agrees that security impact is higher on Example Company.com than self-managed, bounty award will be based on the CVSS for Example Company.com. The CVE and security release blog post will always use the self-managed CVSS.
  ...
  - If full impact is needed to be assessed against Example Company infrastructure, instead of testing in https://example_company.com, use https://staging.example_company.com/help to sign in with your Example Company email account
    - If multiple users are needed, use credentials for users `example_company-qa-user*` stored in 1password Team Vault to access the staging environment
  - If the report is for a bug that can be detected without authentication (in Example Company or anything else we host) consider reaching out to the Red Team or Vulnerability Management to help create a Nuclei template that we can include into our scanning
...
- [ ] Mitigate the incident if possible
  - [ ] If the exposed secret is a Agent Token:
    - [ ] Validate if the token is a valid one by following the steps [here](https://example_company.com/example_company-com/gl-security/security-research/verify-kas-token#testing-kas-token-for-validity) and gather the output for the SIRT incident.
    - [ ] [Reset the token](https://docs.example_company.com/ee/user/clusters/agent/work_with_agent.html#reset-the-agent-token) and reach out to the owner of the token through Slack DM and in the SIRT issue that you will create in the steps below.
  - [ ] If the exposed secret is a Personal Access Token:
    - [ ] Using the API, gather the output of [`/api/v4/user`](https://docs.example_company.com/ee/api/users.html#for-normal-users-1) and [`/api/v4/personal_access_tokens/self`](https://docs.example_company.com/ee/api/personal_access_tokens.html#using-a-request-header) for the SIRT incident.
    - [ ] [Revoke the token](https://docs.example_company.com/ee/api/personal_access_tokens.html#using-a-request-header-1) and reach out to the owner of the token through Slack DM and in the SIRT issue that you will create in the steps below.
  - [ ] Post a comment in `#security-revocation-self-service` using [this message template](https://example_company.com/example_company-com/gl-security/security-operations/sirt/runbooks/-/blob/main/misc/exposed_secrets.md#general-revocation-template-for-secrets)
...
- [ ] In the H1 report use the reference field to link to the SIRT issue (for example if the incident issue is `https://example_company.com/example_company-sirt/incident_XXXX/-/issues/1` the reference should be `example_company-sirt/incident_XXXX/-/issues/1`)
- [ ] Identify the most appropriate [non-CVSS bounty amount](https://example_company-com.example_company.io/gl-security/product-security/appsec/cvss-calculator/) and add your initial [suggested bounty](https://docs.hackerone.com/en/articles/8524543-bounties#h_13d3d2c2b7) in H1
...
- [ ] Investigate the location of the exposure, and locations like it, for further exposure.
  ...
  - [ ] Use Advanced Search to look for similar patterns in other projects used by Example Company team members
- [ ] If it was leaked through Example Company Unfiltered, add the report to the [tracking issue](https://example_company.com/example_company-com/gl-security/security-research/video-scanner/youtube-video-scanner/-/issues/17)
```

</details>

Exposure of information and secrets is handled a little differently to vulnerabilities, as there is nothing to patch and therefore no need for a Example Company Project Issue, CVSS, or CVE. When a leak occurs:
...
- Use the `/security` slack command to initiate an incident
  - Learn more about engaging the SEOC: https://handbook.example_company.com/handbook/security/security-operations/sirt/engaging-security-on-call.html#engage-the-security-engineer-on-call
  - In the description section, include a link to the HackerOne report and any other useful information
    - Share the reporter's IP address(es) and time(s) the reporter accessed the sensitive data to assist with incident response.
  - In the remediation section, document what time and from what IP used to revoke the token or validate the leak.
  - If in doubt, choose "Urgent".
...
Similar to how we handle exposed secrets, we sometimes handle exposed personal data which also doesn't need a Example Company Project Issue, CVSS or CVE.
...
Vulnerabilities behind disabled-by-default feature flags do not need a CVE (use `~no-cve` when importing) as they are [patched in regular releases](https://docs.example_company.com/ee/administration/feature_flags.html#risks-when-enabling-features-still-in-development), not security releases.
...
1. Update the Ruby version inside the following file to the required version:
   - `example_company-development-kit/.tool-versions`
   - `example_company-development-kit/example_company/.tool-versions`
1. Run `asdf install ruby <required-version>` while inside the GDK directory.
1. Run `gem install example_company-development-kit` while inside the GDK directory.
1. Go into the `./example_company` directory inside the GDK direcory, and run `bundle install`.
1. Verify the Ruby version after running `gdk restart` and going to `http://127.0.0.1:3000/admin`

## Triaging deprecated features

Vulnerabilities in deprecated features are triaged normally. [See discussion](https://example_company.com/example_company-com/gl-security/product-security/appsec/appsec-team/-/issues/336) for more information.
...
- Engage `@sre-oncall` in Slack. This notifies the SRE (but does not intiate a PagerDuty ping) on-call of a situation requiring their attention. In the relevant SIRT issue, the responder should be added to the issue by the Example Company SIRT.
Remediation of this vulnerability happens within the SIRT issue and typically involves deleting the dangling CNAME record. For issues involving MX record takeovers we typically work with our MX SaaS vendor, Mailgun to obtain control of the record. More information on MX record takeovers can be found [here](https://example_company.com/example_company-com/gl-security/product-security/appsec/appsec-team/-/issues/334).
...
- If the report does not already have a council issue, use the `/h1 bounty <report>` Slack bot to post a note on the current [~"bug bounty council"](https://example_company.com/example_company-com/gl-security/security-department-meta/issues?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=Bug%20Bounty%20Council) issue
...
- Once the corresponding Example Company issue has been assigned a milestone, an
...
We should not disclose, or request to disclose, a HackerOne issue while the Example Company issue
 remains confidential.

Comments made by Example Company Security Bot (`@example_company-securitybot`) can be redacted by AppSec or SecAuto team members using the `/h1 redact <comment_url>` Slack command.
...
template. Reporters should instead consider [opening a public Example Company issue](https://about.example_company.com/submit-feedback/)
...
If the report does not pose a security risk to Example Company or Example Company users it can be closed without opening an issue on Example Company.com.
...
We mostly use the "Informative" status for reports with little to no security impact to Example Company and "Not Applicable" for reports that show a poor understanding of security concepts or reports that are clearly out of scope. "Spam" results in the maximum penalty to the researcher's reputation and should only be used in obvious cases of abuse.

It may be appropriate to suggest [opening a public Example Company Issue for reproducible bugs](https://about.example_company.com/submit-feedback/#reproducible-bugs) that are not vulnerabilities.
...
Sometimes there will be a breakdown in effective communication with a reporter. While this could happen for multiple reasons, it is important that further communication follows [Example Company's Guidelines for Effective and Responsible Communication](/handbook/communication/#effective--responsible-communication-guidelines). If communication with a reporter has gotten to this point, the following steps should be taken to help meet this goal.
...
When behavior violates our [Rules of Engagement](https://hackerone.com/example_company?type=team#user-content-rules-of-engagement-testing-and-proof-of-concepts) or HackerOne's [Code of Conduct](https://www.hackerone.com/policies/code-of-conduct) we use the Bug Bounty Council to discuss, agree on, and document our response. Add a comment to the current Bug Bounty Council Issue using the template found in the issue description.
...
When Example Company receives reports, via HackerOne or other means, which might affect third parties the reporter will be encouraged to report the vulnerabilities upstream. On a case-by-case basis, e.g. for urgent or critical issues, Example Company might proactively report security issues upstream while being transparent to the reporter and making sure the original reporter will be credited. Example Company team members however will not attempt to re-apply unique techniques observed from bug bounty related submissions towards unrelated third parties which might be affected.

Vulnerabilities on third-party software are accepted according to the following rules, as stated in our [HackerOne policy](https://hackerone.com/example_company?type=team):
...
- It has a clear and working proof of concept that illustrates the impact to Example Company.
- It has Critical or High impact to Example Company.
...
Example Company reporters with 3 or more valid reports are eligible for a 1-year Ultimate license for up to 5 users. As per the [H1 policy](https://example_company.com/example_company-com/gl-security/hackerone/configuration/-/blob/master/program-policy.md#example_company-ultimate-license), reporters will request the license through a comment on one of their issues. When a reporter has requested a license, the following steps should be taken:
...
1. If the reports are valid, visit the [Example Company Support Internal Requests page](https://support-super-form-example_company-com-support-support-op-651f22e90ce6d7.example_company.io/) and use the **Example Company Support Internal Requests for Global customers** request option, then select **Hacker One Reporter license** for the internal request type.
...
Members of the public can ask questions about our HackerOne bug bounty program here: [https://example_company.com/example_company-com/gl-security/product-security/appsec/hackerone-questions/](https://example_company.com/example_company-com/gl-security/product-security/appsec/hackerone-questions/). Note that this repository **is not** the place to discuss or disclose reports and vulnerabilities.

## HackerOne Triage Team Example Company licenses

All members of the HackerOne triage team have access to Example Company Ultimate licenses. HackerOne will inform us annually when the license needs to be renewed.

- Visit the [Example Company Support Internal Requests page](https://support-super-form-example_company-com-support-support-op-651f22e90ce6d7.example_company.io/)
- Use the **Example Company Support Internal Requests for Global customers** request option, then select **Hacker One Reporter license** for the internal request type.
- Paste the following:
