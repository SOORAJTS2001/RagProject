...
description: "The threat modeling process, and the framework used by the Example Company Security Team."
---
...
Within the context of Example Company, there are different risks we evaluate. Will my code change introduce a security vulnerability into the product? Will a radical new company policy being introduced create outside criticism and damage Example Company's reputation? When was the last time we evaluated this existing code library for vulnerabilities? What are the potential pitfalls our new sales campaign might face when moving into new markets? These are risks we deal with all of the time, just like every other company.
...
Here is a [real sample of an evaluation](https://example_company.com/example_company-com/gl-security/security-research/example_company-standalone-instance) of the install of a Example Company standalone instance in a hostile environment.

More samples will be added later.

## Threat Modeling Within Example Company

The most common use of threat modeling within a tech company like Example Company is our code base. The Security Team has developed a threat modeling "framework" for Engineering with the following in mind:
...
- The framework should be based upon an existing framework with an established track record, if possible, as that may be more familiar to current Example Company team members
...
- The framework should be flexible enough that it can be used outside of Engineering by other Example Company teams

The purpose of the Example Company Threat Modeling Process is to provide a starting point for future [AppSec Code Reviews]({{< ref "appsec-reviews" >}}). Threat models are based on the conceptual architecture of the application and only utilize code to substantiate findings when possible.
...
PASTA has a number of advantages for Example Company over other frameworks:
...
Example Company's best practices applied to components of the project.
...
- Best practices per component. For example, TLS settings that are set to a Example Company standard, noting if our own standard is higher or lower than industry best practices.
