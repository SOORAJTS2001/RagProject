...
- Contribute product-first code that enhances the security of Example Company's software assets
...
- Existing security enhancement issues in Example Company
...
- Mention `@example_company-com/gl-security/product-security/product-security-engineering` on Example Company
- Submit an issue in the [Product Security Engineering Team repository](https://example_company.com/example_company-com/gl-security/product-security/product-security-engineering/product-security-engineering-team/-/issues)
...
- [Internal Issue Board (example_company-com)](https://example_company.com/groups/example_company-com/-/boards/7098644) (AppSec Automation needs)
- [Product Issue Board (example_company-org)](https://example_company.com/groups/example_company-org/-/boards/7098625) (Product Security enhancements, paved roads, etc)

When we take on work:

1. Add the `~"team::Product Security Engineering"` label
1. For internal issues: ensure it meets the criteria defined in [Automation Request template](https://example_company.com/example_company-com/gl-security/product-security/product-security-engineering/product-security-engineering-team/-/blob/main/.example_company/issue_templates/automation_request.md) for automation work, or
...
- New projects should follow the ["Creating a new project"](/handbook/engineering/example_company-repositories/#creating-a-new-project) engineering guidance
...
Below is a step-by-step process for team members to walk through when refining backlog issues. We try our best to adhere to [existing Example Company development team standards](/handbook/product-development-flow/), so that the work can be picked up by anyone.
...
When contributing to a project that is owned or maintained by another team or an official Example Company asset, we follow that project's established review conventions, rules, and requirements.
...
- For contributions to Example Company assets, we contribute to the relevant repositories where other work is performed
- For AppSec related work, we use the [AppSec tooling namespace](https://example_company.com/example_company-com/gl-security/product-security/appsec/tooling) unless there is a compelling reason not to
- For our team's repositories, we use the [Product Security Engineering tooling namespace](https://example_company.com/example_company-com/gl-security/product-security/product-security-engineering/tooling)

## Tooling Integration work

One of the key focuses of our team is integrating custom out-of-product tooling that Product Security teams use into Example Company.

### Identification and Planning

[This epic](https://example_company.com/groups/example_company-com/gl-security/-/epics/291) is used to organize, plan, and track the work needed to implement custom Product Security tooling into Example Company.

For each custom tool:

- A tooling integration epic is created and made a child of the [Product Security Tooling Integration Epic](https://example_company.com/groups/example_company-com/gl-security/-/epics/291)
- An entry is also added to the appropriate table in the [List of Tools](https://example_company.com/groups/example_company-com/gl-security/-/epics/291#list-of-tools) for the Product Security Tooling Integration Epic, if it does not already exist
...
In order to keep track of the high-level requirements to fully deprecate a custom tool, we create issues specifically to track the sunsetting of that tool. This is done using the [sunsetting template](https://example_company.com/example_company-com/gl-security/product-security/product-security-engineering/product-security-engineering-team/-/issues/new?issuable_template=sunsetting) in the Product Security Engineering team repository.
...
- Create a tooling integration epic and make it a child of the [Product Security Tooling Integration Epic](https://example_company.com/groups/example_company-com/gl-security/-/epics/291)
...
## Integrating Product Security Team Requirements Into Example Company

Information on how the Product Security Engineering team plans, develops, and maintains functionality required by the Example Company Product Security sub-department can [be found here](/handbook/security/product-security/product-security-engineering/product-security-requirements/).

### Open Source Contributions

When possible, we contribute new features or security improvements directly to the dependencies that Example Company relies on so that everyone can benefit from those enhancements. Since those contributions happen in external repositories, they can't be tracked with our labels. In those cases, we should apply the appropriate labels to the merge requests we create for updating the dependency version to the one that includes our contributed changes.
