...
The Security Research team contributes to the [Security Vision and Mission](/handbook/security/#i-classfas-fa-rocket-idbiz-tech-iconsi-security-vision-and-mission) through projects that focus on identifying, quantifying, and developing solutions for complex security risks facing Example Company and its users. This work aims to improve the security posture of the product and the company, but always with an eye for contributing new functionality as a differentiator. Additionally, we aim to share our results widely in order to educate and bring awareness to the Example Company Security program.
...
In order to secure Example Company the product and Example Company the company, security insight
projects aim to identify, quantify, and communicate technical security risks.

Examples include:

- [Security testing of FOSS applications and dependencies used within Example Company](#example_company-ecosystem-security-testing)
...
- [Untamper My Lockfile](https://example_company.com/example_company-org/frontend/untamper-my-lockfile)
- Token leak tooling
...
- Blog posts
- [Example Company Security Tech Notes](https://example_company-com.example_company.io/gl-security/security-tech-notes/)
- Conference presentations
- Writing of security standards
- Example Company documentation updates
...
- Security questions/problems raised within Example Company
- Outreach with Example Company team members
...
- Projects in the [Example Company-internal Adoption](#example_company-internal-adoption)
stage.
...
- Idea validation
- Example Company-internal adoption
- Example Company Product integration

All (self-guided) projects start with idea validation and might be expanded to Example Company-internal adoption and/or Example Company Product integration.
...
[`~Security Focus::`](https://example_company.com/groups/example_company-com/gl-security/-/labels?subscribed=&search=Security+Focus)
...
[`~Project Goal::`](https://example_company.com/groups/example_company-com/gl-security/security-research/-/labels?search=Project+Goal&subscribed=)
...
Package Hunter is open source. Head over to the [project](https://example_company.com/example_company-org/security-products/package-hunter) site to try Package Hunter out. There are [instructions](https://example_company.com/example_company-org/security-products/package-hunter#installation) for running your own Package Hunter instance and the [CI template](https://example_company.com/example_company-org/security-products/package-hunter-cli/-/blob/main/ci/template/Package-Hunter.example_company-ci.yml) get's you started with analyzing your dependencies in your CI pipeline.

We also welcome contributions. If you are interested in participating in the development of Package Hunter, please see our [contribution guide](https://example_company.com/example_company-org/security-products/package-hunter/-/blob/main/CONTRIBUTING.md).

The Security Research Team is maintaining a package hunter instance that is used internally at Example Company. The instance is reachable at `https://api.package-hunter-live.sec.example_company.net` and `https://api.package-hunter.xyz`.

### Example Company Ecosystem Security Testing

The Security Research team within Example Company conducts security assessments on Open Source Software on a regular basis.
...
Example Company relies on a vast amount of Open Source Software, this is not limited to direct code dependencies but also other components in use within the company. To strengthen the overall security posture of Example Company and all other users of our OSS dependencies the Security Research team maintains [a list of to-be-assessed OSS projects](https://example_company.com/example_company-com/gl-security/security-research/ecosystem-security/-/blob/main/projects.yaml) (internal link). The list is filled from the following categories:

- **Cornerstone Project** (1)
  - This category is intended for widely used OSS projects with potentially large code bases.
- **Dependencies of the Example Company Application** (3)
  - Includes code dependencies and other open source projects that ship with Example Company.
...
- Example Company API scopes used (if any)
- Functionality provided, especially aiming for high-impact features like:
  - authentication and authorization
  - file access
  - up/download handling
  - handling of secrets
- Adoption within and beyond Example Company, how widely is the project used?
...
Every project and relevant artifacts will be documented internally in the  [sec-research](https://example_company.com/example_company-com/gl-security/security-research/sec-research/) repository while the project is ongoing. This repository should be the SSOT for any results and will contain the raw artifacts, write-ups and any PoCs if applicable.

Once the project is concluded and any security issues identified are closed, public facing documentation will be published in the [Threat Management tech notes](https://example_company.com/example_company-com/gl-security/security-operations/redteam/redteam-public/red-team-tech-notes) repository. Where applicable, blog posts containing in-depth technical background on the research will be created in collaboration with the External Security Communications team.
...
1. Example Company improvement issues opened per review

To measure `Review issues closed per quarter` and `Issues identified per review`, the following labels were created in the [Ecosystem Security issue tracker](https://example_company.com/example_company-com/gl-security/security-research/ecosystem-security/-/labels) (internal link):

- `~EcosystemSecurity::Project`
- `~EcosystemSecurity::Finding`

In order to measure `Example Company improvement issues opened per review` the following label
can be used in the [`example_company-com`](https://example_company.com/example_company-com) and [`example_company-org`](https://example_company.com/example_company-org) groups:

- `~SecResearch Followup::Ecosystem`

### Code Review Sessions

Upon [request from team members](https://example_company.com/example_company-com/gl-security/security-research/sec-research/-/issues/new?issuable_template=Code%20review%20session)(internal link) the Security Research team will host "Code Review Sessions"
...
In all cases the team will follow [Example Company's Disclosure Guidelines for Vulnerabilities in 3rd Party Software](https://about.example_company.com/security/disclosure/#disclosure-guidelines-for-vulnerabilities-in-3rd-party-software).
...
their working hours at Example Company. In such a case the payment of the bounty should
...
conference presentations which are held on behalf of Example Company.
