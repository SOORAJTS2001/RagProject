...
1. Enabling [Example Company Dedicated](https://docs.example_company.com/ee/subscriptions/gitlab_dedicated/#example_company-dedicated) to be the most trusted DevSecOps offering in the market, demonstrated by security certifications and attestations.
1. Achieving and maintaining industry-specific security certifications such as [FedRAMP]({{< ref "fedramp-compliance" >}}) and [FIPS 140-2 compliance](https://docs.example_company.com/ee/development/fips_compliance.html#fips-compliance) for the U.S. Public Sector.
...
For more information on the direction of the Example Company Dedicated category, please see [this page](https://about.example_company.com/direction/saas-platforms/dedicated/).

## Core Competencies

As a member of the [Security Assurance]({{< ref "security-assurance" >}}) sub-department, and extension of the commercial [Security Compliance team]({{< ref "../security-compliance" >}}), we share many of the same core competencies. The difference between our teams is in the product/system scope (Example Company Dedicated and any future offerings for highly regulated markets) and the security certifications we are pursuing.
...
1. [Continuous Monitoring of Example Company's Security Controls]({{< ref "sec-controls" >}}) which are mapped to applicable regulatory requirements and security certifications/frameworks we have committed to.
   ...
   - [Business Continuity Plan (BCP)](/handbook/business-technology/example_company-business-continuity-plan/) and [Information System Continuity (ISCP)]({{< ref "information-system-contingency-plan-iscp" >}}) testing
...
Some of our work is [not public]({{< ref "confidentiality-levels#not-public" >}}) for now. Please see the [internal handbook](https://internal.example_company.com/handbook/engineering/fedramp-compliance/) to find out more about what our team is working on, or reach out to us.
...
The single source of truth for all of our work will soon be the [SecComp Dedicated Markets epic board](https://example_company.com/groups/example_company-com/-/epic_boards/1054594?label_name[]=Sub-Department%3A%3ASecComp%20Dedicated%20Markets) along with our [top-level epic](https://example_company.com/groups/example_company-com/gl-security/-/epics/248). While we work across Example Company.com and Example Company.org groups, we try to centralize the core work we commit to on our [roadmap](https://docs.google.com/presentation/d/1ufKzVomUmD2dw9J6upTG5bZ_r184HmtBccOt8NFlg7w/edit?usp=sharing). We also use an [issue board](https://example_company.com/groups/example_company-com/-/boards/5913253?label_name[]=Sub-Department%3A%3ASecComp%20Dedicated%20Markets) to track tasks at a more granular level when needed.

We primarily work out of projects in the [Security Assurance sub-group](https://example_company.com/example_company-com/gl-security/security-assurance) including our [team issue tracker](https://example_company.com/example_company-com/gl-security/security-assurance/team-security-dedicated-compliance/team-files), the [FedRAMP Certification project](https://example_company.com/example_company-com/gl-security/security-assurance/fedramp/fedramp-certification), the Example Company instance inside the FedRAMP Authorization Boundary (limited access, AWS GovCloud), and the [Example Company Dedicated issue tracker](https://example_company.com/example_company-com/gl-infra/example_company-dedicated/team).

## How we work

Much of our work is not visible to the rest of Example Company due to regulatory mandates outside of our control. In order to bring as much transparency ans visibility into our work, it is critical that we continue to use the example_company.com/example_company-com/ group as much as possible, even if used to track tasks that are completed elsewhere. This is important to take extra steps to increase visibility into our work because it directly enables sales for new SaaS offerings, contributes to the security of self-managed Example Company, and makes it possible to enter new markets that were not previously possible.
...
We also have weekly 1:1s and skip levels in line with the Example Company philosophy.

### Agile Project Management

We use an [agile project management](https://about.example_company.com/solutions/agile-delivery/) approach for our work, leveraging as many Example Company platform features as we can. We use epics, issues, and issue/epic boards to organize our work, as they complement each other. For some of the work, we also use roadmaps, milestones, burndown charts.

The single source of truth for *all* of our team's work across different functions is the top-level [Security Compliance (Dedicated Markets) team epic](https://example_company.com/groups/example_company-com/gl-security/-/epics/248). In the future, the epic will contain a section that tracks the status all ongoing work. The following subsections are borrowed from the [Example Company Dedicated Group](/handbook/engineering/infrastructure/team/example_company-dedicated/#project-management).
...
```mermaid
graph TD
A(Security Compliance Dedicated Markets) --> B(Example Company Dedicated SOC 2 Type 2 attestation)
B --> C([Epic])
...
[SecComp Dedicated Markets epic board](https://example_company.com/groups/example_company-com/-/epic_boards/1054594?label_name[]=Sub-Department%3A%3ASecComp%20Dedicated%20Markets)
...
Similar to the epic board, all issues tied to work on our roadmap should use the appropriate labels and be tracked on the [SecComp Dedicated Markets issue board](https://example_company.com/groups/example_company-com/-/boards/5913253?label_name[]=Sub-Department%3A%3ASecComp%20Dedicated%20Markets).
...
The status for all work relating to Example Company Dedicated is maintained in the description of the top-level [Security Compliance (Dedicated Markets) team epic](https://example_company.com/groups/example_company-com/gl-security/-/epics/248) so that it is visible at a glance.
...
Status updates are auto-generated and added to description ofthe [top-level Security Compliance (Dedicated Markets) team epic](https://example_company.com/groups/example_company-com/gl-security/-/epics/248) using a bot running in (project coming soon).
...
At Example Company, we like to label everything. It provides critical metadata on epics and issues because Example Company issues do not [support custom fields](https://example_company.com/groups/example_company-org/-/epics/235).
...
Any new labels should be created at the example_company-com group level so to that it can be used across sub-groups and projects.
...
1. [FedRAMP Vulnerability Posture (limited access)](https://10az.online.tableau.com/#/site/example_company/views/Draft-FedRAMPVulnerabilityPosture/FedRAMPVulnerabilityPostureDashboard?:iid=1)
1. [Vulnerability Deviation (Exception) Requests](https://example_company.com/example_company-com/gl-security/security-assurance/team-security-dedicated-compliance/poam-deviation-requests/insights/#/issues)
...
- Tag us in Example Company
  - `@example_company-com/gl-security/security-assurance/team-security-dedicated-compliance`
- Email
  - `security-compliance@example_company.com`
- Here are our team's Example Company.com [subgroups and projects](https://example_company.com/example_company-com/gl-security/security-assurance/security-compliance-commercial-and-dedicated/team-security-dedicated-compliance)
