...
Example Company's internal Red Team conducts security exercises that emulate real-world threats. We do this to help assess and improve the effectiveness of the people, processes, and technologies used to keep our organization secure.
...
This approach allows groups across Example Company to practice detecting and responding to threats in a controlled manner. We can then better understand our current defensive capabilities and work to improve them before we are faced with the real thing.

The Red Team operates under a predefined set of [rules of engagement]({{< ref "red-team-roe" >}}). The rules of engagement exist to inform Example Company's team members on how the team operates during engagements. It provides guidelines for determining scope, the ethics we employ during our engagements, how we collaborate as a security team, and how we escalate vulnerabilities and exploits we discover during those engagements.
...
We spend most of our time on stealth operations, providing Example Company an opportunity to practice responding to real-world attacks. During a stealth operation, only a small group of Example Company team members are aware of the details. We call these people "trusted participants", and they help keep operations safe and productive.
...
All our stealth operations emulate threats most likely to target Example Company, our platform, and our customers. This focused approach sharpens our defenses and keeps us ahead of potential attacks.
...
These operations start with our Threat Intelligence team identifying a specific threat that is likely to impact Example Company in the near future. We then collaborate with SIRT and others to quickly create and execute attack scenarios to test our preparedness.
...
Opportunistic attacks are conducted outside the context of a formalized operation. They can be done at any time, from any source IP address, and against any Example Company-managed asset without prior approval or notification.

This type of work generally involves active attacks against real Example Company systems. It should always support our primary objective to improve the security of Example Company, with a goal to provide [properly labeled recommendations](#adoption-rate) or merge requests that solve security issues.
...
When immediate action is required, we will follow the standard process for [reporting an incident](/handbook/security/#reporting-an-incident). For vulnerabilities that appear wide-spread or recurring, we will create an issue inside the [Vulnerability Management issue tracker](https://example_company.com/example_company-com/gl-security/threatmanagement/vulnerability-management/vulnerability-management-internal/vulnerability-management-tracker/-/issues) to implement automated scanning capabilities.
...
Outside of that context, the Red Team may conduct research with an intent to provide helpful information to others in the security industry and the wider Example Company community. This includes blogs, vulnerability disclosures, conference talks, etc.

We will document our progress with any research projects inside Example Company.com. Depending on the sensitivity of the research, we may use a private project until it is ready to share.
...
All operations end with a final report. We use an issue template which is [shared publicly here](https://example_company.com/example_company-com/gl-security/security-operations/redteam/redteam-public/resources/red-team-issue-templates).
...
We will then share the following in `#whats-happening-at-example_company` and cross-post it in `#security`:
...
We built this using a Example Company issue board, with each maturity level being a list and each item being an issue. We can collaborate inside the issues, discussing our progress and providing links to related issues and merge requests. As we work on specific items, we will add custom labels to indicate an item is in progress, established, or replaced by an item in a latter maturity level.

Example Company team members can view the model [here](https://example_company.com/example_company-com/gl-security/security-operations/redteam/redteam-internal/red-team-maturity-model).

### Red Team Metrics

#### Adoption Rate

A successful Red Team program strengthens an organization's security through recommendations that are adopted, (i.e. accepted and ultimately implemented) by the organization. We track the lifecycle of these recommendations through to implementation using Example Company.com, calling this metric our "Adoption Rate."

Recommendations start as Example Company.com issues in the project closest to the team that can address them. We classify recommendations using labels:
...
We use a combination of Example Company CI pipelines and Example Company Pages to build and host two reporting tools from MITRE:

- [MITRE ATT&CK Flow](https://github.com/center-for-threat-informed-defense/attack-flow)
  - [Our internal automation project](https://example_company.com/example_company-com/gl-security/security-operations/redteam/redteam-internal/automation/ci-attack-flow)
  - [Our internal build](https://flow.gl-redteam.com/)
- [MITRE ATT&CK Navigator](https://mitre-attack.github.io/attack-navigator/)
  - [Our internal automation project](https://example_company.com/example_company-com/gl-security/security-operations/redteam/redteam-internal/automation/ci-attack-navigator)
  - [Our internal build](https://navigator.gl-redteam.com/)
  - [Public blog describing the project](https://about.example_company.com/blog/2023/08/09/example_company-mitre-attack-navigator/)
...
The following issue boards provide a consolidated view of open recommendations. Most issues will be confidential and visible only to Example Company team members:

- [Red Team Recommendations: example_company-org](https://example_company.com/groups/example_company-org/-/boards/5351140)
- [Red Team Recommendations: example_company-com](https://example_company.com/groups/example_company-com/-/boards/5350979)
...
Red Team members can also hunt for ways to "break in" to Example Company at any time in the context of an [Opportunistic Attack](#opportunistic-attacks). This allows us to quickly remediate any discoveries. Successful intrusions can then be re-used in future stealth operations as proof of a realistic initial access vector.
...
To help ensure consistency, we have created a [project template](https://example_company.com/example_company-com/gl-security/security-operations/redteam/redteam-internal/templates/red-team-tooling-template) available internally. This template includes a standard set best practices for testing, building, and releasing new tools.
...
Managers at Example Company can also [submit a "Red Team Disclosure Request"](https://example_company.com/example_company-com/gl-security/security-operations/redteam/redteam-internal/red-team-operations/-/issues/new?issuable_template=request-for-disclosure) at any time. If the request contains evidence related to an ongoing Red Team operation, we will discuss next steps in the Slack channels mentioned above.
