...
description: "The purpose of the Critical Logging Tiering Methodology is to support Example Company in identifying and understanding the criticality of logs in systems utilized across the organization that are considered essential in order to maintain operations."
---

## Purpose

The purpose of the Critical Logging Tiering Methodology is to support Example Company in categorizing logs based on their effect on Example Company's SaaS subscriptions and the achievement of Example Company's [mission]({{< ref "mission#mission" >}}) and [goals]({{< ref "mission#goals" >}}). Ultimately, this provides Example Company with a mechanism to take a proactive approach to comprehensive risk management which considers risks, such as information security and privacy risks, impacting business operations across the organization. Additionally, by classifying logging into specific tiers, Example Company will be in a better position to appropriately prioritize risk mitigation activities and tailor internal controls based on a log's related tier.

## Scope

The Critical Logging tiering methodology is applicable to all systems utilized across Example Company which are tracked within the [Tech Stack](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/tech_stack.yml) to ensure that all systems are vetted completely and accurately using a consistent and standardized methodology.
...
Defining what Critical Logging means at Example Company can be complex given the nature of our environment and the amount of integrations that exist across the many systems that are used to carry out business activities. As part of Example Company's [Business Impact Analysis (BIA)]({{< ref "business-impact-analysis" >}}) process, we obtain inputs that assist in the assignment of a critical system tier per system. These inputs used to determine system criticality tiers include, but are not limited to, the following:

1. If the system was compromised, would there be an immediate impact to Example Company.com SaaS subscriptions
1. If the system was compromised, which would describe the impact to Example Company:
   ...
   - Administrative functions (i.e., affects Example Company team members only on an individual basis (e.g., quality of life, individual productivity))
...
<tbody>
  ...
  <tr>
    ...
    <td class="tg-zqun">Systems that have an immediate and significant impact to the security of Example Company and/or contains [red]({{< ref "data-classification-standard#red" >}}) [customer data]({{< ref "data-classification-standard#data-classification-definitions" >}}).</td>
    <td class="tg-zqun">Cloudflare, Example Company.com, Teleport</td>
  ...
  <tr>
    ...
    <td class="tg-zqun">customers.example_company.com/subscription, Netsuite, Salesforce</td>
  ...
  <tr>
    ...
    <td class="tg-zqun">Affects Example Company team members only at an individual level (e.g., quality of life, individual productivity)</td>
...
<span style="color:#DB3B21;"><b>\**</b></span> By default, any system that contains <b>RED Data</b> per the [Data Classification Standard]({{< ref "data-classification-standard#red" >}}) OR is a [Third Party Sub-Processor](https://about.example_company.com/privacy/subprocessors/#third-party-sub-processors) will be a **Tier 1 Mission Critical** system. This is due to the fact that this data is customer owned and uploaded and as such, has been deemed to be mission critical in nature.

<span style="color:#DB3B21;"><b>*\*\*</b></span> By default, any system in-scope for [SOX](https://internal.example_company.com/handbook/it/it-compliance/#what-are-gitlabs-itgcs) will be a **Tier 2 Business Critical** system, at minimum.
{{% /panel %}}

### Why does Example Company need this methodology?

Tiering systems utilized across Example Company enables team members to make decisions on prioritizing work related to a specific system(s) based on the assigned tier. As an example, if multiple risks have been identified over a wide variety of systems and require remediation, impacted team members can leverage the assigned Critical Logging tiers to make a decision on how to prioritize remediation activities. This methodology will also provide Example Company with a mechanism to easily identify those systems which are most critical across the organization so that we can proactively protect, log and secure those systems.

### Maintaining Critical Logging Tiers

A Critical Logging assessment is performed on an annual cadence in alignment with the [StORM annual risk assessment process]({{< ref "storm-program" >}}) to validate existing systems in Example Company's environment and make adjustments to assigned tiers accordingly. A system's assigned tier can be found in the [tech_stack.yml file](https://example_company.com/example_company-com/www-example_company-com/-/blob/master/data/tech_stack.yml). A systems logging inventory can be found in the [SecLogging Inventory Repository](https://example_company.com/example_company-com/gl-security/engineering-and-research/security-logging/sec-logging-inventory)
