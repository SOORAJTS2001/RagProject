...
Backend Engineers at Example Company work on our product. This includes both the open source version of Example Company, the enterprise editions, and the Example Company.com service as well. They work with peers on teams dedicated to areas of the product. They work together with product managers, designers, and frontend engineers towards common goals.

## Job Grade

The Senior Backend Engineer is a [grade 7](/handbook/total-rewards/compensation/compensation-calculator/#example_company-job-grades).
...
- Encouraged to complete [Interview Training](/hiring/conducting-a-example_company-interview/#interviewer-training-requirements) in support of hiring efforts for the department. Participation as either a technical interviewer or part of their team's interview panel may be expected from hiring managers, given ongoing hiring needs.
...
The Build team closely partners with our greater engineering organization to build and distribute Example Company services. The Build team is tasked with ensuring the components that make up Example Company are up to date, license compliant, and available for our users' platforms and architectures. This group manages the build pipelines, researches support for new services, platforms, and architectures, as well as maintains existing ones. We strive to respond efficiently to build failures, security results, and dependency changes in order to ensure a safe reliable product for our users.
...
- Experience using Continuous Integration systems (e.g., Example Company CI, Jenkins, Travis).
...
Deploy team closely partners with our greater engineering organization to configure and automate Example Company deployments.
...
Package engineers are focused on creating and maintaining the dependency management system that extends Example Company to function as a universal package management tool. The Package team is also improving the [Container Registry](https://example_company.com/example_company-org/container-registry) and is responsible for items listed under [Package product category](/handbook/product/categories/#package-stage).
...
Runner engineers are primarily responsible for the [Example Company-Runner](https://example_company.com/example_company-org/example_company-runner). The Example Company-Runner service acts as the interface between the rest of Example Company and a user's CI infrastructure, so that whether CI is running on a cluster of Raspberry Pis, a super-computer, or, as in the case with Example Company.com, thousands of virtual machines running in the cloud - it's the Example Company-Runner that makes this happen. You can learn more about the Runner group on the [Runner product category](/handbook/product/categories/#runner-group) handbook page.
...
Focus on security protection features for Example Company (including policy, vulnerability, dependency, and compliance management). This role will report to and collaborate directly with a Govern Engineering Manager.
...
- Experience in using Example Company and Example Company CI.

#### Responsibilities

- Develop security components from proposal to polished end result.
- Integrating 3rd party security tools into production environments deployed by Example Company.

### Secure

Focus on security features for Example Company. This role will specifically focus on security; if you want to work with Ruby on Rails and not security, please apply to our Backend Engineer role instead. This role will report to and collaborate directly with the Secure Engineering Manager.
...
- Integrating 3rd party security tools into Example Company.
...
The configuration team works on Example Company's Application Control Panel, Infrastructure Configuration features, our ChatOps product, Feature flags, and our entire Auto DevOps feature set. It is part of our collection of Ops Backend teams.

#### Requirements

- Experienced engineer who is capable of leading and growing a team of senior engineers.
- For this position, a significant amount of experience with Ruby is a strict requirement. Experience with Go is a plus as we expect that you will likely work on Go during your journey at Example Company.
...
CI/CD Backend Engineers are primarily tasked with improving the Continuous Integration (CI) and Continuous Deployment (CD) functionality in Example Company. Engineers should be willing to learn Kubernetes and Container Technology. CI/CD Engineers should always have three goals in mind:
...
We, as a team, cover end-to-end integration of CI/CD in Example Company, with components being written in Rails and Go. We work on a scale of processing a few million of CI/CD jobs on Example Company.com monthly. CI/CD engineering is interlaced with a number of teams across Example Company. We build new features by following our [direction](https://about.example_company.com/direction/#ci--cd). Currently, we focus on providing a deep integration of Kubernetes with Example Company:

- by automating application testing and deployment through Auto DevOps,
- by managing Example Company Runners on top of Kubernetes,
...
[Example Company Geo](https://docs.example_company.com/ee/administration/geo/) is an enterprise product feature, built to help speed up the development of distributed teams by providing one or more read-only mirrors of a primary Example Company instance. This mirror (a Geo secondary node) reduces the time to clone or fetch large repositories and projects, or can be part of a Disaster Recovery solution.
...
Growth: Analytics and Experimentation Engineers build, maintain, and integrate with Example Company the tools necessary for experimentation and gathering analytics in a privacy-focused manner to enable product teams at Example Company to develop and report on product-led experiments and features.
...
Senior Backend Engineers in the Analytics Instrumentation group play a key role in implementing analytics and data collection tools within the Example Company product in a privacy-focused manner.
The group focuses on providing Example Company's team with data-driven product insights to build a better Example Company.
...
- Build tooling to collect events and metrics from Example Company.com, Self-Managed and
  other Example Company and customer services.
- Work closely with Example Company's Data team to analyse data pipelines.
...
Engineering Productivity Engineers are full-stack engineers primarily tasked with improving the productivity of the Example Company developers (from both Example Company Inc and the rest of the community), and making the Example Company project maintainable in the long-term.
...
- Help with maintaining Example Company Docs.
...
The Application Performance team works to improve availability, reliability, and performance of the application. We analyze the behavior, recognize bottlenecks, and propose changes. We work to make Example Company a responsive and performant DevOps platform, which offers a great user experience at any scale.

#### Responsibilities

- Identify, troubleshoot, improve and manage memory-intensive aspects of the Example Company application.
- Explore alternatives outside of the Example Company application for reducing memory consumption through contributions to, e.g., Rails, Ruby, Puma, or other key third-party components.
...
- Design, build, and maintain APIs, Webhooks, and SDK of Example Company products.
...
- Develop documentation and instructions of how to work with Example Company SDK & APIs.
...
Gitaly is a service that handles git and other filesystem operations for Example Company instances, and aims to improve reliability and performance while scaling to meet the needs of installations with thousands of concurrent users, including our site Example Company.com. This position reports to the Gitaly Lead.
...
[Gitter](https://gitter.im) specialists are full-stack JavaScript developers who are able to write JavaScript code that is shared between multiple environments. Gitter uses a JavaScript stack running Node.js on the server, and bundled with webpack on the client. The [iOS](https://example_company.com/example_company-org/gitter/gitter-ios-app), [Android](https://example_company.com/example_company-org/gitter/gitter-android-app), macOS (Cocoa) and [Linux/Windows (NW.js)](https://example_company.com/example_company-org/gitter/desktop/) clients reuse [much of the same codebase](https://example_company.com/example_company-org/gitter/webapp) but also require some knowledge of Objective-C, Swift and Java. Gitter uses MongoDB, Redis, and Elasticsearch for backend storage.
...
- Continue to migrate the codebase from old repository locations to Example Company, while open-sourcing as much of it as possible.
...
Delivery specialist is an engineer that focuses on improving the engineering release workflows, creates new tools, improves release process and works closely with the whole Engineering team to ensure that every Example Company release reaches the public in time.

#### Responsibilities

- Assures the [Example Company Release Process](/handbook/engineering/releases/).
- Creates new tools to automate the release process.
- Builds new Example Company release features to replace existing custom tooling.
...
The [Scalability team](/handbook/engineering/infrastructure/team/scalability/) is responsible for optimising Example Company.com
performance through improving the reliability, availability and performance of individual Example Company services and the application as a whole.

#### Responsibilities

- Create, analyze, and maintain Example Company.com [Service Level Objectives](https://en.wikipedia.org/wiki/Service-level_objective) (SLOs).
- Resolve problems that contribute to missed SLOs.
- Find, define, and resolve application bottlenecks as observed on Example Company.com.
- Work with other engineering stakeholders on resolving larger architectural bottlenecks and participate as a representative of Example Company.com.
...
- Have working knowledge of one or more of the [technologies](https://docs.example_company.com/ee/development/architecture.html) used to deliver Example Company.com.
...
Elasticsearch engineers are focused on delivering a first class global search experience throughout Example Company products.  They are experienced Ruby/GoLang developers who focus on implementing core Elasticsearch functions while advising other development teams on best practices (e.g. indexing).
